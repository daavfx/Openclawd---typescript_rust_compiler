...
function snapshotEnv() {
  return { home: process.env.HOME, userProfile: process.env.USERPROFILE, homeDrive: process.env.HOMEDRIVE, homePath: process.env.HOMEPATH, stateDir: process.env.OPENCLAW_STATE_DIR };
}
...
export async function withTempHome(fn, opts = {  }) {
  ...
  for (const key of envKeys) {
    if (((((key === "HOME") || (key === "USERPROFILE")) || (key === "HOMEDRIVE")) || (key === "HOMEPATH"))) {
      throw new Error("withTempHome: use built-in home env (got )");
    }
  }
