...
const envSnapshot = () => { PATH: process.env.PATH, SHERPA_ONNX_MODEL_DIR: process.env.SHERPA_ONNX_MODEL_DIR, WHISPER_CPP_MODEL: process.env.WHISPER_CPP_MODEL };
    ...
    try {
      {
        ...
        await writeExecutable(binDir, "sherpa-onnx-offline", ("#!/usr/bin/env bash
" + "echo \"{\\\"text\\\":\\\"sherpa ok\\\"}\"
"));
        ...
        const ctx = { Body: "<media:audio>", MediaPath: filePath, MediaType: "audio/wav" };
    ...
    try {
      {
        ...
        await writeExecutable(binDir, "whisper-cli", ((((((("#!/usr/bin/env bash
" + "out=\"\"
") + "prev=\"\"
") + "for arg in \"$@\"; do
    ...
    try {
      {
        ...
        await writeExecutable(binDir, "gemini", (("#!/usr/bin/env bash
...
"));
        ...
        tempPaths.push(path.dirname(filePath));
        const {applyMediaUnderstanding} = await loadApply();
        const ctx = { Body: "<media:image>", MediaPath: filePath, MediaType: "image/png" };
