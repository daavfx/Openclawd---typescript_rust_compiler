...
function buildOpenAIResponsesSse(text) {
  const events = [{ type: "response.output_item.added", item: { type: "message", id: "msg_test_1", role: "assistant", content: [], status: "in_progress" } }, { type: "response.output_item.done", item: { type: "message", id: "msg_test_1", role: "assistant", status: "completed", content: [{ type: "output_text", text, annotations: [] }] } }, { type: "response.completed", response: { status: "completed", usage: { input_tokens: 10, output_tokens: 10, total_tokens: 20 } } }];
  const sse = "data: [DONE]
...
describe("provider timeouts (e2e)", () => {
  it("falls back when the primary provider aborts with a timeout-like AbortError", { timeout: 60000 }, async () => {
    ...
    const cfg = { agents: { defaults: { model: { primary: "primary/gpt-5.2", fallbacks: ["fallback/gpt-5.2"] } } }, models: { mode: "replace", providers: { primary: { baseUrl: primaryBaseUrl, apiKey: "test", api: "openai-responses", models: [{ id: "gpt-5.2", name: "gpt-5.2", api: "openai-responses", reasoning: false, input: ["text"], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 128000, maxTokens: 4096 }] }, fallback: { baseUrl: fallbackBaseUrl, apiKey: "test", api: "openai-responses", models: [{ id: "gpt-5.2", name: "gpt-5.2", api: "openai-responses", reasoning: false, input: ["text"], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 128000, maxTokens: 4096 }] } } }, gateway: { auth: { token } } };
    ...
    try {
      {
        ...
        await client.request("sessions.patch", { key: sessionKey, model: "primary/gpt-5.2" });
        const runId = randomUUID();
        const payload = await client.request("agent", { sessionKey, idempotencyKey: "idem-", message: "say fallback-ok", deliver: false }, { expectFinal: true });
