...
const createStubOutbound = (id, deliveryMode = "direct") => { deliveryMode, sendText: async ({deps, to, text}) => {
...
const createStubPlugin = (params) => { id: params.id, meta: { id: params.id, label: (params.label ?? String(params.id)), selectionLabel: (params.label ?? String(params.id)), docsPath: "/channels/", blurb: "test stub.", aliases: params.aliases, preferSessionLookupForAnnounceTarget: params.preferSessionLookupForAnnounceTarget }, capabilities: { chatTypes: ["direct", "group"] }, config: { listAccountIds: (cfg) => {
