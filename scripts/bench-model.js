...
async function runModel(opts) {
  ...
  for (let i = 0; (i < opts.runs); i += 1) {
    ...
    const res = await completeSimple(opts.model, { messages: [{ role: "user", content: opts.prompt, timestamp: Date.now() }] }, { apiKey: opts.apiKey, maxTokens: 64 });
...
async function main() {
  ...
  const minimaxModel = { id: minimaxModelId, name: "MiniMax ", api: "openai-completions", provider: "minimax", baseUrl: minimaxBaseUrl, reasoning: false, input: ["text"], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 200000, maxTokens: 8192 };
  const opusModel = getModel("anthropic", "claude-opus-4-5");
  console.log("Prompt: ");
  console.log("Runs: ");
  console.log("");
  const minimaxResults = await runModel({ label: "minimax", model: minimaxModel, apiKey: minimaxKey, runs, prompt });
  const opusResults = await runModel({ label: "opus", model: opusModel, apiKey: anthropicKey, runs, prompt });
