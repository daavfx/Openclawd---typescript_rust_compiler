...
const COLOR_BY_PREFIX = new Map([["channel", "1d76db"], ["app", "6f42c1"], ["extensions", "0e8a16"], ["docs", "0075ca"], ["cli", "f9d0c4"], ["gateway", "d4c5f9"]]);
...
for (const label of missing) {
  ...
  execFileSync("gh", ["api", "-X", "POST", "repos//labels", "-f", "name=", "-f", "color="], { stdio: "inherit" });
...
function resolveRepo() {
  ...
  throw new Error("Unsupported GitHub remote: ");
}
function fetchExistingLabels(repo) {
  const raw = execFileSync("gh", ["api", "repos//labels?per_page=100", "--paginate"], { encoding: "utf8" });
