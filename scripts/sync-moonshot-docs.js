...
import { MOONSHOT_KIMI_K2_CONTEXT_WINDOW, MOONSHOT_KIMI_K2_COST, MOONSHOT_KIMI_K2_INPUT, MOONSHOT_KIMI_K2_MAX_TOKENS, MOONSHOT_KIMI_K2_MODELS } from "../ui/src/ui/data/moonshot-kimi-k2";
...
function renderKimiK2Ids(prefix) {
  return MOONSHOT_KIMI_K2_MODELS.map((model) => "- ``");
}
...
function renderMoonshotModels() {
  ...
  return MOONSHOT_KIMI_K2_MODELS.flatMap((model, index) => {
    ...
    return ["{", "  id: \"\",", "  name: \"\",", "  reasoning: ,", "  input: ,", "  cost: {  },", "  contextWindow: ,", "  maxTokens: ", closing];
  });
}
async function syncMoonshotDocs() {
  ...
  moonshotText = replaceBlockLines(moonshotText, "<!-- moonshot-kimi-k2-ids:start -->", "<!-- moonshot-kimi-k2-ids:end -->", renderKimiK2Ids(""));
  ...
  conceptsText = replaceBlockLines(conceptsText, "<!-- moonshot-kimi-k2-model-refs:start -->", "<!-- moonshot-kimi-k2-model-refs:end -->", renderKimiK2Ids("moonshot/"));
