...
const seedEntries = seedCommit ? parseReadmeEntries(run("git show :README.md")) : [];
const raw = run("gh api \"repos//contributors?per_page=100&anon=1\" --paginate");
...
for (const item of contributors) {
  ...
  apiByLogin.set(item.login.toLowerCase(), { login: item.login, html_url: item.html_url, avatar_url: normalizeAvatar(item.avatar_url) });
}
...
for (const seed of seedEntries) {
  ...
  const resolvedLogin = (login ?? resolveLogin(seed.display, null, apiByLogin, nameToLogin, emailToLogin));
  const key = resolvedLogin ? resolvedLogin.toLowerCase() : "name:";
  const avatar = (seed.avatar_url && !isGhostAvatar(seed.avatar_url)) ? normalizeAvatar(seed.avatar_url) : placeholderAvatar;
  const existing = entriesByKey.get(key);
  if (!existing) {
    const user = resolvedLogin ? apiByLogin.get(key) : null;
    entriesByKey.set(key, { key, login: ((resolvedLogin ?? login) ?? undefined), display: seed.display, html_url: (user?.html_url ?? seed.html_url), avatar_url: (user?.avatar_url ?? avatar), lines: 0 });
  } else {
...
for (const item of contributors) {
  ...
  const resolvedLogin = item.login ? item.login : resolveLogin(baseName, (item.email ?? null), apiByLogin, nameToLogin, emailToLogin);
  if (resolvedLogin) {
    ...
    if (!existing) {
      ...
      if (user) {
        ...
        entriesByKey.set(key, { key, login: user.login, display: pickDisplay(baseName, user.login, existing?.display), html_url: user.html_url, avatar_url: normalizeAvatar(user.avatar_url), lines: (lines > 0) ? lines : contributions });
      }
    } else {
      if (existing) {
  ...
  if (!existingAnon) {
    entriesByKey.set(anonKey, { key: anonKey, display: baseName, html_url: fallbackHref(baseName), avatar_url: placeholderAvatar, lines: (item.contributions ?? 0) });
  } else {
...
for (const [login, lines] of linesByLogin.entries()) {
  ...
  if (user) {
    ...
    entriesByKey.set(login, { key: login, login: user.login, display: (displayName[user.login.toLowerCase()] ?? user.login), html_url: user.html_url, avatar_url: normalizeAvatar(user.avatar_url), lines: (lines > 0) ? lines : contributions });
  } else {
    entriesByKey.set(login, { key: login, display: login, html_url: fallbackHref(login), avatar_url: placeholderAvatar, lines });
  }
}
...
for (let i = 0; (i < entries.length); i += PER_LINE) {
  ...
  const parts = chunk.map((entry) => {
    return "<a href=\"\"><img src=\"\" width=\"48\" height=\"48\" alt=\"\" title=\"\"/></a>";
  });
  lines.push("  ");
}
const block = "
...
const next = "<p align=\"left\">
";
writeFileSync(readmePath, next);
console.log("Updated README clawtributors:  entries");
function run(cmd) {
  return execSync(cmd, { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], maxBuffer: ((1024 * 1024) * 200) }).trim();
}
...
function fetchUser(login) {
  try {
    {
      const data = execSync("gh api users/", { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] });
      const parsed = JSON.parse(data);
      if (((!parsed?.login || !parsed?.html_url) || !parsed?.avatar_url)) {
        return null;
      }
      return { login: parsed.login, html_url: parsed.html_url, avatar_url: normalizeAvatar(parsed.avatar_url) };
    }
  }
  catch {
    {
...
function fallbackHref(value) {
  ...
  return encoded ? "https://github.com/search?q=" : "https://github.com";
}
