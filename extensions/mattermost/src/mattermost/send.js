...
import { createMattermostClient, createMattermostDirectChannel, createMattermostPost, fetchMattermostMe, fetchMattermostUserByUsername, normalizeMattermostBaseUrl, uploadMattermostFile } from "./client.js";
export 
export 
...
async function resolveTargetChannelId(params) {
  ...
  const userId = params.target.id ? params.target.id : await resolveUserIdByUsername({ baseUrl: params.baseUrl, token: params.token, username: (params.target.username ?? "") });
...
export async function sendMessageMattermost(to, text, opts = {  }) {
  ...
  const channelId = await resolveTargetChannelId({ target, baseUrl, token });
  const client = createMattermostClient({ baseUrl, botToken: token });
    ...
    try {
      {
        ...
        const fileInfo = await uploadMattermostFile(client, { channelId, buffer: media.buffer, fileName: (media.fileName ?? "upload"), contentType: (media.contentType ?? undefined) });
        fileIds = [fileInfo.id];
      }
    }
    catch (err) {
      {
        ...
        if (core.logging.shouldLogVerbose()) {
          logger.debug?.("mattermost send: media upload failed, falling back to URL text: ");
        }
  ...
  if ((!message && (!fileIds || (fileIds.length === 0)))) {
    if (uploadError) {
      throw new Error("Mattermost media upload failed: ");
    }
    throw new Error("Mattermost message is empty");
  }
  const post = await createMattermostPost(client, { channelId, message, rootId: opts.replyToId, fileIds });
  core.channel.activity.record({ channel: "mattermost", accountId: account.accountId, direction: "outbound" });
  return { messageId: (post.id ?? "unknown"), channelId };
}
