...
export function createMattermostClient(params) {
  ...
  const request = async (path, init) => {
    ...
    if (!res.ok) {
      ...
      throw new Error("Mattermost API  : ");
    }
...
export async function fetchMattermostUserByUsername(client, username) {
  return await client.request("/users/username/");
}

export async function fetchMattermostChannel(client, channelId) {
  return await client.request("/channels/");
}
...
export async function sendMattermostTyping(client, params) {
  ...
  await client.request("/users/me/typing", { method: "POST", body: JSON.stringify(payload) });
}

export async function createMattermostDirectChannel(client, userIds) {
  return await client.request("/channels/direct", { method: "POST", body: JSON.stringify(userIds) });
}

export async function createMattermostPost(client, params) {
  const payload = { channel_id: params.channelId, message: params.message };
  if (params.rootId) {
  ...
  return await client.request("/posts", { method: "POST", body: JSON.stringify(payload) });
}
...
export async function uploadMattermostFile(client, params) {
  ...
  const res = await fetch("/files", { method: "POST", headers: { Authorization: "Bearer " }, body: form });
  if (!res.ok) {
    const detail = await readMattermostError(res);
    throw new Error("Mattermost API  : ");
  }
