...
vi.mock("../../../src/agents/pi-embedded-runner.js", () => {
  return { runEmbeddedPiAgent: vi.fn(async () => { meta: { startedAt: Date.now() }, payloads: [{ text: "{}" }] }) };
});
import { runEmbeddedPiAgent } from "../../../src/agents/pi-embedded-runner.js";
import { createLlmTaskTool } from "./llm-task-tool.js";
function fakeApi(overrides = {  }) {
  return { id: "llm-task", name: "llm-task", source: "test", config: { agents: { defaults: { workspace: "/tmp", model: { primary: "openai-codex/gpt-5.2" } } } }, pluginConfig: {  }, runtime: { version: "test" }, logger: { debug: function() {
  }, info: function() {
  }, warn: function() {
  }, error: function() {
...
describe("llm-task tool (json-only)", () => {
  ...
  it("returns parsed json", async () => {
    runEmbeddedPiAgent.mockResolvedValueOnce({ meta: {  }, payloads: [{ text: JSON.stringify({ foo: "bar" }) }] });
  ...
  it("strips fenced json", async () => {
    runEmbeddedPiAgent.mockResolvedValueOnce({ meta: {  }, payloads: [{ text: "```json
{\"ok\":true}
```" }] });
    const tool = createLlmTaskTool(fakeApi());
  ...
  it("throws on invalid json", async () => {
    runEmbeddedPiAgent.mockResolvedValueOnce({ meta: {  }, payloads: [{ text: "not-json" }] });
    const tool = createLlmTaskTool(fakeApi());
    await expect(tool.execute("id", { prompt: "x" })).rejects.toThrow(/invalid json/i);
  });
  it("throws on schema mismatch", async () => {
    runEmbeddedPiAgent.mockResolvedValueOnce({ meta: {  }, payloads: [{ text: JSON.stringify({ foo: 1 }) }] });
    const tool = createLlmTaskTool(fakeApi());
  ...
  it("passes provider/model overrides to embedded runner", async () => {
    runEmbeddedPiAgent.mockResolvedValueOnce({ meta: {  }, payloads: [{ text: JSON.stringify({ ok: true }) }] });
    const tool = createLlmTaskTool(fakeApi());
  ...
  it("enforces allowedModels", async () => {
    runEmbeddedPiAgent.mockResolvedValueOnce({ meta: {  }, payloads: [{ text: JSON.stringify({ ok: true }) }] });
    const tool = createLlmTaskTool(fakeApi({ pluginConfig: { allowedModels: ["openai-codex/gpt-5.2"] } }));
    await expect(tool.execute("id", { prompt: "x", provider: "anthropic", model: "claude-4-sonnet" })).rejects.toThrow(/not allowed/i);
  });
