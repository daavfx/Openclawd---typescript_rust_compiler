...
import { clearMaxDurationTimer, rejectTranscriptWaiter, resolveTranscriptWaiter, startMaxDurationTimer } from "./timers.js";
...
function createInboundCall(params) {
  ...
  const callRecord = { callId, providerCallId: params.providerCallId, provider: (params.ctx.provider?.name || "twilio"), direction: "inbound", state: "ringing", from: params.from, to: params.to, startedAt: Date.now(), transcript: [], processedEventIds: [], metadata: { initialMessage: (params.ctx.config.inboundGreeting || "Hello! How can I help you today?") } };
...
export function processEvent(ctx, event) {
  ...
  let call = findCall({ activeCalls: ctx.activeCalls, providerCallIdMap: ctx.providerCallIdMap, callIdOrProviderCallId: event.callId });
  ...
  switch (event.type) {
    ...
    case "call.answered":
      ...
      startMaxDurationTimer({ ctx, callId: call.callId, onTimeout: async (callId) => {
    ...
    case "call.ended":
      ...
      rejectTranscriptWaiter(ctx, call.callId, "Call ended: ");
    ...
    case "call.error":
      if (!event.retryable) {
        ...
        clearMaxDurationTimer(ctx, call.callId);
        rejectTranscriptWaiter(ctx, call.callId, "Call error: ");
        ctx.activeCalls.delete(call.callId);
        if (call.providerCallId) {
          ctx.providerCallIdMap.delete(call.providerCallId);
        }
