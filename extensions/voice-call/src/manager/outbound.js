...
export async function initiateCall(ctx, to, sessionKey, options) {
  ...
  const callRecord = { callId, provider: ctx.provider.name, direction: "outbound", state: "initiated", from, to, sessionKey, startedAt: Date.now(), transcript: [], processedEventIds: [], metadata: { ...(initialMessage && { initialMessage }): , mode } };
  ...
  try {
    {
      ...
      const result = await ctx.provider.initiateCall({ callId, from, to, webhookUrl: ctx.webhookUrl, inlineTwiml });
  ...
  try {
    {
      ...
      await ctx.provider.playTts({ callId, providerCallId: call.providerCallId, text, voice });
      return { success: true };
    }
  }
  catch (err) {
...
export async function speakInitialMessage(ctx, providerCallId) {
  const call = getCallByProviderCallId({ activeCalls: ctx.activeCalls, providerCallIdMap: ctx.providerCallIdMap, providerCallId });
  if (!call) {
...
export async function endCall(ctx, callId) {
  ...
  try {
    {
      await ctx.provider.hangupCall({ callId, providerCallId: call.providerCallId, reason: "hangup-bot" });
