export 
export const OPENAI_TTS_VOICES = ["alloy", "ash", "ballad", "coral", "echo", "fable", "nova", "onyx", "sage", "shimmer", "verse", "marin", "cedar"]
export 
export class OpenAITTSProvider {
  ...
  constructor(text, instructions) {
    const body = { model: this.model, input: text, voice: this.voice, response_format: "pcm", speed: this.speed };
    ...
    const response = await fetch("https://api.openai.com/v1/audio/speech", { method: "POST", headers: { Authorization: "Bearer ", "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (!response.ok) {
...
function linearToMulaw(sample) {
  ...
  sample += BIAS;
  let exponent = 7;
  for (let expMask = 16384; (((sample & expMask) === 0) && (exponent > 0)); (exponent-- , expMask = (expMask >> 1))) {
  }
