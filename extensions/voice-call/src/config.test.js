...
describe("validateProviderConfig", () => {
  ...
  describe("twilio provider", () => {
    ...
    it("fails validation when accountSid is missing everywhere", () => {
      ...
      expect(result.errors).toContain("plugins.entries.voice-call.config.twilio.accountSid is required (or set TWILIO_ACCOUNT_SID env)");
    });
    it("fails validation when authToken is missing everywhere", () => {
      ...
      const result = validateProviderConfig(config);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain("plugins.entries.voice-call.config.twilio.authToken is required (or set TWILIO_AUTH_TOKEN env)");
    });
  });
  describe("telnyx provider", () => {
    ...
    it("fails validation when apiKey is missing everywhere", () => {
      ...
      const result = validateProviderConfig(config);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain("plugins.entries.voice-call.config.telnyx.apiKey is required (or set TELNYX_API_KEY env)");
    });
  });
  describe("plivo provider", () => {
    ...
    it("fails validation when authId is missing everywhere", () => {
      ...
      const result = validateProviderConfig(config);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain("plugins.entries.voice-call.config.plivo.authId is required (or set PLIVO_AUTH_ID env)");
    });
  });
