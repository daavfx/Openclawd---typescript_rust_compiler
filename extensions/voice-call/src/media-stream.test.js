...
const createStubSession = () => { connect: async () => {
}, sendAudio: () => {
}, waitForTranscript: async () => "", onPartial: () => {
}, onTranscript: () => {
}, onSpeechStart: () => {
}, close: () => {
}, isConnected: () => true };
const createStubSttProvider = () => { createSession: () => createStubSession() };
const flush = async () => {
...
describe("MediaStreamHandler TTS queue", () => {
  it("serializes TTS playback and resolves in order", async () => {
    const handler = new MediaStreamHandler({ sttProvider: createStubSttProvider() });
    const started = [];
    const finished = [];
    let resolveFirst;
