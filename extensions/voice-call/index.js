...
import { VoiceCallConfigSchema, resolveVoiceCallConfig, validateProviderConfig } from "./src/config.js";
import { registerVoiceCallCli } from "./src/cli.js";
import { createVoiceCallRuntime } from "./src/runtime.js";
const voiceCallConfigSchema = { parse: function(value) {
...
const voiceCallPlugin = { id: "voice-call", name: "Voice Call", description: "Voice-call plugin with Telnyx/Twilio/Plivo providers", configSchema: voiceCallConfigSchema, register: function(api) {
  ...
  const ensureRuntime = async () => {
    ...
    if (!runtimePromise) {
      runtimePromise = createVoiceCallRuntime({ config, coreConfig: api.config, ttsRuntime: api.runtime.tts, logger: api.logger });
    }
    ...
    try {
      {
        ...
        const result = await rt.manager.initiateCall(to, undefined, { message, mode });
        if (!result.success) {
    ...
    try {
      {
        ...
        const rt = await ensureRuntime();
        const result = await rt.manager.initiateCall(to, undefined, { message: (message || undefined) });
        if (!result.success) {
          respond(false, { error: (result.error || "initiate failed") });
          return;
        }
  ...
  api.registerTool({ name: "voice_call", label: "Voice Call", description: "Make phone calls and have voice conversations via the voice-call plugin.", parameters: VoiceCallToolSchema, execute: async function(_toolCallId, params) {
    const json = (payload) => { content: [{ type: "text", text: JSON.stringify(payload, null, 2) }], details: payload };
    try {
      {
        const rt = await ensureRuntime();
            ...
            case "initiate_call":
              {
                ...
                const result = await rt.manager.initiateCall(to, undefined, { message, mode: ((params.mode === "notify") || (params.mode === "conversation")) ? params.mode : undefined });
                if (!result.success) {
        ...
        const result = await rt.manager.initiateCall(to, undefined, { message: ((typeof params.message === "string") && params.message.trim()) ? params.message.trim() : undefined });
        if (!result.success) {
          throw new Error((result.error || "initiate failed"));
        }
        return json({ callId: result.callId, initiated: true });
  ...
  api.registerCli(({program}) => registerVoiceCallCli({ program, config, ensureRuntime, logger: api.logger }), { commands: ["voicecall"] });
  api.registerService({ id: "voicecall", start: async () => {
    if (!config.enabled) {
      return;
    }
    try {
      ...
      {
        api.logger.error("[voice-call] Failed to start runtime: ");
      }
    }
  }, stop: async () => {
    if (!runtimePromise) {
