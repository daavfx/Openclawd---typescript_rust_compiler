...
describe("msteams polls", () => {
  ...
  it("builds poll cards with fallback text", () => {
    const card = buildMSTeamsPollCard({ question: "Lunch?", options: ["Pizza", "Sushi"] });
  ...
  it("extracts poll votes from activity values", () => {
    const vote = extractMSTeamsPollVote({ value: { openclawPollId: "poll-1", choices: "0,1" } });
    expect(vote).toEqual({ pollId: "poll-1", selections: ["0", "1"] });
  });
  it("stores and records poll votes", async () => {
    ...
    await store.createPoll({ id: "poll-2", question: "Pick one", options: ["A", "B"], maxSelections: 1, createdAt: new Date().toISOString(), votes: {  } });
