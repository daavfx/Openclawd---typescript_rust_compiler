...
function parseRecipient(to) {
  ...
  const finalize = (type, id) => {
    ...
    if (!normalized) {
      throw new Error("Invalid target value: missing  id");
    }
...
export async function resolveMSTeamsSendContext(params) {
  ...
  if (!found) {
    throw new Error(("No conversation reference found for :. " + "The bot must receive a message from this conversation before it can send proactively."));
  }
  ...
  if ((storedConversationType === "personal")) {
    ...
    if ((storedConversationType === "channel")) {
  ...
  return { appId: creds.appId, conversationId, ref, adapter: adapter, log, conversationType, tokenProvider, sharePointSiteId, mediaMaxBytes };
}
