...
export function buildMSTeamsPollCard(params) {
  ...
  const choices = params.options.map((option, index) => { title: option, value: String(index) });
  const hint = (cappedMaxSelections > 1) ? "Select up to  option." : "Select one option.";
  const card = { type: "AdaptiveCard", version: "1.5", body: [{ type: "TextBlock", text: params.question, wrap: true, weight: "Bolder", size: "Medium" }, { type: "Input.ChoiceSet", id: "choices", isMultiSelect: (cappedMaxSelections > 1), style: "expanded", choices }, { type: "TextBlock", text: hint, wrap: true, isSubtle: true, spacing: "Small" }], actions: [{ type: "Action.Submit", title: "Vote", data: { openclawPollId: pollId, pollId }, msteams: { type: "messageBack", text: "openclaw poll vote", displayText: "Vote recorded", value: { openclawPollId: pollId, pollId } } }] };
  const fallbackLines = ["Poll: ", ...params.options.map((option, index) => ". ")];
  return { pollId, question: params.question, options: params.options, maxSelections: cappedMaxSelections, card, fallbackText: fallbackLines.join("
") };
}
...
export function createMSTeamsPollStoreFs(params) {
  const filePath = resolveMSTeamsStorePath({ filename: STORE_FILENAME, env: params?.env, homedir: params?.homedir, stateDir: params?.stateDir, storePath: params?.storePath });
