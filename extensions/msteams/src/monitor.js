...
export async function monitorMSTeamsProvider(opts) {
  ...
  const resolveAllowlistUsers = async (label, entries) => {
    ...
    const mapping = resolved.filter((entry) => (entry.resolved && entry.id)).map((entry) => "→");
      ...
      if ((teamsConfig && (Object.keys(teamsConfig).length > 0))) {
        ...
        for (const [teamKey, teamCfg] of Object.entries(teamsConfig)) {
          ...
          for (const channelKey of channelKeys) {
            entries.push({ input: "/", teamKey, channelKey });
          }
        }
        if ((entries.length > 0)) {
          const resolved = await resolveMSTeamsChannelAllowlist({ cfg, entries: entries.map((entry) => entry.input) });
          const mapping = [];
          const unresolved = [];
          ...
          resolved.forEach((entry, idx) => {
            ...
            const mergedChannels = { ...(sourceTeam.channels ?? {  }): , ...(existing.channels ?? {  }):  };
            ...
            if ((source.channelKey && entry.channelId)) {
              ...
              if (sourceChannel) {
                nextTeams[entry.teamId] = { ...mergedTeam: , channels: { ...mergedChannels: , [entry.channelId]: { ...sourceChannel: , ...(mergedChannels?.[entry.channelId] ?? {  }):  } } };
              }
            }
          });
  ...
  msteamsCfg = { ...msteamsCfg: , allowFrom, groupAllowFrom, teams: teamsConfig };
  cfg = { ...cfg: , channels: { ...cfg.channels: , msteams: msteamsCfg } };
  ...
  log.info("starting provider (port )");
  ...
  const handler = registerMSTeamsHandlers(new ActivityHandler(), { cfg, runtime, appId, adapter: adapter, tokenProvider, textLimit, mediaMaxBytes, conversationStore, pollStore, log });
  ...
  log.debug("listening on paths", { primary: configuredPath, fallback: "/api/messages" });
  const httpServer = expressApp.listen(port, () => {
    log.info("msteams provider started on port ");
  });
