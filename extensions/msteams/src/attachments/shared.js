...
export function inferPlaceholder(params) {
  ...
  const looksLikeImage = ((mime.startsWith("image/") || IMAGE_EXT_RE.test(name)) || IMAGE_EXT_RE.test("x."));
...
export function isLikelyImageAttachment(att) {
  ...
  if (((contentType === "application/vnd.microsoft.teams.file.download.info") && isRecord(att.content))) {
    ...
    if ((fileType && IMAGE_EXT_RE.test("x."))) {
      return true;
    }
...
export function extractHtmlFromAttachment(att) {
  ...
  const text = (typeof att.content.text === "string") ? att.content.text : (typeof att.content.body === "string") ? att.content.body : (typeof att.content.content === "string") ? att.content.content : undefined;
  return text;
}
    ...
    while (match) {
      ...
      if ((src && !src.startsWith("cid:"))) {
        if (src.startsWith("data:")) {
          ...
          out.push({ kind: "url", url: src, fileHint: fileHintFromUrl(src), placeholder: "<media:image>" });
        }
      }
...
function isHostAllowed(host, allowlist) {
  ...
  return allowlist.some((entry) => ((normalized === entry) || normalized.endsWith(".")));
}
