...
export function prepareFileConsentActivity(params) {
  ...
  const uploadId = storePendingUpload({ buffer: media.buffer, filename: media.filename, contentType: media.contentType, conversationId });
  const consentCard = buildFileConsentCard({ filename: media.filename, description: (description || "File: "), sizeInBytes: media.buffer.length, context: { uploadId } });
  const activity = { type: "message", attachments: [consentCard] };
