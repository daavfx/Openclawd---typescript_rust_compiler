...
export async function listMSTeamsDirectoryPeersLive(params) {
  ...
  if (query.includes("@")) {
    ...
    const path = "/users?$search=&$select=id,displayName,mail,userPrincipalName&$top=";
  ...
  return users.map((user) => {
    ...
    return { kind: "user", id: "user:", name: (name || undefined), handle: handle ? "@" : undefined, raw: user };
  }).filter(Boolean);
}
...
export async function listMSTeamsDirectoryGroupsLive(params) {
  ...
  for (const team of teams) {
    ...
    if (!channelQuery) {
      results.push({ kind: "group", id: "team:", name: teamName, handle: teamName ? "#" : undefined, raw: team });
    ...
    for (const channel of channels) {
      ...
      results.push({ kind: "group", id: "conversation:", name: "/", handle: "#", raw: channel });
      if ((results.length >= limit)) {
        return results;
      }
    }
