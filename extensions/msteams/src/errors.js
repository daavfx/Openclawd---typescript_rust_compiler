export function formatUnknownError(err) {
  ...
  if ((typeof err === "function")) {
    return err.name ? "[function ]" : "[function]";
  }
  try {
    {
...
export function classifyMSTeamsSendError(err) {
  ...
  if ((statusCode === 429)) {
    return { kind: "throttled", statusCode, retryAfterMs: (retryAfterMs ?? undefined) };
  }
  if (((statusCode === 408) || ((statusCode != null) && (statusCode >= 500)))) {
    return { kind: "transient", statusCode, retryAfterMs: (retryAfterMs ?? undefined) };
  }
  if (((statusCode != null) && (statusCode >= 400))) {
    return { kind: "permanent", statusCode };
  }
  return { kind: "unknown", statusCode: (statusCode ?? undefined), retryAfterMs: (retryAfterMs ?? undefined) };
}
