...
describe("msteams media-helpers", () => {
  describe("getMimeType", () => {
    ...
    it("detects document types", async () => {
      ...
      expect(await getMimeType("https://example.com/doc.docx")).toBe("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
      expect(await getMimeType("https://example.com/spreadsheet.xlsx")).toBe("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
    });
  });
  describe("extractFilename", () => {
    ...
    it("extracts original filename from embedded pattern", async () => {
      expect(await extractFilename("/media/inbound/report---a1b2c3d4-e5f6-7890-abcd-ef1234567890.pdf")).toBe("report.pdf");
    });
    it("extracts original filename with uppercase UUID", async () => {
      expect(await extractFilename("/media/inbound/Document---A1B2C3D4-E5F6-7890-ABCD-EF1234567890.docx")).toBe("Document.docx");
    });
    it("falls back to UUID filename for legacy paths", async () => {
      expect(await extractFilename("/media/inbound/a1b2c3d4-e5f6-7890-abcd-ef1234567890.pdf")).toBe("a1b2c3d4-e5f6-7890-abcd-ef1234567890.pdf");
    });
