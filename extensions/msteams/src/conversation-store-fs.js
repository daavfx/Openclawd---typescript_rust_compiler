...
export function createMSTeamsConversationStoreFs(params) {
  ...
  const list = async () => {
    ...
    return Object.entries(store.conversations).map(([conversationId, reference]) => { conversationId, reference });
  };
  ...
  const upsert = async (conversationId, reference) => {
    ...
    await withFileLock(filePath, empty, async () => {
      const store = await readStore();
      store.conversations[normalizedId] = { ...reference: , lastSeenAt: new Date().toISOString() };
      const nowMs = Date.now();
