...
const runtimeStub = { state: { resolveStateDir: (env = process.env, homedir) => {
...
describe("msteams conversation store (fs)", () => {
  ...
  it("filters and prunes expired entries (but keeps legacy ones)", async () => {
    ...
    const ref = { conversation: { id: "19:active@thread.tacv2" }, channelId: "msteams", serviceUrl: "https://service.example.com", user: { id: "u1", aadObjectId: "aad1" } };
    await store.upsert("19:active@thread.tacv2", ref);
    const filePath = path.join(stateDir, "msteams-conversations.json");
    const raw = await fs.promises.readFile(filePath, "utf-8");
    const json = JSON.parse(raw);
    json.conversations["19:old@thread.tacv2"] = { ...ref: , conversation: { id: "19:old@thread.tacv2" }, lastSeenAt: new Date((Date.now() - 60000)).toISOString() };
    json.conversations["19:legacy@thread.tacv2"] = { ...ref: , conversation: { id: "19:legacy@thread.tacv2" } };
...
");
    ...
    await store.upsert("19:new@thread.tacv2", { ...ref: , conversation: { id: "19:new@thread.tacv2" } });
    const rawAfter = await fs.promises.readFile(filePath, "utf-8");
    const jsonAfter = JSON.parse(rawAfter);
    expect(Object.keys(jsonAfter.conversations).sort()).toEqual(["19:active@thread.tacv2", "19:legacy@thread.tacv2", "19:new@thread.tacv2"]);
  });
});
