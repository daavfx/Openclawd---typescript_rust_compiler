...
export async function probeMSTeams(cfg) {
  ...
  if (!creds) {
    return { ok: false, error: "missing credentials (appId, appPassword, tenantId)" };
  }
  try {
    {
      ...
      try {
        {
          ...
          graph = { ok: true, roles: readStringArray(payload?.roles), scopes: readScopes(payload?.scp) };
        }
      }
      catch (err) {
        {
    ...
    {
      return { ok: false, appId: creds.appId, error: formatUnknownError(err) };
    }
  }
}
