import { createActionGate, jsonResult, readNumberParam, readReactionParams, readStringParam } from "openclaw/plugin-sdk";
import { listEnabledGoogleChatAccounts, resolveGoogleChatAccount } from "./accounts.js";
import { createGoogleChatReaction, deleteGoogleChatReaction, listGoogleChatReactions, sendGoogleChatMessage, uploadGoogleChatAttachment } from "./api.js";
...
function isReactionsEnabled(accounts, cfg) {
  for (const account of accounts) {
    const gate = createActionGate((account.config.actions ?? cfg.channels?.["googlechat"]?.actions));
    if (gate("reactions")) {
      return true;
    }
  }
...
export const googlechatMessageActions = { listActions: ({cfg}) => {
  ...
  if ((action === "send")) {
    ...
    if (mediaUrl) {
      ...
      await sendGoogleChatMessage({ account, space, text: content, thread: (threadId ?? undefined), attachments: upload.attachmentUploadToken ? [{ attachmentUploadToken: upload.attachmentUploadToken, contentName: loaded.filename }] : undefined });
      return jsonResult({ ok: true, to: space });
    }
    await sendGoogleChatMessage({ account, space, text: content, thread: (threadId ?? undefined) });
    return jsonResult({ ok: true, to: space });
  }
  if ((action === "react")) {
    ...
    const reaction = await createGoogleChatReaction({ account, messageName, emoji });
    return jsonResult({ ok: true, reaction });
  }
  if ((action === "reactions")) {
    const messageName = readStringParam(params, "messageId", { required: true });
    const limit = readNumberParam(params, "limit", { integer: true });
    const reactions = await listGoogleChatReactions({ account, messageName, limit: (limit ?? undefined) });
