...
import { downloadGoogleChatMedia, deleteGoogleChatMessage, sendGoogleChatMessage, updateGoogleChatMessage } from "./api.js";
...
export async function handleGoogleChatWebhookRequest(req, res) {
  ...
  if (((rawObj.commonEventObject?.hostApp === "CHAT") && rawObj.chat?.messagePayload)) {
    ...
    raw = { type: "MESSAGE", space: messagePayload?.space, message: messagePayload?.message, user: chat.user, eventTime: chat.eventTime };
    const systemIdToken = rawObj.authorizationEventObject?.systemIdToken;
    if ((!bearer && systemIdToken)) {
      Object.assign(req.headers, { authorization: "Bearer " });
    }
  }
  const event = raw;
  ...
  for (const target of targets) {
    ...
    const verification = await verifyGoogleChatRequest({ bearer: effectiveBearer, audienceType, audience });
    if (verification.ok) {
      selected = target;
      break;
    }
  ...
  processGoogleChatEvent(event, selected).catch((err) => {
    selected?.runtime.error?.("[] Google Chat webhook failed: ");
  });
  res.statusCode = 200;
...
async function processGoogleChatEvent(event, target) {
  ...
  await processMessageWithPipeline({ event, account: target.account, config: target.config, runtime: target.runtime, core: target.core, statusSink: target.statusSink, mediaMaxMb: target.mediaMaxMb });
}
...
async function processMessageWithPipeline(params) {
  ...
  if (!allowBots) {
    if ((sender?.type?.toUpperCase() === "BOT")) {
      logVerbose(core, runtime, "skip bot-authored message ()");
      return;
    }
  ...
  const groupConfigResolved = resolveGroupConfig({ groupId: spaceId, groupName: (space.displayName ?? null), groups: (account.config.groups ?? undefined) });
  const groupEntry = groupConfigResolved.entry;
  const groupUsers = ((groupEntry?.users ?? account.config.groupAllowFrom) ?? []);
  let effectiveWasMentioned;
  if (isGroup) {
    if ((groupPolicy === "disabled")) {
      logVerbose(core, runtime, "drop group message (groupPolicy=disabled, space=)");
      return;
    }
    ...
    if ((groupPolicy === "allowlist")) {
      if (!groupAllowlistConfigured) {
        logVerbose(core, runtime, "drop group message (groupPolicy=allowlist, no allowlist, space=)");
        return;
      }
      if (!groupAllowed) {
        logVerbose(core, runtime, "drop group message (not allowlisted, space=)");
        return;
      }
    }
    if (((groupEntry?.enabled === false) || (groupEntry?.allow === false))) {
      logVerbose(core, runtime, "drop group message (space disabled, space=)");
      return;
    }
    if ((groupUsers.length > 0)) {
      const ok = isSenderAllowed(senderId, senderEmail, groupUsers.map((v) => String(v)));
      if (!ok) {
        logVerbose(core, runtime, "drop group message (sender not allowed, )");
        return;
      }
    }
  }
  ...
  const commandAuthorized = shouldComputeAuth ? core.channel.commands.resolveCommandAuthorizedFromAuthorizers({ useAccessGroups, authorizers: [{ configured: (commandAllowFrom.length > 0), allowed: senderAllowedForCommands }] }) : undefined;
  if (isGroup) {
    const requireMention = ((groupEntry?.requireMention ?? account.config.requireMention) ?? true);
    const annotations = (message.annotations ?? []);
    const mentionInfo = extractMentionInfo(annotations, account.config.botUser);
    const allowTextCommands = core.channel.commands.shouldHandleTextCommands({ cfg: config, surface: "googlechat" });
    ...
    if (mentionGate.shouldSkip) {
      logVerbose(core, runtime, "drop group message (mention required, space=)");
      return;
    }
  }
  if (!isGroup) {
    if (((dmPolicy === "disabled") || (account.config.dm?.enabled === false))) {
      logVerbose(core, runtime, "Blocked Google Chat DM from  (dmPolicy=disabled)");
      return;
    }
    if ((dmPolicy !== "open")) {
      ...
      if (!allowed) {
        if ((dmPolicy === "pairing")) {
          const {code, created} = await core.channel.pairing.upsertPairingRequest({ channel: "googlechat", id: senderId, meta: { name: (senderName || undefined), email: senderEmail } });
          if (created) {
            logVerbose(core, runtime, "googlechat pairing request sender=");
            try {
              {
                await sendGoogleChatMessage({ account, space: spaceId, text: core.channel.pairing.buildPairingReply({ channel: "googlechat", idLine: "Your Google Chat user id: ", code }) });
                statusSink?.({ lastOutboundAt: Date.now() });
              }
            }
            catch (err) {
              {
                logVerbose(core, runtime, "pairing reply failed for : ");
              }
            }
          }
        } else {
          logVerbose(core, runtime, "Blocked unauthorized Google Chat sender  (dmPolicy=)");
        }
        return;
      }
    }
  ...
  if (((isGroup && core.channel.commands.isControlCommandMessage(rawBody, config)) && (commandAuthorized !== true))) {
    logVerbose(core, runtime, "googlechat: drop control command from ");
    return;
  }
  const route = core.channel.routing.resolveAgentRoute({ cfg: config, channel: "googlechat", accountId: account.accountId, peer: { kind: isGroup ? "group" : "dm", id: spaceId } });
  let mediaPath;
  let mediaType;
  ...
  const fromLabel = isGroup ? (space.displayName || "space:") : (senderName || "user:");
  ...
  const body = core.channel.reply.formatAgentEnvelope({ channel: "Google Chat", from: fromLabel, timestamp: event.eventTime ? Date.parse(event.eventTime) : undefined, previousTimestamp, envelope: envelopeOptions, body: rawBody });
  const groupSystemPrompt = (groupConfigResolved.entry?.systemPrompt?.trim() || undefined);
  const ctxPayload = core.channel.reply.finalizeInboundContext({ Body: body, RawBody: rawBody, CommandBody: rawBody, From: "googlechat:", To: "googlechat:", SessionKey: route.sessionKey, AccountId: route.accountId, ChatType: isGroup ? "channel" : "direct", ConversationLabel: fromLabel, SenderName: (senderName || undefined), SenderId: senderId, SenderUsername: senderEmail, WasMentioned: isGroup ? effectiveWasMentioned : undefined, CommandAuthorized: commandAuthorized, Provider: "googlechat", Surface: "googlechat", MessageSid: message.name, MessageSidFull: message.name, ReplyToId: message.thread?.name, ReplyToIdFull: message.thread?.name, MediaPath: mediaPath, MediaType: mediaType, MediaUrl: mediaPath, GroupSpace: isGroup ? (space.displayName ?? undefined) : undefined, GroupSystemPrompt: isGroup ? groupSystemPrompt : undefined, OriginatingChannel: "googlechat", OriginatingTo: "googlechat:" });
  void core.channel.session.recordSessionMetaFromInbound({ storePath, sessionKey: (ctxPayload.SessionKey ?? route.sessionKey), ctx: ctxPayload }).catch((err) => {
    runtime.error?.("googlechat: failed updating session meta: ");
  });
  let typingIndicator = (account.config.typingIndicator ?? "message");
  if ((typingIndicator === "reaction")) {
    runtime.error?.("[] typingIndicator=\"reaction\" requires user OAuth (not supported with service account). Falling back to \"message\" mode.");
  ...
  if ((typingIndicator === "message")) {
    try {
      {
        const botName = resolveBotDisplayName({ accountName: account.config.name, agentId: route.agentId, config });
        const result = await sendGoogleChatMessage({ account, space: spaceId, text: "_ is typing..._", thread: message.thread?.name });
        typingMessageName = result?.messageName;
      }
    }
    catch (err) {
      {
        runtime.error?.("Failed sending typing message: ");
      }
    }
  }
  await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({ ctx: ctxPayload, cfg: config, dispatcherOptions: { deliver: async (payload) => {
    await deliverGoogleChatReply({ payload, account, spaceId, runtime, core, config, statusSink, typingMessageName });
    typingMessageName = undefined;
  }, onError: (err, info) => {
    runtime.error?.("[] Google Chat  reply failed: ");
  } } });
}
  ...
  if ((mediaList.length > 0)) {
    ...
    if (typingMessageName) {
      try {
        ...
        {
          runtime.error?.("Google Chat typing cleanup failed: ");
          const fallbackText = payload.text?.trim() ? payload.text : (mediaList.length > 1) ? "Sent attachments." : "Sent attachment.";
          try {
            {
              await updateGoogleChatMessage({ account, messageName: typingMessageName, text: fallbackText });
              suppressCaption = Boolean(payload.text?.trim());
            }
          }
          catch (updateErr) {
            {
              runtime.error?.("Google Chat typing update failed: ");
            }
          }
        }
      }
    ...
    for (const mediaUrl of mediaList) {
      ...
      try {
        {
          const loaded = await core.channel.media.fetchRemoteMedia(mediaUrl, { maxBytes: (((account.config.mediaMaxMb ?? 20) * 1024) * 1024) });
          const upload = await uploadAttachmentForReply({ account, spaceId, buffer: loaded.buffer, contentType: loaded.contentType, filename: (loaded.filename ?? "attachment") });
          if (!upload.attachmentUploadToken) {
            throw new Error("missing attachment upload token");
          }
          await sendGoogleChatMessage({ account, space: spaceId, text: caption, thread: payload.replyToId, attachments: [{ attachmentUploadToken: upload.attachmentUploadToken, contentName: loaded.filename }] });
          statusSink?.({ lastOutboundAt: Date.now() });
        }
      }
      catch (err) {
        {
          runtime.error?.("Google Chat attachment send failed: ");
        }
      }
    }
    return;
      ...
      try {
        {
          if (((i === 0) && typingMessageName)) {
            await updateGoogleChatMessage({ account, messageName: typingMessageName, text: chunk });
          } else {
            await sendGoogleChatMessage({ account, space: spaceId, text: chunk, thread: payload.replyToId });
          }
          statusSink?.({ lastOutboundAt: Date.now() });
        }
      }
        ...
        {
          runtime.error?.("Google Chat message send failed: ");
        }
      }
    }
  }
...
async function uploadAttachmentForReply(params) {
  ...
  return await uploadGoogleChatAttachment({ account, space: spaceId, filename, buffer, contentType });
}
export function monitorGoogleChatProvider(options) {
  ...
  if (!webhookPath) {
    options.runtime.error?.("[] invalid webhook path");
    return () => {
    };
  }
  ...
  const unregister = registerGoogleChatWebhookTarget({ account: options.account, config: options.config, runtime: options.runtime, core, path: webhookPath, audienceType, audience, statusSink: options.statusSink, mediaMaxMb });
  return unregister;
}
