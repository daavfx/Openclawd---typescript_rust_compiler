...
export function normalizeGoogleChatTarget(raw) {
  ...
  if (isGoogleChatUserTarget(normalized)) {
    ...
    return suffix.includes("@") ? "users/" : normalized;
  }
...
export async function resolveGoogleChatOutboundSpace(params) {
  ...
  if (isGoogleChatUserTarget(base)) {
    const dm = await findGoogleChatDirectMessage({ account: params.account, userName: base });
    if (!dm?.name) {
      throw new Error("No Google Chat DM found for ");
    }
    return dm.name;
  }
  return base;
