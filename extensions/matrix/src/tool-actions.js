import { deleteMatrixMessage, editMatrixMessage, getMatrixMemberInfo, getMatrixRoomInfo, listMatrixPins, listMatrixReactions, pinMatrixMessage, readMatrixMessages, removeMatrixReactions, sendMatrixMessage, unpinMatrixMessage } from "./matrix/actions.js";
...
export async function handleMatrixAction(params, cfg) {
  ...
  if (reactionActions.has(action)) {
    ...
    if ((action === "react")) {
      ...
      if ((remove || isEmpty)) {
        const result = await removeMatrixReactions(roomId, messageId, { emoji: remove ? emoji : undefined });
      ...
      case "sendMessage":
        {
          ...
          const result = await sendMatrixMessage(to, content, { mediaUrl: (mediaUrl ?? undefined), replyToId: (replyToId ?? undefined), threadId: (threadId ?? undefined) });
      ...
      case "readMessages":
        {
          ...
          const result = await readMatrixMessages(roomId, { limit: (limit ?? undefined), before: (before ?? undefined), after: (after ?? undefined) });
          return jsonResult({ ok: true, ...result:  });
        }
      default:
        break;
  ...
  if ((action === "memberInfo")) {
    ...
    const result = await getMatrixMemberInfo(userId, { roomId: (roomId ?? undefined) });
  ...
  throw new Error("Unsupported Matrix action: ");
}
