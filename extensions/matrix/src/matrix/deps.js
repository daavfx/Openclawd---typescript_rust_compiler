...
export async function ensureMatrixSdkInstalled(params) {
  ...
  params.runtime.log?.("matrix: installing dependencies via  ()…");
  const result = await getMatrixRuntime().system.runCommandWithTimeout(command, { cwd: root, timeoutMs: 300000, env: { COREPACK_ENABLE_DOWNLOAD_PROMPT: "0" } });
  if ((result.code !== 0)) {
