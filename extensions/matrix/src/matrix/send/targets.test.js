...
describe("resolveMatrixRoomId", () => {
  it("uses m.direct when available", async () => {
    ...
    const client = { getAccountData: vi.fn().mockResolvedValue({ [userId]: ["!room:example.org"] }), getJoinedRooms: vi.fn(), getJoinedRoomMembers: vi.fn(), setAccountData: vi.fn() };
  ...
  it("falls back to joined rooms and persists m.direct", async () => {
    ...
    const client = { getAccountData: vi.fn().mockRejectedValue(new Error("nope")), getJoinedRooms: vi.fn().mockResolvedValue([roomId]), getJoinedRoomMembers: vi.fn().mockResolvedValue(["@bot:example.org", userId]), setAccountData };
  ...
  it("continues when a room member lookup fails", async () => {
    ...
    const client = { getAccountData: vi.fn().mockRejectedValue(new Error("nope")), getJoinedRooms: vi.fn().mockResolvedValue(["!bad:example.org", roomId]), getJoinedRoomMembers, setAccountData };
    const resolved = await resolveMatrixRoomId(client, userId);
    expect(resolved).toBe(roomId);
  ...
  it("allows larger rooms when no 1:1 match exists", async () => {
    ...
    const client = { getAccountData: vi.fn().mockRejectedValue(new Error("nope")), getJoinedRooms: vi.fn().mockResolvedValue([roomId]), getJoinedRoomMembers: vi.fn().mockResolvedValue(["@bot:example.org", userId, "@extra:example.org"]), setAccountData: vi.fn().mockResolvedValue(undefined) };
    const resolved = await resolveMatrixRoomId(client, userId);
    expect(resolved).toBe(roomId);
  });
});
