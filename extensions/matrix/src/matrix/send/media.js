...
export function buildMatrixMediaInfo(params) {
  ...
  if (params.imageInfo) {
    const dimensional = { ...base: , ...params.imageInfo:  };
    if ((typeof params.durationMs === "number")) {
      const videoInfo = { ...dimensional: , duration: params.durationMs };
      return videoInfo;
    }
    return dimensional;
  }
  if ((typeof params.durationMs === "number")) {
    const timedInfo = { ...base: , duration: params.durationMs };
    return timedInfo;
  }
...
export function buildMediaContent(params) {
  const info = buildMatrixMediaInfo({ size: params.size, mimetype: params.mimetype, durationMs: params.durationMs, imageInfo: params.imageInfo });
  const base = { msgtype: params.msgtype, body: params.body, filename: params.filename, info: (info ?? undefined) };
...
export async function prepareImageInfo(params) {
  ...
  if ((maxDim > THUMBNAIL_MAX_SIDE)) {
    try {
      {
        const thumbBuffer = await getCore().media.resizeToJpeg({ buffer: params.buffer, maxSide: THUMBNAIL_MAX_SIDE, quality: THUMBNAIL_QUALITY, withoutEnlargement: true });
        ...
        if (thumbMeta) {
          imageInfo.thumbnail_info = { w: thumbMeta.width, h: thumbMeta.height, mimetype: "image/jpeg", size: thumbBuffer.byteLength };
        }
      }
    }
    catch {
...
export async function resolveMediaDurationMs(params) {
  ...
  try {
    {
      const fileInfo = (params.contentType || params.fileName) ? { mimeType: params.contentType, size: params.buffer.byteLength, path: params.fileName } : undefined;
      const metadata = await parseBuffer(params.buffer, fileInfo, { duration: true, skipCovers: true });
