...
export function buildTextContent(body, relation) {
  const content = relation ? { msgtype: MsgType.Text, body, "m.relates_to": relation } : { msgtype: MsgType.Text, body };
...
export function buildThreadRelation(threadId, replyToId) {
  ...
  return { rel_type: RelationType.Thread, event_id: trimmed, is_falling_back: true, "m.in_reply_to": { event_id: (replyToId?.trim() || trimmed) } };
}
