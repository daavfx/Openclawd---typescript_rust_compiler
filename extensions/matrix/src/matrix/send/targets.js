...
async function persistDirectRoom(client, userId, roomId) {
  ...
  try {
    {
      await client.setAccountData(EventType.Direct, { ...existing: , [userId]: next });
    }
  }
  catch {
    {
...
async function resolveDirectRoomId(client, userId) {
  ...
  if (!trimmed.startsWith("@")) {
    throw new Error("Matrix user IDs must be fully qualified (got \"\")");
  }
  ...
  throw new Error("No direct room found for  (m.direct missing)");
}
export async function resolveMatrixRoomId(client, raw) {
  ...
  if (target.startsWith("#")) {
    ...
    if (!resolved) {
      throw new Error("Matrix alias  could not be resolved");
    }
    return resolved;
  }
  return target;
