...
import { LogService, MatrixClient, SimpleFsStorageProvider, RustSdkCryptoStorageProvider } from "@vector-im/matrix-bot-sdk";
...
function sanitizeUserIdList(input, label) {
  ...
  if (!Array.isArray(input)) {
    LogService.warn("MatrixClientLite", "Expected  list to be an array, got ");
    return [];
  }
  const filtered = input.filter((entry) => ((typeof entry === "string") && (entry.trim().length > 0)));
  if ((filtered.length !== input.length)) {
    LogService.warn("MatrixClientLite", "Dropping  invalid  entries from sync payload");
  }
  return filtered;
}
export async function createMatrixClient(params) {
  ...
  const storagePaths = resolveMatrixStoragePaths({ homeserver: params.homeserver, userId: params.userId, accessToken: params.accessToken, accountId: params.accountId, env });
  ...
  writeStorageMeta({ storagePaths, homeserver: params.homeserver, userId: params.userId, accountId: params.accountId });
