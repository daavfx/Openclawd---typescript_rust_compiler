...
function resolveLegacyStoragePaths(env = process.env) {
  ...
  return { storagePath: path.join(stateDir, "matrix", "bot-storage.json"), cryptoPath: path.join(stateDir, "matrix", "crypto") };
}
export function resolveMatrixStoragePaths(params) {
  ...
  const rootDir = path.join(stateDir, "matrix", "accounts", accountKey, "__", tokenHash);
  return { rootDir, storagePath: path.join(rootDir, "bot-storage.json"), cryptoPath: path.join(rootDir, "crypto"), metaPath: path.join(rootDir, STORAGE_META_FILENAME), accountKey, tokenHash };
}
...
export function writeStorageMeta(params) {
  try {
    {
      const payload = { homeserver: params.homeserver, userId: params.userId, accountId: (params.accountId ?? DEFAULT_ACCOUNT_KEY), accessTokenHash: params.storagePaths.tokenHash, createdAt: new Date().toISOString() };
      fs.mkdirSync(params.storagePaths.rootDir, { recursive: true });
