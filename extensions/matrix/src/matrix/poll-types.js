...
export const POLL_EVENT_TYPES = [M_POLL_START, M_POLL_RESPONSE, M_POLL_END, ORG_POLL_START, ORG_POLL_RESPONSE, ORG_POLL_END]
...
export function parsePollStartContent(content) {
  ...
  return { eventId: "", roomId: "", sender: "", senderName: "", question, answers, kind: (poll.kind ?? "m.poll.disclosed"), maxSelections: (poll.max_selections ?? 1) };
}

export function formatPollAsText(summary) {
  const lines = ["[Poll]", summary.question, "", ...summary.answers.map((answer, idx) => ". ")];
  return lines.join("
");
}

function buildTextContent(body) {
  return { "m.text": body, "org.matrix.msc1767.text": body };
}
...
export function buildPollStartContent(poll) {
  ...
  const answers = poll.options.map((option) => option.trim()).filter((option) => (option.length > 0)).map((option, idx) => { id: "answer", ...buildTextContent(option):  });
  ...
  return { [M_POLL_START]: { question: buildTextContent(question), kind: poll.multiple ? "m.poll.undisclosed" : "m.poll.disclosed", max_selections: maxSelections, answers }, "m.text": fallbackText, "org.matrix.msc1767.text": fallbackText };
}
