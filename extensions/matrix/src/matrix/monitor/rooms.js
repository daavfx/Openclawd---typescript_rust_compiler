...
export function resolveMatrixRoomConfig(params) {
  ...
  const candidates = buildChannelKeyCandidates(params.roomId, "room:", ...params.aliases, (params.name ?? ""));
  const {entry: matched, key: matchedKey, wildcardEntry, wildcardKey} = resolveChannelEntryMatch({ entries: rooms, keys: candidates, wildcardKey: "*" });
  ...
  return { allowed, allowlistConfigured, config: resolved, matchKey, matchSource };
}
