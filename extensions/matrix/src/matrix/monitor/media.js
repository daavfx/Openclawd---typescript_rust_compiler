...
async function fetchMatrixMediaBuffer(params) {
  ...
  try {
    ...
    {
      throw new Error("Matrix media download failed: ");
    }
  }
}
...
export async function downloadMatrixMedia(params) {
  ...
  if (params.file) {
    fetched = await fetchEncryptedMediaBuffer({ client: params.client, file: params.file, maxBytes: params.maxBytes });
  } else {
    fetched = await fetchMatrixMediaBuffer({ client: params.client, mxcUrl: params.mxcUrl, maxBytes: params.maxBytes });
  }
  if (!fetched) {
    return null;
  }
  ...
  return { path: saved.path, contentType: saved.contentType, placeholder: "[matrix media]" };
}
