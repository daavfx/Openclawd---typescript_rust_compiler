...
export 
export function listMatrixAccountIds(_cfg) {
...
export function resolveMatrixAccount(params) {
  ...
  const hasStored = (stored && resolved.homeserver) ? credentialsMatchConfig(stored, { homeserver: resolved.homeserver, userId: (resolved.userId || "") }) : false;
  const configured = (hasHomeserver && ((hasAccessToken || hasPasswordAuth) || Boolean(hasStored)));
  return { accountId, enabled, name: (base.name?.trim() || undefined), configured, homeserver: (resolved.homeserver || undefined), userId: (resolved.userId || undefined), config: base };
}
