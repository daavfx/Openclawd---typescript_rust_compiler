...
export function resolveDefaultNextcloudTalkAccountId(cfg) {
...
export function resolveNextcloudTalkAccount(params) {
  ...
  const resolve = (accountId) => {
    ...
    debugAccounts("resolve", { accountId, enabled, secretSource: secretResolution.source, baseUrl: baseUrl ? "[set]" : "[missing]" });
    return { accountId, enabled, name: (merged.name?.trim() || undefined), baseUrl, secret: secretResolution.secret, secretSource: secretResolution.source, config: merged };
  };
