...
function assertMediaWithinLimit(sizeBytes, maxBytes) {
  ...
  throw new Error("Media exceeds MB limit (got MB)");
}
...
export async function sendBlueBubblesMedia(params) {
  ...
  const maxBytes = resolveChannelMediaMaxBytes({ cfg, resolveChannelLimitMb: ({cfg, accountId}) => (cfg.channels?.bluebubbles?.accounts?.[accountId]?.mediaMaxMb ?? cfg.channels?.bluebubbles?.mediaMaxMb), accountId });
  let buffer;
  ...
  if (mediaBuffer) {
    ...
    if (!resolvedContentType) {
      ...
      const detected = await core.media.detectMime({ buffer: Buffer.isBuffer(mediaBuffer) ? mediaBuffer : Buffer.from(mediaBuffer), filePath: hint });
    ...
    if (HTTP_URL_RE.test(source)) {
      const fetched = await core.channel.media.fetchRemoteMedia({ url: source, maxBytes: ((typeof maxBytes === "number") && (maxBytes > 0)) ? maxBytes : undefined });
      ...
      if (!resolvedContentType) {
        const detected = await core.media.detectMime({ buffer: data, filePath: localPath });
        resolvedContentType = (detected ?? undefined);
      }
      if (!resolvedFilename) {
  ...
  const replyToMessageGuid = replyToId?.trim() ? resolveBlueBubblesMessageId(replyToId.trim(), { requireKnownShortId: true }) : undefined;
  const attachmentResult = await sendBlueBubblesAttachment({ to, buffer, filename: (resolvedFilename ?? "attachment"), contentType: (resolvedContentType ?? undefined), replyToMessageGuid, asVoice, opts: { cfg, accountId } });
  const trimmedCaption = caption?.trim();
  if (trimmedCaption) {
    await sendMessageBlueBubbles(to, trimmedCaption, { cfg, accountId, replyToMessageGuid });
  }
