import { BLUEBUBBLES_ACTION_NAMES, BLUEBUBBLES_ACTIONS, createActionGate, jsonResult, readNumberParam, readReactionParams, readStringParam } from "openclaw/plugin-sdk";
import { resolveBlueBubblesAccount } from "./accounts.js";
import { resolveBlueBubblesMessageId } from "./monitor.js";
import { isMacOS26OrHigher } from "./probe.js";
import { sendBlueBubblesReaction } from "./reactions.js";
import { resolveChatGuidForTarget, sendMessageBlueBubbles } from "./send.js";
import { editBlueBubblesMessage, unsendBlueBubblesMessage, renameBlueBubblesChat, setGroupIconBlueBubbles, addBlueBubblesParticipant, removeBlueBubblesParticipant, leaveBlueBubblesChat } from "./chat.js";
...
export const bluebubblesMessageActions = { listActions: ({cfg}) => {
...
}, supportsAction: ({action}) => SUPPORTED_ACTIONS.has(action), extractToolSend: ({args}) => {
  ...
  const account = resolveBlueBubblesAccount({ cfg: cfg, accountId: (accountId ?? undefined) });
  ...
  const resolveChatGuid = async () => {
    ...
    const target = chatIdentifier?.trim() ? { kind: "chat_identifier", chatIdentifier: chatIdentifier.trim() } : (typeof chatId === "number") ? { kind: "chat_id", chatId } : to ? mapTarget(to) : contextTarget ? mapTarget(contextTarget) : null;
    if (!target) {
      throw new Error("BlueBubbles  requires chatGuid, chatIdentifier, chatId, or to.");
    }
    if ((!baseUrl || !password)) {
    ...
    if (!resolved) {
      throw new Error("BlueBubbles  failed: chatGuid not found for target.");
    }
    return resolved;
  };
  if ((action === "react")) {
    ...
    if (!rawMessageId) {
      throw new Error(("BlueBubbles react requires messageId parameter (the message ID to react to). " + "Use action=react with messageId=<message_id>, emoji=<emoji>, and to/chatGuid to identify the chat."));
    }
    const messageId = resolveBlueBubblesMessageId(rawMessageId, { requireKnownShortId: true });
    const partIndex = readNumberParam(params, "partIndex", { integer: true });
    const resolvedChatGuid = await resolveChatGuid();
    await sendBlueBubblesReaction({ chatGuid: resolvedChatGuid, messageGuid: messageId, emoji, remove: (remove || undefined), partIndex: (typeof partIndex === "number") ? partIndex : undefined, opts });
    return jsonResult({ ok: true, ...remove ? { removed: true } : { added: emoji }:  });
  }
  if ((action === "edit")) {
    if (isMacOS26OrHigher((accountId ?? undefined))) {
      throw new Error(("BlueBubbles edit is not supported on macOS 26 or higher. " + "Apple removed the ability to edit iMessages in this version."));
    }
    const rawMessageId = readStringParam(params, "messageId");
    ...
    if ((!rawMessageId || !newText)) {
      ...
      throw new Error(("BlueBubbles edit requires: . " + "Use action=edit with messageId=<message_id>, text=<new_content>."));
    }
    const messageId = resolveBlueBubblesMessageId(rawMessageId, { requireKnownShortId: true });
    const partIndex = readNumberParam(params, "partIndex", { integer: true });
    const backwardsCompatMessage = readStringParam(params, "backwardsCompatMessage");
    await editBlueBubblesMessage(messageId, newText, { ...opts: , partIndex: (typeof partIndex === "number") ? partIndex : undefined, backwardsCompatMessage: (backwardsCompatMessage ?? undefined) });
    return jsonResult({ ok: true, edited: rawMessageId });
  }
  if ((action === "unsend")) {
    const rawMessageId = readStringParam(params, "messageId");
    if (!rawMessageId) {
      throw new Error(("BlueBubbles unsend requires messageId parameter (the message ID to unsend). " + "Use action=unsend with messageId=<message_id>."));
    }
    const messageId = resolveBlueBubblesMessageId(rawMessageId, { requireKnownShortId: true });
    const partIndex = readNumberParam(params, "partIndex", { integer: true });
    await unsendBlueBubblesMessage(messageId, { ...opts: , partIndex: (typeof partIndex === "number") ? partIndex : undefined });
  ...
  if ((action === "reply")) {
    ...
    if (((!rawMessageId || !text) || !to)) {
      ...
      throw new Error(("BlueBubbles reply requires: . " + "Use action=reply with messageId=<message_id>, message=<your reply>, target=<chat_target>."));
    }
    const messageId = resolveBlueBubblesMessageId(rawMessageId, { requireKnownShortId: true });
    const partIndex = readNumberParam(params, "partIndex", { integer: true });
    const result = await sendMessageBlueBubbles(to, text, { ...opts: , replyToMessageGuid: messageId, replyToPartIndex: (typeof partIndex === "number") ? partIndex : undefined });
  ...
  if ((action === "sendWithEffect")) {
    ...
    if (((!text || !to) || !effectId)) {
      ...
      throw new Error(("BlueBubbles sendWithEffect requires: . " + "Use action=sendWithEffect with message=<message>, target=<chat_target>, effectId=<effect_name>."));
    }
    const result = await sendMessageBlueBubbles(to, text, { ...opts: , effectId });
  ...
  if ((action === "setGroupIcon")) {
    ...
    if (!base64Buffer) {
      throw new Error(("BlueBubbles setGroupIcon requires an image. " + "Use action=setGroupIcon with media=<image_url> or path=<local_file_path> to set the group icon."));
    }
    const buffer = Uint8Array.from(atob(base64Buffer), (c) => c.charCodeAt(0));
    await setGroupIconBlueBubbles(resolvedChatGuid, buffer, filename, { ...opts: , contentType: (contentType ?? undefined) });
  ...
  if ((action === "sendAttachment")) {
    ...
    const result = await sendBlueBubblesAttachment({ to, buffer, filename, contentType: (contentType ?? undefined), caption: (caption ?? undefined), asVoice: (asVoice ?? undefined), opts });
