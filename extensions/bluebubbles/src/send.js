...
export 
export 
const EFFECT_MAP = { slam: "com.apple.MobileSMS.expressivesend.impact", loud: "com.apple.MobileSMS.expressivesend.loud", gentle: "com.apple.MobileSMS.expressivesend.gentle", invisible: "com.apple.MobileSMS.expressivesend.invisibleink", "invisible-ink": "com.apple.MobileSMS.expressivesend.invisibleink", "invisible ink": "com.apple.MobileSMS.expressivesend.invisibleink", invisibleink: "com.apple.MobileSMS.expressivesend.invisibleink", echo: "com.apple.messages.effect.CKEchoEffect", spotlight: "com.apple.messages.effect.CKSpotlightEffect", balloons: "com.apple.messages.effect.CKHappyBirthdayEffect", confetti: "com.apple.messages.effect.CKConfettiEffect", love: "com.apple.messages.effect.CKHeartEffect", heart: "com.apple.messages.effect.CKHeartEffect", hearts: "com.apple.messages.effect.CKHeartEffect", lasers: "com.apple.messages.effect.CKLasersEffect", fireworks: "com.apple.messages.effect.CKFireworksEffect", celebration: "com.apple.messages.effect.CKSparklesEffect" };
...
function extractMessageId(payload) {
  ...
  const candidates = [record.messageId, record.messageGuid, record.message_guid, record.guid, record.id, data?.messageId, data?.messageGuid, data?.message_guid, data?.message_id, data?.guid, data?.id];
...
function extractChatGuid(chat) {
  const candidates = [chat.chatGuid, chat.guid, chat.chat_guid, chat.identifier, chat.chatIdentifier, chat.chat_identifier];
  for (const candidate of candidates) {
    if (((typeof candidate === "string") && candidate.trim())) {
      return candidate.trim();
    }
...
async function queryChats(params) {
  const url = buildBlueBubblesApiUrl({ baseUrl: params.baseUrl, path: "/api/v1/chat/query", password: params.password });
  const res = await blueBubblesFetchWithTimeout(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ limit: params.limit, offset: params.offset, with: ["participants"] }) }, params.timeoutMs);
  if (!res.ok) {
    return [];
  }
...
export async function resolveChatGuidForTarget(params) {
  ...
  for (let offset = 0; (offset < 5000); offset += limit) {
    const chats = await queryChats({ baseUrl: params.baseUrl, password: params.password, timeoutMs: params.timeoutMs, offset, limit });
    if ((chats.length === 0)) {
      break;
    }
...
async function createNewChatWithMessage(params) {
  const url = buildBlueBubblesApiUrl({ baseUrl: params.baseUrl, path: "/api/v1/chat/new", password: params.password });
  const payload = { addresses: [params.address], message: params.message };
  const res = await blueBubblesFetchWithTimeout(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }, params.timeoutMs);
  if (!res.ok) {
    const errorText = await res.text();
    if ((((res.status === 400) || (res.status === 403)) || errorText.toLowerCase().includes("private api"))) {
      throw new Error("BlueBubbles send failed: Cannot create new chat - Private API must be enabled. Original error: ");
    }
    throw new Error("BlueBubbles create chat failed (): ");
  }
...
export async function sendMessageBlueBubbles(to, text, opts = {  }) {
  ...
  const account = resolveBlueBubblesAccount({ cfg: (opts.cfg ?? {  }), accountId: opts.accountId });
  ...
  if (!chatGuid) {
    if ((target.kind === "handle")) {
      return createNewChatWithMessage({ baseUrl, password, address: target.address, message: trimmedText, timeoutMs: opts.timeoutMs });
    }
  ...
  const payload = { chatGuid, tempGuid: crypto.randomUUID(), message: trimmedText };
  if (needsPrivateApi) {
  ...
  const url = buildBlueBubblesApiUrl({ baseUrl, path: "/api/v1/message/text", password });
  ...
  if (!res.ok) {
    const errorText = await res.text();
    throw new Error("BlueBubbles send failed (): ");
  }
  const body = await res.text();
  if (!body) {
    return { messageId: "ok" };
