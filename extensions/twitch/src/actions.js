...
function errorResponse(error) {
  return { content: [{ type: "text", text: JSON.stringify({ ok: false, error }) }], details: { ok: false } };
}
function readStringParam(args, key, options = {  }) {
  ...
  throw new Error("Parameter  must be a string, number, or boolean");
}
const TWITCH_ACTIONS = new Set(["send"]);
export const twitchMessageActions = { listActions: () => [...TWITCH_ACTIONS], supportsAction: ({action}) => TWITCH_ACTIONS.has(action), extractToolSend: ({args}) => {
  try {
    {
  ...
  if (!account) {
    return errorResponse("Account not found: . Available accounts: ");
  }
  ...
  try {
    {
      const result = await twitchOutbound.sendText({ cfg: ctx.cfg, to: targetChannel, text: (message ?? ""), accountId });
