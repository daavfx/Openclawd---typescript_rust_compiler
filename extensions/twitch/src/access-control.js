...
export function checkTwitchAccessControl(params) {
  ...
  if ((account.requireMention ?? true)) {
    ...
    if (!mentions.includes(botUsername.toLowerCase())) {
      return { allowed: false, reason: "message does not mention the bot (requireMention is enabled)" };
    }
  }
  if ((account.allowFrom && (account.allowFrom.length > 0))) {
    ...
    if (!senderId) {
      return { allowed: false, reason: "sender user ID not available for allowlist check" };
    }
    if (allowFrom.includes(senderId)) {
      return { allowed: true, matchKey: senderId, matchSource: "allowlist" };
    }
  }
  if ((account.allowedRoles && (account.allowedRoles.length > 0))) {
    const allowedRoles = account.allowedRoles;
    if (allowedRoles.includes("all")) {
      return { allowed: true, matchKey: "all", matchSource: "role" };
    }
    const hasAllowedRole = checkSenderRoles({ message, allowedRoles });
    if (!hasAllowedRole) {
      return { allowed: false, reason: "sender does not have any of the required roles: " };
    }
    return { allowed: true, matchKey: allowedRoles.join(","), matchSource: "role" };
  }
  return { allowed: true };
}
