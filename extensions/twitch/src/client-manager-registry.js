...
export function getOrCreateClientManager(accountId, logger) {
  ...
  registry.set(accountId, { manager, accountId, logger, createdAt: Date.now() });
  logger.info("Registered client manager for account: ");
  return manager;
}
...
export async function removeClientManager(accountId) {
  ...
  entry.logger.info("Unregistered client manager for account: ");
}
