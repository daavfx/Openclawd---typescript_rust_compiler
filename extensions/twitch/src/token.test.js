...
describe("token", () => {
  const mockMultiAccountConfig = { channels: { twitch: { accounts: { default: { username: "testbot", accessToken: "oauth:config-token" }, other: { username: "otherbot", accessToken: "oauth:other-token" } } } } };
  const mockSimplifiedConfig = { channels: { twitch: { username: "testbot", accessToken: "oauth:config-token" } } };
  beforeEach(() => {
    vi.clearAllMocks();
  });
  afterEach(() => {
  ...
  describe("resolveTwitchToken", () => {
    ...
    it("should use env var when config token is empty (simplified config)", () => {
      process.env.OPENCLAW_TWITCH_ACCESS_TOKEN = "oauth:env-token";
      const configWithEmptyToken = { channels: { twitch: { username: "testbot", accessToken: "" } } };
    ...
    it("should return empty token when neither config nor env has token (simplified config)", () => {
      const configWithoutToken = { channels: { twitch: { username: "testbot", accessToken: "" } } };
