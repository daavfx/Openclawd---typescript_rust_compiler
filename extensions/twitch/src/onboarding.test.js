...
const mockPrompter = { text: mockPromptText, confirm: mockPromptConfirm };
const mockAccount = { username: "testbot", accessToken: "oauth:test123", clientId: "test-client-id", channel: "#testchannel" };
describe("onboarding helpers", () => {
  ...
  describe("promptToken", () => {
    it("should return existing token when user confirms to keep it", async () => {
      ...
      expect(mockPromptConfirm).toHaveBeenCalledWith({ message: "Access token already configured. Keep it?", initialValue: true });
    ...
    it("should prompt for new token when user doesn't keep existing", async () => {
      ...
      expect(mockPromptText).toHaveBeenCalledWith({ message: "Twitch OAuth token (oauth:...)", initialValue: "", validate: expect.any(Function) });
    });
    it("should use env token as initial value when provided", async () => {
      ...
      expect(mockPromptText).toHaveBeenCalledWith(expect.objectContaining({ initialValue: "oauth:fromenv" }));
    });
  ...
  describe("promptUsername", () => {
    it("should prompt for username with validation", async () => {
      ...
      expect(mockPromptText).toHaveBeenCalledWith({ message: "Twitch bot username", initialValue: "", validate: expect.any(Function) });
    });
    it("should use existing username as initial value", async () => {
      ...
      expect(mockPromptText).toHaveBeenCalledWith(expect.objectContaining({ initialValue: "testbot" }));
    });
  });
  describe("promptClientId", () => {
    it("should prompt for client ID with validation", async () => {
      ...
      expect(mockPromptText).toHaveBeenCalledWith({ message: "Twitch Client ID", initialValue: "", validate: expect.any(Function) });
    });
  });
  ...
  describe("promptRefreshTokenSetup", () => {
    it("should return empty object when user declines", async () => {
      ...
      expect(mockPromptConfirm).toHaveBeenCalledWith({ message: "Enable automatic token refresh (requires client secret and refresh token)?", initialValue: false });
    });
    it("should prompt for credentials when user accepts", async () => {
      ...
      expect(result).toEqual({ clientSecret: "secret123", refreshToken: "refresh123" });
    });
    it("should use existing values as initial prompts", async () => {
      const {promptRefreshTokenSetup} = await import("./onboarding.js");
      const accountWithRefresh = { ...mockAccount: , clientSecret: "existing-secret", refreshToken: "existing-refresh" };
      mockPromptConfirm.mockResolvedValue(true);
      ...
      expect(mockPromptConfirm).toHaveBeenCalledWith(expect.objectContaining({ initialValue: true }));
    });
  });
  describe("configureWithEnvToken", () => {
    it("should return null when user declines env token", async () => {
      ...
      const result = await configureWithEnvToken({  }, mockPrompter, null, "oauth:fromenv", false, {  });
