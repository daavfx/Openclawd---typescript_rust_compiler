...
describe("status", () => {
  describe("collectTwitchStatusIssues", () => {
    it("should detect unconfigured accounts", () => {
      const snapshots = [{ accountId: "default", configured: false, enabled: true, running: false }];
    ...
    it("should detect disabled accounts", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: false, running: false }];
      const issues = collectTwitchStatusIssues(snapshots);
      expect(issues.length).toBeGreaterThan(0);
    ...
    it("should detect missing clientId when account configured (simplified config)", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false }];
      const mockCfg = { channels: { twitch: { username: "testbot", accessToken: "oauth:test123" } } };
    ...
    it("should warn about oauth: prefix in token (simplified config)", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false }];
      const mockCfg = { channels: { twitch: { username: "testbot", accessToken: "oauth:test123", clientId: "test-id" } } };
      const issues = collectTwitchStatusIssues(snapshots, () => mockCfg);
    ...
    it("should detect clientSecret without refreshToken (simplified config)", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false }];
      const mockCfg = { channels: { twitch: { username: "testbot", accessToken: "oauth:test123", clientId: "test-id", clientSecret: "secret123" } } };
      const issues = collectTwitchStatusIssues(snapshots, () => mockCfg);
    ...
    it("should detect empty allowFrom array (simplified config)", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false }];
      const mockCfg = { channels: { twitch: { username: "testbot", accessToken: "test123", clientId: "test-id", allowFrom: [] } } };
      const issues = collectTwitchStatusIssues(snapshots, () => mockCfg);
    ...
    it("should detect allowedRoles 'all' with allowFrom conflict (simplified config)", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false }];
      const mockCfg = { channels: { twitch: { username: "testbot", accessToken: "test123", clientId: "test-id", allowedRoles: ["all"], allowFrom: ["123456"] } } };
      const issues = collectTwitchStatusIssues(snapshots, () => mockCfg);
    ...
    it("should detect runtime errors", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false, lastError: "Connection timeout" }];
      const issues = collectTwitchStatusIssues(snapshots);
    ...
    it("should detect accounts that never connected", () => {
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: false, lastStartAt: undefined, lastInboundAt: undefined, lastOutboundAt: undefined }];
      const issues = collectTwitchStatusIssues(snapshots);
      const neverConnectedIssue = issues.find((i) => i.message.includes("never connected successfully"));
      expect(neverConnectedIssue).toBeDefined();
    });
    it("should detect long-running connections", () => {
      const oldDate = (Date.now() - ((((8 * 24) * 60) * 60) * 1000));
      const snapshots = [{ accountId: "default", configured: true, enabled: true, running: true, lastStartAt: oldDate }];
      const issues = collectTwitchStatusIssues(snapshots);
    ...
    it("should skip non-Twitch accounts gracefully", () => {
      const snapshots = [{ accountId: undefined, configured: false, enabled: true, running: false }];
      const issues = collectTwitchStatusIssues(snapshots);
