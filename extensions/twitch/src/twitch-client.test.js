...
vi.mock("@twurple/chat", () => { ChatClient: function() {
}, LogLevel: { CRITICAL: "CRITICAL", ERROR: "ERROR", WARNING: "WARNING", INFO: "INFO", DEBUG: "DEBUG", TRACE: "TRACE" } });
const mockAuthProvider = { constructor: vi.fn() };
vi.mock("@twurple/auth", () => { StaticAuthProvider: function() {
}, RefreshingAuthProvider: function() {
} });
vi.mock("./token.js", () => { resolveTwitchToken: vi.fn(() => { token: "oauth:mock-token-from-tests", source: "config" }), DEFAULT_ACCOUNT_ID: "default" });
describe("TwitchClientManager", () => {
  let manager;
  let mockLogger;
  const testAccount = { username: "testbot", token: "oauth:test123456", clientId: "test-client-id", channel: "testchannel", enabled: true };
  const testAccount2 = { username: "testbot2", token: "oauth:test789", clientId: "test-client-id-2", channel: "testchannel2", enabled: true };
  beforeEach(async () => {
    vi.clearAllMocks();
    messageHandlers.length = 0;
    const {resolveTwitchToken} = await import("./token.js");
    vi.mocked(resolveTwitchToken).mockReturnValue({ token: "oauth:mock-token-from-tests", source: "config" });
  ...
  describe("getClient", () => {
    ...
    it("should use account username as default channel when channel not specified", async () => {
      const accountWithoutChannel = { ...testAccount: , channel: undefined };
    ...
    it("should normalize token by removing oauth: prefix", async () => {
      const accountWithPrefix = { ...testAccount: , token: "oauth:actualtoken123" };
      const {resolveTwitchToken} = await import("./token.js");
      vi.mocked(resolveTwitchToken).mockReturnValue({ token: "oauth:actualtoken123", source: "config" });
    ...
    it("should throw error when clientId is missing", async () => {
      const accountWithoutClientId = { ...testAccount: , clientId: undefined };
    ...
    it("should throw error when token is missing", async () => {
      const {resolveTwitchToken} = await import("./token.js");
      vi.mocked(resolveTwitchToken).mockReturnValue({ token: "", source: "none" });
    ...
    it("should create separate clients for same account with different channels", async () => {
      const account1 = { ...testAccount: , channel: "channel1" };
      const account2 = { ...testAccount: , channel: "channel2" };
  ...
  describe("sendMessage", () => {
    ...
    it("should handle sending to account's default channel", async () => {
      const result = await manager.sendMessage(testAccount, (testAccount.channel || testAccount.username), "Test message");
      expect(result.ok).toBe(true);
  ...
  describe("message handling integration", () => {
    ...
    it("should handle incoming chat messages", async () => {
      ...
      onMessageCallback("#testchannel", "testuser", "Hello bot!", { userInfo: { userName: "testuser", displayName: "TestUser", userId: "12345", isMod: false, isBroadcaster: false, isVip: false, isSubscriber: false }, id: "msg123" });
    ...
    it("should normalize channel names without # prefix", async () => {
      await manager.getClient(testAccount);
      const onMessageCallback = messageHandlers[0];
      onMessageCallback("testchannel", "testuser", "Test", { userInfo: { userName: "testuser", displayName: "TestUser", userId: "123", isMod: false, isBroadcaster: false, isVip: false, isSubscriber: false }, id: "msg1" });
      expect(capturedMessage?.channel).toBe("testchannel");
    });
    it("should include user role flags in message", async () => {
      await manager.getClient(testAccount);
      const onMessageCallback = messageHandlers[0];
      onMessageCallback("#testchannel", "moduser", "Test", { userInfo: { userName: "moduser", displayName: "ModUser", userId: "456", isMod: true, isBroadcaster: false, isVip: true, isSubscriber: true }, id: "msg2" });
    ...
    it("should handle broadcaster messages", async () => {
      await manager.getClient(testAccount);
      const onMessageCallback = messageHandlers[0];
      onMessageCallback("#testchannel", "broadcaster", "Test", { userInfo: { userName: "broadcaster", displayName: "Broadcaster", userId: "789", isMod: false, isBroadcaster: true, isVip: false, isSubscriber: false }, id: "msg3" });
  ...
  describe("edge cases", () => {
    it("should handle multiple message handlers for different accounts", async () => {
      ...
      onMessage1("#testchannel", "user1", "msg1", { userInfo: { userName: "user1", displayName: "User1", userId: "1", isMod: false, isBroadcaster: false, isVip: false, isSubscriber: false }, id: "1" });
      const onMessage2 = messageHandlers[1];
      if (!onMessage2) {
        throw new Error("onMessage2 not found");
      }
      onMessage2("#testchannel2", "user2", "msg2", { userInfo: { userName: "user2", displayName: "User2", userId: "2", isMod: false, isBroadcaster: false, isVip: false, isSubscriber: false }, id: "2" });
