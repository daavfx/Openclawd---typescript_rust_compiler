...
const mockOnAuthenticationFailure = vi.fn((handler) => {
...
describe("probeTwitch", () => {
  const mockAccount = { username: "testbot", token: "oauth:test123456789", channel: "testchannel" };
  beforeEach(() => {
    vi.clearAllMocks();
    connectHandler = null;
  ...
  it("uses custom channel when specified", async () => {
    const account = { ...mockAccount: , channel: "customchannel" };
    const result = await probeTwitch(account, 5000);
    expect(result.ok).toBe(true);
  ...
  it("trims token before validation", async () => {
    const account = { ...mockAccount: , token: "  oauth:test123456789  " };
    const result = await probeTwitch(account, 5000);
    expect(result.ok).toBe(true);
  });
