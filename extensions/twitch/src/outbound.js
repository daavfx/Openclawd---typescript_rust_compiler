...
export const twitchOutbound = { deliveryMode: "direct", textChunkLimit: 500, chunker: chunkTextForTwitch, resolveTarget: ({to, allowFrom, mode}) => {
  ...
  return { ok: false, error: missingTargetError("Twitch", "<channel-name> or channels.twitch.accounts.<account>.allowFrom[0]") };
  ...
  if (!account) {
    ...
    throw new Error(("Twitch account not found: . " + "Available accounts: "));
  }
  ...
  return { channel: "twitch", messageId: result.messageId, timestamp: Date.now(), to: normalizeTwitchChannel(channel) };
}, sendMedia: async (params) => {
  const {text, mediaUrl, signal} = params;
  if (signal?.aborted) {
    throw new Error("Outbound delivery aborted");
  }
  const message = mediaUrl ? " ".trim() : text;
  if (!twitchOutbound.sendText) {
    throw new Error("sendText not implemented");
  }
  return twitchOutbound.sendText({ ...params: , text: message });
} }
