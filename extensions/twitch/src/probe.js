...
export async function probeTwitch(account, timeoutMs) {
  ...
  if ((!account.token || !account.username)) {
    return { ok: false, error: "missing credentials (token, username)", username: account.username, elapsedMs: (Date.now() - started) };
  }
  ...
  try {
    {
      ...
      const timeout = new Promise((_, reject) => {
        setTimeout(() => reject(new Error("timeout after ms")), timeoutMs);
      });
      client.connect();
    ...
    {
      return { ok: false, error: (error instanceof Error) ? error.message : String(error), username: account.username, channel: account.channel, elapsedMs: (Date.now() - started) };
    }
  }
  finally {
    {
