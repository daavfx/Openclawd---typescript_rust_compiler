...
describe("checkTwitchAccessControl", () => {
  const mockAccount = { username: "testbot", token: "oauth:test" };
  const mockMessage = { username: "testuser", userId: "123456", message: "hello bot", channel: "testchannel" };
  describe("when no restrictions are configured", () => {
    it("allows messages that mention the bot (default requireMention)", () => {
      const message = { ...mockMessage: , message: "@testbot hello" };
      const result = checkTwitchAccessControl({ message, account: mockAccount, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
  });
  describe("requireMention default", () => {
    it("defaults to true when undefined", () => {
      const message = { ...mockMessage: , message: "hello bot" };
      const result = checkTwitchAccessControl({ message, account: mockAccount, botUsername: "testbot" });
  ...
  describe("requireMention", () => {
    it("allows messages that mention the bot", () => {
      const account = { ...mockAccount: , requireMention: true };
      const message = { ...mockMessage: , message: "@testbot hello" };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
    it("blocks messages that don't mention the bot", () => {
      const account = { ...mockAccount: , requireMention: true };
      const result = checkTwitchAccessControl({ message: mockMessage, account, botUsername: "testbot" });
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain("does not mention the bot");
    });
  ...
  describe("allowFrom allowlist", () => {
    it("allows users in the allowlist", () => {
      const account = { ...mockAccount: , allowFrom: ["123456", "789012"] };
      const message = { ...mockMessage: , message: "@testbot hello" };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    ...
    it("allows users not in allowlist via fallback (open access)", () => {
      const account = { ...mockAccount: , allowFrom: ["789012"] };
      const message = { ...mockMessage: , message: "@testbot hello" };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
    it("blocks messages without userId", () => {
      const account = { ...mockAccount: , allowFrom: ["123456"] };
      const message = { ...mockMessage: , message: "@testbot hello", userId: undefined };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain("user ID not available");
    });
    it("bypasses role checks when user is in allowlist", () => {
      const account = { ...mockAccount: , allowFrom: ["123456"], allowedRoles: ["owner"] };
      const message = { ...mockMessage: , message: "@testbot hello", isOwner: false };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
    it("allows user with role even if not in allowlist", () => {
      const account = { ...mockAccount: , allowFrom: ["789012"], allowedRoles: ["moderator"] };
      const message = { ...mockMessage: , message: "@testbot hello", userId: "123456", isMod: true };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    ...
    it("blocks user with neither allowlist nor role", () => {
      const account = { ...mockAccount: , allowFrom: ["789012"], allowedRoles: ["moderator"] };
      const message = { ...mockMessage: , message: "@testbot hello", userId: "123456", isMod: false };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(false);
  ...
  describe("allowedRoles", () => {
    it("allows users with matching role", () => {
      const account = { ...mockAccount: , allowedRoles: ["moderator"] };
      const message = { ...mockMessage: , message: "@testbot hello", isMod: true };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
      expect(result.matchSource).toBe("role");
    });
    it("allows users with any of multiple roles", () => {
      const account = { ...mockAccount: , allowedRoles: ["moderator", "vip", "subscriber"] };
      const message = { ...mockMessage: , message: "@testbot hello", isVip: true, isMod: false, isSub: false };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
    it("blocks users without matching role", () => {
      const account = { ...mockAccount: , allowedRoles: ["moderator"] };
      const message = { ...mockMessage: , message: "@testbot hello", isMod: false };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain("does not have any of the required roles");
    });
    it("allows all users when role is 'all'", () => {
      const account = { ...mockAccount: , allowedRoles: ["all"] };
      const message = { ...mockMessage: , message: "@testbot hello" };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    ...
    it("handles subscriber role", () => {
      const account = { ...mockAccount: , allowedRoles: ["subscriber"] };
      const message = { ...mockMessage: , message: "@testbot hello", isSub: true };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
    it("handles owner role", () => {
      const account = { ...mockAccount: , allowedRoles: ["owner"] };
      const message = { ...mockMessage: , message: "@testbot hello", isOwner: true };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
    it("handles vip role", () => {
      const account = { ...mockAccount: , allowedRoles: ["vip"] };
      const message = { ...mockMessage: , message: "@testbot hello", isVip: true };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(true);
    });
  });
  describe("combined restrictions", () => {
    it("checks requireMention before allowlist", () => {
      const account = { ...mockAccount: , requireMention: true, allowFrom: ["123456"] };
      const message = { ...mockMessage: , message: "hello" };
      const result = checkTwitchAccessControl({ message, account, botUsername: "testbot" });
      expect(result.allowed).toBe(false);
      expect(result.reason).toContain("does not mention the bot");
    });
