...
export function createSeenTracker(options) {
  ...
  const pruneIntervalMs = (options?.pruneIntervalMs ?? ((10 * 60) * 1000));
  ...
  function add(id) {
    ...
    const newEntry = { seenAt: now, prev: null, next: head };
    if (head) {
      const headEntry = entries.get(head);
      if (headEntry) {
        headEntry.prev = id;
  ...
  return { has, add, peek, delete: deleteEntry, clear, size, stop, seed };
}
