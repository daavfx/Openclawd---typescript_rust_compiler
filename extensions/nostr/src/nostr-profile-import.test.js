...
describe("nostr-profile-import", () => {
  describe("mergeProfiles", () => {
    ...
    it("returns imported when local is undefined", () => {
      const imported = { name: "imported", displayName: "Imported User", about: "Bio from relay" };
      const result = mergeProfiles(undefined, imported);
      expect(result).toEqual(imported);
    });
    it("returns local when imported is undefined", () => {
      const local = { name: "local", displayName: "Local User" };
      const result = mergeProfiles(local, undefined);
      expect(result).toEqual(local);
    });
    it("prefers local values over imported", () => {
      const local = { name: "localname", about: "Local bio" };
      const imported = { name: "importedname", displayName: "Imported Display", about: "Imported bio", picture: "https://example.com/pic.jpg" };
    ...
    it("fills all missing fields from imported", () => {
      const local = { name: "myname" };
      const imported = { name: "theirname", displayName: "Their Name", about: "Their bio", picture: "https://example.com/pic.jpg", banner: "https://example.com/banner.jpg", website: "https://example.com", nip05: "user@example.com", lud16: "user@getalby.com" };
      const result = mergeProfiles(local, imported);
    ...
    it("handles empty strings as falsy (prefers imported)", () => {
      const local = { name: "", displayName: "" };
      const imported = { name: "imported", displayName: "Imported" };
      const result = mergeProfiles(local, imported);
      expect(result.name).toBe("");
      expect(result.displayName).toBe("");
    });
    it("handles null values in local (prefers imported)", () => {
      const local = { name: undefined, displayName: undefined };
      const imported = { name: "imported", displayName: "Imported" };
      const result = mergeProfiles(local, imported);
