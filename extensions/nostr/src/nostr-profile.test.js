...
import { createProfileEvent, profileToContent, contentToProfile, validateProfile, sanitizeProfileForDisplay } from "./nostr-profile.js";
...
describe("profileToContent", () => {
  it("converts full profile to NIP-01 content format", () => {
    const profile = { name: "testuser", displayName: "Test User", about: "A test user for unit testing", picture: "https://example.com/avatar.png", banner: "https://example.com/banner.png", website: "https://example.com", nip05: "testuser@example.com", lud16: "testuser@walletofsatoshi.com" };
  ...
  it("omits undefined fields from content", () => {
    const profile = { name: "minimaluser" };
    const content = profileToContent(profile);
...
describe("contentToProfile", () => {
  it("converts NIP-01 content to profile format", () => {
    const content = { name: "testuser", display_name: "Test User", about: "A test user", picture: "https://example.com/avatar.png", nip05: "test@example.com" };
  ...
  it("round-trips profile data", () => {
    const original = { name: "roundtrip", displayName: "Round Trip Test", about: "Testing round-trip conversion" };
...
describe("createProfileEvent", () => {
  ...
  it("creates a valid kind:0 event", () => {
    const profile = { name: "testbot", about: "A test bot" };
  ...
  it("includes profile content as JSON in event content", () => {
    const profile = { name: "jsontest", displayName: "JSON Test User", about: "Testing JSON serialization" };
    const event = createProfileEvent(TEST_SK, profile);
  ...
  it("ensures monotonic timestamp when lastPublishedAt is in the future", () => {
    const futureTimestamp = (1705320000 + 3600);
  ...
  it("uses current time when lastPublishedAt is in the past", () => {
    const pastTimestamp = (1705320000 - 3600);
...
describe("validateProfile", () => {
  it("validates a correct profile", () => {
    const profile = { name: "validuser", about: "A valid user", picture: "https://example.com/pic.png" };
  ...
  it("rejects profile with invalid URL", () => {
    const profile = { name: "invalidurl", picture: "http://insecure.example.com/pic.png" };
    const result = validateProfile(profile);
  ...
  it("rejects profile with javascript: URL", () => {
    const profile = { name: "xssattempt", picture: "javascript:alert('xss')" };
    const result = validateProfile(profile);
    expect(result.valid).toBe(false);
  });
  it("rejects profile with data: URL", () => {
    const profile = { name: "dataurl", picture: "data:image/png;base64,abc123" };
    const result = validateProfile(profile);
    expect(result.valid).toBe(false);
  });
  it("rejects name exceeding 256 characters", () => {
    const profile = { name: "a".repeat(257) };
    const result = validateProfile(profile);
    expect(result.valid).toBe(false);
    expect(result.errors.some((e) => e.includes("256"))).toBe(true);
  });
  it("rejects about exceeding 2000 characters", () => {
    const profile = { about: "a".repeat(2001) };
    const result = validateProfile(profile);
    expect(result.valid).toBe(false);
...
describe("sanitizeProfileForDisplay", () => {
  ...
  it("escapes HTML in about field", () => {
    const profile = { about: "Check out <img src=\"x\" onerror=\"alert(1)\">" };
    const sanitized = sanitizeProfileForDisplay(profile);
    expect(sanitized.about).toBe("Check out &lt;img src=&quot;x&quot; onerror=&quot;alert(1)&quot;&gt;");
  });
  it("preserves URLs without modification", () => {
    const profile = { picture: "https://example.com/pic.png", website: "https://example.com" };
    const sanitized = sanitizeProfileForDisplay(profile);
  ...
  it("escapes ampersands", () => {
    const profile = { name: "Tom & Jerry" };
    const sanitized = sanitizeProfileForDisplay(profile);
    expect(sanitized.name).toBe("Tom &amp; Jerry");
  });
  it("escapes quotes", () => {
    const profile = { about: "Say \"hello\" to everyone" };
    const sanitized = sanitizeProfileForDisplay(profile);
...
describe("edge cases", () => {
  it("handles emoji in profile fields", () => {
    const profile = { name: "🤖 Bot", about: "I am a 🤖 robot! 🎉" };
    const content = profileToContent(profile);
  ...
  it("handles unicode in profile fields", () => {
    const profile = { name: "日本語ユーザー", about: "Привет мир! 你好世界!" };
    const content = profileToContent(profile);
...
Line 3");
  ...
  it("handles maximum length fields", () => {
    const profile = { name: "a".repeat(256), about: "b".repeat(2000) };
    const result = validateProfile(profile);
    expect(result.valid).toBe(true);
    const event = createProfileEvent(TEST_SK, profile);
    expect(verifyEvent(event)).toBe(true);
