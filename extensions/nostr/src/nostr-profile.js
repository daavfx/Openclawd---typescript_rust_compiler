...
export function createProfileEvent(sk, profile, lastPublishedAt) {
  ...
  const event = finalizeEvent({ kind: 0, content: contentJson, tags: [], created_at: createdAt }, sk);
  return event;
}
...
export async function publishProfileEvent(pool, relays, event) {
  ...
  return { eventId: event.id, successes, failures, createdAt: event.created_at };
}
...
export function validateProfile(profile) {
  ...
  return { valid: false, errors: result.error.issues.map((e) => ": ") };
}
...
export function sanitizeProfileForDisplay(profile) {
  ...
  return { name: escapeHtml(profile.name), displayName: escapeHtml(profile.displayName), about: escapeHtml(profile.about), picture: profile.picture, banner: profile.banner, website: profile.website, nip05: escapeHtml(profile.nip05), lud16: escapeHtml(profile.lud16) };
}
