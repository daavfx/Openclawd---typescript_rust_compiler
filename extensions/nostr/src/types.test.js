...
describe("listNostrAccountIds", () => {
  ...
  it("returns default when privateKey is configured", () => {
    const cfg = { channels: { nostr: { privateKey: TEST_PRIVATE_KEY } } };
...
describe("resolveNostrAccount", () => {
  it("resolves configured account", () => {
    const cfg = { channels: { nostr: { privateKey: TEST_PRIVATE_KEY, name: "Test Bot", relays: ["wss://test.relay"], dmPolicy: "pairing" } } };
  ...
  it("handles disabled channel", () => {
    const cfg = { channels: { nostr: { enabled: false, privateKey: TEST_PRIVATE_KEY } } };
    const account = resolveNostrAccount({ cfg });
  ...
  it("handles allowFrom config", () => {
    const cfg = { channels: { nostr: { privateKey: TEST_PRIVATE_KEY, allowFrom: ["npub1test", "0123456789abcdef"] } } };
    const account = resolveNostrAccount({ cfg });
  ...
  it("preserves all config options", () => {
    const cfg = { channels: { nostr: { privateKey: TEST_PRIVATE_KEY, name: "Bot", enabled: true, relays: ["wss://relay1", "wss://relay2"], dmPolicy: "allowlist", allowFrom: ["pubkey1", "pubkey2"] } } };
    const account = resolveNostrAccount({ cfg });
    expect(account.config).toEqual({ privateKey: TEST_PRIVATE_KEY, name: "Bot", enabled: true, relays: ["wss://relay1", "wss://relay2"], dmPolicy: "allowlist", allowFrom: ["pubkey1", "pubkey2"] });
  });
});
