...
async function withTempStateDir(fn) {
  ...
  setNostrRuntime({ state: { resolveStateDir: (env, homedir) => {
...
describe("nostr bus state store", () => {
  it("persists and reloads state across restarts", async () => {
    await withTempStateDir(async () => {
      ...
      await writeNostrBusState({ accountId: "test-bot", lastProcessedAt: 1700000000, gatewayStartedAt: 1700000100 });
      const state = await readNostrBusState({ accountId: "test-bot" });
      expect(state).toEqual({ version: 2, lastProcessedAt: 1700000000, gatewayStartedAt: 1700000100, recentEventIds: [] });
    });
  });
  it("isolates state by accountId", async () => {
    await withTempStateDir(async () => {
      await writeNostrBusState({ accountId: "bot-a", lastProcessedAt: 1000, gatewayStartedAt: 1000 });
      await writeNostrBusState({ accountId: "bot-b", lastProcessedAt: 2000, gatewayStartedAt: 2000 });
...
describe("computeSinceTimestamp", () => {
  ...
  it("uses lastProcessedAt when available", () => {
    const state = { version: 2, lastProcessedAt: 1699999000, gatewayStartedAt: null, recentEventIds: [] };
    expect(computeSinceTimestamp(state, 1700000000)).toBe(1699999000);
  });
  it("uses gatewayStartedAt when lastProcessedAt is null", () => {
    const state = { version: 2, lastProcessedAt: null, gatewayStartedAt: 1699998000, recentEventIds: [] };
    expect(computeSinceTimestamp(state, 1700000000)).toBe(1699998000);
  });
  it("uses the max of both timestamps", () => {
    const state = { version: 2, lastProcessedAt: 1699999000, gatewayStartedAt: 1699998000, recentEventIds: [] };
    expect(computeSinceTimestamp(state, 1700000000)).toBe(1699999000);
  });
  it("falls back to now if both are null", () => {
    const state = { version: 2, lastProcessedAt: null, gatewayStartedAt: null, recentEventIds: [] };
