...
const ProfileUpdateSchema = NostrProfileSchema.extend({ nip05: nip05FormatSchema, lud16: lud16FormatSchema });
...
export function createNostrProfileHttpHandler(ctx) {
  return async (req, res) => {
    ...
    try {
      ...
      {
        ctx.log?.error("Profile HTTP error: ");
...
async function handleUpdateProfile(accountId, ctx, req, res) {
  ...
  if (profile.picture) {
    ...
    if (!pictureCheck.ok) {
      sendJson(res, 400, { ok: false, error: "picture: " });
      return true;
    }
  }
  if (profile.banner) {
    ...
    if (!bannerCheck.ok) {
      sendJson(res, 400, { ok: false, error: "banner: " });
      return true;
    }
  }
  if (profile.website) {
    ...
    if (!websiteCheck.ok) {
      sendJson(res, 400, { ok: false, error: "website: " });
      return true;
    }
  }
  ...
  try {
    {
      ...
      if ((result.successes.length > 0)) {
        ...
        ctx.log?.info("[] Profile published to  relay(s)");
      } else {
        ctx.log?.warn("[] Profile publish failed on all relays");
      }
      sendJson(res, 200, { ok: true, eventId: result.eventId, createdAt: result.createdAt, successes: result.successes, failures: result.failures, persisted: (result.successes.length > 0) });
    }
  }
  catch (err) {
    {
      ctx.log?.error("[] Profile publish error: ");
      sendJson(res, 500, { ok: false, error: "Publish failed: " });
    }
  }
  return true;
}
async function handleImportProfile(accountId, ctx, req, res) {
  ...
  ctx.log?.info("[] Importing profile for ...");
  const result = await importProfileFromRelays({ pubkey, relays, timeoutMs: 10000 });
  if (!result.ok) {
  ...
  if ((autoMerge && result.profile)) {
    ...
    ctx.log?.info("[] Profile imported and merged");
