...
const CODE_ASSIST_ENDPOINTS = ["https://cloudcode-pa.googleapis.com", "https://daily-cloudcode-pa.sandbox.googleapis.com"];
...
async function startCallbackServer(params) {
  ...
  return { waitForCallback: () => callbackPromise, close: () => new Promise((resolve) => {
    server.close(() => resolve());
  }) };
}
async function exchangeCode(params) {
  const response = await fetch(TOKEN_URL, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: CLIENT_ID, client_secret: CLIENT_SECRET, code: params.code, grant_type: "authorization_code", redirect_uri: REDIRECT_URI, code_verifier: params.verifier }) });
  if (!response.ok) {
...
async function fetchUserEmail(accessToken) {
  try {
    {
      const response = await fetch("https://www.googleapis.com/oauth2/v1/userinfo?alt=json", { headers: { Authorization: "Bearer " } });
      if (!response.ok) {
        return undefined;
      }
      const data = await response.json();
...
async function fetchProjectId(accessToken) {
  const headers = { Authorization: "Bearer ", "Content-Type": "application/json", "User-Agent": "google-api-nodejs-client/9.15.1", "X-Goog-Api-Client": "google-cloud-sdk vscode_cloudshelleditor/0.1", "Client-Metadata": JSON.stringify({ ideType: "IDE_UNSPECIFIED", platform: "PLATFORM_UNSPECIFIED", pluginType: "GEMINI" }) };
  for (const endpoint of CODE_ASSIST_ENDPOINTS) {
    try {
      {
        const response = await fetch("/v1internal:loadCodeAssist", { method: "POST", headers, body: JSON.stringify({ metadata: { ideType: "IDE_UNSPECIFIED", platform: "PLATFORM_UNSPECIFIED", pluginType: "GEMINI" } }) });
        if (!response.ok) {
          continue;
        }
        const data = await response.json();
...
async function loginAntigravity(params) {
  ...
  if (!callbackServer) {
    await params.note(["Open the URL in your local browser.", "After signing in, copy the full redirect URL and paste it back here.", "", "Auth URL: ", "Redirect URI: "].join("
...
const antigravityPlugin = { id: "google-antigravity-auth", name: "Google Antigravity Auth", description: "OAuth flow for Google Antigravity (Cloud Code Assist)", configSchema: emptyPluginConfigSchema(), register: function(api) {
  api.registerProvider({ id: "google-antigravity", label: "Google Antigravity", docsPath: "/providers/models", aliases: ["antigravity"], auth: [{ id: "oauth", label: "Google OAuth", hint: "PKCE + localhost callback", kind: "oauth", run: async (ctx) => {
    const spin = ctx.prompter.progress("Starting Antigravity OAuth…");
    try {
      {
        const result = await loginAntigravity({ isRemote: ctx.isRemote, openUrl: ctx.openUrl, prompt: async (message) => String(await ctx.prompter.text({ message })), note: ctx.prompter.note, log: (message) => ctx.runtime.log(message), progress: spin });
        const profileId = "google-antigravity:";
        return { profiles: [{ profileId, credential: { type: "oauth", provider: "google-antigravity", access: result.access, refresh: result.refresh, expires: result.expires, email: result.email, projectId: result.projectId } }], configPatch: { agents: { defaults: { models: { [DEFAULT_MODEL]: {  } } } } }, defaultModel: DEFAULT_MODEL, notes: ["Antigravity uses Google Cloud project quotas.", "Enable Gemini for Google Cloud on your project if requests fail."] };
      }
    }
    catch (err) {
      {
