...
export 
export 
export async function sendMessageZalouser(threadId, text, options = {  }) {
  const profile = ((options.profile || process.env.ZCA_PROFILE) || "default");
  if (!threadId?.trim()) {
    return { ok: false, error: "No threadId provided" };
  }
  if (options.mediaUrl) {
    return sendMediaZalouser(threadId, options.mediaUrl, { ...options: , caption: (text || options.caption) });
  }
...
export async function sendImageZalouser(threadId, imageUrl, options = {  }) {
  const profile = ((options.profile || process.env.ZCA_PROFILE) || "default");
...
export async function sendLinkZalouser(threadId, url, options = {  }) {
  const profile = ((options.profile || process.env.ZCA_PROFILE) || "default");
