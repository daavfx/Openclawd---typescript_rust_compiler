...
export const ZalouserToolSchema = Type.Object({ action: stringEnum(ACTIONS, { description: "Action to perform: " }), threadId: Type.Optional(Type.String({ description: "Thread ID for messaging" })), message: Type.Optional(Type.String({ description: "Message text" })), isGroup: Type.Optional(Type.Boolean({ description: "Is group chat" })), profile: Type.Optional(Type.String({ description: "Profile name" })), query: Type.Optional(Type.String({ description: "Search query" })), url: Type.Optional(Type.String({ description: "URL for media/link" })) }, { additionalProperties: false })
function json(payload) {
  return { content: [{ type: "text", text: JSON.stringify(payload, null, 2) }], details: payload };
}
export async function executeZalouserTool(_toolCallId, params) {
  try {
    {
      switch (params.action) {
        ...
        case "groups":
          {
            const result = await runZca(["group", "list", "-j"], { profile: params.profile });
            if (!result.ok) {
        ...
        case "me":
          {
            const result = await runZca(["me", "info", "-j"], { profile: params.profile });
            if (!result.ok) {
        ...
        case "status":
          {
            const result = await runZca(["auth", "status"], { profile: params.profile });
            return json({ authenticated: result.ok, output: (result.stdout || result.stderr) });
          }
        default:
          throw new Error("Unknown action: . Valid actions: send, image, link, friends, groups, me, status");
      }
    }
  }
  catch (err) {
