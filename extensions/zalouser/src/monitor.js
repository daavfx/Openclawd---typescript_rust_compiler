...
function isGroupAllowed(params) {
  ...
  const candidates = [params.groupId, "group:", (params.groupName ?? ""), normalizeGroupSlug((params.groupName ?? ""))].filter(Boolean);
...
function startZcaListener(runtime, profile, onMessage, onError, abortSignal) {
  let buffer = "";
  const {proc, promise} = runZcaStreaming(["listen", "-r", "-k"], { profile, onData: (chunk) => {
    buffer += chunk;
...
");
  ...
  proc.stderr?.on("data", (data) => {
    ...
    if (text) {
      runtime.error("[zalouser] zca stderr: ");
    }
  });
  void promise.then((result) => {
    if ((!result.ok && !abortSignal.aborted)) {
      onError(new Error((result.stderr || "zca listen exited with code ")));
    }
  });
...
async function processMessage(message, account, config, core, runtime, statusSink) {
  ...
  if (isGroup) {
    if ((groupPolicy === "disabled")) {
      logVerbose(core, runtime, "zalouser: drop group  (groupPolicy=disabled)");
      return;
    }
    if ((groupPolicy === "allowlist")) {
      const allowed = isGroupAllowed({ groupId: chatId, groupName, groups });
      if (!allowed) {
        logVerbose(core, runtime, "zalouser: drop group  (not allowlisted)");
        return;
      }
    }
  }
  ...
  if (!isGroup) {
    if ((dmPolicy === "disabled")) {
      logVerbose(core, runtime, "Blocked zalouser DM from  (dmPolicy=disabled)");
      return;
    }
    if ((dmPolicy !== "open")) {
      ...
      if (!allowed) {
        if ((dmPolicy === "pairing")) {
          const {code, created} = await core.channel.pairing.upsertPairingRequest({ channel: "zalouser", id: senderId, meta: { name: (senderName || undefined) } });
          if (created) {
            logVerbose(core, runtime, "zalouser pairing request sender=");
            try {
              {
                await sendMessageZalouser(chatId, core.channel.pairing.buildPairingReply({ channel: "zalouser", idLine: "Your Zalo user id: ", code }), { profile: account.profile });
                statusSink?.({ lastOutboundAt: Date.now() });
              }
            }
            catch (err) {
              {
                logVerbose(core, runtime, "zalouser pairing reply failed for : ");
              }
            }
          }
        } else {
          logVerbose(core, runtime, "Blocked unauthorized zalouser sender  (dmPolicy=)");
        }
        return;
      }
    }
  ...
  if (((isGroup && core.channel.commands.isControlCommandMessage(rawBody, config)) && (commandAuthorized !== true))) {
    logVerbose(core, runtime, "zalouser: drop control command from unauthorized sender ");
    return;
  }
  const peer = isGroup ? { kind: "group", id: chatId } : { kind: "group", id: senderId };
  const route = core.channel.routing.resolveAgentRoute({ cfg: config, channel: "zalouser", accountId: account.accountId, peer: { kind: peer.kind, id: peer.id } });
  ...
  const body = core.channel.reply.formatAgentEnvelope({ channel: "Zalo Personal", from: fromLabel, timestamp: timestamp ? (timestamp * 1000) : undefined, previousTimestamp, envelope: envelopeOptions, body: rawBody });
  const ctxPayload = core.channel.reply.finalizeInboundContext({ Body: body, RawBody: rawBody, CommandBody: rawBody, From: isGroup ? "zalouser:group:" : "zalouser:", To: "zalouser:", SessionKey: route.sessionKey, AccountId: route.accountId, ChatType: isGroup ? "group" : "direct", ConversationLabel: fromLabel, SenderName: (senderName || undefined), SenderId: senderId, CommandAuthorized: commandAuthorized, Provider: "zalouser", Surface: "zalouser", MessageSid: (message.msgId ?? ""), OriginatingChannel: "zalouser", OriginatingTo: "zalouser:" });
  await core.channel.session.recordInboundSession({ storePath, sessionKey: (ctxPayload.SessionKey ?? route.sessionKey), ctx: ctxPayload, onRecordError: (err) => {
    runtime.error?.("zalouser: failed updating session meta: ");
  } });
  await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({ ctx: ctxPayload, cfg: config, dispatcherOptions: { deliver: async (payload) => {
    await deliverZalouserReply({ payload: payload, profile: account.profile, chatId, isGroup, runtime, core, config, accountId: account.accountId, statusSink, tableMode: core.channel.text.resolveMarkdownTableMode({ cfg: config, channel: "zalouser", accountId: account.accountId }) });
  }, onError: (err, info) => {
    runtime.error("[] Zalouser  reply failed: ");
  } } });
}
async function deliverZalouserReply(params) {
  ...
  if ((mediaList.length > 0)) {
    ...
    for (const mediaUrl of mediaList) {
      ...
      try {
        {
          logVerbose(core, runtime, "Sending media to ");
          await sendMessageZalouser(chatId, (caption ?? ""), { profile, mediaUrl, isGroup });
          statusSink?.({ lastOutboundAt: Date.now() });
        }
      }
      catch (err) {
        {
          runtime.error("Zalouser media send failed: ");
        }
      }
    }
    return;
  ...
  if (text) {
    ...
    logVerbose(core, runtime, "Sending  text chunk(s) to ");
    for (const chunk of chunks) {
      try {
        ...
        {
          runtime.error("Zalouser message send failed: ");
        }
      }
    }
  }
}
export async function monitorZalouserProvider(options) {
  ...
  try {
    {
      ...
      if ((allowFromEntries.length > 0)) {
        ...
        if (result.ok) {
          ...
          runtime.log?.("zalouser user resolve failed; using config entries. ");
        }
      }
      ...
      if ((groupKeys.length > 0)) {
        ...
        if (result.ok) {
          ...
          runtime.log?.("zalouser group resolve failed; using config entries. ");
        }
      }
    }
  }
    ...
    {
      runtime.log?.("zalouser resolve failed; using config entries. ");
    }
  }
  const stop = () => {
    stopped = true;
  ...
  const startListener = () => {
    ...
    logVerbose(core, runtime, "[] starting zca listener (profile=)");
    proc = startZcaListener(runtime, account.profile, (msg) => {
      logVerbose(core, runtime, "[] inbound message");
      statusSink?.({ lastInboundAt: Date.now() });
      processMessage(msg, account, config, core, runtime, statusSink).catch((err) => {
        runtime.error("[] Failed to process message: ");
      });
    }, (err) => {
      runtime.error("[] zca listener error: ");
      if ((!stopped && !abortSignal.aborted)) {
        logVerbose(core, runtime, "[] restarting listener in 5s...");
