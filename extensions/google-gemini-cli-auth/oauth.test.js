...
vi.mock("node:fs", async (importOriginal) => {
  ...
  return { ...actual: , existsSync: (...args) => mockExistsSync(...args), readFileSync: (...args) => mockReadFileSync(...args), realpathSync: (...args) => mockRealpathSync(...args), readdirSync: (...args) => mockReaddirSync(...args) };
});
describe("extractGeminiCliCredentials", () => {
  ...
  it("extracts credentials from oauth2.js in known path", async () => {
    ...
    const fakeResolvedPath = join(rootDir, "fake", "lib", "node_modules", "@google", "gemini-cli", "dist", "index.js");
    const fakeOauth2Path = join(rootDir, "fake", "lib", "node_modules", "@google", "gemini-cli", "node_modules", "@google", "gemini-cli-core", "dist", "src", "code_assist", "oauth2.js");
    ...
    expect(result).toEqual({ clientId: FAKE_CLIENT_ID, clientSecret: FAKE_CLIENT_SECRET });
  });
  it("returns null when oauth2.js cannot be found", async () => {
    ...
    const fakeResolvedPath = join(rootDir, "fake", "lib", "node_modules", "@google", "gemini-cli", "dist", "index.js");
    process.env.PATH = fakeBinDir;
    mockExistsSync.mockImplementation((p) => (normalizePath(p) === normalizePath(fakeGeminiPath)));
    mockRealpathSync.mockReturnValue(fakeResolvedPath);
