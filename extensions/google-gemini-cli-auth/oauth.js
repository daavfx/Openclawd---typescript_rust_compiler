...
function buildAuthUrl(challenge, verifier) {
  ...
  const params = new URLSearchParams({ client_id: clientId, response_type: "code", redirect_uri: REDIRECT_URI, scope: SCOPES.join(" "), code_challenge: challenge, code_challenge_method: "S256", state: verifier, access_type: "offline", prompt: "consent" });
  return "?";
}
...
async function waitForLocalCallback(params) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, hostname, () => {
      params.onProgress?.("Waiting for OAuth callback on …");
    });
    timeout = setTimeout(() => {
...
async function exchangeCodeForTokens(code, verifier) {
  ...
  const body = new URLSearchParams({ client_id: clientId, code, grant_type: "authorization_code", redirect_uri: REDIRECT_URI, code_verifier: verifier });
  if (clientSecret) {
  ...
  return { refresh: data.refresh_token, access: data.access_token, expires: expiresAt, projectId, email };
}
async function getUserEmail(accessToken) {
  try {
    {
      const response = await fetch(USERINFO_URL, { headers: { Authorization: "Bearer " } });
      if (response.ok) {
        const data = await response.json();
        return data.email;
      }
...
async function discoverProject(accessToken) {
  ...
  const headers = { Authorization: "Bearer ", "Content-Type": "application/json", "User-Agent": "google-api-nodejs-client/9.15.1", "X-Goog-Api-Client": "gl-node/openclaw" };
  const loadBody = { cloudaicompanionProject: envProject, metadata: { ideType: "IDE_UNSPECIFIED", platform: "PLATFORM_UNSPECIFIED", pluginType: "GEMINI", duetProject: envProject } };
  let data = {  };
  try {
    {
      const response = await fetch("/v1internal:loadCodeAssist", { method: "POST", headers, body: JSON.stringify(loadBody) });
      if (!response.ok) {
  ...
  const onboardBody = { tierId, metadata: { ideType: "IDE_UNSPECIFIED", platform: "PLATFORM_UNSPECIFIED", pluginType: "GEMINI" } };
  if (((tierId !== TIER_FREE) && envProject)) {
    onboardBody.cloudaicompanionProject = envProject;
    onboardBody.metadata.duetProject = envProject;
  }
  const onboardResponse = await fetch("/v1internal:onboardUser", { method: "POST", headers, body: JSON.stringify(onboardBody) });
  if (!onboardResponse.ok) {
    throw new Error("onboardUser failed:  ");
  }
...
async function pollOperation(operationName, headers) {
  for (let attempt = 0; (attempt < 24); attempt += 1) {
    ...
    const response = await fetch("/v1internal/", { headers });
    if (!response.ok) {
      continue;
    }
    const data = await response.json();
...
export async function loginGeminiCliOAuth(ctx) {
  ...
  await ctx.note(needsManual ? ["You are running in a remote/VPS environment.", "A URL will be shown for you to open in your LOCAL browser.", "After signing in, copy the redirect URL and paste it back here."].join("
...
");
  ...
  try {
    {
      const {code} = await waitForLocalCallback({ expectedState: verifier, timeoutMs: ((5 * 60) * 1000), onProgress: (msg) => ctx.progress.update(msg) });
      ctx.progress.update("Exchanging authorization code for tokens...");
