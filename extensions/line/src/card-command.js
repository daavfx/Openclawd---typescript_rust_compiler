import { createActionCard, createImageCard, createInfoCard, createListCard, createReceiptCard } from "openclaw/plugin-sdk";
...
function parseActions(actionsStr) {
  ...
  for (const part of actionsStr.split(",")) {
    ...
    if ((actionData.startsWith("http://") || actionData.startsWith("https://"))) {
      results.push({ label, action: { type: "uri", label: label.slice(0, 20), uri: actionData } });
    } else {
      if (actionData.includes("=")) {
        results.push({ label, action: { type: "postback", label: label.slice(0, 20), data: actionData.slice(0, 300), displayText: label } });
      } else {
        results.push({ label, action: { type: "message", label: label.slice(0, 20), text: actionData } });
      }
    }
  }
  return results;
...
function parseReceiptItems(itemsStr) {
  return itemsStr.split(",").map((part) => {
    ...
    return { name: part.slice(0, colonIndex).trim(), value: part.slice((colonIndex + 1)).trim() };
  }).filter((item) => item.name);
}
function parseCardArgs(argsStr) {
  const result = { type: "", args: [], flags: {  } };
...
export function registerLineCardCommand(api) {
  api.registerCommand({ name: "card", description: "Send a rich card message (LINE).", acceptsArgs: true, requireAuth: false, handler: async (ctx) => {
          ...
          case "info":
            {
              ...
              return buildLineReply({ flexMessage: { altText: ": ".slice(0, 400), contents: bubble } });
            }
          case "image":
            {
          ...
          case "list":
            {
              ...
              if ((items.length === 0)) {
                return { text: "Error: List card requires items. Usage: /card list \"Title\" \"Item1|Desc1,Item2|Desc2\"" };
              }
          ...
          case "receipt":
            {
              ...
              if ((items.length === 0)) {
                return { text: "Error: Receipt card requires items. Usage: /card receipt \"Title\" \"Item1:$10,Item2:$20\" --total \"$30\"" };
              }
          ...
          case "confirm":
            {
              ...
              return buildLineReply({ templateMessage: { type: "confirm", text: question, confirmLabel: (yesLabel || "Yes"), confirmData: (yesData || "yes"), cancelLabel: (noLabel || "No"), cancelData: (noData || "no"), altText: question } });
            }
          case "buttons":
            {
              ...
              const templateActions = actionParts.map((a) => {
                ...
                if ((action.type === "postback")) {
                  return { type: "postback", label, data: action.data };
                }
                return { type: "message", label, data: action.text };
              });
              return buildLineReply({ templateMessage: { type: "buttons", title, text, thumbnailImageUrl: (flags.url || flags.image), actions: templateActions } });
            }
          default:
            return { text: "Unknown card type: \"\". Available types: info, image, action, list, receipt, confirm, buttons" };
        }
      }
    }
    catch (err) {
      {
        return { text: "Error creating card: " };
      }
    }
  } });
}
