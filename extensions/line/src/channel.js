import { buildChannelConfigSchema, DEFAULT_ACCOUNT_ID, LineConfigSchema, processLineMessage } from "openclaw/plugin-sdk";
import { getLineRuntime } from "./runtime.js";
const meta = { id: "line", label: "LINE", selectionLabel: "LINE (Messaging API)", detailLabel: "LINE Bot", docsPath: "/channels/line", docsLabel: "line", blurb: "LINE Messaging API bot for Japan/Taiwan/Thailand markets.", systemImage: "message.fill" };
...
export const linePlugin = { id: "line", meta: { ...meta: , quickstartAllowFrom: true }, pairing: { idLabel: "lineUserId", normalizeAllowEntry: (entry) => {
...
} }, capabilities: { chatTypes: ["direct", "group"], reactions: false, threads: false, media: true, nativeCommands: false, blockStreaming: true }, reload: { configPrefixes: ["channels.line"] }, configSchema: buildChannelConfigSchema(LineConfigSchema), config: { listAccountIds: (cfg) => getLineRuntime().channel.line.listLineAccountIds(cfg), resolveAccount: (cfg, accountId) => getLineRuntime().channel.line.resolveLineAccount({ cfg, accountId }), defaultAccountId: (cfg) => getLineRuntime().channel.line.resolveDefaultLineAccountId(cfg), setAccountEnabled: ({cfg, accountId, enabled}) => {
  const lineConfig = (cfg.channels?.line ?? {  });
  if ((accountId === DEFAULT_ACCOUNT_ID)) {
    return { ...cfg: , channels: { ...cfg.channels: , line: { ...lineConfig: , enabled } } };
  }
  return { ...cfg: , channels: { ...cfg.channels: , line: { ...lineConfig: , accounts: { ...lineConfig.accounts: , [accountId]: { ...lineConfig.accounts?.[accountId]: , enabled } } } } };
}, deleteAccount: ({cfg, accountId}) => {
  const lineConfig = (cfg.channels?.line ?? {  });
  if ((accountId === DEFAULT_ACCOUNT_ID)) {
    const {channelAccessToken, channelSecret, tokenFile, secretFile, ...rest} = lineConfig;
    return { ...cfg: , channels: { ...cfg.channels: , line: rest } };
  }
  const accounts = { ...lineConfig.accounts:  };
  delete accounts[accountId];
  return { ...cfg: , channels: { ...cfg.channels: , line: { ...lineConfig: , accounts: (Object.keys(accounts).length > 0) ? accounts : undefined } } };
  ...
  const basePath = useAccountPath ? "channels.line.accounts.." : "channels.line.";
  return { policy: (account.config.dmPolicy ?? "pairing"), allowFrom: (account.config.allowFrom ?? []), policyPath: "dmPolicy", allowFromPath: basePath, approveHint: "openclaw pairing approve line <code>", normalizeEntry: (raw) => raw.replace(/^line:(?:user:)?/i, "") };
  ...
  return ["- LINE groups: groupPolicy=\"open\" allows any member in groups to trigger. Set channels.line.groupPolicy=\"allowlist\" + channels.line.groupAllowFrom to restrict senders."];
...
}, targetResolver: { looksLikeId: (id) => {
  const trimmed = id?.trim();
  if (!trimmed) {
    return false;
  }
  return (/^[UCR][a-f0-9]{32}$/i.test(trimmed) || /^line:/i.test(trimmed));
}, hint: "<userId|groupId|roomId>" } }, directory: { self: async () => null, listPeers: async () => [], listGroups: async () => [] }, setup: { resolveAccountId: ({accountId}) => getLineRuntime().channel.line.normalizeAccountId(accountId), applyAccountName: ({cfg, accountId, name}) => {
  const lineConfig = (cfg.channels?.line ?? {  });
  if ((accountId === DEFAULT_ACCOUNT_ID)) {
    return { ...cfg: , channels: { ...cfg.channels: , line: { ...lineConfig: , name } } };
  }
  return { ...cfg: , channels: { ...cfg.channels: , line: { ...lineConfig: , accounts: { ...lineConfig.accounts: , [accountId]: { ...lineConfig.accounts?.[accountId]: , name } } } } };
  ...
  const sendMessageBatch = async (messages) => {
    ...
    for (let i = 0; (i < messages.length); i += 5) {
      const result = await sendBatch(to, messages.slice(i, (i + 5)), { verbose: false, accountId: (accountId ?? undefined) });
  ...
  const chunkLimit = (runtime.channel.text.resolveTextChunkLimit?.(cfg, "line", (accountId ?? undefined), { fallbackLimit: 5000 }) ?? 5000);
  ...
  if (!shouldSendQuickRepliesInline) {
    if (lineData.flexMessage) {
      lastResult = await sendFlex(to, lineData.flexMessage.altText, lineData.flexMessage.contents, { verbose: false, accountId: (accountId ?? undefined) });
    }
    if (lineData.templateMessage) {
      const template = buildTemplate(lineData.templateMessage);
      if (template) {
        lastResult = await sendTemplate(to, template, { verbose: false, accountId: (accountId ?? undefined) });
      }
    }
    if (lineData.location) {
      lastResult = await sendLocation(to, lineData.location, { verbose: false, accountId: (accountId ?? undefined) });
    }
    for (const flexMsg of processed.flexMessages) {
      lastResult = await sendFlex(to, flexMsg.altText, flexMsg.contents, { verbose: false, accountId: (accountId ?? undefined) });
    }
  }
  const sendMediaAfterText = !(hasQuickReplies && (chunks.length > 0));
  if ((((mediaUrls.length > 0) && !shouldSendQuickRepliesInline) && !sendMediaAfterText)) {
    for (const url of mediaUrls) {
      lastResult = await runtime.channel.line.sendMessageLine(to, "", { verbose: false, mediaUrl: url, accountId: (accountId ?? undefined) });
    }
  }
  if ((chunks.length > 0)) {
    for (let i = 0; (i < chunks.length); i += 1) {
      ...
      if ((isLast && hasQuickReplies)) {
        lastResult = await sendQuickReplies(to, chunks[i], lineData.quickReplies, { verbose: false, accountId: (accountId ?? undefined) });
      } else {
        lastResult = await sendText(to, chunks[i], { verbose: false, accountId: (accountId ?? undefined) });
      }
    }
  } else {
    if (shouldSendQuickRepliesInline) {
      ...
      if (lineData.flexMessage) {
        quickReplyMessages.push({ type: "flex", altText: lineData.flexMessage.altText.slice(0, 400), contents: lineData.flexMessage.contents });
      }
      if (lineData.templateMessage) {
        const template = buildTemplate(lineData.templateMessage);
        if (template) {
      ...
      if (lineData.location) {
        quickReplyMessages.push({ type: "location", title: lineData.location.title.slice(0, 100), address: lineData.location.address.slice(0, 100), latitude: lineData.location.latitude, longitude: lineData.location.longitude });
      }
      for (const flexMsg of processed.flexMessages) {
        quickReplyMessages.push({ type: "flex", altText: flexMsg.altText.slice(0, 400), contents: flexMsg.contents });
      }
      for (const url of mediaUrls) {
        ...
        quickReplyMessages.push({ type: "image", originalContentUrl: trimmed, previewImageUrl: trimmed });
      }
      if (((quickReplyMessages.length > 0) && quickReply)) {
        const lastIndex = (quickReplyMessages.length - 1);
        quickReplyMessages[lastIndex] = { ...quickReplyMessages[lastIndex]: , quickReply };
  ...
  if (processed.text.trim()) {
    result = await sendText(to, processed.text, { verbose: false, accountId: (accountId ?? undefined) });
  } else {
    result = { messageId: "processed", chatId: to };
  }
  for (const flexMsg of processed.flexMessages) {
    await sendFlex(to, flexMsg.altText, flexMsg.contents, { verbose: false, accountId: (accountId ?? undefined) });
  }
  return { channel: "line", ...result:  };
}, sendMedia: async ({to, text, mediaUrl, accountId}) => {
  const send = getLineRuntime().channel.line.sendMessageLine;
  const result = await send(to, text, { verbose: false, mediaUrl, accountId: (accountId ?? undefined) });
  return { channel: "line", ...result:  };
} }, status: { defaultRuntime: { accountId: DEFAULT_ACCOUNT_ID, running: false, lastStartAt: null, lastStopAt: null, lastError: null }, collectStatusIssues: ({account}) => {
  const issues = [];
  if (!account.channelAccessToken?.trim()) {
    issues.push({ level: "error", message: "LINE channel access token not configured" });
  }
  if (!account.channelSecret?.trim()) {
    issues.push({ level: "error", message: "LINE channel secret not configured" });
  }
  return issues;
}, buildChannelSummary: ({snapshot}) => { configured: (snapshot.configured ?? false), tokenSource: (snapshot.tokenSource ?? "none"), running: (snapshot.running ?? false), mode: (snapshot.mode ?? null), lastStartAt: (snapshot.lastStartAt ?? null), lastStopAt: (snapshot.lastStopAt ?? null), lastError: (snapshot.lastError ?? null), probe: snapshot.probe, lastProbeAt: (snapshot.lastProbeAt ?? null) }, probeAccount: async ({account, timeoutMs}) => getLineRuntime().channel.line.probeLineBot(account.channelAccessToken, timeoutMs), buildAccountSnapshot: ({account, runtime, probe}) => {
  ...
  try {
  ...
  catch (err) {
    {
      if (getLineRuntime().logging.shouldLogVerbose()) {
        ctx.log?.debug?.("[] bot probe failed: ");
      }
    }
  }
  ctx.log?.info("[] starting LINE provider");
  return getLineRuntime().channel.line.monitorLineProvider({ channelAccessToken: token, channelSecret: secret, accountId: account.accountId, config: ctx.cfg, runtime: ctx.runtime, abortSignal: ctx.abortSignal, webhookPath: account.config.webhookPath });
