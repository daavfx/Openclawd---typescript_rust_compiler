...
function createRuntime() {
  ...
  const runtime = { config: { writeConfigFile }, channel: { line: { resolveLineAccount } } };
...
describe("linePlugin gateway.logoutAccount", () => {
  ...
  it("clears tokenFile/secretFile on default account logout", async () => {
    ...
    const cfg = { channels: { line: { tokenFile: "/tmp/token", secretFile: "/tmp/secret" } } };
    const result = await linePlugin.gateway.logoutAccount({ accountId: DEFAULT_ACCOUNT_ID, cfg });
  ...
  it("clears tokenFile/secretFile on account logout", async () => {
    const {runtime, mocks} = createRuntime();
    setLineRuntime(runtime);
    const cfg = { channels: { line: { accounts: { primary: { tokenFile: "/tmp/token", secretFile: "/tmp/secret" } } } } };
    const result = await linePlugin.gateway.logoutAccount({ accountId: "primary", cfg });
    expect(result.cleared).toBe(true);
    expect(result.loggedOut).toBe(true);
    expect(mocks.writeConfigFile).toHaveBeenCalledWith({  });
  });
