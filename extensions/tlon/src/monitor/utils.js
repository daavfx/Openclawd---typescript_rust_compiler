...
export function formatModelName(modelString) {
  ...
  const modelMappings = { "claude-opus-4-5": "Claude Opus 4.5", "claude-sonnet-4-5": "Claude Sonnet 4.5", "claude-sonnet-3-5": "Claude Sonnet 3.5", "gpt-4o": "GPT-4o", "gpt-4-turbo": "GPT-4 Turbo", "gpt-4": "GPT-4", "gemini-2.0-flash": "Gemini 2.0 Flash", "gemini-pro": "Gemini Pro" };
...
export function isBotMentioned(messageText, botShipName) {
  if ((!messageText || !botShipName)) {
    return false;
  }
  const normalizedBotShip = normalizeShip(botShipName);
  const escapedShip = normalizedBotShip.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const mentionPattern = new RegExp("(^|\\s)(?=\\s|$)", "i");
...
export function isSummarizationRequest(messageText) {
  const patterns = [/summarize\s+(this\s+)?(channel|chat|conversation)/i, /what\s+did\s+i\s+miss/i, /catch\s+me\s+up/i, /channel\s+summary/i, /tldr/i];
  return patterns.some((pattern) => pattern.test(messageText));
}

export function formatChangesDate(daysAgo = 5) {
  const now = new Date();
  const targetDate = new Date((now.getTime() - ((((daysAgo * 24) * 60) * 60) * 1000)));
  const year = targetDate.getFullYear();
  const month = (targetDate.getMonth() + 1);
  const day = targetDate.getDate();
  return "~....20.19.51..9b9d";
}
