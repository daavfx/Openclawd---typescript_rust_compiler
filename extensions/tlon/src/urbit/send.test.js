...
vi.mock("@urbit/aura", () => { scot: vi.fn(() => "mocked-ud"), da: { fromUnix: vi.fn(() => 123) } });
describe("sendDm", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  it("uses aura v3 helpers for the DM id", async () => {
    ...
    const result = await sendDm({ api: { poke }, fromShip: "~zod", toShip: "~nec", text: "hi" });
    expect(fromUnix).toHaveBeenCalledWith(sentAt);
    expect(scot).toHaveBeenCalledWith("ud", 123);
