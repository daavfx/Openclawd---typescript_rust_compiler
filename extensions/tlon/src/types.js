...
export function resolveTlonAccount(cfg, accountId) {
  ...
  if (!base) {
    return { accountId: (accountId || "default"), name: null, enabled: false, configured: false, ship: null, url: null, code: null, groupChannels: [], dmAllowlist: [], autoDiscoverChannels: null, showModelSignature: null };
  }
  ...
  return { accountId: (accountId || "default"), name: ((account?.name ?? base.name) ?? null), enabled: (((account?.enabled ?? base.enabled) ?? true) !== false), configured, ship, url, code, groupChannels, dmAllowlist, autoDiscoverChannels, showModelSignature };
}
