...
export function parseChannelNest(raw) {
...
export function parseTlonTarget(raw) {
  const trimmed = raw?.trim();
  if (!trimmed) {
    return null;
  }
  ...
  if (groupPrefix) {
    ...
    if (groupTarget.startsWith("chat/")) {
      ...
      return { kind: "group", nest: "chat//", hostShip: parsed.hostShip, channelName: parsed.channelName };
    }
    const parts = groupTarget.split("/");
    if ((parts.length === 2)) {
      const hostShip = normalizeShip(parts[0]);
      const channelName = parts[1];
      return { kind: "group", nest: "chat//", hostShip, channelName };
    }
    return null;
  }
