...
export function withWhatsAppPrefix(number) {
  return number.startsWith("whatsapp:") ? number : "whatsapp:";
}
...
function readLidReverseMapping(lid, opts) {
  const mappingFilename = "lid-mapping-_reverse.json";
...
export function jidToE164(jid, opts) {
  ...
  if (lidMatch) {
    ...
    if (shouldLog) {
      logVerbose("LID mapping not found for ; skipping inbound message");
    }
  }
  return null;
}
...
export async function resolveJidToE164(jid, opts) {
  ...
  try {
    ...
    {
      if (shouldLogVerbose()) {
        logVerbose("LID mapping lookup failed for : ");
      }
      return null;
    }
  }
