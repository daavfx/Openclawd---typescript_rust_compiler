...
describe("canvas host", () => {
  ...
  it("creates a default index.html when missing", async () => {
    ...
    const server = await startCanvasHost({ runtime: defaultRuntime, rootDir: dir, port: 0, listenHost: "127.0.0.1", allowInTests: true });
    try {
      {
  ...
  it("skips live reload injection when disabled", async () => {
    ...
    const server = await startCanvasHost({ runtime: defaultRuntime, rootDir: dir, port: 0, listenHost: "127.0.0.1", allowInTests: true, liveReload: false });
    try {
      {
        const res = await fetch("http://127.0.0.1:/");
        const html = await res.text();
        ...
        const wsRes = await fetch("http://127.0.0.1:");
  ...
  it("serves canvas content from the mounted base path", async () => {
    ...
    const handler = await createCanvasHostHandler({ runtime: defaultRuntime, rootDir: dir, basePath: CANVAS_HOST_PATH, allowInTests: true });
    ...
    try {
      {
        ...
        expect(html).toContain(CANVAS_WS_PATH);
        const miss = await fetch("http://127.0.0.1:/");
  ...
  it("reuses a handler without closing it twice", async () => {
    ...
    const server = await startCanvasHost({ runtime: defaultRuntime, handler, ownsHandler: false, port: 0, listenHost: "127.0.0.1", allowInTests: true });
    try {
      {
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      {
        const res = await fetch("http://127.0.0.1:/__openclaw__/a2ui/");
        const html = await res.text();
        expect(res.status).toBe(200);
        expect(html).toContain("openclaw-a2ui-host");
        expect(html).toContain("openclawCanvasA2UIAction");
        const bundleRes = await fetch("http://127.0.0.1:/__openclaw__/a2ui/a2ui.bundle.js");
