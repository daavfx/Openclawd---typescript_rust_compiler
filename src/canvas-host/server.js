...
export async function createCanvasHostHandler(opts) {
  ...
  if ((isDisabledByEnv() && (opts.allowInTests !== true))) {
    return { rootDir: "", basePath, handleHttpRequest: async () => false, handleUpgrade: () => false, close: async () => {
    } };
  }
  ...
  const watcher = liveReload ? chokidar.watch(rootReal, { ignoreInitial: true, awaitWriteFinish: { stabilityThreshold: 75, pollInterval: 10 }, usePolling: (opts.allowInTests === true), ignored: [/(^|[\\/])\../, /(^|[\\/])node_modules([\\/]|$)/] }) : null;
  ...
  watcher?.on("error", (err) => {
    ...
    watcherClosed = true;
    opts.runtime.error("canvasHost watcher error:  (live reload disabled; consider canvasHost.liveReload=false or a smaller canvasHost.root)");
    ...
    try {
      {
        ...
        if ((basePath !== "/")) {
          if (((urlPath !== basePath) && !urlPath.startsWith("/"))) {
            return false;
          }
...
Create /index.html</pre>");
      ...
      {
        opts.runtime.error("canvasHost request failed: ");
        res.statusCode = 500;
        res.setHeader("Content-Type", "text/plain; charset=utf-8");
        res.end("error");
        return true;
  ...
  return { rootDir, basePath, handleHttpRequest, handleUpgrade, close: async () => {
    if (debounce) {
      clearTimeout(debounce);
    }
    watcherClosed = true;
...
export async function startCanvasHost(opts) {
  ...
  const handler = (opts.handler ?? await createCanvasHostHandler({ runtime: opts.runtime, rootDir: opts.rootDir, basePath: CANVAS_HOST_PATH, allowInTests: opts.allowInTests, liveReload: opts.liveReload }));
  ...
  opts.runtime.log("canvas host listening on http://: (root )");
  return { port: boundPort, rootDir: handler.rootDir, close: async () => {
    if (ownsHandler) {
      await handler.close();
    }
