...
async function resolveA2uiRoot() {
  ...
  const candidates = [path.resolve(here, "a2ui"), path.resolve(here, "../../src/canvas-host/a2ui"), path.resolve(process.cwd(), "src/canvas-host/a2ui"), path.resolve(process.cwd(), "dist/canvas-host/a2ui")];
  if (process.execPath) {
...
async function resolveA2uiFilePath(rootReal, urlPath) {
  ...
  const rootPrefix = rootReal.endsWith(path.sep) ? rootReal : "";
  try {
    {
...
export async function handleA2uiHttpRequest(req, res) {
  ...
  const basePath = ((url.pathname === A2UI_PATH) || url.pathname.startsWith("/")) ? A2UI_PATH : undefined;
  if (!basePath) {
    return false;
  }
