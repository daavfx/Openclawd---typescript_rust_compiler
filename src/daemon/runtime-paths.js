...
const VERSION_MANAGER_MARKERS = ["/.nvm/", "/.fnm/", "/.volta/", "/.asdf/", "/.n/", "/.nodenv/", "/.nodebrew/", "/nvs/"];
...
export async function resolveSystemNodeInfo(params) {
  ...
  return { path: systemNode, version, supported: isSupportedNodeVersion(version) };
}
...
export function renderSystemNodeWarning(systemNode, selectedNodePath) {
  ...
  const selectedLabel = selectedNodePath ? " Using  for the daemon." : "";
  return "System Node  at  is below the required Node 22+. Install Node 22+ from nodejs.org or Homebrew.";
}
