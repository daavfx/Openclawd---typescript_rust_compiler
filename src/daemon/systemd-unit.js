...
function renderEnvLines(env) {
  ...
  return entries.map(([key, value]) => "Environment=");
}
export function buildSystemdUnit({description, programArguments, workingDirectory, environment}) {
  const execStart = programArguments.map(systemdEscapeArg).join(" ");
  const descriptionLine = "Description=";
  const workingDirLine = workingDirectory ? "WorkingDirectory=" : null;
  const envLines = renderEnvLines(environment);
  return ["[Unit]", descriptionLine, "After=network-online.target", "Wants=network-online.target", "", "[Service]", "ExecStart=", "Restart=always", "RestartSec=5", "KillMode=process", workingDirLine, ...envLines, "", "[Install]", "WantedBy=default.target", ""].filter((line) => (line !== null)).join("
");
}
