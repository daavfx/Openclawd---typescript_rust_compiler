  ...
  try {
    {
      ...
      return { programArguments: parseCommandLine(commandLine), ...workingDirectory ? { workingDirectory } : {  }: , ...(Object.keys(environment).length > 0) ? { environment } : {  }:  };
    }
  }
  catch {
    {
...
async function assertSchtasksAvailable() {
  ...
  throw new Error("schtasks unavailable: ".trim());
}
export async function installScheduledTask({env, stdout, programArguments, workingDirectory, environment, description}) {
  ...
  const taskDescription = (description ?? formatGatewayServiceDescription({ profile: env.OPENCLAW_PROFILE, version: (environment?.OPENCLAW_SERVICE_VERSION ?? env.OPENCLAW_SERVICE_VERSION) }));
  const script = buildTaskScript({ description: taskDescription, programArguments, workingDirectory, environment });
  ...
  if ((create.code !== 0)) {
    ...
    throw new Error("schtasks create failed: ".trim());
  }
...
");
    ...
    {
      stdout.write("Task script not found at 
");
    }
  }
}
...
export async function stopScheduledTask({stdout, env}) {
  ...
  if (((res.code !== 0) && !isTaskNotRunning(res))) {
    throw new Error("schtasks end failed: ".trim());
  }
  stdout.write("
");
}
...
export async function restartScheduledTask({stdout, env}) {
  ...
  if ((res.code !== 0)) {
    throw new Error("schtasks run failed: ".trim());
  }
  stdout.write("
");
}
...
export async function readScheduledTaskRuntime(env = process.env) {
  ...
  return { status, state: parsed.status, lastRunTime: parsed.lastRunTime, lastRunResult: parsed.lastRunResult };
}
