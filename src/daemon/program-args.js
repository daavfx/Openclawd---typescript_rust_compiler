...
async function resolveCliEntrypointPathForService() {
  ...
  throw new Error("Cannot find built CLI at . Run \"pnpm build\" first, or use dev mode.");
}
...
function appendDistCandidates(candidates, seen, baseDir) {
  ...
  const distEntries = [path.join(distDir, "index.js"), path.join(distDir, "index.mjs"), path.join(distDir, "entry.js"), path.join(distDir, "entry.mjs")];
...
async function resolveBinaryPath(binary) {
  ...
  try {
    {
      const output = execSync(" ", { encoding: "utf8" }).trim();
...
async function resolveCliProgramArguments(params) {
  ...
  if ((runtime === "node")) {
    ...
    return { programArguments: [nodePath, cliEntrypointPath, ...params.args] };
  }
  if ((runtime === "bun")) {
    if (params.dev) {
      ...
      return { programArguments: [bunPath, devCliPath, ...params.args], workingDirectory: repoRoot };
    }
    const bunPath = isBunRuntime(execPath) ? execPath : await resolveBunPath();
    const cliEntrypointPath = await resolveCliEntrypointPathForService();
    return { programArguments: [bunPath, cliEntrypointPath, ...params.args] };
  }
  if (!params.dev) {
    try {
      {
        const cliEntrypointPath = await resolveCliEntrypointPathForService();
        return { programArguments: [execPath, cliEntrypointPath, ...params.args] };
      }
    }
    catch (error) {
      {
  ...
  if (isBunRuntime(execPath)) {
    return { programArguments: [execPath, devCliPath, ...params.args], workingDirectory: repoRoot };
  }
...
export async function resolveGatewayProgramArguments(params) {
  ...
  return resolveCliProgramArguments({ args: gatewayArgs, dev: params.dev, runtime: params.runtime, nodePath: params.nodePath });
}
...
export async function resolveNodeProgramArguments(params) {
  ...
  return resolveCliProgramArguments({ args, dev: params.dev, runtime: params.runtime, nodePath: params.nodePath });
}
