...
export async function readSystemdUserLingerStatus(env) {
  ...
  try {
    {
      const {stdout} = await runExec("loginctl", ["show-user", user, "-p", "Linger"], { timeoutMs: 5000 });
...
export async function enableSystemdUserLinger(params) {
  ...
  const sudoArgs = (needsSudo && (params.sudoMode !== undefined)) ? ["sudo", ...(params.sudoMode === "non-interactive") ? ["-n"] : []] : [];
  const argv = [...sudoArgs, "loginctl", "enable-linger", user];
  try {
    {
      const result = await runCommandWithTimeout(argv, { timeoutMs: 30000 });
      return { ok: (result.code === 0), stdout: result.stdout, stderr: result.stderr, code: (result.code ?? 1) };
    }
  }
  catch (error) {
    {
