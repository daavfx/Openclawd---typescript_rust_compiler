    ...
    case "darwin":
      {
        ...
        return ["launchctl bootout gui/$UID/", "rm ~/Library/LaunchAgents/.plist"];
      }
    case "linux":
      {
        const unit = resolveGatewaySystemdServiceName(profile);
        return ["systemctl --user disable --now .service", "rm ~/.config/systemd/user/.service"];
      }
    case "win32":
      {
        const task = resolveGatewayWindowsTaskName(profile);
        return ["schtasks /Delete /TN \"\" /F"];
      }
    default:
      return [];
  }
...
async function scanLaunchdDir(params) {
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({ platform: "darwin", label, detail: "plist: ", scope: params.scope, marker: isLegacyLabel(label) ? "clawdbot" : "moltbot", legacy: true });
      continue;
    }
    ...
    results.push({ platform: "darwin", label, detail: "plist: ", scope: params.scope, marker, legacy: ((marker !== "openclaw") || isLegacyLabel(label)) });
  }
  return results;
}
async function scanSystemdDir(params) {
  ...
  for (const entry of entries) {
    ...
    results.push({ platform: "linux", label: entry, detail: "unit: ", scope: params.scope, marker, legacy: (marker !== "openclaw") });
  }
  return results;
}
...
async function execSchtasks(args) {
  try {
    {
      const {stdout, stderr} = await execFileAsync("schtasks", args, { encoding: "utf8", windowsHide: true });
...
export async function findExtraGatewayServices(env, opts = {  }) {
    ...
    try {
      {
        ...
        for (const svc of await scanLaunchdDir({ dir: userDir, scope: "user" })) {
          push(svc);
        }
        if (opts.deep) {
          for (const svc of await scanLaunchdDir({ dir: path.join(path.sep, "Library", "LaunchAgents"), scope: "system" })) {
            push(svc);
          }
          for (const svc of await scanLaunchdDir({ dir: path.join(path.sep, "Library", "LaunchDaemons"), scope: "system" })) {
            push(svc);
          }
        }
      }
    ...
    try {
      {
        ...
        for (const svc of await scanSystemdDir({ dir: userDir, scope: "user" })) {
          push(svc);
        }
        if (opts.deep) {
          for (const dir of ["/etc/systemd/system", "/usr/lib/systemd/system", "/lib/systemd/system"]) {
            for (const svc of await scanSystemdDir({ dir, scope: "system" })) {
              push(svc);
            }
          }
        }
  ...
  if ((process.platform === "win32")) {
    ...
    for (const task of tasks) {
      ...
      push({ platform: "win32", label: name, detail: task.taskToRun ? "task: , run: " : name, scope: "system", marker, legacy: (marker !== "openclaw") });
    }
    return results;
  }
  return results;
