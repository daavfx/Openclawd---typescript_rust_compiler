...
function resolveSystemdUnitPathForName(env, name) {
  ...
  return path.posix.join(home, ".config", "systemd", "user", ".service");
}
...
")) {
      ...
      return { programArguments, ...workingDirectory ? { workingDirectory } : {  }: , ...(Object.keys(environment).length > 0) ? { environment } : {  }: , sourcePath: unitPath };
    }
  }
  catch {
    {
...
async function assertSystemdAvailable() {
  ...
  throw new Error("systemctl --user unavailable: ".trim());
}
export async function installSystemdService({env, stdout, programArguments, workingDirectory, environment, description}) {
  ...
  const unit = buildSystemdUnit({ description: serviceDescription, programArguments, workingDirectory, environment });
  await fs.writeFile(unitPath, unit, "utf8");
  const serviceName = resolveGatewaySystemdServiceName(env.OPENCLAW_PROFILE);
  const unitName = ".service";
  const reload = await execSystemctl(["--user", "daemon-reload"]);
  if ((reload.code !== 0)) {
    throw new Error("systemctl daemon-reload failed: ".trim());
  }
  const enable = await execSystemctl(["--user", "enable", unitName]);
  if ((enable.code !== 0)) {
    throw new Error("systemctl enable failed: ".trim());
  }
  const restart = await execSystemctl(["--user", "restart", unitName]);
  if ((restart.code !== 0)) {
    throw new Error("systemctl restart failed: ".trim());
  }
  stdout.write("
");
  stdout.write("
...
");
    ...
    {
      stdout.write("Systemd service not found at 
");
    }
  }
}
...
export async function stopSystemdService({stdout, env}) {
  ...
  if ((res.code !== 0)) {
    throw new Error("systemctl stop failed: ".trim());
  }
  stdout.write("
");
}
...
export async function readSystemdServiceRuntime(env = process.env) {
  ...
  const serviceName = resolveSystemdServiceName(env);
  const unitName = ".service";
  const res = await execSystemctl(["--user", "show", unitName, "--no-page", "--property", "ActiveState,SubState,MainPID,ExecMainStatus,ExecMainCode"]);
  if ((res.code !== 0)) {
    const detail = (res.stderr || res.stdout).trim();
    const missing = detail.toLowerCase().includes("not found");
    return { status: missing ? "stopped" : "unknown", detail: (detail || undefined), missingUnit: missing };
  }
  ...
  return { status, state: parsed.activeState, subState: parsed.subState, pid: parsed.mainPid, lastExitStatus: parsed.execMainStatus, lastExitReason: parsed.execMainCode };
}
...
export async function findLegacySystemdUnits(env) {
  ...
  for (const name of LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES) {
    ...
    if (systemctlAvailable) {
      const res = await execSystemctl(["--user", "is-enabled", ".service"]);
      enabled = (res.code === 0);
    }
    if ((exists || enabled)) {
...
export async function uninstallLegacySystemdUnits({env, stdout}) {
  ...
  for (const unit of units) {
    if (systemctlAvailable) {
      await execSystemctl(["--user", "disable", "--now", ".service"]);
    } else {
      stdout.write("systemctl unavailable; removed legacy unit file only: .service
");
    }
    try {
      {
...
");
      ...
      {
        stdout.write("Legacy systemd unit not found at 
");
      }
    }
  }
