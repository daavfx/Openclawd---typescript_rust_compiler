...
vi.mock("node:fs/promises", () => { default: { access: fsMocks.access }, access: fsMocks.access });
import { renderSystemNodeWarning, resolvePreferredNodePath, resolveSystemNodeInfo } from "./runtime-paths.js";
afterEach(() => {
  vi.resetAllMocks();
});
...
", stderr: "" });
    const result = await resolvePreferredNodePath({ env: {  }, runtime: "node", platform: "darwin", execFile });
...
", stderr: "" });
    ...
    expect(result).toEqual({ path: darwinNode, version: "22.0.0", supported: true });
  });
  it("renders a warning when system node is too old", () => {
    const warning = renderSystemNodeWarning({ path: darwinNode, version: "18.19.0", supported: false }, "/Users/me/.fnm/node-22/bin/node");
