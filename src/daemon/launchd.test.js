...
import { installLaunchAgent, isLaunchAgentListed, parseLaunchctlPrint, repairLaunchAgentBootstrap, resolveLaunchAgentPlistPath } from "./launchd.js";
  ...
  try {
    {
      ...
      await fs.writeFile(stubJsPath, ["import fs from \"node:fs\";", "const args = process.argv.slice(2);", "const logPath = process.env.OPENCLAW_TEST_LAUNCHCTL_LOG;", "if (logPath) {", "  fs.appendFileSync(logPath, JSON.stringify(args) + \"\\n\", \"utf8\");", "}", "if (args[0] === \"list\") {", "  const output = process.env.OPENCLAW_TEST_LAUNCHCTL_LIST_OUTPUT || \"\";", "  process.stdout.write(output);", "}", "process.exit(0);", ""].join("
"), "utf8");
      if ((process.platform === "win32")) {
        await fs.writeFile(path.join(binDir, "launchctl.cmd"), "@echo off
...
", "utf8");
      ...
      process.env.PATH = "";
      await run({ env: { HOME: homeDir, OPENCLAW_PROFILE: "default" }, logPath });
    }
  }
  finally {
    {
...
");
    expect(parseLaunchctlPrint(output)).toEqual({ state: "running", pid: 4242, lastExitStatus: 1, lastExitReason: "exited" });
  });
});
...
").filter(Boolean).map((line) => JSON.parse(line));
      const domain = (typeof process.getuid === "function") ? "gui/" : "gui/501";
      const label = "ai.openclaw.gateway";
      const plistPath = resolveLaunchAgentPlistPath(env);
      expect(calls).toContainEqual(["bootstrap", domain, plistPath]);
      expect(calls).toContainEqual(["kickstart", "-k", "/"]);
    });
  });
});
    ...
    try {
      {
        ...
        await fs.mkdir(homeDir, { recursive: true });
        const stubJsPath = path.join(binDir, "launchctl.js");
        await fs.writeFile(stubJsPath, ["import fs from \"node:fs\";", "const logPath = process.env.OPENCLAW_TEST_LAUNCHCTL_LOG;", "if (logPath) {", "  fs.appendFileSync(logPath, JSON.stringify(process.argv.slice(2)) + \"\\n\", \"utf8\");", "}", "process.exit(0);", ""].join("
"), "utf8");
        if ((process.platform === "win32")) {
          await fs.writeFile(path.join(binDir, "launchctl.cmd"), "@echo off
node \"%~dp0\\launchctl.js\" %*
...
", "utf8");
        ...
        process.env.OPENCLAW_TEST_LAUNCHCTL_LOG = logPath;
        process.env.PATH = "";
        const env = { HOME: homeDir, OPENCLAW_PROFILE: "default" };
        await installLaunchAgent({ env, stdout: new PassThrough(), programArguments: ["node", "-e", "process.exit(0)"] });
        const calls = await fs.readFile(logPath, "utf8").split("
").filter(Boolean).map((line) => JSON.parse(line));
        const domain = (typeof process.getuid === "function") ? "gui/" : "gui/501";
        const label = "ai.openclaw.gateway";
        ...
        const bootstrapIndex = calls.findIndex((c) => (((c[0] === "bootstrap") && (c[1] === domain)) && (c[2] === plistPath)));
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when OPENCLAW_PROFILE is default", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe("/Users/test/Library/LaunchAgents/ai.openclaw.gateway.plist");
  });
  ...
  it("uses profile-specific label when OPENCLAW_PROFILE is set to a custom value", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe("/Users/test/Library/LaunchAgents/ai.openclaw.jbphoenix.plist");
  });
  it("prefers OPENCLAW_LAUNCHD_LABEL over OPENCLAW_PROFILE", () => {
    const env = { HOME: "/Users/test", OPENCLAW_PROFILE: "jbphoenix", OPENCLAW_LAUNCHD_LABEL: "com.custom.label" };
    expect(resolveLaunchAgentPlistPath(env)).toBe("/Users/test/Library/LaunchAgents/com.custom.label.plist");
  });
  it("trims whitespace from OPENCLAW_LAUNCHD_LABEL", () => {
    const env = { HOME: "/Users/test", OPENCLAW_LAUNCHD_LABEL: "  com.custom.label  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe("/Users/test/Library/LaunchAgents/com.custom.label.plist");
  });
  it("ignores empty OPENCLAW_LAUNCHD_LABEL and falls back to profile", () => {
    const env = { HOME: "/Users/test", OPENCLAW_PROFILE: "myprofile", OPENCLAW_LAUNCHD_LABEL: "   " };
    expect(resolveLaunchAgentPlistPath(env)).toBe("/Users/test/Library/LaunchAgents/ai.openclaw.myprofile.plist");
  });
