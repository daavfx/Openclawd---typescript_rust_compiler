...
describe("createEditorSubmitHandler", () => {
  it("adds submitted messages to editor history", () => {
    ...
    const handler = createEditorSubmitHandler({ editor, handleCommand: vi.fn(), sendMessage: vi.fn(), handleBangLine: vi.fn() });
    handler("hello world");
  ...
  it("routes slash commands to handleCommand", () => {
    ...
    const handler = createEditorSubmitHandler({ editor, handleCommand, sendMessage, handleBangLine: vi.fn() });
    handler("/models");
  ...
  it("routes bang-prefixed lines to handleBangLine", () => {
    ...
    const handler = createEditorSubmitHandler({ editor, handleCommand: vi.fn(), sendMessage: vi.fn(), handleBangLine });
    handler("!ls");
  ...
  it("treats a lone ! as a normal message", () => {
    const editor = { setText: vi.fn(), addToHistory: vi.fn() };
    const sendMessage = vi.fn();
    const handler = createEditorSubmitHandler({ editor, handleCommand: vi.fn(), sendMessage, handleBangLine: vi.fn() });
    handler("!");
