...
const createSelector = () => {
  const selector = { onSelect: undefined, onCancel: undefined, render: () => ["selector"], invalidate: () => {
  } };
  return selector;
};
describe("createLocalShellRunner", () => {
  it("logs denial on subsequent ! attempts without re-prompting", async () => {
    ...
    const {runLocalShellLine} = createLocalShellRunner({ chatLog, tui, openOverlay, closeOverlay, createSelector: createSelectorSpy, spawnCommand });
