...
export function createSessionActions(context) {
  ...
  const applyAgentsResult = (result) => {
    ...
    state.agents = result.agents.map((agent) => { id: normalizeAgentId(agent.id), name: (agent.name?.trim() || undefined) });
    agentNames.clear();
  ...
  const refreshAgents = async () => {
    try {
      ...
      {
        chatLog.addSystem("agents list failed: ");
      }
    }
  };
  ...
  const refreshSessionInfo = async () => {
    ...
    refreshSessionInfoPromise = async () => {
      try {
        {
          const listAgentId = ((state.currentSessionKey === "global") || (state.currentSessionKey === "unknown")) ? undefined : state.currentAgentId;
          const result = await client.listSessions({ includeGlobal: false, includeUnknown: false, agentId: listAgentId });
  ...
  const loadHistory = async () => {
    try {
      {
        const history = await client.loadHistory({ sessionKey: state.currentSessionKey, limit: (opts.historyLimit ?? 200) });
        ...
        for (const entry of (record.messages ?? [])) {
          ...
          if ((message.role === "toolResult")) {
            ...
            component.setResult({ content: Array.isArray(message.content) ? message.content : [], details: ((typeof message.details === "object") && message.details) ? message.details : undefined }, { isError: Boolean(message.isError) });
          }
        }
      ...
      {
        chatLog.addSystem("history failed: ");
      }
    }
  ...
  const abortActive = async () => {
    ...
    try {
      {
        await client.abortChat({ sessionKey: state.currentSessionKey, runId: state.activeChatRunId });
        setActivityStatus("aborted");
      }
    }
    catch (err) {
      {
        chatLog.addSystem("abort failed: ");
        setActivityStatus("abort failed");
      }
    }
    tui.requestRender();
  };
  return { applyAgentsResult, refreshAgents, refreshSessionInfo, loadHistory, setSession, abortActive };
}
