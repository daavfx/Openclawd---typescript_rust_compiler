...
export function serveAcpGateway(opts = {  }) {
  ...
  const token = (((opts.gatewayToken ?? isRemoteMode ? remote?.token?.trim() : undefined) ?? process.env.OPENCLAW_GATEWAY_TOKEN) ?? auth.token);
  const password = (((opts.gatewayPassword ?? isRemoteMode ? remote?.password?.trim() : undefined) ?? process.env.OPENCLAW_GATEWAY_PASSWORD) ?? auth.password);
  let agent = null;
  const gateway = new GatewayClient({ url: connection.url, token: (token || undefined), password: (password || undefined), clientName: GATEWAY_CLIENT_NAMES.CLI, clientDisplayName: "ACP", clientVersion: "acp", mode: GATEWAY_CLIENT_MODES.CLI, onEvent: (evt) => {
    void agent?.handleGatewayEvent(evt);
  }, onHelloOk: () => {
    agent?.handleGatewayReconnect();
  }, onClose: (code, reason) => {
    agent?.handleGatewayDisconnect(": ");
  } });
