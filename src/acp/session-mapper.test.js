...
describe("acp session mapper", () => {
  it("prefers explicit sessionLabel over sessionKey", async () => {
    ...
    const key = await resolveSessionKey({ meta, fallbackKey: "acp:fallback", gateway, opts: {  } });
  ...
  it("lets meta sessionKey override default label", async () => {
    ...
    const key = await resolveSessionKey({ meta, fallbackKey: "acp:fallback", gateway, opts: { defaultSessionLabel: "default-label" } });
