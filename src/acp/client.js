...
[tool]  ()");
    ...
    case "available_commands_update":
      {
        const names = update.availableCommands?.map((cmd) => "/").join(" ");
        if (names) {
          console.log("
[commands] ");
        }
...
export async function createAcpClient(opts = {  }) {
  ...
  const log = verbose ? (msg) => console.error("[acp-client] ") : () => {
  };
  ensureOpenClawCliOnPath({ cwd });
  const serverCommand = (opts.serverCommand ?? "openclaw");
  const serverArgs = buildServerArgs(opts);
  log("spawning:  ");
  const agent = spawn(serverCommand, serverArgs, { stdio: ["pipe", "pipe", "inherit"], cwd });
  ...
  const client = new ClientSideConnection(() => { sessionUpdate: async (params) => {
...
[permission requested]", (params.toolCall?.title ?? "tool"));
    ...
    return { outcome: { outcome: "selected", optionId: ((allowOnce?.optionId ?? fallback?.optionId) ?? "allow") } };
  } }, stream);
  log("initializing");
  await client.initialize({ protocolVersion: PROTOCOL_VERSION, clientCapabilities: { fs: { readTextFile: true, writeTextFile: true }, terminal: true }, clientInfo: { name: "openclaw-acp-client", version: "1.0.0" } });
