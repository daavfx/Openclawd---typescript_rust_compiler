...
export function resolveChannelEntryMatchWithFallback(params) {
  const direct = resolveChannelEntryMatch({ entries: params.entries, keys: params.keys, wildcardKey: params.wildcardKey });
    ...
    if ((normalizedKeys.length > 0)) {
      for (const [entryKey, entry] of Object.entries((params.entries ?? {  }))) {
        ...
        if ((normalizedEntry && normalizedKeys.includes(normalizedEntry))) {
          return { ...direct: , entry, key: entryKey, matchKey: entryKey, matchSource: "direct" };
        }
      }
    }
  }
  ...
  if ((parentKeys.length > 0)) {
    ...
    if ((parent.entry && parent.key)) {
      return { ...direct: , entry: parent.entry, key: parent.key, parentEntry: parent.entry, parentKey: parent.key, matchKey: parent.key, matchSource: "parent" };
    }
    if (normalizeKey) {
      ...
      if ((normalizedParentKeys.length > 0)) {
        for (const [entryKey, entry] of Object.entries((params.entries ?? {  }))) {
          ...
          if ((normalizedEntry && normalizedParentKeys.includes(normalizedEntry))) {
            return { ...direct: , entry, key: entryKey, parentEntry: entry, parentKey: entryKey, matchKey: entryKey, matchSource: "parent" };
          }
        }
      }
    }
  ...
  if ((direct.wildcardEntry && direct.wildcardKey)) {
    return { ...direct: , entry: direct.wildcardEntry, key: direct.wildcardKey, matchKey: direct.wildcardKey, matchSource: "wildcard" };
  }
  return direct;
}
