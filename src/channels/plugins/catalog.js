...
export 
export 
const ORIGIN_PRIORITY = { config: 0, workspace: 1, global: 2, bundled: 3 };
const DEFAULT_CATALOG_PATHS = [path.join(CONFIG_DIR, "mpm", "plugins.json"), path.join(CONFIG_DIR, "mpm", "catalog.json"), path.join(CONFIG_DIR, "plugins", "catalog.json")];
...
function resolveInstallInfo(params) {
  ...
  return { npmSpec, ...localPath ? { localPath } : {  }: , ...defaultChoice ? { defaultChoice } : {  }:  };
}
function buildCatalogEntry(candidate) {
  ...
  const install = resolveInstallInfo({ manifest, packageName: candidate.packageName, packageDir: candidate.packageDir, workspaceDir: candidate.workspaceDir });
  if (!install) {
    return null;
  }
...
function buildExternalCatalogEntry(entry) {
  ...
  return buildCatalogEntry({ packageName: entry.name, packageManifest: manifest });
}
export function buildChannelUiCatalog(plugins) {
  const entries = plugins.map((plugin) => {
    ...
    return { id: plugin.id, label: plugin.meta.label, detailLabel, ...plugin.meta.systemImage ? { systemImage: plugin.meta.systemImage } : {  }:  };
  });
...
export function listChannelPluginCatalogEntries(options = {  }) {
  ...
  return Array.from(resolved.values()).map(({entry}) => entry).sort((a, b) => {
