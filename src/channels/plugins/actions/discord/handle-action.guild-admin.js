...
export async function tryHandleDiscordMessageActionGuildAdmin(params) {
  ...
  if ((action === "emoji-upload")) {
    ...
    const mediaUrl = readStringParam(actionParams, "media", { required: true, trim: false });
    const roleIds = readStringArrayParam(actionParams, "roleIds");
    return await handleDiscordAction({ action: "emojiUpload", accountId: (accountId ?? undefined), guildId, name, mediaUrl, roleIds }, cfg);
  }
  if ((action === "sticker-upload")) {
    ...
    const mediaUrl = readStringParam(actionParams, "media", { required: true, trim: false });
    return await handleDiscordAction({ action: "stickerUpload", accountId: (accountId ?? undefined), guildId, name, description, tags, mediaUrl }, cfg);
  }
  if (((action === "role-add") || (action === "role-remove"))) {
    const guildId = readStringParam(actionParams, "guildId", { required: true });
    const userId = readStringParam(actionParams, "userId", { required: true });
    const roleId = readStringParam(actionParams, "roleId", { required: true });
    return await handleDiscordAction({ action: (action === "role-add") ? "roleAdd" : "roleRemove", accountId: (accountId ?? undefined), guildId, userId, roleId }, cfg);
  }
  ...
  if ((action === "channel-create")) {
    ...
    return await handleDiscordAction({ action: "channelCreate", accountId: (accountId ?? undefined), guildId, name, type: (type ?? undefined), parentId: (parentId ?? undefined), topic: (topic ?? undefined), position: (position ?? undefined), nsfw }, cfg);
  }
  if ((action === "channel-edit")) {
    ...
    return await handleDiscordAction({ action: "channelEdit", accountId: (accountId ?? undefined), channelId, name: (name ?? undefined), topic: (topic ?? undefined), position: (position ?? undefined), parentId: (parentId === undefined) ? undefined : parentId, nsfw, rateLimitPerUser: (rateLimitPerUser ?? undefined) }, cfg);
  }
  ...
  if ((action === "channel-move")) {
    ...
    const parentId = readParentIdParam(actionParams);
    const position = readNumberParam(actionParams, "position", { integer: true });
    return await handleDiscordAction({ action: "channelMove", accountId: (accountId ?? undefined), guildId, channelId, parentId: (parentId === undefined) ? undefined : parentId, position: (position ?? undefined) }, cfg);
  }
  if ((action === "category-create")) {
    const guildId = readStringParam(actionParams, "guildId", { required: true });
    const name = readStringParam(actionParams, "name", { required: true });
    const position = readNumberParam(actionParams, "position", { integer: true });
    return await handleDiscordAction({ action: "categoryCreate", accountId: (accountId ?? undefined), guildId, name, position: (position ?? undefined) }, cfg);
  }
  if ((action === "category-edit")) {
    const categoryId = readStringParam(actionParams, "categoryId", { required: true });
    const name = readStringParam(actionParams, "name");
    const position = readNumberParam(actionParams, "position", { integer: true });
    return await handleDiscordAction({ action: "categoryEdit", accountId: (accountId ?? undefined), categoryId, name: (name ?? undefined), position: (position ?? undefined) }, cfg);
  }
  ...
  if ((action === "event-create")) {
    ...
    return await handleDiscordAction({ action: "eventCreate", accountId: (accountId ?? undefined), guildId, name, startTime, endTime, description, channelId, location, entityType }, cfg);
  }
  if ((((action === "timeout") || (action === "kick")) || (action === "ban"))) {
    ...
    return await handleDiscordAction({ action: discordAction, accountId: (accountId ?? undefined), guildId, userId, durationMinutes, until, reason, deleteMessageDays }, cfg);
  }
  if ((action === "thread-list")) {
    ...
    return await handleDiscordAction({ action: "threadList", accountId: (accountId ?? undefined), guildId, channelId, includeArchived, before, limit }, cfg);
  }
  if ((action === "thread-reply")) {
    ...
    return await handleDiscordAction({ action: "threadReply", accountId: (accountId ?? undefined), channelId, content, mediaUrl: (mediaUrl ?? undefined), replyTo: (replyTo ?? undefined) }, cfg);
  }
  if ((action === "search")) {
    const guildId = readStringParam(actionParams, "guildId", { required: true });
    const query = readStringParam(actionParams, "query", { required: true });
    return await handleDiscordAction({ action: "searchMessages", accountId: (accountId ?? undefined), guildId, content: query, channelId: readStringParam(actionParams, "channelId"), channelIds: readStringArrayParam(actionParams, "channelIds"), authorId: readStringParam(actionParams, "authorId"), authorIds: readStringArrayParam(actionParams, "authorIds"), limit: readNumberParam(actionParams, "limit", { integer: true }) }, cfg);
  }
  return undefined;
}
