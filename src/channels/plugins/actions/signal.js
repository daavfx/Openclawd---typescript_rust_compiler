...
export const signalMessageActions = { listActions: ({cfg}) => {
...
}, supportsAction: ({action}) => (action !== "send"), handleAction: async ({action, params, cfg, accountId}) => {
  if ((action === "send")) {
    throw new Error("Send should be handled by outbound, not actions handler.");
  }
  if ((action === "react")) {
    const reactionLevelInfo = resolveSignalReactionLevel({ cfg, accountId: (accountId ?? undefined) });
    if (!reactionLevelInfo.agentReactionsEnabled) {
      throw new Error(("Signal agent reactions disabled (reactionLevel=\"\"). " + "Set channels.signal.reactionLevel to \"minimal\" or \"extensive\" to enable."));
    }
    ...
    const recipientRaw = (readStringParam(params, "recipient") ?? readStringParam(params, "to", { required: true, label: "recipient (UUID, phone number, or group)" }));
    ...
    const messageId = readStringParam(params, "messageId", { required: true, label: "messageId (timestamp)" });
    ...
    if (!Number.isFinite(timestamp)) {
      throw new Error("Invalid messageId: . Expected numeric timestamp.");
    }
    if (remove) {
      if (!emoji) {
        throw new Error("Emoji required to remove reaction.");
      }
      await removeReactionSignal((target.recipient ?? ""), timestamp, emoji, { accountId: (accountId ?? undefined), groupId: target.groupId, targetAuthor, targetAuthorUuid });
      return jsonResult({ ok: true, removed: emoji });
    }
    if (!emoji) {
      throw new Error("Emoji required to add reaction.");
    }
    await sendReactionSignal((target.recipient ?? ""), timestamp, emoji, { accountId: (accountId ?? undefined), groupId: target.groupId, targetAuthor, targetAuthorUuid });
    return jsonResult({ ok: true, added: emoji });
  }
  throw new Error("Action  not supported for .");
} }
