...
function getSessionRecipients(cfg) {
  ...
  const recipients = Object.entries(store).filter(([key]) => ((key !== "global") && (key !== "unknown"))).filter(([key]) => (!isGroupKey(key) && !isCronKey(key))).map(([_, entry]) => { to: ((normalizeChatChannelId(entry?.lastChannel) === "whatsapp") && entry?.lastTo) ? normalizeE164(entry.lastTo) : "", updatedAt: (entry?.updatedAt ?? 0) }).filter(({to}) => (to.length > 1)).sort((a, b) => (b.updatedAt - a.updatedAt));
  const seen = new Set();
...
export function resolveWhatsAppHeartbeatRecipients(cfg, opts = {  }) {
  ...
  if ((sessionRecipients.length > 1)) {
    return { recipients: sessionRecipients.map((s) => s.to), source: "session-ambiguous" };
  }
