...
import { buildChannelKeyCandidates, normalizeChannelSlug, resolveChannelEntryMatch, resolveChannelEntryMatchWithFallback, resolveNestedAllowlistDecision, applyChannelMatchMeta, resolveChannelMatchConfig } from "./channel-config.js";
...
describe("resolveChannelEntryMatch", () => {
  it("returns matched entry and wildcard metadata", () => {
    ...
    const match = resolveChannelEntryMatch({ entries, keys: ["missing", "a"], wildcardKey: "*" });
...
describe("resolveChannelEntryMatchWithFallback", () => {
  it("prefers direct matches over parent and wildcard", () => {
    ...
    const match = resolveChannelEntryMatchWithFallback({ entries, keys: ["a"], parentKeys: ["parent"], wildcardKey: "*" });
    expect(match.entry).toBe(entries.a);
  ...
  it("falls back to parent when direct misses", () => {
    ...
    const match = resolveChannelEntryMatchWithFallback({ entries, keys: ["missing"], parentKeys: ["parent"], wildcardKey: "*" });
  ...
  it("falls back to wildcard when no direct or parent match", () => {
    ...
    const match = resolveChannelEntryMatchWithFallback({ entries, keys: ["missing"], parentKeys: ["still-missing"], wildcardKey: "*" });
  ...
  it("matches normalized keys when normalizeKey is provided", () => {
    ...
    const match = resolveChannelEntryMatchWithFallback({ entries, keys: ["my-team"], normalizeKey: normalizeChannelSlug });
...
describe("applyChannelMatchMeta", () => {
  it("copies match metadata onto resolved configs", () => {
    const resolved = applyChannelMatchMeta({ allowed: true }, { matchKey: "general", matchSource: "direct" });
...
describe("resolveChannelMatchConfig", () => {
  ...
  it("resolves entry and applies match metadata", () => {
    const resolved = resolveChannelMatchConfig({ entry: { allow: true }, matchKey: "*", matchSource: "wildcard" }, () => { allowed: true });
...
describe("resolveNestedAllowlistDecision", () => {
  it("allows when outer allowlist is disabled", () => {
    expect(resolveNestedAllowlistDecision({ outerConfigured: false, outerMatched: false, innerConfigured: false, innerMatched: false })).toBe(true);
  });
  it("blocks when outer allowlist is configured but missing match", () => {
    expect(resolveNestedAllowlistDecision({ outerConfigured: true, outerMatched: false, innerConfigured: false, innerMatched: false })).toBe(false);
  });
  it("requires inner match when inner allowlist is configured", () => {
    expect(resolveNestedAllowlistDecision({ outerConfigured: true, outerMatched: true, innerConfigured: true, innerMatched: false })).toBe(false);
    expect(resolveNestedAllowlistDecision({ outerConfigured: true, outerMatched: true, innerConfigured: true, innerMatched: true })).toBe(true);
  });
});
