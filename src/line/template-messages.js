export function createConfirmTemplate(text, confirmAction, cancelAction, altText) {
  const template = { type: "confirm", text: text.slice(0, 240), actions: [confirmAction, cancelAction] };
  return { type: "template", altText: (altText?.slice(0, 400) ?? text.slice(0, 400)), template };
}
...
export function createButtonTemplate(title, text, actions, options) {
  ...
  const template = { type: "buttons", title: title.slice(0, 40), text: text.slice(0, textLimit), actions: actions.slice(0, 4), thumbnailImageUrl: options?.thumbnailImageUrl, imageAspectRatio: (options?.imageAspectRatio ?? "rectangle"), imageSize: (options?.imageSize ?? "cover"), imageBackgroundColor: options?.imageBackgroundColor, defaultAction: options?.defaultAction };
  return { type: "template", altText: (options?.altText?.slice(0, 400) ?? ": ".slice(0, 400)), template };
}

export function createTemplateCarousel(columns, options) {
  const template = { type: "carousel", columns: columns.slice(0, 10), imageAspectRatio: (options?.imageAspectRatio ?? "rectangle"), imageSize: (options?.imageSize ?? "cover") };
  return { type: "template", altText: (options?.altText?.slice(0, 400) ?? "View carousel"), template };
}

export function createCarouselColumn(params) {
  return { title: params.title?.slice(0, 40), text: params.text.slice(0, 120), actions: params.actions.slice(0, 3), thumbnailImageUrl: params.thumbnailImageUrl, imageBackgroundColor: params.imageBackgroundColor, defaultAction: params.defaultAction };
}

export function createImageCarousel(columns, altText) {
  const template = { type: "image_carousel", columns: columns.slice(0, 10) };
  return { type: "template", altText: (altText?.slice(0, 400) ?? "View images"), template };
}

export function createImageCarouselColumn(imageUrl, action) {
  return { imageUrl, action };
}
...
export function createButtonMenu(title, text, buttons, options) {
  ...
  return createButtonTemplate(title, text, actions, { thumbnailImageUrl: options?.thumbnailImageUrl, altText: options?.altText });
}
...
export function createProductCarousel(products, altText) {
  const columns = products.slice(0, 10).map((product) => {
    ...
    return createCarouselColumn({ title: product.title, text: product.price ? "
".slice(0, 120) : product.description, thumbnailImageUrl: product.imageUrl, actions });
  });
    ...
    case "buttons":
      {
        ...
        return createButtonTemplate(payload.title, payload.text, actions, { thumbnailImageUrl: payload.thumbnailImageUrl, altText: payload.altText });
      }
    case "carousel":
      {
        const columns = payload.columns.slice(0, 10).map((col) => {
          ...
          return createCarouselColumn({ title: col.title, text: col.text, thumbnailImageUrl: col.thumbnailImageUrl, actions: colActions });
        });
