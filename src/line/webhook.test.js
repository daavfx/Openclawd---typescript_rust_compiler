...
const createRes = () => {
  const res = { status: vi.fn(), json: vi.fn(), headersSent: false };
...
describe("createLineWebhookMiddleware", () => {
  it("parses JSON from raw string body", async () => {
    ...
    const req = { headers: { "x-line-signature": sign(rawBody, secret) }, body: rawBody };
    const res = createRes();
  ...
  it("parses JSON from raw buffer body", async () => {
    ...
    const middleware = createLineWebhookMiddleware({ channelSecret: secret, onEvents });
    const req = { headers: { "x-line-signature": sign(rawBody, secret) }, body: Buffer.from(rawBody, "utf-8") };
    const res = createRes();
    await middleware(req, res, {  });
    expect(res.status).toHaveBeenCalledWith(200);
    expect(onEvents).toHaveBeenCalledWith(expect.objectContaining({ events: expect.any(Array) }));
  ...
  it("rejects webhooks with invalid signatures", async () => {
    ...
    const rawBody = JSON.stringify({ events: [{ type: "message" }] });
    const middleware = createLineWebhookMiddleware({ channelSecret: secret, onEvents });
    const req = { headers: { "x-line-signature": "invalid-signature" }, body: rawBody };
    const res = createRes();
    await middleware(req, res, {  });
  ...
  it("rejects webhooks with signatures computed using wrong secret", async () => {
    ...
    const req = { headers: { "x-line-signature": sign(rawBody, wrongSecret) }, body: rawBody };
    const res = createRes();
    await middleware(req, res, {  });
    expect(res.status).toHaveBeenCalledWith(401);
    expect(onEvents).not.toHaveBeenCalled();
