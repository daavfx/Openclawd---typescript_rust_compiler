...
export async function deliverLineAutoReply(params) {
  ...
  const pushLineMessages = async (messages) => {
    ...
    for (let i = 0; (i < messages.length); i += 5) {
      await deps.pushMessagesLine(to, messages.slice(i, (i + 5)), { accountId });
    }
  };
  ...
  if ((chunks.length > 0)) {
    ...
    const {replyTokenUsed: nextReplyTokenUsed} = await deps.sendLineReplyChunks({ to, chunks, quickReplies: lineData.quickReplies, replyToken, replyTokenUsed, accountId, replyMessageLine: deps.replyMessageLine, pushMessageLine: deps.pushMessageLine, pushTextMessageWithQuickReplies: deps.pushTextMessageWithQuickReplies, createTextMessageWithQuickReplies: deps.createTextMessageWithQuickReplies });
