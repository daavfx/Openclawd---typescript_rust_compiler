...
function resolveLineGroupConfig(params) {
  ...
  if (params.groupId) {
    return ((groups[params.groupId] ?? groups["group:"]) ?? groups["*"]);
  }
  if (params.roomId) {
    return ((groups[params.roomId] ?? groups["room:"]) ?? groups["*"]);
  }
  return groups["*"];
}
async function sendLinePairingReply(params) {
  const {senderId, replyToken, context} = params;
  const {code, created} = await upsertChannelPairingRequest({ channel: "line", id: senderId });
  if (!created) {
    return;
  }
  logVerbose("line pairing request sender=");
  const idLabel = () => {
    try {
      {
  ...
  const text = buildPairingReply({ channel: "line", idLine: "Your : ", code });
  try {
    {
      if (replyToken) {
        await replyMessageLine(replyToken, [{ type: "text", text }], { accountId: context.account.accountId, channelAccessToken: context.account.channelAccessToken });
        return;
      }
    }
  }
    ...
    {
      logVerbose("line pairing reply failed for : ");
    }
  }
  try {
    {
      await pushMessageLine("line:", text, { accountId: context.account.accountId, channelAccessToken: context.account.channelAccessToken });
    }
  }
  catch (err) {
    {
...
async function shouldProcessLineEvent(event, context) {
  ...
  const effectiveDmAllow = normalizeAllowFromWithStore({ allowFrom: account.config.allowFrom, storeAllowFrom });
  ...
  const effectiveGroupAllow = normalizeAllowFromWithStore({ allowFrom: groupAllowFrom, storeAllowFrom });
  const dmPolicy = (account.config.dmPolicy ?? "pairing");
  const defaultGroupPolicy = cfg.channels?.defaults?.groupPolicy;
  const groupPolicy = ((account.config.groupPolicy ?? defaultGroupPolicy) ?? "allowlist");
  if (isGroup) {
    if ((groupConfig?.enabled === false)) {
      logVerbose("Blocked line group  (group disabled)");
      return false;
    }
    if ((typeof groupAllowOverride !== "undefined")) {
      ...
      if (!isSenderAllowed({ allow: effectiveGroupAllow, senderId })) {
        logVerbose("Blocked line group sender  (group allowFrom override)");
        return false;
      }
    }
    ...
    if ((groupPolicy === "allowlist")) {
      ...
      if (!isSenderAllowed({ allow: effectiveGroupAllow, senderId })) {
        logVerbose("Blocked line group message from  (groupPolicy: allowlist)");
        return false;
      }
    }
    return true;
  ...
  if (!dmAllowed) {
    if ((dmPolicy === "pairing")) {
      ...
      await sendLinePairingReply({ senderId, replyToken: ("replyToken" in event) ? event.replyToken : undefined, context });
    } else {
      logVerbose("Blocked line sender  (dmPolicy: )");
    }
    return false;
  }
  return true;
    ...
    try {
    ...
    catch (err) {
      {
        ...
        if ((errMsg.includes("exceeds") && errMsg.includes("limit"))) {
          logVerbose("line: media exceeds size limit for message ");
        } else {
          runtime.error?.(danger("line: failed to download media: "));
        }
      }
    }
  }
  const messageContext = await buildLineMessageContext({ event, allMedia, cfg, account });
  if (!messageContext) {
...
async function handleFollowEvent(event, _context) {
  ...
  logVerbose("line: user  followed");
}
async function handleUnfollowEvent(event, _context) {
  const userId = (event.source.type === "user") ? event.source.userId : undefined;
  logVerbose("line: user  unfollowed");
}
async function handleJoinEvent(event, _context) {
  ...
  logVerbose("line: bot joined ");
}
async function handleLeaveEvent(event, _context) {
  const groupId = (event.source.type === "group") ? event.source.groupId : undefined;
  const roomId = (event.source.type === "room") ? event.source.roomId : undefined;
  logVerbose("line: bot left ");
}
async function handlePostbackEvent(event, context) {
  const data = event.postback.data;
  logVerbose("line: received postback: ");
  if (!await shouldProcessLineEvent(event, context)) {
    return;
  }
  const postbackContext = await buildLinePostbackContext({ event, cfg: context.cfg, account: context.account });
  if (!postbackContext) {
    return;
  }
    ...
    try {
      {
        switch (event.type) {
          ...
          case "postback":
            ...
            break;
          default:
            logVerbose("line: unhandled event type: ");
        }
      }
    }
    catch (err) {
      {
        context.runtime.error?.(danger("line: event handler failed: "));
      }
    }
  }
}
