...
import { createConfirmTemplate, createButtonTemplate, createTemplateCarousel, createCarouselColumn, createImageCarousel, createImageCarouselColumn, createYesNoConfirm, createButtonMenu, createLinkMenu, createProductCarousel, messageAction, uriAction, postbackAction, datetimePickerAction } from "./template-messages.js";
...
describe("datetimePickerAction", () => {
  ...
  it("includes min/max/initial when provided", () => {
    const action = datetimePickerAction("Pick", "data", "datetime", { initial: "2024-01-01T12:00", min: "2024-01-01T00:00", max: "2024-12-31T23:59" });
...
describe("createConfirmTemplate", () => {
  ...
  it("uses custom altText when provided", () => {
    const template = createConfirmTemplate("Question?", messageAction("Yes"), messageAction("No"), "Custom alt");
...
describe("createButtonTemplate", () => {
  ...
  it("limits actions to 4", () => {
    const actions = Array.from({ length: 6 }, (_, i) => messageAction("Button "));
  ...
  it("includes thumbnail when provided", () => {
    const template = createButtonTemplate("Title", "Text", [messageAction("OK")], { thumbnailImageUrl: "https://example.com/thumb.jpg" });
    expect(template.template.thumbnailImageUrl).toBe("https://example.com/thumb.jpg");
  });
  ...
  it("keeps longer text when thumbnail is provided", () => {
    const longText = "x".repeat(100);
    const template = createButtonTemplate("Title", longText, [messageAction("OK")], { thumbnailImageUrl: "https://example.com/thumb.jpg" });
    expect(template.template.text.length).toBe(100);
  });
});
describe("createTemplateCarousel", () => {
  it("creates a carousel template", () => {
    const columns = [createCarouselColumn({ text: "Column 1", actions: [messageAction("Select")] }), createCarouselColumn({ text: "Column 2", actions: [messageAction("Select")] })];
  ...
  it("limits columns to 10", () => {
    const columns = Array.from({ length: 15 }, () => createCarouselColumn({ text: "Text", actions: [messageAction("OK")] }));
    const template = createTemplateCarousel(columns);
...
describe("createCarouselColumn", () => {
  it("creates a carousel column", () => {
    const column = createCarouselColumn({ title: "Item", text: "Description", actions: [messageAction("View")], thumbnailImageUrl: "https://example.com/img.jpg" });
  ...
  it("limits actions to 3", () => {
    const column = createCarouselColumn({ text: "Text", actions: [messageAction("A1"), messageAction("A2"), messageAction("A3"), messageAction("A4"), messageAction("A5")] });
...
describe("createImageCarousel", () => {
  it("creates an image carousel", () => {
    const columns = [createImageCarouselColumn("https://example.com/1.jpg", messageAction("View 1")), createImageCarouselColumn("https://example.com/2.jpg", messageAction("View 2"))];
    const template = createImageCarousel(columns);
    expect(template.type).toBe("template");
    expect(template.template.type).toBe("image_carousel");
  });
  it("limits columns to 10", () => {
    const columns = Array.from({ length: 15 }, (_, i) => createImageCarouselColumn("https://example.com/.jpg", messageAction("View")));
    const template = createImageCarousel(columns);
    expect(template.template.columns.length).toBe(10);
  });
});
...
describe("createYesNoConfirm", () => {
  ...
  it("allows custom button text", () => {
    const template = createYesNoConfirm("Delete?", { yesText: "Delete", noText: "Cancel" });
    const actions = template.template.actions;
    expect(actions[0].label).toBe("Delete");
    expect(actions[1].label).toBe("Cancel");
  });
  it("uses postback actions when data provided", () => {
    const template = createYesNoConfirm("Confirm?", { yesData: "action=confirm", noData: "action=cancel" });
    const actions = template.template.actions;
...
describe("createButtonMenu", () => {
  it("creates a button menu with text buttons", () => {
    const template = createButtonMenu("Menu", "Choose an option", [{ label: "Option 1" }, { label: "Option 2", text: "selected option 2" }]);
    expect(template.type).toBe("template");
    expect(template.template.type).toBe("buttons");
    const actions = template.template.actions;
...
describe("createLinkMenu", () => {
  it("creates a button menu with URL links", () => {
    const template = createLinkMenu("Links", "Visit our sites", [{ label: "Site 1", url: "https://site1.com" }, { label: "Site 2", url: "https://site2.com" }]);
    expect(template.type).toBe("template");
    const actions = template.template.actions;
...
describe("createProductCarousel", () => {
  it("creates a product carousel", () => {
    const template = createProductCarousel([{ title: "Product 1", description: "Desc 1", price: "$10" }, { title: "Product 2", description: "Desc 2", imageUrl: "https://example.com/p2.jpg" }]);
    expect(template.type).toBe("template");
    expect(template.template.type).toBe("carousel");
  ...
  it("uses URI action when actionUrl provided", () => {
    const template = createProductCarousel([{ title: "Product", description: "Desc", actionLabel: "Buy", actionUrl: "https://shop.com/buy" }]);
    const columns = template.template.columns;
    expect(columns[0].actions[0].type).toBe("uri");
  });
  it("uses postback action when actionData provided", () => {
    const template = createProductCarousel([{ title: "Product", description: "Desc", actionLabel: "Select", actionData: "product_id=123" }]);
    const columns = template.template.columns;
    expect(columns[0].actions[0].type).toBe("postback");
  });
  it("limits to 10 products", () => {
    const products = Array.from({ length: 15 }, (_, i) => { title: "Product ", description: "Desc " });
