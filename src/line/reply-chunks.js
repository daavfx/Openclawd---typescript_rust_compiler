...
export async function sendLineReplyChunks(params) {
    ...
    try {
      {
        ...
        const replyMessages = replyBatch.map((chunk) => { type: "text", text: chunk });
        ...
        for (let i = 0; (i < remaining.length); i += 1) {
          ...
          if ((isLastChunk && hasQuickReplies)) {
            ...
            await params.pushMessageLine(params.to, remaining[i], { accountId: params.accountId });
          }
        }
        return { replyTokenUsed };
      }
  ...
  for (let i = 0; (i < params.chunks.length); i += 1) {
    ...
    if ((isLastChunk && hasQuickReplies)) {
      ...
      await params.pushMessageLine(params.to, params.chunks[i], { accountId: params.accountId });
    }
  }
  return { replyTokenUsed };
}
