...
export function extractMarkdownTables(text) {
  ...
  while ((match = MARKDOWN_TABLE_REGEX.exec(text) !== null)) {
    ...
    if (((headers.length > 0) && (rows.length > 0))) {
      matches.push({ fullMatch, table: { headers, rows } });
    }
  }
...
export function convertTableToFlexBubble(table) {
  ...
  if (((table.headers.length === 2) && !hasInlineMarkup)) {
    const items = rowCells.map((row) => { name: (row[0]?.text ?? "-"), value: (row[1]?.text ?? "-") });
    return createReceiptCard({ title: headerCells.map((cell) => cell.text).join(" / "), items });
  }
  const headerRow = { type: "box", layout: "horizontal", contents: headerCells.map((cell) => { type: "text", text: cell.text, weight: "bold", size: "sm", color: "#333333", flex: 1, wrap: true }), paddingBottom: "sm" };
  const dataRows = rowCells.slice(0, 10).map((row, rowIndex) => {
    const rowContents = table.headers.map((_, colIndex) => {
      ...
      return { type: "text", text: cell.text, size: "sm", color: "#666666", flex: 1, wrap: true, weight: cell.bold ? "bold" : undefined };
    });
    return { type: "box", layout: "horizontal", contents: rowContents, margin: (rowIndex === 0) ? "md" : "sm" };
  });
  return { type: "bubble", body: { type: "box", layout: "vertical", contents: [headerRow, { type: "separator", margin: "sm" }, ...dataRows], paddingAll: "lg" } };
}
...
export function extractCodeBlocks(text) {
  ...
  while ((match = MARKDOWN_CODE_BLOCK_REGEX.exec(text) !== null)) {
    ...
    const code = match[2];
    matches.push({ fullMatch, block: { language, code: code.trim() } });
  }
  for (let i = (matches.length - 1); (i >= 0); i--) {
...
export function convertCodeBlockToFlexBubble(block) {
  const titleText = block.language ? "Code ()" : "Code";
  const displayCode = (block.code.length > 2000) ? (block.code.slice(0, 2000) + "
...") : block.code;
  return { type: "bubble", body: { type: "box", layout: "vertical", contents: [{ type: "text", text: titleText, weight: "bold", size: "sm", color: "#666666" }, { type: "box", layout: "vertical", contents: [{ type: "text", text: displayCode, size: "xs", color: "#333333", wrap: true }], backgroundColor: "#F5F5F5", paddingAll: "md", cornerRadius: "md", margin: "sm" }], paddingAll: "lg" } };
}
...
export function convertLinksToFlexBubble(links) {
  const buttons = links.slice(0, 4).map((link, index) => { type: "button", action: { type: "uri", label: link.text.slice(0, 20), uri: link.url }, style: (index === 0) ? "primary" : "secondary", margin: (index > 0) ? "sm" : undefined });
  return { type: "bubble", body: { type: "box", layout: "vertical", contents: [{ type: "text", text: "Links", weight: "bold", size: "md", color: "#333333" }], paddingAll: "lg", paddingBottom: "sm" }, footer: { type: "box", layout: "vertical", contents: buttons, paddingAll: "md" } };
}
