...
export function createLineBot(opts) {
  ...
  const handleWebhook = async (body) => {
    ...
    await handleLineWebhookEvents(body.events, { cfg, account, runtime, mediaMaxBytes, processMessage });
  };
  return { handleWebhook, account };
}

export function createLineWebhookCallback(bot, channelSecret, path = "/line/webhook") {
  const {handler} = startLineWebhook({ channelSecret, onEvents: bot.handleWebhook, path });
  return { path, handler };
}
