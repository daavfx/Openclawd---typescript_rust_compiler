...
import { createFlexMessage, createQuickReplyItems, createTextMessageWithQuickReplies } from "./send.js";
...
describe("createQuickReplyItems", () => {
  ...
  it("limits items to 13 (LINE maximum)", () => {
    const labels = Array.from({ length: 20 }, (_, i) => "Option ");
    const quickReply = createQuickReplyItems(labels);
    expect(quickReply.items).toHaveLength(13);
  });
  it("truncates labels to 20 characters", () => {
    const quickReply = createQuickReplyItems(["This is a very long option label that exceeds the limit"]);
    expect(quickReply.items[0].action.label).toBe("This is a very long ");
    expect(quickReply.items[0].action.text).toBe("This is a very long option label that exceeds the limit");
  });
