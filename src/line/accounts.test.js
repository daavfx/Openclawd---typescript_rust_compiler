...
import { resolveLineAccount, listLineAccountIds, resolveDefaultLineAccountId, normalizeAccountId, DEFAULT_ACCOUNT_ID } from "./accounts.js";
describe("LINE accounts", () => {
  ...
  describe("resolveLineAccount", () => {
    it("resolves account from config", () => {
      const cfg = { channels: { line: { enabled: true, channelAccessToken: "test-token", channelSecret: "test-secret", name: "Test Bot" } } };
    ...
    it("resolves account from environment variables", () => {
      ...
      const cfg = { channels: { line: { enabled: true } } };
      const account = resolveLineAccount({ cfg });
    ...
    it("resolves named account", () => {
      const cfg = { channels: { line: { enabled: true, accounts: { business: { enabled: true, channelAccessToken: "business-token", channelSecret: "business-secret", name: "Business Bot" } } } } };
  ...
  describe("listLineAccountIds", () => {
    it("returns default account when configured at base level", () => {
      const cfg = { channels: { line: { channelAccessToken: "test-token" } } };
      const ids = listLineAccountIds(cfg);
      expect(ids).toContain(DEFAULT_ACCOUNT_ID);
    });
    it("returns named accounts", () => {
      const cfg = { channels: { line: { accounts: { business: { enabled: true }, personal: { enabled: true } } } } };
      const ids = listLineAccountIds(cfg);
  ...
  describe("resolveDefaultLineAccountId", () => {
    ...
    it("returns first named account when default not configured", () => {
      const cfg = { channels: { line: { accounts: { business: { enabled: true } } } } };
      const id = resolveDefaultLineAccountId(cfg);
