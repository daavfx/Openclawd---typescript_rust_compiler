...
import { extractMarkdownTables, extractCodeBlocks, extractLinks, stripMarkdown, processLineMessage, convertTableToFlexBubble, convertCodeBlockToFlexBubble, hasMarkdownToConvert } from "./markdown-to-line.js";
...
And some more text.";
    ...
    expect(tables[0].rows).toEqual([["foo", "123"], ["bar", "456"]]);
...
describe("convertTableToFlexBubble", () => {
  it("creates a receipt-style card for 2-column tables", () => {
    const table = { headers: ["Item", "Price"], rows: [["Apple", "$1"], ["Banana", "$2"]] };
  ...
  it("strips bold markers and applies weight for fully bold cells", () => {
    const table = { headers: ["**Name**", "Status"], rows: [["**Alpha**", "OK"]] };
    const bubble = convertTableToFlexBubble(table);
    const body = bubble.body;
