...
const createFlexMessage = (altText, contents) => { type: "flex", altText, contents };
const createImageMessage = (url) => { type: "image", originalContentUrl: url, previewImageUrl: url };
const createLocationMessage = (location) => { type: "location", ...location:  };
describe("deliverLineAutoReply", () => {
  ...
  it("uses reply token for rich-only payloads", async () => {
    ...
    const createQuickReplyItems = vi.fn((labels) => { items: labels });
    const pushMessagesLine = vi.fn(async () => { messageId: "push", chatId: "u1" });
    const lineData = { flexMessage: { altText: "Card", contents: { type: "bubble" } }, quickReplies: ["A"] };
    ...
    expect(result.replyTokenUsed).toBe(true);
    expect(replyMessageLine).toHaveBeenCalledTimes(1);
    expect(replyMessageLine).toHaveBeenCalledWith("token", [{ ...createFlexMessage("Card", { type: "bubble" }): , quickReply: { items: ["A"] } }], { accountId: "acc" });
  ...
  it("sends rich messages before quick-reply text so quick replies remain visible", async () => {
    ...
    const pushMessageLine = vi.fn(async () => {  });
    const pushTextMessageWithQuickReplies = vi.fn(async () => {  });
    const createTextMessageWithQuickReplies = vi.fn((text, _quickReplies) => { type: "text", text, quickReply: { items: ["A"] } });
    const createQuickReplyItems = vi.fn((labels) => { items: labels });
    const pushMessagesLine = vi.fn(async () => { messageId: "push", chatId: "u1" });
    const lineData = { flexMessage: { altText: "Card", contents: { type: "bubble" } }, quickReplies: ["A"] };
    await deliverLineAutoReply({ payload: { text: "hello", channelData: { line: lineData } }, lineData, to: "line:user:1", replyToken: "token", replyTokenUsed: false, accountId: "acc", textLimit: 5000, deps: { buildTemplateMessageFromPayload: () => null, processLineMessage: (text) => { text, flexMessages: [] }, chunkMarkdownText: (text) => [text], sendLineReplyChunks, replyMessageLine, pushMessageLine, pushTextMessageWithQuickReplies, createTextMessageWithQuickReplies, createQuickReplyItems, pushMessagesLine, createFlexMessage, createImageMessage, createLocationMessage } });
    expect(pushMessagesLine).toHaveBeenCalledWith("line:user:1", [createFlexMessage("Card", { type: "bubble" })], { accountId: "acc" });
    expect(replyMessageLine).toHaveBeenCalledWith("token", [{ type: "text", text: "hello", quickReply: { items: ["A"] } }], { accountId: "acc" });
