...
function normalizeAllowFromEntry(params) {
  const normalized = formatAllowFromList({ dock: params.dock, cfg: params.cfg, accountId: params.accountId, allowFrom: [params.value] });
...
export function resolveCommandAuthorization(params) {
  ...
  const allowFromList = formatAllowFromList({ dock, cfg, accountId: ctx.AccountId, allowFrom: Array.isArray(allowFromRaw) ? allowFromRaw : [] });
  ...
  const senderCandidates = resolveSenderCandidates({ dock, providerId, cfg, accountId: ctx.AccountId, senderId: ctx.SenderId, senderE164: ctx.SenderE164, from });
  const matchedSender = ownerList.length ? senderCandidates.find((candidate) => ownerList.includes(candidate)) : undefined;
  const senderId = (matchedSender ?? senderCandidates[0]);
  const enforceOwner = Boolean(dock?.commands?.enforceOwnerForCommands);
  const isOwner = (((!enforceOwner || allowAll) || (ownerList.length === 0)) || Boolean(matchedSender));
  const isAuthorizedSender = (commandAuthorized && isOwner);
  return { providerId, ownerList, senderId: (senderId || undefined), isAuthorizedSender, from: (from || undefined), to: (to || undefined) };
}
