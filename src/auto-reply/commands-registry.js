...
function buildSkillCommandDefinitions(skillCommands) {
  ...
  return skillCommands.map((spec) => { key: "skill:", nativeName: spec.name, description: spec.description, textAliases: ["/"], acceptsArgs: true, argsParsing: "none", scope: "both" });
}
...
export function listNativeCommandSpecs(params) {
  return listChatCommands({ skillCommands: params?.skillCommands }).filter((command) => ((command.scope !== "text") && command.nativeName)).map((command) => { name: (resolveNativeName(command, params?.provider) ?? command.key), description: command.description, acceptsArgs: Boolean(command.acceptsArgs), args: command.args });
}

export function listNativeCommandSpecsForConfig(cfg, params) {
  return listChatCommandsForConfig(cfg, params).filter((command) => ((command.scope !== "text") && command.nativeName)).map((command) => { name: (resolveNativeName(command, params?.provider) ?? command.key), description: command.description, acceptsArgs: Boolean(command.acceptsArgs), args: command.args });
}
...
function resolveDefaultCommandContext(cfg) {
  const resolved = resolveConfiguredModelRef({ cfg: (cfg ?? {  }), defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL });
  return { provider: (resolved.provider ?? DEFAULT_PROVIDER), model: (resolved.model ?? DEFAULT_MODEL) };
}
export 
export function resolveCommandArgChoices(params) {
  ...
  const raw = Array.isArray(provided) ? provided : () => {
    ...
    const context = { cfg, provider: (params.provider ?? defaults.provider), model: (params.model ?? defaults.model), command, arg };
...
export function resolveCommandArgMenu(params) {
  ...
  const argName = (argSpec === "auto") ? command.args.find((arg) => (resolveCommandArgChoices({ command, arg, cfg }).length > 0))?.name : argSpec.arg;
  if (!argName) {
    return null;
  }
...
");
  ...
  const commandBody = (mentionMatch && (mentionMatch[2].toLowerCase() === normalizedBotUsername)) ? "/" : normalized;
  ...
  const normalizedRest = rest?.trimStart();
  return normalizedRest ? " " : tokenSpec.canonical;
}
...
export function getCommandDetection(_cfg) {
  ...
  cachedDetection = { exact, regex: patterns.length ? new RegExp("^(?:)$", "i") : /$^/ };
