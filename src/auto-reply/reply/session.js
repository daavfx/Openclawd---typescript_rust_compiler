  ...
  try {
    {
      ...
      const sessionFile = path.join(manager.getSessionDir(), "_.jsonl");
      const header = { type: "session", version: CURRENT_SESSION_VERSION, id: sessionId, timestamp, cwd: manager.getCwd(), parentSession: parentSessionFile };
...
export async function initSessionState(params) {
  ...
  const resetAuthorized = resolveCommandAuthorization({ ctx, cfg, commandAuthorized }).isAuthorizedSender;
  ...
  const now = Date.now();
  const isThread = resolveThreadFlag({ sessionKey, messageThreadId: ctx.MessageThreadId, threadLabel: ctx.ThreadLabel, threadStarterBody: ctx.ThreadStarterBody, parentSessionKey: ctx.ParentSessionKey });
  const resetType = resolveSessionResetType({ sessionKey, isGroup, isThread });
  const channelReset = resolveChannelResetConfig({ sessionCfg, channel: (((groupResolution?.channel ?? ctx.OriginatingChannel) ?? ctx.Surface) ?? ctx.Provider) });
  ...
  const sessionCtx = { ...ctx: , BodyStripped: formatInboundBodyWithSenderMeta({ ctx, body: normalizeInboundTextNewlines(((((((bodyStripped ?? ctx.BodyForAgent) ?? ctx.Body) ?? ctx.CommandBody) ?? ctx.RawBody) ?? ctx.BodyForCommands) ?? "")) }), SessionId: sessionId, IsNewSession: isNewSession ? "true" : "false" };
  return { sessionCtx, sessionEntry, previousSessionEntry, sessionStore, sessionKey, sessionId: (sessionId ?? crypto.randomUUID()), isNewSession, resetTriggered, systemSent, abortedLastRun, storePath, sessionScope, groupResolution, isGroup, bodyStripped, triggerBodyNormalized };
}
