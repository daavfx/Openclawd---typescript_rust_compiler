...
describe("runReplyAgent typing (heartbeat)", () => {
  it("signals typing for normal runs", async () => {
    ...
    const {run, typing} = createMinimalRun({ opts: { isHeartbeat: false, onPartialReply } });
    await run();
  ...
  it("signals typing even without consumer partial handler", async () => {
    ...
    const {run, typing} = createMinimalRun({ typingMode: "message" });
    await run();
    expect(typing.startTypingOnText).toHaveBeenCalledWith("hi");
  ...
  it("never signals typing for heartbeat runs", async () => {
    ...
    const {run, typing} = createMinimalRun({ opts: { isHeartbeat: true, onPartialReply } });
    await run();
    expect(onPartialReply).toHaveBeenCalled();
  ...
  it("suppresses partial streaming for NO_REPLY", async () => {
    ...
    const {run, typing} = createMinimalRun({ opts: { isHeartbeat: false, onPartialReply }, typingMode: "message" });
    await run();
...
_step_" });
    ...
    const {run, typing} = createMinimalRun({ typingMode: "thinking" });
    await run();
    expect(typing.startTypingLoop).toHaveBeenCalled();
    expect(typing.startTypingOnText).not.toHaveBeenCalled();
  });
  it("suppresses typing in never mode", async () => {
    ...
    const {run, typing} = createMinimalRun({ typingMode: "never" });
    await run();
    expect(typing.startTypingOnText).not.toHaveBeenCalled();
    expect(typing.startTypingLoop).not.toHaveBeenCalled();
  });
