...
import { formatDurationShort, formatRunLabel, formatRunStatus, resolveSubagentLabel, sortSubagentRuns } from "./subagents-utils.js";
const baseRun = { runId: "run-1", childSessionKey: "agent:main:subagent:abc", requesterSessionKey: "agent:main:main", requesterDisplayKey: "main", task: "do thing", cleanup: "keep", createdAt: 1000, startedAt: 1000 };
describe("subagents utils", () => {
  ...
  it("sorts subagent runs by newest start/created time", () => {
    const runs = [{ ...baseRun: , runId: "run-1", createdAt: 1000, startedAt: 1000 }, { ...baseRun: , runId: "run-2", createdAt: 1200, startedAt: 1200 }, { ...baseRun: , runId: "run-3", createdAt: 900 }];
  ...
  it("formats duration short for seconds and minutes", () => {
    ...
    expect(formatDurationShort(65000)).toBe("1m5s");
  });
});
