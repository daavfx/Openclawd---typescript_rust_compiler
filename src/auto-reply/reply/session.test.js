...
describe("initSessionState thread forking", () => {
  it("forks a new session from the parent session file", async () => {
    ...
    const header = { type: "session", version: 3, id: parentSessionId, timestamp: new Date().toISOString(), cwd: process.cwd() };
    const message = { type: "message", id: "m1", parentId: null, timestamp: new Date().toISOString(), message: { role: "user", content: "Parent prompt" } };
    await fs.writeFile(parentSessionFile, "

", "utf-8");
    const storePath = path.join(root, "sessions.json");
    const parentSessionKey = "agent:main:slack:channel:c1";
    await saveSessionStore(storePath, { [parentSessionKey]: { sessionId: parentSessionId, sessionFile: parentSessionFile, updatedAt: Date.now() } });
    const cfg = { session: { store: storePath } };
    const threadSessionKey = "agent:main:slack:channel:c1:thread:123";
    const threadLabel = "Slack thread #general: starter";
    const result = await initSessionState({ ctx: { Body: "Thread reply", SessionKey: threadSessionKey, ParentSessionKey: parentSessionKey, ThreadLabel: threadLabel }, cfg, commandAuthorized: true });
  ...
  it("records topic-specific session files when MessageThreadId is present", async () => {
    ...
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } };
    const result = await initSessionState({ ctx: { Body: "Hello topic", SessionKey: "agent:main:telegram:group:123:topic:456", MessageThreadId: 456 }, cfg, commandAuthorized: true });
    const sessionFile = result.sessionEntry.sessionFile;
    expect(sessionFile).toBeTruthy();
    expect(path.basename((sessionFile ?? ""))).toBe("-topic-456.jsonl");
  });
});
...
[from: Jake McInteer (+6421807830)]", RawBody: "/status", ChatType: "group", SessionKey: "agent:main:whatsapp:group:g1" };
    const result = await initSessionState({ ctx: groupMessageCtx, cfg, commandAuthorized: true });
...
Jake: /new
[from: Jake]", RawBody: "/new", ChatType: "group", SessionKey: "agent:main:whatsapp:group:g1" };
    const result = await initSessionState({ ctx: groupMessageCtx, cfg, commandAuthorized: true });
  ...
  it("preserves argument casing while still matching reset triggers case-insensitively", async () => {
    ...
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath, resetTriggers: ["/new"] } };
    const ctx = { RawBody: "/NEW KeepThisCase", ChatType: "direct", SessionKey: "agent:main:whatsapp:dm:s1" };
    const result = await initSessionState({ ctx, cfg, commandAuthorized: true });
    expect(result.isNewSession).toBe(true);
  ...
  it("falls back to Body when RawBody is undefined", async () => {
    ...
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } };
    const ctx = { Body: "/status", SessionKey: "agent:main:whatsapp:dm:s1" };
    const result = await initSessionState({ ctx, cfg, commandAuthorized: true });
    expect(result.triggerBodyNormalized).toBe("/status");
  });
});
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 18, 3, 0, 0).getTime() } });
        const cfg = { session: { store: storePath } };
        const result = await initSessionState({ ctx: { Body: "hello", SessionKey: sessionKey }, cfg, commandAuthorized: true });
        expect(result.isNewSession).toBe(true);
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 17, 3, 30, 0).getTime() } });
        const cfg = { session: { store: storePath } };
        const result = await initSessionState({ ctx: { Body: "hello", SessionKey: sessionKey }, cfg, commandAuthorized: true });
        expect(result.isNewSession).toBe(true);
        expect(result.sessionId).not.toBe(existingSessionId);
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 18, 4, 45, 0).getTime() } });
        const cfg = { session: { store: storePath, reset: { mode: "daily", atHour: 4, idleMinutes: 30 } } };
        const result = await initSessionState({ ctx: { Body: "hello", SessionKey: sessionKey }, cfg, commandAuthorized: true });
        expect(result.isNewSession).toBe(true);
        expect(result.sessionId).not.toBe(existingSessionId);
      }
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 18, 3, 0, 0).getTime() } });
        const cfg = { session: { store: storePath, reset: { mode: "daily", atHour: 4 }, resetByType: { thread: { mode: "idle", idleMinutes: 180 } } } };
        const result = await initSessionState({ ctx: { Body: "reply", SessionKey: sessionKey, ThreadLabel: "Slack thread" }, cfg, commandAuthorized: true });
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 18, 3, 0, 0).getTime() } });
        const cfg = { session: { store: storePath, resetByType: { thread: { mode: "idle", idleMinutes: 180 } } } };
        const result = await initSessionState({ ctx: { Body: "reply", SessionKey: sessionKey, ThreadLabel: "Discord thread" }, cfg, commandAuthorized: true });
        expect(result.isNewSession).toBe(false);
        expect(result.sessionId).toBe(existingSessionId);
      }
    }
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 18, 3, 0, 0).getTime() } });
        const cfg = { session: { store: storePath, resetByType: { thread: { mode: "idle", idleMinutes: 60 } } } };
        const result = await initSessionState({ ctx: { Body: "hello", SessionKey: sessionKey }, cfg, commandAuthorized: true });
        expect(result.isNewSession).toBe(true);
        expect(result.sessionId).not.toBe(existingSessionId);
      }
    ...
    try {
      {
        ...
        await saveSessionStore(storePath, { [sessionKey]: { sessionId: existingSessionId, updatedAt: new Date(2026, 0, 18, 3, 30, 0).getTime() } });
        const cfg = { session: { store: storePath, idleMinutes: 240 } };
        const result = await initSessionState({ ctx: { Body: "hello", SessionKey: sessionKey }, cfg, commandAuthorized: true });
        expect(result.isNewSession).toBe(false);
        expect(result.sessionId).toBe(existingSessionId);
      }
...
describe("initSessionState channel reset overrides", () => {
  it("uses channel-specific reset policy when configured", async () => {
    ...
    const updatedAt = (Date.now() - ((10080 - 1) * 60000));
    await saveSessionStore(storePath, { [sessionKey]: { sessionId, updatedAt } });
    const cfg = { session: { store: storePath, idleMinutes: 60, resetByType: { dm: { mode: "idle", idleMinutes: 10 } }, resetByChannel: { discord: { mode: "idle", idleMinutes: 10080 } } } };
    const result = await initSessionState({ ctx: { Body: "Hello", SessionKey: sessionKey, Provider: "discord" }, cfg, commandAuthorized: true });
    expect(result.isNewSession).toBe(false);
