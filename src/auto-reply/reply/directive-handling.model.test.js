...
vi.mock("../../agents/agent-scope.js", () => { resolveAgentConfig: vi.fn(() => {  }), resolveAgentDir: vi.fn(() => "/tmp/agent"), resolveSessionAgentId: vi.fn(() => "main") });
vi.mock("../../agents/sandbox.js", () => { resolveSandboxRuntimeStatus: vi.fn(() => { sandboxed: false }) });
vi.mock("../../config/sessions.js", () => { updateSessionStore: vi.fn(async () => {
}) });
...
function baseConfig() {
  return { commands: { text: true }, agents: { defaults: {  } } };
}
describe("/model chat UX", () => {
  it("shows summary for /model with no args", async () => {
    ...
    const reply = await maybeHandleModelDirectiveInfo({ directives, cfg, agentDir: "/tmp/agent", activeAgentId: "main", provider: "anthropic", model: "claude-opus-4-5", defaultProvider: "anthropic", defaultModel: "claude-opus-4-5", aliasIndex: baseAliasIndex(), allowedModelCatalog: [], resetModelOverride: false });
  ...
  it("auto-applies closest match for typos", () => {
    ...
    const cfg = { commands: { text: true } };
    const resolved = resolveModelSelectionFromDirective({ directives, cfg, agentDir: "/tmp/agent", defaultProvider: "anthropic", defaultModel: "claude-opus-4-5", aliasIndex: baseAliasIndex(), allowedModelKeys: new Set(["anthropic/claude-opus-4-5"]), allowedModelCatalog: [{ provider: "anthropic", id: "claude-opus-4-5" }], provider: "anthropic" });
    expect(resolved.modelSelection).toEqual({ provider: "anthropic", model: "claude-opus-4-5", isDefault: true });
...
describe("handleDirectiveOnly model persist behavior (fixes #1435)", () => {
  ...
  const allowedModelCatalog = [{ provider: "anthropic", id: "claude-opus-4-5" }, { provider: "openai", id: "gpt-4o" }];
