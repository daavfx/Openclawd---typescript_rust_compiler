...
export const handleBashCommand = async (params, allowTextCommands) => {
  ...
  if (!command.isAuthorizedSender) {
    logVerbose("Ignoring /bash from unauthorized sender: ");
    return { shouldContinue: false };
  }
  const reply = await handleBashChatCommand({ ctx: params.ctx, cfg: params.cfg, agentId: params.agentId, sessionKey: params.sessionKey, isGroup: params.isGroup, elevated: params.elevated });
