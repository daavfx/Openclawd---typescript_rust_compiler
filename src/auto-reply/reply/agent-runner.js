...
export async function runReplyAgent(params) {
  ...
  const blockReplyCoalescing = (blockStreamingEnabled && opts?.onBlockReply) ? resolveBlockStreamingCoalescing(cfg, sessionCtx.Provider, sessionCtx.AccountId, blockReplyChunking) : undefined;
  ...
  activeSessionEntry = await runMemoryFlushIfNeeded({ cfg, followupRun, sessionCtx, opts, defaultModel, agentCfgContextTokens, resolvedVerboseLevel, sessionEntry: activeSessionEntry, sessionStore: activeSessionStore, sessionKey, storePath, isHeartbeat });
  const runFollowupTurn = createFollowupRunner({ opts, typing, typingMode, sessionEntry: activeSessionEntry, sessionStore: activeSessionStore, sessionKey, storePath, defaultModel, agentCfgContextTokens });
  ...
  const resetSession = async ({failureLabel, buildLogMessage, cleanupTranscripts}) => {
    ...
    const nextEntry = { ...prevEntry: , sessionId: nextSessionId, updatedAt: Date.now(), systemSent: false, abortedLastRun: false };
    ...
    try {
      ...
      {
        defaultRuntime.error("Failed to persist session reset after  (): ");
      }
    }
  ...
  const resetSessionAfterCompactionFailure = async (reason) => resetSession({ failureLabel: "compaction failure", buildLogMessage: (nextSessionId) => "Auto-compaction failed (). Restarting session  ->  and retrying." });
  const resetSessionAfterRoleOrderingConflict = async (reason) => resetSession({ failureLabel: "role ordering conflict", buildLogMessage: (nextSessionId) => "Role ordering conflict (). Restarting session  -> .", cleanupTranscripts: true });
  try {
    {
      const runStartedAt = Date.now();
      const runOutcome = await runAgentTurnWithFallback({ commandBody, followupRun, sessionCtx, opts, typingSignals, blockReplyPipeline, blockStreamingEnabled, blockReplyChunking, resolvedBlockStreamingBreak, applyReplyToMode, shouldEmitToolResult, shouldEmitToolOutput, pendingToolTasks, resetSessionAfterCompactionFailure, resetSessionAfterRoleOrderingConflict, isHeartbeat, sessionKey, getActiveSessionEntry: () => activeSessionEntry, activeSessionStore, storePath, resolvedVerboseLevel });
      ...
      await persistSessionUsageUpdate({ storePath, sessionKey, usage, modelUsed, providerUsed, contextTokensUsed, systemPromptReport: runResult.meta.systemPromptReport, cliSessionId });
      if ((payloadArray.length === 0)) {
        return finalizeWithFollowup(undefined, queueKey, runFollowupTurn);
      }
      const payloadResult = buildReplyPayloads({ payloads: payloadArray, isHeartbeat, didLogHeartbeatStrip, blockStreamingEnabled, blockReplyPipeline, directlySentBlockKeys, replyToMode, replyToChannel, currentMessageId: (sessionCtx.MessageSidFull ?? sessionCtx.MessageSid), messageProvider: followupRun.run.messageProvider, messagingToolSentTexts: runResult.messagingToolSentTexts, messagingToolSentTargets: runResult.messagingToolSentTargets, originatingTo: (sessionCtx.OriginatingTo ?? sessionCtx.To), accountId: sessionCtx.AccountId });
      ...
      if ((isDiagnosticsEnabled(cfg) && hasNonzeroUsage(usage))) {
        ...
        const costConfig = resolveModelCostConfig({ provider: providerUsed, model: modelUsed, config: cfg });
        const costUsd = estimateUsageCost({ usage, cost: costConfig });
        emitDiagnosticEvent({ type: "model.usage", sessionKey, sessionId: followupRun.run.sessionId, channel: replyToChannel, provider: providerUsed, model: modelUsed, usage: { input, output, cacheRead, cacheWrite, promptTokens, total: totalTokens }, context: { limit: contextTokensUsed, used: totalTokens }, costUsd, durationMs: (Date.now() - runStartedAt) });
      }
      const responseUsageRaw = (activeSessionEntry?.responseUsage ?? sessionKey ? activeSessionStore?.[sessionKey]?.responseUsage : undefined);
      ...
      if (autoCompactionCompleted) {
        ...
        if (verboseEnabled) {
          ...
          finalPayloads = [{ text: "🧹 Auto-compaction complete." }, ...finalPayloads];
        }
      }
      if ((verboseEnabled && activeIsNewSession)) {
        finalPayloads = [{ text: "🧭 New session: " }, ...finalPayloads];
      }
