...
function normalizeAllowFrom(params) {
  ...
  if (dock?.config?.formatAllowFrom) {
    return dock.config.formatAllowFrom({ cfg: params.cfg, accountId: params.accountId, allowFrom: params.values });
  }
...
function resolveAccountTarget(parsed, channelId, accountId) {
  const channels = parsed.channels = (parsed.channels ?? {  });
  const channel = channels[channelId] = (channels[channelId] ?? {  });
  const normalizedAccountId = normalizeAccountId(accountId);
  const hasAccounts = Boolean((channel.accounts && (typeof channel.accounts === "object")));
  const useAccount = ((normalizedAccountId !== DEFAULT_ACCOUNT_ID) || hasAccounts);
  if (!useAccount) {
    return { target: channel, pathPrefix: "channels.", accountId: normalizedAccountId };
  }
  const accounts = channel.accounts = (channel.accounts ?? {  });
  const account = accounts[normalizedAccountId] = (accounts[normalizedAccountId] ?? {  });
  return { target: account, pathPrefix: "channels..accounts.", accountId: normalizedAccountId };
}
...
export const handleAllowlistCommand = async (params, allowTextCommands) => {
  ...
  if (!params.command.isAuthorizedSender) {
    logVerbose("Ignoring /allowlist from unauthorized sender: ");
  ...
  if (!channelId) {
    return { shouldContinue: false, reply: { text: "⚠️ Unknown channel. Add channel=<id> to the command." } };
  }
  ...
  if ((parsed.action === "list")) {
      ...
      for (const [groupId, groupCfg] of Object.entries(groups)) {
        ...
        for (const [topicId, topicCfg] of Object.entries(topics)) {
          ...
          if ((topicEntries.length > 0)) {
            groupOverrides.push({ label: " topic ", entries: topicEntries });
          }
        }
      }
    } else {
            ...
            if ((channelId === "slack")) {
              ...
              if ((channelId === "discord")) {
                ...
                for (const [guildKey, guildCfg] of Object.entries(guilds)) {
                  ...
                  if ((entries.length > 0)) {
                    groupOverrides.push({ label: "guild ", entries });
                  }
                  ...
                  for (const [channelKey, channelCfg] of Object.entries(channels)) {
                    ...
                    if ((channelEntries.length > 0)) {
                      groupOverrides.push({ label: "guild  / channel ", entries: channelEntries });
                    }
                  }
                }
              }
    ...
    const dmDisplay = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId, values: dmAllowFrom });
    const groupDisplay = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId, values: groupAllowFrom });
    const groupOverrideEntries = groupOverrides.flatMap((entry) => entry.entries);
    const groupOverrideDisplay = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId, values: groupOverrideEntries });
    const resolvedDm = ((parsed.resolve && (dmDisplay.length > 0)) && (channelId === "slack")) ? await resolveSlackNames({ cfg: params.cfg, accountId, entries: dmDisplay }) : ((parsed.resolve && (dmDisplay.length > 0)) && (channelId === "discord")) ? await resolveDiscordNames({ cfg: params.cfg, accountId, entries: dmDisplay }) : undefined;
    const resolvedGroup = ((parsed.resolve && (groupOverrideDisplay.length > 0)) && (channelId === "slack")) ? await resolveSlackNames({ cfg: params.cfg, accountId, entries: groupOverrideDisplay }) : ((parsed.resolve && (groupOverrideDisplay.length > 0)) && (channelId === "discord")) ? await resolveDiscordNames({ cfg: params.cfg, accountId, entries: groupOverrideDisplay }) : undefined;
    const lines = ["🧾 Allowlist"];
    lines.push("Channel: ");
    if (dmPolicy) {
      lines.push("DM policy: ");
    }
    if (groupPolicy) {
      lines.push("Group policy: ");
    }
    const showDm = ((scope === "dm") || (scope === "all"));
    const showGroup = ((scope === "group") || (scope === "all"));
    if (showDm) {
      lines.push("DM allowFrom (config): ");
    }
    if ((supportsStore && (storeAllowFrom.length > 0))) {
      const storeLabel = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId, values: storeAllowFrom });
      lines.push("Paired allowFrom (store): ");
    }
    if (showGroup) {
      if ((groupAllowFrom.length > 0)) {
        lines.push("Group allowFrom (config): ");
      }
      if ((groupOverrides.length > 0)) {
        lines.push("Group overrides:");
        for (const entry of groupOverrides) {
          const normalized = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId, values: entry.entries });
          lines.push("- : ");
        }
      }
    }
...
") } };
  ...
  if ((params.cfg.commands?.config !== true)) {
    return { shouldContinue: false, reply: { text: "⚠️ /allowlist edits are disabled. Set commands.config=true to enable." } };
  }
  ...
  if (shouldUpdateConfig) {
    ...
    if (!allowWrites) {
      const hint = "channels..configWrites=true";
    ...
    if (!allowlistPath) {
      return { shouldContinue: false, reply: { text: "⚠️  does not support  allowlist edits via /allowlist." } };
    }
    const snapshot = await readConfigFileSnapshot();
    if (((!snapshot.valid || !snapshot.parsed) || (typeof snapshot.parsed !== "object"))) {
      return { shouldContinue: false, reply: { text: "⚠️ Config file is invalid; fix it before using /allowlist." } };
    }
    ...
    const normalizedEntry = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId: normalizedAccountId, values: [parsed.entry] });
    if ((normalizedEntry.length === 0)) {
      return { shouldContinue: false, reply: { text: "⚠️ Invalid allowlist entry." } };
    }
    const existingNormalized = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId: normalizedAccountId, values: existing });
    ...
    if ((parsed.action === "remove")) {
      ...
      for (const entry of existing) {
        const normalized = normalizeAllowFrom({ cfg: params.cfg, channelId, accountId: normalizedAccountId, values: [entry] });
    ...
    if (configChanged) {
      ...
      if (!validated.ok) {
        ...
        return { shouldContinue: false, reply: { text: "⚠️ Config invalid after update (: )." } };
      }
    ...
    return { shouldContinue: false, reply: { text: "✅  allowlist : ." } };
  }
  if (!shouldTouchStore) {
  ...
  const actionLabel = (parsed.action === "add") ? "added" : "removed";
  const scopeLabel = (scope === "dm") ? "DM" : "group";
  return { shouldContinue: false, reply: { text: "✅  allowlist  in pairing store." } };
}
