...
export async function resolveReplyDirectives(params) {
  ...
  const command = buildCommandContext({ ctx, cfg, agentId, sessionKey, isGroup, triggerBodyNormalized, commandAuthorized });
  ...
  const skillCommands = shouldResolveSkillCommands ? listSkillCommandsForWorkspace({ workspaceDir, cfg, skillFilter }) : [];
  ...
  let parsedDirectives = parseInlineDirectives(commandText, { modelAliases: configuredAliases, allowStatusDirective });
  ...
  if (((isGroup && (ctx.WasMentioned !== true)) && parsedDirectives.hasElevatedDirective)) {
    if ((parsedDirectives.elevatedLevel !== "off")) {
      parsedDirectives = { ...parsedDirectives: , hasElevatedDirective: false, elevatedLevel: undefined, rawElevatedLevel: undefined };
    }
  }
  if (((isGroup && (ctx.WasMentioned !== true)) && parsedDirectives.hasExecDirective)) {
    if ((parsedDirectives.execSecurity !== "deny")) {
      parsedDirectives = { ...parsedDirectives: , hasExecDirective: false, execHost: undefined, execSecurity: undefined, execAsk: undefined, execNode: undefined, rawExecHost: undefined, rawExecSecurity: undefined, rawExecAsk: undefined, rawExecNode: undefined, hasExecOptions: false, invalidExecHost: false, invalidExecSecurity: false, invalidExecAsk: false, invalidExecNode: false };
    }
  }
  ...
  if (hasInlineDirective) {
    ...
    if ((noMentions.trim().length > 0)) {
      const directiveOnlyCheck = parseInlineDirectives(noMentions, { modelAliases: configuredAliases });
      if ((directiveOnlyCheck.cleaned.trim().length > 0)) {
        const allowInlineStatus = ((parsedDirectives.hasStatusDirective && allowTextCommands) && command.isAuthorizedSender);
        parsedDirectives = allowInlineStatus ? { ...clearInlineDirectives(parsedDirectives.cleaned): , hasStatusDirective: true } : clearInlineDirectives(parsedDirectives.cleaned);
      }
    }
  }
  let directives = commandAuthorized ? parsedDirectives : { ...parsedDirectives: , hasThinkDirective: false, hasVerboseDirective: false, hasReasoningDirective: false, hasStatusDirective: false, hasModelDirective: false, hasQueueDirective: false, queueReset: false };
  ...
  let cleanedBody = () => {
    ...
    if ((!sessionCtx.CommandBody && !sessionCtx.RawBody)) {
      return parseInlineDirectives(existingBody, { modelAliases: configuredAliases, allowStatusDirective }).cleaned;
    }
    ...
    const cleanedTail = parseInlineDirectives(tail, { modelAliases: configuredAliases, allowStatusDirective }).cleaned;
    return "";
  }();
  ...
  const elevated = resolveElevatedPermissions({ cfg, agentId, ctx, provider: messageProviderKey });
  ...
  if ((directives.hasElevatedDirective && (!elevatedEnabled || !elevatedAllowed))) {
    typing.cleanup();
    const runtimeSandboxed = resolveSandboxRuntimeStatus({ cfg, sessionKey: ctx.SessionKey }).sandboxed;
    return { kind: "reply", reply: { text: formatElevatedUnavailableMessage({ runtimeSandboxed, failures: elevatedFailures, sessionKey: ctx.SessionKey }) } };
  }
  const requireMention = resolveGroupRequireMention({ cfg, ctx: sessionCtx, groupResolution });
  ...
  const blockReplyChunking = blockStreamingEnabled ? resolveBlockStreamingChunking(cfg, sessionCtx.Provider, sessionCtx.AccountId) : undefined;
  const modelState = await createModelSelectionState({ cfg, agentCfg, sessionEntry, sessionStore, sessionKey, parentSessionKey: ctx.ParentSessionKey, storePath, defaultProvider, defaultModel, provider, model, hasModelDirective: directives.hasModelDirective });
  provider = modelState.provider;
  model = modelState.model;
  let contextTokens = resolveContextTokens({ agentCfg, model });
  const initialModelLabel = "/";
  const formatModelSwitchEvent = (label, alias) => alias ? "Model switched to  ()." : "Model switched to .";
  ...
  const applyResult = await applyInlineDirectiveOverrides({ ctx, cfg, agentId, agentDir, agentCfg, sessionEntry, sessionStore, sessionKey, storePath, sessionScope, isGroup, allowTextCommands, command, directives, messageProviderKey, elevatedEnabled, elevatedAllowed, elevatedFailures, defaultProvider, defaultModel, aliasIndex: params.aliasIndex, provider, model, modelState, initialModelLabel, formatModelSwitchEvent, resolvedElevatedLevel, defaultActivation: () => defaultActivation, contextTokens, effectiveModelDirective, typing });
