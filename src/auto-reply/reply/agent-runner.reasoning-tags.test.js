...
vi.mock("../../agents/model-fallback.js", () => { runWithModelFallback: (params) => runWithModelFallbackMock(params) });
...
describe("runReplyAgent fallback reasoning tags", () => {
  ...
  it("enforces <final> when the fallback provider requires reasoning tags", async () => {
    ...
    runWithModelFallbackMock.mockImplementationOnce(async ({run}) => { result: await run("google-antigravity", "gemini-3"), provider: "google-antigravity", model: "gemini-3" });
    await createRun();
  ...
  it("enforces <final> during memory flush on fallback providers", async () => {
    ...
    runWithModelFallbackMock.mockImplementation(async ({run}) => { result: await run("google-antigravity", "gemini-3"), provider: "google-antigravity", model: "gemini-3" });
    await createRun({ sessionEntry: { sessionId: "session", updatedAt: Date.now(), totalTokens: 1000000, compactionCount: 0 } });
    const flushCall = runEmbeddedPiAgentMock.mock.calls.find(([params]) => (params?.prompt === DEFAULT_MEMORY_FLUSH_PROMPT))?.[0];
