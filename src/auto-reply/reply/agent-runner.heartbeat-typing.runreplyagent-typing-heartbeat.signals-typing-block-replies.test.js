...
describe("runReplyAgent typing (heartbeat)", () => {
  it("signals typing on block replies", async () => {
    ...
    const {run, typing} = createMinimalRun({ typingMode: "message", blockStreamingEnabled: true, opts: { onBlockReply } });
    await run();
    ...
    expect(blockOpts).toMatchObject({ abortSignal: expect.any(AbortSignal), timeoutMs: expect.any(Number) });
  });
  it("signals typing on tool results", async () => {
    ...
    const {run, typing} = createMinimalRun({ typingMode: "message", opts: { onToolResult } });
    await run();
  ...
  it("announces auto-compaction in verbose mode and tracks count", async () => {
    ...
    const {run} = createMinimalRun({ resolvedVerboseLevel: "on", sessionEntry, sessionStore, sessionKey: "main", storePath });
    const res = await run();
