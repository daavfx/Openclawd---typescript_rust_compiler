...
function createRun() {
  ...
  const sessionCtx = { Provider: "webchat", OriginatingTo: "session:1", AccountId: "primary", MessageSid: "msg" };
...
describe("runReplyAgent claude-cli routing", () => {
  it("uses claude-cli runner for claude-cli provider", async () => {
    ...
    runCliAgentMock.mockResolvedValueOnce({ payloads: [{ text: "ok" }], meta: { agentMeta: { provider: "claude-cli", model: "opus-4.5" } } });
