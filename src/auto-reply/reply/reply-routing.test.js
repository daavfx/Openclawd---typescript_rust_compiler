...
describe("createReplyDispatcher", () => {
  it("drops empty payloads and silent tokens without media", async () => {
    ...
    expect(dispatcher.sendFinalReply({ text: " -- nope" })).toBe(false);
    expect(dispatcher.sendFinalReply({ text: "interject." })).toBe(false);
  ...
  it("strips heartbeat tokens and applies responsePrefix", async () => {
    ...
    const dispatcher = createReplyDispatcher({ deliver, responsePrefix: "PFX", onHeartbeatStrip });
  ...
  it("avoids double-prefixing and keeps media when heartbeat is the only text", async () => {
    const deliver = vi.fn().mockResolvedValue(undefined);
    const dispatcher = createReplyDispatcher({ deliver, responsePrefix: "PFX" });
    expect(dispatcher.sendFinalReply({ text: "PFX already", mediaUrl: "file:///tmp/photo.jpg" })).toBe(true);
    expect(dispatcher.sendFinalReply({ text: HEARTBEAT_TOKEN, mediaUrl: "file:///tmp/photo.jpg" })).toBe(true);
    expect(dispatcher.sendFinalReply({ text: " -- explanation", mediaUrl: "file:///tmp/photo.jpg" })).toBe(true);
    await dispatcher.waitForIdle();
  ...
  it("delays block replies after the first when humanDelay is natural", async () => {
    ...
    const deliver = vi.fn().mockResolvedValue(undefined);
    const dispatcher = createReplyDispatcher({ deliver, humanDelay: { mode: "natural" } });
  ...
  it("uses custom bounds for humanDelay and clamps when max <= min", async () => {
    vi.useFakeTimers();
    const deliver = vi.fn().mockResolvedValue(undefined);
    const dispatcher = createReplyDispatcher({ deliver, humanDelay: { mode: "custom", minMs: 1200, maxMs: 400 } });
    dispatcher.sendBlockReply({ text: "first" });
    await Promise.resolve();
    expect(deliver).toHaveBeenCalledTimes(1);
    dispatcher.sendBlockReply({ text: "second" });
...
describe("resolveReplyToMode", () => {
  ...
  it("uses configured value when present", () => {
    const cfg = { channels: { telegram: { replyToMode: "all" }, discord: { replyToMode: "first" }, slack: { replyToMode: "all" } } };
  ...
  it("uses chat-type replyToMode overrides for Slack when configured", () => {
    const cfg = { channels: { slack: { replyToMode: "off", replyToModeByChatType: { direct: "all", group: "first" } } } };
  ...
  it("falls back to top-level replyToMode when no chat-type override is set", () => {
    const cfg = { channels: { slack: { replyToMode: "first" } } };
  ...
  it("uses legacy dm.replyToMode for direct messages when no chat-type override exists", () => {
    const cfg = { channels: { slack: { replyToMode: "off", dm: { replyToMode: "all" } } } };
    expect(resolveReplyToMode(cfg, "slack", null, "direct")).toBe("all");
    expect(resolveReplyToMode(cfg, "slack", null, "channel")).toBe("off");
  });
});
