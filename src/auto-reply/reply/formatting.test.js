...
describe("block reply coalescer", () => {
  ...
  it("coalesces chunks within the idle window", async () => {
    vi.useFakeTimers();
    const flushes = [];
    const coalescer = createBlockReplyCoalescer({ config: { minChars: 1, maxChars: 200, idleMs: 100, joiner: " " }, shouldAbort: () => false, onFlush: (payload) => {
  ...
  it("waits until minChars before idle flush", async () => {
    vi.useFakeTimers();
    const flushes = [];
    const coalescer = createBlockReplyCoalescer({ config: { minChars: 10, maxChars: 200, idleMs: 50, joiner: " " }, shouldAbort: () => false, onFlush: (payload) => {
      flushes.push((payload.text ?? ""));
    } });
  ...
  it("flushes buffered text before media payloads", () => {
    const flushes = [];
    const coalescer = createBlockReplyCoalescer({ config: { minChars: 1, maxChars: 200, idleMs: 0, joiner: " " }, shouldAbort: () => false, onFlush: (payload) => {
      flushes.push({ text: payload.text, mediaUrls: payload.mediaUrls });
    } });
    coalescer.enqueue({ text: "Hello" });
    coalescer.enqueue({ text: "world" });
...
describe("createReplyReferencePlanner", () => {
  it("disables references when mode is off", () => {
    const planner = createReplyReferencePlanner({ replyToMode: "off", startId: "parent" });
    expect(planner.use()).toBeUndefined();
    expect(planner.hasReplied()).toBe(false);
  });
  it("uses startId once when mode is first", () => {
    const planner = createReplyReferencePlanner({ replyToMode: "first", startId: "parent" });
  ...
  it("returns startId for every call when mode is all", () => {
    const planner = createReplyReferencePlanner({ replyToMode: "all", startId: "parent" });
    expect(planner.use()).toBe("parent");
    expect(planner.use()).toBe("parent");
  });
  it("prefers existing thread id regardless of mode", () => {
    const planner = createReplyReferencePlanner({ replyToMode: "off", existingId: "thread-1", startId: "parent" });
    expect(planner.use()).toBe("thread-1");
    expect(planner.hasReplied()).toBe(true);
  });
  it("honors allowReference=false", () => {
    const planner = createReplyReferencePlanner({ replyToMode: "all", startId: "parent", allowReference: false });
    expect(planner.use()).toBeUndefined();
    expect(planner.hasReplied()).toBe(false);
    planner.markSent();
    expect(planner.hasReplied()).toBe(true);
