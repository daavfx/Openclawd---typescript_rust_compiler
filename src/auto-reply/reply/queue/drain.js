        ...
        while (((queue.items.length > 0) || (queue.droppedCount > 0))) {
          ...
          if ((queue.mode === "collect")) {
            ...
            const isCrossChannel = hasCrossChannelItems(queue.items, (item) => {
              ...
              return { key: [channel, to, (accountId || ""), threadKey].join("|") };
            });
            if (isCrossChannel) {
...
Queued #
".trim() });
            await runFollowup({ prompt, run, enqueuedAt: Date.now(), originatingChannel, originatingTo, originatingAccountId, originatingThreadId });
            continue;
          }
          ...
          if (summaryPrompt) {
            ...
            await runFollowup({ prompt: summaryPrompt, run, enqueuedAt: Date.now() });
            continue;
          }
          const next = queue.items.shift();
          if (!next) {
      ...
      {
        defaultRuntime.error?.("followup queue drain failed for : ");
      }
    }
    finally {
      {
