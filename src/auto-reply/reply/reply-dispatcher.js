...
function normalizeReplyPayloadInternal(payload, opts) {
  ...
  return normalizeReplyPayload(payload, { responsePrefix: opts.responsePrefix, responsePrefixContext: prefixContext, onHeartbeatStrip: opts.onHeartbeatStrip, onSkip: opts.onSkip });
}
export function createReplyDispatcher(options) {
  ...
  return { sendToolResult: (payload) => enqueue("tool", payload), sendBlockReply: (payload) => enqueue("block", payload), sendFinalReply: (payload) => enqueue("final", payload), waitForIdle: () => sendChain, getQueuedCounts: () => { ...queuedCounts:  } };
}
...
export function createReplyDispatcherWithTyping(options) {
  ...
  const dispatcher = createReplyDispatcher({ ...dispatcherOptions: , onIdle: () => {
    typingController?.markDispatchIdle();
    onIdle?.();
  } });
  return { dispatcher, replyOptions: { onReplyStart, onTypingController: (typing) => {
