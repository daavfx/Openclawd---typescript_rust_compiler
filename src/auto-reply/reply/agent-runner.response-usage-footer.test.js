...
function createRun(params) {
  ...
  const sessionEntry = { sessionId: "session", updatedAt: Date.now(), responseUsage: params.responseUsage };
...
describe("runReplyAgent response usage footer", () => {
  ...
  it("appends session key when responseUsage=full", async () => {
    runEmbeddedPiAgentMock.mockResolvedValueOnce({ payloads: [{ text: "ok" }], meta: { agentMeta: { provider: "anthropic", model: "claude", usage: { input: 12, output: 3 } } } });
    runWithModelFallbackMock.mockImplementationOnce(async ({run}) => { result: await run("anthropic", "claude"), provider: "anthropic", model: "claude" });
    ...
    expect(String((payload?.text ?? ""))).toContain("· session ");
  });
