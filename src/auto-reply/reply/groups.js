...
export function resolveGroupRequireMention(params) {
  ...
  const requireMention = getChannelDock(channel)?.groups?.resolveRequireMention?.({ cfg, groupId, groupChannel, groupSpace, accountId: ctx.AccountId });
...
export function buildGroupIntro(params) {
  ...
  const subjectLine = subject ? "You are replying inside the  group \"\"." : "You are replying inside a  group chat.";
  const membersLine = members ? "Group members: ." : undefined;
  ...
  const providerIdsLine = providerId ? getChannelDock(providerId)?.groups?.resolveGroupIntroHint?.({ cfg: params.cfg, groupId, groupChannel, groupSpace, accountId: params.sessionCtx.AccountId }) : undefined;
  const silenceLine = (activation === "always") ? "If no response is needed, reply with exactly \"\" (and nothing else) so OpenClaw stays silent. Do not add any other words, punctuation, tags, markdown/code blocks, or explanations." : undefined;
  const cautionLine = (activation === "always") ? "Be extremely selective: reply only when directly addressed or clearly helpful. Otherwise stay silent." : undefined;
  ...
  return [subjectLine, membersLine, activationLine, providerIdsLine, silenceLine, cautionLine, lurkLine, styleLine].filter(Boolean).join(" ").concat(" Address the specific sender noted in the message context.");
}
