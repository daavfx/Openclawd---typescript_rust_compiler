...
const splitTrailingDirective = (text) => {
  ...
  return { text: text.slice(0, openIndex), tail: text.slice(openIndex) };
};
const parseChunk = (raw, options) => {
  const split = splitMediaFromOutput(raw);
  let text = (split.text ?? "");
  const replyParsed = parseInlineDirectives(text, { stripAudioTag: false, stripReplyTags: true });
  ...
  return { text, mediaUrls: split.mediaUrls, mediaUrl: split.mediaUrl, replyToId: replyParsed.replyToId, replyToExplicitId: replyParsed.replyToExplicitId, replyToCurrent: replyParsed.replyToCurrent, replyToTag: replyParsed.hasReplyTag, audioAsVoice: split.audioAsVoice, isSilent };
};
...
export function createStreamingDirectiveAccumulator() {
  ...
  const consume = (raw, options = {  }) => {
    ...
    const combinedResult = { ...parsed: , replyToId: explicitId, replyToCurrent: sawCurrent, replyToTag: hasTag };
    if (!hasRenderableContent(combinedResult)) {
      if (hasTag) {
        pendingReply = { explicitId, sawCurrent, hasTag };
      }
      return null;
    }
    pendingReply = { sawCurrent: false, hasTag: false };
