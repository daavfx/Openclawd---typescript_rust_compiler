...
export const resolveAuthLabel = async (provider, cfg, modelsPath, agentDir, mode = "compact") => {
  ...
  if ((order.length > 0)) {
    if ((mode === "compact")) {
      ...
      if (missing) {
        return { label: " missing", source: "" };
      }
      if ((profile.type === "api_key")) {
        return { label: " api-key ", source: "" };
      }
      if ((profile.type === "token")) {
        const exp = (((typeof profile.expires === "number") && Number.isFinite(profile.expires)) && (profile.expires > 0)) ? (profile.expires <= now) ? " expired" : " exp " : "";
        return { label: " token ", source: "" };
      }
      const display = resolveAuthProfileDisplayLabel({ cfg, store, profileId });
      const label = (display === profileId) ? profileId : display;
      const exp = (((typeof profile.expires === "number") && Number.isFinite(profile.expires)) && (profile.expires > 0)) ? (profile.expires <= now) ? " expired" : " exp " : "";
      return { label: " oauth", source: "" };
    }
    const labels = order.map((profileId) => {
      ...
      if ((profile.type === "token")) {
        if ((((typeof profile.expires === "number") && Number.isFinite(profile.expires)) && (profile.expires > 0))) {
          flags.push((profile.expires <= now) ? "expired" : "exp ");
        }
        const suffix = (flags.length > 0) ? " ()" : "";
        return "=token:";
      }
      const display = resolveAuthProfileDisplayLabel({ cfg, store, profileId });
      const suffix = (display === profileId) ? "" : display.startsWith(profileId) ? display.slice(profileId.length).trim() : "()";
      if ((((typeof profile.expires === "number") && Number.isFinite(profile.expires)) && (profile.expires > 0))) {
        flags.push((profile.expires <= now) ? "expired" : "exp ");
      }
      const suffixLabel = suffix ? " " : "";
      const suffixFlags = (flags.length > 0) ? " ()" : "";
      return "=OAuth";
    });
    return { label: labels.join(", "), source: "auth-profiles.json: " };
  }
  ...
  if (customKey) {
    return { label: maskApiKey(customKey), source: (mode === "verbose") ? "models.json: " : "" };
  }
  return { label: "missing", source: "missing" };
}
...
export const resolveProfileOverride = (params) => {
  ...
  if (!profile) {
    return { error: "Auth profile \"\" not found." };
  }
  if ((profile.provider !== params.provider)) {
    return { error: "Auth profile \"\" is for , not ." };
  }
