...
function buildUsageReply() {
  return { text: ["⚙️ Usage:", "- ! <command>", "- !poll | ! poll", "- !stop | ! stop", "- /bash ... (alias; same subcommands as !)"].join("
") };
}
function formatElevatedUnavailableMessage(params) {
  const lines = [];
  lines.push("elevated is not available right now (runtime=).");
...
export async function handleBashChatCommand(params) {
  if ((params.cfg.commands?.bash !== true)) {
    return { text: "⚠️ bash is disabled. Set commands.bash=true to enable. Docs: https://docs.openclaw.ai/tools/slash-commands#config" };
  }
  const agentId = (params.agentId ?? resolveSessionAgentId({ sessionKey: params.sessionKey, config: params.cfg }));
  if ((!params.elevated.enabled || !params.elevated.allowed)) {
    const runtimeSandboxed = resolveSandboxRuntimeStatus({ cfg: params.cfg, sessionKey: params.ctx.SessionKey }).sandboxed;
    return { text: formatElevatedUnavailableMessage({ runtimeSandboxed, failures: params.elevated.failures, sessionKey: params.ctx.SessionKey }) };
  }
  const rawBody = resolveRawCommandBody({ ctx: params.ctx, cfg: params.cfg, agentId, isGroup: params.isGroup }).trim();
  ...
  if ((request.action === "poll")) {
    ...
    if (running) {
      ...
      return { text: ["⚙️ bash still running (session , s).", formatOutputBlock(tail), "Hint: !stop (or /bash stop)"].join("
") };
    }
    if (finished) {
      if (((activeJob?.state === "running") && (activeJob.sessionId === sessionId))) {
      ...
      const exitLabel = finished.exitSignal ? "signal " : "code ";
      const prefix = (finished.status === "completed") ? "⚙️" : "⚠️";
      return { text: [" bash finished (session ).", "Exit: ", formatOutputBlock((finished.aggregated || finished.tail))].join("
") };
    }
    if (((activeJob?.state === "running") && (activeJob.sessionId === sessionId))) {
      activeJob = null;
    }
    return { text: "⚙️ No bash session found for ." };
  }
  if ((request.action === "stop")) {
    ...
    if (!running) {
      ...
      return { text: "⚙️ No running bash job found for ." };
    }
    if (!running.backgrounded) {
      return { text: "⚠️ Session  is not backgrounded." };
    }
    ...
    return { text: "⚙️ bash stopped (session )." };
  }
  if (liveJob) {
    const label = (liveJob.state === "running") ? formatSessionSnippet(liveJob.sessionId) : "starting";
    return { text: "⚠️ A bash job is already running (). Use !poll / !stop (or /bash poll / /bash stop)." };
  }
  ...
  try {
    {
      ...
      const execTool = createExecTool({ scopeKey: CHAT_BASH_SCOPE_KEY, allowBackground: true, timeoutSec, sessionKey: params.sessionKey, notifyOnExit, elevated: { enabled: params.elevated.enabled, allowed: params.elevated.allowed, defaultLevel: "on" } });
      const result = await execTool.execute("chat-bash", { command: commandText, background: shouldBackgroundImmediately, yieldMs: shouldBackgroundImmediately ? undefined : foregroundMs, timeout: timeoutSec, elevated: true });
      if ((result.details?.status === "running")) {
        const sessionId = result.details.sessionId;
        activeJob = { state: "running", sessionId, startedAt: result.details.startedAt, command: commandText, watcherAttached: false };
        attachActiveWatcher(sessionId);
        const snippet = formatSessionSnippet(sessionId);
        logVerbose("Started bash session : ");
        return { text: "⚙️ bash started (session ). Still running; use !poll / !stop (or /bash poll / /bash stop)." };
      }
      activeJob = null;
...
");
      return { text: ["⚙️ bash: ", "Exit: ", formatOutputBlock((output || "(no output)"))].join("
") };
    }
  }
  catch (err) {
    {
      ...
      return { text: ["⚠️ bash failed: ", formatOutputBlock(message)].join("
") };
    }
  }
}
