...
export function createAudioAsVoiceBuffer(params) {
  ...
  return { onEnqueue: (payload) => {
    if (payload.audioAsVoice) {
      seenAudioAsVoice = true;
    }
  }, shouldBuffer: (payload) => params.isAudioPayload(payload), finalize: (payload) => seenAudioAsVoice ? { ...payload: , audioAsVoice: true } : payload };
}
...
export function createBlockReplyPipeline(params) {
  ...
  const sendPayload = (payload, skipSeen) => {
    ...
    const timeoutError = new Error("block reply delivery timed out after ms");
    const abortController = new AbortController();
    sendChain = sendChain.then(async () => {
      if (aborted) {
        return false;
      }
      await withTimeout((onBlockReply(payload, { abortSignal: abortController.signal, timeoutMs }) ?? Promise.resolve()), timeoutMs, timeoutError);
      return true;
    }).then((didSend) => {
      if (!didSend) {
        return;
      ...
      if ((err === timeoutError)) {
        ...
        if (!didLogTimeout) {
          didLogTimeout = true;
          logVerbose("block reply delivery timed out after ms; skipping remaining block replies to preserve ordering");
        }
        return;
      }
      logVerbose("block reply delivery failed: ");
    }).finally(() => {
      pendingKeys.delete(payloadKey);
    });
  };
  const coalescer = coalescing ? createBlockReplyCoalescer({ config: coalescing, shouldAbort: () => aborted, onFlush: (payload) => {
    bufferedKeys.clear();
    sendPayload(payload);
  } }) : null;
  ...
  return { enqueue, flush, stop, hasBuffered: () => Boolean((coalescer?.hasBuffered() || (bufferedPayloads.length > 0))), didStream: () => didStream, isAborted: () => aborted, hasSentPayload: (payload) => {
    const payloadKey = createBlockReplyPayloadKey(payload);
