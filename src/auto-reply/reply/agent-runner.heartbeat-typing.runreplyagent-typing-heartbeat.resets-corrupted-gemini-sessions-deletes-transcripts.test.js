...
describe("runReplyAgent typing (heartbeat)", () => {
    ...
    try {
      {
        ...
        const res = await run();
        expect(res).toMatchObject({ text: expect.stringContaining("Session history was corrupted") });
    ...
    try {
      {
        ...
        const {run} = createMinimalRun({ sessionEntry, sessionStore, sessionKey: "main", storePath });
        const res = await run();
        expect(res).toMatchObject({ text: expect.stringContaining("Agent failed before reply") });
  ...
  it("returns friendly message for role ordering errors thrown as exceptions", async () => {
    ...
    const res = await run();
    expect(res).toMatchObject({ text: expect.stringContaining("Message ordering conflict") });
    expect(res).toMatchObject({ text: expect.not.stringContaining("400") });
  });
