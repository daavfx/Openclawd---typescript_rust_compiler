...
export function createBlockReplyCoalescer(params) {
  ...
  const flush = async (options) => {
    ...
    const payload = { text: bufferText, replyToId: bufferReplyToId, audioAsVoice: bufferAudioAsVoice };
    resetBuffer();
  ...
  const enqueue = (payload) => {
    ...
    const nextText = bufferText ? "" : text;
  ...
  return { enqueue, flush, hasBuffered: () => Boolean(bufferText), stop: () => clearIdleTimer() };
}
