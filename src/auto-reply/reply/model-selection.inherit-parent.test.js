...
vi.mock("../../agents/model-catalog.js", () => { loadModelCatalog: vi.fn(async () => [{ provider: "openai", id: "gpt-4o-mini", name: "GPT-4o mini" }, { provider: "openai", id: "gpt-4o", name: "GPT-4o" }, { provider: "anthropic", id: "claude-opus-4-5", name: "Claude Opus 4.5" }]) });
const defaultProvider = "openai";
const defaultModel = "gpt-4o-mini";
const makeEntry = (overrides = {  }) => { sessionId: "session-id", updatedAt: Date.now(), ...overrides:  };
...
describe("createModelSelectionState parent inheritance", () => {
  it("inherits parent override from explicit parentSessionKey", async () => {
    ...
    const parentEntry = makeEntry({ providerOverride: "openai", modelOverride: "gpt-4o" });
    const sessionEntry = makeEntry();
    const sessionStore = { [parentKey]: parentEntry, [sessionKey]: sessionEntry };
    const state = await resolveState({ cfg, sessionEntry, sessionStore, sessionKey, parentSessionKey: parentKey });
  ...
  it("derives parent key from topic session suffix", async () => {
    ...
    const sessionEntry = makeEntry();
    const sessionStore = { [parentKey]: parentEntry, [sessionKey]: sessionEntry };
    const state = await resolveState({ cfg, sessionEntry, sessionStore, sessionKey });
    expect(state.provider).toBe("openai");
    expect(state.model).toBe("gpt-4o");
  });
  it("prefers child override over parent", async () => {
    ...
    const sessionKey = "agent:main:telegram:group:123:topic:99";
    const parentEntry = makeEntry({ providerOverride: "openai", modelOverride: "gpt-4o" });
    const sessionEntry = makeEntry({ providerOverride: "anthropic", modelOverride: "claude-opus-4-5" });
    const sessionStore = { [parentKey]: parentEntry, [sessionKey]: sessionEntry };
    const state = await resolveState({ cfg, sessionEntry, sessionStore, sessionKey });
  ...
  it("ignores parent override when disallowed", async () => {
    ...
    const parentEntry = makeEntry({ providerOverride: "anthropic", modelOverride: "claude-opus-4-5" });
    const sessionEntry = makeEntry();
    const sessionStore = { [parentKey]: parentEntry, [sessionKey]: sessionEntry };
    const state = await resolveState({ cfg, sessionEntry, sessionStore, sessionKey });
