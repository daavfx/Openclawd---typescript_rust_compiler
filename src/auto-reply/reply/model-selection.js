...
export 
const FUZZY_VARIANT_TOKENS = ["lightning", "preview", "mini", "fast", "turbo", "lite", "beta", "small", "nano"];
...
function resolveStoredModelOverride(params) {
  ...
  const parentKey = resolveParentSessionKeyCandidate({ sessionKey: params.sessionKey, parentSessionKey: params.parentSessionKey });
...
function scoreFuzzyMatch(params) {
  ...
  return { score, isDefault, variantCount, variantMatchCount, modelLength: modelLower.length, key };
}
...
export function resolveModelDirectiveSelection(params) {
  ...
  const buildSelection = (provider, model) => {
    ...
    return { provider, model, isDefault: ((provider === defaultProvider) && (model === defaultModel)), ...alias ? { alias } : undefined:  };
  };
  const resolveFuzzy = (params) => {
    ...
    const scored = candidates.map((candidate) => {
      const details = scoreFuzzyMatch({ provider: candidate.provider, model: candidate.model, fragment, aliasIndex, defaultProvider, defaultModel });
  ...
  if (!resolved) {
    ...
    return { error: "Unrecognized model \"\". Use /models to list providers, or /models <provider> to list models." };
  }
  ...
  return { error: "Model \"/\" is not allowed. Use /models to list providers, or /models <provider> to list models." };
}
