...
export function resolveBlockStreamingChunking(cfg, provider, accountId) {
  ...
  const providerChunkLimit = providerId ? getChannelDock(providerId)?.outbound?.textChunkLimit : undefined;
  const textLimit = resolveTextChunkLimit(cfg, providerKey, accountId, { fallbackLimit: providerChunkLimit });
...
export function resolveBlockStreamingCoalescing(cfg, provider, accountId, chunking) {
  ...
  const providerChunkLimit = providerId ? getChannelDock(providerId)?.outbound?.textChunkLimit : undefined;
  const textLimit = resolveTextChunkLimit(cfg, providerKey, accountId, { fallbackLimit: providerChunkLimit });
  const providerDefaults = providerId ? getChannelDock(providerId)?.streaming?.blockStreamingCoalesceDefaults : undefined;
...
";
  return { minChars, maxChars, idleMs, joiner };
}
