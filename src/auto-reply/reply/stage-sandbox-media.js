...
export async function stageSandboxMedia(params) {
  ...
  const sandbox = await ensureSandboxWorkspaceForSession({ config: cfg, sessionKey, workspaceDir });
  ...
  try {
    ...
    {
      logVerbose("Failed to stage inbound media for sandbox: ");
    }
  }
}
...
async function scpFile(remoteHost, remotePath, localPath) {
  return new Promise((resolve, reject) => {
    const child = spawn("/usr/bin/scp", ["-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new", ":", localPath], { stdio: ["ignore", "ignore", "pipe"] });
    let stderr = "";
