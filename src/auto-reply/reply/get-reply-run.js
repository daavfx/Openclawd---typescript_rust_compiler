...
export async function runPreparedReply(params) {
  ...
  const typingMode = resolveTypingMode({ configured: (sessionCfg?.typingMode ?? agentCfg?.typingMode), isGroupChat, wasMentioned, isHeartbeat });
  const shouldInjectGroupIntro = Boolean((isGroupChat && (isFirstTurnInSession || sessionEntry?.groupActivationNeedsSystemIntro)));
  const groupIntro = shouldInjectGroupIntro ? buildGroupIntro({ cfg, sessionCtx, sessionEntry, defaultActivation, silentToken: SILENT_REPLY_TOKEN }) : "";
...
");
  ...
  let prefixedBodyBase = await applySessionHints({ baseBody: baseBodyFinal, abortedLastRun, sessionEntry, sessionStore, sessionKey, storePath, abortKey: command.abortKey, messageId: sessionCtx.MessageSid });
  const isGroupSession = ((sessionEntry?.chatType === "group") || (sessionEntry?.chatType === "channel"));
  const isMainSession = (!isGroupSession && (sessionKey === normalizeMainKey(sessionCfg?.mainKey)));
  prefixedBodyBase = await prependSystemEvents({ cfg, sessionKey, isMainSession, isNewSession, prefixedBodyBase });
  const threadStarterBody = ctx.ThreadStarterBody?.trim();
  const threadStarterNote = (isNewSession && threadStarterBody) ? "[Thread starter - for context]
" : undefined;
  const skillResult = await ensureSkillSnapshot({ sessionEntry, sessionStore, sessionKey, storePath, sessionId, isFirstTurnInSession, workspaceDir, cfg, skillFilter: opts?.skillFilter });
...
");
  ...
  const mediaReplyHint = mediaNote ? "To send an image back, prefer the message tool (media/path/filePath). If you must inline, use MEDIA:/path or MEDIA:https://example.com/image.jpg (spaces ok, quote if needed). Keep caption in the text body." : undefined;
...
").trim() : prefixedBody;
  ...
  if (((resolvedThinkLevel === "xhigh") && !supportsXHighThinking(provider, model))) {
    ...
    if (explicitThink) {
      typing.cleanup();
      return { text: "Thinking level \"xhigh\" is only supported for . Use /think high or switch to one of those models." };
    }
  ...
  if ((resetTriggered && command.isAuthorizedSender)) {
    ...
    if ((channel && to)) {
      const modelLabel = "/";
      const defaultLabel = "/";
      const text = (modelLabel === defaultLabel) ? "✅ New session started · model: " : "✅ New session started · model:  (default: )";
      await routeReply({ payload: { text }, channel, to, sessionKey, accountId: ctx.AccountId, threadId: ctx.MessageThreadId, cfg });
    }
  }
...
");
  ...
  const queueMessageIdHint = queueMessageId ? "[message_id: ]" : "";
...
").trim() : queueBodyWithId;
  const resolvedQueue = resolveQueueSettings({ cfg, channel: sessionCtx.Provider, sessionEntry, inlineMode: perMessageQueueMode, inlineOptions: perMessageQueueOptions });
  const sessionLaneKey = resolveEmbeddedSessionLane((sessionKey ?? sessionIdFinal));
  const laneSize = getQueueSize(sessionLaneKey);
  if (((resolvedQueue.mode === "interrupt") && (laneSize > 0))) {
    const cleared = clearCommandLane(sessionLaneKey);
    const aborted = abortEmbeddedPiRun(sessionIdFinal);
    logVerbose("Interrupting  (cleared , aborted=)");
  }
  ...
  const authProfileId = await resolveSessionAuthProfileOverride({ cfg, provider, agentDir, sessionEntry, sessionStore, sessionKey, storePath, isNewSession });
  const authProfileIdSource = sessionEntry?.authProfileOverrideSource;
  const followupRun = { prompt: queuedBody, messageId: (sessionCtx.MessageSidFull ?? sessionCtx.MessageSid), summaryLine: baseBodyTrimmedRaw, enqueuedAt: Date.now(), originatingChannel: ctx.OriginatingChannel, originatingTo: ctx.OriginatingTo, originatingAccountId: ctx.AccountId, originatingThreadId: ctx.MessageThreadId, originatingChatType: ctx.ChatType, run: { agentId, agentDir, sessionId: sessionIdFinal, sessionKey, messageProvider: (sessionCtx.Provider?.trim().toLowerCase() || undefined), agentAccountId: sessionCtx.AccountId, groupId: (resolveGroupSessionKey(sessionCtx)?.id ?? undefined), groupChannel: (sessionCtx.GroupChannel?.trim() ?? sessionCtx.GroupSubject?.trim()), groupSpace: (sessionCtx.GroupSpace?.trim() ?? undefined), senderId: (sessionCtx.SenderId?.trim() || undefined), senderName: (sessionCtx.SenderName?.trim() || undefined), senderUsername: (sessionCtx.SenderUsername?.trim() || undefined), senderE164: (sessionCtx.SenderE164?.trim() || undefined), sessionFile, workspaceDir, config: cfg, skillsSnapshot, provider, model, authProfileId, authProfileIdSource, thinkLevel: resolvedThinkLevel, verboseLevel: resolvedVerboseLevel, reasoningLevel: resolvedReasoningLevel, elevatedLevel: resolvedElevatedLevel, execOverrides, bashElevated: { enabled: elevatedEnabled, allowed: elevatedAllowed, defaultLevel: (resolvedElevatedLevel ?? "off") }, timeoutMs, blockReplyBreak: resolvedBlockStreamingBreak, ownerNumbers: (command.ownerList.length > 0) ? command.ownerList : undefined, extraSystemPrompt: (extraSystemPrompt || undefined), ...isReasoningTagProvider(provider) ? { enforceFinalTag: true } : {  }:  } };
  return runReplyAgent({ commandBody: prefixedCommandBody, followupRun, queueKey, resolvedQueue, shouldSteer, shouldFollowup, isActive, isStreaming, opts, typing, sessionEntry, sessionStore, sessionKey, storePath, defaultModel, agentCfgContextTokens: agentCfg?.contextTokens, resolvedVerboseLevel: (resolvedVerboseLevel ?? "off"), isNewSession, blockStreamingEnabled, blockReplyChunking, resolvedBlockStreamingBreak, sessionCtx, shouldInjectGroupIntro, typingMode });
}
