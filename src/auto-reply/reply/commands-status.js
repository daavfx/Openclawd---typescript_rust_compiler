...
function resolveModelAuthLabel(provider, cfg, sessionEntry, agentDir) {
  ...
  const order = resolveAuthProfileOrder({ cfg, store, provider: providerKey, preferredProfile: profileOverride });
...
export async function buildStatusReply(params) {
  ...
  if (currentUsageProvider) {
    try {
      {
        const usageSummary = await loadProviderUsageSummary({ timeoutMs: 3500, providers: [currentUsageProvider], agentDir: statusAgentDir });
        const usageEntry = usageSummary.providers[0];
        if (((usageEntry && !usageEntry.error) && (usageEntry.windows.length > 0))) {
          const summaryLine = formatUsageWindowSummary(usageEntry, { now: Date.now(), maxWindows: 2, includeResets: true });
          if (summaryLine) {
            usageLine = "📊 Usage: ";
          }
        }
  ...
  const queueSettings = resolveQueueSettings({ cfg, channel: command.channel, sessionEntry });
  ...
  if (sessionKey) {
    ...
    if ((runs.length > 0)) {
      ...
      if (verboseEnabled) {
        ...
        subagentsLine = "🤖 Subagents:  active ·  done";
      } else {
        if ((active.length > 0)) {
          subagentsLine = "🤖 Subagents:  active";
        }
      }
    }
  }
  const groupActivation = isGroup ? (normalizeGroupActivation(sessionEntry?.groupActivation) ?? defaultGroupActivation()) : undefined;
  const agentDefaults = (cfg.agents?.defaults ?? {  });
  const statusText = buildStatusMessage({ config: cfg, agent: { ...agentDefaults: , model: { ...agentDefaults.model: , primary: "/" }, contextTokens, thinkingDefault: agentDefaults.thinkingDefault, verboseDefault: agentDefaults.verboseDefault, elevatedDefault: agentDefaults.elevatedDefault }, sessionEntry, sessionKey, sessionScope, groupActivation, resolvedThink: (resolvedThinkLevel ?? await resolveDefaultThinkingLevel()), resolvedVerbose: resolvedVerboseLevel, resolvedReasoning: resolvedReasoningLevel, resolvedElevated: resolvedElevatedLevel, modelAuth: resolveModelAuthLabel(provider, cfg, sessionEntry, statusAgentDir), usageLine: (usageLine ?? undefined), queue: { mode: queueSettings.mode, depth: queueDepth, debounceMs: queueSettings.debounceMs, cap: queueSettings.cap, dropPolicy: queueSettings.dropPolicy, showDetails: queueOverrides }, subagentsLine, mediaDecisions: params.mediaDecisions, includeTranscriptUsage: false });
