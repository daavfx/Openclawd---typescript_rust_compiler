...
export function parseLineDirectives(payload) {
  ...
  if ((locationMatch && !lineData.location)) {
    ...
    if ((parts.length >= 4)) {
      ...
      if ((!isNaN(latitude) && !isNaN(longitude))) {
        lineData.location = { title: (title || "Location"), address: (address || ""), latitude, longitude };
      }
    }
  ...
  if ((confirmMatch && !lineData.templateMessage)) {
    ...
    if ((parts.length >= 3)) {
      ...
      lineData.templateMessage = { type: "confirm", text: question, confirmLabel: yesLabel, confirmData: yesData, cancelLabel: noLabel, cancelData: noData, altText: question };
    }
  ...
  if ((buttonsMatch && !lineData.templateMessage)) {
    ...
    if ((parts.length >= 3)) {
      ...
      if ((actions.length > 0)) {
        lineData.templateMessage = { type: "buttons", title, text: bodyText, actions: actions.slice(0, 4), altText: ": " };
      }
    }
  ...
  if ((mediaPlayerMatch && !lineData.flexMessage)) {
    ...
    if ((parts.length >= 1)) {
      ...
      const card = createMediaPlayerCard({ title: (title || "Unknown Track"), subtitle: (artist || undefined), source: (source || undefined), imageUrl: validImageUrl, isPlaying: statusStr ? isPlaying : undefined, controls: { previous: { data: lineActionData("previous", { "line.device": deviceKey }) }, play: { data: lineActionData("play", { "line.device": deviceKey }) }, pause: { data: lineActionData("pause", { "line.device": deviceKey }) }, next: { data: lineActionData("next", { "line.device": deviceKey }) } } });
      lineData.flexMessage = { altText: "🎵 ", contents: card };
    }
  ...
  if ((eventMatch && !lineData.flexMessage)) {
    ...
    if ((parts.length >= 2)) {
      ...
      const card = createEventCard({ title: (title || "Event"), date: (date || "TBD"), time: (time || undefined), location: (location || undefined), description: (description || undefined) });
      lineData.flexMessage = { altText: "📅  - ", contents: card };
    }
  ...
  if ((appleTvMatch && !lineData.flexMessage)) {
    ...
    if ((parts.length >= 1)) {
      ...
      const card = createAppleTvRemoteCard({ deviceName: (deviceName || "Apple TV"), status: (status || undefined), actionData: { up: lineActionData("up", { "line.device": deviceKey }), down: lineActionData("down", { "line.device": deviceKey }), left: lineActionData("left", { "line.device": deviceKey }), right: lineActionData("right", { "line.device": deviceKey }), select: lineActionData("select", { "line.device": deviceKey }), menu: lineActionData("menu", { "line.device": deviceKey }), home: lineActionData("home", { "line.device": deviceKey }), play: lineActionData("play", { "line.device": deviceKey }), pause: lineActionData("pause", { "line.device": deviceKey }), volumeUp: lineActionData("volume_up", { "line.device": deviceKey }), volumeDown: lineActionData("volume_down", { "line.device": deviceKey }), mute: lineActionData("mute", { "line.device": deviceKey }) } });
      lineData.flexMessage = { altText: "📺  Remote", contents: card };
    }
  ...
  if ((agendaMatch && !lineData.flexMessage)) {
    ...
    if ((parts.length >= 2)) {
      ...
      const events = eventsStr.split(",").map((eventStr) => {
        ...
        if ((colonIdx > 0)) {
          return { title: trimmed.slice(0, colonIdx).trim(), time: trimmed.slice((colonIdx + 1)).trim() };
        }
        return { title: trimmed };
      });
      const card = createAgendaCard({ title: (title || "Agenda"), events });
      lineData.flexMessage = { altText: "📋  ( events)", contents: card };
    }
  ...
  if ((deviceMatch && !lineData.flexMessage)) {
    ...
    if ((parts.length >= 1)) {
      ...
      const card = createDeviceControlCard({ deviceName: (deviceName || "Device"), deviceType: (deviceType || undefined), status: (status || undefined), controls });
      lineData.flexMessage = { altText: "📱 ", contents: card };
    }
