...
export async function handleInlineActions(params) {
  ...
  const skillCommands = (shouldLoadSkillCommands && params.skillCommands) ? params.skillCommands : shouldLoadSkillCommands ? listSkillCommandsForWorkspace({ workspaceDir, cfg, skillFilter }) : [];
  const skillInvocation = (allowTextCommands && (skillCommands.length > 0)) ? resolveSkillCommandInvocation({ commandBodyNormalized: command.commandBodyNormalized, skillCommands }) : null;
  if (skillInvocation) {
    if (!command.isAuthorizedSender) {
      logVerbose("Ignoring / from unauthorized sender: ");
      typing.cleanup();
    ...
    if ((dispatch?.kind === "tool")) {
      ...
      const tools = createOpenClawTools({ agentSessionKey: sessionKey, agentChannel: channel, agentAccountId: ctx.AccountId, agentTo: (ctx.OriginatingTo ?? ctx.To), agentThreadId: (ctx.MessageThreadId ?? undefined), agentDir, workspaceDir, config: cfg });
      const tool = tools.find((candidate) => (candidate.name === dispatch.toolName));
      if (!tool) {
        typing.cleanup();
        return { kind: "reply", reply: { text: "❌ Tool not available: " } };
      }
      const toolCallId = "cmd__";
      try {
        {
          const result = await tool.execute(toolCallId, { command: rawArgs, commandName: skillInvocation.command.name, skillName: skillInvocation.command.skillName });
    ...
    const promptParts = ["Use the \"\" skill for this request.", skillInvocation.args ? "User input:
...
");
  ...
  const handleInlineStatus = (!isDirectiveOnly({ directives, cleanedBody: directives.cleaned, ctx, cfg, agentId, isGroup }) && inlineStatusRequested);
  ...
  const commandResult = await handleCommands({ ctx, cfg, command, agentId, directives, elevated: { enabled: elevatedEnabled, allowed: elevatedAllowed, failures: elevatedFailures }, sessionEntry, previousSessionEntry, sessionStore, sessionKey, storePath, sessionScope, workspaceDir, defaultGroupActivation: defaultActivation, resolvedThinkLevel, resolvedVerboseLevel: (resolvedVerboseLevel ?? "off"), resolvedReasoningLevel, resolvedElevatedLevel, resolveDefaultThinkingLevel, provider, model, contextTokens, isGroup, skillCommands });
  if (!commandResult.shouldContinue) {
    typing.cleanup();
    return { kind: "reply", reply: commandResult.reply };
  }
  return { kind: "continue", directives, abortedLastRun };
}
