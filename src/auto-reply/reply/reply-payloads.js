...
export function applyReplyTagsToPayload(payload, currentMessageId) {
  if ((typeof payload.text !== "string")) {
    ...
    return { ...payload: , replyToId: (currentMessageId?.trim() || undefined) };
  }
  ...
  if (!shouldParseTags) {
    ...
    return { ...payload: , replyToId: (currentMessageId?.trim() || undefined), replyToTag: (payload.replyToTag ?? true) };
  }
  const {cleaned, replyToId, replyToCurrent, hasTag} = extractReplyToTag(payload.text, currentMessageId);
  return { ...payload: , text: cleaned ? cleaned : undefined, replyToId: (replyToId ?? payload.replyToId), replyToTag: (hasTag || payload.replyToTag), replyToCurrent: (replyToCurrent || payload.replyToCurrent) };
}
