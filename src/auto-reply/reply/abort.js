...
export function stopSubagentsForRequester(params) {
  ...
  if ((stopped > 0)) {
    logVerbose("abort: stopped  subagent run(s) for ");
  }
  return { stopped };
}
...
export async function tryFastAbortFromMessage(params) {
  ...
  const agentId = resolveSessionAgentId({ sessionKey: ((targetKey ?? ctx.SessionKey) ?? ""), config: cfg });
  ...
  const auth = resolveCommandAuthorization({ ctx, cfg, commandAuthorized });
  ...
  if (targetKey) {
    ...
    if (((cleared.followupCleared > 0) || (cleared.laneCleared > 0))) {
      logVerbose("abort: cleared followups= lane= keys=");
    }
    if ((entry && key)) {
