...
import { buildMentionRegexes, matchesMentionPatterns, normalizeMentionText } from "./reply/mentions.js";
...
describe("finalizeInboundContext", () => {
  it("fills BodyForAgent/BodyForCommands and normalizes newlines", () => {
    const ctx = { Body: "a\\nb
c", RawBody: "raw\\nline", ChatType: "channel", From: "whatsapp:group:123@g.us", GroupSubject: "Test" };
...
line");
  ...
  it("can force BodyForCommands to follow updated CommandBody", () => {
    const ctx = { Body: "base", BodyForCommands: "<media:audio>", CommandBody: "say hi", From: "signal:+15550001111", ChatType: "direct" };
...
[from: Alice (A1)]");
  ...
  it("prefers SenderE164 in the label when present", () => {
    const ctx = { ChatType: "group", SenderName: "Bob", SenderId: "bob@s.whatsapp.net", SenderE164: "+222" };
    expect(formatInboundBodyWithSenderMeta({ ctx, body: "[X] hi" })).toBe("[X] hi
[from: Bob (+222)]");
  });
...
[from: Alice (A1)]");
  ...
  it("does not append when the sender prefix follows an envelope header", () => {
    const ctx = { ChatType: "group", SenderName: "Alice", SenderId: "A1" };
    expect(formatInboundBodyWithSenderMeta({ ctx, body: "[Signal Group] Alice (A1): hi" })).toBe("[Signal Group] Alice (A1): hi");
  });
});
describe("inbound dedupe", () => {
  it("builds a stable key when MessageSid is present", () => {
    const ctx = { Provider: "telegram", OriginatingChannel: "telegram", OriginatingTo: "telegram:123", MessageSid: "42" };
    expect(buildInboundDedupeKey(ctx)).toBe("telegram|telegram:123|42");
  });
  it("skips duplicates with the same key", () => {
    resetInboundDedupe();
    const ctx = { Provider: "whatsapp", OriginatingChannel: "whatsapp", OriginatingTo: "whatsapp:+1555", MessageSid: "msg-1" };
  ...
  it("does not dedupe when the peer changes", () => {
    resetInboundDedupe();
    const base = { Provider: "whatsapp", OriginatingChannel: "whatsapp", MessageSid: "msg-1" };
  ...
  it("does not dedupe across session keys", () => {
    resetInboundDedupe();
    const base = { Provider: "whatsapp", OriginatingChannel: "whatsapp", OriginatingTo: "whatsapp:+1555", MessageSid: "msg-1" };
    expect(shouldSkipDuplicateInbound({ ...base: , SessionKey: "agent:alpha:main" }, { now: 100 })).toBe(false);
    expect(shouldSkipDuplicateInbound({ ...base: , SessionKey: "agent:bravo:main" }, { now: 200 })).toBe(false);
...
describe("createInboundDebouncer", () => {
  it("debounces and combines items", async () => {
    vi.useFakeTimers();
    const calls = [];
    const debouncer = createInboundDebouncer({ debounceMs: 10, buildKey: (item) => item.key, onFlush: async (items) => {
  ...
  it("flushes buffered items before non-debounced item", async () => {
    vi.useFakeTimers();
    const calls = [];
    const debouncer = createInboundDebouncer({ debounceMs: 50, buildKey: (item) => item.key, shouldDebounce: (item) => item.debounce, onFlush: async (items) => {
      calls.push(items.map((entry) => entry.id));
    } });
...
describe("initSessionState sender meta", () => {
  it("injects sender meta into BodyStripped for group chats", async () => {
    ...
    const result = await initSessionState({ ctx: { Body: "[WhatsApp 123@g.us] ping", ChatType: "group", SenderName: "Bob", SenderE164: "+222", SenderId: "222@s.whatsapp.net", SessionKey: "agent:main:whatsapp:group:123@g.us" }, cfg, commandAuthorized: true });
...
[from: Bob (+222)]");
  ...
  it("does not inject sender meta for direct chats", async () => {
    ...
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } };
    const result = await initSessionState({ ctx: { Body: "[WhatsApp +1] ping", ChatType: "direct", SenderName: "Bob", SenderE164: "+222", SessionKey: "agent:main:whatsapp:dm:+222" }, cfg, commandAuthorized: true });
    expect(result.sessionCtx.BodyStripped).toBe("[WhatsApp +1] ping");
  });
});
describe("mention helpers", () => {
  it("builds regexes and skips invalid patterns", () => {
    const regexes = buildMentionRegexes({ messages: { groupChat: { mentionPatterns: ["\\bopenclaw\\b", "(invalid"] } } });
  ...
  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({ messages: { groupChat: { mentionPatterns: ["\\bopenclaw\\b"] } } });
    expect(matchesMentionPatterns("OPENCLAW: hi", regexes)).toBe(true);
  });
  it("uses per-agent mention patterns when configured", () => {
    const regexes = buildMentionRegexes({ messages: { groupChat: { mentionPatterns: ["\\bglobal\\b"] } }, agents: { list: [{ id: "work", groupChat: { mentionPatterns: ["\\bworkbot\\b"] } }] } }, "work");
...
describe("resolveGroupRequireMention", () => {
  it("respects Discord guild/channel requireMention settings", () => {
    const cfg = { channels: { discord: { guilds: { "145": { requireMention: false, channels: { general: { allow: true } } } } } } };
    const ctx = { Provider: "discord", From: "discord:group:123", GroupChannel: "#general", GroupSpace: "145" };
    const groupResolution = { channel: "discord", id: "123", chatType: "group" };
    expect(resolveGroupRequireMention({ cfg, ctx, groupResolution })).toBe(false);
  });
  it("respects Slack channel requireMention settings", () => {
    const cfg = { channels: { slack: { channels: { C123: { requireMention: false } } } } };
    const ctx = { Provider: "slack", From: "slack:channel:C123", GroupSubject: "#general" };
    const groupResolution = { channel: "slack", id: "C123", chatType: "group" };
    expect(resolveGroupRequireMention({ cfg, ctx, groupResolution })).toBe(false);
  });
});
