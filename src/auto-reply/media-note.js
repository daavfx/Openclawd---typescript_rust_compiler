function formatMediaAttachedLine(params) {
  const prefix = ((typeof params.index === "number") && (typeof params.total === "number")) ? "[media attached /: " : "[media attached: ";
  const typePart = params.type?.trim() ? " ()" : "";
  const urlRaw = params.url?.trim();
  const urlPart = urlRaw ? " | " : "";
  return "]";
}
export function buildInboundMediaNote(ctx) {
  ...
  const entries = paths.map((entry, index) => { path: (entry ?? ""), type: (types?.[index] ?? ctx.MediaType), url: (urls?.[index] ?? ctx.MediaUrl), index }).filter((entry) => !suppressed.has(entry.index));
  if ((entries.length === 0)) {
    return undefined;
  }
  if ((entries.length === 1)) {
    return formatMediaAttachedLine({ path: (entries[0]?.path ?? ""), type: entries[0]?.type, url: entries[0]?.url });
  }
  const count = entries.length;
  const lines = ["[media attached:  files]"];
  for (const [idx, entry] of entries.entries()) {
    lines.push(formatMediaAttachedLine({ path: entry.path, index: (idx + 1), total: count, type: entry.type, url: entry.url }));
  }
  return lines.join("
");
}
