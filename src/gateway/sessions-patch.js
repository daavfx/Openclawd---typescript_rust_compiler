...
export async function applySessionsPatchToStore(params) {
  ...
  const next = existing ? { ...existing: , updatedAt: Math.max((existing.updatedAt ?? 0), now) } : { sessionId: randomUUID(), updatedAt: now };
      ...
      if ((raw !== undefined)) {
        ...
        for (const [key, entry] of Object.entries(store)) {
          ...
          if ((entry?.label === parsed.label)) {
            return invalid("label already in use: ");
          }
        }
  ...
  if (("thinkingLevel" in patch)) {
    ...
    if ((raw === null)) {
      ...
      if ((raw !== undefined)) {
        ...
        if (!normalized) {
          const resolvedDefault = resolveConfiguredModelRef({ cfg, defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL });
          ...
          return invalid("invalid thinkingLevel (use )");
        }
        if ((normalized === "off")) {
          delete next.thinkingLevel;
        } else {
  ...
  if (("model" in patch)) {
    ...
    const resolvedDefault = resolveConfiguredModelRef({ cfg, defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL });
    if ((raw === null)) {
      applyModelOverrideToSessionEntry({ entry: next, selection: { provider: resolvedDefault.provider, model: resolvedDefault.model, isDefault: true } });
    } else {
      if ((raw !== undefined)) {
        const trimmed = String(raw).trim();
        if (!trimmed) {
        ...
        if (!params.loadGatewayModelCatalog) {
          return { ok: false, error: errorShape(ErrorCodes.UNAVAILABLE, "model catalog unavailable") };
        }
        const catalog = await params.loadGatewayModelCatalog();
        const resolved = resolveAllowedModelRef({ cfg, catalog, raw: trimmed, defaultProvider: resolvedDefault.provider, defaultModel: resolvedDefault.model });
        if (("error" in resolved)) {
          return invalid(resolved.error);
        }
        const isDefault = ((resolved.ref.provider === resolvedDefault.provider) && (resolved.ref.model === resolvedDefault.model));
        applyModelOverrideToSessionEntry({ entry: next, selection: { provider: resolved.ref.provider, model: resolved.ref.model, isDefault } });
      }
    }
  }
  if ((next.thinkingLevel === "xhigh")) {
    ...
    if (!supportsXHighThinking(effectiveProvider, effectiveModel)) {
      if (("thinkingLevel" in patch)) {
        return invalid("thinkingLevel \"xhigh\" is only supported for ");
      }
