...
import { extractHookToken, normalizeAgentPayload, normalizeWakePayload, resolveHooksConfig } from "./hooks.js";
describe("gateway hooks helpers", () => {
  ...
  test("extractHookToken prefers bearer > header > query", () => {
    ...
    const req2 = { headers: { "x-openclaw-token": "header" } };
  ...
  test("normalizeWakePayload trims + validates", () => {
    expect(normalizeWakePayload({ text: "  hi " })).toEqual({ ok: true, value: { text: "hi", mode: "now" } });
  ...
  test("normalizeAgentPayload defaults + validates channel", () => {
    ...
    const explicitNoDeliver = normalizeAgentPayload({ message: "hello", deliver: false }, { idFactory: () => "fixed" });
    ...
    const imsg = normalizeAgentPayload({ message: "yo", channel: "imsg" }, { idFactory: () => "x" });
    expect(imsg.ok).toBe(true);
    if (imsg.ok) {
      expect(imsg.value.channel).toBe("imessage");
    }
    setActivePluginRegistry(createTestRegistry([{ pluginId: "msteams", source: "test", plugin: createMSTeamsPlugin({ aliases: ["teams"] }) }]));
    const teams = normalizeAgentPayload({ message: "yo", channel: "teams" }, { idFactory: () => "x" });
