  ...
  test("closes on missing ticks", async () => {
    ...
    wss.on("connection", (socket) => {
      socket.once("message", (data) => {
        ...
        const helloOk = { type: "hello-ok", protocol: 2, server: { version: "dev", connId: "c1" }, features: { methods: [], events: [] }, snapshot: { presence: [], health: {  }, stateVersion: { presence: 1, health: 1 }, uptimeMs: 1 }, policy: { maxPayload: (512 * 1024), maxBufferedBytes: (1024 * 1024), tickIntervalMs: 5 } };
        socket.send(JSON.stringify({ type: "res", id, ok: true, payload: helloOk }));
      });
    });
    const closed = new Promise((resolve) => {
      const client = new GatewayClient({ url: "ws://127.0.0.1:", onClose: (code, reason) => resolve({ code, reason }) });
      client.start();
    });
...
-----END CERTIFICATE-----";
    ...
    const error = await new Promise((resolve) => {
      ...
      client = new GatewayClient({ url: "wss://127.0.0.1:", tlsFingerprint: "deadbeef", onConnectError: (err) => {
