...
import { capArrayByJsonBytes, classifySessionKey, deriveSessionTitle, listSessionsFromStore, parseGroupKey, resolveGatewaySessionStoreTarget, resolveSessionStoreKey } from "./session-utils.js";
describe("gateway session utils", () => {
  ...
  test("parseGroupKey handles group keys", () => {
    expect(parseGroupKey("discord:group:dev")).toEqual({ channel: "discord", kind: "group", id: "dev" });
    expect(parseGroupKey("agent:ops:discord:group:dev")).toEqual({ channel: "discord", kind: "group", id: "dev" });
  ...
  test("resolveSessionStoreKey honors global scope", () => {
    const cfg = { session: { scope: "global", mainKey: "work" }, agents: { list: [{ id: "ops", default: true }] } };
  ...
  test("resolveGatewaySessionStoreTarget uses canonical key for main alias", () => {
    const storeTemplate = path.join(os.tmpdir(), "openclaw-session-utils", "{agentId}", "sessions.json");
    const cfg = { session: { mainKey: "main", store: storeTemplate }, agents: { list: [{ id: "ops", default: true }] } };
    const target = resolveGatewaySessionStoreTarget({ cfg, key: "main" });
...
describe("deriveSessionTitle", () => {
  ...
  test("falls back to sessionId prefix with date", () => {
    const entry = { sessionId: "abcd1234-5678-90ef-ghij-klmnopqrstuv", updatedAt: new Date("2024-03-15T10:30:00Z").getTime() };
  ...
  test("falls back to sessionId prefix without date when updatedAt missing", () => {
    const entry = { sessionId: "abcd1234-5678-90ef-ghij-klmnopqrstuv", updatedAt: 0 };
    const result = deriveSessionTitle(entry);
    expect(result).toBe("abcd1234");
  });
  test("trims whitespace from displayName", () => {
    const entry = { sessionId: "abc123", updatedAt: Date.now(), displayName: "  Padded Name  " };
    expect(deriveSessionTitle(entry)).toBe("Padded Name");
  });
  test("ignores empty displayName and falls through", () => {
    const entry = { sessionId: "abc123", updatedAt: Date.now(), displayName: "   ", subject: "Actual Subject" };
    expect(deriveSessionTitle(entry)).toBe("Actual Subject");
  });
});
describe("listSessionsFromStore search", () => {
  const baseCfg = { session: { mainKey: "main" }, agents: { list: [{ id: "main", default: true }] } };
  const makeStore = () => { "agent:main:work-project": { sessionId: "sess-work-1", updatedAt: Date.now(), displayName: "Work Project Alpha", label: "work" }, "agent:main:personal-chat": { sessionId: "sess-personal-1", updatedAt: (Date.now() - 1000), displayName: "Personal Chat", subject: "Family Reunion Planning" }, "agent:main:discord:group:dev-team": { sessionId: "sess-discord-1", updatedAt: (Date.now() - 2000), label: "discord", subject: "Dev Team Discussion" } };
  test("returns all sessions when search is empty", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "" } });
    expect(result.sessions.length).toBe(3);
  });
  test("returns all sessions when search is undefined", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: {  } });
    expect(result.sessions.length).toBe(3);
  });
  test("filters by displayName case-insensitively", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "WORK PROJECT" } });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0].displayName).toBe("Work Project Alpha");
  });
  test("filters by subject", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "reunion" } });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0].subject).toBe("Family Reunion Planning");
  });
  test("filters by label", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "discord" } });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0].label).toBe("discord");
  });
  test("filters by sessionId", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "sess-personal" } });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0].sessionId).toBe("sess-personal-1");
  });
  test("filters by key", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "dev-team" } });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0].key).toBe("agent:main:discord:group:dev-team");
  });
  test("returns empty array when no matches", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "nonexistent-term" } });
    expect(result.sessions.length).toBe(0);
  });
  test("matches partial strings", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "alpha" } });
    expect(result.sessions.length).toBe(1);
    expect(result.sessions[0].displayName).toBe("Work Project Alpha");
  });
  test("trims whitespace from search query", () => {
    const store = makeStore();
    const result = listSessionsFromStore({ cfg: baseCfg, storePath: "/tmp/sessions.json", store, opts: { search: "  personal  " } });
    expect(result.sessions.length).toBe(1);
  });
});
