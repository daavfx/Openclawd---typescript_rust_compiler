...
export async function startGatewaySidecars(params) {
  ...
  try {
    ...
    {
      params.logBrowser.error("server failed to start: ");
    }
  }
    ...
    try {
      {
        ...
        if (gmailResult.started) {
          ...
          if (((gmailResult.reason && (gmailResult.reason !== "hooks not enabled")) && (gmailResult.reason !== "no gmail account configured"))) {
            params.logHooks.warn("gmail watcher not started: ");
          }
        }
      }
    }
      ...
      {
        params.logHooks.error("gmail watcher failed to start: ");
      }
    }
  }
  if (params.cfg.hooks?.gmail?.model) {
    ...
    if (hooksModelRef) {
      const {provider: defaultProvider, model: defaultModel} = resolveConfiguredModelRef({ cfg: params.cfg, defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL });
      const catalog = await loadModelCatalog({ config: params.cfg });
      const status = getModelRefStatus({ cfg: params.cfg, catalog, ref: hooksModelRef, defaultProvider, defaultModel });
      if (!status.allowed) {
        params.logHooks.warn("hooks.gmail.model \"\" not in agents.defaults.models allowlist (will use primary instead)");
      }
      if (!status.inCatalog) {
        params.logHooks.warn("hooks.gmail.model \"\" not in the model catalog (may fail at runtime)");
      }
    }
  }
  try {
    {
      ...
      if ((loadedCount > 0)) {
        params.logHooks.info("loaded  internal hook handler");
      }
    }
  }
  catch (err) {
    {
      params.logHooks.error("failed to load hooks: ");
    }
  }
  ...
  if (!skipChannels) {
    try {
      ...
      {
        params.logChannels.error("channel startup failed: ");
      }
    }
  } else {
  ...
  try {
    {
      pluginServices = await startPluginServices({ registry: params.pluginRegistry, config: params.cfg, workspaceDir: params.defaultWorkspaceDir });
    }
  }
  catch (err) {
    {
      params.log.warn("plugin services failed to start: ");
    }
  }
