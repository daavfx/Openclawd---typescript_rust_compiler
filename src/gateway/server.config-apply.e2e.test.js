...
describe("gateway config.apply", () => {
  it("writes config, stores sentinel, and schedules restart", async () => {
    ...
    try {
      {
        const id = "req-1";
        ws.send(JSON.stringify({ type: "req", id, method: "config.apply", params: { raw: "{ \"agents\": { \"list\": [{ \"id\": \"main\", \"workspace\": \"~/openclaw\" }] } }", sessionKey: "agent:main:whatsapp:dm:+15555550123", restartDelayMs: 0 } }));
  ...
  it("rejects invalid raw config", async () => {
    ...
    try {
      {
        const id = "req-2";
        ws.send(JSON.stringify({ type: "req", id, method: "config.apply", params: { raw: "{" } }));
        const res = await onceMessage(ws, (o) => ((o.type === "res") && (o.id === id)));
