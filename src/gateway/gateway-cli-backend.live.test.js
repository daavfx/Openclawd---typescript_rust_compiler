...
async function connectClient(params) {
  return await new Promise((resolve, reject) => {
    ...
    const client = new GatewayClient({ url: params.url, token: params.token, clientName: "vitest-live-cli-backend", clientVersion: "dev", mode: "test", onHelloOk: () => stop(undefined, client), onConnectError: (err) => stop(err), onClose: (code, reason) => stop(new Error("gateway closed during connect (): ")) });
...
describeLive("gateway live (cli backend)", () => {
  it("runs the agent pipeline against the local CLI backend", async () => {
    ...
    if (!parsed) {
      throw new Error("OPENCLAW_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ");
    }
    ...
    if (!cliCommand) {
      throw new Error("OPENCLAW_LIVE_CLI_BACKEND_COMMAND is required for provider \"\".");
    }
    const baseCliArgs = (parseJsonStringArray("OPENCLAW_LIVE_CLI_BACKEND_ARGS", process.env.OPENCLAW_LIVE_CLI_BACKEND_ARGS) ?? providerDefaults?.args);
    if ((!baseCliArgs || (baseCliArgs.length === 0))) {
      throw new Error("OPENCLAW_LIVE_CLI_BACKEND_ARGS is required for provider \"\".");
    }
    const cliClearEnv = (parseJsonStringArray("OPENCLAW_LIVE_CLI_BACKEND_CLEAR_ENV", process.env.OPENCLAW_LIVE_CLI_BACKEND_CLEAR_ENV) ?? (providerId === "claude-cli") ? DEFAULT_CLEAR_ENV : []);
    ...
    try {
      {
        ...
        const message = (providerId === "codex-cli") ? "Please include the token CLI-BACKEND- in your reply." : "Reply with exactly: CLI backend OK .";
        const payload = await client.request("agent", { sessionKey, idempotencyKey: "idem-", message, deliver: false }, { expectFinal: true });
        if ((payload?.status !== "ok")) {
          throw new Error("agent status=");
        }
        const text = extractPayloadText(payload?.result);
        if ((providerId === "codex-cli")) {
          expect(text).toContain("CLI-BACKEND-");
        } else {
          expect(text).toContain("CLI backend OK .");
        }
        if (CLI_RESUME) {
          const runIdResume = randomUUID();
          const resumeNonce = randomBytes(3).toString("hex").toUpperCase();
          const resumeMessage = (providerId === "codex-cli") ? "Please include the token CLI-RESUME- in your reply." : "Reply with exactly: CLI backend RESUME OK .";
          const resumePayload = await client.request("agent", { sessionKey, idempotencyKey: "idem-", message: resumeMessage, deliver: false }, { expectFinal: true });
          if ((resumePayload?.status !== "ok")) {
            throw new Error("resume status=");
          }
          const resumeText = extractPayloadText(resumePayload?.result);
          if ((providerId === "codex-cli")) {
            expect(resumeText).toContain("CLI-RESUME-");
          } else {
            expect(resumeText).toContain("CLI backend RESUME OK .");
          }
        }
        if (CLI_IMAGE) {
          ...
          const imageProbe = await client.request("agent", { sessionKey, idempotencyKey: "idem--image", message: (("Look at the attached image. Reply with exactly two tokens separated by a single space: " + "(1) the animal shown or written in the image, lowercase; ") + "(2) the code printed in the image, uppercase. No extra text."), attachments: [{ mimeType: "image/png", fileName: "probe-.png", content: imageBase64 }], deliver: false }, { expectFinal: true });
          if ((imageProbe?.status !== "ok")) {
            throw new Error("image probe failed: status=");
          }
          const imageText = extractPayloadText(imageProbe?.result);
          if (!/\bcat\b/i.test(imageText)) {
            throw new Error("image probe missing 'cat': ");
          }
          ...
          if (!(bestDistance <= 5)) {
            throw new Error("image probe missing code (): ");
          }
        }
      }
    }
