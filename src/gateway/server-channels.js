...
function createRuntimeStore() {
  return { aborts: new Map(), tasks: new Map(), runtimes: new Map() };
}
...
export function createChannelManager(opts) {
  ...
  const startChannel = async (channelId, accountId) => {
    ...
    await Promise.all(accountIds.map(async (id) => {
      ...
      if (!enabled) {
        setRuntime(channelId, id, { accountId: id, running: false, lastError: (plugin.config.disabledReason?.(account, cfg) ?? "disabled") });
        return;
      }
      let configured = true;
      ...
      if (!configured) {
        setRuntime(channelId, id, { accountId: id, running: false, lastError: (plugin.config.unconfiguredReason?.(account, cfg) ?? "not configured") });
        return;
      }
      ...
      const task = startAccount({ cfg, accountId: id, account, runtime: channelRuntimeEnvs[channelId], abortSignal: abort.signal, log, getStatus: () => getRuntime(channelId, id), setStatus: (next) => setRuntime(channelId, id, next) });
      const tracked = Promise.resolve(task).catch((err) => {
        const message = formatErrorMessage(err);
        setRuntime(channelId, id, { accountId: id, lastError: message });
        log.error?.("[] channel exited: ");
      }).finally(() => {
        store.aborts.delete(id);
        store.tasks.delete(id);
        setRuntime(channelId, id, { accountId: id, running: false, lastStopAt: Date.now() });
      });
  ...
  const stopChannel = async (channelId, accountId) => {
    ...
    const cfg = loadConfig();
    const store = getStore(channelId);
    const knownIds = new Set([...store.aborts.keys(), ...store.tasks.keys(), ...plugin ? plugin.config.listAccountIds(cfg) : []]);
    if (accountId) {
      knownIds.clear();
    ...
    await Promise.all(Array.from(knownIds.values()).map(async (id) => {
      ...
      if (plugin?.gateway?.stopAccount) {
        const account = plugin.config.resolveAccount(cfg, id);
        await plugin.gateway.stopAccount({ cfg, accountId: id, account, runtime: channelRuntimeEnvs[channelId], abortSignal: (abort?.signal ?? new AbortController().signal), log: channelLogs[channelId], getStatus: () => getRuntime(channelId, id), setStatus: (next) => setRuntime(channelId, id, next) });
      }
      try {
        {
          await task;
  ...
  const markChannelLoggedOut = (channelId, cleared, accountId) => {
    ...
    const cfg = loadConfig();
    const resolvedId = (accountId ?? resolveChannelDefaultAccountId({ plugin, cfg }));
    const current = getRuntime(channelId, resolvedId);
    const next = { accountId: resolvedId, running: false, lastError: cleared ? "logged out" : current.lastError };
  ...
  const getRuntimeSnapshot = () => {
    ...
    for (const plugin of listChannelPlugins()) {
      ...
      for (const id of accountIds) {
        ...
        if (!next.running) {
          if (!enabled) {
            next.lastError = (next.lastError ?? (plugin.config.disabledReason?.(account, cfg) ?? "disabled"));
          } else {
            if ((configured === false)) {
              next.lastError = (next.lastError ?? (plugin.config.unconfiguredReason?.(account, cfg) ?? "not configured"));
            }
          }
        }
        accounts[id] = next;
  ...
  return { getRuntimeSnapshot, startChannels, startChannel, stopChannel, markChannelLoggedOut };
}
