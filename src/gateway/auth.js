...
function resolveRequestClientIp(req, trustedProxies) {
  ...
  return resolveGatewayClientIp({ remoteAddr: (req.socket?.remoteAddress ?? ""), forwardedFor: headerValue(req.headers?.["x-forwarded-for"]), realIp: headerValue(req.headers?.["x-real-ip"]), trustedProxies });
}
...
function getTailscaleUser(req) {
  ...
  return { login: login.trim(), name, profilePic: ((typeof profilePic === "string") && profilePic.trim()) ? profilePic.trim() : undefined };
}
...
async function resolveVerifiedTailscaleUser(params) {
  ...
  return { ok: true, user: { login: whois.login, name: (whois.name ?? tailscaleUser.name), profilePic: tailscaleUser.profilePic } };
}
export function resolveGatewayAuth(params) {
  const authConfig = (params.authConfig ?? {  });
  const env = (params.env ?? process.env);
  const token = (((authConfig.token ?? env.OPENCLAW_GATEWAY_TOKEN) ?? env.CLAWDBOT_GATEWAY_TOKEN) ?? undefined);
  const password = (((authConfig.password ?? env.OPENCLAW_GATEWAY_PASSWORD) ?? env.CLAWDBOT_GATEWAY_PASSWORD) ?? undefined);
