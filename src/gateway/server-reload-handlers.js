...
export function createGatewayReloadHandlers(params) {
  const applyHotReload = async (plan, nextConfig) => {
    ...
    if (plan.reloadHooks) {
      try {
        ...
        {
          params.logHooks.warn("hooks config reload failed: ");
        }
      }
    }
    ...
    if (plan.restartCron) {
      ...
      nextState.cronState = buildGatewayCronService({ cfg: nextConfig, deps: params.deps, broadcast: params.broadcast });
      void nextState.cronState.cron.start().catch((err) => params.logCron.error("failed to start: "));
    }
    ...
    if ((plan.restartChannels.size > 0)) {
      if ((isTruthyEnvValue(process.env.OPENCLAW_SKIP_CHANNELS) || isTruthyEnvValue(process.env.OPENCLAW_SKIP_PROVIDERS))) {
        ...
        const restartChannel = async (name) => {
          params.logChannels.info("restarting  channel");
    ...
    if ((plan.hotReasons.length > 0)) {
      params.logReload.info("config hot reload applied ()");
    } else {
      if ((plan.noopPaths.length > 0)) {
        params.logReload.info("config change applied (dynamic reads: )");
      }
    }
  ...
  const requestGatewayRestart = (plan, nextConfig) => {
    ...
    params.logReload.warn("config change requires gateway restart ()");
