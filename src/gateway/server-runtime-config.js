...
export async function resolveGatewayRuntimeConfig(params) {
  ...
  const authConfig = { ...authBase: , ...authOverrides:  };
  const tailscaleBase = (params.cfg.gateway?.tailscale ?? {  });
  const tailscaleOverrides = (params.tailscale ?? {  });
  const tailscaleConfig = { ...tailscaleBase: , ...tailscaleOverrides:  };
  const tailscaleMode = (tailscaleConfig.mode ?? "off");
  const resolvedAuth = resolveGatewayAuth({ authConfig, env: process.env, tailscaleMode });
  ...
  if ((!isLoopbackHost(bindHost) && !hasSharedSecret)) {
    throw new Error("refusing to bind gateway to : without auth (set gateway.auth.token/password, or set OPENCLAW_GATEWAY_TOKEN/OPENCLAW_GATEWAY_PASSWORD)");
  }
  return { bindHost, controlUiEnabled, openAiChatCompletionsEnabled, openResponsesEnabled, openResponsesConfig: openResponsesConfig ? { ...openResponsesConfig: , enabled: openResponsesEnabled } : undefined, controlUiBasePath, resolvedAuth, authMode, tailscaleConfig, tailscaleMode, hooksConfig, canvasHostEnabled };
}
