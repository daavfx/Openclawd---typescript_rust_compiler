...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return { ...actual: , loadConfig, resolveGatewayPort };
});
vi.mock("../infra/tailnet.js", () => { pickPrimaryTailnetIPv4 });
vi.mock("./client.js", () => { describeGatewayCloseCode: (code) => {
  if ((code === 1000)) {
...
describe("callGateway url resolution", () => {
  ...
  it("uses url override in remote mode even when remote url is missing", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "remote", bind: "loopback", remote: {  } } });
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses explicit url overrides and omits bind details", () => {
    ...
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue("100.64.0.1");
    const details = buildGatewayConnectionDetails({ url: "wss://example.com/ws" });
  ...
  it("prefers remote url when configured", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "remote", bind: "tailnet", remote: { url: "wss://remote.example.com/ws" } } });
    resolveGatewayPort.mockReturnValue(18800);
...
describe("callGateway error details", () => {
  ...
  it("fails fast when remote mode is missing remote url", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "remote", bind: "loopback", remote: {  } } });
    await expect(callGateway({ method: "health", timeoutMs: 10 })).rejects.toThrow("gateway remote mode misconfigured");
  });
});
describe("callGateway password resolution", () => {
  ...
  it("uses local config password when env is unset", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback", auth: { password: "secret" } } });
    await callGateway({ method: "health" });
  ...
  it("prefers env password over local config password", async () => {
    ...
    loadConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback", auth: { password: "from-config" } } });
    await callGateway({ method: "health" });
    expect(lastClientOptions?.password).toBe("from-env");
  });
  it("uses remote password in remote mode when env is unset", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "remote", remote: { url: "ws://remote.example:18789", password: "remote-secret" }, auth: { password: "from-config" } } });
    await callGateway({ method: "health" });
...
describe("callGateway token resolution", () => {
  ...
  it("uses remote token when remote mode uses url override", async () => {
    ...
    loadConfig.mockReturnValue({ gateway: { mode: "remote", remote: { token: "remote-token" }, auth: { token: "local-token" } } });
    await callGateway({ method: "health", url: "wss://override.example/ws" });
