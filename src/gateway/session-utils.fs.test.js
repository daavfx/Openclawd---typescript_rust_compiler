...
import { readFirstUserMessageFromTranscript, readLastMessagePreviewFromTranscript, readSessionPreviewItemsFromTranscript } from "./session-utils.fs.js";
describe("readFirstUserMessageFromTranscript", () => {
  ...
  test("returns first user message from transcript with string content", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "user", content: "Hello world" } }), JSON.stringify({ message: { role: "assistant", content: "Hi there" } })];
...
"), "utf-8");
  ...
  test("returns first user message from transcript with array content", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "user", content: [{ type: "text", text: "Array message content" }] } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readFirstUserMessageFromTranscript(sessionId, storePath);
  ...
  test("returns first user message from transcript with input_text content", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "user", content: [{ type: "input_text", text: "Input text content" }] } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readFirstUserMessageFromTranscript(sessionId, storePath);
  ...
  test("skips non-user messages to find first user message", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "system", content: "System prompt" } }), JSON.stringify({ message: { role: "assistant", content: "Greeting" } }), JSON.stringify({ message: { role: "user", content: "First user question" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readFirstUserMessageFromTranscript(sessionId, storePath);
  ...
  test("returns null when no user messages exist", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "system", content: "System prompt" } }), JSON.stringify({ message: { role: "assistant", content: "Greeting" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readFirstUserMessageFromTranscript(sessionId, storePath);
    expect(result).toBeNull();
...
"), "utf-8");
  ...
  test("uses sessionFile parameter when provided", () => {
    ...
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "user", content: "Custom file message" } })];
...
"), "utf-8");
  ...
  test("returns null for empty content", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "user", content: "" } }), JSON.stringify({ message: { role: "user", content: "Second message" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readFirstUserMessageFromTranscript(sessionId, storePath);
...
describe("readLastMessagePreviewFromTranscript", () => {
  ...
  test("returns last user message from transcript", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "user", content: "First user" } }), JSON.stringify({ message: { role: "assistant", content: "First assistant" } }), JSON.stringify({ message: { role: "user", content: "Last user message" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
  ...
  test("returns last assistant message from transcript", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "user", content: "User question" } }), JSON.stringify({ message: { role: "assistant", content: "Final assistant reply" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
  ...
  test("skips system messages to find last user/assistant", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "user", content: "Real last" } }), JSON.stringify({ message: { role: "system", content: "System at end" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
  ...
  test("returns null when no user/assistant messages exist", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "system", content: "Only system" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
    expect(result).toBeNull();
...
"), "utf-8");
  ...
  test("handles array content format", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "assistant", content: [{ type: "text", text: "Array content response" }] } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
  ...
  test("handles output_text content format", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "assistant", content: [{ type: "output_text", text: "Output text response" }] } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
...
"), "utf-8");
  ...
  test("trims whitespace from message content", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "assistant", content: "  Padded response  " } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
  ...
  test("skips empty content to find previous message", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ message: { role: "assistant", content: "Has content" } }), JSON.stringify({ message: { role: "user", content: "" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readLastMessagePreviewFromTranscript(sessionId, storePath);
...
"), "utf-8");
  ...
  test("handles valid UTF-8 content", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const validLine = JSON.stringify({ message: { role: "user", content: "Valid UTF-8: 你好世界 🌍" } });
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  test("returns recent preview items with tool summary", () => {
    ...
    const transcriptPath = path.join(tmpDir, ".jsonl");
    const lines = [JSON.stringify({ type: "session", version: 1, id: sessionId }), JSON.stringify({ message: { role: "user", content: "Hello" } }), JSON.stringify({ message: { role: "assistant", content: "Hi" } }), JSON.stringify({ message: { role: "assistant", content: [{ type: "toolcall", name: "weather" }] } }), JSON.stringify({ message: { role: "assistant", content: "Forecast ready" } })];
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readSessionPreviewItemsFromTranscript(sessionId, storePath, undefined, undefined, 3, 120);
  ...
  test("truncates preview text to max chars", () => {
    ...
    fs.writeFileSync(transcriptPath, lines.join("
"), "utf-8");
    const result = readSessionPreviewItemsFromTranscript(sessionId, storePath, undefined, undefined, 1, 24);
