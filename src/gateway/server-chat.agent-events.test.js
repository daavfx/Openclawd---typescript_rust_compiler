...
describe("agent event handler", () => {
  it("emits chat delta for assistant text-only events", () => {
    ...
    const handler = createAgentEventHandler({ broadcast, nodeSendToSession, agentRunSeq, chatRunState, resolveSessionKeyForRun: () => undefined, clearAgentRunContext: vi.fn() });
    handler({ runId: "run-1", seq: 1, stream: "assistant", ts: Date.now(), data: { text: "Hello world" } });
