...
export async function writeSessionStore(params) {
  ...
  for (const [requestKey, entry] of Object.entries(params.entries)) {
    ...
    const storeKey = ((rawKey === "global") || (rawKey === "unknown")) ? rawKey : toAgentStoreSessionKey({ agentId, requestKey, mainKey: params.mainKey });
...
export function onceMessage(ws, filter, timeoutMs = 10000) {
...
export async function startServerWithClient(token, opts) {
  ...
  const fallbackToken = (token ?? (typeof testState.gatewayAuth?.token === "string") ? testState.gatewayAuth.token : undefined);
...
export async function connectReq(ws, opts) {
  ...
  const id = randomUUID();
  const client = (opts?.client ?? { id: GATEWAY_CLIENT_NAMES.TEST, version: "1.0.0", platform: "test", mode: GATEWAY_CLIENT_MODES.TEST });
  const role = (opts?.role ?? "operator");
  const defaultToken = (opts?.skipDefaultAuth === true) ? undefined : (typeof testState.gatewayAuth?.token === "string") ? (testState.gatewayAuth.token ?? undefined) : process.env.OPENCLAW_GATEWAY_TOKEN;
  const defaultPassword = (opts?.skipDefaultAuth === true) ? undefined : (typeof testState.gatewayAuth?.password === "string") ? (testState.gatewayAuth.password ?? undefined) : process.env.OPENCLAW_GATEWAY_PASSWORD;
  ...
  const device = () => {
    ...
    const payload = buildDeviceAuthPayload({ deviceId: identity.deviceId, clientId: client.id, clientMode: client.mode, role, scopes: requestedScopes, signedAtMs, token: (token ?? null) });
    return { id: identity.deviceId, publicKey: publicKeyRawBase64UrlFromPem(identity.publicKeyPem), signature: signDevicePayload(identity.privateKeyPem, payload), signedAt: signedAtMs, nonce: opts?.device?.nonce };
  }();
  ws.send(JSON.stringify({ type: "req", id, method: "connect", params: { minProtocol: (opts?.minProtocol ?? PROTOCOL_VERSION), maxProtocol: (opts?.maxProtocol ?? PROTOCOL_VERSION), client, caps: (opts?.caps ?? []), commands: (opts?.commands ?? []), permissions: (opts?.permissions ?? undefined), role, scopes: opts?.scopes, auth: (token || password) ? { token, password } : undefined, device } }));
