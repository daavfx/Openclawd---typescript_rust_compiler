...
import { connectOk, getFreePort, installGatewayTestHooks, rpcReq, startGatewayServer, startServerWithClient } from "./test-helpers.js";
const hoisted = vi.hoisted(() => {
  ...
  const startBrowserControlServerIfEnabled = vi.fn(async () => { stop: browserStop });
  const heartbeatStop = vi.fn();
  const heartbeatUpdateConfig = vi.fn();
  const startHeartbeatRunner = vi.fn(() => { stop: heartbeatStop, updateConfig: heartbeatUpdateConfig });
  const startGmailWatcher = vi.fn(async () => { started: true });
  const stopGmailWatcher = vi.fn(async () => {
  });
  const providerManager = { getRuntimeSnapshot: vi.fn(() => { providers: { whatsapp: { running: false, connected: false, reconnectAttempts: 0, lastConnectedAt: null, lastDisconnect: null, lastMessageAt: null, lastEventAt: null, lastError: null }, telegram: { running: false, lastStartAt: null, lastStopAt: null, lastError: null, mode: null }, discord: { running: false, lastStartAt: null, lastStopAt: null, lastError: null }, slack: { running: false, lastStartAt: null, lastStopAt: null, lastError: null }, signal: { running: false, lastStartAt: null, lastStopAt: null, lastError: null, baseUrl: null }, imessage: { running: false, lastStartAt: null, lastStopAt: null, lastError: null, cliPath: null, dbPath: null }, msteams: { running: false, lastStartAt: null, lastStopAt: null, lastError: null } }, providerAccounts: { whatsapp: {  }, telegram: {  }, discord: {  }, slack: {  }, signal: {  }, imessage: {  }, msteams: {  } } }), startChannels: vi.fn(async () => {
  ...
  return { CronService: CronServiceMock, cronInstances, browserStop, startBrowserControlServerIfEnabled, heartbeatStop, heartbeatUpdateConfig, startHeartbeatRunner, startGmailWatcher, stopGmailWatcher, providerManager, createChannelManager, startGatewayConfigReloader, reloaderStop, getOnHotReload: () => onHotReload, getOnRestart: () => onRestart };
});
vi.mock("../cron/service.js", () => { CronService: hoisted.CronService });
vi.mock("./server-browser.js", () => { startBrowserControlServerIfEnabled: hoisted.startBrowserControlServerIfEnabled });
vi.mock("../infra/heartbeat-runner.js", () => { startHeartbeatRunner: hoisted.startHeartbeatRunner });
vi.mock("../hooks/gmail-watcher.js", () => { startGmailWatcher: hoisted.startGmailWatcher, stopGmailWatcher: hoisted.stopGmailWatcher });
vi.mock("./server-channels.js", () => { createChannelManager: hoisted.createChannelManager });
vi.mock("./config-reload.js", () => { startGatewayConfigReloader: hoisted.startGatewayConfigReloader });
...
describe("gateway hot reload", () => {
  ...
  it("applies hot reload actions and emits restart signal", async () => {
    ...
    const nextConfig = { hooks: { enabled: true, token: "secret", gmail: { account: "me@example.com" } }, cron: { enabled: true, store: "/tmp/cron.json" }, agents: { defaults: { heartbeat: { every: "1m" }, maxConcurrent: 2 } }, browser: { enabled: true }, web: { enabled: true }, channels: { telegram: { botToken: "token" }, discord: { token: "token" }, signal: { account: "+15550000000" }, imessage: { enabled: true } } };
    ...
    onRestart?.({ changedPaths: ["gateway.port"], restartGateway: true, restartReasons: ["gateway.port"], hotReasons: [], reloadHooks: false, restartGmailWatcher: false, restartBrowserControl: false, restartCron: false, restartHeartbeat: false, restartChannels: new Set(), noopPaths: [] }, {  });
