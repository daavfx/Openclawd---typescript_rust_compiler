...
export function attachGatewayWsConnectionHandler(params) {
  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    const canvasHostUrl = resolveCanvasHostUrl({ canvasPort: canvasHostPortForWs, hostOverride: canvasHostServerPort ? canvasHostOverride : undefined, requestHost: upgradeReq.headers.host, forwardedProto: upgradeReq.headers["x-forwarded-proto"], localAddress: upgradeReq.socket?.localAddress });
    ...
    socket.once("error", (err) => {
      logWsControl.warn("error conn= remote=: ");
      close();
    });
    ...
    socket.once("close", (code, reason) => {
      ...
      if (!client) {
        ...
        logFn("closed before connect conn= remote= fwd= origin= host= ua= code= reason=", closeContext);
      }
      if ((client && isWebchatClient(client.connect.client))) {
        logWsControl.info("webchat disconnected code= reason= conn=");
      }
      ...
      logWs("out", "close", { connId, code, reason: reason?.toString(), durationMs, cause: closeCause, handshake: handshakeState, lastFrameType, lastFrameMethod, lastFrameId });
      close();
    });
    ...
    const handshakeTimer = setTimeout(() => {
      if (!client) {
        ...
        logWsControl.warn("handshake timeout conn= remote=");
        close();
      }
    }, handshakeTimeoutMs);
    attachGatewayWsMessageHandler({ socket, upgradeReq, connId, remoteAddr, forwardedFor, realIp, requestHost, requestOrigin, requestUserAgent, canvasHostUrl, connectNonce, resolvedAuth, gatewayMethods, events, extraHandlers, buildRequestContext, send, close, isClosed: () => closed, clearHandshakeTimer: () => clearTimeout(handshakeTimer), getClient: () => client, setClient: (next) => {
      client = next;
      clients.add(next);
