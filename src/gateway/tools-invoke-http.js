...
function mergeActionIntoArgsIfSupported(params) {
  ...
  const hasAction = Boolean((((schemaObj && (typeof schemaObj === "object")) && schemaObj.properties) && ("action" in schemaObj.properties)));
  if (!hasAction) {
    return args;
  }
  return { ...args: , action };
...
export async function handleToolsInvokeHttpRequest(req, res, opts) {
  ...
  const authResult = await authorizeGatewayConnect({ auth: opts.auth, connectAuth: token ? { token, password: token } : null, req, trustedProxies: (opts.trustedProxies ?? cfg.gateway?.trustedProxies) });
  if (!authResult.ok) {
  ...
  if ((process.env.VITEST && MEMORY_TOOL_NAMES.has(toolName))) {
    ...
    if ((reasons.length > 0)) {
      ...
      sendJson(res, 400, { ok: false, error: { type: "invalid_request", message: ("memory tools are disabled in tests. " + "Enable by setting plugins.slots.memory=\"memory-core\" (and ensure plugins.enabled is not false).") } });
      return true;
    }
  }
  ...
  const groupPolicy = resolveGroupToolPolicy({ config: cfg, sessionKey, messageProvider: (messageChannel ?? undefined), accountId: (accountId ?? null) });
  const subagentPolicy = isSubagentSessionKey(sessionKey) ? resolveSubagentToolPolicy(cfg) : undefined;
  const allTools = createOpenClawTools({ agentSessionKey: sessionKey, agentChannel: (messageChannel ?? undefined), agentAccountId: accountId, config: cfg, pluginToolAllowlist: collectExplicitAllowlist([profilePolicy, providerProfilePolicy, globalPolicy, globalProviderPolicy, agentPolicy, agentProviderPolicy, groupPolicy, subagentPolicy]) });
  ...
  const pluginGroups = buildPluginToolGroups({ tools: allTools, toolMeta: (tool) => getPluginToolMeta(tool) });
  ...
  if (!tool) {
    sendJson(res, 404, { ok: false, error: { type: "not_found", message: "Tool not available: " } });
    return true;
  }
  try {
    {
      const toolArgs = mergeActionIntoArgsIfSupported({ toolSchema: tool.parameters, action, args });
      const result = await tool.execute?.("http-", toolArgs);
      sendJson(res, 200, { ok: true, result });
    }
  }
  catch (err) {
    {
      sendJson(res, 400, { ok: false, error: { type: "tool_error", message: (err instanceof Error) ? err.message : String(err) } });
    }
  }
  return true;
}
