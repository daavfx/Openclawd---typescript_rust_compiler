...
import { agentCommand, getFreePort, installGatewayTestHooks, startGatewayServer } from "./test-helpers.js";
...
", "utf8");
    ...
    const result = await tool.execute("call-loopback", { sessionKey: "main", message: "ping", timeoutSeconds: 5 });
...
", "utf8");
    ...
    await callGateway({ method: "sessions.patch", params: { key: "test-labeled-session", label: "my-test-worker" }, timeoutMs: 5000 });
    const tool = createOpenClawTools().find((candidate) => (candidate.name === "sessions_send"));
    if (!tool) {
      throw new Error("missing sessions_send tool");
    }
    const result = await tool.execute("call-by-label", { label: "my-test-worker", message: "hello labeled session", timeoutSeconds: 5 });
    const details = result.details;
    expect(details.status).toBe("ok");
  ...
  it("returns error when label not found", { timeout: 60000 }, async () => {
    ...
    const result = await tool.execute("call-missing-label", { label: "nonexistent-label", message: "hello", timeoutSeconds: 5 });
    const details = result.details;
  ...
  it("returns error when neither sessionKey nor label provided", { timeout: 60000 }, async () => {
    ...
    const result = await tool.execute("call-no-key", { message: "hello", timeoutSeconds: 5 });
    const details = result.details;
    expect(details.status).toBe("error");
