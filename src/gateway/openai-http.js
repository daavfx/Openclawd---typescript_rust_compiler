...
function buildAgentPrompt(messagesUnknown) {
  ...
  for (const msg of messages) {
    ...
    const sender = (normalizedRole === "assistant") ? "Assistant" : (normalizedRole === "user") ? "User" : name ? "Tool:" : "Tool";
...
export async function handleOpenAiHttpRequest(req, res, opts) {
  ...
  if (!prompt.message) {
    sendJson(res, 400, { error: { message: "Missing user message in `messages`.", type: "invalid_request_error" } });
    return true;
  }
  const runId = "chatcmpl_";
  const deps = createDefaultDeps();
  if (!stream) {
    try {
      {
        const result = await agentCommand({ message: prompt.message, extraSystemPrompt: prompt.extraSystemPrompt, sessionKey, runId, deliver: false, messageChannel: "webchat", bestEffortDeliver: false }, defaultRuntime, deps);
...
") : "No response from OpenClaw.";
        sendJson(res, 200, { id: runId, object: "chat.completion", created: Math.floor((Date.now() / 1000)), model, choices: [{ index: 0, message: { role: "assistant", content }, finish_reason: "stop" }], usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } });
      }
    }
    catch (err) {
      {
  ...
  const unsubscribe = onAgentEvent((evt) => {
    ...
    if ((evt.stream === "assistant")) {
      ...
      if (!wroteRole) {
        wroteRole = true;
        writeSse(res, { id: runId, object: "chat.completion.chunk", created: Math.floor((Date.now() / 1000)), model, choices: [{ index: 0, delta: { role: "assistant" } }] });
      }
      sawAssistantDelta = true;
      writeSse(res, { id: runId, object: "chat.completion.chunk", created: Math.floor((Date.now() / 1000)), model, choices: [{ index: 0, delta: { content }, finish_reason: null }] });
      return;
    }
...
") : "No response from OpenClaw.";
    ...
    catch (err) {
      {
        ...
        writeSse(res, { id: runId, object: "chat.completion.chunk", created: Math.floor((Date.now() / 1000)), model, choices: [{ index: 0, delta: { content: "Error: " }, finish_reason: "stop" }] });
