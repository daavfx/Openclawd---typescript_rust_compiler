...
const AVATAR_MIME_BY_EXT = { ".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg", ".webp": "image/webp", ".gif": "image/gif", ".svg": "image/svg+xml", ".bmp": "image/bmp", ".tif": "image/tiff", ".tiff": "image/tiff" };
...
export function listAgentsForGateway(cfg) {
  ...
  for (const entry of (cfg.agents?.list ?? [])) {
    ...
    const identity = entry.identity ? { name: (entry.identity.name?.trim() || undefined), theme: (entry.identity.theme?.trim() || undefined), emoji: (entry.identity.emoji?.trim() || undefined), avatar: (entry.identity.avatar?.trim() || undefined), avatarUrl: resolveIdentityAvatarUrl(cfg, normalizeAgentId(entry.id), entry.identity.avatar?.trim()) } : undefined;
    configuredById.set(normalizeAgentId(entry.id), { name: ((typeof entry.name === "string") && entry.name.trim()) ? entry.name.trim() : undefined, identity });
  }
  ...
  const agents = agentIds.map((id) => {
    ...
    return { id, name: meta?.name, identity: meta?.identity };
  });
...
export function resolveSessionStoreKey(params) {
  ...
  if (parsed) {
    ...
    const canonical = canonicalizeMainSessionAlias({ cfg: params.cfg, agentId, sessionKey: raw });
    if ((canonical !== raw)) {
      return canonical;
    }
    return raw;
...
export function resolveGatewaySessionStoreTarget(params) {
  ...
  return { agentId, storePath, canonicalKey, storeKeys: Array.from(storeKeys) };
}
...
function mergeSessionEntryIntoCombined(params) {
  ...
  if ((existing && ((existing.updatedAt ?? 0) > (entry.updatedAt ?? 0)))) {
    combined[canonicalKey] = { ...entry: , ...existing: , spawnedBy: canonicalizeSpawnedByForAgent(agentId, (existing.spawnedBy ?? entry.spawnedBy)) };
  } else {
    combined[canonicalKey] = { ...existing: , ...entry: , spawnedBy: canonicalizeSpawnedByForAgent(agentId, (entry.spawnedBy ?? existing?.spawnedBy)) };
  }
}
export function loadCombinedSessionStoreForGateway(cfg) {
  ...
  if ((storeConfig && !isStorePathTemplate(storeConfig))) {
    ...
    for (const [key, entry] of Object.entries(store)) {
      ...
      mergeSessionEntryIntoCombined({ combined, entry, agentId: defaultAgentId, canonicalKey });
    }
...
export function getSessionDefaults(cfg) {
  ...
  return { modelProvider: (resolved.provider ?? null), model: (resolved.model ?? null), contextTokens: (contextTokens ?? null) };
}
...
export function listSessionsFromStore(params) {
  ...
  const activeMinutes = ((typeof opts.activeMinutes === "number") && Number.isFinite(opts.activeMinutes)) ? Math.max(1, Math.floor(opts.activeMinutes)) : undefined;
  let sessions = Object.entries(store).filter(([key]) => {
    ...
    const displayName = (((entry?.displayName ?? channel ? buildGroupDisplayName({ provider: channel, subject, groupChannel, space, id, key }) : undefined) ?? entry?.label) ?? originLabel);
    const deliveryFields = normalizeSessionDeliveryFields(entry);
    return { key, entry, kind: classifySessionKey(key, entry), label: entry?.label, displayName, channel, subject, groupChannel, space, chatType: entry?.chatType, origin, updatedAt, sessionId: entry?.sessionId, systemSent: entry?.systemSent, abortedLastRun: entry?.abortedLastRun, thinkingLevel: entry?.thinkingLevel, verboseLevel: entry?.verboseLevel, reasoningLevel: entry?.reasoningLevel, elevatedLevel: entry?.elevatedLevel, sendPolicy: entry?.sendPolicy, inputTokens: entry?.inputTokens, outputTokens: entry?.outputTokens, totalTokens: total, responseUsage: entry?.responseUsage, modelProvider: entry?.modelProvider, model: entry?.model, contextTokens: entry?.contextTokens, deliveryContext: deliveryFields.deliveryContext, lastChannel: (deliveryFields.lastChannel ?? entry?.lastChannel), lastTo: (deliveryFields.lastTo ?? entry?.lastTo), lastAccountId: (deliveryFields.lastAccountId ?? entry?.lastAccountId) };
  ...
  return { ts: now, path: storePath, count: finalSessions.length, defaults: getSessionDefaults(cfg), sessions: finalSessions };
}
