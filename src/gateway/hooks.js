...
export function resolveHooksConfig(cfg) {
  ...
  const withSlash = rawPath.startsWith("/") ? rawPath : "/";
  ...
  return { basePath: trimmed, token, maxBodyBytes, mappings };
}
...
export const getHookChannelError = () => "channel must be "
...
export function normalizeAgentPayload(payload, opts) {
  ...
  const sessionKey = ((typeof sessionKeyRaw === "string") && sessionKeyRaw.trim()) ? sessionKeyRaw.trim() : "hook:";
  ...
  const timeoutSeconds = (((typeof timeoutRaw === "number") && Number.isFinite(timeoutRaw)) && (timeoutRaw > 0)) ? Math.floor(timeoutRaw) : undefined;
  return { ok: true, value: { message, name, wakeMode, sessionKey, deliver, channel, to, model, thinking, timeoutSeconds } };
}
