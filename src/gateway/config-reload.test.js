...
describe("buildGatewayReloadPlan", () => {
  ...
  const telegramPlugin = { id: "telegram", meta: { id: "telegram", label: "Telegram", selectionLabel: "Telegram", docsPath: "/channels/telegram", blurb: "test" }, capabilities: { chatTypes: ["direct"] }, config: { listAccountIds: () => [], resolveAccount: () => {  } }, reload: { configPrefixes: ["channels.telegram"] } };
  const whatsappPlugin = { id: "whatsapp", meta: { id: "whatsapp", label: "WhatsApp", selectionLabel: "WhatsApp", docsPath: "/channels/whatsapp", blurb: "test" }, capabilities: { chatTypes: ["direct"] }, config: { listAccountIds: () => [], resolveAccount: () => {  } }, reload: { configPrefixes: ["web"], noopPrefixes: ["channels.whatsapp"] } };
  const registry = createTestRegistry([{ pluginId: "telegram", plugin: telegramPlugin, source: "test" }, { pluginId: "whatsapp", plugin: whatsappPlugin, source: "test" }]);
  beforeEach(() => {
  ...
  it("restarts providers when provider config prefixes change", () => {
    ...
    expect(plan.restartGateway).toBe(false);
    const expected = new Set(listChannelPlugins().filter((plugin) => (plugin.reload?.configPrefixes ?? []).some((prefix) => changedPaths.some((path) => ((path === prefix) || path.startsWith("."))))).map((plugin) => plugin.id));
