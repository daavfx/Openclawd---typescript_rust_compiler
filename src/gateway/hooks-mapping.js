...
export 
export 
const hookPresetMappings = { gmail: [{ id: "gmail", match: { path: "gmail" }, action: "agent", wakeMode: "now", name: "Gmail", sessionKey: "hook:gmail:{{messages[0].id}}", messageTemplate: "New email from {{messages[0].from}}
...
export function resolveHookMappings(hooks) {
  ...
  for (const preset of presets) {
    ...
    if (((preset === "gmail") && (typeof gmailAllowUnsafe === "boolean"))) {
      mappings.push(...presetMappings.map((mapping) => { ...mapping: , allowUnsafeExternalContent: gmailAllowUnsafe }));
      continue;
    }
...
function normalizeHookMapping(mapping, index, transformsDir) {
  const id = (mapping.id?.trim() || "mapping-");
  ...
  const transform = mapping.transform ? { modulePath: resolvePath(transformsDir, mapping.transform.module), exportName: (mapping.transform.export?.trim() || undefined) } : undefined;
  return { id, matchPath, matchSource, action, wakeMode, name: mapping.name, sessionKey: mapping.sessionKey, messageTemplate: mapping.messageTemplate, textTemplate: mapping.textTemplate, deliver: mapping.deliver, allowUnsafeExternalContent: mapping.allowUnsafeExternalContent, channel: mapping.channel, to: mapping.to, model: mapping.model, thinking: mapping.thinking, timeoutSeconds: mapping.timeoutSeconds, transform };
}
...
function buildActionFromMapping(mapping, ctx) {
  if ((mapping.action === "wake")) {
    ...
    return { ok: true, action: { kind: "wake", text, mode: (mapping.wakeMode ?? "now") } };
  }
  const message = renderTemplate((mapping.messageTemplate ?? ""), ctx);
  return { ok: true, action: { kind: "agent", message, name: renderOptional(mapping.name, ctx), wakeMode: (mapping.wakeMode ?? "now"), sessionKey: renderOptional(mapping.sessionKey, ctx), deliver: mapping.deliver, allowUnsafeExternalContent: mapping.allowUnsafeExternalContent, channel: mapping.channel, to: renderOptional(mapping.to, ctx), model: renderOptional(mapping.model, ctx), thinking: renderOptional(mapping.thinking, ctx), timeoutSeconds: mapping.timeoutSeconds } };
}
