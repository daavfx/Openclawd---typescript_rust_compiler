...
export async function startGatewayDiscovery(params) {
  ...
  if (bonjourEnabled) {
    try {
      {
        const bonjour = await startGatewayBonjourAdvertiser({ instanceName: formatBonjourInstanceName(params.machineDisplayName), gatewayPort: params.port, gatewayTlsEnabled: (params.gatewayTls?.enabled ?? false), gatewayTlsFingerprintSha256: params.gatewayTls?.fingerprintSha256, canvasPort: params.canvasPort, sshPort, tailnetDns, cliPath, minimal: mdnsMinimal });
        bonjourStop = bonjour.stop;
      }
    }
    catch (err) {
      {
        params.logDiscovery.warn("bonjour advertising failed: ");
      }
    }
  }
  if (params.wideAreaDiscoveryEnabled) {
    const wideAreaDomain = resolveWideAreaDiscoveryDomain({ configDomain: (params.wideAreaDiscoveryDomain ?? undefined) });
    if (!wideAreaDomain) {
      ...
      try {
        {
          ...
          const result = await writeWideAreaGatewayZone({ domain: wideAreaDomain, gatewayPort: params.port, displayName: formatBonjourInstanceName(params.machineDisplayName), tailnetIPv4, tailnetIPv6: (tailnetIPv6 ?? undefined), gatewayTlsEnabled: (params.gatewayTls?.enabled ?? false), gatewayTlsFingerprintSha256: params.gatewayTls?.fingerprintSha256, tailnetDns, sshPort, cliPath: resolveBonjourCliPath() });
          params.logDiscovery.info("wide-area DNS-SD  ( → )");
        }
      }
      catch (err) {
        {
          params.logDiscovery.warn("wide-area discovery update failed: ");
        }
      }
    }
  }
