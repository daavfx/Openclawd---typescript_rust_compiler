...
function applyToolChoice(params) {
  ...
  if ((toolChoice === "required")) {
    ...
    return { tools, extraSystemPrompt: "You must call one of the available tools before responding." };
  }
  if (((typeof toolChoice === "object") && (toolChoice.type === "function"))) {
    ...
    if ((matched.length === 0)) {
      throw new Error("tool_choice requested unknown tool: ");
    }
    return { tools: matched, extraSystemPrompt: "You must call the  tool before responding." };
  }
  return { tools };
}
export function buildAgentPrompt(input) {
  ...
  for (const item of input) {
    if ((item.type === "message")) {
      ...
      conversationEntries.push({ role: normalizedRole, entry: { sender, body: content } });
    } else {
      if ((item.type === "function_call_output")) {
        conversationEntries.push({ role: "tool", entry: { sender: "Tool:", body: item.output } });
      }
    }
  }
  let message = "";
...
function toUsage(value) {
  ...
  return { input_tokens: Math.max(0, input), output_tokens: Math.max(0, output), total_tokens: Math.max(0, total) };
}
...
function createResponseResource(params) {
  return { id: params.id, object: "response", created_at: Math.floor((Date.now() / 1000)), status: params.status, model: params.model, output: params.output, usage: (params.usage ?? createEmptyUsage()), error: params.error };
}
function createAssistantOutputItem(params) {
  return { type: "message", id: params.id, role: "assistant", content: [{ type: "output_text", text: params.text }], status: params.status };
}
export async function handleOpenResponsesHttpRequest(req, res, opts) {
  ...
  const authResult = await authorizeGatewayConnect({ auth: opts.auth, connectAuth: { token, password: token }, req, trustedProxies: opts.trustedProxies });
  if (!authResult.ok) {
  ...
  if (!parseResult.success) {
    ...
    const message = issue ? ": " : "Invalid request body";
  ...
  try {
          ...
          if (((item.type === "message") && (typeof item.content !== "string"))) {
            for (const part of item.content) {
              if ((part.type === "input_image")) {
                ...
                const imageSource = { type: sourceType, url: source.url, data: source.data, mediaType: source.media_type };
              ...
              if ((part.type === "input_file")) {
                ...
                const file = await extractFileContentFromSource({ source: { type: sourceType, url: source.url, data: source.data, mediaType: source.media_type, filename: source.filename }, limits: limits.files });
                if (file.text?.trim()) {
                  ...
                  if ((file.images && (file.images.length > 0))) {
                    fileContexts.push("<file name=\"\">[PDF content rendered to images]</file>");
                  }
                }
                if ((file.images && (file.images.length > 0))) {
    ...
    {
      sendJson(res, 400, { error: { message: String(err), type: "invalid_request_error" } });
      return true;
    }
  }
  ...
  try {
    {
      const toolChoiceResult = applyToolChoice({ tools: clientTools, toolChoice: payload.tool_choice });
...
") : undefined;
  ...
  const extraSystemPrompt = [payload.instructions, prompt.extraSystemPrompt, toolChoiceContext, fileContext].filter(Boolean).join("

");
  if (!prompt.message) {
    sendJson(res, 400, { error: { message: "Missing user message in `input`.", type: "invalid_request_error" } });
    return true;
  }
  const responseId = "resp_";
  const outputItemId = "msg_";
  const deps = createDefaultDeps();
  const streamParams = (typeof payload.max_output_tokens === "number") ? { maxTokens: payload.max_output_tokens } : undefined;
  if (!stream) {
    try {
      {
        const result = await agentCommand({ message: prompt.message, images: (images.length > 0) ? images : undefined, clientTools: (resolvedClientTools.length > 0) ? resolvedClientTools : undefined, extraSystemPrompt: (extraSystemPrompt || undefined), streamParams: (streamParams ?? undefined), sessionKey, runId: responseId, deliver: false, messageChannel: "webchat", bestEffortDeliver: false }, defaultRuntime, deps);
        ...
        if ((((stopReason === "tool_calls") && pendingToolCalls) && (pendingToolCalls.length > 0))) {
          ...
          const functionCallItemId = "call_";
          const response = createResponseResource({ id: responseId, model, status: "incomplete", output: [{ type: "function_call", id: functionCallItemId, call_id: functionCall.id, name: functionCall.name, arguments: functionCall.arguments }], usage });
...
") : "No response from OpenClaw.";
        const response = createResponseResource({ id: responseId, model, status: "completed", output: [createAssistantOutputItem({ id: outputItemId, text: content, status: "completed" })], usage });
        sendJson(res, 200, response);
      }
    }
    catch (err) {
      {
        const response = createResponseResource({ id: responseId, model, status: "failed", output: [], error: { code: "api_error", message: String(err) } });
  ...
  const maybeFinalize = () => {
    ...
    writeSseEvent(res, { type: "response.content_part.done", item_id: outputItemId, output_index: 0, content_index: 0, part: { type: "output_text", text: finalizeRequested.text } });
    ...
    const finalResponse = createResponseResource({ id: responseId, model, status: finalizeRequested.status, output: [completedItem], usage });
  ...
  const initialResponse = createResponseResource({ id: responseId, model, status: "in_progress", output: [] });
  ...
  const outputItem = createAssistantOutputItem({ id: outputItemId, text: "", status: "in_progress" });
  writeSseEvent(res, { type: "response.output_item.added", output_index: 0, item: outputItem });
  writeSseEvent(res, { type: "response.content_part.added", item_id: outputItemId, output_index: 0, content_index: 0, part: { type: "output_text", text: "" } });
    ...
    try {
      {
        ...
        if (!sawAssistantDelta) {
          ...
          if ((((stopReason === "tool_calls") && pendingToolCalls) && (pendingToolCalls.length > 0))) {
            ...
            writeSseEvent(res, { type: "response.output_text.done", item_id: outputItemId, output_index: 0, content_index: 0, text: "" });
            writeSseEvent(res, { type: "response.content_part.done", item_id: outputItemId, output_index: 0, content_index: 0, part: { type: "output_text", text: "" } });
            const completedItem = createAssistantOutputItem({ id: outputItemId, text: "", status: "completed" });
            writeSseEvent(res, { type: "response.output_item.done", output_index: 0, item: completedItem });
            const functionCallItemId = "call_";
            const functionCallItem = { type: "function_call", id: functionCallItemId, call_id: functionCall.id, name: functionCall.name, arguments: functionCall.arguments };
            writeSseEvent(res, { type: "response.output_item.added", output_index: 1, item: functionCallItem });
            writeSseEvent(res, { type: "response.output_item.done", output_index: 1, item: { ...functionCallItem: , status: "completed" } });
            const incompleteResponse = createResponseResource({ id: responseId, model, status: "incomplete", output: [completedItem, functionCallItem], usage });
            closed = true;
            unsubscribe();
...
") : "No response from OpenClaw.";
    ...
    catch (err) {
      {
        ...
        const errorResponse = createResponseResource({ id: responseId, model, status: "failed", output: [], error: { code: "api_error", message: String(err) }, usage: finalUsage });
      ...
      {
        if (!closed) {
          emitAgentEvent({ runId: responseId, stream: "lifecycle", data: { phase: "end" } });
        }
      }
    }
  }();
