...
const createStubOutboundAdapter = (channelId) => { deliveryMode: "direct", sendText: async () => { channel: channelId, messageId: "-msg" }, sendMedia: async () => { channel: channelId, messageId: "-msg" } };
const createStubChannelPlugin = (params) => { id: params.id, meta: { id: params.id, label: params.label, selectionLabel: params.label, docsPath: "/channels/", blurb: "test stub." }, capabilities: { chatTypes: ["direct"] }, config: { listAccountIds: () => [DEFAULT_ACCOUNT_ID], resolveAccount: () => {  }, isConfigured: async () => false }, status: { buildChannelSummary: async () => { configured: false, ...params.summary ? params.summary : {  }:  } }, outbound: createStubOutboundAdapter(params.id), messaging: { normalizeTarget: (raw) => raw }, gateway: { logoutAccount: async () => { cleared: false, envToken: false, loggedOut: false } } };
const createStubPluginRegistry = () => { plugins: [], tools: [], hooks: [], typedHooks: [], channels: [{ pluginId: "whatsapp", source: "test", plugin: createStubChannelPlugin({ id: "whatsapp", label: "WhatsApp" }) }, { pluginId: "telegram", source: "test", plugin: createStubChannelPlugin({ id: "telegram", label: "Telegram", summary: { tokenSource: "none", lastProbeAt: null } }) }, { pluginId: "discord", source: "test", plugin: createStubChannelPlugin({ id: "discord", label: "Discord" }) }, { pluginId: "slack", source: "test", plugin: createStubChannelPlugin({ id: "slack", label: "Slack" }) }, { pluginId: "signal", source: "test", plugin: createStubChannelPlugin({ id: "signal", label: "Signal", summary: { lastProbeAt: null } }) }, { pluginId: "imessage", source: "test", plugin: createStubChannelPlugin({ id: "imessage", label: "iMessage" }) }, { pluginId: "msteams", source: "test", plugin: createStubChannelPlugin({ id: "msteams", label: "Microsoft Teams" }) }, { pluginId: "matrix", source: "test", plugin: createStubChannelPlugin({ id: "matrix", label: "Matrix" }) }, { pluginId: "zalo", source: "test", plugin: createStubChannelPlugin({ id: "zalo", label: "Zalo" }) }, { pluginId: "zalouser", source: "test", plugin: createStubChannelPlugin({ id: "zalouser", label: "Zalo Personal" }) }, { pluginId: "bluebubbles", source: "test", plugin: createStubChannelPlugin({ id: "bluebubbles", label: "BlueBubbles" }) }], providers: [], gatewayHandlers: {  }, httpHandlers: [], httpRoutes: [], cliRegistrars: [], services: [], commands: [], diagnostics: [] };
const hoisted = vi.hoisted(() => { testTailnetIPv4: { value: undefined }, piSdkMock: { enabled: false, discoverCalls: 0, models: [] }, cronIsolatedRun: vi.fn(async () => { status: "ok", summary: "ok" }), agentCommand: vi.fn().mockResolvedValue(undefined), testIsNixMode: { value: false }, sessionStoreSaveDelayMs: { value: 0 }, embeddedRunMock: { activeIds: new Set(), abortCalls: [], waitCalls: [], waitResults: new Map() }, getReplyFromConfig: vi.fn().mockResolvedValue(undefined), sendWhatsAppMock: vi.fn().mockResolvedValue({ messageId: "msg-1", toJid: "jid-1" }) });
const pluginRegistryState = { registry: createStubPluginRegistry() };
...
const testConfigRoot = { value: path.join(os.tmpdir(), "openclaw-gateway-test--") };
...
export const testState = { agentConfig: undefined, agentsConfig: undefined, bindingsConfig: undefined, channelsConfig: undefined, sessionStorePath: undefined, sessionConfig: undefined, allowFrom: undefined, cronStorePath: undefined, cronEnabled: false, gatewayBind: undefined, gatewayAuth: undefined, gatewayControlUi: undefined, hooksConfig: undefined, canvasHostPort: undefined, legacyIssues: [], legacyParsed: {  }, migrationConfig: null, migrationChanges: [] }
...
vi.mock("../cron/isolated-agent.js", () => { runCronIsolatedAgentTurn: (...args) => cronIsolatedRun(...args) });
vi.mock("../infra/tailnet.js", () => { pickPrimaryTailnetIPv4: () => testTailnetIPv4.value, pickPrimaryTailnetIPv6: () => undefined });
vi.mock("../config/sessions.js", async () => {
  const actual = await vi.importActual("../config/sessions.js");
  return { ...actual: , saveSessionStore: vi.fn(async (storePath, store) => {
...
vi.mock("../config/config.js", async () => {
  ...
  const readConfigFileSnapshot = async () => {
    if ((testState.legacyIssues.length > 0)) {
      ...
      return { path: resolveConfigPath(), exists: true, raw, parsed: (testState.legacyParsed ?? {  }), valid: false, config: {  }, hash: hashConfigRaw(raw), issues: testState.legacyIssues.map((issue) => { path: issue.path, message: issue.message }), legacyIssues: testState.legacyIssues };
    }
    ...
    try {
      ...
      {
        return { path: configPath, exists: false, raw: null, parsed: {  }, valid: true, config: {  }, hash: hashConfigRaw(null), issues: [], legacyIssues: [] };
      }
    }
    try {
      {
        ...
        return { path: configPath, exists: true, raw, parsed, valid: true, config: parsed, hash: hashConfigRaw(raw), issues: [], legacyIssues: [] };
      }
    }
    catch (err) {
      {
        return { path: configPath, exists: true, raw: null, parsed: {  }, valid: false, config: {  }, hash: hashConfigRaw(null), issues: [{ path: "", message: "read failed: " }], legacyIssues: [] };
      }
    }
  };
...
");
  ...
  return { ...actual: , CONFIG_PATH: function() {
    return resolveConfigPath();
  }, STATE_DIR: function() {
    return path.dirname(resolveConfigPath());
  }, isNixMode: function() {
    return testIsNixMode.value;
  }, migrateLegacyConfig: (raw) => { config: (testState.migrationConfig ?? raw), changes: testState.migrationChanges }, loadConfig: () => {
    const configPath = resolveConfigPath();
    let fileConfig = {  };
    try {
      {
    ...
    const defaults = { model: { primary: "anthropic/claude-opus-4-5" }, workspace: path.join(os.tmpdir(), "openclaw-gateway-test"), ...fileDefaults: , ...testState.agentConfig:  };
    ...
    if ((testState.allowFrom !== undefined)) {
      ...
      mergedChannels.whatsapp = { ...existing: , allowFrom: testState.allowFrom };
    }
    ...
    const session = { ...fileSession: , mainKey: (fileSession.mainKey ?? "main") };
    ...
    const config = { ...fileConfig: , agents, bindings: (testState.bindingsConfig ?? fileBindings), channels, session, gateway, canvasHost, hooks, cron };
  ...
  }, validateConfigObject: (parsed) => { ok: true, config: parsed, issues: [] }, readConfigFileSnapshot, writeConfigFile };
});
vi.mock("../agents/pi-embedded.js", async () => {
  const actual = await vi.importActual("../agents/pi-embedded.js");
  return { ...actual: , isEmbeddedPiRunActive: (sessionId) => embeddedRunMock.activeIds.has(sessionId), abortEmbeddedPiRun: (sessionId) => {
...
vi.mock("../web/outbound.js", () => { sendMessageWhatsApp: (...args) => hoisted.sendWhatsAppMock(...args), sendPollWhatsApp: (...args) => hoisted.sendWhatsAppMock(...args) });
vi.mock("../channels/web/index.js", async () => {
  const actual = await vi.importActual("../channels/web/index.js");
  return { ...actual: , sendMessageWhatsApp: (...args) => hoisted.sendWhatsAppMock(...args) };
});
...
vi.mock("../cli/deps.js", async () => {
  ...
  return { ...actual: , createDefaultDeps: () => { ...base: , sendMessageWhatsApp: (...args) => hoisted.sendWhatsAppMock(...args) } };
});
