...
function drawText(params) {
  ...
  for (const raw of text) {
    ...
    drawGlyph5x7({ buf: params.buf, width: params.width, x: cursorX, y: params.y, char: ch, scale: params.scale, color: params.color });
...
export function renderCatNoncePngBase64(nonce) {
  ...
  drawText({ buf, width, x: Math.floor(((width - topWidth) / 2)), y: pad, text: top, scale, color: black });
  drawText({ buf, width, x: Math.floor(((width - bottomWidth) / 2)), y: ((pad + (7 * scale)) + gap), text: bottom, scale, color: black });
