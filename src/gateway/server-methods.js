...
const PAIRING_METHODS = new Set(["node.pair.request", "node.pair.list", "node.pair.approve", "node.pair.reject", "node.pair.verify", "device.pair.list", "device.pair.approve", "device.pair.reject", "device.token.rotate", "device.token.revoke", "node.rename"]);
const ADMIN_METHOD_PREFIXES = ["exec.approvals."];
const READ_METHODS = new Set(["health", "logs.tail", "channels.status", "status", "usage.status", "usage.cost", "tts.status", "tts.providers", "models.list", "agents.list", "agent.identity.get", "skills.status", "voicewake.get", "sessions.list", "sessions.preview", "cron.list", "cron.status", "cron.runs", "system-presence", "last-heartbeat", "node.list", "node.describe", "chat.history"]);
const WRITE_METHODS = new Set(["send", "agent", "agent.wait", "wake", "talk.mode", "tts.enable", "tts.disable", "tts.convert", "tts.setProvider", "voicewake.set", "node.invoke", "chat.send", "chat.abort", "browser.request"]);
function authorizeGatewayMethod(method, client) {
  ...
  if (NODE_ROLE_METHODS.has(method)) {
    ...
    return errorShape(ErrorCodes.INVALID_REQUEST, "unauthorized role: ");
  }
  if ((role === "node")) {
    return errorShape(ErrorCodes.INVALID_REQUEST, "unauthorized role: ");
  }
...
export const coreGatewayHandlers = { ...connectHandlers: , ...logsHandlers: , ...voicewakeHandlers: , ...healthHandlers: , ...channelsHandlers: , ...chatHandlers: , ...cronHandlers: , ...deviceHandlers: , ...execApprovalsHandlers: , ...webHandlers: , ...modelsHandlers: , ...configHandlers: , ...wizardHandlers: , ...talkHandlers: , ...ttsHandlers: , ...skillsHandlers: , ...sessionsHandlers: , ...systemHandlers: , ...updateHandlers: , ...nodeHandlers: , ...sendHandlers: , ...usageHandlers: , ...agentHandlers: , ...agentsHandlers: , ...browserHandlers:  }
export async function handleGatewayRequest(opts) {
  ...
  await handler({ req, params: (req.params ?? {  }), client, isWebchatConnect, respond, context });
}
