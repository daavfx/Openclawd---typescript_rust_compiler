...
function connectReq(ws, params) {
  const id = "c-";
  ws.send(JSON.stringify({ type: "req", id, method: "connect", params: { minProtocol: PROTOCOL_VERSION, maxProtocol: PROTOCOL_VERSION, client: { id: params.clientId, version: "dev", platform: params.platform, mode: "node" }, auth: { token: params.token, password: params.password }, role: "node", scopes: [], caps: ["canvas"], commands: ["system.notify"], permissions: {  } } }));
