...
export const configHandlers = { "config.get": async ({params, respond}) => {
  if (!validateConfigGetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid config.get params: "));
    return;
  }
  ...
  if (!validateConfigSchemaParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid config.schema params: "));
    return;
  }
  ...
  const schema = buildConfigSchema({ plugins: pluginRegistry.plugins.map((plugin) => { id: plugin.id, name: plugin.name, description: plugin.description, configUiHints: plugin.configUiHints, configSchema: plugin.configJsonSchema }), channels: listChannelPlugins().map((entry) => { id: entry.id, label: entry.meta.label, description: entry.meta.blurb, configSchema: entry.configSchema?.schema, configUiHints: entry.configSchema?.uiHints }) });
  respond(true, schema, undefined);
}, "config.set": async ({params, respond}) => {
  if (!validateConfigSetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid config.set params: "));
    return;
  }
  const snapshot = await readConfigFileSnapshot();
  ...
  if (!validated.ok) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid config", { details: { issues: validated.issues } }));
    return;
  }
  await writeConfigFile(validated.config);
  respond(true, { ok: true, path: CONFIG_PATH, config: validated.config }, undefined);
}, "config.patch": async ({params, respond}) => {
  if (!validateConfigPatchParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid config.patch params: "));
    return;
  }
  const snapshot = await readConfigFileSnapshot();
  if (!requireConfigBaseHash(params, snapshot, respond)) {
  ...
  await writeConfigFile(validated.config);
  const sessionKey = (typeof params.sessionKey === "string") ? (params.sessionKey?.trim() || undefined) : undefined;
  const note = (typeof params.note === "string") ? (params.note?.trim() || undefined) : undefined;
  const restartDelayMsRaw = params.restartDelayMs;
  const restartDelayMs = ((typeof restartDelayMsRaw === "number") && Number.isFinite(restartDelayMsRaw)) ? Math.max(0, Math.floor(restartDelayMsRaw)) : undefined;
  const payload = { kind: "config-apply", status: "ok", ts: Date.now(), sessionKey, message: (note ?? null), doctorHint: formatDoctorNonInteractiveHint(), stats: { mode: "config.patch", root: CONFIG_PATH } };
  ...
  const restart = scheduleGatewaySigusr1Restart({ delayMs: restartDelayMs, reason: "config.patch" });
  respond(true, { ok: true, path: CONFIG_PATH, config: validated.config, restart, sentinel: { path: sentinelPath, payload } }, undefined);
}, "config.apply": async ({params, respond}) => {
  if (!validateConfigApplyParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid config.apply params: "));
    return;
  }
  const snapshot = await readConfigFileSnapshot();
  if (!requireConfigBaseHash(params, snapshot, respond)) {
  ...
  const restartDelayMsRaw = params.restartDelayMs;
  const restartDelayMs = ((typeof restartDelayMsRaw === "number") && Number.isFinite(restartDelayMsRaw)) ? Math.max(0, Math.floor(restartDelayMsRaw)) : undefined;
  const payload = { kind: "config-apply", status: "ok", ts: Date.now(), sessionKey, message: (note ?? null), doctorHint: formatDoctorNonInteractiveHint(), stats: { mode: "config.apply", root: CONFIG_PATH } };
  let sentinelPath = null;
  try {
    {
      sentinelPath = await writeRestartSentinel(payload);
  ...
  const restart = scheduleGatewaySigusr1Restart({ delayMs: restartDelayMs, reason: "config.apply" });
  respond(true, { ok: true, path: CONFIG_PATH, config: validated.config, restart, sentinel: { path: sentinelPath, payload } }, undefined);
} }
