...
async function resolveLogFile(file) {
  ...
  const candidates = await Promise.all(entries.filter((entry) => (entry.isFile() && ROLLING_LOG_RE.test(entry.name))).map(async (entry) => {
    const fullPath = path.join(dir, entry.name);
    const fileStat = await fs.stat(fullPath).catch(() => null);
    return fileStat ? { path: fullPath, mtimeMs: fileStat.mtimeMs } : null;
  }));
  const sorted = candidates.filter((entry) => Boolean(entry)).sort((a, b) => (b.mtimeMs - a.mtimeMs));
...
async function readLogSlice(params) {
  ...
  let cursor = ((typeof params.cursor === "number") && Number.isFinite(params.cursor)) ? Math.max(0, Math.floor(params.cursor)) : undefined;
  let reset = false;
  let truncated = false;
  let start = 0;
  if ((cursor != null)) {
...
export const logsHandlers = { "logs.tail": async ({params, respond}) => {
  if (!validateLogsTailParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid logs.tail params: "));
    return;
  }
  const p = params;
  ...
  try {
    {
      ...
      const result = await readLogSlice({ file, cursor: p.cursor, limit: (p.limit ?? DEFAULT_LIMIT), maxBytes: (p.maxBytes ?? DEFAULT_MAX_BYTES) });
