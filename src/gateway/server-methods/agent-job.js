...
const AGENT_RUN_CACHE_TTL_MS = (10 * 60000);
...
function ensureAgentRunListener() {
  ...
  onAgentEvent((evt) => {
    ...
    recordAgentRunSnapshot({ runId: evt.runId, status: (phase === "error") ? "error" : "ok", startedAt, endedAt, error, ts: Date.now() });
  });
}
...
export async function waitForAgentJob(params) {
  ...
  return await new Promise((resolve) => {
    ...
    const unsubscribe = onAgentEvent((evt) => {
      ...
      const endedAt = (typeof evt.data?.endedAt === "number") ? evt.data.endedAt : undefined;
      const error = (typeof evt.data?.error === "string") ? evt.data.error : undefined;
      const snapshot = { runId: evt.runId, status: (phase === "error") ? "error" : "ok", startedAt, endedAt, error, ts: Date.now() };
