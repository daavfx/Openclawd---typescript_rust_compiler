...
export const execApprovalsHandlers = { "exec.approvals.get": ({params, respond}) => {
  if (!validateExecApprovalsGetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid exec.approvals.get params: "));
    return;
  }
  ensureExecApprovals();
  const snapshot = readExecApprovalsSnapshot();
  respond(true, { path: snapshot.path, exists: snapshot.exists, hash: snapshot.hash, file: redactExecApprovals(snapshot.file) }, undefined);
}, "exec.approvals.set": ({params, respond}) => {
  if (!validateExecApprovalsSetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid exec.approvals.set params: "));
    return;
  }
  ensureExecApprovals();
  const snapshot = readExecApprovalsSnapshot();
  ...
  respond(true, { path: nextSnapshot.path, exists: nextSnapshot.exists, hash: nextSnapshot.hash, file: redactExecApprovals(nextSnapshot.file) }, undefined);
}, "exec.approvals.node.get": async ({params, respond, context}) => {
  if (!validateExecApprovalsNodeGetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid exec.approvals.node.get params: "));
    return;
  }
  const {nodeId} = params;
  ...
  await respondUnavailableOnThrow(respond, async () => {
    const res = await context.nodeRegistry.invoke({ nodeId: id, command: "system.execApprovals.get", params: {  } });
    if (!res.ok) {
  ...
  if (!validateExecApprovalsNodeSetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid exec.approvals.node.set params: "));
    return;
  }
  ...
  await respondUnavailableOnThrow(respond, async () => {
    const res = await context.nodeRegistry.invoke({ nodeId: id, command: "system.execApprovals.set", params: { file, baseHash } });
    if (!res.ok) {
      respond(false, undefined, errorShape(ErrorCodes.UNAVAILABLE, (res.error?.message ?? "node invoke failed"), { details: { nodeError: (res.error ?? null) } }));
      return;
    }
