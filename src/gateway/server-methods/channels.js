...
export async function logoutChannelAccount(params) {
  ...
  const result = await params.plugin.gateway?.logoutAccount?.({ cfg: params.cfg, accountId: resolvedAccountId, account, runtime: defaultRuntime });
  if (!result) {
    throw new Error("Channel  does not support logout");
  }
  ...
  return { channel: params.channelId, accountId: resolvedAccountId, ...result: , cleared };
}

export const channelsHandlers = { "channels.status": async ({params, respond, context}) => {
  if (!validateChannelsStatusParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid channels.status params: "));
    return;
  }
  const probe = (params.probe === true);
  const timeoutMsRaw = params.timeoutMs;
  const timeoutMs = (typeof timeoutMsRaw === "number") ? Math.max(1000, timeoutMsRaw) : 10000;
  ...
  const buildChannelAccounts = async (channelId) => {
    ...
    if (!plugin) {
      return { accounts: [], defaultAccountId: DEFAULT_ACCOUNT_ID, defaultAccount: undefined, resolvedAccounts: {  } };
    }
    ...
    for (const accountId of accountIds) {
      ...
      if (((probe && enabled) && plugin.status?.probeAccount)) {
        ...
        if (configured) {
          probeResult = await plugin.status.probeAccount({ account, timeoutMs, cfg });
      ...
      if (((probe && enabled) && plugin.status?.auditAccount)) {
        ...
        if (configured) {
          auditResult = await plugin.status.auditAccount({ account, timeoutMs, cfg, probe: probeResult });
        }
      }
      const runtimeSnapshot = resolveRuntimeSnapshot(channelId, accountId, defaultAccountId);
      const snapshot = await buildChannelAccountSnapshot({ plugin, cfg, accountId, runtime: runtimeSnapshot, probe: probeResult, audit: auditResult });
      if (lastProbeAt) {
        snapshot.lastProbeAt = lastProbeAt;
      }
      const activity = getChannelActivity({ channel: channelId, accountId });
  ...
  const payload = { ts: Date.now(), channelOrder: uiCatalog.order, channelLabels: uiCatalog.labels, channelDetailLabels: uiCatalog.detailLabels, channelSystemImages: uiCatalog.systemImages, channelMeta: uiCatalog.entries, channels: {  }, channelAccounts: {  }, channelDefaultAccountId: {  } };
  ...
  for (const plugin of plugins) {
    ...
    const summary = plugin.status?.buildChannelSummary ? await plugin.status.buildChannelSummary({ account: fallbackAccount, cfg, defaultAccountId, snapshot: (defaultAccount ?? { accountId: defaultAccountId }) }) : { configured: (defaultAccount?.configured ?? false) };
  ...
  if (!validateChannelsLogoutParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid channels.logout params: "));
    return;
  }
  ...
  if (!plugin?.gateway?.logoutAccount) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "channel  does not support logout"));
    return;
  }
  ...
  try {
    {
      const payload = await logoutChannelAccount({ channelId, accountId, cfg: (snapshot.config ?? {  }), context, plugin });
      respond(true, payload, undefined);
    }
  }
  catch (err) {
