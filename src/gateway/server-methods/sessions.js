...
export const sessionsHandlers = { "sessions.list": ({params, respond}) => {
  if (!validateSessionsListParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.list params: "));
    return;
  }
  const p = params;
  const cfg = loadConfig();
  const {storePath, store} = loadCombinedSessionStoreForGateway(cfg);
  const result = listSessionsFromStore({ cfg, storePath, store, opts: p });
  respond(true, result, undefined);
}, "sessions.preview": ({params, respond}) => {
  if (!validateSessionsPreviewParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.preview params: "));
    return;
  }
  const p = params;
  ...
  if (!validateSessionsResolveParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.resolve params: "));
    return;
  }
  const p = params;
  const cfg = loadConfig();
  ...
  if (!validateSessionsPatchParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.patch params: "));
    return;
  }
  const p = params;
  ...
  const applied = await updateSessionStore(storePath, async (store) => {
    ...
    return await applySessionsPatchToStore({ cfg, store, storeKey: primaryKey, patch: p, loadGatewayModelCatalog: context.loadGatewayModelCatalog });
  });
  if (!applied.ok) {
    respond(false, undefined, applied.error);
    return;
  }
  const result = { ok: true, path: storePath, key: target.canonicalKey, entry: applied.entry };
  respond(true, result, undefined);
}, "sessions.reset": async ({params, respond}) => {
  if (!validateSessionsResetParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.reset params: "));
    return;
  }
  const p = params;
  const key = String((p.key ?? "")).trim();
  ...
  const next = await updateSessionStore(storePath, (store) => {
    ...
    const now = Date.now();
    const nextEntry = { sessionId: randomUUID(), updatedAt: now, systemSent: false, abortedLastRun: false, thinkingLevel: entry?.thinkingLevel, verboseLevel: entry?.verboseLevel, reasoningLevel: entry?.reasoningLevel, responseUsage: entry?.responseUsage, model: entry?.model, contextTokens: entry?.contextTokens, sendPolicy: entry?.sendPolicy, label: entry?.label, origin: snapshotSessionOrigin(entry), lastChannel: entry?.lastChannel, lastTo: entry?.lastTo, skillsSnapshot: entry?.skillsSnapshot, inputTokens: 0, outputTokens: 0, totalTokens: 0 };
  ...
  if (!validateSessionsDeleteParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.delete params: "));
    return;
  }
  const p = params;
  const key = String((p.key ?? "")).trim();
  ...
  if ((target.canonicalKey === mainKey)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "Cannot delete the main session ()."));
    return;
  }
  ...
  if (sessionId) {
    ...
    const ended = await waitForEmbeddedPiRunEnd(sessionId, 15000);
    if (!ended) {
      respond(false, undefined, errorShape(ErrorCodes.UNAVAILABLE, "Session  is still active; try again in a moment."));
      return;
    }
  }
  ...
  if (!validateSessionsCompactParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid sessions.compact params: "));
    return;
  }
  const p = params;
  const key = String((p.key ?? "")).trim();
  ...
  if (!filePath) {
    respond(true, { ok: true, key: target.canonicalKey, compacted: false, reason: "no transcript" }, undefined);
    return;
  }
  const raw = fs.readFileSync(filePath, "utf-8");
  const lines = raw.split(/\r?\n/).filter((l) => (l.trim().length > 0));
  if ((lines.length <= maxLines)) {
    respond(true, { ok: true, key: target.canonicalKey, compacted: false, kept: lines.length }, undefined);
    return;
  }
...
", "utf-8");
  ...
  respond(true, { ok: true, key: target.canonicalKey, compacted: true, archived, kept: keptLines.length }, undefined);
} }
