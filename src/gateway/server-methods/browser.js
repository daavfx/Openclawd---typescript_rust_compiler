...
function resolveBrowserNode(nodes, query) {
  ...
  throw new Error("ambiguous node:  (matches: )");
}
function resolveBrowserNodeTarget(params) {
  ...
  if (requested) {
    ...
    if (!resolved) {
      throw new Error("Configured browser node not connected: ");
    }
    return resolved;
  }
  if ((mode === "manual")) {
...
export const browserHandlers = { "browser.request": async ({params, respond, context}) => {
  const typed = params;
  ...
  const timeoutMs = ((typeof typed.timeoutMs === "number") && Number.isFinite(typed.timeoutMs)) ? Math.max(1, Math.floor(typed.timeoutMs)) : undefined;
  if ((!methodRaw || !path)) {
  ...
  try {
    {
      nodeTarget = resolveBrowserNodeTarget({ cfg, nodes: context.nodeRegistry.listConnected() });
    }
  }
  catch (err) {
    {
  ...
  if (nodeTarget) {
    ...
    const allowed = isNodeCommandAllowed({ command: "browser.proxy", declaredCommands: nodeTarget.commands, allowlist });
    if (!allowed.ok) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "node command not allowed", { details: { reason: allowed.reason, command: "browser.proxy" } }));
      return;
    }
    const proxyParams = { method: methodRaw, path, query, body, timeoutMs, profile: (typeof query?.profile === "string") ? query.profile : undefined };
    const res = await context.nodeRegistry.invoke({ nodeId: nodeTarget.nodeId, command: "browser.proxy", params: proxyParams, timeoutMs, idempotencyKey: crypto.randomUUID() });
    if (!res.ok) {
  ...
  const result = await dispatcher.dispatch({ method: methodRaw, path, query, body });
  if ((result.status >= 400)) {
    const message = ((result.body && (typeof result.body === "object")) && ("error" in result.body)) ? String(result.body.error) : "browser request failed ()";
