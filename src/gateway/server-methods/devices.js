...
export const deviceHandlers = { "device.pair.list": async ({params, respond}) => {
  if (!validateDevicePairListParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid device.pair.list params: "));
    return;
  }
  const list = await listDevicePairing();
  respond(true, { pending: list.pending, paired: list.paired.map((device) => redactPairedDevice(device)) }, undefined);
}, "device.pair.approve": async ({params, respond, context}) => {
  if (!validateDevicePairApproveParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid device.pair.approve params: "));
    return;
  }
  ...
  context.logGateway.info("device pairing approved device= role=");
  ...
  if (!validateDevicePairRejectParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid device.pair.reject params: "));
    return;
  }
  const {requestId} = params;
  ...
  context.broadcast("device.pair.resolved", { requestId, deviceId: rejected.deviceId, decision: "rejected", ts: Date.now() }, { dropIfSlow: true });
  respond(true, rejected, undefined);
}, "device.token.rotate": async ({params, respond, context}) => {
  if (!validateDeviceTokenRotateParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid device.token.rotate params: "));
    return;
  }
  ...
  context.logGateway.info("device token rotated device= role= scopes=");
  respond(true, { deviceId, role: entry.role, token: entry.token, scopes: entry.scopes, rotatedAtMs: (entry.rotatedAtMs ?? entry.createdAtMs) }, undefined);
}, "device.token.revoke": async ({params, respond, context}) => {
  if (!validateDeviceTokenRevokeParams(params)) {
    respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "invalid device.token.revoke params: "));
    return;
  }
  ...
  context.logGateway.info("device token revoked device= role=");
