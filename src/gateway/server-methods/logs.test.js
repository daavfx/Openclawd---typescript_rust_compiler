...
");
    ...
    const respond = vi.fn();
    await logsHandlers["logs.tail"]({ params: {  }, respond, context: {  }, client: null, req: { id: "req-1", type: "req", method: "logs.tail" }, isWebchatConnect: noop });
    expect(respond).toHaveBeenCalledWith(true, expect.objectContaining({ file: newer, lines: ["{\"msg\":\"new\"}"] }), undefined);
