...
function normalizeNodeInvokeResultParams(params) {
  ...
  if ((normalized.payloadJSON === null)) {
    ...
    if (((normalized.payloadJSON !== undefined) && (typeof normalized.payloadJSON !== "string"))) {
...
export const nodeHandlers = { "node.pair.request": async ({params, respond, context}) => {
  if (!validateNodePairRequestParams(params)) {
    respondInvalidParams({ respond, method: "node.pair.request", validator: validateNodePairRequestParams });
    return;
  }
  const p = params;
  await respondUnavailableOnThrow(respond, async () => {
    const result = await requestNodePairing({ nodeId: p.nodeId, displayName: p.displayName, platform: p.platform, version: p.version, coreVersion: p.coreVersion, uiVersion: p.uiVersion, deviceFamily: p.deviceFamily, modelIdentifier: p.modelIdentifier, caps: p.caps, commands: p.commands, remoteIp: p.remoteIp, silent: p.silent });
  ...
  if (!validateNodePairListParams(params)) {
    respondInvalidParams({ respond, method: "node.pair.list", validator: validateNodePairListParams });
    return;
  }
  await respondUnavailableOnThrow(respond, async () => {
  ...
  if (!validateNodePairApproveParams(params)) {
    respondInvalidParams({ respond, method: "node.pair.approve", validator: validateNodePairApproveParams });
    return;
  }
  ...
  await respondUnavailableOnThrow(respond, async () => {
    ...
    context.broadcast("node.pair.resolved", { requestId, nodeId: approved.node.nodeId, decision: "approved", ts: Date.now() }, { dropIfSlow: true });
    respond(true, approved, undefined);
  });
}, "node.pair.reject": async ({params, respond, context}) => {
  if (!validateNodePairRejectParams(params)) {
    respondInvalidParams({ respond, method: "node.pair.reject", validator: validateNodePairRejectParams });
    return;
  }
  const {requestId} = params;
  await respondUnavailableOnThrow(respond, async () => {
    ...
    context.broadcast("node.pair.resolved", { requestId, nodeId: rejected.nodeId, decision: "rejected", ts: Date.now() }, { dropIfSlow: true });
    respond(true, rejected, undefined);
  });
}, "node.pair.verify": async ({params, respond}) => {
  if (!validateNodePairVerifyParams(params)) {
    respondInvalidParams({ respond, method: "node.pair.verify", validator: validateNodePairVerifyParams });
    return;
  }
  ...
  await respondUnavailableOnThrow(respond, async () => {
    ...
    const nodes = [...nodeIds].map((nodeId) => {
      ...
      return { nodeId, displayName: (live?.displayName ?? paired?.displayName), platform: (live?.platform ?? paired?.platform), version: (live?.version ?? paired?.version), coreVersion: (live?.coreVersion ?? paired?.coreVersion), uiVersion: (live?.uiVersion ?? paired?.uiVersion), deviceFamily: (live?.deviceFamily ?? paired?.deviceFamily), modelIdentifier: (live?.modelIdentifier ?? paired?.modelIdentifier), remoteIp: (live?.remoteIp ?? paired?.remoteIp), caps, commands, pathEnv: live?.pathEnv, permissions: (live?.permissions ?? paired?.permissions), connectedAtMs: live?.connectedAtMs, paired: Boolean(paired), connected: Boolean(live) };
    });
    nodes.sort((a, b) => {
  ...
  if (!validateNodeDescribeParams(params)) {
    respondInvalidParams({ respond, method: "node.describe", validator: validateNodeDescribeParams });
    return;
  }
  const {nodeId} = params;
  ...
  await respondUnavailableOnThrow(respond, async () => {
    ...
    if (!nodeSession) {
      respond(false, undefined, errorShape(ErrorCodes.UNAVAILABLE, "node not connected", { details: { code: "NOT_CONNECTED" } }));
      return;
    }
    const cfg = loadConfig();
    const allowlist = resolveNodeCommandAllowlist(cfg, nodeSession);
    const allowed = isNodeCommandAllowed({ command, declaredCommands: nodeSession.commands, allowlist });
    if (!allowed.ok) {
      respond(false, undefined, errorShape(ErrorCodes.INVALID_REQUEST, "node command not allowed", { details: { reason: allowed.reason, command } }));
      return;
    }
    const res = await context.nodeRegistry.invoke({ nodeId, command, params: p.params, timeoutMs: p.timeoutMs, idempotencyKey: p.idempotencyKey });
    if (!res.ok) {
    ...
    respond(true, { ok: true, nodeId, command, payload, payloadJSON: (res.payloadJSON ?? null) }, undefined);
  });
  ...
  if (!validateNodeInvokeResultParams(normalizedParams)) {
    respondInvalidParams({ respond, method: "node.invoke.result", validator: validateNodeInvokeResultParams });
    return;
  }
  ...
  const ok = context.nodeRegistry.handleInvokeResult({ id: p.id, nodeId: p.nodeId, ok: p.ok, payload: p.payload, payloadJSON: (p.payloadJSON ?? null), error: (p.error ?? null) });
  if (!ok) {
    context.logGateway.debug("late invoke result ignored: id= node=");
