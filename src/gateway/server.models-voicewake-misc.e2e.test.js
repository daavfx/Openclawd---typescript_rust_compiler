...
import { connectOk, getFreePort, installGatewayTestHooks, occupyPort, onceMessage, piSdkMock, rpcReq, startGatewayServer, startServerWithClient, testState, testTailnetIPv4 } from "./test-helpers.js";
...
const whatsappOutbound = { deliveryMode: "direct", sendText: async ({deps, to, text}) => {
...
const whatsappPlugin = createOutboundTestPlugin({ id: "whatsapp", outbound: whatsappOutbound, label: "WhatsApp" });
...
const whatsappRegistry = createRegistry([{ pluginId: "whatsapp", source: "test", plugin: whatsappPlugin }]);
...
describe("gateway server models + voicewake", () => {
  ...
  test("voicewake.get returns defaults and voicewake.set broadcasts", { timeout: 60000 }, async () => {
    ...
    const setRes = await rpcReq(ws, "voicewake.set", { triggers: ["  hi  ", "", "there"] });
    ...
    const onDisk = JSON.parse(await fs.readFile(path.join(homeDir, ".openclaw", "settings", "voicewake.json"), "utf8"));
  ...
  test("pushes voicewake.changed to nodes on connect and on updates", async () => {
    ...
    await connectOk(nodeWs, { role: "node", client: { id: GATEWAY_CLIENT_NAMES.NODE_HOST, version: "1.0.0", platform: "ios", mode: GATEWAY_CLIENT_MODES.NODE } });
    const first = await firstEventP;
    expect(first.event).toBe("voicewake.changed");
    expect(first.payload?.triggers).toEqual(["openclaw", "claude", "computer"]);
    const broadcastP = onceMessage(nodeWs, (o) => ((o.type === "event") && (o.event === "voicewake.changed")));
    const setRes = await rpcReq(ws, "voicewake.set", { triggers: ["openclaw", "computer"] });
    expect(setRes.ok).toBe(true);
    const broadcast = await broadcastP;
    expect(broadcast.event).toBe("voicewake.changed");
    expect(broadcast.payload?.triggers).toEqual(["openclaw", "computer"]);
    nodeWs.close();
    restoreHome();
  });
  test("models.list returns model catalog", async () => {
    piSdkMock.enabled = true;
    piSdkMock.models = [{ id: "gpt-test-z", provider: "openai", contextWindow: 0 }, { id: "gpt-test-a", name: "A-Model", provider: "openai", contextWindow: 8000 }, { id: "claude-test-b", name: "B-Model", provider: "anthropic", contextWindow: 1000 }, { id: "claude-test-a", name: "A-Model", provider: "anthropic", contextWindow: 200000 }];
    const res1 = await rpcReq(ws, "models.list");
    const res2 = await rpcReq(ws, "models.list");
    expect(res1.ok).toBe(true);
    expect(res2.ok).toBe(true);
    const models = (res1.payload?.models ?? []);
    expect(models).toEqual([{ id: "claude-test-a", name: "A-Model", provider: "anthropic", contextWindow: 200000 }, { id: "claude-test-b", name: "B-Model", provider: "anthropic", contextWindow: 1000 }, { id: "gpt-test-a", name: "A-Model", provider: "openai", contextWindow: 8000 }, { id: "gpt-test-z", name: "gpt-test-z", provider: "openai" }]);
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const canvasHostUrl = resolveCanvasHostUrl({ canvasPort, requestHost: "100.64.0.1:", localAddress: "127.0.0.1" });
    expect(canvasHostUrl).toBe("http://100.64.0.1:");
    ...
    try {
      {
        ...
        const sendReq = (id) => ws.send(JSON.stringify({ type: "req", id, method: "send", params: { to: "+15550000000", message: "hi", idempotencyKey: idem } }));
        sendReq("a1");
        sendReq("a2");
  ...
  test("auto-enables configured channel plugins on startup", async () => {
    ...
    await fs.writeFile(configPath, JSON.stringify({ channels: { discord: { token: "token-123" } } }, null, 2), "utf-8");
    ...
    expect(updated.channels?.discord).toMatchObject({ token: "token-123" });
  });
