...
export function createChatRunState() {
  ...
  return { registry, buffers, deltaSentAt, abortedRuns, clear };
}
...
export function createAgentEventHandler({broadcast, nodeSendToSession, agentRunSeq, chatRunState, resolveSessionKeyForRun, clearAgentRunContext}) {
  const emitChatDelta = (sessionKey, clientRunId, seq, text) => {
    ...
    const payload = { runId: clientRunId, sessionKey, seq, state: "delta", message: { role: "assistant", content: [{ type: "text", text }], timestamp: now } };
  ...
  const emitChatFinal = (sessionKey, clientRunId, seq, jobState, error) => {
    ...
    if ((jobState === "done")) {
      const payload = { runId: clientRunId, sessionKey, seq, state: "final", message: text ? { role: "assistant", content: [{ type: "text", text }], timestamp: Date.now() } : undefined };
      if (!shouldSuppressHeartbeatBroadcast(clientRunId)) {
    ...
    const payload = { runId: clientRunId, sessionKey, seq, state: "error", errorMessage: error ? formatForLog(error) : undefined };
    broadcast("chat", payload);
    nodeSendToSession(sessionKey, "chat", payload);
  };
  ...
  return (evt) => {
    ...
    if ((evt.seq !== (last + 1))) {
      broadcast("agent", { runId: evt.runId, stream: "error", ts: Date.now(), sessionKey, data: { reason: "seq gap", expected: (last + 1), received: evt.seq } });
    }
    agentRunSeq.set(evt.runId, evt.seq);
