...
import { connectOk, getFreePort, installGatewayTestHooks, onceMessage, startGatewayServer, startServerWithClient } from "./test-helpers.js";
...
describe("gateway server health/presence", () => {
  ...
  test("broadcasts heartbeat events and serves last-heartbeat", async () => {
    ...
    ws.send(JSON.stringify({ type: "req", id: "hb-last", method: "last-heartbeat" }));
    ...
    ws.send(JSON.stringify({ type: "req", id: "hb-toggle-off", method: "set-heartbeats", params: { enabled: false } }));
  ...
  test("presence events carry seq + stateVersion", { timeout: 8000 }, async () => {
    ...
    ws.send(JSON.stringify({ type: "req", id: "evt-1", method: "system-event", params: { text: "note from test" } }));
  ...
  test("agent events stream with seq", { timeout: 8000 }, async () => {
    ...
    const evtPromise = onceMessage(ws, (o) => ((((o.type === "event") && (o.event === "agent")) && (o.payload?.runId === runId)) && (o.payload?.stream === "lifecycle")));
  ...
  test("presence broadcast reaches multiple clients", { timeout: 8000 }, async () => {
    ...
    clients[0].send(JSON.stringify({ type: "req", id: "broadcast", method: "system-event", params: { text: "fanout" } }));
  ...
  test("presence includes client fingerprint", async () => {
    const identityPath = path.join(os.tmpdir(), "openclaw-device-.json");
    const identity = loadOrCreateDeviceIdentity(identityPath);
    const role = "operator";
    const scopes = [];
    const signedAtMs = Date.now();
    const payload = buildDeviceAuthPayload({ deviceId: identity.deviceId, clientId: GATEWAY_CLIENT_NAMES.FINGERPRINT, clientMode: GATEWAY_CLIENT_MODES.UI, role, scopes, signedAtMs, token: null });
    const ws = await openClient({ role, scopes, client: { id: GATEWAY_CLIENT_NAMES.FINGERPRINT, version: "9.9.9", platform: "test", deviceFamily: "iPad", modelIdentifier: "iPad16,6", mode: GATEWAY_CLIENT_MODES.UI, instanceId: "abc" }, device: { id: identity.deviceId, publicKey: publicKeyRawBase64UrlFromPem(identity.publicKeyPem), signature: signDevicePayload(identity.privateKeyPem, payload), signedAt: signedAtMs } });
    const presenceP = onceMessage(ws, (o) => ((o.type === "res") && (o.id === "fingerprint")), 4000);
    ws.send(JSON.stringify({ type: "req", id: "fingerprint", method: "system-presence" }));
  ...
  test("cli connections are not tracked as instances", async () => {
    const cliId = "cli-";
    const ws = await openClient({ client: { id: GATEWAY_CLIENT_NAMES.CLI, version: "dev", platform: "test", mode: GATEWAY_CLIENT_MODES.CLI, instanceId: cliId } });
    const presenceP = onceMessage(ws, (o) => ((o.type === "res") && (o.id === "cli-presence")), 4000);
    ws.send(JSON.stringify({ type: "req", id: "cli-presence", method: "system-presence" }));
    const presenceRes = await presenceP;
    const entries = presenceRes.payload;
