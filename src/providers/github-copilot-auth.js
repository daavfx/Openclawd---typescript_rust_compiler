...
async function requestDeviceCode(params) {
  ...
  const res = await fetch(DEVICE_CODE_URL, { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/x-www-form-urlencoded" }, body });
  if (!res.ok) {
    throw new Error("GitHub device code failed: HTTP ");
  }
...
async function pollForAccessToken(params) {
  const bodyBase = new URLSearchParams({ client_id: CLIENT_ID, device_code: params.deviceCode, grant_type: "urn:ietf:params:oauth:grant-type:device_code" });
  while ((Date.now() < params.expiresAt)) {
    const res = await fetch(ACCESS_TOKEN_URL, { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/x-www-form-urlencoded" }, body: bodyBase });
    if (!res.ok) {
      throw new Error("GitHub device token failed: HTTP ");
    }
    const json = parseJsonResponse(await res.json());
    ...
    throw new Error("GitHub device flow error: ");
  }
  throw new Error("GitHub device code expired; run login again");
}
export async function githubCopilotLoginCommand(opts, runtime) {
  ...
  if ((store.profiles[profileId] && !opts.yes)) {
    note("Auth profile already exists: 
...
"), stylePromptTitle("Authorize"));
  ...
  const accessToken = await pollForAccessToken({ deviceCode: device.device_code, intervalMs, expiresAt });
  polling.stop("GitHub access token acquired");
  upsertAuthProfile({ profileId, credential: { type: "token", provider: "github-copilot", token: accessToken } });
  await updateConfig((cfg) => applyAuthProfileConfig(cfg, { provider: "github-copilot", profileId, mode: "token" }));
  logConfigUpdated(runtime);
  runtime.log("Auth profile:  (github-copilot/token)");
  outro("Done");
}
