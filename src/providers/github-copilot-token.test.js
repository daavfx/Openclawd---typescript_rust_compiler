...
vi.mock("../infra/json-file.js", () => { loadJsonFile, saveJsonFile });
...
describe("github-copilot token", () => {
  ...
  it("derives baseUrl from token", async () => {
    ...
    expect(deriveCopilotApiBaseUrlFromToken("token;proxy-ep=proxy.example.com;")).toBe("https://api.example.com");
    expect(deriveCopilotApiBaseUrlFromToken("token;proxy-ep=https://proxy.foo.bar;")).toBe("https://api.foo.bar");
  });
  it("uses cache when token is still valid", async () => {
    const now = Date.now();
    loadJsonFile.mockReturnValue({ token: "cached;proxy-ep=proxy.example.com;", expiresAt: (now + ((60 * 60) * 1000)), updatedAt: now });
    const {resolveCopilotApiToken} = await import("./github-copilot-token.js");
    const fetchImpl = vi.fn();
    const res = await resolveCopilotApiToken({ githubToken: "gh", fetchImpl: fetchImpl });
  ...
  it("fetches and stores token when cache is missing", async () => {
    ...
    const fetchImpl = vi.fn().mockResolvedValue({ ok: true, status: 200, json: async () => { token: "fresh;proxy-ep=https://proxy.contoso.test;", expires_at: (Math.floor((Date.now() / 1000)) + 3600) } });
    const {resolveCopilotApiToken} = await import("./github-copilot-token.js");
    const res = await resolveCopilotApiToken({ githubToken: "gh", fetchImpl: fetchImpl });
