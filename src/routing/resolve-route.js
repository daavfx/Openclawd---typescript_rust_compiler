...
export function buildAgentSessionKey(params) {
  ...
  return buildAgentPeerSessionKey({ agentId: params.agentId, mainKey: DEFAULT_MAIN_KEY, channel, accountId: params.accountId, peerKind: (peer?.kind ?? "dm"), peerId: peer ? (normalizeId(peer.id) || "unknown") : null, dmScope: params.dmScope, identityLinks: params.identityLinks });
}
...
export function resolveAgentRoute(input) {
  ...
  const choose = (agentId, matchedBy) => {
    ...
    const sessionKey = buildAgentSessionKey({ agentId: resolvedAgentId, channel, accountId, peer, dmScope, identityLinks }).toLowerCase();
    const mainSessionKey = buildAgentMainSessionKey({ agentId: resolvedAgentId, mainKey: DEFAULT_MAIN_KEY }).toLowerCase();
