...
describe("renderTable", () => {
  it("prefers shrinking flex columns to avoid wrapping non-flex labels", () => {
    const out = renderTable({ width: 40, columns: [{ key: "Item", header: "Item", minWidth: 10 }, { key: "Value", header: "Value", flex: true, minWidth: 24 }], rows: [{ Item: "Dashboard", Value: "http://127.0.0.1:18789/" }] });
    expect(out).toContain("Dashboard");
    expect(out).toMatch(/│ Dashboard\s+│/);
  });
  it("expands flex columns to fill available width", () => {
    const width = 60;
    const out = renderTable({ width, columns: [{ key: "Item", header: "Item", minWidth: 10 }, { key: "Value", header: "Value", flex: true, minWidth: 24 }], rows: [{ Item: "OS", Value: "macos 26.2 (arm64)" }] });
...
")[0] ?? "");
  ...
  it("wraps ANSI-colored cells without corrupting escape sequences", () => {
    const out = renderTable({ width: 36, columns: [{ key: "K", header: "K", minWidth: 3 }, { key: "V", header: "V", flex: true, minWidth: 10 }], rows: [{ K: "X", V: "[33m[0m" }] });
    const ESC = "";
    for (let i = 0; (i < out.length); i += 1) {
      ...
      if (((out[(i + 1)] === "]") && (out.slice((i + 2), (i + 5)) === "8;;"))) {
        const st = out.indexOf("\\", (i + 5));
        expect(st).toBeGreaterThanOrEqual(0);
        i = (st + 1);
        continue;
      }
      throw new Error("Unexpected escape sequence at index ");
    }
  });
  it("resets ANSI styling on wrapped lines", () => {
    const reset = "[0m";
    const out = renderTable({ width: 24, columns: [{ key: "K", header: "K", minWidth: 3 }, { key: "V", header: "V", flex: true, minWidth: 10 }], rows: [{ K: "X", V: "[31m" }] });
