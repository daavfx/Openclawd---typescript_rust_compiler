...
const normalizePluginEntries = (entries) => {
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    const entry = value;
    normalized[key] = { enabled: (typeof entry.enabled === "boolean") ? entry.enabled : undefined, config: ("config" in entry) ? entry.config : undefined };
  }
  return normalized;
};
export const normalizePluginsConfig = (config) => {
  const memorySlot = normalizeSlotValue(config?.slots?.memory);
  return { enabled: (config?.enabled !== false), allow: normalizeList(config?.allow), deny: normalizeList(config?.deny), loadPaths: normalizeList(config?.load?.paths), slots: { memory: (memorySlot === undefined) ? defaultSlotIdForKey("memory") : memorySlot }, entries: normalizePluginEntries(config?.entries) };
}
...
export function applyTestPluginDefaults(cfg, env = process.env) {
  ...
  if (explicitConfig) {
    ...
    return { ...cfg: , plugins: { ...plugins: , slots: { ...plugins?.slots: , memory: "none" } } };
  }
  return { ...cfg: , plugins: { ...plugins: , enabled: false, slots: { ...plugins?.slots: , memory: "none" } } };
}
...
export function resolveMemorySlotDecision(params) {
  ...
  if ((typeof params.slot === "string")) {
    ...
    return { enabled: false, reason: "memory slot set to \"\"" };
  }
  if ((params.selectedId && (params.selectedId !== params.id))) {
    return { enabled: false, reason: "memory slot already filled by \"\"" };
  }
  return { enabled: true, selected: true };
}
