...
export function loadPluginManifest(rootDir) {
  ...
  if (!fs.existsSync(manifestPath)) {
    return { ok: false, error: "plugin manifest not found: ", manifestPath };
  }
  let raw;
  try {
    {
    ...
    {
      return { ok: false, error: "failed to parse plugin manifest: ", manifestPath };
    }
  }
  if (!isRecord(raw)) {
  ...
  return { ok: true, manifest: { id, configSchema, kind, channels, providers, skills, name, description, version, uiHints }, manifestPath };
}
