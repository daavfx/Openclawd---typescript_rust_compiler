...
export function applyExclusiveSlotSelection(params) {
  ...
  const slots = { ...pluginsConfig.slots: , [slotKey]: params.selectedId };
  const inferredPrevSlot = (prevSlot ?? defaultSlotIdForKey(slotKey));
  if ((inferredPrevSlot && (inferredPrevSlot !== params.selectedId))) {
    warnings.push("Exclusive slot \"\" switched from \"\" to \"\".");
  }
  ...
  if ((disabledIds.length > 0)) {
    warnings.push("Disabled other \"\" slot plugins: .");
  }
  const changed = ((prevSlot !== params.selectedId) || (disabledIds.length > 0));
  if (!changed) {
    return { config: params.config, warnings: [], changed: false };
  }
  return { config: { ...params.config: , plugins: { ...pluginsConfig: , slots, entries } }, warnings, changed: true };
}
