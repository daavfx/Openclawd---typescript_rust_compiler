...
async function installPluginFromPackageDir(params) {
  ...
  if ((params.expectedPluginId && (params.expectedPluginId !== pluginId))) {
    return { ok: false, error: "plugin id mismatch: expected , got " };
  }
  ...
  if (((mode === "install") && await fileExists(targetDir))) {
    return { ok: false, error: "plugin already exists:  (delete it first)" };
  }
  if (dryRun) {
    return { ok: true, pluginId, targetDir, manifestName: (pkgName || undefined), version: (typeof manifest.version === "string") ? manifest.version : undefined, extensions };
  }
  ...
  if (hasDeps) {
    ...
    const npmRes = await runCommandWithTimeout(["npm", "install", "--omit=dev", "--silent"], { timeoutMs: Math.max(timeoutMs, 300000), cwd: targetDir });
    if ((npmRes.code !== 0)) {
      if (backupDir) {
        await fs.rm(targetDir, { recursive: true, force: true }).catch(() => undefined);
        await fs.rename(backupDir, targetDir).catch(() => undefined);
...
export async function installPluginFromArchive(params) {
  ...
  return await installPluginFromPackageDir({ packageDir, extensionsDir: params.extensionsDir, timeoutMs, logger, mode, dryRun: params.dryRun, expectedPluginId: params.expectedPluginId });
}
...
export async function installPluginFromDir(params) {
  ...
  return await installPluginFromPackageDir({ packageDir: dirPath, extensionsDir: params.extensionsDir, timeoutMs: params.timeoutMs, logger: params.logger, mode: params.mode, dryRun: params.dryRun, expectedPluginId: params.expectedPluginId });
}
...
export async function installPluginFromFile(params) {
  ...
  const targetFile = path.join(extensionsDir, "");
  if (((mode === "install") && await fileExists(targetFile))) {
    return { ok: false, error: "plugin already exists:  (delete it first)" };
  }
  if (dryRun) {
    return { ok: true, pluginId, targetDir: targetFile, manifestName: undefined, version: undefined, extensions: [path.basename(targetFile)] };
  }
  logger.info?.("Installing to …");
...
export async function installPluginFromNpmSpec(params) {
  ...
  const res = await runCommandWithTimeout(["npm", "pack", spec], { timeoutMs: Math.max(timeoutMs, 300000), cwd: tmpDir, env: { COREPACK_ENABLE_DOWNLOAD_PROMPT: "0" } });
  if ((res.code !== 0)) {
...
").map((l) => l.trim()).filter(Boolean).pop();
  ...
  return await installPluginFromArchive({ archivePath, extensionsDir: params.extensionsDir, timeoutMs, logger, mode, dryRun, expectedPluginId });
}
...
export async function installPluginFromPath(params) {
  ...
  if (stat.isDirectory()) {
    return await installPluginFromDir({ dirPath: resolved, extensionsDir: params.extensionsDir, timeoutMs: params.timeoutMs, logger: params.logger, mode: params.mode, dryRun: params.dryRun, expectedPluginId: params.expectedPluginId });
  }
  const archiveKind = resolveArchiveKind(resolved);
  if (archiveKind) {
    return await installPluginFromArchive({ archivePath: resolved, extensionsDir: params.extensionsDir, timeoutMs: params.timeoutMs, logger: params.logger, mode: params.mode, dryRun: params.dryRun, expectedPluginId: params.expectedPluginId });
  }
  return await installPluginFromFile({ filePath: resolved, extensionsDir: params.extensionsDir, logger: params.logger, mode: params.mode, dryRun: params.dryRun });
}
