...
let runtimeStub;
vi.mock("../../extensions/voice-call/src/runtime.js", () => { createVoiceCallRuntime: vi.fn(async () => runtimeStub) });
import plugin from "../../extensions/voice-call/index.js";
const noopLogger = { info: vi.fn(), warn: vi.fn(), error: vi.fn(), debug: vi.fn() };
function setup(config) {
  const methods = new Map();
  const tools = [];
  plugin.register({ id: "voice-call", name: "Voice Call", description: "test", version: "0", source: "test", config: {  }, pluginConfig: config, runtime: { tts: { textToSpeechTelephony: vi.fn() } }, logger: noopLogger, registerGatewayMethod: (method, handler) => methods.set(method, handler), registerTool: (tool) => tools.push(tool), registerCli: () => {
  }, registerService: () => {
  }, resolvePath: (p) => p });
...
describe("voice-call plugin", () => {
  ...
  it("tool get_status returns json payload", async () => {
    ...
    const tool = tools[0];
    const result = await tool.execute("id", { action: "get_status", callId: "call-1" });
  ...
  it("CLI start prints JSON", async () => {
    ...
    await register({ id: "voice-call", name: "Voice Call", description: "test", version: "0", source: "test", config: {  }, pluginConfig: { provider: "mock" }, runtime: { tts: { textToSpeechTelephony: vi.fn() } }, logger: noopLogger, registerGatewayMethod: () => {
    }, registerTool: () => {
    }, registerCli: (fn) => fn({ program, config: {  }, workspaceDir: undefined, logger: noopLogger }), registerService: () => {
    }, resolvePath: (p) => p });
    await program.parseAsync(["voicecall", "start", "--to", "+1", "--message", "Hello"], { from: "user" });
