...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), "openclaw-plugins-");
...
describe("discoverOpenClawPlugins", () => {
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(path.join(globalExt, "package.json"), JSON.stringify({ name: "pack", openclaw: { extensions: ["./src/one.ts", "./src/two.ts"] } }), "utf-8");
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.mkdirSync(path.join(globalExt, "src"), { recursive: true });
    fs.writeFileSync(path.join(globalExt, "package.json"), JSON.stringify({ name: "@openclaw/voice-call", openclaw: { extensions: ["./src/index.ts"] } }), "utf-8");
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    fs.writeFileSync(path.join(packDir, "package.json"), JSON.stringify({ name: "@openclaw/demo-plugin-dir", openclaw: { extensions: ["./index.js"] } }), "utf-8");
