...
export function normalizeToolParameters(tool) {
  ...
  const mergedRequired = (baseRequired && (baseRequired.length > 0)) ? baseRequired : (objectVariants > 0) ? Array.from(requiredCounts.entries()).filter(([, count]) => (count === objectVariants)).map(([key]) => key) : undefined;
  const nextSchema = { ...schema:  };
  return { ...tool: , parameters: cleanSchemaForGemini({ type: "object", ...(typeof nextSchema.title === "string") ? { title: nextSchema.title } : {  }: , ...(typeof nextSchema.description === "string") ? { description: nextSchema.description } : {  }: , properties: (Object.keys(mergedProperties).length > 0) ? mergedProperties : (schema.properties ?? {  }), ...(mergedRequired && (mergedRequired.length > 0)) ? { required: mergedRequired } : {  }: , additionalProperties: ("additionalProperties" in schema) ? schema.additionalProperties : true }) };
}
