...
export const CHUTES_AUTHORIZE_ENDPOINT = "/idp/authorize"
export const CHUTES_TOKEN_ENDPOINT = "/idp/token"
export const CHUTES_USERINFO_ENDPOINT = "/idp/userinfo"
...
export async function fetchChutesUserInfo(params) {
  ...
  const response = await fetchFn(CHUTES_USERINFO_ENDPOINT, { headers: { Authorization: "Bearer " } });
  if (!response.ok) {
    return null;
  }
...
export async function exchangeChutesCodeForTokens(params) {
  ...
  const body = new URLSearchParams({ grant_type: "authorization_code", client_id: params.app.clientId, code: params.code, redirect_uri: params.app.redirectUri, code_verifier: params.codeVerifier });
  if (params.app.clientSecret) {
    body.set("client_secret", params.app.clientSecret);
  }
  const response = await fetchFn(CHUTES_TOKEN_ENDPOINT, { method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body });
  if (!response.ok) {
    const text = await response.text();
    throw new Error("Chutes token exchange failed: ");
  }
  const data = await response.json();
  ...
  return { access, refresh, expires: coerceExpiresAt(expiresIn, now), email: info?.username, accountId: info?.sub, clientId: params.app.clientId };
}
...
export async function refreshChutesTokens(params) {
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error("Chutes token refresh failed: ");
  }
  const data = await response.json();
  const access = data.access_token?.trim();
  ...
  return { ...params.credential: , access, refresh: (newRefresh || refreshToken), expires: coerceExpiresAt(expiresIn, now), clientId };
}
