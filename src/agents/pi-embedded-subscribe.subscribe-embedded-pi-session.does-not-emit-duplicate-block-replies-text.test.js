...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("does not emit duplicate block replies when text_end repeats", () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session: session, runId: "run", onBlockReply, blockReplyBreak: "text_end" });
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_delta", delta: "Hello block" } });
  ...
  it("does not duplicate assistantTexts when message_end repeats", () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session: session, runId: "run" });
  ...
  it("does not duplicate assistantTexts when message_end repeats with trailing whitespace changes", () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session: session, runId: "run" });
    const assistantMessageWithNewline = { role: "assistant", content: [{ type: "text", text: "Hello world
" }] };
    const assistantMessageTrimmed = { role: "assistant", content: [{ type: "text", text: "Hello world" }] };
  ...
  it("does not duplicate assistantTexts when message_end repeats with reasoning blocks", () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session: session, runId: "run", reasoningMode: "on" });
    const assistantMessage = { role: "assistant", content: [{ type: "thinking", thinking: "Because" }, { type: "text", text: "Hello world" }] };
    handler?.({ type: "message_end", message: assistantMessage });
    handler?.({ type: "message_end", message: assistantMessage });
    expect(subscription.assistantTexts).toEqual(["Hello world"]);
  });
  it("populates assistantTexts for non-streaming models with chunking enabled", () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session: session, runId: "run", blockReplyChunking: { minChars: 50, maxChars: 200 } });
    handler?.({ type: "message_start", message: { role: "assistant" } });
    const assistantMessage = { role: "assistant", content: [{ type: "text", text: "Response from non-streaming model" }] };
    handler?.({ type: "message_end", message: assistantMessage });
