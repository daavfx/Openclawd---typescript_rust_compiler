...
describeLive("pi embedded extra params (live)", () => {
  it("applies config maxTokens to openai streamFn", async () => {
    ...
    const cfg = { agents: { defaults: { models: { "openai/gpt-5.2": { params: { maxTokens: 16 } } } } } };
    const agent = { streamFn: streamSimple };
    applyExtraParamsToAgent(agent, cfg, "openai", model.id);
    const stream = agent.streamFn(model, { messages: [{ role: "user", content: "Write the alphabet letters A through Z as words separated by commas.", timestamp: Date.now() }] }, { apiKey: OPENAI_KEY });
    let stopReason;
    let outputTokens;
