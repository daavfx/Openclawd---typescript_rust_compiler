...
describe("createOpenClawCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    const config = { tools: { exec: { applyPatch: { enabled: true } } } };
  ...
  it("respects apply_patch allowModels", () => {
    const config = { tools: { exec: { applyPatch: { enabled: true, allowModels: ["gpt-5.2"] } } } };
    const allowed = createOpenClawCodingTools({ config, modelProvider: "openai", modelId: "gpt-5.2" });
    expect(allowed.some((tool) => (tool.name === "apply_patch"))).toBe(true);
    const denied = createOpenClawCodingTools({ config, modelProvider: "openai", modelId: "gpt-5-mini" });
  ...
  it("keeps canonical tool names for Anthropic OAuth (pi-ai remaps on the wire)", () => {
    const tools = createOpenClawCodingTools({ modelProvider: "anthropic", modelAuthMode: "oauth" });
  ...
  it("provides top-level object schemas for all tools", () => {
    ...
    const offenders = tools.map((tool) => {
      ...
      return { name: tool.name, type: schema?.type, keys: schema ? Object.keys(schema).sort() : null };
