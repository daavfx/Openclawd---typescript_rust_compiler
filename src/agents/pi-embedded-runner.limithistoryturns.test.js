...
describe("limitHistoryTurns", () => {
  const makeMessages = (roles) => roles.map((role, i) => { role, content: [{ type: "text", text: "message " }] });
  ...
  it("preserves message content integrity", () => {
    const messages = [{ role: "user", content: [{ type: "text", text: "first" }] }, { role: "assistant", content: [{ type: "toolCall", id: "1", name: "exec", arguments: {  } }] }, { role: "user", content: [{ type: "text", text: "second" }] }, { role: "assistant", content: [{ type: "text", text: "response" }] }];
    const limited = limitHistoryTurns(messages, 1);
