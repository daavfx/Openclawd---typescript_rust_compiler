...
describe("subscribeEmbeddedPiSession reply tags", () => {
  it("carries reply_to_current across tag-only block chunks", () => {
    ...
    subscribeEmbeddedPiSession({ session: session, runId: "run", onBlockReply, blockReplyBreak: "text_end", blockReplyChunking: { minChars: 1, maxChars: 50, breakPreference: "newline" } });
    handler?.({ type: "message_start", message: { role: "assistant" } });
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_delta", delta: "[[reply_to_current]]
Hello" } });
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_end" } });
    const assistantMessage = { role: "assistant", content: [{ type: "text", text: "[[reply_to_current]]
Hello" }] };
  ...
  it("flushes trailing directive tails on stream end", () => {
    ...
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_end" } });
    const assistantMessage = { role: "assistant", content: [{ type: "text", text: "Hello [[" }] };
    handler?.({ type: "message_end", message: assistantMessage });
