...
function normalizeToolErrorText(text) {
  ...
  return (firstLine.length > TOOL_ERROR_MAX_CHARS) ? "…" : firstLine;
}
...
export function extractMessagingToolSend(toolName, args) {
  ...
  return to ? { tool: toolName, provider: providerId, accountId: (extracted.accountId ?? accountId), to } : undefined;
}
