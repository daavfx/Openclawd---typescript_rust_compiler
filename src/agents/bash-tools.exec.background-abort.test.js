...
test("background exec is not killed when tool signal aborts", async () => {
  ...
  const result = await tool.execute("toolcall", { command: "node -e \"setTimeout(() => {}, 5000)\"", background: true }, abortController.signal);
...
test("background exec still times out after tool signal abort", async () => {
  const tool = createExecTool({ allowBackground: true, backgroundMs: 0 });
  const abortController = new AbortController();
  const result = await tool.execute("toolcall", { command: "node -e \"setTimeout(() => {}, 5000)\"", background: true, timeout: 0.2 }, abortController.signal);
  expect(result.details.status).toBe("running");
  const sessionId = result.details.sessionId;
  abortController.abort();
  let finished = getFinishedSession(sessionId);
  const deadline = (Date.now() + (process.platform === "win32") ? 10000 : 2000);
...
test("yielded background exec is not killed when tool signal aborts", async () => {
  ...
  const abortController = new AbortController();
  const result = await tool.execute("toolcall", { command: "node -e \"setTimeout(() => {}, 5000)\"", yieldMs: 5 }, abortController.signal);
  expect(result.details.status).toBe("running");
  const sessionId = result.details.sessionId;
  abortController.abort();
  await sleep(150);
...
test("yielded background exec still times out", async () => {
  const tool = createExecTool({ allowBackground: true, backgroundMs: 10 });
  const result = await tool.execute("toolcall", { command: "node -e \"setTimeout(() => {}, 5000)\"", yieldMs: 5, timeout: 0.2 });
  expect(result.details.status).toBe("running");
  const sessionId = result.details.sessionId;
  let finished = getFinishedSession(sessionId);
  const deadline = (Date.now() + (process.platform === "win32") ? 10000 : 2000);
