...
describe("openclaw-tools: subagents", () => {
  ...
  it("sessions_spawn runs cleanup flow after subagent completion", async () => {
    ...
    callGatewayMock.mockImplementation(async (opts) => {
      ...
      if ((request.method === "sessions.list")) {
        return { sessions: [{ key: "main", lastChannel: "whatsapp", lastTo: "+123" }] };
      }
    ...
    const result = await tool.execute("call2", { task: "do thing", runTimeoutSeconds: 1, label: "my-task" });
  ...
  it("sessions_spawn only allows same-agent by default", async () => {
    ...
    const result = await tool.execute("call6", { task: "do thing", agentId: "beta" });
