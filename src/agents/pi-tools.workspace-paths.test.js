...
describe("workspace path resolution", () => {
        ...
        try {
          {
            ...
            await writeTool?.execute("ws-write", { path: testFile, content: contents });
        ...
        try {
          {
            ...
            await editTool?.execute("ws-edit", { path: testFile, oldText: "world", newText: "openclaw" });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("openclaw-ws-", async (workspaceDir) => {
      ...
      const result = await execTool?.execute("ws-exec", { command: "echo ok" });
      const cwd = ((result?.details && (typeof result.details === "object")) && ("cwd" in result.details)) ? result.details.cwd : undefined;
      expect(cwd).toBeTruthy();
      const [resolvedOutput, resolvedWorkspace] = await Promise.all([fs.realpath(String(cwd)), fs.realpath(workspaceDir)]);
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openclaw-ws-", async (workspaceDir) => {
      await withTempDir("openclaw-override-", async (overrideDir) => {
        ...
        const execTool = tools.find((tool) => (tool.name === "exec"));
        expect(execTool).toBeDefined();
        const result = await execTool?.execute("ws-exec-override", { command: "echo ok", workdir: overrideDir });
        const cwd = ((result?.details && (typeof result.details === "object")) && ("cwd" in result.details)) ? result.details.cwd : undefined;
        expect(cwd).toBeTruthy();
        const [resolvedOutput, resolvedOverride] = await Promise.all([fs.realpath(String(cwd)), fs.realpath(overrideDir)]);
  ...
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("openclaw-workspace-", async (workspaceDir) => {
        const sandbox = { enabled: true, sessionKey: "sandbox:test", workspaceDir: sandboxDir, agentWorkspaceDir: workspaceDir, workspaceAccess: "rw", containerName: "openclaw-sbx-test", containerWorkdir: "/workspace", docker: { image: "openclaw-sandbox:bookworm-slim", containerPrefix: "openclaw-sbx-", workdir: "/workspace", readOnlyRoot: true, tmpfs: [], network: "none", user: "1000:1000", capDrop: ["ALL"], env: { LANG: "C.UTF-8" } }, tools: { allow: [], deny: [] }, browserAllowHostControl: false };
        ...
        await writeTool?.execute("sbx-write", { path: "new.txt", content: "sandbox write" });
        const written = await fs.readFile(path.join(sandboxDir, "new.txt"), "utf8");
        expect(written).toBe("sandbox write");
        await editTool?.execute("sbx-edit", { path: "new.txt", oldText: "write", newText: "edit" });
