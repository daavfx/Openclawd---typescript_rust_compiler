...
export function suggestOAuthProfileIdForLegacyDefault(params) {
  ...
  if (configuredEmail) {
    const byEmail = oauthProfiles.find((id) => {
      ...
      return ((email === configuredEmail) || (id === ":"));
    });
    if (byEmail) {
      return byEmail;
    }
...
export function repairOAuthProfileIdMismatch(params) {
  const legacyProfileId = (params.legacyProfileId ?? ":default");
  ...
  const toProfileId = suggestOAuthProfileIdForLegacyDefault({ cfg: params.cfg, store: params.store, provider: params.provider, legacyProfileId });
  ...
  nextProfiles[toProfileId] = { ...legacyCfg: , ...toEmail ? { email: toEmail } : {  }:  };
  const providerKey = normalizeProviderId(params.provider);
  const nextOrder = () => {
  ...
  const nextCfg = { ...params.cfg: , auth: { ...params.cfg.auth: , profiles: nextProfiles, ...nextOrder ? { order: nextOrder } : {  }:  } };
  const changes = ["Auth: migrate  →  (OAuth profile id)"];
  return { config: nextCfg, changes, migrated: true, fromProfileId: legacyProfileId, toProfileId };
}
