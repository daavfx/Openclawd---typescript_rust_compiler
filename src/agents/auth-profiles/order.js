...
export function resolveAuthProfileOrder(params) {
  ...
  if ((explicitOrder && (explicitOrder.length > 0))) {
    ...
    const cooldownSorted = inCooldown.sort((a, b) => (a.cooldownUntil - b.cooldownUntil)).map((entry) => entry.profileId);
...
function orderProfilesByMode(order, store) {
  ...
  const cooldownSorted = inCooldown.map((profileId) => { profileId, cooldownUntil: (resolveProfileUnusableUntil((store.usageStats?.[profileId] ?? {  })) ?? now) }).sort((a, b) => (a.cooldownUntil - b.cooldownUntil)).map((entry) => entry.profileId);
