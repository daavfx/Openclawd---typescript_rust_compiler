...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("splits long single-line fenced blocks with reopen/close", () => {
    ...
    subscribeEmbeddedPiSession({ session: session, runId: "run", onBlockReply, blockReplyBreak: "message_end", blockReplyChunking: { minChars: 10, maxChars: 40, breakPreference: "paragraph" } });
    const text = "```json
...
```";
  ...
  it("waits for auto-compaction retry and clears buffered text", async () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session, runId: "run-1" });
    const assistantMessage = { role: "assistant", content: [{ type: "text", text: "oops" }] };
  ...
  it("resolves after compaction ends without retry", async () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session, runId: "run-2" });
    for (const listener of listeners) {
