...
export function resolveEffectiveMessagesConfig(cfg, agentId, opts) {
  return { messagePrefix: resolveMessagePrefix(cfg, agentId, { hasAllowFrom: opts?.hasAllowFrom, fallback: opts?.fallbackMessagePrefix }), responsePrefix: resolveResponsePrefix(cfg, agentId) };
}
...
export function resolveHumanDelayConfig(cfg, agentId) {
  ...
  return { mode: (overrides?.mode ?? defaults?.mode), minMs: (overrides?.minMs ?? defaults?.minMs), maxMs: (overrides?.maxMs ?? defaults?.maxMs) };
}
