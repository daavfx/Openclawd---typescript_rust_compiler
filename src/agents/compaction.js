...
export async function summarizeWithFallback(params) {
  ...
  for (const msg of messages) {
    if (isOversizedForSummary(msg, contextWindow)) {
      ...
      oversizedNotes.push("[Large  (~K tokens) omitted from summary]");
    } else {
...
" : "";
  ...
  return ("Context contained  messages ( oversized). " + "Summary unavailable due to size limits.");
}
...
export async function summarizeInStages(params) {
  ...
  const summaryMessages = partialSummaries.map((summary) => { role: "user", content: summary, timestamp: Date.now() });
...
export function pruneHistoryForContextShare(params) {
  ...
  return { messages: keptMessages, droppedMessagesList: allDroppedMessages, droppedChunks, droppedMessages, droppedTokens, keptTokens: estimateMessagesTokens(keptMessages), budgetTokens };
}
