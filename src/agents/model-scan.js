...
const TOOL_PING = { name: "ping", description: "Return OK.", parameters: Type.Object({  }) };
export 
export 
export 
...
function parseOpenRouterPricing(value) {
  ...
  return { prompt, completion, request, image, webSearch, internalReasoning };
}
...
async function fetchOpenRouterModels(fetchImpl) {
  ...
  if (!res.ok) {
    throw new Error("OpenRouter /models failed: HTTP ");
  }
  ...
  return entries.map((entry) => {
    ...
    const inferredParamB = inferParamBFromIdOrName(" ");
    const createdAtMs = normalizeCreatedAtMs(obj.created_at);
    const pricing = parseOpenRouterPricing(obj.pricing);
    return { id, name, contextLength, maxCompletionTokens, supportedParameters, supportedParametersCount, supportsToolsMeta, modality, inferredParamB, createdAtMs, pricing };
  }).filter((entry) => Boolean(entry));
}
async function probeTool(model, apiKey, timeoutMs) {
  const context = { messages: [{ role: "user", content: "Call the ping tool with {} and nothing else.", timestamp: Date.now() }], tools: [TOOL_PING] };
  const startedAt = Date.now();
  try {
    {
      const message = await withTimeout(timeoutMs, (signal) => complete(model, context, { apiKey, maxTokens: 32, temperature: 0, toolChoice: "required", signal }));
...
async function probeImage(model, apiKey, timeoutMs) {
  const context = { messages: [{ role: "user", content: [{ type: "text", text: "Reply with OK." }, { type: "image", data: BASE_IMAGE_PNG, mimeType: "image/png" }], timestamp: Date.now() }] };
  const startedAt = Date.now();
  try {
    {
      await withTimeout(timeoutMs, (signal) => complete(model, context, { apiKey, maxTokens: 16, temperature: 0, signal }));
      return { ok: true, latencyMs: (Date.now() - startedAt) };
    }
  }
  catch (err) {
...
function ensureImageInput(model) {
  ...
  return { ...model: , input: Array.from(new Set([...model.input, "image"])) };
}
...
export async function scanOpenRouterModels(options = {  }) {
  ...
  options.onProgress?.({ phase: "probe", completed: 0, total: filtered.length });
  return mapWithConcurrency(filtered, concurrency, async (entry) => {
    const isFree = isFreeOpenRouterModel(entry);
    if (!probe) {
      return { id: entry.id, name: entry.name, provider: "openrouter", modelRef: "openrouter/", contextLength: entry.contextLength, maxCompletionTokens: entry.maxCompletionTokens, supportedParametersCount: entry.supportedParametersCount, supportsToolsMeta: entry.supportsToolsMeta, modality: entry.modality, inferredParamB: entry.inferredParamB, createdAtMs: entry.createdAtMs, pricing: entry.pricing, isFree, tool: { ok: false, latencyMs: null, skipped: true }, image: { ok: false, latencyMs: null, skipped: true } };
    }
    const model = { ...baseModel: , id: entry.id, name: (entry.name || entry.id), contextWindow: (entry.contextLength ?? baseModel.contextWindow), maxTokens: (entry.maxCompletionTokens ?? baseModel.maxTokens), input: parseModality(entry.modality), reasoning: baseModel.reasoning };
    ...
    return { id: entry.id, name: entry.name, provider: "openrouter", modelRef: "openrouter/", contextLength: entry.contextLength, maxCompletionTokens: entry.maxCompletionTokens, supportedParametersCount: entry.supportedParametersCount, supportsToolsMeta: entry.supportsToolsMeta, modality: entry.modality, inferredParamB: entry.inferredParamB, createdAtMs: entry.createdAtMs, pricing: entry.pricing, isFree, tool: toolResult, image: imageResult };
  }, { onProgress: (completed, total) => options.onProgress?.({ phase: "probe", completed, total }) });
}
