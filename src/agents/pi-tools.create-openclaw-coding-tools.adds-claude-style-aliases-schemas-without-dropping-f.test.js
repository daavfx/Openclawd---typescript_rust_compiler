    ...
    try {
      {
        ...
        const result = await readTool?.execute("tool-ws-1", { path: testFile });
    ...
    try {
      {
        ...
        await writeTool?.execute("tool-ws-2", { path: testFile, content: testContent });
    ...
    try {
      {
        ...
        await editTool?.execute("tool-ws-3", { path: testFile, oldText: "world", newText: "universe" });
    ...
    try {
      {
        ...
        await writeTool?.execute("tool-alias-1", { file_path: filePath, content: "hello world" });
        await editTool?.execute("tool-alias-2", { file_path: filePath, old_string: "world", new_string: "universe" });
        const result = await readTool?.execute("tool-alias-3", { file_path: filePath });
        const textBlocks = result?.content?.filter((block) => (block.type === "text"));
        const combinedText = textBlocks?.map((block) => (block.text ?? "")).join("
");
