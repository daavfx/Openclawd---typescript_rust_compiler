...
export class EmbeddedBlockChunker {
  ...
  constructor(params) {
    ...
    while (((this.#buffer.length >= minChars) || (force && (this.#buffer.length > 0)))) {
      const breakResult = (force && (this.#buffer.length <= maxChars)) ? this.#pickSoftBreakIndex(this.#buffer, 1) : this.#pickBreakIndex(this.#buffer, force ? 1 : undefined);
...
", (newlineIdx - 1));
    ...
    if ((buffer.length >= maxChars)) {
      ...
      if (fence) {
        return { index: maxChars, fenceSplit: { closeFenceLine: "", reopenFenceLine: fence.openLine } };
      }
      return { index: maxChars };
    }
    return { index: -1 };
