...
function readCodexKeychainCredentials(options) {
  ...
  try {
    {
      const secret = execSyncImpl("security find-generic-password -s \"Codex Auth\" -a \"\" -w", { encoding: "utf8", timeout: 5000, stdio: ["pipe", "pipe", "pipe"] }).trim();
      ...
      log.info("read codex credentials from keychain", { source: "keychain", expires: new Date(expires).toISOString() });
      return { type: "oauth", provider: "openai-codex", access: accessToken, refresh: refreshToken, expires, accountId };
    }
  }
  catch {
    {
...
function readQwenCliCredentials(options) {
  ...
  return { type: "oauth", provider: "qwen-portal", access: accessToken, refresh: refreshToken, expires: expiresAt };
}
function readClaudeCliKeychainCredentials(execSyncImpl = execSync) {
  try {
    {
      const result = execSyncImpl("security find-generic-password -s \"\" -w", { encoding: "utf8", timeout: 5000, stdio: ["pipe", "pipe", "pipe"] });
      ...
      if (((typeof refreshToken === "string") && refreshToken)) {
        return { type: "oauth", provider: "anthropic", access: accessToken, refresh: refreshToken, expires: expiresAt };
      }
...
export function readClaudeCliCredentialsCached(options) {
  ...
  const value = readClaudeCliCredentials({ allowKeychainPrompt: options?.allowKeychainPrompt, platform: options?.platform, homeDir: options?.homeDir, execSync: options?.execSync });
  if ((ttlMs > 0)) {
...
export function writeClaudeCliKeychainCredentials(newCredentials, options) {
  ...
  try {
    {
      const existingResult = execSyncImpl("security find-generic-password -s \"\" -w 2>/dev/null", { encoding: "utf8", timeout: 5000, stdio: ["pipe", "pipe", "pipe"] });
      ...
      existingData.claudeAiOauth = { ...existingOauth: , accessToken: newCredentials.access, refreshToken: newCredentials.refresh, expiresAt: newCredentials.expires };
      const newValue = JSON.stringify(existingData);
      execSyncImpl("security add-generic-password -U -s \"\" -a \"\" -w ''", { encoding: "utf8", timeout: 5000, stdio: ["pipe", "pipe", "pipe"] });
      log.info("wrote refreshed credentials to claude cli keychain", { expires: new Date(newCredentials.expires).toISOString() });
      return true;
    }
  }
  catch (error) {
    {
      log.warn("failed to write credentials to claude cli keychain", { error: (error instanceof Error) ? error.message : String(error) });
      return false;
    }
  }
}
  ...
  try {
    {
      ...
      data.claudeAiOauth = { ...existingOauth: , accessToken: newCredentials.access, refreshToken: newCredentials.refresh, expiresAt: newCredentials.expires };
      saveJsonFile(credPath, data);
      log.info("wrote refreshed credentials to claude cli file", { expires: new Date(newCredentials.expires).toISOString() });
      return true;
    }
  }
  catch (error) {
    {
      log.warn("failed to write credentials to claude cli file", { error: (error instanceof Error) ? error.message : String(error) });
      return false;
    }
  }
}
...
export function readCodexCliCredentials(options) {
  const keychain = readCodexKeychainCredentials({ platform: options?.platform, execSync: options?.execSync });
  if (keychain) {
    return keychain;
  }
  ...
  return { type: "oauth", provider: "openai-codex", access: accessToken, refresh: refreshToken, expires, accountId: (typeof tokens.account_id === "string") ? tokens.account_id : undefined };
}
...
export function readCodexCliCredentialsCached(options) {
  ...
  const value = readCodexCliCredentials({ platform: options?.platform, execSync: options?.execSync });
  if ((ttlMs > 0)) {
