...
function resolveCacheTraceConfig(params) {
  ...
  return { enabled, filePath, includeMessages: (includeMessages ?? true), includePrompt: (includePrompt ?? true), includeSystem: (includeSystem ?? true) };
}
function getWriter(filePath) {
  ...
  const writer = { filePath, write: (line) => {
    queue = queue.then(() => ready).then(() => fs.appendFile(filePath, line, "utf8")).catch(() => undefined);
  } };
...
function stableStringify(value) {
  ...
  if ((value instanceof Uint8Array)) {
    return stableStringify({ type: "Uint8Array", data: Buffer.from(value).toString("base64") });
  }
...
function summarizeMessages(messages) {
  ...
  return { messageCount: messages.length, messageRoles: messages.map((msg) => msg.role), messageFingerprints, messagesDigest: digest(messageFingerprints.join("|")) };
}
...
export function createCacheTrace(params) {
  ...
  let seq = 0;
  const base = { runId: params.runId, sessionId: params.sessionId, sessionKey: params.sessionKey, provider: params.provider, modelId: params.modelId, modelApi: params.modelApi, workspaceDir: params.workspaceDir };
  const recordStage = (stage, payload = {  }) => {
    const event = { ...base: , ts: new Date().toISOString(), seq: seq += 1, stage };
...
");
  ...
  const wrapStreamFn = (streamFn) => {
    const wrapped = (model, context, options) => {
      recordStage("stream:context", { model: { id: model?.id, provider: model?.provider, api: model?.api }, system: context.system, messages: (context.messages ?? []), options: (options ?? {  }) });
      return streamFn(model, context, options);
    };
    return wrapped;
  };
  return { enabled: true, filePath: cfg.filePath, recordStage, wrapStreamFn };
}
