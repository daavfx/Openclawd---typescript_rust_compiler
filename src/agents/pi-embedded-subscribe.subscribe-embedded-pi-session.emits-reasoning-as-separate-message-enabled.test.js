...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("emits reasoning as a separate message when enabled", () => {
    ...
    subscribeEmbeddedPiSession({ session: session, runId: "run", onBlockReply, blockReplyBreak: "message_end", reasoningMode: "on" });
...
_Because it helps_");
  ...
  it.each(THINKING_TAG_CASES)("promotes <%s> tags to thinking blocks at write-time", ({open, close}) => {
    let handler;
    const session = { subscribe: (fn) => {
      handler = fn;
      return () => {
