...
describe("scanOpenRouterModels", () => {
  it("lists free models without probing", async () => {
    const fetchImpl = createFetchFixture({ data: [{ id: "acme/free-by-pricing", name: "Free By Pricing", context_length: 16384, max_completion_tokens: 1024, supported_parameters: ["tools", "tool_choice", "temperature"], modality: "text", pricing: { prompt: "0", completion: "0", request: "0", image: "0" }, created_at: 1700000000 }, { id: "acme/free-by-suffix:free", name: "Free By Suffix", context_length: 8192, supported_parameters: [], modality: "text", pricing: { prompt: "0", completion: "0" } }, { id: "acme/paid", name: "Paid", context_length: 4096, supported_parameters: ["tools"], modality: "text", pricing: { prompt: "0.000001", completion: "0.000002" } }] });
    const results = await scanOpenRouterModels({ fetchImpl, probe: false });
    expect(results.map((entry) => entry.id)).toEqual(["acme/free-by-pricing", "acme/free-by-suffix:free"]);
    ...
    try {
      {
        ...
        await expect(scanOpenRouterModels({ fetchImpl, probe: true, apiKey: "" })).rejects.toThrow(/Missing OpenRouter API key/);
      }
    }
    finally {
      {
