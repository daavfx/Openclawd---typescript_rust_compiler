...
");
  ...
  const wrapStreamFn = (streamFn) => {
    const wrapped = (model, context, options) => {
      ...
      const nextOnPayload = (payload) => {
        record({ ...base: , ts: new Date().toISOString(), stage: "request", payload, payloadDigest: digest(payload) });
  ...
  const recordUsage = (messages, error) => {
    ...
    if (!usage) {
      if (errorMessage) {
        record({ ...base: , ts: new Date().toISOString(), stage: "usage", error: errorMessage });
      }
      return;
    }
    record({ ...base: , ts: new Date().toISOString(), stage: "usage", usage, error: errorMessage });
    log.info("anthropic usage", { runId: params.runId, sessionId: params.sessionId, usage });
  };
