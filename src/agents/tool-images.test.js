...
describe("tool image sanitizing", () => {
  it("shrinks oversized images to <=5MB", async () => {
    ...
    const blocks = [{ type: "image", data: bigPng.toString("base64"), mimeType: "image/png" }];
  ...
  it("shrinks images that exceed max dimension even if size is small", async () => {
    ...
    const raw = Buffer.alloc(((width * height) * 3), 127);
    const png = await sharp(raw, { raw: { width, height, channels: 3 } }).png({ compressionLevel: 9 }).toBuffer();
    const blocks = [{ type: "image", data: png.toString("base64"), mimeType: "image/png" }];
    const out = await sanitizeContentBlocksImages(blocks, "test");
    const image = out.find((b) => (b.type === "image"));
    if ((!image || (image.type !== "image"))) {
      throw new Error("expected image block");
  ...
  it("corrects mismatched jpeg mimeType", async () => {
    const jpeg = await sharp({ create: { width: 10, height: 10, channels: 3, background: { r: 255, g: 0, b: 0 } } }).jpeg().toBuffer();
    const blocks = [{ type: "image", data: jpeg.toString("base64"), mimeType: "image/png" }];
    const out = await sanitizeContentBlocksImages(blocks, "test");
    const image = out.find((b) => (b.type === "image"));
    if ((!image || (image.type !== "image"))) {
      throw new Error("expected image block");
