...
describe("applySkillEnvOverrides", () => {
  it("sets and restores env vars", async () => {
    ...
    await writeSkill({ dir: skillDir, name: "env-skill", description: "Needs env", metadata: "{\"openclaw\":{\"requires\":{\"env\":[\"ENV_KEY\"]},\"primaryEnv\":\"ENV_KEY\"}}" });
    const entries = loadWorkspaceSkillEntries(workspaceDir, { managedSkillsDir: path.join(workspaceDir, ".managed") });
    const originalEnv = process.env.ENV_KEY;
    delete process.env.ENV_KEY;
    const restore = applySkillEnvOverrides({ skills: entries, config: { skills: { entries: { "env-skill": { apiKey: "injected" } } } } });
    try {
      {
  ...
  it("applies env overrides from snapshots", async () => {
    ...
    const skillDir = path.join(workspaceDir, "skills", "env-skill");
    await writeSkill({ dir: skillDir, name: "env-skill", description: "Needs env", metadata: "{\"openclaw\":{\"requires\":{\"env\":[\"ENV_KEY\"]},\"primaryEnv\":\"ENV_KEY\"}}" });
    const snapshot = buildWorkspaceSkillSnapshot(workspaceDir, { managedSkillsDir: path.join(workspaceDir, ".managed"), config: { skills: { entries: { "env-skill": { apiKey: "snap-key" } } } } });
    const originalEnv = process.env.ENV_KEY;
    delete process.env.ENV_KEY;
    const restore = applySkillEnvOverridesFromSnapshot({ snapshot, config: { skills: { entries: { "env-skill": { apiKey: "snap-key" } } } } });
    try {
      {
