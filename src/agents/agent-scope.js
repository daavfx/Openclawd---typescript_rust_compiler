...
export function resolveAgentConfig(cfg, agentId) {
  ...
  return { name: (typeof entry.name === "string") ? entry.name : undefined, workspace: (typeof entry.workspace === "string") ? entry.workspace : undefined, agentDir: (typeof entry.agentDir === "string") ? entry.agentDir : undefined, model: ((typeof entry.model === "string") || (entry.model && (typeof entry.model === "object"))) ? entry.model : undefined, memorySearch: entry.memorySearch, humanDelay: entry.humanDelay, heartbeat: entry.heartbeat, identity: entry.identity, groupChat: entry.groupChat, subagents: ((typeof entry.subagents === "object") && entry.subagents) ? entry.subagents : undefined, sandbox: entry.sandbox, tools: entry.tools };
}
