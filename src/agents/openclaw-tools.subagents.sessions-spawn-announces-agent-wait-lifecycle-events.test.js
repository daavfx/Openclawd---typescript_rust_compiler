  ...
  it("sessions_spawn deletes session when cleanup=delete via agent.wait", async () => {
    ...
    callGatewayMock.mockImplementation(async (opts) => {
      ...
      if ((request.method === "agent.wait")) {
        ...
        return { runId: (params?.runId ?? "run-1"), status: "ok", startedAt: 3000, endedAt: 4000 };
      }
    ...
    const result = await tool.execute("call1b", { task: "do thing", runTimeoutSeconds: 1, cleanup: "delete" });
