...
function assistantToolCall(id) {
  return { role: "assistant", content: [{ type: "toolCall", id, name: "n", arguments: {  } }] };
}
describe("guardSessionManager integration", () => {
  it("persists synthetic toolResult before subsequent assistant message", () => {
    ...
    sm.appendMessage({ role: "assistant", content: [{ type: "text", text: "followup" }] });
    ...
    expect(sanitizeToolUseResultPairing(messages).map((m) => m.role)).toEqual(["assistant", "toolResult", "assistant"]);
  });
});
