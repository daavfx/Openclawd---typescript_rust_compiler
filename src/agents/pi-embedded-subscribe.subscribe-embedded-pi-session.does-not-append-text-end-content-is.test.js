...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("does not append when text_end content is a prefix of deltas", () => {
    ...
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_end", content: "Hello" } });
  ...
  it("does not append when text_end content is already contained", () => {
    ...
    const subscription = subscribeEmbeddedPiSession({ session: session, runId: "run", onBlockReply, blockReplyBreak: "text_end" });
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_delta", delta: "Hello world" } });
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_end", content: "world" } });
    expect(onBlockReply).toHaveBeenCalledTimes(1);
    expect(subscription.assistantTexts).toEqual(["Hello world"]);
  });
  it("appends suffix when text_end content extends deltas", () => {
    ...
    handler?.({ type: "message_update", message: { role: "assistant" }, assistantMessageEvent: { type: "text_end", content: "Hello world" } });
    expect(onBlockReply).toHaveBeenCalledTimes(1);
    expect(subscription.assistantTexts).toEqual(["Hello world"]);
  });
});
