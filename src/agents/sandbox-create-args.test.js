...
describe("buildSandboxCreateArgs", () => {
  it("includes hardening and resource flags", () => {
    ...
    expect(ulimitValues).toEqual(expect.arrayContaining(["nofile=1024:2048", "nproc=128", "core=0"]));
  });
  it("emits -v flags for custom binds", () => {
    const cfg = { image: "openclaw-sandbox:bookworm-slim", containerPrefix: "openclaw-sbx-", workdir: "/workspace", readOnlyRoot: false, tmpfs: [], network: "none", capDrop: [], binds: ["/home/user/source:/source:rw", "/var/run/docker.sock:/var/run/docker.sock"] };
    const args = buildSandboxCreateArgs({ name: "openclaw-sbx-binds", cfg, scopeKey: "main", createdAtMs: 1700000000000 });
  ...
  it("omits -v flags when binds is empty or undefined", () => {
    const cfg = { image: "openclaw-sandbox:bookworm-slim", containerPrefix: "openclaw-sbx-", workdir: "/workspace", readOnlyRoot: false, tmpfs: [], network: "none", capDrop: [], binds: [] };
    const args = buildSandboxCreateArgs({ name: "openclaw-sbx-no-binds", cfg, scopeKey: "main", createdAtMs: 1700000000000 });
    const customVFlags = [];
    for (let i = 0; (i < args.length); i++) {
      if ((args[i] === "-v")) {
        const value = args[(i + 1)];
