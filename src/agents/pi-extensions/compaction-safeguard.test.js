...
import { getCompactionSafeguardRuntime, setCompactionSafeguardRuntime } from "./compaction-safeguard-runtime.js";
import { __testing } from "./compaction-safeguard.js";
const {collectToolFailures, formatToolFailuresSection, computeAdaptiveChunkRatio, isOversizedForSummary, BASE_CHUNK_RATIO, MIN_CHUNK_RATIO, SAFETY_MARGIN} = __testing;
describe("compaction-safeguard tool failures", () => {
  it("formats tool failures with meta and summary", () => {
    const messages = [{ role: "toolResult", toolCallId: "call-1", toolName: "exec", isError: true, details: { status: "failed", exitCode: 1 }, content: [{ type: "text", text: "ENOENT: missing file" }], timestamp: Date.now() }, { role: "toolResult", toolCallId: "call-2", toolName: "read", isError: false, content: [{ type: "text", text: "ok" }], timestamp: Date.now() }];
  ...
  it("dedupes by toolCallId and handles empty output", () => {
    const messages = [{ role: "toolResult", toolCallId: "call-1", toolName: "exec", isError: true, details: { exitCode: 2 }, content: [], timestamp: Date.now() }, { role: "toolResult", toolCallId: "call-1", toolName: "exec", isError: true, content: [{ type: "text", text: "ignored" }], timestamp: Date.now() }];
    const failures = collectToolFailures(messages);
    expect(failures).toHaveLength(1);
    const section = formatToolFailuresSection(failures);
  ...
  it("caps the number of failures and adds overflow line", () => {
    const messages = Array.from({ length: 9 }, (_, idx) => { role: "toolResult", toolCallId: "call-", toolName: "exec", isError: true, content: [{ type: "text", text: "error " }], timestamp: Date.now() });
    const failures = collectToolFailures(messages);
    const section = formatToolFailuresSection(failures);
    expect(section).toContain("## Tool Failures");
  ...
  it("omits section when there are no tool failures", () => {
    const messages = [{ role: "toolResult", toolCallId: "ok", toolName: "exec", isError: false, content: [{ type: "text", text: "ok" }], timestamp: Date.now() }];
    const failures = collectToolFailures(messages);
    const section = formatToolFailuresSection(failures);
    expect(section).toBe("");
  });
...
describe("computeAdaptiveChunkRatio", () => {
  ...
  it("returns BASE_CHUNK_RATIO for normal messages", () => {
    const messages = [{ role: "user", content: "x".repeat(1000), timestamp: Date.now() }, { role: "assistant", content: [{ type: "text", text: "y".repeat(1000) }], timestamp: Date.now() }];
  ...
  it("reduces ratio when average message > 10% of context", () => {
    const messages = [{ role: "user", content: "x".repeat((50000 * 4)), timestamp: Date.now() }, { role: "assistant", content: [{ type: "text", text: "y".repeat((50000 * 4)) }], timestamp: Date.now() }];
    const ratio = computeAdaptiveChunkRatio(messages, CONTEXT_WINDOW);
  ...
  it("respects MIN_CHUNK_RATIO floor", () => {
    const messages = [{ role: "user", content: "x".repeat((150000 * 4)), timestamp: Date.now() }];
    const ratio = computeAdaptiveChunkRatio(messages, CONTEXT_WINDOW);
    expect(ratio).toBeGreaterThanOrEqual(MIN_CHUNK_RATIO);
  });
  ...
  it("handles single huge message", () => {
    const messages = [{ role: "user", content: "x".repeat((180000 * 4)), timestamp: Date.now() }];
    const ratio = computeAdaptiveChunkRatio(messages, CONTEXT_WINDOW);
    expect(ratio).toBeGreaterThanOrEqual(MIN_CHUNK_RATIO);
...
describe("isOversizedForSummary", () => {
  ...
  it("returns true for messages > 50% of context", () => {
    const msg = { role: "user", content: "x".repeat((120000 * 4)), timestamp: Date.now() };
  ...
  it("applies safety margin", () => {
    ...
    const msg = { role: "user", content: "x".repeat(Math.floor((halfContextChars * 4))), timestamp: Date.now() };
