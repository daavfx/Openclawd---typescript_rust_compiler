...
function resolveImageFallbackCandidates(params) {
  ...
  const addRaw = (raw, enforceAllowlist) => {
    const resolved = resolveModelRefFromString({ raw: String((raw ?? "")), defaultProvider: params.defaultProvider, aliasIndex });
    if (!resolved) {
      return;
    }
...
function resolveFallbackCandidates(params) {
  const primary = params.cfg ? resolveConfiguredModelRef({ cfg: params.cfg, defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL }) : null;
...
export async function runWithModelFallback(params) {
  ...
  for (let i = 0; (i < candidates.length); i += 1) {
    ...
    if (authStore) {
      ...
      if (((profileIds.length > 0) && !isAnyProfileAvailable)) {
        attempts.push({ provider: candidate.provider, model: candidate.model, error: "Provider  is in cooldown (all profiles unavailable)", reason: "rate_limit" });
        continue;
      }
    }
    try {
    ...
    catch (err) {
      {
        ...
        const normalized = (coerceToFailoverError(err, { provider: candidate.provider, model: candidate.model }) ?? err);
        if (!isFailoverError(normalized)) {
          throw err;
        }
        lastError = normalized;
        const described = describeFailoverError(normalized);
        attempts.push({ provider: candidate.provider, model: candidate.model, error: described.message, reason: described.reason, status: described.status, code: described.code });
        await params.onError?.({ provider: candidate.provider, model: candidate.model, error: normalized, attempt: (i + 1), total: candidates.length });
      }
    }
  }
  ...
  const summary = (attempts.length > 0) ? attempts.map((attempt) => "/: ").join(" | ") : "unknown";
  throw new Error("All models failed (): ", { cause: (lastError instanceof Error) ? lastError : undefined });
}

export async function runWithImageModelFallback(params) {
  const candidates = resolveImageFallbackCandidates({ cfg: params.cfg, defaultProvider: DEFAULT_PROVIDER, modelOverride: params.modelOverride });
  ...
  for (let i = 0; (i < candidates.length); i += 1) {
    ...
    try {
    ...
    catch (err) {
      {
        ...
        lastError = err;
        attempts.push({ provider: candidate.provider, model: candidate.model, error: (err instanceof Error) ? err.message : String(err) });
  ...
  const summary = (attempts.length > 0) ? attempts.map((attempt) => "/: ").join(" | ") : "unknown";
  throw new Error("All image models failed (): ", { cause: (lastError instanceof Error) ? lastError : undefined });
}
