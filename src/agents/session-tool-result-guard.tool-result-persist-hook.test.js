...
function writeTempPlugin(params) {
  ...
  const file = path.join(pluginDir, ".mjs");
  fs.writeFileSync(file, params.body, "utf-8");
  fs.writeFileSync(path.join(pluginDir, "openclaw.plugin.json"), JSON.stringify({ id: params.id, configSchema: EMPTY_PLUGIN_SCHEMA }, null, 2), "utf-8");
  return file;
}
afterEach(() => {
...
describe("tool_result_persist hook", () => {
  it("does not modify persisted toolResult messages when no hook is registered", () => {
    const sm = guardSessionManager(SessionManager.inMemory(), { agentId: "main", sessionKey: "main" });
    sm.appendMessage({ role: "assistant", content: [{ type: "toolCall", id: "call_1", name: "read", arguments: {  } }] });
    sm.appendMessage({ role: "toolResult", toolCallId: "call_1", isError: false, content: [{ type: "text", text: "ok" }], details: { big: "x".repeat(10000) } });
  ...
  it("composes transforms in priority order and allows stripping toolResult.details", () => {
    ...
    const pluginA = writeTempPlugin({ dir: tmp, id: "persist-a", body: "export default { id: \"persist-a\", register(api) {
  api.on(\"tool_result_persist\", (event, ctx) => {
    ...
    const pluginB = writeTempPlugin({ dir: tmp, id: "persist-b", body: "export default { id: \"persist-b\", register(api) {
  api.on(\"tool_result_persist\", (event) => {
    ...
    loadOpenClawPlugins({ cache: false, workspaceDir: tmp, config: { plugins: { load: { paths: [pluginA, pluginB] }, allow: ["persist-a", "persist-b"] } } });
    const sm = guardSessionManager(SessionManager.inMemory(), { agentId: "main", sessionKey: "main" });
    sm.appendMessage({ role: "assistant", content: [{ type: "toolCall", id: "call_1", name: "read", arguments: {  } }] });
    sm.appendMessage({ role: "toolResult", toolCallId: "call_1", isError: false, content: [{ type: "text", text: "ok" }], details: { big: "x".repeat(10000) } });
    const messages = sm.getEntries().filter((e) => (e.type === "message")).map((e) => e.message);
