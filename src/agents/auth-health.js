...
function buildProfileHealth(params) {
  ...
  if ((credential.type === "api_key")) {
    return { profileId, provider: credential.provider, type: "api_key", status: "static", source, label };
  }
  if ((credential.type === "token")) {
    const expiresAt = ((typeof credential.expires === "number") && Number.isFinite(credential.expires)) ? credential.expires : undefined;
    if ((!expiresAt || (expiresAt <= 0))) {
      return { profileId, provider: credential.provider, type: "token", status: "static", source, label };
    }
    const {status, remainingMs} = resolveOAuthStatus(expiresAt, now, warnAfterMs);
    return { profileId, provider: credential.provider, type: "token", status, expiresAt, remainingMs, source, label };
  }
  const {status, remainingMs} = resolveOAuthStatus(credential.expires, now, warnAfterMs);
  return { profileId, provider: credential.provider, type: "oauth", status, expiresAt: credential.expires, remainingMs, source, label };
}
export function buildAuthHealthSummary(params) {
  ...
  const profiles = Object.entries(params.store.profiles).filter(([_, cred]) => providerFilter ? providerFilter.has(cred.provider) : true).map(([profileId, credential]) => buildProfileHealth({ profileId, credential, store: params.store, cfg: params.cfg, now, warnAfterMs })).sort((a, b) => {
  ...
  for (const profile of profiles) {
    ...
    if (!existing) {
      providersMap.set(profile.provider, { provider: profile.provider, status: "missing", profiles: [profile] });
    } else {
  ...
  if (providerFilter) {
    for (const provider of providerFilter) {
      if (!providersMap.has(provider)) {
        providersMap.set(provider, { provider, status: "missing", profiles: [] });
      }
    }
  }
