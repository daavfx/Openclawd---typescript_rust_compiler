...
function resolveAwsSdkAuthInfo() {
  ...
  if (process.env[AWS_BEARER_ENV]?.trim()) {
    return { mode: "aws-sdk", source: resolveEnvSourceLabel({ applied, envVars: [AWS_BEARER_ENV], label: AWS_BEARER_ENV }) };
  }
  if ((process.env[AWS_ACCESS_KEY_ENV]?.trim() && process.env[AWS_SECRET_KEY_ENV]?.trim())) {
    return { mode: "aws-sdk", source: resolveEnvSourceLabel({ applied, envVars: [AWS_ACCESS_KEY_ENV, AWS_SECRET_KEY_ENV], label: " + " }) };
  }
  if (process.env[AWS_PROFILE_ENV]?.trim()) {
    return { mode: "aws-sdk", source: resolveEnvSourceLabel({ applied, envVars: [AWS_PROFILE_ENV], label: AWS_PROFILE_ENV }) };
  }
...
export async function resolveApiKeyForProvider(params) {
  ...
  if (profileId) {
    const resolved = await resolveApiKeyForProfile({ cfg, store, profileId, agentDir: params.agentDir });
    if (!resolved) {
      throw new Error("No credentials found for profile \"\".");
    }
    const mode = store.profiles[profileId]?.type;
    return { apiKey: resolved.apiKey, profileId, source: "profile:", mode: (mode === "oauth") ? "oauth" : (mode === "token") ? "token" : "api-key" };
  }
  ...
  for (const candidate of order) {
    try {
      {
        const resolved = await resolveApiKeyForProfile({ cfg, store, profileId: candidate, agentDir: params.agentDir });
        if (resolved) {
          const mode = store.profiles[candidate]?.type;
          return { apiKey: resolved.apiKey, profileId: candidate, source: "profile:", mode: (mode === "oauth") ? "oauth" : (mode === "token") ? "token" : "api-key" };
        }
      }
    }
    catch {
  ...
  if (envResolved) {
    return { apiKey: envResolved.apiKey, source: envResolved.source, mode: envResolved.source.includes("OAUTH_TOKEN") ? "oauth" : "api-key" };
  }
  ...
  throw new Error(["No API key found for provider \"\".", "Auth store:  (agentDir: ).", "Configure auth for this agent () or copy auth-profiles.json from the main agentDir."].join(" "));
}
...
export function resolveEnvApiKey(provider) {
  ...
  const pick = (envVar) => {
    ...
    const source = applied.has(envVar) ? "shell env: " : "env: ";
  ...
  const envMap = { openai: "OPENAI_API_KEY", google: "GEMINI_API_KEY", groq: "GROQ_API_KEY", deepgram: "DEEPGRAM_API_KEY", cerebras: "CEREBRAS_API_KEY", xai: "XAI_API_KEY", openrouter: "OPENROUTER_API_KEY", "vercel-ai-gateway": "AI_GATEWAY_API_KEY", moonshot: "MOONSHOT_API_KEY", "kimi-code": "KIMICODE_API_KEY", minimax: "MINIMAX_API_KEY", xiaomi: "XIAOMI_API_KEY", synthetic: "SYNTHETIC_API_KEY", venice: "VENICE_API_KEY", mistral: "MISTRAL_API_KEY", opencode: "OPENCODE_API_KEY" };
...
export async function getApiKeyForModel(params) {
  return resolveApiKeyForProvider({ provider: params.model.provider, cfg: params.cfg, profileId: params.profileId, preferredProfile: params.preferredProfile, store: params.store, agentDir: params.agentDir });
}
