...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return { ...actual: , loadConfig: () => configOverride, resolveGatewayPort: () => 18789 };
});
...
describe("openclaw-tools: subagents", () => {
  ...
  it("sessions_spawn normalizes allowlisted agent ids", async () => {
    ...
    configOverride = { session: { mainKey: "main", scope: "per-sender" }, agents: { list: [{ id: "main", subagents: { allowAgents: ["Research"] } }] } };
    let childSessionKey;
    ...
    const tool = createOpenClawTools({ agentSessionKey: "main", agentChannel: "whatsapp" }).find((candidate) => (candidate.name === "sessions_spawn"));
    if (!tool) {
      throw new Error("missing sessions_spawn tool");
    }
    const result = await tool.execute("call10", { task: "do thing", agentId: "research" });
  ...
  it("sessions_spawn forbids cross-agent spawning when not allowed", async () => {
    resetSubagentRegistryForTests();
    callGatewayMock.mockReset();
    configOverride = { session: { mainKey: "main", scope: "per-sender" }, agents: { list: [{ id: "main", subagents: { allowAgents: ["alpha"] } }] } };
    const tool = createOpenClawTools({ agentSessionKey: "main", agentChannel: "whatsapp" }).find((candidate) => (candidate.name === "sessions_spawn"));
    if (!tool) {
      throw new Error("missing sessions_spawn tool");
    }
    const result = await tool.execute("call9", { task: "do thing", agentId: "beta" });
  ...
  it("sessions_spawn runs cleanup via lifecycle events", async () => {
    ...
    const tool = createOpenClawTools({ agentSessionKey: "discord:group:req", agentChannel: "discord" }).find((candidate) => (candidate.name === "sessions_spawn"));
    if (!tool) {
      throw new Error("missing sessions_spawn tool");
    }
    const result = await tool.execute("call1", { task: "do thing", runTimeoutSeconds: 1, cleanup: "delete" });
    expect(result.details).toMatchObject({ status: "accepted", runId: "run-1" });
    if (!childRunId) {
      throw new Error("missing child runId");
    }
    emitAgentEvent({ runId: childRunId, stream: "lifecycle", data: { phase: "end", startedAt: 1234, endedAt: 2345 } });
  ...
  it("sessions_spawn announces with requester accountId", async () => {
    ...
    const tool = createOpenClawTools({ agentSessionKey: "main", agentChannel: "whatsapp", agentAccountId: "kev" }).find((candidate) => (candidate.name === "sessions_spawn"));
    if (!tool) {
      throw new Error("missing sessions_spawn tool");
    }
    const result = await tool.execute("call2", { task: "do thing", runTimeoutSeconds: 1, cleanup: "keep" });
    expect(result.details).toMatchObject({ status: "accepted", runId: "run-1" });
    if (!childRunId) {
      throw new Error("missing child runId");
    }
    emitAgentEvent({ runId: childRunId, stream: "lifecycle", data: { phase: "end", startedAt: 1000, endedAt: 2000 } });
    await new Promise((resolve) => setTimeout(resolve, 0));
    await new Promise((resolve) => setTimeout(resolve, 0));
    await new Promise((resolve) => setTimeout(resolve, 0));
    const agentCalls = calls.filter((call) => (call.method === "agent"));
