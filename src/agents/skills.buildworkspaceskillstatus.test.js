...
describe("buildWorkspaceSkillStatus", () => {
  it("reports missing requirements and install options", async () => {
    ...
    await writeSkill({ dir: skillDir, name: "status-skill", description: "Needs setup", metadata: "{\"openclaw\":{\"requires\":{\"bins\":[\"fakebin\"],\"env\":[\"ENV_KEY\"],\"config\":[\"browser.enabled\"]},\"install\":[{\"id\":\"brew\",\"kind\":\"brew\",\"formula\":\"fakebin\",\"bins\":[\"fakebin\"],\"label\":\"Install fakebin\"}]}}" });
    const report = buildWorkspaceSkillStatus(workspaceDir, { managedSkillsDir: path.join(workspaceDir, ".managed"), config: { browser: { enabled: false } } });
  ...
  it("respects OS-gated skills", async () => {
    ...
    await writeSkill({ dir: skillDir, name: "os-skill", description: "Darwin only", metadata: "{\"openclaw\":{\"os\":[\"darwin\"]}}" });
    const report = buildWorkspaceSkillStatus(workspaceDir, { managedSkillsDir: path.join(workspaceDir, ".managed") });
    ...
    try {
      {
        ...
        const report = buildWorkspaceSkillStatus(workspaceDir, { managedSkillsDir: path.join(workspaceDir, ".managed"), config: { skills: { allowBundled: ["other-skill"] } } });
