  ...
  for (const msg of messages) {
    ...
    if (((msgRole === lastRole) && (lastRole === "assistant"))) {
      ...
      if ((lastMsg && (typeof lastMsg === "object"))) {
        ...
        const merged = { ...lastAsst: , content: mergedContent, ...(currentMsg.usage && { usage: currentMsg.usage }): , ...(currentMsg.stopReason && { stopReason: currentMsg.stopReason }): , ...(currentMsg.errorMessage && { errorMessage: currentMsg.errorMessage }):  };
...
export function mergeConsecutiveUserTurns(previous, current) {
  ...
  return { ...current: , content: mergedContent, timestamp: (current.timestamp ?? previous.timestamp) };
}
