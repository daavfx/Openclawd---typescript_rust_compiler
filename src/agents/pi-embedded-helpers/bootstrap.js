...
function trimBootstrapContent(content, fileName, maxChars) {
  ...
  const marker = ["", "[...truncated, read  for full content...]", "…(truncated : kept + chars of )…", ""].join("
");
...
export async function ensureSessionHeader(params) {
  ...
  const entry = { type: "session", version: sessionVersion, id: params.sessionId, timestamp: new Date().toISOString(), cwd: params.cwd };
...
export function buildBootstrapContextFiles(files, opts) {
  ...
  for (const file of files) {
    if (file.missing) {
      result.push({ path: file.name, content: "[MISSING] Expected at: " });
      continue;
    }
    ...
    if (trimmed.truncated) {
      opts?.warn?.("workspace bootstrap file  is  chars (limit ); truncating in injected context");
    }
...
export function sanitizeGoogleTurnOrdering(messages) {
  ...
  const bootstrap = { role: "user", content: GOOGLE_TURN_ORDER_BOOTSTRAP_TEXT, timestamp: Date.now() };
