...
const HTTP_ERROR_HINTS = ["error", "bad request", "not found", "unauthorized", "forbidden", "internal server", "service unavailable", "gateway", "rate limit", "overloaded", "timeout", "timed out", "invalid", "too many requests", "permission"];
...
export function parseApiErrorInfo(raw) {
  ...
  const requestId = (typeof payload.request_id === "string") ? payload.request_id : (typeof payload.requestId === "string") ? payload.requestId : undefined;
  ...
  return { httpCode, type: (errType ?? topType), message: (errMessage ?? topMessage), requestId };
}
...
export function formatRawAssistantErrorForUi(raw) {
  ...
  if (info?.message) {
    const prefix = info.httpCode ? "HTTP " : "LLM error";
    const type = info.type ? " " : "";
    const requestId = info.requestId ? " (request_id: )" : "";
    return ": ";
  }
  return (trimmed.length > 600) ? "…" : trimmed;
}
...
export function formatAssistantErrorText(msg, opts) {
  ...
  if (unknownTool?.[1]) {
    const rewritten = formatSandboxToolPolicyBlockedMessage({ cfg: opts?.cfg, sessionKey: opts?.sessionKey, toolName: unknownTool[1] });
    if (rewritten) {
      return rewritten;
    }
  }
...
const ERROR_PATTERNS = { rateLimit: [/rate[_ ]limit|too many requests|429/, "exceeded your current quota", "resource has been exhausted", "quota exceeded", "resource_exhausted", "usage limit"], overloaded: [/overloaded_error|"type"\s*:\s*"overloaded_error"/i, "overloaded"], timeout: ["timeout", "timed out", "deadline exceeded", "context deadline exceeded"], billing: [/\b402\b/, "payment required", "insufficient credits", "credit balance", "plans & billing"], auth: [/invalid[_ ]?api[_ ]?key/, "incorrect api key", "invalid token", "authentication", "re-authenticate", "oauth token refresh failed", "unauthorized", "forbidden", "access denied", "expired", "token has expired", /\b401\b/, /\b403\b/, "no credentials found", "no api key found"], format: ["string should match pattern", "tool_use.id", "tool_use_id", "messages.1.content.1.tool_use.id", "invalid request format"] };
...
export function parseImageDimensionError(raw) {
  ...
  return { maxDimensionPx: limitMatch?.[1] ? Number.parseInt(limitMatch[1], 10) : undefined, messageIndex: pathMatch?.[1] ? Number.parseInt(pathMatch[1], 10) : undefined, contentIndex: pathMatch?.[2] ? Number.parseInt(pathMatch[2], 10) : undefined, raw };
}
...
export function parseImageSizeError(raw) {
  ...
  return { maxMb: match?.[1] ? Number.parseFloat(match[1]) : undefined, raw };
}
