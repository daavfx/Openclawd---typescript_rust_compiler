...
describe("buildAuthHealthSummary", () => {
  ...
  it("classifies OAuth and API key profiles", () => {
    ...
    const store = { version: 1, profiles: { "anthropic:ok": { type: "oauth", provider: "anthropic", access: "access", refresh: "refresh", expires: ((now + DEFAULT_OAUTH_WARN_MS) + 60000) }, "anthropic:expiring": { type: "oauth", provider: "anthropic", access: "access", refresh: "refresh", expires: (now + 10000) }, "anthropic:expired": { type: "oauth", provider: "anthropic", access: "access", refresh: "refresh", expires: (now - 10000) }, "anthropic:api": { type: "api_key", provider: "anthropic", key: "sk-ant-api" } } };
    const summary = buildAuthHealthSummary({ store, warnAfterMs: DEFAULT_OAUTH_WARN_MS });
    const statuses = Object.fromEntries(summary.profiles.map((profile) => [profile.profileId, profile.status]));
