...
describe("resolveAgentAvatar", () => {
  it("resolves local avatar from config when inside workspace", async () => {
    ...
    const cfg = { agents: { list: [{ id: "main", workspace, identity: { avatar: "avatars/main.png" } }] } };
  ...
  it("rejects avatars outside the workspace", async () => {
    ...
    const cfg = { agents: { list: [{ id: "main", workspace, identity: { avatar: outsidePath } }] } };
    const resolved = resolveAgentAvatar(cfg, "main");
  ...
  it("falls back to IDENTITY.md when config has no avatar", async () => {
    ...
    await writeFile(avatarPath);
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(path.join(workspace, "IDENTITY.md"), "- Avatar: avatars/fallback.png
", "utf-8");
    const cfg = { agents: { list: [{ id: "main", workspace }] } };
    const workspaceReal = await fs.realpath(workspace);
    const resolved = resolveAgentAvatar(cfg, "main");
    expect(resolved.kind).toBe("local");
    if ((resolved.kind === "local")) {
  ...
  it("accepts remote and data avatars", () => {
    const cfg = { agents: { list: [{ id: "main", identity: { avatar: "https://example.com/avatar.png" } }, { id: "data", identity: { avatar: "data:image/png;base64,aaaa" } }] } };
