...
async function sendAnnounce(item) {
  ...
  await callGateway({ method: "agent", params: { sessionKey: item.sessionKey, message: item.prompt, channel: origin?.channel, accountId: origin?.accountId, to: origin?.to, threadId, deliver: true, idempotencyKey: crypto.randomUUID() }, expectFinal: true, timeoutMs: 60000 });
}
...
async function maybeQueueSubagentAnnounce(params) {
  ...
  const queueSettings = resolveQueueSettings({ cfg, channel: (entry?.channel ?? entry?.lastChannel), sessionEntry: entry });
  ...
  if ((isActive && (shouldFollowup || (queueSettings.mode === "steer")))) {
    ...
    enqueueAnnounce({ key: canonicalKey, item: { prompt: params.triggerMessage, summaryLine: params.summaryLine, enqueuedAt: Date.now(), sessionKey: canonicalKey, origin }, settings: queueSettings, send: sendAnnounce });
    return "queued";
  }
  return "none";
}
async function buildSubagentStatsLine(params) {
  ...
  const sessionId = entry?.sessionId;
  const transcriptPath = (sessionId && storePath) ? path.join(path.dirname(storePath), ".jsonl") : undefined;
  const input = entry?.inputTokens;
  const output = entry?.outputTokens;
  const total = (entry?.totalTokens ?? ((typeof input === "number") && (typeof output === "number")) ? (input + output) : undefined);
  const runtimeMs = ((typeof params.startedAt === "number") && (typeof params.endedAt === "number")) ? Math.max(0, (params.endedAt - params.startedAt)) : undefined;
  const provider = entry?.modelProvider;
  const model = entry?.model;
  const costConfig = resolveModelCost({ provider, model, config: cfg });
  const cost = ((costConfig && (typeof input === "number")) && (typeof output === "number")) ? (((input * costConfig.input) + (output * costConfig.output)) / 1000000) : undefined;
  const parts = [];
...
export function buildSubagentSystemPrompt(params) {
  ...
  const lines = ["# Subagent Context", "", "You are a **subagent** spawned by the main agent for a specific task.", "", "## Your Role", "- You were created to handle: ", "- Complete this task. That's your entire purpose.", "- You are NOT the main agent. Don't try to be.", "", "## Rules", "1. **Stay focused** - Do your assigned task, nothing else", "2. **Complete the task** - Your final message will be automatically reported to the main agent", "3. **Don't initiate** - No heartbeats, no proactive actions, no side quests", "4. **Be ephemeral** - You may be terminated after task completion. That's fine.", "", "## Output Format", "When complete, your final response should include:", "- What you accomplished or found", "- Any relevant details the main agent should know", "- Keep it concise but informative", "", "## What You DON'T Do", "- NO user conversations (that's main agent's job)", "- NO external messages (email, tweets, etc.) unless explicitly tasked", "- NO cron jobs or persistent state", "- NO pretending to be the main agent", "- NO using the `message` tool directly", "", "## Session Context", params.label ? "- Label: " : undefined, params.requesterSessionKey ? "- Requester session: ." : undefined, params.requesterOrigin?.channel ? "- Requester channel: ." : undefined, "- Your session: .", ""].filter((line) => (line !== undefined));
  return lines.join("
");
}
  ...
  try {
    {
      ...
      if ((!reply && (params.waitForCompletion !== false))) {
        ...
        const wait = await callGateway({ method: "agent.wait", params: { runId: params.childRunId, timeoutMs: waitMs }, timeoutMs: (waitMs + 2000) });
      ...
      const statsLine = await buildSubagentStatsLine({ sessionKey: params.childSessionKey, startedAt: params.startedAt, endedAt: params.endedAt });
      const statusLabel = (outcome.status === "ok") ? "completed successfully" : (outcome.status === "timeout") ? "timed out" : (outcome.status === "error") ? "failed: " : "finished with unknown status";
      const taskLabel = ((params.label || params.task) || "background task");
      const triggerMessage = ["A background task \"\" just .", "", "Findings:", (reply || "(no output)"), "", statsLine, "", "Summarize this naturally for the user. Keep it brief (1-2 sentences). Flow it into the conversation naturally.", "Do not mention technical details like tokens, stats, or that this was a background task.", "You can respond with NO_REPLY if no announcement is needed (e.g., internal task with no user-facing result)."].join("
");
      const queued = await maybeQueueSubagentAnnounce({ requesterSessionKey: params.requesterSessionKey, triggerMessage, summaryLine: taskLabel, requesterOrigin });
      if ((queued === "steered")) {
        didAnnounce = true;
        return true;
      }
  ...
  finally {
    {
      if (params.label) {
        try {
          {
            await callGateway({ method: "sessions.patch", params: { key: params.childSessionKey, label: params.label }, timeoutMs: 10000 });
          }
        }
        catch {
          {
      ...
      if ((params.cleanup === "delete")) {
        try {
          {
            await callGateway({ method: "sessions.delete", params: { key: params.childSessionKey, deleteTranscript: true }, timeoutMs: 10000 });
          }
        }
        catch {
          {
