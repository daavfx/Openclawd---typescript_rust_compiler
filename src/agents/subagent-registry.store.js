...
export function loadSubagentRegistryFromDisk() {
  ...
  for (const [runId, entry] of Object.entries(runsRaw)) {
    ...
    const legacyCompletedAt = (isLegacy && (typeof typed.announceCompletedAt === "number")) ? typed.announceCompletedAt : undefined;
    const cleanupCompletedAt = (typeof typed.cleanupCompletedAt === "number") ? typed.cleanupCompletedAt : legacyCompletedAt;
    const cleanupHandled = (typeof typed.cleanupHandled === "boolean") ? typed.cleanupHandled : isLegacy ? Boolean((typed.announceHandled ?? cleanupCompletedAt)) : undefined;
    const requesterOrigin = normalizeDeliveryContext((typed.requesterOrigin ?? { channel: (typeof typed.requesterChannel === "string") ? typed.requesterChannel : undefined, accountId: (typeof typed.requesterAccountId === "string") ? typed.requesterAccountId : undefined }));
    ...
    out.set(runId, { ...rest: , requesterOrigin, cleanupCompletedAt, cleanupHandled });
    if (isLegacy) {
      migrated = true;
    }
  }
...
export function saveSubagentRegistryToDisk(runs) {
  ...
  const out = { version: REGISTRY_VERSION, runs: serialized };
