...
describe("resolveHumanDelayConfig", () => {
  ...
  it("merges defaults with per-agent overrides", () => {
    const cfg = { agents: { defaults: { humanDelay: { mode: "natural", minMs: 800, maxMs: 1800 } }, list: [{ id: "main", humanDelay: { mode: "custom", minMs: 400 } }] } };
    expect(resolveHumanDelayConfig(cfg, "main")).toEqual({ mode: "custom", minMs: 400, maxMs: 1800 });
  });
});
