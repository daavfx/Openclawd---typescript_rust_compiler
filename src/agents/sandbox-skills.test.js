...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess) => {
    ...
    await writeSkill({ dir: path.join(workspaceDir, "skills", "demo-skill"), name: "demo-skill", description: "Demo skill" });
    const cfg = { agents: { defaults: { sandbox: { mode: "all", scope: "session", workspaceAccess, workspaceRoot: path.join(stateDir, "sandboxes") } } } };
    const context = await resolveSandboxContext({ config: cfg, sessionKey: "agent:main:main", workspaceDir });
