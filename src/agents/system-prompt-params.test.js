...
async function makeTempDir(label) {
  return fs.mkdtemp(path.join(os.tmpdir(), "openclaw--"));
}
async function makeRepoRoot(root) {
  await fs.mkdir(path.join(root, ".git"), { recursive: true });
}
function buildParams(params) {
  return buildSystemPromptParams({ config: params.config, workspaceDir: params.workspaceDir, cwd: params.cwd, runtime: { host: "host", os: "os", arch: "arch", node: "node", model: "model" } });
}
describe("buildSystemPromptParams repo root", () => {
  ...
  it("uses configured repoRoot when valid", async () => {
    ...
    const config = { agents: { defaults: { repoRoot } } };
  ...
  it("ignores invalid repoRoot config and auto-detects", async () => {
    ...
    await fs.mkdir(workspaceDir, { recursive: true });
    await makeRepoRoot(repoRoot);
    const config = { agents: { defaults: { repoRoot: path.join(temp, "missing") } } };
    const {runtimeInfo} = buildParams({ config, workspaceDir });
    expect(runtimeInfo.repoRoot).toBe(repoRoot);
  });
