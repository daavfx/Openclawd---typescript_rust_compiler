...
function createStreamFnWithExtraParams(baseStreamFn, extraParams, provider, modelId) {
  ...
  log.debug("creating streamFn wrapper with params: ");
  const underlying = (baseStreamFn ?? streamSimple);
  const wrappedStreamFn = (model, context, options) => underlying(model, context, { ...streamParams: , ...options:  });
...
export function applyExtraParamsToAgent(agent, cfg, provider, modelId, extraParamsOverride) {
  ...
  const override = (extraParamsOverride && (Object.keys(extraParamsOverride).length > 0)) ? Object.fromEntries(Object.entries(extraParamsOverride).filter(([, value]) => (value !== undefined))) : undefined;
  const merged = Object.assign({  }, extraParams, override);
  const wrappedStreamFn = createStreamFnWithExtraParams(agent.streamFn, merged, provider, modelId);
  if (wrappedStreamFn) {
    log.debug("applying extraParams to agent streamFn for /");
