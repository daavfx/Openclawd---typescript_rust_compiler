export function buildEmbeddedSandboxInfo(sandbox, execElevated) {
  ...
  return { enabled: true, workspaceDir: sandbox.workspaceDir, workspaceAccess: sandbox.workspaceAccess, agentWorkspaceMount: (sandbox.workspaceAccess === "ro") ? "/agent" : undefined, browserBridgeUrl: sandbox.browser?.bridgeUrl, browserNoVncUrl: sandbox.browser?.noVncUrl, hostBrowserAllowed: sandbox.browserAllowHostControl, ...elevatedAllowed ? { elevated: { allowed: true, defaultLevel: (execElevated?.defaultLevel ?? "off") } } : {  }:  };
}
