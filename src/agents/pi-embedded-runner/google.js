...
const GOOGLE_SCHEMA_UNSUPPORTED_KEYWORDS = new Set(["patternProperties", "additionalProperties", "$schema", "$id", "$ref", "$defs", "definitions", "examples", "minLength", "maxLength", "minimum", "maximum", "multipleOf", "pattern", "format", "minItems", "maxItems", "uniqueItems", "minProperties", "maxProperties"]);
  ...
  for (const msg of messages) {
    ...
    for (const block of assistant.content) {
      ...
      if ((rec.thinkingSignature !== candidate)) {
        const nextBlock = { ...block: , thinkingSignature: candidate };
...
function findUnsupportedSchemaKeywords(schema, path) {
  ...
  if (Array.isArray(schema)) {
    return schema.flatMap((item, index) => findUnsupportedSchemaKeywords(item, "[]"));
  }
  const record = schema;
  const violations = [];
  ...
  if (properties) {
    for (const [key, value] of Object.entries(properties)) {
      violations.push(...findUnsupportedSchemaKeywords(value, ".properties."));
    }
  }
  for (const [key, value] of Object.entries(record)) {
    ...
    if ((value && (typeof value === "object"))) {
      violations.push(...findUnsupportedSchemaKeywords(value, "."));
    }
  }
  return violations;
}
export function sanitizeToolsForGoogle(params) {
  ...
  return params.tools.map((tool) => {
    ...
    return { ...tool: , parameters: cleanToolSchemaForGemini(tool.parameters) };
  });
}
...
export function logToolSchemasForGoogle(params) {
  ...
  const toolNames = params.tools.map((tool, index) => ":");
  const tools = sanitizeToolsForGoogle(params);
  log.info("google tool schema snapshot", { provider: params.provider, toolCount: tools.length, tools: toolNames });
  for (const [index, tool] of tools.entries()) {
    const violations = findUnsupportedSchemaKeywords(tool.parameters, ".parameters");
    if ((violations.length > 0)) {
      log.warn("google tool schema has unsupported keywords", { index, tool: tool.name, violations: violations.slice(0, 12), violationCount: violations.length });
    }
  }
}
...
registerUnhandledRejectionHandler((reason) => {
  ...
  log.error("Auto-compaction failed (unhandled): ");
...
export function applyGoogleTurnOrderingFix(params) {
  ...
  if ((didPrepend && !hasGoogleTurnOrderingMarker(params.sessionManager))) {
    ...
    warn("google turn ordering fixup: prepended user bootstrap (sessionId=)");
...
export async function sanitizeSessionHistory(params) {
  const policy = (params.policy ?? resolveTranscriptPolicy({ modelApi: params.modelApi, provider: params.provider, modelId: params.modelId }));
  const sanitizedImages = await sanitizeSessionMessagesImages(params.messages, "session:history", { sanitizeMode: policy.sanitizeMode, sanitizeToolCallIds: policy.sanitizeToolCallIds, toolCallIdMode: policy.toolCallIdMode, preserveSignatures: policy.preserveSignatures, sanitizeThoughtSignatures: policy.sanitizeThoughtSignatures });
  ...
  const repairedTools = policy.repairToolUseResultPairing ? sanitizeToolUseResultPairing(sanitizedThinking) : sanitizedThinking;
  ...
  const modelChanged = priorSnapshot ? !isSameModelSnapshot(priorSnapshot, { timestamp: 0, provider: params.provider, modelApi: params.modelApi, modelId: params.modelId }) : false;
  ...
  if ((hasSnapshot && (!priorSnapshot || modelChanged))) {
    appendModelSnapshot(params.sessionManager, { timestamp: Date.now(), provider: params.provider, modelApi: params.modelApi, modelId: params.modelId });
  }
  if (!policy.applyGoogleTurnOrdering) {
    return sanitizedOpenAI;
  }
  return applyGoogleTurnOrderingFix({ messages: sanitizedOpenAI, modelApi: params.modelApi, sessionManager: params.sessionManager, sessionId: params.sessionId }).messages;
}
