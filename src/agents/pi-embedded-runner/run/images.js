...
async function sanitizeImagesWithLog(images, label) {
  ...
  if ((dropped > 0)) {
    log.warn("Native image: dropped  image(s) after sanitization ().");
  }
  return sanitized;
}
  ...
  try {
    {
      ...
      if ((ref.type === "url")) {
        log.debug("Native image: rejecting remote URL (local-only): ");
        return null;
      }
      ...
      if (((ref.type === "path") && options?.sandboxRoot)) {
        try {
          {
            const validated = await assertSandboxPath({ filePath: targetPath, cwd: options.sandboxRoot, root: options.sandboxRoot });
            targetPath = validated.resolved;
          }
        }
        catch (err) {
          {
            log.debug("Native image: sandbox validation failed for : ");
            return null;
          }
        }
      }
      if ((ref.type === "path")) {
        try {
          ...
          {
            log.debug("Native image: file not found: ");
            return null;
          }
        }
      }
      ...
      if ((media.kind !== "image")) {
        log.debug("Native image: not an image file:  (got )");
        return null;
      }
    ...
    {
      log.debug("Native image: failed to load : ");
      return null;
    }
  }
}
...
export async function detectAndLoadPromptImages(params) {
  if (!modelSupportsImages(params.model)) {
    return { images: [], historyImagesByIndex: new Map(), detectedRefs: [], loadedCount: 0, skippedCount: 0 };
  }
  ...
  if ((allRefs.length === 0)) {
    return { images: (params.existingImages ?? []), historyImagesByIndex: new Map(), detectedRefs: [], loadedCount: 0, skippedCount: 0 };
  }
  log.debug("Native image: detected  image refs ( in prompt,  in history)");
  const promptImages = [...(params.existingImages ?? [])];
  const historyImagesByIndex = new Map();
  let loadedCount = 0;
  let skippedCount = 0;
  for (const ref of allRefs) {
    const image = await loadImageFromRef(ref, params.workspaceDir, { maxBytes: params.maxBytes, sandboxRoot: params.sandboxRoot });
    if (image) {
  ...
  for (const [index, images] of historyImagesByIndex) {
    const sanitized = await sanitizeImagesWithLog(images, "history:images:");
    if ((sanitized.length > 0)) {
      sanitizedHistoryImagesByIndex.set(index, sanitized);
    }
  }
  return { images: sanitizedPromptImages, historyImagesByIndex: sanitizedHistoryImagesByIndex, detectedRefs: allRefs, loadedCount, skippedCount };
}
