...
export function buildEmbeddedRunPayloads(params) {
  ...
  const errorText = params.lastAssistant ? formatAssistantErrorText(params.lastAssistant, { cfg: params.config, sessionKey: params.sessionKey }) : undefined;
  const rawErrorMessage = lastAssistantErrored ? (params.lastAssistant?.errorMessage?.trim() || undefined) : undefined;
  ...
  if (inlineToolResults) {
    for (const {toolName, meta} of params.toolMetas) {
      const agg = formatToolAggregate(toolName, meta ? [meta] : [], { markdown: useMarkdown });
      const {text: cleanedText, mediaUrls, audioAsVoice, replyToId, replyToTag, replyToCurrent} = parseReplyDirectives(agg);
      if (cleanedText) {
        replyItems.push({ text: cleanedText, media: mediaUrls, audioAsVoice, replyToId, replyToTag, replyToCurrent });
      }
    }
  }
  ...
  if (params.lastToolError) {
    ...
    if ((!hasUserFacingReply && !isRecoverableError)) {
      ...
      replyItems.push({ text: "⚠️  failed", isError: true });
    }
  }
  const hasAudioAsVoiceTag = replyItems.some((item) => item.audioAsVoice);
  return replyItems.map((item) => { text: item.text?.trim() ? item.text.trim() : undefined, mediaUrls: item.media?.length ? item.media : undefined, mediaUrl: item.media?.[0], isError: item.isError, replyToId: item.replyToId, replyToTag: item.replyToTag, replyToCurrent: item.replyToCurrent, audioAsVoice: (item.audioAsVoice || Boolean((hasAudioAsVoiceTag && item.media?.length))) }).filter((p) => {
