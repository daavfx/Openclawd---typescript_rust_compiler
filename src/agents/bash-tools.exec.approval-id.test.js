...
vi.mock("./tools/nodes-utils.js", () => { listNodes: vi.fn(async () => [{ nodeId: "node-1", commands: ["system.run"], platform: "darwin" }]), resolveNodeIdFromList: vi.fn((nodes) => nodes[0]?.nodeId) });
describe("exec approvals", () => {
  ...
  it("reuses approval id as the node runId", async () => {
    ...
    const tool = createExecTool({ host: "node", ask: "always", approvalRunningNoticeMs: 0 });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    ...
    const approvalsFile = { version: 1, defaults: { security: "allowlist", ask: "on-miss", askFallback: "deny" }, agents: { main: { allowlist: [{ pattern: exePath }] } } };
    const calls = [];
    ...
    const {createExecTool} = await import("./bash-tools.exec.js");
    const tool = createExecTool({ host: "node", ask: "on-miss", approvalRunningNoticeMs: 0 });
    const result = await tool.execute("call2", { command: "\"\" --help" });
  ...
  it("honors ask=off for elevated gateway exec without prompting", async () => {
    ...
    const {createExecTool} = await import("./bash-tools.exec.js");
    const tool = createExecTool({ ask: "off", security: "full", approvalRunningNoticeMs: 0, elevated: { enabled: true, allowed: true, defaultLevel: "ask" } });
  ...
  it("requires approval for elevated ask when allowlist misses", async () => {
    ...
    const {createExecTool} = await import("./bash-tools.exec.js");
    const tool = createExecTool({ ask: "on-miss", security: "allowlist", approvalRunningNoticeMs: 0, elevated: { enabled: true, allowed: true, defaultLevel: "ask" } });
