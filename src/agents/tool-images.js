...
async function resizeImageBase64IfNeeded(params) {
  ...
  if ((((hasDimensions && !overBytes) && (width <= params.maxDimensionPx)) && (height <= params.maxDimensionPx))) {
    return { base64: params.base64, mimeType: params.mimeType, resized: false, width, height };
  }
  if ((hasDimensions && (((width > params.maxDimensionPx) || (height > params.maxDimensionPx)) || overBytes))) {
    log.warn("Image exceeds limits; resizing", { label: params.label, width, height, maxDimensionPx: params.maxDimensionPx, maxBytes: params.maxBytes });
  }
  const qualities = [85, 75, 65, 55, 45, 35];
  const maxDim = hasDimensions ? Math.max((width ?? 0), (height ?? 0)) : params.maxDimensionPx;
  const sideStart = (maxDim > 0) ? Math.min(params.maxDimensionPx, maxDim) : params.maxDimensionPx;
  const sideGrid = [sideStart, 1800, 1600, 1400, 1200, 1000, 800].map((v) => Math.min(params.maxDimensionPx, v)).filter((v, i, arr) => ((v > 0) && (arr.indexOf(v) === i))).sort((a, b) => (b - a));
  let smallest = null;
  for (const side of sideGrid) {
    for (const quality of qualities) {
      const out = await resizeToJpeg({ buffer: buf, maxSide: side, quality, withoutEnlargement: true });
      if ((!smallest || (out.byteLength < smallest.size))) {
        smallest = { buffer: out, size: out.byteLength };
      }
      if ((out.byteLength <= params.maxBytes)) {
        log.info("Image resized", { label: params.label, width, height, maxDimensionPx: params.maxDimensionPx, maxBytes: params.maxBytes, originalBytes: buf.byteLength, resizedBytes: out.byteLength, quality, side });
        return { base64: out.toString("base64"), mimeType: "image/jpeg", resized: true, width, height };
      }
    }
  }
  ...
  throw new Error("Image could not be reduced below MB (got MB)");
}
export async function sanitizeContentBlocksImages(blocks, label, opts = {  }) {
  ...
  for (const block of blocks) {
    ...
    if (!data) {
      out.push({ type: "text", text: "[] omitted empty image payload" });
      continue;
    }
    try {
      {
        ...
        const resized = await resizeImageBase64IfNeeded({ base64: data, mimeType, maxDimensionPx, maxBytes, label });
        out.push({ ...block: , data: resized.base64, mimeType: resized.resized ? resized.mimeType : mimeType });
      }
    }
    catch (err) {
      {
        out.push({ type: "text", text: "[] omitted image payload: " });
      }
    }
  }
  return out;
