...
export function installSessionToolResultGuard(sessionManager, opts) {
  ...
  const flushPendingToolResults = () => {
    ...
    if (allowSyntheticToolResults) {
      for (const [id, name] of pending.entries()) {
        ...
        originalAppend(persistToolResult(synthetic, { toolCallId: id, toolName: name, isSynthetic: true }));
      }
    }
    pending.clear();
  };
  const guardedAppend = (message) => {
    ...
    if ((role === "toolResult")) {
      ...
      return originalAppend(persistToolResult(message, { toolCallId: (id ?? undefined), toolName, isSynthetic: false }));
    }
    const toolCalls = (role === "assistant") ? extractAssistantToolCalls(message) : [];
    if (allowSyntheticToolResults) {
