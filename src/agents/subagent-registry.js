...
async function sweepSubagentRuns() {
  ...
  for (const [runId, entry] of subagentRuns.entries()) {
    ...
    try {
      {
        await callGateway({ method: "sessions.delete", params: { key: entry.childSessionKey, deleteTranscript: true }, timeoutMs: 10000 });
      }
    }
    catch {
      {
...
export function registerSubagentRun(params) {
  ...
  subagentRuns.set(params.runId, { runId: params.runId, childSessionKey: params.childSessionKey, requesterSessionKey: params.requesterSessionKey, requesterOrigin, requesterDisplayKey: params.requesterDisplayKey, task: params.task, cleanup: params.cleanup, label: params.label, createdAt: now, startedAt: now, archiveAtMs, cleanupHandled: false });
  ensureListener();
  persistSubagentRuns();
  if (archiveAfterMs) {
    startSweeper();
  ...
  try {
    {
      ...
      const wait = await callGateway({ method: "agent.wait", params: { runId, timeoutMs }, timeoutMs: (timeoutMs + 10000) });
      ...
      entry.outcome = (wait.status === "error") ? { status: "error", error: wait.error } : { status: "ok" };
      mutated = true;
      if (mutated) {
        persistSubagentRuns();
      }
