...
export function appendOutput(session, stream, chunk) {
  ...
  session.pendingStdoutChars = (session.pendingStdoutChars ?? sumPendingChars(session.pendingStdout));
  session.pendingStderrChars = (session.pendingStderrChars ?? sumPendingChars(session.pendingStderr));
...
function moveToFinished(session, status) {
  ...
  finishedSessions.set(session.id, { id: session.id, command: session.command, scopeKey: session.scopeKey, startedAt: session.startedAt, endedAt: Date.now(), cwd: session.cwd, status, exitCode: session.exitCode, exitSignal: session.exitSignal, aggregated: session.aggregated, tail: session.tail, truncated: session.truncated, totalOutputChars: session.totalOutputChars });
}
