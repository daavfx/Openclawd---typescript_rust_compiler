...
import { estimateMessagesTokens, pruneHistoryForContextShare, splitMessagesByTokenShare } from "./compaction.js";
...
describe("splitMessagesByTokenShare", () => {
  it("splits messages into two non-empty parts", () => {
    const messages = [makeMessage(1, 4000), makeMessage(2, 4000), makeMessage(3, 4000), makeMessage(4, 4000)];
  ...
  it("preserves message order across parts", () => {
    const messages = [makeMessage(1, 4000), makeMessage(2, 4000), makeMessage(3, 4000), makeMessage(4, 4000), makeMessage(5, 4000), makeMessage(6, 4000)];
...
describe("pruneHistoryForContextShare", () => {
  it("drops older chunks until the history budget is met", () => {
    ...
    const pruned = pruneHistoryForContextShare({ messages, maxContextTokens, maxHistoryShare: 0.5, parts: 2 });
  ...
  it("returns droppedMessagesList containing dropped messages", () => {
    ...
    const allIds = [...pruned.droppedMessagesList.map((m) => m.timestamp), ...pruned.messages.map((m) => m.timestamp)].sort((a, b) => (a - b));
  ...
  it("returns empty droppedMessagesList when no pruning needed", () => {
    ...
    const pruned = pruneHistoryForContextShare({ messages, maxContextTokens: 100000, maxHistoryShare: 0.5, parts: 2 });
    expect(pruned.droppedChunks).toBe(0);
    expect(pruned.droppedMessagesList).toEqual([]);
