...
const pluginGroups = { all: ["lobster", "workflow_tool"], byPlugin: new Map([["lobster", ["lobster", "workflow_tool"]]]) };
...
describe("stripPluginOnlyAllowlist", () => {
  ...
  it("keeps allowlist when it mixes plugin and core entries", () => {
    const policy = stripPluginOnlyAllowlist({ allow: ["lobster", "read"] }, pluginGroups, coreTools);
  ...
  it("keeps allowlist with core tools and reports unknown entries", () => {
    const emptyPlugins = { all: [], byPlugin: new Map() };
    const policy = stripPluginOnlyAllowlist({ allow: ["read", "lobster"] }, emptyPlugins, coreTools);
