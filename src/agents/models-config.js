...
export async function ensureOpenClawModelsJson(config, agentDirOverride) {
  ...
  const providers = mergeProviders({ implicit: implicitProviders, explicit: explicitProviders });
  ...
  const normalizedProviders = normalizeProviders({ providers: mergedProviders, agentDir });
  const next = "
";
  try {
    {
