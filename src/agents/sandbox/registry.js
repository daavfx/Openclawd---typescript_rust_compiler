...
async function writeRegistry(registry) {
  ...
  await fs.writeFile(SANDBOX_REGISTRY_PATH, "
", "utf-8");
}
export async function updateRegistry(entry) {
  ...
  next.push({ ...entry: , createdAtMs: (existing?.createdAtMs ?? entry.createdAtMs), image: (existing?.image ?? entry.image), configHash: (entry.configHash ?? existing?.configHash) });
...
async function writeBrowserRegistry(registry) {
  await fs.mkdir(SANDBOX_STATE_DIR, { recursive: true });
  await fs.writeFile(SANDBOX_BROWSER_REGISTRY_PATH, "
", "utf-8");
}
export async function updateBrowserRegistry(entry) {
  ...
  const existing = registry.entries.find((item) => (item.containerName === entry.containerName));
  const next = registry.entries.filter((item) => (item.containerName !== entry.containerName));
  next.push({ ...entry: , createdAtMs: (existing?.createdAtMs ?? entry.createdAtMs), image: (existing?.image ?? entry.image) });
