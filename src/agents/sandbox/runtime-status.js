...
function resolveComparableSessionKeyForSandbox(params) {
  return canonicalizeMainSessionAlias({ cfg: params.cfg, agentId: params.agentId, sessionKey: params.sessionKey });
}
export function resolveSandboxRuntimeStatus(params) {
  ...
  const sandboxed = sessionKey ? shouldSandboxSession(sandboxCfg, resolveComparableSessionKeyForSandbox({ cfg, agentId, sessionKey }), mainSessionKey) : false;
  return { agentId, sessionKey, mainSessionKey, mode: sandboxCfg.mode, sandboxed, toolPolicy: resolveSandboxToolPolicyForAgent(cfg, agentId) };
}
...
export function formatSandboxToolPolicyBlockedMessage(params) {
  ...
  if (blockedByDeny) {
    ...
    fixes.push("Remove \"\" from .");
  }
  if (blockedByAllow) {
    reasons.push("allow list");
    fixes.push("Add \"\" to  (or set it to [] to allow all).");
  }
  const lines = [];
  lines.push("Tool \"\" blocked by sandbox tool policy (mode=).");
  lines.push("Session: ");
  lines.push("Reason: ");
  lines.push("Fix:");
  ...
  if ((runtime.mode === "non-main")) {
    lines.push("- Use main session key (direct): ");
  }
  lines.push("- See: ");
  return lines.join("
");
