  ...
  try {
    {
      ...
      const requiresRaw = ((typeof metadataObj.requires === "object") && (metadataObj.requires !== null)) ? metadataObj.requires : undefined;
      ...
      return { always: (typeof metadataObj.always === "boolean") ? metadataObj.always : undefined, emoji: (typeof metadataObj.emoji === "string") ? metadataObj.emoji : undefined, homepage: (typeof metadataObj.homepage === "string") ? metadataObj.homepage : undefined, skillKey: (typeof metadataObj.skillKey === "string") ? metadataObj.skillKey : undefined, primaryEnv: (typeof metadataObj.primaryEnv === "string") ? metadataObj.primaryEnv : undefined, os: (osRaw.length > 0) ? osRaw : undefined, requires: requiresRaw ? { bins: normalizeStringList(requiresRaw.bins), anyBins: normalizeStringList(requiresRaw.anyBins), env: normalizeStringList(requiresRaw.env), config: normalizeStringList(requiresRaw.config) } : undefined, install: (install.length > 0) ? install : undefined };
    }
  }
  catch {
    {
...
export function resolveSkillInvocationPolicy(frontmatter) {
  return { userInvocable: parseFrontmatterBool(getFrontmatterValue(frontmatter, "user-invocable"), true), disableModelInvocation: parseFrontmatterBool(getFrontmatterValue(frontmatter, "disable-model-invocation"), false) };
}
