...
");
  return { prompt, skills: eligible.map((entry) => { name: entry.skill.name, primaryEnv: entry.metadata?.primaryEnv }), resolvedSkills, version: opts?.snapshotVersion };
}
...
export function resolveSkillsPromptForRun(params) {
  ...
  if ((params.entries && (params.entries.length > 0))) {
    const prompt = buildWorkspaceSkillsPrompt(params.workspaceDir, { entries: params.entries, config: params.config });
...
export async function syncSkillsToWorkspace(params) {
  ...
  await serializeByKey("syncSkills:", async () => {
    const targetSkillsDir = path.join(targetDir, "skills");
    const entries = loadSkillEntries(sourceDir, { config: params.config, managedSkillsDir: params.managedSkillsDir, bundledSkillsDir: params.bundledSkillsDir });
    ...
    for (const entry of entries) {
      ...
      try {
      ...
      catch (error) {
        {
          ...
          console.warn("[skills] Failed to copy  to sandbox: ");
        }
      }
    }
  });
...
export function buildWorkspaceSkillCommandSpecs(workspaceDir, opts) {
  ...
  for (const entry of userInvocable) {
    ...
    if ((base !== rawName)) {
      debugSkillCommandOnce("sanitize::", "Sanitized skill command name \"\" to \"/\".", { rawName, sanitized: "/" });
    }
    const unique = resolveUniqueSkillCommandName(base, used);
    if ((unique !== base)) {
      debugSkillCommandOnce("dedupe::", "De-duplicated skill command name for \"\" to \"/\".", { rawName, deduped: "/" });
    }
    ...
    const dispatch = () => {
      ...
      if (!toolName) {
        debugSkillCommandOnce("dispatch:missingTool:", "Skill command \"/\" requested tool dispatch but did not provide command-tool. Ignoring dispatch.", { skillName: rawName, command: unique });
        return undefined;
      }
      ...
      if (!argMode) {
        debugSkillCommandOnce("dispatch:badArgMode::", "Skill command \"/\" requested tool dispatch but has unknown command-arg-mode. Falling back to raw.", { skillName: rawName, command: unique, argMode: argModeRaw });
      }
      return { kind: "tool", toolName, argMode: "raw" };
    }();
    specs.push({ name: unique, skillName: rawName, description, ...dispatch ? { dispatch } : {  }:  });
  }
  return specs;
}
