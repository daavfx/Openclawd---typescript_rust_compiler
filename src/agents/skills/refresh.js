...
let globalVersion = 0;
export const DEFAULT_SKILLS_WATCH_IGNORED = [/(^|[\\/])\.git([\\/]|$)/, /(^|[\\/])node_modules([\\/]|$)/, /(^|[\\/])dist([\\/]|$)/]
...
function emit(event) {
  for (const listener of listeners) {
    try {
      ...
      {
        log.warn("skills change listener failed: ");
      }
    }
  }
}
...
export function ensureSkillsWatcher(params) {
  ...
  const watcher = chokidar.watch(watchPaths, { ignoreInitial: true, awaitWriteFinish: { stabilityThreshold: debounceMs, pollInterval: 100 }, ignored: DEFAULT_SKILLS_WATCH_IGNORED });
