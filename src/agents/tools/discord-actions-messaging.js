    ...
    case "reactions":
      {
        ...
        const reactions = await fetchReactionsDiscord(channelId, messageId, { ...accountId ? { accountId } : {  }: , limit });
    ...
    case "sticker":
      {
        ...
        const stickerIds = readStringArrayParam(params, "stickerIds", { required: true, label: "stickerIds" });
        await sendStickerDiscord(to, stickerIds, { ...accountId ? { accountId } : {  }: , content });
    ...
    case "poll":
      {
        ...
        const answers = readStringArrayParam(params, "answers", { required: true, label: "answers" });
    ...
    case "fetchMessage":
      {
        ...
        return jsonResult({ ok: true, message: normalizeMessage(message), guildId, channelId, messageId });
      }
    case "readMessages":
      {
        if (!isActionEnabled("messages")) {
        ...
        const channelId = resolveChannelId();
        const query = { limit: ((typeof params.limit === "number") && Number.isFinite(params.limit)) ? params.limit : undefined, before: readStringParam(params, "before"), after: readStringParam(params, "after"), around: readStringParam(params, "around") };
        const messages = accountId ? await readMessagesDiscord(channelId, query, { accountId }) : await readMessagesDiscord(channelId, query);
        return jsonResult({ ok: true, messages: messages.map((message) => normalizeMessage(message)) });
      }
    case "sendMessage":
      {
        if (!isActionEnabled("messages")) {
        ...
        const result = await sendMessageDiscord(to, content, { ...accountId ? { accountId } : {  }: , mediaUrl, replyTo, embeds });
    ...
    case "threadCreate":
      {
        ...
        const autoArchiveMinutes = ((typeof autoArchiveMinutesRaw === "number") && Number.isFinite(autoArchiveMinutesRaw)) ? autoArchiveMinutesRaw : undefined;
        const thread = accountId ? await createThreadDiscord(channelId, { name, messageId, autoArchiveMinutes }, { accountId }) : await createThreadDiscord(channelId, { name, messageId, autoArchiveMinutes });
    ...
    case "threadList":
      {
        ...
        const threads = accountId ? await listThreadsDiscord({ guildId, channelId, includeArchived, before, limit }, { accountId }) : await listThreadsDiscord({ guildId, channelId, includeArchived, before, limit });
    ...
    case "threadReply":
      {
        ...
        const mediaUrl = readStringParam(params, "mediaUrl");
        const replyTo = readStringParam(params, "replyTo");
        const result = await sendMessageDiscord("channel:", content, { ...accountId ? { accountId } : {  }: , mediaUrl, replyTo });
        return jsonResult({ ok: true, result });
      }
    case "pinMessage":
      {
    ...
    case "searchMessages":
      {
        ...
        const results = accountId ? await searchMessagesDiscord({ guildId, content, channelIds: channelIdList.length ? channelIdList : undefined, authorIds: authorIdList.length ? authorIdList : undefined, limit }, { accountId }) : await searchMessagesDiscord({ guildId, content, channelIds: channelIdList.length ? channelIdList : undefined, authorIds: authorIdList.length ? authorIdList : undefined, limit });
        ...
        return jsonResult({ ok: true, results: { ...resultsRecord: , messages: normalizedMessages } });
      }
    default:
