...
const SessionsListToolSchema = Type.Object({ kinds: Type.Optional(Type.Array(Type.String())), limit: Type.Optional(Type.Number({ minimum: 1 })), activeMinutes: Type.Optional(Type.Number({ minimum: 1 })), messageLimit: Type.Optional(Type.Number({ minimum: 0 })) });
function resolveSandboxSessionToolsVisibility(cfg) {
  return (cfg.agents?.defaults?.sandbox?.sessionToolsVisibility ?? "spawned");
}
export function createSessionsListTool(opts) {
  return { label: "Sessions", name: "sessions_list", description: "List sessions with optional filters and last messages.", parameters: SessionsListToolSchema, execute: async (_toolCallId, args) => {
    const params = args;
    ...
    const limit = ((typeof params.limit === "number") && Number.isFinite(params.limit)) ? Math.max(1, Math.floor(params.limit)) : undefined;
    const activeMinutes = ((typeof params.activeMinutes === "number") && Number.isFinite(params.activeMinutes)) ? Math.max(1, Math.floor(params.activeMinutes)) : undefined;
    ...
    const list = await callGateway({ method: "sessions.list", params: { limit, activeMinutes, includeGlobal: !restrictToSpawned, includeUnknown: !restrictToSpawned, spawnedBy: restrictToSpawned ? requesterInternalKey : undefined } });
    ...
    for (const entry of sessions) {
      ...
      const deliveryContext = (entry.deliveryContext && (typeof entry.deliveryContext === "object")) ? entry.deliveryContext : undefined;
