...
const SessionStatusToolSchema = Type.Object({ sessionKey: Type.Optional(Type.String()), model: Type.Optional(Type.String()) });
...
function resolveModelAuthLabel(params) {
  ...
  const order = resolveAuthProfileOrder({ cfg: params.cfg, store, provider: providerKey, preferredProfile: profileOverride });
  ...
  for (const profileId of candidates) {
    ...
    const label = resolveAuthProfileDisplayLabel({ cfg: params.cfg, store, profileId });
...
function resolveSessionEntry(params) {
  ...
  const internal = resolveInternalSessionKey({ key: keyRaw, alias: params.alias, mainKey: params.mainKey });
  ...
  if ((keyRaw === "main")) {
    candidates.add(buildAgentMainSessionKey({ agentId: DEFAULT_AGENT_ID, mainKey: params.mainKey }));
  }
...
async function resolveModelOverride(params) {
  ...
  const configDefault = resolveDefaultModelForAgent({ cfg: params.cfg, agentId: params.agentId });
  const currentProvider = (params.sessionEntry?.providerOverride?.trim() || configDefault.provider);
  const currentModel = (params.sessionEntry?.modelOverride?.trim() || configDefault.model);
  const aliasIndex = buildModelAliasIndex({ cfg: params.cfg, defaultProvider: currentProvider });
  const catalog = await loadModelCatalog({ config: params.cfg });
  const allowed = buildAllowedModelSet({ cfg: params.cfg, catalog, defaultProvider: currentProvider, defaultModel: currentModel });
  const resolved = resolveModelRefFromString({ raw, defaultProvider: currentProvider, aliasIndex });
  if (!resolved) {
  ...
  return { kind: "set", provider: resolved.ref.provider, model: resolved.ref.model, isDefault };
}
export function createSessionStatusTool(opts) {
  return { label: "Session Status", name: "session_status", description: "Show a /status-equivalent session status card (usage + time + cost when available). Use for model-use questions (📊 session_status). Optional: set per-session model override (model=default resets overrides).", parameters: SessionStatusToolSchema, execute: async (_toolCallId, args) => {
    const params = args;
    ...
    let resolved = resolveSessionEntry({ store, keyRaw: requestedKeyRaw, alias, mainKey });
    if ((!resolved && shouldResolveSessionIdInput(requestedKeyRaw))) {
      const resolvedKey = resolveSessionKeyFromSessionId({ cfg, sessionId: requestedKeyRaw, agentId: a2aPolicy.enabled ? undefined : requesterAgentId });
      if (resolvedKey) {
        ...
        resolved = resolveSessionEntry({ store, keyRaw: requestedKeyRaw, alias, mainKey });
      }
    }
    if (!resolved) {
    ...
    if ((typeof modelRaw === "string")) {
      const selection = await resolveModelOverride({ cfg, raw: modelRaw, sessionEntry: resolved.entry, agentId });
      const nextEntry = { ...resolved.entry:  };
      const applied = applyModelOverrideToSessionEntry({ entry: nextEntry, selection: (selection.kind === "reset") ? { provider: configured.provider, model: configured.model, isDefault: true } : { provider: selection.provider, model: selection.model, isDefault: selection.isDefault } });
      if (applied.updated) {
    ...
    if (usageProvider) {
      try {
        {
          const usageSummary = await loadProviderUsageSummary({ timeoutMs: 3500, providers: [usageProvider], agentDir });
          const snapshot = usageSummary.providers.find((entry) => (entry.provider === usageProvider));
          if (snapshot) {
            const formatted = formatUsageWindowSummary(snapshot, { now: Date.now(), maxWindows: 2, includeResets: true });
    ...
    const groupActivation = isGroup ? (normalizeGroupActivation(resolved.entry.groupActivation) ?? "mention") : undefined;
    const queueSettings = resolveQueueSettings({ cfg, channel: ((resolved.entry.channel ?? resolved.entry.lastChannel) ?? "unknown"), sessionEntry: resolved.entry });
    ...
    const timeLine = userTime ? "🕒 Time:  ()" : "🕒 Time zone: ";
    ...
    const statusText = buildStatusMessage({ config: cfg, agent: { ...agentDefaults: , model: agentModel }, sessionEntry: resolved.entry, sessionKey: resolved.key, groupActivation, modelAuth: resolveModelAuthLabel({ provider: providerForCard, cfg, sessionEntry: resolved.entry, agentDir }), usageLine, timeLine, queue: { mode: queueSettings.mode, depth: queueDepth, debounceMs: queueSettings.debounceMs, cap: queueSettings.cap, dropPolicy: queueSettings.dropPolicy, showDetails: queueOverrides }, includeTranscriptUsage: false });
