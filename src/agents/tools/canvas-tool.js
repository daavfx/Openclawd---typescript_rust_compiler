...
export function createCanvasTool() {
  return { label: "Canvas", name: "canvas", description: "Control node canvases (present/hide/navigate/eval/snapshot/A2UI). Use snapshot to capture the rendered UI.", parameters: CanvasToolSchema, execute: async (_toolCallId, args) => {
    ...
    const invoke = async (command, invokeParams) => await callGatewayTool("node.invoke", gatewayOpts, { nodeId, command, params: invokeParams, idempotencyKey: crypto.randomUUID() });
    switch (action) {
      case "present":
        {
          const placement = { x: (typeof params.x === "number") ? params.x : undefined, y: (typeof params.y === "number") ? params.y : undefined, width: (typeof params.width === "number") ? params.width : undefined, height: (typeof params.height === "number") ? params.height : undefined };
          const invokeParams = {  };
      ...
      case "snapshot":
        {
          ...
          return await imageResult({ label: "canvas:snapshot", path: filePath, base64: payload.base64, mimeType, details: { format: payload.format } });
        }
      case "a2ui_push":
        {
