    ...
    case "emojiUpload":
      {
        ...
        const emoji = accountId ? await uploadEmojiDiscord({ guildId, name, mediaUrl, roleIds: roleIds?.length ? roleIds : undefined }, { accountId }) : await uploadEmojiDiscord({ guildId, name, mediaUrl, roleIds: roleIds?.length ? roleIds : undefined });
    ...
    case "stickerUpload":
      {
        ...
        const mediaUrl = readStringParam(params, "mediaUrl", { required: true });
        const sticker = accountId ? await uploadStickerDiscord({ guildId, name, description, tags, mediaUrl }, { accountId }) : await uploadStickerDiscord({ guildId, name, description, tags, mediaUrl });
    ...
    case "eventCreate":
      {
        ...
        const payload = { name, description, scheduled_start_time: startTime, scheduled_end_time: endTime, entity_type: entityType, channel_id: channelId, entity_metadata: ((entityType === 3) && location) ? { location } : undefined, privacy_level: 2 };
    ...
    case "channelEdit":
      {
        ...
        const channel = accountId ? await editChannelDiscord({ channelId, name: (name ?? undefined), topic: (topic ?? undefined), position: (position ?? undefined), parentId, nsfw, rateLimitPerUser: (rateLimitPerUser ?? undefined) }, { accountId }) : await editChannelDiscord({ channelId, name: (name ?? undefined), topic: (topic ?? undefined), position: (position ?? undefined), parentId, nsfw, rateLimitPerUser: (rateLimitPerUser ?? undefined) });
        return jsonResult({ ok: true, channel });
      }
    case "channelDelete":
      {
    ...
    case "channelMove":
      {
        ...
        const position = readNumberParam(params, "position", { integer: true });
        if (accountId) {
          await moveChannelDiscord({ guildId, channelId, parentId, position: (position ?? undefined) }, { accountId });
        } else {
          await moveChannelDiscord({ guildId, channelId, parentId, position: (position ?? undefined) });
        }
        return jsonResult({ ok: true });
      }
    case "categoryCreate":
      {
        ...
        const name = readStringParam(params, "name", { required: true });
        const position = readNumberParam(params, "position", { integer: true });
        const channel = accountId ? await createChannelDiscord({ guildId, name, type: 4, position: (position ?? undefined) }, { accountId }) : await createChannelDiscord({ guildId, name, type: 4, position: (position ?? undefined) });
    ...
    case "categoryEdit":
      {
        ...
        const name = readStringParam(params, "name");
        const position = readNumberParam(params, "position", { integer: true });
        const channel = accountId ? await editChannelDiscord({ channelId: categoryId, name: (name ?? undefined), position: (position ?? undefined) }, { accountId }) : await editChannelDiscord({ channelId: categoryId, name: (name ?? undefined), position: (position ?? undefined) });
        return jsonResult({ ok: true, category: channel });
      }
    case "categoryDelete":
      {
    ...
    case "channelPermissionSet":
      {
        ...
        const targetTypeRaw = readStringParam(params, "targetType", { required: true });
        const targetType = (targetTypeRaw === "member") ? 1 : 0;
        const allow = readStringParam(params, "allow");
        const deny = readStringParam(params, "deny");
        if (accountId) {
          await setChannelPermissionDiscord({ channelId, targetId, targetType, allow: (allow ?? undefined), deny: (deny ?? undefined) }, { accountId });
        } else {
          await setChannelPermissionDiscord({ channelId, targetId, targetType, allow: (allow ?? undefined), deny: (deny ?? undefined) });
        }
        return jsonResult({ ok: true });
      }
