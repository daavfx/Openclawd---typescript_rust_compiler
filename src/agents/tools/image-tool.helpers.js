...
export function coerceImageAssistantText(params) {
  ...
  if (((stop === "error") || (stop === "aborted"))) {
    throw new Error(errorMessage ? "Image model failed (/): " : "Image model failed (/)");
  }
  if (errorMessage) {
  ...
  throw new Error("Image model returned no text (/).");
}
...
export function coerceImageModelConfig(cfg) {
  ...
  return { ...primary?.trim() ? { primary: primary.trim() } : {  }: , ...(fallbacks.length > 0) ? { fallbacks } : {  }:  };
}
...
export function resolveProviderVisionModelFromConfig(params) {
  ...
  const preferMinimaxVl = (params.provider === "minimax") ? models.find((m) => ((((m?.id ?? "").trim() === "MiniMax-VL-01") && Array.isArray(m?.input)) && m.input.includes("image"))) : null;
