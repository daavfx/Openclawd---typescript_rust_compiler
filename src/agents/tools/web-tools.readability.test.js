...
describe("web fetch readability", () => {
  it("extracts readable text", async () => {
    const result = await extractReadableContent({ html: SAMPLE_HTML, url: "https://example.com/article", extractMode: "text" });
    expect(result?.text).toContain("Main content starts here");
    expect(result?.title).toBe("Example Article");
  });
  it("extracts readable markdown", async () => {
    const result = await extractReadableContent({ html: SAMPLE_HTML, url: "https://example.com/article", extractMode: "markdown" });
    expect(result?.text).toContain("Main content starts here");
    expect(result?.title).toBe("Example Article");
  });
});
