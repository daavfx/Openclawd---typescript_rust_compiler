...
export const __testing = { decodeDataUrl, coerceImageAssistantText }
...
export function resolveImageModelConfigForTool(params) {
  ...
  const providerVisionFromConfig = resolveProviderVisionModelFromConfig({ cfg: params.cfg, provider: primary.provider });
  ...
  if (openaiOk) {
    ...
    return { primary: "openai/gpt-5-mini", ...fallbacks.length ? { fallbacks } : {  }:  };
  }
  if (anthropicOk) {
...
function buildImageContext(prompt, base64, mimeType) {
  return { messages: [{ role: "user", content: [{ type: "text", text: prompt }, { type: "image", data: base64, mimeType }], timestamp: Date.now() }] };
}
async function resolveSandboxedImagePath(params) {
  ...
  try {
    {
      const out = await assertSandboxPath({ filePath, cwd: params.sandboxRoot, root: params.sandboxRoot });
  ...
  catch (err) {
    {
      ...
      const out = await assertSandboxPath({ filePath: candidateRel, cwd: params.sandboxRoot, root: params.sandboxRoot });
      return { resolved: out.resolved, rewrittenFrom: filePath };
    }
  }
}
async function runImagePrompt(params) {
  const effectiveCfg = params.cfg ? { ...params.cfg: , agents: { ...params.cfg.agents: , defaults: { ...params.cfg.agents?.defaults: , imageModel: params.imageModelConfig } } } : undefined;
  await ensureOpenClawModelsJson(effectiveCfg, params.agentDir);
  const authStorage = discoverAuthStorage(params.agentDir);
  const modelRegistry = discoverModels(authStorage, params.agentDir);
  const result = await runWithImageModelFallback({ cfg: effectiveCfg, modelOverride: params.modelOverride, run: async (provider, modelId) => {
    ...
    const apiKeyInfo = await getApiKeyForModel({ model, cfg: effectiveCfg, agentDir: params.agentDir });
    const apiKey = requireApiKey(apiKeyInfo, model.provider);
    authStorage.setRuntimeApiKey(model.provider, apiKey);
    const imageDataUrl = "data:;base64,";
    if ((model.provider === "minimax")) {
      const text = await minimaxUnderstandImage({ apiKey, prompt: params.prompt, imageDataUrl, modelBaseUrl: model.baseUrl });
    ...
    const message = await complete(model, context, { apiKey, maxTokens: 512 });
    const text = coerceImageAssistantText({ message, provider: model.provider, model: model.id });
    return { text, provider: model.provider, model: model.id };
  } });
  return { text: result.result.text, provider: result.result.provider, model: result.result.model, attempts: result.attempts.map((attempt) => { provider: attempt.provider, model: attempt.model, error: attempt.error }) };
}
export function createImageTool(options) {
  ...
  const imageModelConfig = resolveImageModelConfigForTool({ cfg: options?.config, agentDir });
  if (!imageModelConfig) {
    return null;
  }
  ...
  return { label: "Image", name: "image", description, parameters: Type.Object({ prompt: Type.Optional(Type.String()), image: Type.String(), model: Type.Optional(Type.String()), maxBytesMb: Type.Optional(Type.Number()) }), execute: async (_toolCallId, args) => {
    ...
    if (((((hasScheme && !looksLikeWindowsDrivePath) && !isFileUrl) && !isHttpUrl) && !isDataUrl)) {
      return { content: [{ type: "text", text: "Unsupported image reference: . Use a file path, a file:// URL, a data: URL, or an http(s) URL." }], details: { error: "unsupported_image_reference", image: imageRawInput } };
    }
    ...
    const resolvedPathInfo = isDataUrl ? { resolved: "" } : sandboxRoot ? await resolveSandboxedImagePath({ sandboxRoot, imagePath: resolvedImage }) : { resolved: resolvedImage.startsWith("file://") ? resolvedImage.slice("file://".length) : resolvedImage };
    ...
    const result = await runImagePrompt({ cfg: options?.config, agentDir, imageModelConfig, modelOverride, prompt: promptRaw, base64, mimeType });
    return { content: [{ type: "text", text: result.text }], details: { model: "/", image: resolvedImage, ...resolvedPathInfo.rewrittenFrom ? { rewrittenFrom: resolvedPathInfo.rewrittenFrom } : {  }: , attempts: result.attempts } };
  } };
}
