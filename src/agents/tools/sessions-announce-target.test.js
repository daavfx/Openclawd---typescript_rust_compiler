...
describe("resolveAnnounceTarget", () => {
  ...
  it("derives non-WhatsApp announce targets from the session key", async () => {
    ...
    const target = await resolveAnnounceTarget({ sessionKey: "agent:main:discord:group:dev", displayKey: "agent:main:discord:group:dev" });
  ...
  it("hydrates WhatsApp accountId from sessions.list when available", async () => {
    const {resolveAnnounceTarget} = await loadResolveAnnounceTarget();
    callGatewayMock.mockResolvedValueOnce({ sessions: [{ key: "agent:main:whatsapp:group:123@g.us", deliveryContext: { channel: "whatsapp", to: "123@g.us", accountId: "work" } }] });
    const target = await resolveAnnounceTarget({ sessionKey: "agent:main:whatsapp:group:123@g.us", displayKey: "agent:main:whatsapp:group:123@g.us" });
    expect(target).toEqual({ channel: "whatsapp", to: "123@g.us", accountId: "work" });
