...
export function createAgentsListTool(opts) {
  return { label: "Agents", name: "agents_list", description: "List agent ids you can target with sessions_spawn (based on allowlists).", parameters: AgentsListToolSchema, execute: async () => {
    const cfg = loadConfig();
    const {mainKey, alias} = resolveMainSessionAlias(cfg);
    const requesterInternalKey = ((typeof opts?.agentSessionKey === "string") && opts.agentSessionKey.trim()) ? resolveInternalSessionKey({ key: opts.agentSessionKey, alias, mainKey }) : alias;
    ...
    const agents = ordered.map((id) => { id, name: configuredNameMap.get(id), configured: configuredIds.includes(id) });
    return jsonResult({ requester: requesterAgentId, allowAny, agents });
  } };
}
