...
import { CacheEntry, DEFAULT_CACHE_TTL_MINUTES, DEFAULT_TIMEOUT_SECONDS, normalizeCacheKey, readCache, readResponseText, resolveCacheTtlMs, resolveTimeoutSeconds, withTimeout, writeCache } from "./web-shared.js";
...
const WebFetchSchema = Type.Object({ url: Type.String({ description: "HTTP or HTTPS URL to fetch." }), extractMode: Type.Optional(stringEnum(EXTRACT_MODES, { description: "Extraction mode (\"markdown\" or \"text\").", default: "markdown" })), maxChars: Type.Optional(Type.Number({ description: "Maximum characters to return (truncates when exceeded).", minimum: 100 })) });
...
function resolveFirecrawlMaxAgeMs(firecrawl) {
  const raw = ((firecrawl && ("maxAgeMs" in firecrawl)) && (typeof firecrawl.maxAgeMs === "number")) ? firecrawl.maxAgeMs : undefined;
...
async function fetchWithRedirects(params) {
  ...
  while (true) {
    ...
    try {
      {
        res = await fetch(parsedUrl.toString(), { method: "GET", headers: { Accept: "*/*", "User-Agent": params.userAgent, "Accept-Language": "en-US,en;q=0.9" }, signal, redirect: "manual", dispatcher });
      }
    }
    catch (err) {
      {
    ...
    if (isRedirectStatus(res.status)) {
      ...
      if ((redirectCount > params.maxRedirects)) {
        await closeDispatcher(dispatcher);
        throw new Error("Too many redirects (limit: )");
      }
...
export async function fetchFirecrawlContent(params) {
  ...
  const body = { url: params.url, formats: ["markdown"], onlyMainContent: params.onlyMainContent, timeout: (params.timeoutSeconds * 1000), maxAge: params.maxAgeMs, proxy: params.proxy, storeInCache: params.storeInCache };
  const res = await fetch(endpoint, { method: "POST", headers: { Authorization: "Bearer ", "Content-Type": "application/json" }, body: JSON.stringify(body), signal: withTimeout(undefined, (params.timeoutSeconds * 1000)) });
  const payload = await res.json();
  if ((!res.ok || (payload?.success === false))) {
    const detail = (payload?.error || res.statusText);
    throw new Error("Firecrawl fetch failed (): ".trim());
  }
  ...
  return { text, title: data.metadata?.title, finalUrl: data.metadata?.sourceURL, status: data.metadata?.statusCode, warning: payload?.warning };
}

async function runWebFetch(params) {
  const cacheKey = normalizeCacheKey("fetch:::");
  ...
  try {
    {
      const result = await fetchWithRedirects({ url: params.url, maxRedirects: params.maxRedirects, timeoutSeconds: params.timeoutSeconds, userAgent: params.userAgent });
      res = result.response;
  ...
  catch (error) {
    {
      ...
      if ((params.firecrawlEnabled && params.firecrawlApiKey)) {
        const firecrawl = await fetchFirecrawlContent({ url: finalUrl, extractMode: params.extractMode, apiKey: params.firecrawlApiKey, baseUrl: params.firecrawlBaseUrl, onlyMainContent: params.firecrawlOnlyMainContent, maxAgeMs: params.firecrawlMaxAgeMs, proxy: params.firecrawlProxy, storeInCache: params.firecrawlStoreInCache, timeoutSeconds: params.firecrawlTimeoutSeconds });
        const truncated = truncateText(firecrawl.text, params.maxChars);
        const payload = { url: params.url, finalUrl: (firecrawl.finalUrl || finalUrl), status: (firecrawl.status ?? 200), contentType: "text/markdown", title: firecrawl.title, extractMode: params.extractMode, extractor: "firecrawl", truncated: truncated.truncated, length: truncated.text.length, fetchedAt: new Date().toISOString(), tookMs: (Date.now() - start), text: truncated.text, warning: firecrawl.warning };
  ...
  try {
    {
      if (!res.ok) {
        if ((params.firecrawlEnabled && params.firecrawlApiKey)) {
          const firecrawl = await fetchFirecrawlContent({ url: params.url, extractMode: params.extractMode, apiKey: params.firecrawlApiKey, baseUrl: params.firecrawlBaseUrl, onlyMainContent: params.firecrawlOnlyMainContent, maxAgeMs: params.firecrawlMaxAgeMs, proxy: params.firecrawlProxy, storeInCache: params.firecrawlStoreInCache, timeoutSeconds: params.firecrawlTimeoutSeconds });
          const truncated = truncateText(firecrawl.text, params.maxChars);
        ...
        const detail = formatWebFetchErrorDetail({ detail: rawDetail, contentType: res.headers.get("content-type"), maxChars: DEFAULT_ERROR_MAX_CHARS });
      ...
      if (contentType.includes("text/html")) {
        if (params.readabilityEnabled) {
          const readable = await extractReadableContent({ html: body, url: finalUrl, extractMode: params.extractMode });
          if (readable?.text) {
            text = readable.text;
      ...
      const payload = { url: params.url, finalUrl, status: res.status, contentType, title, extractMode: params.extractMode, extractor, truncated: truncated.truncated, length: truncated.text.length, fetchedAt: new Date().toISOString(), tookMs: (Date.now() - start), text: truncated.text };
      writeCache(FETCH_CACHE, cacheKey, payload, params.cacheTtlMs);
      return payload;
    }
  }
