...
export async function imageResult(params) {
  const content = [{ type: "text", text: (params.extraText ?? "MEDIA:") }, { type: "image", data: params.base64, mimeType: params.mimeType }];
  const result = { content, details: { path: params.path, ...params.details:  } };
...
export async function imageResultFromFile(params) {
  ...
  return await imageResult({ label: params.label, path: params.path, base64: buf.toString("base64"), mimeType, extraText: params.extraText, details: params.details });
}
