...
export async function handleSlackAction(params, cfg, context) {
  ...
  const buildActionOpts = (operation) => {
    ...
    return { ...accountId ? { accountId } : {  }: , ...tokenOverride ? { token: tokenOverride } : {  }:  };
  };
      ...
      case "sendMessage":
        {
          ...
          const result = await sendSlackMessage(to, content, { ...writeOpts: , mediaUrl: (mediaUrl ?? undefined), threadTs: (threadTs ?? undefined) });
      ...
      case "readMessages":
        {
          ...
          const result = await readSlackMessages(channelId, { ...readOpts: , limit, before: (before ?? undefined), after: (after ?? undefined), threadId: (threadId ?? undefined) });
