  ...
  const rows = raw.map((row, rowIndex) => {
    ...
    return row.map((button, buttonIndex) => {
      ...
      if ((!text || !callbackData)) {
        throw new Error("buttons[][] requires text and callback_data");
      }
      if ((callbackData.length > 64)) {
        throw new Error("buttons[][] callback_data too long (max 64 chars)");
      }
...
export async function handleTelegramAction(params, cfg) {
  ...
  if ((action === "react")) {
    const reactionLevelInfo = resolveTelegramReactionLevel({ cfg, accountId: (accountId ?? undefined) });
    if (!reactionLevelInfo.agentReactionsEnabled) {
      throw new Error(("Telegram agent reactions disabled (reactionLevel=\"\"). " + "Set channels.telegram.reactionLevel to \"minimal\" or \"extensive\" to enable."));
    }
    ...
    const messageId = readNumberParam(params, "messageId", { required: true, integer: true });
    ...
    await reactMessageTelegram((chatId ?? ""), (messageId ?? 0), (emoji ?? ""), { token, remove, accountId: (accountId ?? undefined) });
    if ((!remove && !isEmpty)) {
  ...
  if ((action === "sendMessage")) {
    ...
    const content = (readStringParam(params, "content", { required: !mediaUrl, allowEmpty: true }) ?? "");
    const buttons = readTelegramButtons(params);
    if (buttons) {
      const inlineButtonsScope = resolveTelegramInlineButtonsScope({ cfg, accountId: (accountId ?? undefined) });
      ...
      if (((inlineButtonsScope === "dm") || (inlineButtonsScope === "group"))) {
        ...
        if ((targetType === "unknown")) {
          throw new Error("Telegram inline buttons require a numeric chat id when inlineButtons=\"\".");
        }
    ...
    const result = await sendMessageTelegram(to, content, { token, accountId: (accountId ?? undefined), mediaUrl: (mediaUrl || undefined), buttons, replyToMessageId: (replyToMessageId ?? undefined), messageThreadId: (messageThreadId ?? undefined), quoteText: (quoteText ?? undefined), asVoice: (typeof params.asVoice === "boolean") ? params.asVoice : undefined, silent: (typeof params.silent === "boolean") ? params.silent : undefined });
  ...
  if ((action === "deleteMessage")) {
    ...
    await deleteMessageTelegram((chatId ?? ""), (messageId ?? 0), { token, accountId: (accountId ?? undefined) });
  ...
  if ((action === "editMessage")) {
    ...
    const result = await editMessageTelegram((chatId ?? ""), (messageId ?? 0), content, { token, accountId: (accountId ?? undefined), buttons });
    return jsonResult({ ok: true, messageId: result.messageId, chatId: result.chatId });
  }
  if ((action === "sendSticker")) {
    ...
    const result = await sendStickerTelegram(to, fileId, { token, accountId: (accountId ?? undefined), replyToMessageId: (replyToMessageId ?? undefined), messageThreadId: (messageThreadId ?? undefined) });
    return jsonResult({ ok: true, messageId: result.messageId, chatId: result.chatId });
  }
  if ((action === "searchSticker")) {
    ...
    return jsonResult({ ok: true, count: results.length, stickers: results.map((s) => { fileId: s.fileId, emoji: s.emoji, description: s.description, setName: s.setName }) });
  }
