...
export function handleAgentStart(ctx) {
  ctx.log.debug("embedded run agent start: runId=");
...
export function handleAutoCompactionStart(ctx) {
  ...
  ctx.log.debug("embedded run compaction start: runId=");
  emitAgentEvent({ runId: ctx.params.runId, stream: "compaction", data: { phase: "start" } });
  void ctx.params.onAgentEvent?.({ stream: "compaction", data: { phase: "start" } });
}
...
export function handleAutoCompactionEnd(ctx, evt) {
  ...
  if (willRetry) {
    ...
    ctx.log.debug("embedded run compaction retry: runId=");
  } else {
    ctx.maybeResolveCompactionWait();
  }
  emitAgentEvent({ runId: ctx.params.runId, stream: "compaction", data: { phase: "end", willRetry } });
  void ctx.params.onAgentEvent?.({ stream: "compaction", data: { phase: "end", willRetry } });
}

export function handleAgentEnd(ctx) {
  ctx.log.debug("embedded run agent end: runId=");
