...
function matchesProviderFilter(summary, filter) {
  ...
  const providerName = (summary.providerName ?? (typeof summary.modelId === "string") ? summary.modelId.split(".")[0] : undefined);
...
function toModelDefinition(summary, defaults) {
  ...
  return { id, name: (summary.modelName?.trim() || id), reasoning: inferReasoningSupport(summary), input: mapInputModalities(summary), cost: DEFAULT_COST, contextWindow: defaults.contextWindow, maxTokens: defaults.maxTokens };
}
...
export async function discoverBedrockModels(params) {
  ...
  const discoveryPromise = async () => {
    ...
    for (const summary of (response.modelSummaries ?? [])) {
      ...
      discovered.push(toModelDefinition(summary, { contextWindow: defaultContextWindow, maxTokens: defaultMaxTokens }));
    }
    return discovered.sort((a, b) => a.name.localeCompare(b.name));
  }();
  if ((refreshIntervalSeconds > 0)) {
    discoveryCache.set(cacheKey, { expiresAt: (now + (refreshIntervalSeconds * 1000)), inFlight: discoveryPromise });
  }
  try {
    {
      const value = await discoveryPromise;
      if ((refreshIntervalSeconds > 0)) {
        discoveryCache.set(cacheKey, { expiresAt: (now + (refreshIntervalSeconds * 1000)), value });
      }
      return value;
    }
  }
  catch (error) {
    {
      ...
      if (!hasLoggedBedrockError) {
        ...
        console.warn("[bedrock-discovery] Failed to list models: ");
      }
      return [];
    }
  }
