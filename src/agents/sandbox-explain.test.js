...
import { formatSandboxToolPolicyBlockedMessage, resolveSandboxConfigForAgent, resolveSandboxToolPolicyForAgent } from "./sandbox.js";
describe("sandbox explain helpers", () => {
  it("prefers agent overrides > global > defaults (sandbox tool policy)", () => {
    const cfg = { agents: { defaults: { sandbox: { mode: "all", scope: "agent" } }, list: [{ id: "work", workspace: "~/openclaw-work", tools: { sandbox: { tools: { allow: ["write"] } } } }] }, tools: { sandbox: { tools: { allow: ["read"], deny: ["browser"] } } } };
  ...
  it("expands group tool shorthands inside sandbox tool policy", () => {
    const cfg = { agents: { defaults: { sandbox: { mode: "all", scope: "agent" } }, list: [{ id: "work", workspace: "~/openclaw-work", tools: { sandbox: { tools: { allow: ["group:memory", "group:fs"] } } } }] } };
    const policy = resolveSandboxToolPolicyForAgent(cfg, "work");
    expect(policy.allow).toEqual(["memory_search", "memory_get", "read", "write", "edit", "apply_patch", "image"]);
  });
  it("denies still win after group expansion", () => {
    const cfg = { agents: { defaults: { sandbox: { mode: "all", scope: "agent" } } }, tools: { sandbox: { tools: { allow: ["group:memory"], deny: ["memory_get"] } } } };
  ...
  it("includes config key paths + main-session hint for non-main mode", () => {
    const cfg = { agents: { defaults: { sandbox: { mode: "non-main", scope: "agent" } } }, tools: { sandbox: { tools: { deny: ["browser"] } } } };
    const msg = formatSandboxToolPolicyBlockedMessage({ cfg, sessionKey: "agent:main:whatsapp:group:g1", toolName: "browser" });
