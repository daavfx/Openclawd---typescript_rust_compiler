...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("does not call onBlockReplyFlush when callback is not provided", () => {
    ...
    subscribeEmbeddedPiSession({ session: session, runId: "run-no-flush", onBlockReply, blockReplyBreak: "text_end" });
    expect(() => {
      handler?.({ type: "tool_execution_start", toolName: "bash", toolCallId: "tool-no-flush", args: { command: "echo test" } });
    }).not.toThrow();
  });
});
