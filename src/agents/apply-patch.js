...
export function createApplyPatchTool(options = {  }) {
  ...
  return { name: "apply_patch", label: "apply_patch", description: "Apply a patch to one or more files using the apply_patch format. The input should include *** Begin Patch and *** End Patch markers.", parameters: applyPatchSchema, execute: async (_toolCallId, args, signal) => {
    const params = args;
    ...
    const result = await applyPatch(input, { cwd, sandboxRoot, signal });
    return { content: [{ type: "text", text: result.text }], details: { summary: result.summary } };
  } };
}
...
async function resolvePatchPath(filePath, options) {
  if (options.sandboxRoot) {
    const resolved = await assertSandboxPath({ filePath, cwd: options.cwd, root: options.sandboxRoot });
...
function parseOneHunk(lines, lineNumber) {
  if ((lines.length === 0)) {
    throw new Error("Invalid patch hunk at line : empty hunk");
  }
  const firstLine = lines[0].trim();
...
";
  ...
  if (firstLine.startsWith(UPDATE_FILE_MARKER)) {
    ...
    if ((chunks.length === 0)) {
      throw new Error("Invalid patch hunk at line : Update file hunk for path '' is empty");
    }
    return { hunk: { kind: "update", path: targetPath, movePath, chunks }, consumed };
  }
  throw new Error("Invalid patch hunk at line : '' is not a valid hunk header. Valid hunk headers: '*** Add File: {path}', '*** Delete File: {path}', '*** Update File: {path}'");
}
function parseUpdateFileChunk(lines, lineNumber, allowMissingContext) {
  if ((lines.length === 0)) {
    throw new Error("Invalid patch hunk at line : Update hunk does not contain any lines");
  }
  let changeContext;
  let startIndex = 0;
  if ((lines[0] === EMPTY_CHANGE_CONTEXT_MARKER)) {
    ...
    if (lines[0].startsWith(CHANGE_CONTEXT_MARKER)) {
      ...
      if (!allowMissingContext) {
        throw new Error("Invalid patch hunk at line : Expected update hunk to start with a @@ context marker, got: ''");
      }
    }
  }
  ...
  for (const line of lines.slice(startIndex)) {
    ...
    if ((parsedLines === 0)) {
      throw new Error("Invalid patch hunk at line : Unexpected line found in update hunk: ''. Every line should start with ' ' (context line), '+' (added line), or '-' (removed line)");
    }
    break;
  }
