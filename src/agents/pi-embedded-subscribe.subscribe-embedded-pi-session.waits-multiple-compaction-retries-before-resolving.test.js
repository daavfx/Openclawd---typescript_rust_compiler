...
describe("subscribeEmbeddedPiSession", () => {
  ...
  it("waits for multiple compaction retries before resolving", async () => {
    const listeners = [];
    const session = { subscribe: (listener) => {
      listeners.push(listener);
      return () => {
      };
    } };
    const subscription = subscribeEmbeddedPiSession({ session, runId: "run-3" });
  ...
  it("emits compaction events on the agent event bus", async () => {
    ...
    const stop = onAgentEvent((evt) => {
      ...
      events.push({ phase, willRetry: (typeof evt.data?.willRetry === "boolean") ? evt.data.willRetry : undefined });
    });
    subscribeEmbeddedPiSession({ session: session, runId: "run-compaction" });
    ...
    stop();
    expect(events).toEqual([{ phase: "start" }, { phase: "end", willRetry: true }, { phase: "end", willRetry: false }]);
  });
  it("emits tool summaries at tool start when verbose is on", async () => {
    ...
    subscribeEmbeddedPiSession({ session: session, runId: "run-tool", verboseLevel: "on", onToolResult });
    handler?.({ type: "tool_execution_start", toolName: "read", toolCallId: "tool-1", args: { path: "/tmp/a.txt" } });
    await Promise.resolve();
    expect(onToolResult).toHaveBeenCalledTimes(1);
    const payload = onToolResult.mock.calls[0][0];
    expect(payload.text).toContain("/tmp/a.txt");
    handler?.({ type: "tool_execution_end", toolName: "read", toolCallId: "tool-1", isError: false, result: "ok" });
    expect(onToolResult).toHaveBeenCalledTimes(1);
  });
  it("includes browser action metadata in tool summaries", async () => {
    ...
    const onToolResult = vi.fn();
    subscribeEmbeddedPiSession({ session: session, runId: "run-browser-tool", verboseLevel: "on", onToolResult });
    handler?.({ type: "tool_execution_start", toolName: "browser", toolCallId: "tool-browser-1", args: { action: "snapshot", targetUrl: "https://example.com" } });
    await Promise.resolve();
    expect(onToolResult).toHaveBeenCalledTimes(1);
    const payload = onToolResult.mock.calls[0][0];
  ...
  it("emits exec output in full verbose mode and includes PTY indicator", async () => {
    ...
    const onToolResult = vi.fn();
    subscribeEmbeddedPiSession({ session: session, runId: "run-exec-full", verboseLevel: "full", onToolResult });
    handler?.({ type: "tool_execution_start", toolName: "exec", toolCallId: "tool-exec-1", args: { command: "claude", pty: true } });
    await Promise.resolve();
    expect(onToolResult).toHaveBeenCalledTimes(1);
...
world" }] } });
    ...
    handler?.({ type: "tool_execution_end", toolName: "read", toolCallId: "tool-read-1", isError: false, result: { content: [{ type: "text", text: "file data" }] } });
    await Promise.resolve();
