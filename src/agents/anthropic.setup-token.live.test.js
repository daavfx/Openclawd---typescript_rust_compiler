...
import { ANTHROPIC_SETUP_TOKEN_PREFIX, validateAnthropicSetupToken } from "../commands/auth-token.js";
...
async function resolveTokenSource() {
  ...
  if (explicitToken) {
    ...
    const profileId = "anthropic:setup-token-live-";
    const store = ensureAuthProfileStore(tempDir, { allowKeychainPrompt: false });
    store.profiles[profileId] = { type: "token", provider: "anthropic", token: explicitToken };
    saveAuthProfileStore(store, tempDir);
    return { agentDir: tempDir, profileId, cleanup: async () => {
  ...
  if (SETUP_TOKEN_PROFILE) {
    if (!candidates.includes(SETUP_TOKEN_PROFILE)) {
      ...
      throw new Error("Setup-token profile \"\" not found. Available: .");
    }
...
function pickModel(models, raw) {
  ...
  if (normalized) {
    ...
    return (models.find((model) => ((normalizeProviderId(model.provider) === parsed.provider) && (model.id === parsed.model))) ?? null);
  }
  const preferred = ["claude-opus-4-5", "claude-sonnet-4-5", "claude-sonnet-4-0", "claude-haiku-3-5"];
  for (const id of preferred) {
    ...
    try {
      {
        ...
        if (!model) {
          throw new Error(SETUP_TOKEN_MODEL ? "Model not found: " : "No Anthropic models available.");
        }
        const apiKeyInfo = await getApiKeyForModel({ model, cfg, profileId: tokenSource.profileId, agentDir: tokenSource.agentDir });
        const apiKey = requireApiKey(apiKeyInfo, model.provider);
        const tokenError = validateAnthropicSetupToken(apiKey);
        if (tokenError) {
          throw new Error("Resolved profile is not a setup-token: ");
        }
        const res = await completeSimple(model, { messages: [{ role: "user", content: "Reply with the word ok.", timestamp: Date.now() }] }, { apiKey, maxTokens: 64, temperature: 0 });
