export const GEMINI_UNSUPPORTED_SCHEMA_KEYWORDS = new Set(["patternProperties", "additionalProperties", "$schema", "$id", "$ref", "$defs", "definitions", "examples", "minLength", "maxLength", "minimum", "maximum", "multipleOf", "pattern", "format", "minItems", "maxItems", "uniqueItems", "minProperties", "maxProperties"])
...
function extendSchemaDefs(defs, schema) {
  const defsEntry = ((schema.$defs && (typeof schema.$defs === "object")) && !Array.isArray(schema.$defs)) ? schema.$defs : undefined;
  const legacyDefsEntry = ((schema.definitions && (typeof schema.definitions === "object")) && !Array.isArray(schema.definitions)) ? schema.definitions : undefined;
...
function cleanSchemaForGeminiWithDefs(schema, defs, refStack) {
  ...
  let cleanedAnyOf = hasAnyOf ? obj.anyOf.map((variant) => cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack)) : undefined;
  let cleanedOneOf = hasOneOf ? obj.oneOf.map((variant) => cleanSchemaForGeminiWithDefs(variant, nextDefs, refStack)) : undefined;
  if (hasAnyOf) {
    ...
    if (flattened) {
      const result = { type: flattened.type, enum: flattened.enum };
      for (const key of ["description", "title", "default"]) {
        if (((key in obj) && (obj[key] !== undefined))) {
          result[key] = obj[key];
        }
