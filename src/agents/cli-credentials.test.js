  ...
  it("updates the Claude Code keychain item in place", async () => {
    ...
    execSyncMock.mockImplementation((command) => {
      ...
      if (cmd.includes("find-generic-password")) {
        return JSON.stringify({ claudeAiOauth: { accessToken: "old-access", refreshToken: "old-refresh", expiresAt: (Date.now() + 60000) } });
      }
      return "";
    });
    const {writeClaudeCliKeychainCredentials} = await import("./cli-credentials.js");
    const ok = writeClaudeCliKeychainCredentials({ access: "new-access", refresh: "new-refresh", expires: (Date.now() + 60000) }, { execSync: execSyncMock });
    expect(ok).toBe(true);
...
", "utf8");
    ...
    const ok = writeClaudeCliCredentials({ access: "new-access", refresh: "new-refresh", expires: (Date.now() + 120000) }, { platform: "darwin", homeDir: tempDir, writeKeychain });
    expect(ok).toBe(true);
  ...
  it("caches Claude Code CLI credentials within the TTL window", async () => {
    execSyncMock.mockImplementation(() => JSON.stringify({ claudeAiOauth: { accessToken: "cached-access", refreshToken: "cached-refresh", expiresAt: (Date.now() + 60000) } }));
    vi.setSystemTime(new Date("2025-01-01T00:00:00Z"));
    const {readClaudeCliCredentialsCached} = await import("./cli-credentials.js");
    const first = readClaudeCliCredentialsCached({ allowKeychainPrompt: true, ttlMs: ((15 * 60) * 1000), platform: "darwin", execSync: execSyncMock });
    const second = readClaudeCliCredentialsCached({ allowKeychainPrompt: false, ttlMs: ((15 * 60) * 1000), platform: "darwin", execSync: execSyncMock });
    expect(first).toBeTruthy();
    expect(second).toEqual(first);
    expect(execSyncMock).toHaveBeenCalledTimes(1);
  });
  it("refreshes Claude Code CLI credentials after the TTL window", async () => {
    execSyncMock.mockImplementation(() => JSON.stringify({ claudeAiOauth: { accessToken: "token-", refreshToken: "refresh", expiresAt: (Date.now() + 60000) } }));
    vi.setSystemTime(new Date("2025-01-01T00:00:00Z"));
    const {readClaudeCliCredentialsCached} = await import("./cli-credentials.js");
    const first = readClaudeCliCredentialsCached({ allowKeychainPrompt: true, ttlMs: ((15 * 60) * 1000), platform: "darwin", execSync: execSyncMock });
    vi.advanceTimersByTime((((15 * 60) * 1000) + 1));
    const second = readClaudeCliCredentialsCached({ allowKeychainPrompt: true, ttlMs: ((15 * 60) * 1000), platform: "darwin", execSync: execSyncMock });
    expect(first).toBeTruthy();
  ...
  it("reads Codex credentials from keychain when available", async () => {
    ...
    execSyncMock.mockImplementation((command) => {
      ...
      return JSON.stringify({ tokens: { access_token: "keychain-access", refresh_token: "keychain-refresh" }, last_refresh: "2026-01-01T00:00:00Z" });
    });
    ...
    expect(creds).toMatchObject({ access: "keychain-access", refresh: "keychain-refresh", provider: "openai-codex" });
  });
  it("falls back to Codex auth.json when keychain is unavailable", async () => {
    ...
    fs.writeFileSync(authPath, JSON.stringify({ tokens: { access_token: "file-access", refresh_token: "file-refresh" } }), "utf8");
    const {readCodexCliCredentials} = await import("./cli-credentials.js");
    const creds = readCodexCliCredentials({ execSync: execSyncMock });
    expect(creds).toMatchObject({ access: "file-access", refresh: "file-refresh", provider: "openai-codex" });
  });
});
