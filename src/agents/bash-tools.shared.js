...
export function buildSandboxEnv(params) {
  const env = { PATH: params.defaultPath, HOME: params.containerWorkdir };
...
export async function resolveSandboxWorkdir(params) {
  ...
  try {
    {
      const resolved = await assertSandboxPath({ filePath: params.workdir, cwd: process.cwd(), root: params.sandbox.workspaceDir });
    ...
    {
      params.warnings.push("Warning: workdir \"\" is unavailable; using \"\".");
      return { hostWorkdir: fallback, containerWorkdir: params.sandbox.containerWorkdir };
    }
  }
}
...
export function resolveWorkdir(workdir, warnings) {
  ...
  warnings.push("Warning: workdir \"\" is unavailable; using \"\".");
  return fallback;
}
