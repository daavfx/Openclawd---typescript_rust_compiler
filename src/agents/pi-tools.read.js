...
async function normalizeReadImageResult(result, filePath) {
  ...
  if (!image.data.trim()) {
    throw new Error("read: image payload is empty ()");
  }
  ...
  if (!sniffed.startsWith("image/")) {
    throw new Error("read: file looks like  but was treated as  ()");
  }
...
export const CLAUDE_PARAM_GROUPS = { read: [{ keys: ["path", "file_path"], label: "path (path or file_path)" }], write: [{ keys: ["path", "file_path"], label: "path (path or file_path)" }], edit: [{ keys: ["path", "file_path"], label: "path (path or file_path)" }, { keys: ["oldText", "old_string"], label: "oldText (oldText or old_string)" }, { keys: ["newText", "new_string"], label: "newText (newText or new_string)" }] }
...
export function patchToolSchemaForClaudeCompatibility(tool) {
  const schema = (tool.parameters && (typeof tool.parameters === "object")) ? tool.parameters : undefined;
  ...
  let changed = false;
  const aliasPairs = [{ original: "path", alias: "file_path" }, { original: "oldText", alias: "old_string" }, { original: "newText", alias: "new_string" }];
  ...
  return { ...tool: , parameters: { ...schema: , properties, ...(required.length > 0) ? { required } : {  }:  } };
}

export function assertRequiredParams(record, groups, toolName) {
  if ((!record || (typeof record !== "object"))) {
    throw new Error("Missing parameters for ");
  }
...
export function wrapToolParamNormalization(tool, requiredParamGroups) {
  ...
  return { ...patched: , execute: async (toolCallId, params, signal, onUpdate) => {
...
function wrapSandboxPathGuard(tool, root) {
  return { ...tool: , execute: async (toolCallId, args, signal, onUpdate) => {
...
export function createOpenClawReadTool(base) {
  ...
  return { ...patched: , execute: async (toolCallId, params, signal) => {
    const normalized = normalizeToolParams(params);
    const record = (normalized ?? (params && (typeof params === "object")) ? params : undefined);
    ...
    return sanitizeToolResultImages(normalizedResult, "read:");
  } };
}
