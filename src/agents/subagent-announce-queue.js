...
function getAnnounceQueue(key, settings, send) {
  ...
  const created = { items: [], draining: false, lastEnqueuedAt: 0, mode: settings.mode, debounceMs: (typeof settings.debounceMs === "number") ? Math.max(0, settings.debounceMs) : 1000, cap: ((typeof settings.cap === "number") && (settings.cap > 0)) ? Math.floor(settings.cap) : 20, dropPolicy: (settings.dropPolicy ?? "summarize"), droppedCount: 0, summaryLines: [], send };
    ...
    try {
      {
        ...
        while (((queue.items.length > 0) || (queue.droppedCount > 0))) {
          ...
          if ((queue.mode === "collect")) {
            ...
            const prompt = buildCollectPrompt({ title: "[Queued announce messages while agent was busy]", items, summary, renderItem: (item, idx) => "---
Queued #
".trim() });
      ...
      {
        defaultRuntime.error?.("announce queue drain failed for : ");
      }
    }
    finally {
      {
...
export function enqueueAnnounce(params) {
  ...
  const shouldEnqueue = applyQueueDropPolicy({ queue, summarize: (item) => (item.summaryLine?.trim() || item.prompt.trim()) });
  if (!shouldEnqueue) {
