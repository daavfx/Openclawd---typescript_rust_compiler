...
vi.mock("../gateway/call.js", () => { callGateway: vi.fn(async () => { status: "ok", startedAt: 111, endedAt: 222 }) });
vi.mock("../infra/agent-events.js", () => { onAgentEvent: vi.fn(() => noop) });
const announceSpy = vi.fn(async () => true);
vi.mock("./subagent-announce.js", () => { runSubagentAnnounceFlow: (...args) => announceSpy(...args) });
describe("subagent registry persistence", () => {
  ...
  it("persists runs to disk and resumes after restart", async () => {
    ...
    mod1.registerSubagentRun({ runId: "run-1", childSessionKey: "agent:main:subagent:test", requesterSessionKey: "agent:main:main", requesterOrigin: { channel: " whatsapp ", accountId: " acct-main " }, requesterDisplayKey: "main", task: "do the thing", cleanup: "keep" });
  ...
  it("skips cleanup when cleanupHandled was persisted", async () => {
    ...
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
    const registryPath = path.join(tempStateDir, "subagents", "runs.json");
    const persisted = { version: 2, runs: { "run-2": { runId: "run-2", childSessionKey: "agent:main:subagent:two", requesterSessionKey: "agent:main:main", requesterDisplayKey: "main", task: "do the other thing", cleanup: "keep", createdAt: 1, startedAt: 1, endedAt: 2, cleanupHandled: true } } };
...
", "utf8");
  ...
  it("maps legacy announce fields into cleanup state", async () => {
    ...
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
    const registryPath = path.join(tempStateDir, "subagents", "runs.json");
    const persisted = { version: 1, runs: { "run-legacy": { runId: "run-legacy", childSessionKey: "agent:main:subagent:legacy", requesterSessionKey: "agent:main:main", requesterDisplayKey: "main", task: "legacy announce", cleanup: "keep", createdAt: 1, startedAt: 1, endedAt: 2, announceCompletedAt: 9, announceHandled: true, requesterChannel: "whatsapp", requesterAccountId: "legacy-account" } } };
    await fs.mkdir(path.dirname(registryPath), { recursive: true });
    await fs.writeFile(registryPath, "
", "utf8");
    vi.resetModules();
  ...
  it("retries cleanup announce after a failed announce", async () => {
    ...
    process.env.OPENCLAW_STATE_DIR = tempStateDir;
    const registryPath = path.join(tempStateDir, "subagents", "runs.json");
    const persisted = { version: 2, runs: { "run-3": { runId: "run-3", childSessionKey: "agent:main:subagent:three", requesterSessionKey: "agent:main:main", requesterDisplayKey: "main", task: "retry announce", cleanup: "keep", createdAt: 1, startedAt: 1, endedAt: 2 } } };
    await fs.mkdir(path.dirname(registryPath), { recursive: true });
    await fs.writeFile(registryPath, "
", "utf8");
