...
function buildModelAliasLines(cfg) {
  ...
  return entries.sort((a, b) => a.alias.localeCompare(b.alias)).map((entry) => "- : ");
}
export function buildSystemPrompt(params) {
  const defaultModelRef = resolveDefaultModelForAgent({ cfg: (params.config ?? {  }), agentId: params.agentId });
  const defaultModelLabel = "/";
  const {runtimeInfo, userTimezone, userTime, userTimeFormat} = buildSystemPromptParams({ config: params.config, agentId: params.agentId, workspaceDir: params.workspaceDir, cwd: process.cwd(), runtime: { host: "openclaw", os: " ", arch: os.arch(), node: process.version, model: params.modelDisplay, defaultModel: defaultModelLabel } });
  const ttsHint = params.config ? buildTtsSystemPromptHint(params.config) : undefined;
  return buildAgentSystemPrompt({ workspaceDir: params.workspaceDir, defaultThinkLevel: params.defaultThinkLevel, extraSystemPrompt: params.extraSystemPrompt, ownerNumbers: params.ownerNumbers, reasoningTagHint: false, heartbeatPrompt: params.heartbeatPrompt, docsPath: params.docsPath, runtimeInfo, toolNames: params.tools.map((tool) => tool.name), modelAliasLines: buildModelAliasLines(params.config), userTimezone, userTime, userTimeFormat, contextFiles: params.contextFiles, ttsHint });
}
...
function pickSessionId(parsed, backend) {
  const fields = (backend.sessionIdFields ?? ["session_id", "sessionId", "conversation_id", "conversationId"]);
...
export async function writeCliImages(images) {
  ...
  for (let i = 0; (i < images.length); i += 1) {
    ...
    const filePath = path.join(tempDir, "image-.");
