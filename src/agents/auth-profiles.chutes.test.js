...
describe("auth-profiles (chutes)", () => {
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    ...
    const store = { version: 1, profiles: { "chutes:default": { type: "oauth", provider: "chutes", access: "at_old", refresh: "rt_old", expires: (Date.now() - 60000), clientId: "cid_test" } } };
    await fs.writeFile(authProfilePath, "
");
    const fetchSpy = vi.fn(async (input) => {
      ...
      return new Response(JSON.stringify({ access_token: "at_new", expires_in: 3600 }), { status: 200, headers: { "Content-Type": "application/json" } });
    });
    vi.stubGlobal("fetch", fetchSpy);
    const loaded = ensureAuthProfileStore();
    const resolved = await resolveApiKeyForProfile({ store: loaded, profileId: "chutes:default" });
