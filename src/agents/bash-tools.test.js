...
const defaultShell = isWin ? undefined : (((process.env.OPENCLAW_TEST_SHELL || resolveShellFromPath("bash")) || process.env.SHELL) || "sh");
...
const echoAfterDelay = (message) => joinCommands([shortDelayCmd, "echo "]);
const echoLines = (lines) => joinCommands(lines.map((line) => "echo "));
...
async function waitForCompletion(sessionId) {
  ...
  while (((Date.now() < deadline) && (status === "running"))) {
    const poll = await processTool.execute("call-wait", { action: "poll", sessionId });
...
describe("exec tool backgrounding", () => {
  ...
  it("backgrounds after yield and can be polled", async () => {
    const result = await execTool.execute("call1", { command: joinCommands([yieldDelayCmd, "echo done"]), yieldMs: 10 });
    ...
    const deadline = (Date.now() + (process.platform === "win32") ? 8000 : 2000);
    while (((Date.now() < deadline) && (status === "running"))) {
      const poll = await processTool.execute("call2", { action: "poll", sessionId });
      status = poll.details.status;
  ...
  it("supports explicit background", async () => {
    const result = await execTool.execute("call1", { command: echoAfterDelay("later"), background: true });
    expect(result.details.status).toBe("running");
    const sessionId = result.details.sessionId;
  ...
  it("derives a session name from the command", async () => {
    const result = await execTool.execute("call1", { command: "echo hello", background: true });
    const sessionId = result.details.sessionId;
    await sleep(25);
    const list = await processTool.execute("call2", { action: "list" });
    const sessions = list.details.sessions;
  ...
  it("uses default timeout when timeout is omitted", async () => {
    ...
    const result = await customBash.execute("call1", { command: longDelayCmd, background: true });
    const sessionId = result.details.sessionId;
    let status = "running";
    const deadline = (Date.now() + 5000);
    while (((Date.now() < deadline) && (status === "running"))) {
      const poll = await customProcess.execute("call2", { action: "poll", sessionId });
      status = poll.details.status;
      if ((status === "running")) {
        await sleep(50);
      }
  ...
  it("rejects elevated requests when not allowed", async () => {
    const customBash = createExecTool({ elevated: { enabled: true, allowed: false, defaultLevel: "off" }, messageProvider: "telegram", sessionKey: "agent:main:main" });
    await expect(customBash.execute("call1", { command: "echo hi", elevated: true })).rejects.toThrow("Context: provider=telegram session=agent:main:main");
  });
  it("does not default to elevated when not allowed", async () => {
    const customBash = createExecTool({ elevated: { enabled: true, allowed: false, defaultLevel: "on" }, backgroundMs: 1000, timeoutSec: 5 });
    const result = await customBash.execute("call1", { command: "echo hi" });
    const text = (result.content.find((c) => (c.type === "text"))?.text ?? "");
    expect(text).toContain("hi");
  });
  it("logs line-based slices and defaults to last lines", async () => {
    const result = await execTool.execute("call1", { command: echoLines(["one", "two", "three"]), background: true });
    const sessionId = result.details.sessionId;
    const status = await waitForCompletion(sessionId);
    const log = await processTool.execute("call3", { action: "log", sessionId, limit: 2 });
...
three");
  ...
  it("supports line offsets for log slices", async () => {
    const result = await execTool.execute("call1", { command: echoLines(["alpha", "beta", "gamma"]), background: true });
    const sessionId = result.details.sessionId;
    await waitForCompletion(sessionId);
    const log = await processTool.execute("call2", { action: "log", sessionId, offset: 1, limit: 1 });
    const textBlock = log.content.find((c) => (c.type === "text"));
  ...
  it("scopes process sessions by scopeKey", async () => {
    ...
    const resultA = await bashA.execute("call1", { command: shortDelayCmd, background: true });
    const resultB = await bashB.execute("call2", { command: shortDelayCmd, background: true });
    ...
    const pollB = await processB.execute("call4", { action: "poll", sessionId: sessionA });
...
describe("exec notifyOnExit", () => {
  it("enqueues a system event when a backgrounded exec exits", async () => {
    const tool = createExecTool({ allowBackground: true, backgroundMs: 0, notifyOnExit: true, sessionKey: "agent:main:main" });
    const result = await tool.execute("call1", { command: echoAfterDelay("notify"), background: true });
    expect(result.details.status).toBe("running");
    const sessionId = result.details.sessionId;
...
describe("exec PATH handling", () => {
  ...
  it("prepends configured path entries", async () => {
    ...
    const result = await tool.execute("call1", { command: isWin ? "Write-Output $env:PATH" : "echo $PATH" });
...
describe("buildDockerExecArgs", () => {
  it("prepends custom PATH after login shell sourcing to preserve both custom and system tools", () => {
    const args = buildDockerExecArgs({ containerName: "test-container", command: "echo hello", env: { PATH: "/custom/bin:/usr/local/bin:/usr/bin", HOME: "/home/user" }, tty: false });
    ...
    expect(commandArg).toBe("export PATH=\"${OPENCLAW_PREPEND_PATH}:$PATH\"; unset OPENCLAW_PREPEND_PATH; echo hello");
  });
  it("does not interpolate PATH into the shell command", () => {
    const injectedPath = "$(touch /tmp/openclaw-path-injection)";
    const args = buildDockerExecArgs({ containerName: "test-container", command: "echo hello", env: { PATH: injectedPath, HOME: "/home/user" }, tty: false });
    const commandArg = args[(args.length - 1)];
    expect(args).toContain("OPENCLAW_PREPEND_PATH=");
  ...
  it("does not add PATH export when PATH is not in env", () => {
    const args = buildDockerExecArgs({ containerName: "test-container", command: "echo hello", env: { HOME: "/home/user" }, tty: false });
    const commandArg = args[(args.length - 1)];
    expect(commandArg).toBe("echo hello");
    expect(commandArg).not.toContain("export PATH");
  });
  it("includes workdir flag when specified", () => {
    const args = buildDockerExecArgs({ containerName: "test-container", command: "pwd", workdir: "/workspace", env: { HOME: "/home/user" }, tty: false });
    expect(args).toContain("-w");
    expect(args).toContain("/workspace");
  });
  it("uses login shell for consistent environment", () => {
    const args = buildDockerExecArgs({ containerName: "test-container", command: "echo test", env: { HOME: "/home/user" }, tty: false });
    expect(args).toContain("sh");
    expect(args).toContain("-lc");
  });
  it("includes tty flag when requested", () => {
    const args = buildDockerExecArgs({ containerName: "test-container", command: "bash", env: { HOME: "/home/user" }, tty: true });
