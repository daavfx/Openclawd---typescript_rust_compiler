...
export async function discoverVeniceModels() {
  ...
  try {
    {
      const response = await fetch("/models", { signal: AbortSignal.timeout(5000) });
      if (!response.ok) {
        console.warn("[venice-models] Failed to discover models: HTTP , using static catalog");
        return VENICE_MODEL_CATALOG.map(buildVeniceModelDefinition);
      }
      ...
      for (const apiModel of data.data) {
        ...
        if (catalogEntry) {
          ...
          models.push({ id: apiModel.id, name: (apiModel.model_spec.name || apiModel.id), reasoning: isReasoning, input: hasVision ? ["text", "image"] : ["text"], cost: VENICE_DEFAULT_COST, contextWindow: (apiModel.model_spec.availableContextTokens || 128000), maxTokens: 8192 });
        }
      }
    ...
    {
      console.warn("[venice-models] Discovery failed: , using static catalog");
      return VENICE_MODEL_CATALOG.map(buildVeniceModelDefinition);
    }
  }
}
