...
export async function gatherDaemonStatus(opts) {
  ...
  const [loaded, command, runtime] = await Promise.all([service.isLoaded({ env: process.env }).catch(() => false), service.readCommand(process.env).catch(() => null), service.readRuntime(process.env).catch((err) => { status: "unknown", detail: String(err) })]);
  const configAudit = await auditGatewayServiceConfig({ env: process.env, command });
  const serviceEnv = (command?.environment ?? undefined);
  const mergedDaemonEnv = { ...process.env: , ...(serviceEnv ?? undefined):  };
  ...
  const daemonIO = createConfigIO({ env: mergedDaemonEnv, configPath: daemonConfigPath });
  ...
  const cliConfigSummary = { path: (cliSnapshot?.path ?? cliConfigPath), exists: (cliSnapshot?.exists ?? false), valid: (cliSnapshot?.valid ?? true), ...cliSnapshot?.issues?.length ? { issues: cliSnapshot.issues } : {  }: , controlUi: cliCfg.gateway?.controlUi };
  const daemonConfigSummary = { path: (daemonSnapshot?.path ?? daemonConfigPath), exists: (daemonSnapshot?.exists ?? false), valid: (daemonSnapshot?.valid ?? true), ...daemonSnapshot?.issues?.length ? { issues: daemonSnapshot.issues } : {  }: , controlUi: daemonCfg.gateway?.controlUi };
  ...
  const probeUrl = (probeUrlOverride ?? "ws://:");
  const probeNote = (!probeUrlOverride && (bindMode === "lan")) ? "Local probe uses loopback (127.0.0.1). bind=lan listens on 0.0.0.0 (all interfaces); use a LAN IP for remote clients." : (!probeUrlOverride && (bindMode === "loopback")) ? "Loopback-only gateway; only local clients can connect." : undefined;
  ...
  const portStatus = portDiagnostics ? { port: portDiagnostics.port, status: portDiagnostics.status, listeners: portDiagnostics.listeners, hints: portDiagnostics.hints } : undefined;
  const portCliStatus = portCliDiagnostics ? { port: portCliDiagnostics.port, status: portCliDiagnostics.status, listeners: portCliDiagnostics.listeners, hints: portCliDiagnostics.hints } : undefined;
  ...
  const rpc = opts.probe ? await probeGatewayStatus({ url: probeUrl, token: ((opts.rpc.token || mergedDaemonEnv.OPENCLAW_GATEWAY_TOKEN) || daemonCfg.gateway?.auth?.token), password: ((opts.rpc.password || mergedDaemonEnv.OPENCLAW_GATEWAY_PASSWORD) || daemonCfg.gateway?.auth?.password), timeoutMs, json: opts.rpc.json, configPath: daemonConfigSummary.path }) : undefined;
  let lastError;
...
export function renderPortDiagnosticsForCli(status, rpcOk) {
  ...
  return formatPortDiagnostics({ port: status.port.port, status: status.port.status, listeners: status.port.listeners, hints: status.port.hints });
}

export function resolvePortListeningAddresses(status) {
  const addrs = Array.from(new Set((status.port?.listeners?.map((l) => l.address ? normalizeListenerAddress(l.address) : "").filter((v) => Boolean(v)) ?? [])));
  return addrs;
}
