...
describe("parseCliProfileArgs", () => {
  it("leaves gateway --dev for subcommands", () => {
    const res = parseCliProfileArgs(["node", "openclaw", "gateway", "--dev", "--allow-unconfigured"]);
    if (!res.ok) {
...
describe("applyCliProfileEnv", () => {
  it("fills env defaults for dev profile", () => {
    const env = {  };
    applyCliProfileEnv({ profile: "dev", env, homedir: () => "/home/peter" });
  ...
  it("does not override explicit env values", () => {
    const env = { OPENCLAW_STATE_DIR: "/custom", OPENCLAW_GATEWAY_PORT: "19099" };
    applyCliProfileEnv({ profile: "dev", env, homedir: () => "/home/peter" });
...
describe("formatCliCommand", () => {
  ...
  it("returns command unchanged when profile is default", () => {
    expect(formatCliCommand("openclaw doctor --fix", { OPENCLAW_PROFILE: "default" })).toBe("openclaw doctor --fix");
  });
  it("returns command unchanged when profile is Default (case-insensitive)", () => {
    expect(formatCliCommand("openclaw doctor --fix", { OPENCLAW_PROFILE: "Default" })).toBe("openclaw doctor --fix");
  });
  it("returns command unchanged when profile is invalid", () => {
    expect(formatCliCommand("openclaw doctor --fix", { OPENCLAW_PROFILE: "bad profile" })).toBe("openclaw doctor --fix");
  });
  it("returns command unchanged when --profile is already present", () => {
    expect(formatCliCommand("openclaw --profile work doctor --fix", { OPENCLAW_PROFILE: "work" })).toBe("openclaw --profile work doctor --fix");
  });
  it("returns command unchanged when --dev is already present", () => {
    expect(formatCliCommand("openclaw --dev doctor", { OPENCLAW_PROFILE: "dev" })).toBe("openclaw --dev doctor");
  });
  ...
  it("handles pnpm wrapper", () => {
    expect(formatCliCommand("pnpm openclaw doctor", { OPENCLAW_PROFILE: "work" })).toBe("pnpm openclaw --profile work doctor");
  });
});
