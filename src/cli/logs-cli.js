...
function createLogWriters() {
  const writer = createSafeStreamWriter({ beforeWrite: () => clearActiveProgressLine(), onBrokenPipe: (err, stream) => {
    const code = (err.code ?? "EPIPE");
    const target = (stream === process.stdout) ? "stdout" : "stderr";
    const message = "openclaw logs: output  closed (). Stopping tail.";
    try {
      {
...
");
  ...
  return { logLine: (text) => writer.writeLine(process.stdout, text), errorLine: (text) => writer.writeLine(process.stderr, text), emitJsonLine: (payload, toStdErr = false) => writer.write(toStdErr ? process.stderr : process.stdout, "
") };
}
function emitGatewayError(err, opts, mode, rich, emitJsonLine, errorLine) {
  ...
  if ((mode === "json")) {
    if (!emitJsonLine({ type: "error", message, error: errorText, details, hint }, true)) {
      return;
    }
    return;
  }
...
");
  ...
  logs.action(async (opts) => {
    ...
    while (true) {
      ...
      if (jsonMode) {
        if (first) {
          if (!emitJsonLine({ type: "meta", file: payload.file, cursor: payload.cursor, size: payload.size })) {
            return;
          }
        }
        ...
        if (payload.truncated) {
          if (!emitJsonLine({ type: "notice", message: "Log tail truncated (increase --max-bytes)." })) {
            return;
          }
        }
        if (payload.reset) {
          if (!emitJsonLine({ type: "notice", message: "Log cursor reset (file rotated)." })) {
            return;
          }
        }
      } else {
        ...
        for (const line of lines) {
          if (!logLine(formatLogLine(line, { pretty, rich }))) {
            return;
          }
        }
        if (payload.truncated) {
