...
vi.mock("../commands/models.js", async () => {
  ...
  return { ...actual: , githubCopilotLoginCommand };
});
describe("models cli", () => {
  it("registers github-copilot login command", { timeout: 60000 }, async () => {
    ...
    await program.parseAsync(["models", "auth", "login-github-copilot", "--yes"], { from: "user" });
    expect(githubCopilotLoginCommand).toHaveBeenCalledTimes(1);
    expect(githubCopilotLoginCommand).toHaveBeenCalledWith(expect.objectContaining({ yes: true }), expect.any(Object));
  });
});
