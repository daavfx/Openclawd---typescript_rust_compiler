...
function loadSnapshotLocal() {
  ...
  return { path: snapshot.path, exists: snapshot.exists, hash: snapshot.hash, file: snapshot.file };
}
...
function renderApprovalsSnapshot(snapshot, targetLabel) {
  ...
  const defaultsParts = [defaults.security ? "security=" : null, defaults.ask ? "ask=" : null, defaults.askFallback ? "askFallback=" : null, (typeof defaults.autoAllowSkills === "boolean") ? "autoAllowSkills=" : null].filter(Boolean);
  ...
  for (const [agentId, agent] of Object.entries(agents)) {
    ...
    for (const entry of allowlist) {
      ...
      allowlistRows.push({ Target: targetLabel, Agent: agentId, Pattern: pattern, LastUsed: lastUsedAt ? " ago" : muted("unknown") });
    }
  }
  const summaryRows = [{ Field: "Target", Value: targetLabel }, { Field: "Path", Value: snapshot.path }, { Field: "Exists", Value: snapshot.exists ? "yes" : "no" }, { Field: "Hash", Value: snapshot.hash }, { Field: "Version", Value: String((file.version ?? 1)) }, { Field: "Socket", Value: (file.socket?.path ?? "default") }, { Field: "Defaults", Value: (defaultsParts.length > 0) ? defaultsParts.join(", ") : "none" }, { Field: "Agents", Value: String(Object.keys(agents).length) }, { Field: "Allowlist", Value: String(allowlistRows.length) }];
  defaultRuntime.log(heading("Approvals"));
  defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Field", header: "Field", minWidth: 8 }, { key: "Value", header: "Value", minWidth: 24, flex: true }], rows: summaryRows }).trimEnd());
  ...
  defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Target", header: "Target", minWidth: 10 }, { key: "Agent", header: "Agent", minWidth: 8 }, { key: "Pattern", header: "Pattern", minWidth: 20, flex: true }, { key: "LastUsed", header: "Last Used", minWidth: 10 }], rows: allowlistRows }).trimEnd());
}
...
function isEmptyAgent(agent) {
  const allowlist = Array.isArray(agent.allowlist) ? agent.allowlist : [];
  return ((((!agent.security && !agent.ask) && !agent.askFallback) && (agent.autoAllowSkills === undefined)) && (allowlist.length === 0));
}
export function registerExecApprovalsCli(program) {
  const formatExample = (cmd, desc) => "  
    ";
    ...
    try {
      {
        ...
        const targetLabel = (source === "local") ? "local" : nodeId ? "node:" : "gateway";
    ...
    try {
      {
        ...
        try {
          ...
          {
            defaultRuntime.error("Failed to parse approvals JSON: ");
            defaultRuntime.exit(1);
            return;
          }
        }
        ...
        defaultRuntime.log(theme.muted("Target: "));
