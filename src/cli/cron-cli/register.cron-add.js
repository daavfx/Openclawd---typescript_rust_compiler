    ...
    try {
      {
        const schedule = () => {
          ...
          return { kind: "cron", expr: cronExpr, tz: ((typeof opts.tz === "string") && opts.tz.trim()) ? opts.tz.trim() : undefined };
        }();
        ...
        const agentId = ((typeof opts.agent === "string") && opts.agent.trim()) ? sanitizeAgentId(opts.agent.trim()) : undefined;
        const payload = () => {
          ...
          return { kind: "agentTurn", message, model: ((typeof opts.model === "string") && opts.model.trim()) ? opts.model.trim() : undefined, thinking: ((typeof opts.thinking === "string") && opts.thinking.trim()) ? opts.thinking.trim() : undefined, timeoutSeconds: (timeoutSeconds && Number.isFinite(timeoutSeconds)) ? timeoutSeconds : undefined, deliver: opts.deliver ? true : undefined, channel: (typeof opts.channel === "string") ? opts.channel : "last", to: ((typeof opts.to === "string") && opts.to.trim()) ? opts.to.trim() : undefined, bestEffortDeliver: opts.bestEffortDeliver ? true : undefined };
        }();
        ...
        const isolation = (sessionTarget === "isolated") ? { postToMainPrefix: ((typeof opts.postPrefix === "string") && opts.postPrefix.trim()) ? opts.postPrefix.trim() : "Cron", postToMainMode: ((opts.postMode === "full") || (opts.postMode === "summary")) ? opts.postMode : undefined, postToMainMaxChars: ((typeof opts.postMaxChars === "string") && /^\d+$/.test(opts.postMaxChars)) ? Number.parseInt(opts.postMaxChars, 10) : undefined } : undefined;
        ...
        const params = { name, description, enabled: !opts.disabled, deleteAfterRun: Boolean(opts.deleteAfterRun), agentId, schedule, sessionTarget, wakeMode, payload, isolation };
