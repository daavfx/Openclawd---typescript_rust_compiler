    ...
    try {
      {
        ...
        if (opts.at) {
          ...
          if (opts.every) {
            ...
            if (opts.cron) {
              patch.schedule = { kind: "cron", expr: String(opts.cron), tz: ((typeof opts.tz === "string") && opts.tz.trim()) ? opts.tz.trim() : undefined };
            }
          }
        }
        const hasSystemEventPatch = (typeof opts.systemEvent === "string");
        const model = ((typeof opts.model === "string") && opts.model.trim()) ? opts.model.trim() : undefined;
        const thinking = ((typeof opts.thinking === "string") && opts.thinking.trim()) ? opts.thinking.trim() : undefined;
        ...
        if (hasSystemEventPatch) {
          patch.payload = { kind: "systemEvent", text: String(opts.systemEvent) };
        } else {
        ...
        if ((typeof opts.postPrefix === "string")) {
          patch.isolation = { postToMainPrefix: opts.postPrefix.trim() ? opts.postPrefix : "Cron" };
        }
        const res = await callGatewayFromCli("cron.update", opts, { id, patch });
