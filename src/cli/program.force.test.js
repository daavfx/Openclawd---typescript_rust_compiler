...
import { forceFreePort, forceFreePortAndWait, listPortListeners, parseLsofOutput } from "./ports.js";
...
");
    ...
    expect(parsed).toEqual([{ pid: 123, command: "node" }, { pid: 456, command: "python" }]);
  });
...
"));
    ...
    expect(killed).toEqual([{ pid: 42, command: "node" }, { pid: 99, command: "ssh" }]);
  });
...
");
    ...
    const killMock = vi.fn();
    process.kill = killMock;
    const promise = forceFreePortAndWait(18789, { timeoutMs: 500, intervalMs: 100, sigtermTimeoutMs: 400 });
...
");
    ...
    const killMock = vi.fn();
    process.kill = killMock;
    const promise = forceFreePortAndWait(18789, { timeoutMs: 800, intervalMs: 100, sigtermTimeoutMs: 300 });
    await vi.runAllTimersAsync();
    const res = await promise;
    expect(killMock).toHaveBeenCalledWith(42, "SIGTERM");
