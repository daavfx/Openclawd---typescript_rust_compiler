...
export 
const routeHealth = { match: (path) => (path[0] === "health"), loadPlugins: true, run: async (argv) => {
...
const routeStatus = { match: (path) => (path[0] === "status"), loadPlugins: true, run: async (argv) => {
  const json = hasFlag(argv, "--json");
...
const routeSessions = { match: (path) => (path[0] === "sessions"), run: async (argv) => {
  const json = hasFlag(argv, "--json");
...
const routeAgentsList = { match: (path) => ((path[0] === "agents") && (path[1] === "list")), run: async (argv) => {
  const json = hasFlag(argv, "--json");
  const bindings = hasFlag(argv, "--bindings");
  await agentsListCommand({ json, bindings }, defaultRuntime);
  return true;
} };
const routeMemoryStatus = { match: (path) => ((path[0] === "memory") && (path[1] === "status")), run: async (argv) => {
