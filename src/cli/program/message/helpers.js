...
export function createMessageCliHelpers(message, messageChannelOptions) {
  const withMessageBase = (command) => command.option("--channel <channel>", "Channel: ").option("--account <id>", "Channel account id (accountId)").option("--json", "Output result as JSON", false).option("--dry-run", "Print payload and skip sending", false).option("--verbose", "Verbose logging", false);
  ...
  const runMessageAction = async (action, opts) => {
    ...
    await runCommandWithRuntime(defaultRuntime, async () => {
      await messageCommand({ ...() => {
        ...
        return { ...rest: , accountId: (typeof account === "string") ? account : undefined };
      }(): , action }, deps, defaultRuntime);
    }, (err) => {
