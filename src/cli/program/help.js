...
export function configureProgramHelp(program, ctx) {
  ...
  program.configureHelp({ optionTerm: (option) => theme.option(option.flags), subcommandTerm: (cmd) => theme.command(cmd.name()) });
  program.configureOutput({ writeOut: (str) => {
  ...
  }, writeErr: (str) => process.stderr.write(str), outputError: (str, write) => write(theme.error(str)) });
...
";
  ...
  const fmtExamples = EXAMPLES.map(([cmd, desc]) => "  
    ").join("
");
