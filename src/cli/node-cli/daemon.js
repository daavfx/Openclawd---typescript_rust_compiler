...
function renderNodeServiceStartHints() {
  ...
  switch (process.platform) {
    case "darwin":
      return [...base, "launchctl bootstrap gui/$UID ~/Library/LaunchAgents/.plist"];
    case "linux":
      return [...base, "systemctl --user start .service"];
    case "win32":
      return [...base, "schtasks /Run /TN \"\""];
    default:
      return base;
  }
}
function buildNodeRuntimeHints(env = process.env) {
  if ((process.platform === "darwin")) {
    ...
    return ["Launchd stdout (if installed): ", "Launchd stderr (if installed): "];
  }
  if ((process.platform === "linux")) {
    const unit = resolveNodeSystemdServiceName();
    return ["Logs: journalctl --user -u .service -n 200 --no-pager"];
  }
  if ((process.platform === "win32")) {
    const task = resolveNodeWindowsTaskName();
    return ["Logs: schtasks /Query /TN \"\" /V /FO LIST"];
  }
  return [];
}
...
export async function runNodeDaemonInstall(opts) {
  ...
  try {
    ...
    {
      fail("Node service check failed: ");
      return;
    }
  }
  if ((loaded && !opts.force)) {
    emit({ ok: true, result: "already-installed", message: "Node service already .", service: buildDaemonServiceSnapshot(service, loaded), warnings: warnings.length ? warnings : undefined });
    if (!json) {
      defaultRuntime.log("Node service already .");
      defaultRuntime.log("Reinstall with: ");
    }
    return;
  }
  ...
  const {programArguments, workingDirectory, environment, description} = await buildNodeInstallPlan({ env: process.env, host, port: (port ?? 18789), tls, tlsFingerprint: (tlsFingerprint || undefined), nodeId: opts.nodeId, displayName: opts.displayName, runtime: runtimeRaw, warn: (message) => {
    if (json) {
  ...
  try {
    {
      await service.install({ env: process.env, stdout, programArguments, workingDirectory, environment, description });
    }
  }
  catch (err) {
    {
  ...
  emit({ ok: true, result: "installed", service: buildDaemonServiceSnapshot(service, installed), warnings: warnings.length ? warnings : undefined });
}
...
export async function runNodeDaemonStart(opts = {  }) {
  ...
  if (!loaded) {
    ...
    emit({ ok: true, result: "not-loaded", message: "Node service .", hints, service: buildDaemonServiceSnapshot(service, loaded) });
    if (!json) {
      defaultRuntime.log("Node service .");
      for (const hint of hints) {
...
export async function runNodeDaemonRestart(opts = {  }) {
  ...
  try {
    ...
    {
      const hints = renderNodeServiceStartHints();
      fail("Node restart failed: ", hints);
      return;
    }
  }
  let restarted = true;
...
export async function runNodeDaemonStop(opts = {  }) {
  ...
  if (!loaded) {
    emit({ ok: true, result: "not-loaded", message: "Node service .", service: buildDaemonServiceSnapshot(service, loaded) });
    if (!json) {
      defaultRuntime.log("Node service .");
    }
    return;
...
export async function runNodeDaemonStatus(opts = {  }) {
  ...
  const payload = { service: { ...buildDaemonServiceSnapshot(service, loaded): , command, runtime } };
  if (json) {
  ...
  const baseEnv = { ...process.env: , ...(command?.environment ?? undefined):  };
  const hintEnv = { ...baseEnv: , OPENCLAW_LOG_PREFIX: (baseEnv.OPENCLAW_LOG_PREFIX ?? "node") };
