  ...
  try {
    {
      ...
      const out = execFileSync(lsof, ["-nP", "-iTCP:", "-sTCP:LISTEN", "-FpFc"], { encoding: "utf-8" });
...
export function forceFreePort(port) {
  ...
  for (const proc of listeners) {
    try {
      ...
      {
        throw new Error("failed to kill pid : ");
      }
    }
  }
  return listeners;
...
export async function forceFreePortAndWait(port, opts = {  }) {
  ...
  throw new Error("port  still has listeners after --force: ");
}
