...
vi.mock("../agents/agent-scope.js", () => { resolveDefaultAgentId });
afterEach(async () => {
  vi.restoreAllMocks();
...
describe("memory cli", () => {
  it("prints vector status when available", async () => {
    ...
    getMemorySearchManager.mockResolvedValueOnce({ manager: { probeVectorAvailability: vi.fn(async () => true), status: () => { files: 2, chunks: 5, dirty: false, workspaceDir: "/tmp/openclaw", dbPath: "/tmp/memory.sqlite", provider: "openai", model: "text-embedding-3-small", requestedProvider: "openai", cache: { enabled: true, entries: 123, maxEntries: 50000 }, fts: { enabled: true, available: true }, vector: { enabled: true, available: true, extensionPath: "/opt/sqlite-vec.dylib", dims: 1024 } }, close } });
  ...
  it("prints vector error when unavailable", async () => {
    ...
    getMemorySearchManager.mockResolvedValueOnce({ manager: { probeVectorAvailability: vi.fn(async () => false), status: () => { files: 0, chunks: 0, dirty: true, workspaceDir: "/tmp/openclaw", dbPath: "/tmp/memory.sqlite", provider: "openai", model: "text-embedding-3-small", requestedProvider: "openai", vector: { enabled: true, available: false, loadError: "load failed" } }, close } });
    const log = vi.spyOn(defaultRuntime, "log").mockImplementation(() => {
    });
    const program = new Command();
    program.name("test");
  ...
  it("enables verbose logging with --verbose", async () => {
    ...
    getMemorySearchManager.mockResolvedValueOnce({ manager: { probeVectorAvailability: vi.fn(async () => true), status: () => { files: 0, chunks: 0, dirty: false, workspaceDir: "/tmp/openclaw", dbPath: "/tmp/memory.sqlite", provider: "openai", model: "text-embedding-3-small", requestedProvider: "openai", vector: { enabled: true, available: true } }, close } });
    const program = new Command();
    program.name("test");
    registerMemoryCli(program);
  ...
  it("logs close failure after status", async () => {
    ...
    getMemorySearchManager.mockResolvedValueOnce({ manager: { probeVectorAvailability: vi.fn(async () => true), status: () => { files: 1, chunks: 1, dirty: false, workspaceDir: "/tmp/openclaw", dbPath: "/tmp/memory.sqlite", provider: "openai", model: "text-embedding-3-small", requestedProvider: "openai" }, close } });
    ...
    await program.parseAsync(["memory", "status"], { from: "user" });
    expect(close).toHaveBeenCalled();
    expect(error).toHaveBeenCalledWith(expect.stringContaining("Memory manager close failed: close boom"));
  ...
  it("reindexes on status --index", async () => {
    ...
    expect(sync).toHaveBeenCalledWith(expect.objectContaining({ reason: "cli", progress: expect.any(Function) }));
    expect(probeEmbeddingAvailability).toHaveBeenCalled();
    expect(close).toHaveBeenCalled();
  });
  it("closes manager after index", async () => {
    ...
    getMemorySearchManager.mockResolvedValueOnce({ manager: { sync, close } });
    const log = vi.spyOn(defaultRuntime, "log").mockImplementation(() => {
    });
    const program = new Command();
    program.name("test");
    ...
    expect(sync).toHaveBeenCalledWith(expect.objectContaining({ reason: "cli", force: false, progress: expect.any(Function) }));
    expect(close).toHaveBeenCalled();
  ...
  it("logs close failure after search", async () => {
    ...
    const search = vi.fn(async () => [{ path: "memory/2026-01-12.md", startLine: 1, endLine: 2, score: 0.5, snippet: "Hello" }]);
    getMemorySearchManager.mockResolvedValueOnce({ manager: { search, close } });
    const error = vi.spyOn(defaultRuntime, "error").mockImplementation(() => {
    });
    const program = new Command();
    program.name("test");
