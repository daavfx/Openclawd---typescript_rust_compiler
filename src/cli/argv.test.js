...
import { buildParseArgv, getFlagValue, getCommandPath, getPrimaryCommand, getPositiveIntFlagValue, getVerboseFlag, hasHelpOrVersion, hasFlag, shouldMigrateState, shouldMigrateStateFromPath } from "./argv.js";
describe("argv helpers", () => {
  ...
  it("extracts flag values with equals and missing values", () => {
    expect(getFlagValue(["node", "openclaw", "status", "--timeout", "5000"], "--timeout")).toBe("5000");
    ...
    expect(getFlagValue(["node", "openclaw", "status", "--timeout", "--json"], "--timeout")).toBe(null);
  ...
  it("parses verbose flags", () => {
    ...
    expect(getVerboseFlag(["node", "openclaw", "status", "--debug"], { includeDebug: true })).toBe(true);
  });
  it("parses positive integer flag values", () => {
    expect(getPositiveIntFlagValue(["node", "openclaw", "status"], "--timeout")).toBeUndefined();
    expect(getPositiveIntFlagValue(["node", "openclaw", "status", "--timeout"], "--timeout")).toBeNull();
    expect(getPositiveIntFlagValue(["node", "openclaw", "status", "--timeout", "5000"], "--timeout")).toBe(5000);
    expect(getPositiveIntFlagValue(["node", "openclaw", "status", "--timeout", "nope"], "--timeout")).toBeUndefined();
  });
  it("builds parse argv from raw args", () => {
    const nodeArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["node", "openclaw", "status"] });
    expect(nodeArgv).toEqual(["node", "openclaw", "status"]);
    const versionedNodeArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["node-22", "openclaw", "status"] });
    expect(versionedNodeArgv).toEqual(["node-22", "openclaw", "status"]);
    const versionedNodeWindowsArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["node-22.2.0.exe", "openclaw", "status"] });
    expect(versionedNodeWindowsArgv).toEqual(["node-22.2.0.exe", "openclaw", "status"]);
    const versionedNodePatchlessArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["node-22.2", "openclaw", "status"] });
    expect(versionedNodePatchlessArgv).toEqual(["node-22.2", "openclaw", "status"]);
    const versionedNodeWindowsPatchlessArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["node-22.2.exe", "openclaw", "status"] });
    expect(versionedNodeWindowsPatchlessArgv).toEqual(["node-22.2.exe", "openclaw", "status"]);
    const versionedNodeWithPathArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["/usr/bin/node-22.2.0", "openclaw", "status"] });
    expect(versionedNodeWithPathArgv).toEqual(["/usr/bin/node-22.2.0", "openclaw", "status"]);
    const nodejsArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["nodejs", "openclaw", "status"] });
    expect(nodejsArgv).toEqual(["nodejs", "openclaw", "status"]);
    const nonVersionedNodeArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["node-dev", "openclaw", "status"] });
    expect(nonVersionedNodeArgv).toEqual(["node", "openclaw", "node-dev", "openclaw", "status"]);
    const directArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["openclaw", "status"] });
    expect(directArgv).toEqual(["node", "openclaw", "status"]);
    const bunArgv = buildParseArgv({ programName: "openclaw", rawArgs: ["bun", "src/entry.ts", "status"] });
    expect(bunArgv).toEqual(["bun", "src/entry.ts", "status"]);
  });
  it("builds parse argv from fallback args", () => {
    const fallbackArgv = buildParseArgv({ programName: "openclaw", fallbackArgv: ["status"] });
