...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180000 }, async () => {
    ...
    const env = { ...process.env: , OPENCLAW_NO_RESPAWN: "1", OPENCLAW_STATE_DIR: stateDir, OPENCLAW_SKIP_CHANNELS: "1", OPENCLAW_SKIP_GMAIL_WATCHER: "1", OPENCLAW_SKIP_CRON: "1", OPENCLAW_SKIP_BROWSER_CONTROL_SERVER: "1", OPENCLAW_SKIP_CANVAS_HOST: "1" };
    ...
    fs.writeFileSync(bootstrapPath, ["import { pathToFileURL } from \"node:url\";", "const runLoopUrl = ;", "const runtimeUrl = ;", "const { runGatewayLoop } = await import(runLoopUrl);", "const { defaultRuntime } = await import(runtimeUrl);", "await runGatewayLoop({", "  start: async () => {", "    process.stdout.write(\"READY\\\\n\");", "    if (process.send) process.send({ ready: true });", "    const keepAlive = setInterval(() => {}, 1000);", "    return { close: async () => clearInterval(keepAlive) };", "  },", "  runtime: defaultRuntime,", "});"].join("
"), "utf8");
    const childArgs = ["--import", "tsx", bootstrapPath];
    child = spawn(nodeBin, childArgs, { cwd: process.cwd(), env, stdio: ["ignore", "pipe", "pipe", "ipc"] });
    const proc = child;
    if (!proc) {
    ...
    if (((result.code !== 0) && !((result.code === null) && (result.signal === "SIGTERM")))) {
      ...
      throw new Error(("expected exit code 0, got code= signal=
" + "--- stdout ---
