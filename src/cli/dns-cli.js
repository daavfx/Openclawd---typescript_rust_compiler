...
function run(cmd, args, opts) {
  const res = spawnSync(cmd, args, { encoding: "utf-8", stdio: opts?.inherit ? "inherit" : "pipe" });
  if (res.error) {
    throw res.error;
  }
  if ((!opts?.allowFailure && (res.status !== 0))) {
    const errText = ((typeof res.stderr === "string") && res.stderr.trim()) ? res.stderr.trim() : "exit ";
...
function writeFileSudoIfNeeded(filePath, content) {
  ...
  const res = spawnSync("sudo", ["tee", filePath], { input: content, encoding: "utf-8", stdio: ["pipe", "ignore", "inherit"] });
  if (res.error) {
    throw res.error;
  }
  if ((res.status !== 0)) {
    throw new Error("sudo tee  failed: exit ");
  }
}
...
export function registerDnsCli(program) {
  const dns = program.command("dns").description("DNS helpers for wide-area discovery (Tailscale + CoreDNS)").addHelpText("after", () => "
 
");
  dns.command("setup").description("Set up CoreDNS to serve your discovery domain for unicast DNS-SD (Wide-Area Bonjour)").option("--domain <domain>", "Wide-area discovery domain (e.g. openclaw.internal)").option("--apply", "Install/update CoreDNS config and (re)start the service (requires sudo)", false).action(async (opts) => {
    ...
    defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Key", header: "Key", minWidth: 18 }, { key: "Value", header: "Value", minWidth: 24, flex: true }], rows: [{ Key: "Domain", Value: wideAreaDomain }, { Key: "Zone file", Value: zonePath }, { Key: "Tailnet IP", Value: "" }] }).trimEnd());
    defaultRuntime.log("");
    defaultRuntime.log(theme.heading("Recommended ~/.openclaw/openclaw.json:"));
    defaultRuntime.log(JSON.stringify({ gateway: { bind: "auto" }, discovery: { wideArea: { enabled: true, domain: wideAreaDomain } } }, null, 2));
    defaultRuntime.log("");
    defaultRuntime.log(theme.heading("Tailscale admin (DNS → Nameservers):"));
    defaultRuntime.log(theme.muted("- Add nameserver: "));
    defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): "));
    if (!opts.apply) {
      defaultRuntime.log("");
...
");
    ...
    if (zoneFileNeedsBootstrap(zonePath)) {
      ...
      const serial = "01";
      const zoneLines = ["; created by openclaw dns setup (will be overwritten by the gateway when wide-area discovery is enabled)", "$ORIGIN ", "$TTL 60", "@ IN SOA ns1 hostmaster  7200 3600 1209600 60", "@ IN NS ns1", tailnetIPv4 ? "ns1 IN A " : null, tailnetIPv6 ? "ns1 IN AAAA " : null, ""].filter((line) => Boolean(line));
