import { colorize, theme } from "../../terminal/theme.js";
export 
export function parseDiscoverTimeoutMs(raw, fallbackMs) {
  if (((raw === undefined) || (raw === null))) {
    return fallbackMs;
  }
  const value = (typeof raw === "string") ? raw.trim() : ((typeof raw === "number") || (typeof raw === "bigint")) ? String(raw) : null;
  if ((value === null)) {
    throw new Error("invalid --timeout");
  }
  if (!value) {
    return fallbackMs;
  }
  const parsed = Number.parseInt(value, 10);
  if ((!Number.isFinite(parsed) || (parsed <= 0))) {
    throw new Error("invalid --timeout: ");
  }
  return parsed;
}

export function pickBeaconHost(beacon) {
  const host = ((beacon.tailnetDns || beacon.lanHost) || beacon.host);
  return host?.trim() ? host.trim() : null;
}

export function pickGatewayPort(beacon) {
  const port = (beacon.gatewayPort ?? 18789);
  return (port > 0) ? port : 18789;
}

export function dedupeBeacons(beacons) {
  const out = [];
  const seen = new Set();
  for (const b of beacons) {
    const host = (pickBeaconHost(b) ?? "");
    const key = [(b.domain ?? ""), (b.instanceName ?? ""), (b.displayName ?? ""), host, String((b.port ?? "")), String((b.gatewayPort ?? ""))].join("|");
    if (seen.has(key)) {
      continue;
    }
    seen.add(key);
    out.push(b);
  }
  return out;
}

export function renderBeaconLines(beacon, rich) {
  const nameRaw = ((beacon.displayName || beacon.instanceName) || "Gateway").trim();
  const domainRaw = (beacon.domain || "local.").trim();
  const title = colorize(rich, theme.accentBright, nameRaw);
  const domain = colorize(rich, theme.muted, domainRaw);
  const host = pickBeaconHost(beacon);
  const gatewayPort = pickGatewayPort(beacon);
  const scheme = beacon.gatewayTls ? "wss" : "ws";
  const wsUrl = host ? "://:" : null;
  const lines = ["-  "];
  if (beacon.tailnetDns) {
    lines.push("  : ");
  }
  if (beacon.lanHost) {
    lines.push("  : ");
  }
  if (beacon.host) {
    lines.push("  : ");
  }
  if (wsUrl) {
    lines.push("  : ");
  }
  if (beacon.role) {
    lines.push("  : ");
  }
  if (beacon.transport) {
    lines.push("  : ");
  }
  if (beacon.gatewayTls) {
    const fingerprint = beacon.gatewayTlsFingerprintSha256 ? "sha256 " : "enabled";
    lines.push("  : ");
  }
  if ((((typeof beacon.sshPort === "number") && (beacon.sshPort > 0)) && host)) {
    const ssh = "ssh -N -L 18789:127.0.0.1:18789 <user>@ -p ";
    lines.push("  : ");
  }
  return lines;
}

