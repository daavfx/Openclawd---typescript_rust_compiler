...
async function ensureDevWorkspace(dir) {
  ...
  const [agents, soul, tools, identity, user] = await Promise.all([loadDevTemplate("AGENTS.dev.md", "# AGENTS.md - OpenClaw Dev Workspace
...
export async function ensureDevGatewayConfig(opts) {
  ...
  await writeConfigFile({ gateway: { mode: "local", bind: "loopback" }, agents: { defaults: { workspace, skipBootstrap: true }, list: [{ id: "dev", default: true, workspace, identity: { name: DEV_IDENTITY_NAME, theme: DEV_IDENTITY_THEME, emoji: DEV_IDENTITY_EMOJI } }] } });
  await ensureDevWorkspace(workspace);
  defaultRuntime.log("Dev config ready: ");
  defaultRuntime.log("Dev workspace ready: ");
}
