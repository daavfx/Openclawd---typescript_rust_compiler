...
async function runGatewayCommand(opts) {
  ...
  if (opts.force) {
    try {
      {
        const {killed, waitedMs, escalatedToSigkill} = await forceFreePortAndWait(port, { timeoutMs: 2000, intervalMs: 100, sigtermTimeoutMs: 700 });
        if ((killed.length === 0)) {
          gatewayLog.info("force: no listeners on port ");
        } else {
          for (const proc of killed) {
            gatewayLog.info("force: killed pid  on port ");
          }
          if (escalatedToSigkill) {
            gatewayLog.info("force: escalated to SIGKILL while freeing port ");
          }
          if ((waitedMs > 0)) {
            gatewayLog.info("force: waited ms for port  to free");
          }
        }
      }
    }
  ...
  if ((!opts.allowUnconfigured && (mode !== "local"))) {
    if (!configExists) {
      defaultRuntime.error("Missing config. Run `` or set gateway.mode=local (or pass --allow-unconfigured).");
    } else {
      defaultRuntime.error("Gateway start blocked: set gateway.mode=local (current: ) or pass --allow-unconfigured.");
    }
    defaultRuntime.exit(1);
    return;
  }
  ...
  const authConfig = { ...cfg.gateway?.auth: , ...authMode ? { mode: authMode } : {  }: , ...passwordRaw ? { password: passwordRaw } : {  }: , ...tokenRaw ? { token: tokenRaw } : {  }:  };
  const resolvedAuth = resolveGatewayAuth({ authConfig, env: process.env, tailscaleMode: ((tailscaleMode ?? cfg.gateway?.tailscale?.mode) ?? "off") });
  ...
  if ((((resolvedAuthMode === "token") && !hasToken) && !resolvedAuth.allowTailscale)) {
    defaultRuntime.error(["Gateway auth is set to token, but no token is configured.", "Set gateway.auth.token (or OPENCLAW_GATEWAY_TOKEN), or pass --token.", ...authHints].filter(Boolean).join("
"));
    defaultRuntime.exit(1);
    return;
  }
  if (((resolvedAuthMode === "password") && !hasPassword)) {
    defaultRuntime.error(["Gateway auth is set to password, but no password is configured.", "Set gateway.auth.password (or OPENCLAW_GATEWAY_PASSWORD), or pass --password.", ...authHints].filter(Boolean).join("
"));
    defaultRuntime.exit(1);
    return;
  }
  if (((bind !== "loopback") && !hasSharedSecret)) {
    defaultRuntime.error(["Refusing to bind gateway to  without auth.", "Set gateway.auth.token/password (or OPENCLAW_GATEWAY_TOKEN/OPENCLAW_GATEWAY_PASSWORD) or pass --token/--password.", ...authHints].filter(Boolean).join("
"));
    defaultRuntime.exit(1);
    return;
  }
  try {
    {
      await runGatewayLoop({ runtime: defaultRuntime, start: async () => await startGatewayServer(port, { bind, auth: (((authMode || passwordRaw) || tokenRaw) || authModeRaw) ? { mode: (authMode ?? undefined), token: tokenRaw, password: passwordRaw } : undefined, tailscale: (tailscaleMode || opts.tailscaleResetOnExit) ? { mode: (tailscaleMode ?? undefined), resetOnExit: Boolean(opts.tailscaleResetOnExit) } : undefined }) });
    }
  }
  catch (err) {
    {
      if (((err instanceof GatewayLockError) || ((err && (typeof err === "object")) && (err.name === "GatewayLockError")))) {
        ...
        defaultRuntime.error("Gateway failed to start: 
If the gateway is supervised, stop it with: ");
        try {
          {
