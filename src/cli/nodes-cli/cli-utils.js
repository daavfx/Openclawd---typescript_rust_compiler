...
export function getNodesTheme() {
  ...
  return { rich, heading: color(theme.heading), ok: color(theme.success), warn: color(theme.warn), muted: color(theme.muted), error: color(theme.error) };
}
...
export function runNodesCommand(label, action) {
  return runCommandWithRuntime(defaultRuntime, action, (err) => {
    ...
    defaultRuntime.error(error("nodes  failed: "));
