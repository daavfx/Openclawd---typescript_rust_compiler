...
const parseFacing = (value) => {
  ...
  throw new Error("invalid facing:  (expected front|back)");
};
export function registerNodesCameraCommands(nodes) {
  ...
  nodesCallOpts(camera.command("list").description("List available cameras on a node").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").action(async (opts) => {
    await runNodesCommand("camera list", async () => {
      ...
      const raw = await callGatewayCli("node.invoke", opts, { nodeId, command: "camera.list", params: {  }, idempotencyKey: randomIdempotencyKey() });
      ...
      const rows = devices.map((device) => { Name: (typeof device.name === "string") ? device.name : "Unknown Camera", Position: (typeof device.position === "string") ? device.position : muted("unspecified"), ID: (typeof device.id === "string") ? device.id : "" });
      defaultRuntime.log(heading("Cameras"));
      defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Name", header: "Name", minWidth: 14, flex: true }, { key: "Position", header: "Position", minWidth: 10 }, { key: "ID", header: "ID", minWidth: 10, flex: true }], rows }).trimEnd());
    });
  }), { timeoutMs: 60000 });
  nodesCallOpts(camera.command("snap").description("Capture a photo from a node camera (prints MEDIA:<path>)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--facing <front|back|both>", "Camera facing", "both").option("--device-id <id>", "Camera device id (from nodes camera list)").option("--max-width <px>", "Max width in px (optional)").option("--quality <0-1>", "JPEG quality (default 0.9)").option("--delay-ms <ms>", "Delay before capture in ms (macOS default 2000)").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 20000)", "20000").action(async (opts) => {
    await runNodesCommand("camera snap", async () => {
      ...
      const facings = (facingOpt === "both") ? ["front", "back"] : ((facingOpt === "front") || (facingOpt === "back")) ? [facingOpt] : () => {
        throw new Error("invalid facing:  (expected front|back|both)");
      }();
      ...
      for (const facing of facings) {
        const invokeParams = { nodeId, command: "camera.snap", params: { facing, maxWidth: Number.isFinite(maxWidth) ? maxWidth : undefined, quality: Number.isFinite(quality) ? quality : undefined, format: "jpg", delayMs: Number.isFinite(delayMs) ? delayMs : undefined, deviceId: (deviceId || undefined) }, idempotencyKey: randomIdempotencyKey() };
        ...
        const filePath = cameraTempPath({ kind: "snap", facing, ext: (payload.format === "jpeg") ? "jpg" : payload.format });
        await writeBase64ToFile(filePath, payload.base64);
        results.push({ facing, path: filePath, width: payload.width, height: payload.height });
      }
      if (opts.json) {
        defaultRuntime.log(JSON.stringify({ files: results }, null, 2));
        return;
      }
      defaultRuntime.log(results.map((r) => "MEDIA:").join("
"));
    });
  }), { timeoutMs: 60000 });
  nodesCallOpts(camera.command("clip").description("Capture a short video clip from a node camera (prints MEDIA:<path>)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").option("--facing <front|back>", "Camera facing", "front").option("--device-id <id>", "Camera device id (from nodes camera list)").option("--duration <ms|10s|1m>", "Duration (default 3000ms; supports ms/s/m, e.g. 10s)", "3000").option("--no-audio", "Disable audio capture").option("--invoke-timeout <ms>", "Node invoke timeout in ms (default 90000)", "90000").action(async (opts) => {
    await runNodesCommand("camera clip", async () => {
      ...
      const timeoutMs = opts.invokeTimeout ? Number.parseInt(String(opts.invokeTimeout), 10) : undefined;
      const deviceId = opts.deviceId ? String(opts.deviceId).trim() : undefined;
      const invokeParams = { nodeId, command: "camera.clip", params: { facing, durationMs: Number.isFinite(durationMs) ? durationMs : undefined, includeAudio, format: "mp4", deviceId: (deviceId || undefined) }, idempotencyKey: randomIdempotencyKey() };
      if (((typeof timeoutMs === "number") && Number.isFinite(timeoutMs))) {
        invokeParams.timeoutMs = timeoutMs;
      }
      const raw = await callGatewayCli("node.invoke", opts, invokeParams);
      ...
      await writeBase64ToFile(filePath, payload.base64);
      if (opts.json) {
        defaultRuntime.log(JSON.stringify({ file: { facing, path: filePath, durationMs: payload.durationMs, hasAudio: payload.hasAudio } }, null, 2));
        return;
      }
      defaultRuntime.log("MEDIA:");
    });
  }), { timeoutMs: 90000 });
}
