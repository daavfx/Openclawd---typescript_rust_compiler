...
export function formatPermissions(raw) {
  ...
  const entries = Object.entries(raw).map(([key, value]) => [String(key).trim(), (value === true)]).filter(([key]) => (key.length > 0)).sort((a, b) => a[0].localeCompare(b[0]));
  if ((entries.length === 0)) {
    return null;
  }
  const parts = entries.map(([key, granted]) => "=");
  return "[]";
}
