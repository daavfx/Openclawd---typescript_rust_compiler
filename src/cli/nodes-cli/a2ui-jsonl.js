const A2UI_ACTION_KEYS = ["beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface", "createSurface"];
export 
export function buildA2UITextJsonl(text) {
  const surfaceId = "main";
  const rootId = "root";
  const textId = "text";
  const payloads = [{ surfaceUpdate: { surfaceId, components: [{ id: rootId, component: { Column: { children: { explicitList: [textId] } } } }, { id: textId, component: { Text: { text: { literalString: text }, usageHint: "body" } } }] } }, { beginRendering: { surfaceId, root: rootId } }];
...
export function validateA2UIJsonl(jsonl) {
  ...
  lines.forEach((line, idx) => {
    ...
    if (((!obj || (typeof obj !== "object")) || Array.isArray(obj))) {
      errors.push("line : expected JSON object");
      return;
    }
    const record = obj;
    const actionKeys = A2UI_ACTION_KEYS.filter((key) => (key in record));
    if ((actionKeys.length !== 1)) {
      errors.push("line : expected exactly one action key ()");
      return;
    }
