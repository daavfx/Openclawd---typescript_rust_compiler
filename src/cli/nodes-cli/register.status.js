...
function formatVersionLabel(raw) {
  ...
  return /^\d/.test(trimmed) ? "v" : trimmed;
}
...
function formatPathEnv(raw) {
  ...
  const display = (parts.length <= 3) ? trimmed : ":…:";
...
function parseSinceMs(raw, label) {
  ...
  if ((value === null)) {
    defaultRuntime.error(": invalid duration value");
...
export function registerNodesStatusCommands(nodes) {
  nodesCallOpts(nodes.command("status").description("List known nodes with connection status and capabilities").option("--connected", "Only show connected nodes").option("--last-connected <duration>", "Only show nodes connected within duration (e.g. 24h)").action(async (opts) => {
    await runNodesCommand("status", async () => {
      ...
      const lastConnectedById = (sinceMs !== undefined) ? new Map(parsePairingList(await callGatewayCli("node.pair.list", opts, {  })).paired.map((entry) => [entry.nodeId, entry])) : null;
      const filtered = nodes.filter((n) => {
        ...
        if ((sinceMs !== undefined)) {
          ...
          const lastConnectedAtMs = (typeof paired?.lastConnectedAtMs === "number") ? paired.lastConnectedAtMs : (typeof n.connectedAtMs === "number") ? n.connectedAtMs : undefined;
      ...
      const filteredLabel = (filtered.length !== nodes.length) ? " (of )" : "";
      defaultRuntime.log("Known:  · Paired:  · Connected: ");
      ...
      const rows = filtered.map((n) => {
        ...
        const detailParts = [n.deviceFamily ? "device: " : null, n.modelIdentifier ? "hw: " : null, perms ? "perms: " : null, versions, pathEnv ? "path: " : null].filter(Boolean);
        const caps = Array.isArray(n.caps) ? n.caps.map(String).filter(Boolean).sort().join(", ") : "?";
        const paired = n.paired ? ok("paired") : warn("unpaired");
        const connected = n.connected ? ok("connected") : muted("disconnected");
        const since = (typeof n.connectedAtMs === "number") ? " ( ago)" : "";
        return { Node: name, ID: n.nodeId, IP: (n.remoteIp ?? ""), Detail: detailParts.join(" · "), Status: " · ", Caps: caps };
      });
      defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Node", header: "Node", minWidth: 14, flex: true }, { key: "ID", header: "ID", minWidth: 10 }, { key: "IP", header: "IP", minWidth: 10 }, { key: "Detail", header: "Detail", minWidth: 18, flex: true }, { key: "Status", header: "Status", minWidth: 18 }, { key: "Caps", header: "Caps", minWidth: 12, flex: true }], rows }).trimEnd());
    });
  }));
  nodesCallOpts(nodes.command("describe").description("Describe a node (capabilities + supported invoke commands)").requiredOption("--node <idOrNameOrIp>", "Node id, name, or IP").action(async (opts) => {
    await runNodesCommand("describe", async () => {
      ...
      const commands = Array.isArray(obj.commands) ? obj.commands.map(String).filter(Boolean).sort() : [];
      ...
      const status = " · ";
      const tableWidth = Math.max(60, ((process.stdout.columns ?? 120) - 1));
      const rows = [{ Field: "ID", Value: nodeId }, displayName ? { Field: "Name", Value: displayName } : null, ip ? { Field: "IP", Value: ip } : null, family ? { Field: "Device", Value: family } : null, model ? { Field: "Model", Value: model } : null, perms ? { Field: "Perms", Value: perms } : null, versions ? { Field: "Version", Value: versions } : null, pathEnv ? { Field: "PATH", Value: pathEnv } : null, { Field: "Status", Value: status }, { Field: "Caps", Value: caps ? caps.join(", ") : "?" }].filter(Boolean);
      defaultRuntime.log(heading("Node"));
      defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Field", header: "Field", minWidth: 8 }, { key: "Value", header: "Value", minWidth: 24, flex: true }], rows }).trimEnd());
      defaultRuntime.log("");
  ...
  nodesCallOpts(nodes.command("list").description("List pending and paired nodes").option("--connected", "Only show connected nodes").option("--last-connected <duration>", "Only show nodes connected within duration (e.g. 24h)").action(async (opts) => {
    await runNodesCommand("list", async () => {
      ...
      const filteredPaired = paired.filter((node) => {
        ...
        if ((sinceMs !== undefined)) {
          const live = connectedById?.get(node.nodeId);
          const lastConnectedAtMs = (typeof node.lastConnectedAtMs === "number") ? node.lastConnectedAtMs : (typeof live?.connectedAtMs === "number") ? live.connectedAtMs : undefined;
          if ((typeof lastConnectedAtMs !== "number")) {
            return false;
          }
          if (((now - lastConnectedAtMs) > sinceMs)) {
      ...
      const filteredLabel = (hasFilters && (filteredPaired.length !== paired.length)) ? " (of )" : "";
      defaultRuntime.log("Pending:  · Paired: ");
      if (opts.json) {
        defaultRuntime.log(JSON.stringify({ pending: pendingRows, paired: filteredPaired }, null, 2));
        return;
      }
      if ((pendingRows.length > 0)) {
        const pendingRowsRendered = pendingRows.map((r) => { Request: r.requestId, Node: r.displayName?.trim() ? r.displayName.trim() : r.nodeId, IP: (r.remoteIp ?? ""), Requested: (typeof r.ts === "number") ? " ago" : muted("unknown"), Repair: r.isRepair ? warn("yes") : "" });
        defaultRuntime.log("");
        defaultRuntime.log(heading("Pending"));
        defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Request", header: "Request", minWidth: 8 }, { key: "Node", header: "Node", minWidth: 14, flex: true }, { key: "IP", header: "IP", minWidth: 10 }, { key: "Requested", header: "Requested", minWidth: 12 }, { key: "Repair", header: "Repair", minWidth: 6 }], rows: pendingRowsRendered }).trimEnd());
      }
      if ((filteredPaired.length > 0)) {
        const pairedRows = filteredPaired.map((n) => {
          const live = connectedById?.get(n.nodeId);
          const lastConnectedAtMs = (typeof n.lastConnectedAtMs === "number") ? n.lastConnectedAtMs : (typeof live?.connectedAtMs === "number") ? live.connectedAtMs : undefined;
          return { Node: n.displayName?.trim() ? n.displayName.trim() : n.nodeId, Id: n.nodeId, IP: (n.remoteIp ?? ""), LastConnect: (typeof lastConnectedAtMs === "number") ? " ago" : muted("unknown") };
        });
        defaultRuntime.log("");
        defaultRuntime.log(heading("Paired"));
        defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Node", header: "Node", minWidth: 14, flex: true }, { key: "Id", header: "ID", minWidth: 10 }, { key: "IP", header: "IP", minWidth: 10 }, { key: "LastConnect", header: "Last Connect", minWidth: 14 }], rows: pairedRows }).trimEnd());
      }
    });
  }));
}
