...
export function registerBrowserExtensionCommands(browser, parentOpts) {
  ...
  ext.command("install").description("Install the Chrome extension to a stable local path").action(async (_opts, cmd) => {
    ...
    defaultRuntime.error(info([copied ? "Copied to clipboard." : "Copy to clipboard unavailable.", "Next:", "- Chrome → chrome://extensions → enable “Developer mode”", "- “Load unpacked” → select: ", "- Pin “OpenClaw Browser Relay”, then click it on the tab (badge shows ON)", "", " "].join("
")));
  });
  ext.command("path").description("Print the path to the installed Chrome extension (load unpacked)").action(async (_opts, cmd) => {
    ...
    if (!hasManifest(dir)) {
      defaultRuntime.error(danger(["Chrome extension is not installed. Run: \"\"", "Docs: "].join("
")));
      defaultRuntime.exit(1);
    }
    if (parent?.json) {
