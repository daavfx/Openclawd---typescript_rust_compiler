...
const resolveGatewayProgramArguments = vi.fn(async () => { programArguments: ["/bin/node", "cli", "gateway", "--port", "18789"] });
...
const inspectPortUsage = vi.fn(async (port) => { port, status: "free", listeners: [], hints: [] });
const runtimeLogs = [];
const runtimeErrors = [];
const defaultRuntime = { log: (msg) => runtimeLogs.push(msg), error: (msg) => runtimeErrors.push(msg), exit: (code) => {
  throw new Error("__exit__:");
} };
vi.mock("../gateway/call.js", () => { callGateway: (opts) => callGateway(opts) });
vi.mock("../daemon/program-args.js", () => { resolveGatewayProgramArguments: (opts) => resolveGatewayProgramArguments(opts) });
vi.mock("../daemon/service.js", () => { resolveGatewayService: () => { label: "LaunchAgent", loadedText: "loaded", notLoadedText: "not loaded", install: serviceInstall, uninstall: serviceUninstall, stop: serviceStop, restart: serviceRestart, isLoaded: serviceIsLoaded, readCommand: serviceReadCommand, readRuntime: serviceReadRuntime } });
vi.mock("../daemon/legacy.js", () => { findLegacyGatewayServices: async () => [] });
vi.mock("../daemon/inspect.js", () => { findExtraGatewayServices: (env, opts) => findExtraGatewayServices(env, opts), renderGatewayServiceCleanupHints: () => [] });
vi.mock("../infra/ports.js", () => { inspectPortUsage: (port) => inspectPortUsage(port), formatPortDiagnostics: () => ["Port 18789 is already in use."] });
vi.mock("../runtime.js", () => { defaultRuntime });
vi.mock("./deps.js", () => { createDefaultDeps: () => {
} });
vi.mock("./progress.js", () => { withProgress: async (_opts, fn) => await fn() });
describe("daemon-cli coverage", () => {
  const originalEnv = { OPENCLAW_STATE_DIR: process.env.OPENCLAW_STATE_DIR, OPENCLAW_CONFIG_PATH: process.env.OPENCLAW_CONFIG_PATH, OPENCLAW_GATEWAY_PORT: process.env.OPENCLAW_GATEWAY_PORT, OPENCLAW_PROFILE: process.env.OPENCLAW_PROFILE };
  beforeEach(() => {
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({ programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"], environment: { OPENCLAW_PROFILE: "dev", OPENCLAW_STATE_DIR: "/tmp/openclaw-daemon-state", OPENCLAW_CONFIG_PATH: "/tmp/openclaw-daemon-state/openclaw.json", OPENCLAW_GATEWAY_PORT: "19001" }, sourcePath: "/tmp/bot.molt.gateway.plist" });
    const {registerDaemonCli} = await import("./daemon-cli.js");
    const program = new Command();
    program.exitOverride();
    registerDaemonCli(program);
    await program.parseAsync(["daemon", "status", "--json"], { from: "user" });
    expect(callGateway).toHaveBeenCalledWith(expect.objectContaining({ url: "ws://127.0.0.1:19001", method: "status" }));
  ...
  it("installs the daemon when requested", async () => {
    ...
    program.exitOverride();
    registerDaemonCli(program);
    await program.parseAsync(["daemon", "install", "--port", "18789"], { from: "user" });
  ...
  it("installs the daemon with json output", async () => {
    ...
    program.exitOverride();
    registerDaemonCli(program);
    await program.parseAsync(["daemon", "install", "--port", "18789", "--json"], { from: "user" });
    const jsonLine = runtimeLogs.find((line) => line.trim().startsWith("{"));
    const parsed = JSON.parse((jsonLine ?? "{}"));
