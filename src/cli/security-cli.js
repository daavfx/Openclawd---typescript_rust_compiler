...
function formatSummary(summary) {
  ...
  const parts = [];
  parts.push(rich ? theme.error(" critical") : " critical");
  parts.push(rich ? theme.warn(" warn") : " warn");
  parts.push(rich ? theme.muted(" info") : " info");
  return parts.join(" · ");
}
export function registerSecurityCli(program) {
  const security = program.command("security").description("Security tools (audit)").addHelpText("after", () => "
...
");
  security.command("audit").description("Audit config + local state for common security foot-guns").option("--deep", "Attempt live Gateway probe (best-effort)", false).option("--fix", "Apply safe fixes (tighten defaults + chmod state/config)", false).option("--json", "Print JSON", false).action(async (opts) => {
    ...
    const report = await runSecurityAudit({ config: cfg, deep: Boolean(opts.deep), includeFilesystem: true, includeChannelSecurity: true });
    if (opts.json) {
    ...
    lines.push(muted("Run deeper: "));
    if (opts.fix) {
      lines.push(muted("Fix: "));
      if (!fixResult) {
        lines.push(muted("Fixes: failed to apply (unexpected error)"));
      } else {
        if ((((fixResult.errors.length === 0) && (fixResult.changes.length === 0)) && fixResult.actions.every((a) => (a.ok === false)))) {
          ...
          for (const action of fixResult.actions) {
            if ((action.kind === "chmod")) {
              ...
              if (action.ok) {
                lines.push(muted("  chmod  "));
              } else {
                if (action.skipped) {
                  lines.push(muted("  skip chmod   ()"));
                } else {
                  if (action.error) {
                    lines.push(muted("  chmod   failed: "));
                  }
                }
              }
              continue;
            ...
            if (action.ok) {
              ...
              if (action.skipped) {
                lines.push(muted("  skip  ()"));
              } else {
                if (action.error) {
                  lines.push(muted("   failed: "));
                }
              }
            }
          }
