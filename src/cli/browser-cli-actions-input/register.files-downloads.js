...
export function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
    ...
    try {
      {
        ...
        const result = await callBrowserRequest(parent, { method: "POST", path: "/hooks/file-chooser", query: profile ? { profile } : undefined, body: { paths, ref: (opts.ref?.trim() || undefined), inputRef: (opts.inputRef?.trim() || undefined), element: (opts.element?.trim() || undefined), targetId: (opts.targetId?.trim() || undefined), timeoutMs } }, { timeoutMs: (timeoutMs ?? 20000) });
        if (parent?.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
        defaultRuntime.log("upload armed for  file(s)");
      }
    }
    catch (err) {
      {
  ...
  browser.command("waitfordownload").description("Wait for the next download (and save it)").argument("[path]", "Save path (default: /tmp/openclaw/downloads/...)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next download (default: 120000)", (v) => Number(v)).action(async (outPath, opts, cmd) => {
    ...
    try {
      {
        const timeoutMs = Number.isFinite(opts.timeoutMs) ? opts.timeoutMs : undefined;
        const result = await callBrowserRequest(parent, { method: "POST", path: "/wait/download", query: profile ? { profile } : undefined, body: { path: (outPath?.trim() || undefined), targetId: (opts.targetId?.trim() || undefined), timeoutMs } }, { timeoutMs: (timeoutMs ?? 20000) });
        if (parent?.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
        defaultRuntime.log("downloaded: ");
      }
    }
    catch (err) {
      {
  ...
  browser.command("download").description("Click a ref and save the resulting download").argument("<ref>", "Ref id from snapshot to click").argument("<path>", "Save path").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the download to start (default: 120000)", (v) => Number(v)).action(async (ref, outPath, opts, cmd) => {
    ...
    try {
      {
        const timeoutMs = Number.isFinite(opts.timeoutMs) ? opts.timeoutMs : undefined;
        const result = await callBrowserRequest(parent, { method: "POST", path: "/download", query: profile ? { profile } : undefined, body: { ref, path: outPath, targetId: (opts.targetId?.trim() || undefined), timeoutMs } }, { timeoutMs: (timeoutMs ?? 20000) });
        if (parent?.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
  ...
  browser.command("dialog").description("Arm the next modal dialog (alert/confirm/prompt)").option("--accept", "Accept the dialog", false).option("--dismiss", "Dismiss the dialog", false).option("--prompt <text>", "Prompt response text").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next dialog (default: 120000)", (v) => Number(v)).action(async (opts, cmd) => {
    ...
    try {
      {
        const timeoutMs = Number.isFinite(opts.timeoutMs) ? opts.timeoutMs : undefined;
        const result = await callBrowserRequest(parent, { method: "POST", path: "/hooks/dialog", query: profile ? { profile } : undefined, body: { accept, promptText: (opts.prompt?.trim() || undefined), targetId: (opts.targetId?.trim() || undefined), timeoutMs } }, { timeoutMs: (timeoutMs ?? 20000) });
        if (parent?.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
