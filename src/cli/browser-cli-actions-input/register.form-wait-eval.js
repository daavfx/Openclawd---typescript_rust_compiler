...
export function registerBrowserFormWaitEvalCommands(browser, parentOpts) {
  browser.command("fill").description("Fill a form with JSON field descriptors").option("--fields <json>", "JSON array of field objects").option("--fields-file <path>", "Read JSON array from a file").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (opts, cmd) => {
    ...
    try {
      {
        const fields = await readFields({ fields: opts.fields, fieldsFile: opts.fieldsFile });
        const result = await callBrowserAct({ parent, profile, body: { kind: "fill", fields, targetId: (opts.targetId?.trim() || undefined) } });
        if (parent?.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
        defaultRuntime.log("filled  field(s)");
      }
    }
    catch (err) {
      {
  ...
  browser.command("evaluate").description("Evaluate a function against the page or a ref").option("--fn <code>", "Function source, e.g. (el) => el.textContent").option("--ref <id>", "Ref from snapshot").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (opts, cmd) => {
    ...
    try {
      {
        const result = await callBrowserAct({ parent, profile, body: { kind: "evaluate", fn: opts.fn, ref: (opts.ref?.trim() || undefined), targetId: (opts.targetId?.trim() || undefined) } });
        if (parent?.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
