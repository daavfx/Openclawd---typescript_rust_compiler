...
export async function callBrowserAct(params) {
  return await callBrowserRequest(params.parent, { method: "POST", path: "/act", query: params.profile ? { profile: params.profile } : undefined, body: params.body }, { timeoutMs: (params.timeoutMs ?? 20000) });
}
...
export async function readFields(opts) {
  ...
  return parsed.map((entry, index) => {
    ...
    if ((!ref || !type)) {
      throw new Error("fields[] must include ref and type");
    }
    ...
    throw new Error("fields[].value must be string, number, boolean, or null");
  });
}
