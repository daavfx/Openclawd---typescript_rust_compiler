...
function parsePath(raw) {
  ...
  while ((i < trimmed.length)) {
    ...
    if ((ch === "[")) {
      ...
      if ((close === -1)) {
        throw new Error("Invalid path (missing \"]\"): ");
      }
...
function parseValue(raw, opts) {
  ...
  if (opts.json) {
    try {
      ...
      {
        throw new Error("Failed to parse JSON5 value: ");
      }
    }
  }
  try {
...
function setAtPath(root, path, value) {
  ...
  for (let i = 0; (i < (path.length - 1)); i += 1) {
    ...
    if (Array.isArray(current)) {
      if (!isIndexSegment(segment)) {
        throw new Error("Expected numeric index for array segment \"\"");
      }
      const index = Number.parseInt(segment, 10);
    ...
    if ((!current || (typeof current !== "object"))) {
      throw new Error("Cannot traverse into \"\" (not an object)");
    }
    const record = current;
  ...
  if ((!current || (typeof current !== "object"))) {
    throw new Error("Cannot set \"\" (parent is not an object)");
  }
  current[last] = value;
}
...
async function loadValidConfig() {
  ...
  defaultRuntime.error("Config invalid at .");
  for (const issue of snapshot.issues) {
    defaultRuntime.error("- : ");
  }
  defaultRuntime.error("Run `` to repair, then retry.");
...
").option("--section <section>", "Configure wizard sections (repeatable). Use with no subcommand.", (value, previous) => [...previous, value], []).action(async (opts) => {
    ...
    if ((invalid.length > 0)) {
      defaultRuntime.error("Invalid --section: . Expected one of: .");
      defaultRuntime.exit(1);
      return;
    }
    ...
    try {
      {
        ...
        if (!res.found) {
          defaultRuntime.error(danger("Config path not found: "));
          defaultRuntime.exit(1);
          return;
        }
        if (opts.json) {
    ...
    try {
      {
        ...
        defaultRuntime.log(info("Updated . Restart the gateway to apply."));
      }
    }
    catch (err) {
      {
    ...
    try {
      {
        ...
        await writeConfigFile(next);
        defaultRuntime.log(info("Removed . Restart the gateway to apply."));
      }
    }
    catch (err) {
      {
