...
describe("nodes camera helpers", () => {
  it("parses camera.snap payload", () => {
    expect(parseCameraSnapPayload({ format: "jpg", base64: "aGk=", width: 10, height: 20 })).toEqual({ format: "jpg", base64: "aGk=", width: 10, height: 20 });
  });
  ...
  it("parses camera.clip payload", () => {
    expect(parseCameraClipPayload({ format: "mp4", base64: "AAEC", durationMs: 1234, hasAudio: true })).toEqual({ format: "mp4", base64: "AAEC", durationMs: 1234, hasAudio: true });
  });
  it("builds stable temp paths when id provided", () => {
    const p = cameraTempPath({ kind: "snap", facing: "front", ext: "jpg", tmpDir: "/tmp", id: "id1" });
