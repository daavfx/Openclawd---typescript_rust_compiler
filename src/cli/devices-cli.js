...
const devicesCallOpts = (cmd, defaults) => cmd.option("--url <url>", "Gateway WebSocket URL (defaults to gateway.remote.url when configured)").option("--token <token>", "Gateway token (if required)").option("--password <password>", "Gateway password (password auth)").option("--timeout <ms>", "Timeout in ms", String((defaults?.timeoutMs ?? 10000))).option("--json", "Output JSON", false);
const callGatewayCli = async (method, opts, params) => withProgress({ label: "Devices ", indeterminate: true, enabled: (opts.json !== true) }, async () => await callGateway({ url: opts.url, token: opts.token, password: opts.password, method, params, timeoutMs: Number((opts.timeout ?? 10000)), clientName: GATEWAY_CLIENT_NAMES.CLI, mode: GATEWAY_CLIENT_MODES.CLI }));
function parseDevicePairingList(value) {
  const obj = ((typeof value === "object") && (value !== null)) ? value : {  };
  return { pending: Array.isArray(obj.pending) ? obj.pending : [], paired: Array.isArray(obj.paired) ? obj.paired : [] };
}
function formatTokenSummary(tokens) {
  if ((!tokens || (tokens.length === 0))) {
    return "none";
  }
  const parts = tokens.map((t) => "").sort((a, b) => a.localeCompare(b));
  return parts.join(", ");
}
export function registerDevicesCli(program) {
  ...
  devicesCallOpts(devices.command("list").description("List pending and paired devices").action(async (opts) => {
    ...
    if (list.pending?.length) {
      ...
      defaultRuntime.log(" ");
      defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Request", header: "Request", minWidth: 10 }, { key: "Device", header: "Device", minWidth: 16, flex: true }, { key: "Role", header: "Role", minWidth: 8 }, { key: "IP", header: "IP", minWidth: 12 }, { key: "Age", header: "Age", minWidth: 8 }, { key: "Flags", header: "Flags", minWidth: 8 }], rows: list.pending.map((req) => { Request: req.requestId, Device: (req.displayName || req.deviceId), Role: (req.role ?? ""), IP: (req.remoteIp ?? ""), Age: (typeof req.ts === "number") ? " ago" : "", Flags: req.isRepair ? "repair" : "" }) }).trimEnd());
    }
    if (list.paired?.length) {
      const tableWidth = Math.max(60, ((process.stdout.columns ?? 120) - 1));
      defaultRuntime.log(" ");
      defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "Device", header: "Device", minWidth: 16, flex: true }, { key: "Roles", header: "Roles", minWidth: 12, flex: true }, { key: "Scopes", header: "Scopes", minWidth: 12, flex: true }, { key: "Tokens", header: "Tokens", minWidth: 12, flex: true }, { key: "IP", header: "IP", minWidth: 12 }], rows: list.paired.map((device) => { Device: (device.displayName || device.deviceId), Roles: device.roles?.length ? device.roles.join(", ") : "", Scopes: device.scopes?.length ? device.scopes.join(", ") : "", Tokens: formatTokenSummary(device.tokens), IP: (device.remoteIp ?? "") }) }).trimEnd());
    }
  ...
  devicesCallOpts(devices.command("rotate").description("Rotate a device token for a role").requiredOption("--device <id>", "Device id").requiredOption("--role <role>", "Role name").option("--scope <scope...>", "Scopes to attach to the token (repeatable)").action(async (opts) => {
    ...
    const result = await callGatewayCli("device.token.rotate", opts, { deviceId, role, scopes: Array.isArray(opts.scope) ? opts.scope : undefined });
    defaultRuntime.log(JSON.stringify(result, null, 2));
  }));
  devicesCallOpts(devices.command("revoke").description("Revoke a device token for a role").requiredOption("--device <id>", "Device id").requiredOption("--role <role>", "Role name").action(async (opts) => {
    ...
    const result = await callGatewayCli("device.token.revoke", opts, { deviceId, role });
    defaultRuntime.log(JSON.stringify(result, null, 2));
  }));
}
