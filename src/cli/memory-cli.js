...
function formatSourceLabel(source, workspaceDir, agentId) {
  if ((source === "memory")) {
    return shortenHomeInString("memory (MEMORY.md + *.md)");
  }
  if ((source === "sessions")) {
    const stateDir = resolveStateDir(process.env, os.homedir);
    return shortenHomeInString("sessions (*.jsonl)");
  }
  return source;
}
...
async function checkReadableFile(pathname) {
  try {
  ...
  catch (err) {
    {
      ...
      return { exists: true, issue: " not readable ()" };
    }
  }
}
async function scanSessionFiles(agentId) {
  ...
  try {
  ...
  catch (err) {
    {
      ...
      issues.push("sessions directory not accessible (): ");
...
async function scanMemoryFiles(workspaceDir, extraPaths = []) {
  ...
  for (const extraPath of resolvedExtraPaths) {
    try {
    ...
    catch (err) {
      {
        const code = err.code;
        if ((code === "ENOENT")) {
          issues.push("additional memory path missing ()");
        } else {
          issues.push("additional memory path not accessible (): ");
        }
      }
    }
  }
  ...
  try {
  ...
  catch (err) {
    {
      ...
      if ((code === "ENOENT")) {
        ...
        issues.push("memory directory not accessible (): ");
        dirReadable = null;
      }
    }
  }
  ...
  try {
  ...
  catch (err) {
    {
      const code = err.code;
      if ((dirReadable !== null)) {
        issues.push("memory directory scan failed (): ");
        dirReadable = null;
      }
    }
  }
  ...
  if ((((totalFiles ?? 0) === 0) && (issues.length === 0))) {
    issues.push("no memory files found in ");
  }
...
export async function runMemoryStatus(opts) {
  ...
  for (const agentId of agentIds) {
    await withManager({ getManager: () => getMemorySearchManager({ cfg, agentId }), onMissing: (error) => defaultRuntime.log((error ?? "Memory search disabled.")), onCloseError: (err) => defaultRuntime.error("Memory manager close failed: "), close: (manager) => manager.close(), run: async (manager) => {
      ...
      if (deep) {
        ...
        if (opts.index) {
          await withProgressTotals({ label: "Indexing memory…", total: 0, fallback: opts.verbose ? "line" : undefined }, async (update, progress) => {
            try {
              {
                await manager.sync({ reason: "cli", progress: (syncUpdate) => {
                  update({ completed: syncUpdate.completed, total: syncUpdate.total, label: syncUpdate.label });
                  if (syncUpdate.label) {
            ...
            catch (err) {
              {
                ...
                defaultRuntime.error("Memory index failed: ");
                process.exitCode = 1;
              }
            }
          });
      ...
      const scan = await scanMemorySources({ workspaceDir: status.workspaceDir, agentId, sources, extraPaths: status.extraPaths });
  ...
  for (const result of allResults) {
    ...
    const indexedLabel = (totalFiles === null) ? "/? files ·  chunks" : "/ files ·  chunks";
    if (opts.index) {
      const line = indexError ? "Memory index failed: " : "Memory index complete.";
      defaultRuntime.log(line);
    }
    const extraPaths = formatExtraPaths(status.workspaceDir, (status.extraPaths ?? []));
    const lines = [" ", "  ", " ", status.sources?.length ? " " : null, extraPaths.length ? " " : null, " ", " ", " ", " "].filter(Boolean);
    if (embeddingProbe) {
    ...
    if (status.sourceCounts?.length) {
      ...
      for (const entry of status.sourceCounts) {
        ...
        const counts = (total === null) ? "/? files ·  chunks" : "/ files ·  chunks";
        lines.push("    ");
      }
    }
    if (status.fallback) {
    ...
    if (status.vector) {
      const vectorState = status.vector.enabled ? (status.vector.available === undefined) ? "unknown" : status.vector.available ? "ready" : "unavailable" : "disabled";
    ...
    if (status.cache) {
      ...
      const suffix = (status.cache.enabled && (typeof status.cache.entries === "number")) ? " ( entries)" : "";
      lines.push(" ");
        ...
        try {
          {
            if (opts.verbose) {
              ...
              const extraPaths = formatExtraPaths(status.workspaceDir, (status.extraPaths ?? []));
              const lines = [" ", "  ", " ", sourceLabels.length ? " " : null, extraPaths.length ? " " : null].filter(Boolean);
              if (status.fallback) {
                lines.push(" ");
              }
              defaultRuntime.log(lines.join("
"));
            ...
            const buildLabel = () => {
              ...
              const eta = formatEta();
              return eta ? " · elapsed  · eta " : " · elapsed ";
            };
            await withProgressTotals({ label: "Indexing memory…", total: 0, fallback: opts.verbose ? "line" : undefined }, async (update, progress) => {
              ...
              try {
                {
                  await manager.sync({ reason: "cli", force: opts.force, progress: (syncUpdate) => {
                    if (syncUpdate.label) {
                      lastLabel = syncUpdate.label;
                    }
                    lastCompleted = syncUpdate.completed;
                    lastTotal = syncUpdate.total;
                    update({ completed: syncUpdate.completed, total: syncUpdate.total, label: buildLabel() });
                    progress.setLabel(buildLabel());
                  } });
                }
              }
  ...
  memory.command("search").description("Search memory files").argument("<query>", "Search query").option("--agent <id>", "Agent id (default: default agent)").option("--max-results <n>", "Max results", (value) => Number(value)).option("--min-score <n>", "Minimum score", (value) => Number(value)).option("--json", "Print JSON").action(async (query, opts) => {
    ...
    await withManager({ getManager: () => getMemorySearchManager({ cfg, agentId }), onMissing: (error) => defaultRuntime.log((error ?? "Memory search disabled.")), onCloseError: (err) => defaultRuntime.error("Memory manager close failed: "), close: (manager) => manager.close(), run: async (manager) => {
      ...
      try {
        {
          results = await manager.search(query, { maxResults: opts.maxResults, minScore: opts.minScore });
        }
      }
      catch (err) {
        {
          const message = formatErrorMessage(err);
          defaultRuntime.error("Memory search failed: ");
          process.exitCode = 1;
          return;
        }
      }
