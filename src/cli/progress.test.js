...
describe("cli progress", () => {
  it("logs progress when non-tty and fallback=log", () => {
    const writes = [];
    const stream = { isTTY: false, write: vi.fn((chunk) => {
      writes.push(chunk);
    }) };
    const progress = createCliProgress({ label: "Indexing memory...", total: 10, stream, fallback: "log" });
  ...
  it("does not log without a tty when fallback is none", () => {
    ...
    const progress = createCliProgress({ label: "Nope", total: 2, stream, fallback: "none" });
    progress.setPercent(50);
    progress.done();
