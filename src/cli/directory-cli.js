...
function buildRows(entries) {
  return entries.map((entry) => { ID: entry.id, Name: (entry.name?.trim() ?? "") });
}
...
").action(() => {
  ...
  const resolve = async (opts) => {
    ...
    const selection = await resolveMessageChannelSelection({ cfg, channel: (opts.channel ?? null) });
  ...
  withChannel(directory.command("self").description("Show the current account user")).action(async (opts) => {
    try {
      {
        const {cfg, channelId, accountId, plugin} = await resolve({ channel: opts.channel, account: opts.account });
        const fn = plugin.directory?.self;
        if (!fn) {
          throw new Error("Channel  does not support directory self");
        }
        ...
        defaultRuntime.log("");
        defaultRuntime.log(renderTable({ width: tableWidth, columns: [{ key: "ID", header: "ID", minWidth: 16, flex: true }, { key: "Name", header: "Name", minWidth: 18, flex: true }], rows: buildRows([result]) }).trimEnd());
      }
    }
    catch (err) {
      {
    ...
    try {
      {
        ...
        if (!fn) {
          throw new Error("Channel  does not support directory peers");
        }
        const result = await fn({ cfg, accountId, query: (opts.query ?? null), limit: parseLimit(opts.limit), runtime: defaultRuntime });
        if (opts.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
    ...
    try {
      {
        ...
        if (!fn) {
          throw new Error("Channel  does not support directory groups");
        }
        const result = await fn({ cfg, accountId, query: (opts.query ?? null), limit: parseLimit(opts.limit), runtime: defaultRuntime });
        if (opts.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
    ...
    try {
      {
        ...
        if (!fn) {
          throw new Error("Channel  does not support group members listing");
        }
        const groupId = String((opts.groupId ?? "")).trim();
        if (!groupId) {
          throw new Error("Missing --group-id");
        }
        const result = await fn({ cfg, accountId, groupId, limit: parseLimit(opts.limit), runtime: defaultRuntime });
        if (opts.json) {
          defaultRuntime.log(JSON.stringify(result, null, 2));
          return;
        }
