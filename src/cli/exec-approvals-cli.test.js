...
const callGatewayFromCli = vi.fn(async (method, _opts, params) => {
  if (method.endsWith(".get")) {
    return { path: "/tmp/exec-approvals.json", exists: true, hash: "hash-1", file: { version: 1, agents: {  } } };
  }
...
const localSnapshot = { path: "/tmp/local-exec-approvals.json", exists: true, raw: "{}", hash: "hash-local", file: { version: 1, agents: {  } } };
vi.mock("./gateway-rpc.js", () => { callGatewayFromCli: (method, opts, params) => callGatewayFromCli(method, opts, params) });
vi.mock("./nodes-cli/rpc.js", async () => {
  const actual = await vi.importActual("./nodes-cli/rpc.js");
  return { ...actual: , resolveNodeId: vi.fn(async () => "node-1") };
});
vi.mock("../runtime.js", () => { defaultRuntime });
vi.mock("../infra/exec-approvals.js", async () => {
  const actual = await vi.importActual("../infra/exec-approvals.js");
  return { ...actual: , readExecApprovalsSnapshot: () => localSnapshot, saveExecApprovals: vi.fn() };
});
describe("exec approvals CLI", () => {
  ...
  it("loads node approvals when --node is set", async () => {
    ...
    expect(callGatewayFromCli).toHaveBeenCalledWith("exec.approvals.node.get", expect.anything(), { nodeId: "node-1" });
    expect(runtimeErrors).toHaveLength(0);
  });
  it("defaults allowlist add to wildcard agent", async () => {
    ...
    expect(callGatewayFromCli).not.toHaveBeenCalledWith("exec.approvals.set", expect.anything(), {  });
    expect(saveExecApprovals).toHaveBeenCalledWith(expect.objectContaining({ agents: expect.objectContaining({ "*": expect.anything() }) }));
  });
});
