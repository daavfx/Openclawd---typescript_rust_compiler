...
vi.mock("./gateway-rpc.js", async () => {
  ...
  return { ...actual: , callGatewayFromCli: (...args) => callGatewayFromCli(...args) };
});
describe("logs cli", () => {
  afterEach(() => {
  ...
  it("writes output directly to stdout/stderr", async () => {
    callGatewayFromCli.mockResolvedValueOnce({ file: "/tmp/openclaw.log", cursor: 1, size: 123, lines: ["raw line"], truncated: true, reset: true });
    const stdoutWrites = [];
    const stderrWrites = [];
  ...
  it("warns when the output pipe closes", async () => {
    callGatewayFromCli.mockResolvedValueOnce({ file: "/tmp/openclaw.log", lines: ["line one"] });
    const stderrWrites = [];
