...
import { buildTelegramThreadParams, buildTypingThreadParams, normalizeForwardedContext, resolveTelegramForumThreadId } from "./helpers.js";
...
describe("normalizeForwardedContext", () => {
  it("handles forward_origin users", () => {
    const ctx = normalizeForwardedContext({ forward_origin: { type: "user", sender_user: { first_name: "Ada", last_name: "Lovelace", username: "ada", id: 42 }, date: 123 } });
  ...
  it("handles hidden forward_origin names", () => {
    const ctx = normalizeForwardedContext({ forward_origin: { type: "hidden_user", sender_user_name: "Hidden Name", date: 456 } });
    expect(ctx).not.toBeNull();
  ...
  it("handles legacy forwards with signatures", () => {
    const ctx = normalizeForwardedContext({ forward_from_chat: { title: "OpenClaw Updates", username: "openclaw", id: 99, type: "channel" }, forward_signature: "Stan", forward_date: 789 });
    expect(ctx).not.toBeNull();
  ...
  it("handles legacy hidden sender names", () => {
    const ctx = normalizeForwardedContext({ forward_sender_name: "Legacy Hidden", forward_date: 111 });
    expect(ctx).not.toBeNull();
