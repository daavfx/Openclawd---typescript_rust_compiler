...
import { listNativeCommandSpecs, listNativeCommandSpecsForConfig } from "../auto-reply/commands-registry.js";
import { escapeRegExp, formatEnvelopeTimestamp } from "../../test/helpers/envelope-timestamp.js";
import { expectInboundContextContract } from "../../test/helpers/inbound-contract.js";
import { resolveTelegramFetch } from "./fetch.js";
let createTelegramBot;
let getTelegramSequentialKey;
let resetInboundDedupe;
let replyModule;
const {listSkillCommandsForAgents} = vi.hoisted(() => { listSkillCommandsForAgents: vi.fn(() => []) });
vi.mock("../auto-reply/skill-commands.js", () => { listSkillCommandsForAgents });
const {sessionStorePath} = vi.hoisted(() => { sessionStorePath: "/tmp/openclaw-telegram-bot-.json" });
...
vi.mock("./sent-message-cache.js", () => { wasSentByBot, recordSentMessage: vi.fn(), clearSentMessageCache: vi.fn() });
const useSpy = vi.fn();
...
const apiStub = { config: { use: useSpy }, answerCallbackQuery: answerCallbackQuerySpy, sendChatAction: sendChatActionSpy, editMessageText: editMessageTextSpy, setMessageReaction: setMessageReactionSpy, setMyCommands: setMyCommandsSpy, sendMessage: sendMessageSpy, sendAnimation: sendAnimationSpy, sendPhoto: sendPhotoSpy };
...
describe("createTelegramBot", () => {
  beforeEach(async () => {
    ...
    process.env.TZ = "UTC";
    resetInboundDedupe();
    loadConfig.mockReturnValue({ agents: { defaults: { envelopeTimezone: "utc" } }, channels: { telegram: { dmPolicy: "open", allowFrom: ["*"] } } });
  ...
  it("merges custom commands with native commands", () => {
    const config = { channels: { telegram: { customCommands: [{ command: "custom_backup", description: "Git backup" }, { command: "/Custom_Generate", description: "Create an image" }] } } };
    loadConfig.mockReturnValue(config);
    createTelegramBot({ token: "tok" });
    const registered = setMyCommandsSpy.mock.calls[0]?.[0];
    const skillCommands = resolveSkillCommands(config);
    const native = listNativeCommandSpecsForConfig(config, { skillCommands }).map((command) => { command: command.name, description: command.description });
    expect(registered.slice(0, native.length)).toEqual(native);
    expect(registered.slice(native.length)).toEqual([{ command: "custom_backup", description: "Git backup" }, { command: "custom_generate", description: "Create an image" }]);
  });
  it("ignores custom commands that collide with native commands", () => {
    const errorSpy = vi.fn();
    const config = { channels: { telegram: { customCommands: [{ command: "status", description: "Custom status" }, { command: "custom_backup", description: "Git backup" }] } } };
    loadConfig.mockReturnValue(config);
    createTelegramBot({ token: "tok", runtime: { log: vi.fn(), error: errorSpy, exit: (code) => {
  ...
  it("registers custom commands when native commands are disabled", () => {
    const config = { commands: { native: false }, channels: { telegram: { customCommands: [{ command: "custom_backup", description: "Git backup" }, { command: "custom_generate", description: "Create an image" }] } } };
    loadConfig.mockReturnValue(config);
    createTelegramBot({ token: "tok" });
    const registered = setMyCommandsSpy.mock.calls[0]?.[0];
    expect(registered).toEqual([{ command: "custom_backup", description: "Git backup" }, { command: "custom_generate", description: "Create an image" }]);
  ...
  it("sequentializes updates by chat and thread", () => {
    ...
    expect(getTelegramSequentialKey({ message: { chat: { id: 123, type: "private" }, message_thread_id: 9 } })).toBe("telegram:123:topic:9");
    expect(getTelegramSequentialKey({ message: { chat: { id: 123, type: "supergroup" }, message_thread_id: 9 } })).toBe("telegram:123");
    expect(getTelegramSequentialKey({ message: { chat: { id: 123, type: "supergroup", is_forum: true } } })).toBe("telegram:123:topic:1");
    expect(getTelegramSequentialKey({ update: { message: { chat: { id: 555 } } } })).toBe("telegram:555");
  });
  it("routes callback_query payloads as messages and answers callbacks", async () => {
    ...
    await callbackHandler({ callbackQuery: { id: "cbq-1", data: "cmd:option_a", from: { id: 9, first_name: "Ada", username: "ada_bot" }, message: { chat: { id: 1234, type: "private" }, date: 1736380800, message_id: 10 } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
  ...
  it("blocks callback_query when inline buttons are allowlist-only and sender not authorized", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    createTelegramBot({ token: "tok", config: { channels: { telegram: { dmPolicy: "pairing", capabilities: { inlineButtons: "allowlist" }, allowFrom: [] } } } });
    const callbackHandler = onSpy.mock.calls.find((call) => (call[0] === "callback_query"))?.[1];
    expect(callbackHandler).toBeDefined();
    await callbackHandler({ callbackQuery: { id: "cbq-2", data: "cmd:option_b", from: { id: 9, first_name: "Ada", username: "ada_bot" }, message: { chat: { id: 1234, type: "private" }, date: 1736380800, message_id: 11 } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
  ...
  it("edits commands list for pagination callbacks", async () => {
    ...
    const callbackHandler = onSpy.mock.calls.find((call) => (call[0] === "callback_query"))?.[1];
    expect(callbackHandler).toBeDefined();
    await callbackHandler({ callbackQuery: { id: "cbq-3", data: "commands_page_2:main", from: { id: 9, first_name: "Ada", username: "ada_bot" }, message: { chat: { id: 1234, type: "private" }, date: 1736380800, message_id: 12 } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(listSkillCommandsForAgents).toHaveBeenCalledWith({ cfg: expect.any(Object), agentIds: ["main"] });
    ...
    expect(params).toEqual(expect.objectContaining({ reply_markup: expect.any(Object) }));
  });
  it("blocks pagination callbacks when allowlist rejects sender", async () => {
    ...
    const callbackHandler = onSpy.mock.calls.find((call) => (call[0] === "callback_query"))?.[1];
    expect(callbackHandler).toBeDefined();
    await callbackHandler({ callbackQuery: { id: "cbq-4", data: "commands_page_2", from: { id: 9, first_name: "Ada", username: "ada_bot" }, message: { chat: { id: 1234, type: "private" }, date: 1736380800, message_id: 13 } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    ...
    try {
      {
        ...
        const message = { chat: { id: 1234, type: "private" }, text: "hello world", date: 1736380800, from: { first_name: "Ada", last_name: "Lovelace", username: "ada_bot" } };
        await handler({ message, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
        expect(replySpy).toHaveBeenCalledTimes(1);
        const payload = replySpy.mock.calls[0][0];
        const expectedTimestamp = formatEnvelopeTimestamp(new Date("2025-01-09T00:00:00Z"));
        const timestampPattern = escapeRegExp(expectedTimestamp);
        expect(payload.Body).toMatch(new RegExp("^\\[Telegram Ada Lovelace \\(@ada_bot\\) id:1234 (\\+\\d+[smhd] )?\\]"));
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "pairing" } } });
    readTelegramAllowFromStore.mockResolvedValue([]);
    upsertTelegramPairingRequest.mockResolvedValue({ code: "PAIRME12", created: true });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 1234, type: "private" }, text: "hello", date: 1736380800, from: { id: 999, username: "random" } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).not.toHaveBeenCalled();
  ...
  it("does not resend pairing code when a request is already pending", async () => {
    ...
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "pairing" } } });
    readTelegramAllowFromStore.mockResolvedValue([]);
    upsertTelegramPairingRequest.mockResolvedValueOnce({ code: "PAIRME12", created: true }).mockResolvedValueOnce({ code: "PAIRME12", created: false });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    const message = { chat: { id: 1234, type: "private" }, text: "hello", date: 1736380800, from: { id: 999, username: "random" } };
    await handler({ message, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    await handler({ message: { ...message: , text: "hello again" }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).not.toHaveBeenCalled();
    expect(sendMessageSpy).toHaveBeenCalledTimes(1);
  });
  it("triggers typing cue via onReplyStart", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 42, type: "private" }, text: "hi" }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
  ...
  it("uses quote text when a Telegram partial reply is received", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 7, type: "private" }, text: "Sure, see below", date: 1736380800, reply_to_message: { message_id: 9001, text: "Can you summarize this?", from: { first_name: "Ada" } }, quote: { text: "summarize this" } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
  ...
  it("handles quote-only replies without reply metadata", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 7, type: "private" }, text: "Sure, see below", date: 1736380800, quote: { text: "summarize this" } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
  ...
  it("sends replies without native reply threading", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 5, type: "private" }, text: "hi", date: 1736380800, message_id: 101 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
  ...
  it("honors replyToMode=first for threaded replies", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    replySpy.mockResolvedValue({ text: "a".repeat(4500), replyToId: "101" });
  ...
  it("prefixes final replies with responsePrefix", async () => {
    ...
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "open", allowFrom: ["*"] } }, messages: { responsePrefix: "PFX" } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 5, type: "private" }, text: "hi", date: 1736380800 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(sendMessageSpy).toHaveBeenCalledTimes(1);
  ...
  it("blocks group messages when telegram.groups is set without a wildcard", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groups: { "123": { requireMention: false } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 456, type: "group", title: "Ops" }, text: "@openclaw_bot hello", date: 1736380800 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).not.toHaveBeenCalled();
  });
  it("skips group messages without mention when requireMention is enabled", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groups: { "*": { requireMention: true } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 123, type: "group", title: "Dev Chat" }, text: "hello", date: 1736380800 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).not.toHaveBeenCalled();
  });
  it("accepts group replies to the bot without explicit mention when requireMention is enabled", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 456, type: "group", title: "Ops Chat" }, text: "following up", date: 1736380800, reply_to_message: { message_id: 42, text: "original reply", from: { id: 999, first_name: "OpenClaw" } } }, me: { id: 999, username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
    expect(payload.WasMentioned).toBe(true);
  });
  it("honors routed group activation from session store", async () => {
    ...
    fs.writeFileSync(storePath, JSON.stringify({ "agent:ops:telegram:group:123": { groupActivation: "always" } }), "utf-8");
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "open", groups: { "*": { requireMention: true } } } }, bindings: [{ agentId: "ops", match: { channel: "telegram", peer: { kind: "group", id: "123" } } }], session: { store: storePath } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 123, type: "group", title: "Routing" }, text: "hello", date: 1736380800 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
  });
  it("routes DMs by telegram accountId binding", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { accounts: { opie: { botToken: "tok-opie", dmPolicy: "open" } } } }, bindings: [{ agentId: "opie", match: { channel: "telegram", accountId: "opie" } }] });
    createTelegramBot({ token: "tok", accountId: "opie" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 123, type: "private" }, from: { id: 999, username: "testuser" }, text: "hello", date: 1736380800, message_id: 42 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
  ...
  it("allows per-group requireMention override", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "open", groups: { "*": { requireMention: true }, "123": { requireMention: false } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 123, type: "group", title: "Dev Chat" }, text: "hello", date: 1736380800 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
  ...
  it("allows per-topic requireMention override", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "open", groups: { "*": { requireMention: true }, "-1001234567890": { requireMention: true, topics: { "99": { requireMention: false } } } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: -1001234567890, type: "supergroup", title: "Forum Group", is_forum: true }, text: "hello", date: 1736380800, message_thread_id: 99 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
  });
  it("inherits group allowlist + requireMention in topics", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "allowlist", groups: { "-1001234567890": { requireMention: false, allowFrom: ["123456789"], topics: { "99": {  } } } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: -1001234567890, type: "supergroup", title: "Forum Group", is_forum: true }, from: { id: 123456789, username: "testuser" }, text: "hello", date: 1736380800, message_thread_id: 99 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
  });
  it("prefers topic allowFrom over group allowFrom", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "allowlist", groups: { "-1001234567890": { allowFrom: ["123456789"], topics: { "99": { allowFrom: ["999999999"] } } } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: -1001234567890, type: "supergroup", title: "Forum Group", is_forum: true }, from: { id: 123456789, username: "testuser" }, text: "hello", date: 1736380800, message_thread_id: 99 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
  ...
  it("honors groups default when no explicit group override exists", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 456, type: "group", title: "Ops" }, text: "hello", date: 1736380800 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
  });
  it("does not block group messages when bot username is unknown", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "open", groups: { "*": { requireMention: true } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 789, type: "group", title: "No Me" }, text: "hello", date: 1736380800 }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
  });
  it("sends GIF replies as animations", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    replySpy.mockResolvedValueOnce({ text: "caption", mediaUrl: "https://example.com/fun" });
    loadWebMedia.mockResolvedValueOnce({ buffer: Buffer.from("GIF89a"), contentType: "image/gif", fileName: "fun.gif" });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: 1234, type: "private" }, text: "hello world", date: 1736380800, message_id: 5, from: { first_name: "Ada" } }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(sendAnimationSpy).toHaveBeenCalledTimes(1);
    expect(sendAnimationSpy).toHaveBeenCalledWith("1234", expect.anything(), { caption: "caption", parse_mode: "HTML", reply_to_message_id: undefined });
  ...
  it("applies topic skill filters and system prompts", async () => {
    ...
    const replySpy = replyModule.__replySpy;
    replySpy.mockReset();
    loadConfig.mockReturnValue({ channels: { telegram: { groupPolicy: "open", groups: { "-1001234567890": { requireMention: false, systemPrompt: "Group prompt", skills: ["group-skill"], topics: { "99": { skills: [], systemPrompt: "Topic prompt" } } } } } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: -1001234567890, type: "supergroup", title: "Forum Group", is_forum: true }, from: { id: 12345, username: "testuser" }, text: "hello", date: 1736380800, message_id: 42, message_thread_id: 99 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(replySpy).toHaveBeenCalledTimes(1);
...
Topic prompt");
  ...
  it("passes message_thread_id to topic replies", async () => {
    ...
    const handler = getOnHandler("message");
    await handler({ message: { chat: { id: -1001234567890, type: "supergroup", title: "Forum Group", is_forum: true }, from: { id: 12345, username: "testuser" }, text: "hello", date: 1736380800, message_id: 42, message_thread_id: 99 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } });
    expect(sendMessageSpy).toHaveBeenCalledWith("-1001234567890", expect.any(String), expect.objectContaining({ message_thread_id: 99 }));
  });
  it("threads native command replies inside topics", async () => {
    ...
    replySpy.mockReset();
    replySpy.mockResolvedValue({ text: "response" });
    loadConfig.mockReturnValue({ commands: { native: true }, channels: { telegram: { dmPolicy: "open", allowFrom: ["*"], groups: { "*": { requireMention: false } } } } });
    createTelegramBot({ token: "tok" });
    expect(commandSpy).toHaveBeenCalled();
    const handler = commandSpy.mock.calls[0][1];
    await handler({ message: { chat: { id: -1001234567890, type: "supergroup", title: "Forum Group", is_forum: true }, from: { id: 12345, username: "testuser" }, text: "/status", date: 1736380800, message_id: 42, message_thread_id: 99 }, match: "" });
    expect(sendMessageSpy).toHaveBeenCalledWith("-1001234567890", expect.any(String), expect.objectContaining({ message_thread_id: 99 }));
  });
  it("sets command target session key for dm topic commands", async () => {
    ...
    replySpy.mockReset();
    replySpy.mockResolvedValue({ text: "response" });
    loadConfig.mockReturnValue({ commands: { native: true }, channels: { telegram: { dmPolicy: "pairing" } } });
    ...
    await handler({ message: { chat: { id: 12345, type: "private" }, from: { id: 12345, username: "testuser" }, text: "/status", date: 1736380800, message_id: 42, message_thread_id: 99 }, match: "" });
    expect(replySpy).toHaveBeenCalledTimes(1);
    const payload = replySpy.mock.calls[0][0];
  ...
  it("allows native DM commands for paired users", async () => {
    ...
    await handler({ message: { chat: { id: 12345, type: "private" }, from: { id: 12345, username: "testuser" }, text: "/status", date: 1736380800, message_id: 42 }, match: "" });
    expect(replySpy).toHaveBeenCalledTimes(1);
    expect(sendMessageSpy.mock.calls.some((call) => (call[1] === "You are not authorized to use this command."))).toBe(false);
  });
  ...
  it("skips tool summaries for native slash commands", async () => {
    ...
    loadConfig.mockReturnValue({ commands: { native: true }, channels: { telegram: { dmPolicy: "open", allowFrom: ["*"] } } });
    createTelegramBot({ token: "tok" });
    const verboseHandler = commandSpy.mock.calls.find((call) => (call[0] === "verbose"))?.[1];
    if (!verboseHandler) {
      throw new Error("verbose command handler missing");
    }
    await verboseHandler({ message: { chat: { id: 12345, type: "private" }, from: { id: 12345, username: "testuser" }, text: "/verbose on", date: 1736380800, message_id: 42 }, match: "on" });
    expect(sendMessageSpy).toHaveBeenCalledTimes(1);
  ...
  it("dedupes duplicate message updates by update_id", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message");
    const ctx = { update: { update_id: 111 }, message: { chat: { id: 123, type: "private" }, from: { id: 456, username: "testuser" }, text: "hello", date: 1736380800, message_id: 42 }, me: { username: "openclaw_bot" }, getFile: async () => { download: async () => new Uint8Array() } };
    await handler(ctx);
    await handler(ctx);
    expect(replySpy).toHaveBeenCalledTimes(1);
  });
  ...
  it("enqueues system event for reaction", async () => {
    ...
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "open", reactionNotifications: "all" } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 500 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 42, user: { id: 9, first_name: "Ada", username: "ada_bot" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "👍" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEvent).toHaveBeenCalledWith("Telegram reaction added: 👍 by Ada (@ada_bot) on msg 42", expect.objectContaining({ contextKey: expect.stringContaining("telegram:reaction:add:1234:42:9") }));
  });
  it("skips reaction when reactionNotifications is off", async () => {
    onSpy.mockReset();
    enqueueSystemEvent.mockReset();
    wasSentByBot.mockReturnValue(true);
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "open", reactionNotifications: "off" } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 501 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 42, user: { id: 9, first_name: "Ada" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "👍" }] } });
  ...
  it("defaults reactionNotifications to own", async () => {
    ...
    enqueueSystemEvent.mockReset();
    wasSentByBot.mockReturnValue(true);
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "open" } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 502 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 43, user: { id: 9, first_name: "Ada" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "👍" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
  });
  it("allows reaction in all mode regardless of message sender", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 503 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 99, user: { id: 9, first_name: "Ada" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "🎉" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEvent).toHaveBeenCalledWith("Telegram reaction added: 🎉 by Ada on msg 99", expect.any(Object));
  });
  it("skips reaction in own mode when message is not sent by bot", async () => {
    onSpy.mockReset();
    enqueueSystemEvent.mockReset();
    wasSentByBot.mockReturnValue(false);
    loadConfig.mockReturnValue({ channels: { telegram: { dmPolicy: "open", reactionNotifications: "own" } } });
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 503 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 99, user: { id: 9, first_name: "Ada" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "🎉" }] } });
    expect(enqueueSystemEvent).not.toHaveBeenCalled();
  ...
  it("skips reaction from bot users", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 503 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 99, user: { id: 9, first_name: "Bot", is_bot: true }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "🎉" }] } });
    expect(enqueueSystemEvent).not.toHaveBeenCalled();
  });
  it("skips reaction removal (only processes added reactions)", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 504 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 42, user: { id: 9, first_name: "Ada" }, date: 1736380800, old_reaction: [{ type: "emoji", emoji: "👍" }], new_reaction: [] } });
    expect(enqueueSystemEvent).not.toHaveBeenCalled();
  });
  it("uses correct session key for forum group reactions with topic", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 505 }, messageReaction: { chat: { id: 5678, type: "supergroup", is_forum: true }, message_id: 100, message_thread_id: 42, user: { id: 10, first_name: "Bob", username: "bob_user" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "🔥" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEvent).toHaveBeenCalledWith("Telegram reaction added: 🔥 by Bob (@bob_user) on msg 100", expect.objectContaining({ sessionKey: expect.stringContaining("telegram:group:5678:topic:42"), contextKey: expect.stringContaining("telegram:reaction:add:5678:100:10") }));
  });
  it("uses correct session key for forum group reactions in general topic", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 506 }, messageReaction: { chat: { id: 5678, type: "supergroup", is_forum: true }, message_id: 101, user: { id: 10, first_name: "Bob" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "👀" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEvent).toHaveBeenCalledWith("Telegram reaction added: 👀 by Bob on msg 101", expect.objectContaining({ sessionKey: expect.stringContaining("telegram:group:5678:topic:1"), contextKey: expect.stringContaining("telegram:reaction:add:5678:101:10") }));
  });
  it("uses correct session key for regular group reactions without topic", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 507 }, messageReaction: { chat: { id: 9999, type: "group" }, message_id: 200, user: { id: 11, first_name: "Charlie" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "❤️" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEvent).toHaveBeenCalledWith("Telegram reaction added: ❤️ by Charlie on msg 200", expect.objectContaining({ sessionKey: expect.stringContaining("telegram:group:9999"), contextKey: expect.stringContaining("telegram:reaction:add:9999:200:11") }));
  ...
  it("uses thread session key for dm reactions with topic id", async () => {
    ...
    createTelegramBot({ token: "tok" });
    const handler = getOnHandler("message_reaction");
    await handler({ update: { update_id: 508 }, messageReaction: { chat: { id: 1234, type: "private" }, message_id: 300, message_thread_id: 42, user: { id: 12, first_name: "Dana" }, date: 1736380800, old_reaction: [], new_reaction: [{ type: "emoji", emoji: "🔥" }] } });
    expect(enqueueSystemEvent).toHaveBeenCalledTimes(1);
    expect(enqueueSystemEvent).toHaveBeenCalledWith("Telegram reaction added: 🔥 by Dana on msg 300", expect.objectContaining({ sessionKey: expect.stringContaining(":thread:42"), contextKey: expect.stringContaining("telegram:reaction:add:1234:300:12") }));
  });
});
