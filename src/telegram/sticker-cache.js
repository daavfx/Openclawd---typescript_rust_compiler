...
export function searchStickers(query, limit = 10) {
  ...
  return results.sort((a, b) => (b.score - a.score)).slice(0, limit).map((r) => r.sticker);
}
...
export function getCacheStats() {
  ...
  return { count: stickers.length, oldestAt: sorted[0]?.cachedAt, newestAt: sorted[(sorted.length - 1)]?.cachedAt };
}
...
export async function describeStickerImage(params) {
  ...
  if (!resolved) {
    resolved = await resolveAutoImageModel({ cfg, agentDir, activeModel });
  }
  if (!resolved?.model) {
  ...
  logVerbose("telegram: describing sticker with /");
  try {
    {
      const buffer = await fs.readFile(imagePath);
      const {describeImageWithModel} = await import("../media-understanding/providers/image.js");
      const result = await describeImageWithModel({ buffer, fileName: "sticker.webp", mime: "image/webp", prompt: STICKER_DESCRIPTION_PROMPT, cfg, agentDir: (agentDir ?? ""), provider, model, maxTokens: 150, timeoutMs: 30000 });
      return result.text;
    }
  }
  catch (err) {
    {
      logVerbose("telegram: failed to describe sticker: ");
      return null;
    }
  }
}
