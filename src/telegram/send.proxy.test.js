...
const {botApi, botCtorSpy} = vi.hoisted(() => { botApi: { sendMessage: vi.fn(), setMessageReaction: vi.fn(), deleteMessage: vi.fn() }, botCtorSpy: vi.fn() });
const {loadConfig} = vi.hoisted(() => { loadConfig: vi.fn(() => {  }) });
const {makeProxyFetch} = vi.hoisted(() => { makeProxyFetch: vi.fn() });
const {resolveTelegramFetch} = vi.hoisted(() => { resolveTelegramFetch: vi.fn() });
vi.mock("../config/config.js", async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual: , loadConfig };
});
vi.mock("./proxy.js", () => { makeProxyFetch });
vi.mock("./fetch.js", () => { resolveTelegramFetch });
...
describe("telegram proxy client", () => {
  ...
  beforeEach(() => {
    ...
    loadConfig.mockReturnValue({ channels: { telegram: { accounts: { foo: { proxy: proxyUrl } } } } });
  ...
  it("uses proxy fetch for sendMessage", async () => {
    ...
    expect(botCtorSpy).toHaveBeenCalledWith("tok", expect.objectContaining({ client: expect.objectContaining({ fetch: fetchImpl }) }));
  });
