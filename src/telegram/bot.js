...
export function getTelegramSequentialKey(ctx) {
  ...
  if ((typeof chatId === "number")) {
    return (threadId != null) ? "telegram::topic:" : "telegram:";
  }
...
export function createTelegramBot(opts) {
  ...
  const timeoutSeconds = ((typeof telegramCfg?.timeoutSeconds === "number") && Number.isFinite(telegramCfg.timeoutSeconds)) ? Math.max(1, Math.floor(telegramCfg.timeoutSeconds)) : undefined;
  const client = (shouldProvideFetch || timeoutSeconds) ? { ...(shouldProvideFetch && fetchImpl) ? { fetch: fetchImpl } : {  }: , ...timeoutSeconds ? { timeoutSeconds } : {  }:  } : undefined;
  const bot = new Bot(opts.token, client ? { client } : undefined);
  bot.api.config.use(apiThrottler());
  bot.use(sequentialize(getTelegramSequentialKey));
  bot.catch((err) => {
    runtime.error?.(danger("telegram bot error: "));
  });
  bot.catch((err) => {
  ...
  const shouldSkipUpdate = (ctx) => {
    ...
    if (((skipped && key) && shouldLogVerbose())) {
      logVerbose("telegram dedupe: skipped ");
    }
    return skipped;
  };
  ...
  const stringifyUpdate = (update) => {
    ...
    return JSON.stringify((update ?? null), (key, value) => {
      ...
      if ((Array.isArray(value) && (value.length > MAX_RAW_UPDATE_ARRAY))) {
        return [...value.slice(0, MAX_RAW_UPDATE_ARRAY), "...( more)"];
      }
      ...
      try {
        {
          ...
          const preview = (raw.length > MAX_RAW_UPDATE_CHARS) ? "..." : raw;
          rawUpdateLogger.debug("telegram update: ");
        }
      }
      catch (err) {
        {
          rawUpdateLogger.debug("telegram update log failed: ");
        }
      }
    }
    await next();
  ...
  const groupAllowFrom = (((opts.groupAllowFrom ?? telegramCfg.groupAllowFrom) ?? (telegramCfg.allowFrom && (telegramCfg.allowFrom.length > 0)) ? telegramCfg.allowFrom : undefined) ?? (opts.allowFrom && (opts.allowFrom.length > 0)) ? opts.allowFrom : undefined);
  const replyToMode = ((opts.replyToMode ?? telegramCfg.replyToMode) ?? "first");
  const streamMode = resolveTelegramStreamMode(telegramCfg);
  const nativeEnabled = resolveNativeCommandsEnabled({ providerId: "telegram", providerSetting: telegramCfg.commands?.native, globalSetting: cfg.commands?.native });
  const nativeSkillsEnabled = resolveNativeSkillsEnabled({ providerId: "telegram", providerSetting: telegramCfg.commands?.nativeSkills, globalSetting: cfg.commands?.nativeSkills });
  const nativeDisabledExplicit = isNativeCommandsExplicitlyDisabled({ providerSetting: telegramCfg.commands?.native, globalSetting: cfg.commands?.native });
  ...
  const resolveBotTopicsEnabled = async (ctx) => {
    ...
    try {
      {
        const me = await withTelegramApiErrorLogging({ operation: "getMe", runtime, fn: () => bot.api.getMe() });
        botHasTopicsEnabled = Boolean(me?.has_topics_enabled);
      }
    }
    catch (err) {
      {
        logVerbose("telegram getMe failed: ");
        botHasTopicsEnabled = false;
      }
    }
    return botHasTopicsEnabled;
  };
  const resolveGroupPolicy = (chatId) => resolveChannelGroupPolicy({ cfg, channel: "telegram", accountId: account.accountId, groupId: String(chatId) });
  const resolveGroupActivation = (params) => {
    const agentId = (params.agentId ?? resolveDefaultAgentId(cfg));
    const sessionKey = (params.sessionKey ?? "agent::telegram:group:");
    ...
    try {
      ...
      {
        logVerbose("Failed to load session for activation check: ");
      }
    }
    return undefined;
  };
  const resolveGroupRequireMention = (chatId) => resolveChannelGroupRequireMention({ cfg, channel: "telegram", accountId: account.accountId, groupId: String(chatId), requireMentionOverride: opts.requireMention, overrideOrder: "after-config" });
  ...
  registerTelegramNativeCommands({ bot, cfg, runtime, accountId: account.accountId, telegramCfg, allowFrom, groupAllowFrom, replyToMode, textLimit, useAccessGroups, nativeEnabled, nativeSkillsEnabled, nativeDisabledExplicit, resolveGroupPolicy, resolveTelegramGroupConfig, shouldSkipUpdate, opts });
    ...
    try {
      {
        ...
        const senderName = user ? ([user.first_name, user.last_name].filter(Boolean).join(" ").trim() || user.username) : undefined;
        const senderUsername = user?.username ? "@" : undefined;
        ...
        for (const r of addedReactions) {
          const emoji = r.emoji;
          const text = "Telegram reaction added:  by  on msg ";
          enqueueSystemEvent(text, { sessionKey: sessionKey, contextKey: "telegram:reaction:add::::" });
          logVerbose("telegram: reaction event enqueued: ");
        }
      }
    }
    catch (err) {
      {
        runtime.error?.(danger("telegram reaction handler failed: "));
      }
    }
  });
  registerTelegramHandlers({ cfg, accountId: account.accountId, bot, opts, runtime, mediaMaxBytes, telegramCfg, groupAllowFrom, resolveGroupPolicy, resolveTelegramGroupConfig, shouldSkipUpdate, processMessage, logger });
  return bot;
}
