...
function createTelegramHttpLogger(cfg) {
  ...
  return (label, err) => {
    ...
    diagLogger.warn("telegram http error (): ");
  };
}
function resolveTelegramClientOptions(account) {
  ...
  return (fetchImpl || timeoutSeconds) ? { ...fetchImpl ? { fetch: fetchImpl } : {  }: , ...timeoutSeconds ? { timeoutSeconds } : {  }:  } : undefined;
}
function resolveToken(explicit, params) {
  ...
  if (!params.token) {
    throw new Error("Telegram bot token missing for account \"\" (set channels.telegram.accounts..botToken/tokenFile or TELEGRAM_BOT_TOKEN for default).");
  }
...
export function buildInlineKeyboard(buttons) {
  ...
  const rows = buttons.map((row) => row.filter((button) => (button?.text && button?.callback_data)).map((button) => { text: button.text, callback_data: button.callback_data })).filter((row) => (row.length > 0));
  if ((rows.length === 0)) {
    return undefined;
  }
...
export async function sendMessageTelegram(to, text, opts = {  }) {
  ...
  if ((opts.replyToMessageId != null)) {
    if (quoteText) {
      threadParams.reply_parameters = { message_id: Math.trunc(opts.replyToMessageId), quote: quoteText };
    } else {
      threadParams.reply_to_message_id = Math.trunc(opts.replyToMessageId);
    }
  }
  const hasThreadParams = (Object.keys(threadParams).length > 0);
  const request = createTelegramRetryRunner({ retry: opts.retry, configRetry: account.config.retry, verbose: opts.verbose, shouldRetry: (err) => isRecoverableTelegramNetworkError(err, { context: "send" }) });
  const logHttpError = createTelegramHttpLogger(cfg);
  const requestWithDiag = (fn, label) => withTelegramApiErrorLogging({ operation: (label ?? "request"), fn: () => request(fn, label) }).catch((err) => {
  ...
  const wrapChatNotFound = (err) => {
    ...
    return new Error(["Telegram send failed: chat not found (chat_id=).", "Likely: bot not started in DM, bot removed from group/channel, group migrated (new -100… id), or wrong bot token.", "Input was: ."].join(" "));
  };
  ...
  const sendTelegramText = async (rawText, params, fallbackText) => {
    ...
    const sendParams = { parse_mode: "HTML", ...baseParams: , ...(opts.silent === true) ? { disable_notification: true } : {  }:  };
  ...
  if (mediaUrl) {
    ...
    const baseMediaParams = { ...hasThreadParams ? threadParams : {  }: , ...(!needsSeparateText && replyMarkup) ? { reply_markup: replyMarkup } : {  }:  };
    const mediaParams = { caption: htmlCaption, ...htmlCaption ? { parse_mode: "HTML" } : {  }: , ...baseMediaParams: , ...(opts.silent === true) ? { disable_notification: true } : {  }:  };
    let result;
    if (isGif) {
    ...
    if ((needsSeparateText && followUpText)) {
      const textParams = (hasThreadParams || replyMarkup) ? { ...threadParams: , ...replyMarkup ? { reply_markup: replyMarkup } : {  }:  } : undefined;
      const textRes = await sendTelegramText(followUpText, textParams);
      return { messageId: String((textRes?.message_id ?? mediaMessageId)), chatId: resolvedChatId };
    }
...
export async function deleteMessageTelegram(chatIdInput, messageIdInput, opts = {  }) {
  ...
  logVerbose("[telegram] Deleted message  from chat ");
  return { ok: true };
}
...
export async function editMessageTelegram(chatIdInput, messageIdInput, text, opts = {  }) {
  ...
  logVerbose("[telegram] Edited message  in chat ");
