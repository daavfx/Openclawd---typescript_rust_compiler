...
export const registerTelegramHandlers = ({cfg, accountId, bot, opts, runtime, mediaMaxBytes, telegramCfg, groupAllowFrom, resolveGroupPolicy, resolveTelegramGroupConfig, shouldSkipUpdate, processMessage, logger}) => {
  ...
  const inboundDebouncer = createInboundDebouncer({ debounceMs, buildKey: (entry) => entry.debounceKey, shouldDebounce: (entry) => {
...
");
    ...
    const syntheticMessage = { ...first.msg: , text: combinedText, caption: undefined, caption_entities: undefined, entities: undefined, date: (last.msg.date ?? first.msg.date) };
    ...
    runtime.error?.(danger("telegram debounce flush failed: "));
  } });
    ...
    try {
      {
        ...
        for (const {ctx} of entry.messages) {
          ...
          if (media) {
            allMedia.push({ path: media.path, contentType: media.contentType, stickerMetadata: media.stickerMetadata });
          }
        }
      ...
      {
        runtime.error?.(danger("media group handler failed: "));
      }
    }
  };
  const flushTextFragments = async (entry) => {
    try {
      ...
      {
        runtime.error?.(danger("text fragment handler failed: "));
      }
    }
  };
  ...
  bot.on("callback_query", async (ctx) => {
    ...
    await withTelegramApiErrorLogging({ operation: "answerCallbackQuery", runtime, fn: () => bot.api.answerCallbackQuery(callback.id) }).catch(() => {
    });
    try {
      {
        ...
        const effectiveDmAllow = normalizeAllowFromWithStore({ allowFrom: telegramCfg.allowFrom, storeAllowFrom });
        ...
        if (isGroup) {
          ...
          if ((typeof groupAllowOverride !== "undefined")) {
            const allowed = (senderId && isSenderAllowed({ allow: effectiveGroupAllow, senderId, senderUsername }));
            if (!allowed) {
          ...
          if ((groupPolicy === "allowlist")) {
            ...
            if (!isSenderAllowed({ allow: effectiveGroupAllow, senderId, senderUsername })) {
              logVerbose("Blocked telegram group message from  (groupPolicy: allowlist)");
              return;
            }
          }
        ...
        if ((inlineButtonsScope === "allowlist")) {
          if (!isGroup) {
            ...
            if ((dmPolicy !== "open")) {
              const allowed = (effectiveDmAllow.hasWildcard || (effectiveDmAllow.hasEntries && isSenderAllowed({ allow: effectiveDmAllow, senderId, senderUsername })));
              if (!allowed) {
                return;
              }
            }
          } else {
            const allowed = (effectiveGroupAllow.hasWildcard || (effectiveGroupAllow.hasEntries && isSenderAllowed({ allow: effectiveGroupAllow, senderId, senderUsername })));
            if (!allowed) {
              return;
            }
          }
        ...
        const syntheticMessage = { ...callbackMessage: , from: callback.from, text: data, caption: undefined, caption_entities: undefined, entities: undefined };
        const getFile = (typeof ctx.getFile === "function") ? ctx.getFile.bind(ctx) : async () => {  };
        await processMessage({ message: syntheticMessage, me: ctx.me, getFile }, [], storeAllowFrom, { forceWasMentioned: true, messageIdOverride: callback.id });
      }
    }
    catch (err) {
      {
        runtime.error?.(danger("callback handler failed: "));
      }
    }
  });
    ...
    try {
      {
        ...
        runtime.log?.(warn("[telegram] Group migrated: \"\"  → "));
        ...
        if (migration.migrated) {
          runtime.log?.(warn("[telegram] Migrating group config from  to "));
          migrateTelegramGroupConfig({ cfg, accountId, oldChatId, newChatId });
          await writeConfigFile(currentConfig);
          runtime.log?.(warn("[telegram] Group config migrated and saved successfully"));
        } else {
          if (migration.skippedExisting) {
            runtime.log?.(warn("[telegram] Group config already exists for ; leaving  unchanged"));
          } else {
            runtime.log?.(warn("[telegram] No config found for old group ID , migration logged only"));
          }
        }
      }
    }
      ...
      {
        runtime.error?.(danger("[telegram] Group migration handler failed: "));
      }
    }
  });
    ...
    try {
      {
        ...
        if (isGroup) {
          ...
          if (hasGroupAllowOverride) {
            ...
            const allowed = ((senderId != null) && isSenderAllowed({ allow: effectiveGroupAllow, senderId: String(senderId), senderUsername }));
            if (!allowed) {
              logVerbose("Blocked telegram group sender  (group allowFrom override)");
              return;
            }
          ...
          if ((groupPolicy === "allowlist")) {
            ...
            const senderUsername = (msg.from?.username ?? "");
            if (!isSenderAllowed({ allow: effectiveGroupAllow, senderId: String(senderId), senderUsername })) {
              logVerbose("Blocked telegram group message from  (groupPolicy: allowlist)");
              return;
            }
          }
        ...
        if ((text && !isCommandLike)) {
          ...
          if (shouldStart) {
            const entry = { key, messages: [{ msg, ctx, receivedAtMs: nowMs }], timer: setTimeout(() => {
            }, TELEGRAM_TEXT_FRAGMENT_MAX_GAP_MS) };
        ...
        if (mediaGroupId) {
          ...
          if (existing) {
            ...
            const entry = { messages: [{ msg, ctx }], timer: setTimeout(async () => {
              mediaGroupBuffer.delete(mediaGroupId);
              mediaGroupProcessing = mediaGroupProcessing.then(async () => {
        ...
        try {
        ...
        catch (mediaErr) {
          {
            ...
            if ((errMsg.includes("exceeds") && errMsg.includes("MB limit"))) {
              ...
              await withTelegramApiErrorLogging({ operation: "sendMessage", runtime, fn: () => bot.api.sendMessage(chatId, "⚠️ File too large. Maximum size is MB.", { reply_to_message_id: msg.message_id }) }).catch(() => {
              });
        ...
        const conversationKey = (resolvedThreadId != null) ? ":topic:" : String(chatId);
        const debounceKey = senderId ? "telegram:::" : null;
        await inboundDebouncer.enqueue({ ctx, msg, allMedia, storeAllowFrom, debounceKey, botUsername: ctx.me?.username });
      }
    }
    catch (err) {
      {
        runtime.error?.(danger("handler failed: "));
      }
    }
  });
}
