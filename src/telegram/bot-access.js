...
export const normalizeAllowFrom = (list) => {
  ...
  return { entries: normalized, entriesLower: normalizedLower, hasWildcard, hasEntries: (entries.length > 0) };
}
...
export const isSenderAllowed = (params) => {
  ...
  return allow.entriesLower.some((entry) => ((entry === username) || (entry === "@")));
}
export const resolveSenderAllowMatch = (params) => {
  ...
  const entry = allow.entriesLower.find((candidate) => ((candidate === username) || (candidate === "@")));
  if (entry) {
