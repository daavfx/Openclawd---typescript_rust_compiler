...
describe("telegram download", () => {
  it("fetches file info", async () => {
    ...
    vi.spyOn(global, "fetch").mockResolvedValueOnce({ ok: true, status: 200, statusText: "OK", json });
    const info = await getTelegramFile("tok", "fid");
    expect(info.file_path).toBe("photos/1.jpg");
  });
  it("downloads and saves", async () => {
    const info = { file_id: "fid", file_path: "photos/1.jpg" };
    const arrayBuffer = async () => new Uint8Array([1, 2, 3, 4]).buffer;
    vi.spyOn(global, "fetch").mockResolvedValueOnce({ ok: true, status: 200, statusText: "OK", body: true, arrayBuffer, headers: { get: () => "image/jpeg" } });
