...
export function normalizeHooksPath(raw) {
  ...
  const withSlash = base.startsWith("/") ? base : "/";
...
export function resolveGmailHookRuntimeConfig(cfg, overrides) {
  ...
  const tailscaleTarget = (((tailscaleMode !== "off") && (typeof tailscaleTargetRaw === "string")) && (tailscaleTargetRaw.trim().length > 0)) ? tailscaleTargetRaw.trim() : undefined;
  ...
  return { ok: true, value: { account, label: ((overrides.label ?? gmail?.label) ?? DEFAULT_GMAIL_LABEL), topic, subscription, pushToken, hookToken, hookUrl, includeBody, maxBytes, renewEveryMinutes, serve: { bind: serveBind, port: servePort, path: servePath }, tailscale: { mode: tailscaleMode, path: tailscalePath, target: tailscaleTarget } } };
}

export function buildGogWatchStartArgs(cfg) {
  return ["gmail", "watch", "start", "--account", cfg.account, "--label", cfg.label, "--topic", cfg.topic];
}

export function buildGogWatchServeArgs(cfg) {
  const args = ["gmail", "watch", "serve", "--account", cfg.account, "--bind", cfg.serve.bind, "--port", String(cfg.serve.port), "--path", cfg.serve.path, "--token", cfg.pushToken, "--hook-url", cfg.hookUrl, "--hook-token", cfg.hookToken];
  if (cfg.includeBody) {
