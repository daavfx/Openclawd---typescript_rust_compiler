...
describe("resolveOpenClawMetadata", () => {
  it("extracts openclaw metadata from parsed frontmatter", () => {
    const frontmatter = { name: "test-hook", metadata: JSON.stringify({ openclaw: { emoji: "🔥", events: ["command:new", "command:reset"], requires: { config: ["workspace.dir"], bins: ["git"] } } }) };
  ...
  it("returns undefined when openclaw key is missing", () => {
    const frontmatter = { metadata: JSON.stringify({ other: "data" }) };
    const result = resolveOpenClawMetadata(frontmatter);
    expect(result).toBeUndefined();
  });
  it("returns undefined for invalid JSON", () => {
    const frontmatter = { metadata: "not valid json {" };
    const result = resolveOpenClawMetadata(frontmatter);
    expect(result).toBeUndefined();
  });
  it("handles install specs", () => {
    const frontmatter = { metadata: JSON.stringify({ openclaw: { events: ["command"], install: [{ id: "bundled", kind: "bundled", label: "Bundled with OpenClaw" }, { id: "npm", kind: "npm", package: "@openclaw/hook" }] } }) };
    const result = resolveOpenClawMetadata(frontmatter);
  ...
  it("handles os restrictions", () => {
    const frontmatter = { metadata: JSON.stringify({ openclaw: { events: ["command"], os: ["darwin", "linux"] } }) };
    const result = resolveOpenClawMetadata(frontmatter);
