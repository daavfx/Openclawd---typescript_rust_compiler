...
describe("hooks install (e2e)", () => {
  ...
  it("installs a hook pack and triggers the handler", async () => {
    ...
    await fs.writeFile(path.join(packDir, "package.json"), JSON.stringify({ name: "@acme/hello-hooks", version: "0.0.0", openclaw: { hooks: ["./hooks/hello-hook"] } }, null, 2), "utf-8");
    await fs.writeFile(path.join(hookDir, "HOOK.md"), ["---", "name: \"hello-hook\"", "description: \"Test hook\"", "metadata: {\"openclaw\":{\"events\":[\"command:new\"]}}", "---", "", "# Hello Hook", ""].join("
"), "utf-8");
    await fs.writeFile(path.join(hookDir, "handler.js"), "export default async function(event) { event.messages.push('hook-ok'); }
", "utf-8");
    ...
    clearInternalHooks();
    const loaded = await loadInternalHooks({ hooks: { internal: { enabled: true } } }, workspaceDir);
