...
function timeOfDayMsInTimezone(date, timeZone) {
  try {
    {
      const parts = new Intl().DateTimeFormat("en-US", { timeZone, hour: "2-digit", minute: "2-digit", second: "2-digit", hourCycle: "h23" }).formatToParts(date);
      const map = {  };
...
export function decideSoulEvil(params) {
  ...
  const inPurge = isWithinDailyPurgeWindow({ at: evil.purge?.at, duration: evil.purge?.duration, now, timeZone });
  if (inPurge) {
...
export async function applySoulEvilOverride(params) {
  const decision = decideSoulEvil({ config: params.config, userTimezone: params.userTimezone, now: params.now, random: params.random });
  if (!decision.useEvil) {
    return params.files;
  }
  ...
  try {
    ...
    {
      params.log?.warn?.("SOUL_EVIL active () but file missing: ");
      return params.files;
    }
  }
  if (!evilContent.trim()) {
    params.log?.warn?.("SOUL_EVIL active () but file empty: ");
    return params.files;
  }
  ...
  params.log?.debug?.("SOUL_EVIL active () using ");
  return updated;
}
