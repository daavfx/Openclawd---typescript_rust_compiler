  ...
  try {
    {
      ...
      if (!handlerPath) {
        console.warn("[hooks] Hook \"\" has HOOK.md but no handler file in ");
        return null;
      }
      return { name, description, source: params.source, pluginId: params.pluginId, filePath: hookMdPath, baseDir: params.hookDir, handlerPath };
    }
  }
  catch (err) {
    {
      console.warn("[hooks] Failed to load hook from :", err);
      return null;
    }
  }
}
  ...
  for (const entry of entries) {
    ...
    if ((packageHooks.length > 0)) {
      for (const hookPath of packageHooks) {
        ...
        const hook = loadHookFromDir({ hookDir: resolvedHookDir, source, pluginId, nameHint: path.basename(resolvedHookDir) });
        if (hook) {
          hooks.push(hook);
        }
      }
...
export function loadHookEntriesFromDir(params) {
  const hooks = loadHooksFromDir({ dir: params.dir, source: params.source, pluginId: params.pluginId });
  return hooks.map((hook) => {
    ...
    const entry = { hook: { ...hook: , source: params.source, pluginId: params.pluginId }, frontmatter, metadata: resolveOpenClawMetadata(frontmatter), invocation: resolveHookInvocationPolicy(frontmatter) };
    return entry;
  });
}
...
function loadHookEntries(workspaceDir, opts) {
  ...
  const bundledHooks = bundledHooksDir ? loadHooksFromDir({ dir: bundledHooksDir, source: "openclaw-bundled" }) : [];
  const extraHooks = extraDirs.flatMap((dir) => {
    const resolved = resolveUserPath(dir);
    return loadHooksFromDir({ dir: resolved, source: "openclaw-workspace" });
  });
  const managedHooks = loadHooksFromDir({ dir: managedHooksDir, source: "openclaw-managed" });
  const workspaceHooks = loadHooksFromDir({ dir: workspaceHooksDir, source: "openclaw-workspace" });
  ...
  return Array.from(merged.values()).map((hook) => {
    ...
    return { hook, frontmatter, metadata: resolveOpenClawMetadata(frontmatter), invocation: resolveHookInvocationPolicy(frontmatter) };
  });
}
export function buildWorkspaceHookSnapshot(workspaceDir, opts) {
  ...
  return { hooks: eligible.map((entry) => { name: entry.hook.name, events: (entry.metadata?.events ?? []) }), resolvedHooks: eligible.map((entry) => entry.hook), version: opts?.snapshotVersion };
}
