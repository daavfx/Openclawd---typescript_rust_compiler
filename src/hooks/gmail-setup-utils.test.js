...
describe("resolvePythonExecutablePath", () => {
  itUnix("resolves a working python path and caches the result", async () => {
...
describe("ensureTailscaleEndpoint", () => {
  it("includes stdout and exit code when tailscale serve fails", async () => {
    vi.doMock("../process/exec.js", () => { runCommandWithTimeout: vi.fn() });
    ...
    runCommand.mockResolvedValueOnce({ stdout: JSON.stringify({ Self: { DNSName: "host.tailnet.ts.net." } }), stderr: "", code: 0, signal: null, killed: false }).mockResolvedValueOnce({ stdout: "tailscale output", stderr: "Warning: client version mismatch", code: 1, signal: null, killed: false });
    let message = "";
    try {
      {
        await ensureTailscaleEndpoint({ mode: "serve", path: "/gmail-pubsub", port: 8788 });
      }
    }
    catch (err) {
      {
  ...
  it("includes JSON parse failure details with stdout", async () => {
    ...
    const {runCommandWithTimeout} = await import("../process/exec.js");
    const runCommand = vi.mocked(runCommandWithTimeout);
    runCommand.mockResolvedValueOnce({ stdout: "not-json", stderr: "", code: 0, signal: null, killed: false });
    let message = "";
    try {
      {
        await ensureTailscaleEndpoint({ mode: "funnel", path: "/gmail-pubsub", port: 8788 });
      }
    }
    catch (err) {
      {
