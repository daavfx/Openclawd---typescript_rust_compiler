...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), "openclaw-hook-install-");
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    const zip = new JSZip();
    zip.file("package/package.json", JSON.stringify({ name: "@openclaw/zip-hooks", version: "0.0.1", openclaw: { hooks: ["./hooks/zip-hook"] } }));
    zip.file("package/hooks/zip-hook/HOOK.md", ["---", "name: zip-hook", "description: Zip hook", "metadata: {\"openclaw\":{\"events\":[\"command:new\"]}}", "---", "", "# Zip Hook"].join("
"));
...
");
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(path.join(pkgDir, "package.json"), JSON.stringify({ name: "@openclaw/tar-hooks", version: "0.0.1", openclaw: { hooks: ["./hooks/tar-hook"] } }), "utf-8");
    fs.writeFileSync(path.join(pkgDir, "hooks", "tar-hook", "HOOK.md"), ["---", "name: tar-hook", "description: Tar hook", "metadata: {\"openclaw\":{\"events\":[\"command:new\"]}}", "---", "", "# Tar Hook"].join("
"), "utf-8");
    fs.writeFileSync(path.join(pkgDir, "hooks", "tar-hook", "handler.ts"), "export default async () => {};
", "utf-8");
...
describe("installHooksFromPath", () => {
  it("installs a single hook directory", async () => {
    ...
    fs.writeFileSync(path.join(hookDir, "HOOK.md"), ["---", "name: my-hook", "description: My hook", "metadata: {\"openclaw\":{\"events\":[\"command:new\"]}}", "---", "", "# My Hook"].join("
"), "utf-8");
