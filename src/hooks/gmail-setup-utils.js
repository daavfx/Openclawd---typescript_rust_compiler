...
function formatCommandFailure(command, result) {
  ...
  const signal = result.signal ? ", signal=" : "";
  const killed = result.killed ? ", killed=true" : "";
  const stderr = trimOutput(result.stderr);
  const stdout = trimOutput(result.stdout);
  const lines = [" failed (code=)"];
  if (stderr) {
    lines.push("stderr: ");
  }
  if (stdout) {
...
function ensureGcloudOnPath() {
  ...
  const candidates = ["/opt/homebrew/share/google-cloud-sdk/bin/gcloud", "/usr/local/share/google-cloud-sdk/bin/gcloud", "/opt/homebrew/Caskroom/google-cloud-sdk/latest/google-cloud-sdk/bin/gcloud", "/usr/local/Caskroom/google-cloud-sdk/latest/google-cloud-sdk/bin/gcloud"];
...
export async function resolvePythonExecutablePath() {
  ...
  for (const candidate of candidates) {
    const res = await runCommandWithTimeout([candidate, "-c", "import os, sys; print(os.path.realpath(sys.executable))"], { timeoutMs: 2000 });
    if ((res.code !== 0)) {
      continue;
    }
...
async function runGcloudCommand(args, timeoutMs) {
  return await runCommandWithTimeout(["gcloud", ...args], { timeoutMs, env: await gcloudEnv() });
}
export async function ensureDependency(bin, brewArgs) {
  ...
  if ((process.platform !== "darwin")) {
    throw new Error(" not installed; install it and retry");
  }
  if (!hasBinary("brew")) {
    throw new Error("Homebrew not installed (install brew and retry)");
  }
  const brewEnv = (bin === "gcloud") ? await gcloudEnv() : undefined;
  const result = await runCommandWithTimeout(["brew", "install", ...brewArgs], { timeoutMs: 600000, env: brewEnv });
  if ((result.code !== 0)) {
    throw new Error("brew install failed for : ");
  }
  if (!hasBinary(bin)) {
    throw new Error(" still not available after brew install");
  }
}

export async function ensureGcloudAuth() {
  const res = await runGcloudCommand(["auth", "list", "--filter", "status:ACTIVE", "--format", "value(account)"], 30000);
...
export async function ensureTopic(projectId, topicName) {
  const describe = await runGcloudCommand(["pubsub", "topics", "describe", topicName, "--project", projectId], 30000);
  if ((describe.code === 0)) {
    return;
  }
...
export async function ensureSubscription(projectId, subscription, topicName, pushEndpoint) {
  const describe = await runGcloudCommand(["pubsub", "subscriptions", "describe", subscription, "--project", projectId], 30000);
  if ((describe.code === 0)) {
    await runGcloud(["pubsub", "subscriptions", "update", subscription, "--project", projectId, "--push-endpoint", pushEndpoint]);
    return;
  }
  await runGcloud(["pubsub", "subscriptions", "create", subscription, "--project", projectId, "--topic", topicName, "--push-endpoint", pushEndpoint]);
}
...
export async function ensureTailscaleEndpoint(params) {
  ...
  const status = await runCommandWithTimeout(["tailscale", ...statusArgs], { timeoutMs: 30000 });
  if ((status.code !== 0)) {
  ...
  const funnelResult = await runCommandWithTimeout(["tailscale", ...funnelArgs], { timeoutMs: 30000 });
  if ((funnelResult.code !== 0)) {
    throw new Error(formatCommandFailure(funnelCommand, funnelResult));
  }
  const baseUrl = "https://";
  return params.token ? "?token=" : baseUrl;
}
    ...
    try {
      {
        ...
        const res = await runGcloudCommand(["projects", "list", "--filter", "projectNumber=", "--format", "value(projectId)"], 30000);
        if ((res.code !== 0)) {
          continue;
        }
