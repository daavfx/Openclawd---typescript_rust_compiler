...
export async function runGmailSetup(opts) {
  ...
  const normalizedTailscaleTarget = ((typeof configuredTailscaleTarget === "string") && (configuredTailscaleTarget.trim().length > 0)) ? configuredTailscaleTarget.trim() : undefined;
  ...
  await runGcloud(["services", "enable", "gmail.googleapis.com", "pubsub.googleapis.com", "--project", projectId, "--quiet"]);
  await ensureTopic(projectId, topicName);
  await runGcloud(["pubsub", "topics", "add-iam-policy-binding", topicName, "--project", projectId, "--member", DEFAULT_GMAIL_TOPIC_IAM_MEMBER, "--role", "roles/pubsub.publisher", "--quiet"]);
  const pushEndpoint = opts.pushEndpoint ? opts.pushEndpoint : await ensureTailscaleEndpoint({ mode: tailscaleMode, path: tailscalePath, port: servePort, target: normalizedTailscaleTarget, token: pushToken });
  if (!pushEndpoint) {
  ...
  await startGmailWatch({ account: opts.account, label, topic: topicPath }, true);
  const nextConfig = { ...baseConfig: , hooks: { ...baseConfig.hooks: , enabled: true, path: hooksPath, token: hookToken, presets: mergeHookPresets(baseConfig.hooks?.presets, "gmail"), gmail: { ...baseConfig.hooks?.gmail: , account: opts.account, label, topic: topicPath, subscription, pushToken, hookUrl, includeBody, maxBytes, renewEveryMinutes, serve: { ...baseConfig.hooks?.gmail?.serve: , bind: serveBind, port: servePort, path: servePath }, tailscale: { ...baseConfig.hooks?.gmail?.tailscale: , mode: tailscaleMode, path: tailscalePath, target: normalizedTailscaleTarget } } } };
  const validated = validateConfigObjectWithPlugins(nextConfig);
  if (!validated.ok) {
    throw new Error("Config validation failed: ");
  }
  await writeConfigFile(validated.config);
  const summary = { projectId, topic: topicPath, subscription, pushEndpoint, hookUrl, hookToken, pushToken, serve: { bind: serveBind, port: servePort, path: servePath } };
  if (opts.json) {
    defaultRuntime.log(JSON.stringify(summary, null, 2));
    return;
  }
  defaultRuntime.log("Gmail hooks configured:");
  defaultRuntime.log("- project: ");
  defaultRuntime.log("- topic: ");
  defaultRuntime.log("- subscription: ");
  defaultRuntime.log("- push endpoint: ");
  defaultRuntime.log("- hook url: ");
  defaultRuntime.log("- config: ");
  defaultRuntime.log("Next: ");
}

export async function runGmailService(opts) {
  await ensureDependency("gog", ["gogcli"]);
  const config = loadConfig();
  const overrides = { account: opts.account, topic: opts.topic, subscription: opts.subscription, label: opts.label, hookToken: opts.hookToken, pushToken: opts.pushToken, hookUrl: opts.hookUrl, serveBind: opts.bind, servePort: opts.port, servePath: opts.path, includeBody: opts.includeBody, maxBytes: opts.maxBytes, renewEveryMinutes: opts.renewEveryMinutes, tailscaleMode: opts.tailscale, tailscalePath: opts.tailscalePath, tailscaleTarget: opts.tailscaleTarget };
...
function spawnGogServe(cfg) {
  ...
  defaultRuntime.log("Starting gog ");
