...
describe("loader", () => {
  ...
  beforeEach(async () => {
    clearInternalHooks();
    tmpDir = path.join(os.tmpdir(), "openclaw-test-");
  ...
  describe("loadInternalHooks", () => {
    it("should return 0 when hooks are not enabled", async () => {
      const cfg = { hooks: { internal: { enabled: false } } };
    ...
    it("should load a handler from a module", async () => {
      ...
      const cfg = { hooks: { internal: { enabled: true, handlers: [{ event: "command:new", module: handlerPath }] } } };
      const count = await loadInternalHooks(cfg, tmpDir);
      expect(count).toBe(1);
    ...
    it("should load multiple handlers", async () => {
      ...
      const cfg = { hooks: { internal: { enabled: true, handlers: [{ event: "command:new", module: handler1Path }, { event: "command:stop", module: handler2Path }] } } };
      const count = await loadInternalHooks(cfg, tmpDir);
      expect(count).toBe(2);
      const keys = getRegisteredEventKeys();
      expect(keys).toContain("command:new");
    ...
    it("should support named exports", async () => {
      ...
      ";
      await fs.writeFile(handlerPath, handlerCode, "utf-8");
      const cfg = { hooks: { internal: { enabled: true, handlers: [{ event: "command:new", module: handlerPath, export: "myHandler" }] } } };
      const count = await loadInternalHooks(cfg, tmpDir);
      expect(count).toBe(1);
    });
    it("should handle module loading errors gracefully", async () => {
      ...
      const cfg = { hooks: { internal: { enabled: true, handlers: [{ event: "command:new", module: "/nonexistent/path/handler.js" }] } } };
      const count = await loadInternalHooks(cfg, tmpDir);
      expect(count).toBe(0);
      expect(consoleError).toHaveBeenCalledWith(expect.stringContaining("Failed to load hook handler"), expect.any(String));
    ...
    it("should handle relative paths", async () => {
      ...
      const cfg = { hooks: { internal: { enabled: true, handlers: [{ event: "command:new", module: relativePath }] } } };
      const count = await loadInternalHooks(cfg, tmpDir);
      expect(count).toBe(1);
    });
