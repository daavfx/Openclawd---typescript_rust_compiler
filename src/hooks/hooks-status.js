    ...
    if (!label) {
      if ((spec.kind === "bundled")) {
        ...
        if (((spec.kind === "npm") && spec.package)) {
          ...
          if (((spec.kind === "git") && spec.repository)) {
            label = "Install from ";
          } else {
            label = "Run installer";
          }
...
function buildHookStatus(entry, config, eligibility) {
  ...
  const missing = always ? { bins: [], anyBins: [], env: [], config: [], os: [] } : { bins: missingBins, anyBins: missingAnyBins, env: missingEnv, config: missingConfig, os: missingOs };
  ...
  return { name: entry.hook.name, description: entry.hook.description, source: entry.hook.source, pluginId: entry.hook.pluginId, filePath: entry.hook.filePath, baseDir: entry.hook.baseDir, handlerPath: entry.hook.handlerPath, hookKey, emoji, homepage, events, always, disabled, eligible, managedByPlugin, requirements: { bins: requiredBins, anyBins: requiredAnyBins, env: requiredEnv, config: requiredConfig, os: requiredOs }, missing, configChecks, install: normalizeInstallOptions(entry) };
}
export function buildWorkspaceHookStatus(workspaceDir, opts) {
  ...
  return { workspaceDir, managedHooksDir, hooks: hookEntries.map((entry) => buildHookStatus(entry, opts?.config, opts?.eligibility)) };
}
