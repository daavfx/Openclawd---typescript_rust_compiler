...
async function resolveHookNameFromDir(hookDir) {
  ...
  if (!await fileExists(hookMdPath)) {
    throw new Error("HOOK.md missing in ");
  }
...
async function validateHookDir(hookDir) {
  ...
  if (!hasHandler) {
    throw new Error("handler.ts/handler.js/index.ts/index.js missing in ");
  }
}
async function installHookPackageFromDir(params) {
  ...
  if ((params.expectedHookPackId && (params.expectedHookPackId !== hookPackId))) {
    return { ok: false, error: "hook pack id mismatch: expected , got " };
  }
  ...
  if (((mode === "install") && await fileExists(targetDir))) {
    return { ok: false, error: "hook pack already exists:  (delete it first)" };
  }
  const resolvedHooks = [];
  ...
  if (dryRun) {
    return { ok: true, hookPackId, hooks: resolvedHooks, targetDir, version: (typeof manifest.version === "string") ? manifest.version : undefined };
  }
  ...
  try {
  ...
  catch (err) {
    {
      ...
      return { ok: false, error: "failed to copy hook pack: " };
    }
  }
...
async function installHookFromDir(params) {
  ...
  if ((params.expectedHookPackId && (params.expectedHookPackId !== hookName))) {
    return { ok: false, error: "hook id mismatch: expected , got " };
  }
  const hooksDir = params.hooksDir ? resolveUserPath(params.hooksDir) : path.join(CONFIG_DIR, "hooks");
  await fs.mkdir(hooksDir, { recursive: true });
  const targetDir = resolveHookInstallDir(hookName, hooksDir);
  if (((mode === "install") && await fileExists(targetDir))) {
    return { ok: false, error: "hook already exists:  (delete it first)" };
  }
  if (dryRun) {
  ...
  try {
  ...
  catch (err) {
    {
      ...
      return { ok: false, error: "failed to copy hook: " };
    }
  }
  if (backupDir) {
    await fs.rm(backupDir, { recursive: true, force: true }).catch(() => undefined);
...
export async function installHooksFromArchive(params) {
  ...
  if (await fileExists(manifestPath)) {
    return await installHookPackageFromDir({ packageDir: rootDir, hooksDir: params.hooksDir, timeoutMs, logger, mode: params.mode, dryRun: params.dryRun, expectedHookPackId: params.expectedHookPackId });
  }
...
").map((l) => l.trim()).filter(Boolean).pop();
  ...
  return await installHooksFromArchive({ archivePath, hooksDir: params.hooksDir, timeoutMs, logger, mode, dryRun, expectedHookPackId });
}
...
export async function installHooksFromPath(params) {
  ...
  if (stat.isDirectory()) {
    ...
    if (await fileExists(manifestPath)) {
      return await installHookPackageFromDir({ packageDir: resolved, hooksDir: params.hooksDir, timeoutMs: params.timeoutMs, logger: params.logger, mode: params.mode, dryRun: params.dryRun, expectedHookPackId: params.expectedHookPackId });
    }
    return await installHookFromDir({ hookDir: resolved, hooksDir: params.hooksDir, logger: params.logger, mode: params.mode, dryRun: params.dryRun, expectedHookPackId: params.expectedHookPackId });
  }
  if (!resolveArchiveKind(resolved)) {
    return { ok: false, error: "unsupported hook file: " };
  }
  return await installHooksFromArchive({ archivePath: resolved, hooksDir: params.hooksDir, timeoutMs: params.timeoutMs, logger: params.logger, mode: params.mode, dryRun: params.dryRun, expectedHookPackId: params.expectedHookPackId });
}
