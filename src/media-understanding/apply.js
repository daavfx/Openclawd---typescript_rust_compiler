...
const EXTRA_TEXT_MIMES = ["application/xml", "text/xml", "application/x-yaml", "text/yaml", "application/yaml", "application/javascript", "text/javascript", "text/tab-separated-values"];
const TEXT_EXT_MIME = new Map([[".csv", "text/csv"], [".tsv", "text/tab-separated-values"], [".txt", "text/plain"], [".md", "text/markdown"], [".log", "text/plain"], [".ini", "text/plain"], [".cfg", "text/plain"], [".conf", "text/plain"], [".env", "text/plain"], [".json", "application/json"], [".yaml", "text/yaml"], [".yml", "text/yaml"], [".xml", "application/xml"]]);
...
async function extractFileBlocks(params) {
  ...
  for (const attachment of attachments) {
    ...
    if (((!limits.allowUrl && attachment.url) && !attachment.path)) {
      if (shouldLogVerbose()) {
        logVerbose("media: file attachment skipped (url disabled) index=");
      }
      continue;
    }
    let bufferResult;
    try {
      {
        bufferResult = await cache.getBuffer({ attachmentIndex: attachment.index, maxBytes: limits.maxBytes, timeoutMs: limits.timeoutMs });
      }
    }
    catch (err) {
      {
        if (shouldLogVerbose()) {
          logVerbose("media: file attachment skipped (buffer): ");
        }
        continue;
      }
    }
    ...
    if (((textHint && rawMime) && !rawMime.startsWith("text/"))) {
      logVerbose("media: MIME override from \"\" to \"\" for index=");
    }
    if (!mimeType) {
      if (shouldLogVerbose()) {
        logVerbose("media: file attachment skipped (unknown mime) index=");
      }
      continue;
    }
    ...
    if (!allowedMimes.has(mimeType)) {
      if (shouldLogVerbose()) {
        logVerbose("media: file attachment skipped (unsupported mime ) index=");
      }
      continue;
    }
    let extracted;
    try {
      {
        const mediaType = utf16Charset ? "; charset=" : mimeType;
        extracted = await extractFileContentFromSource({ source: { type: "base64", data: bufferResult.buffer.toString("base64"), mediaType, filename: bufferResult.fileName }, limits: { ...limits: , allowedMimes } });
      }
    }
    catch (err) {
      {
        if (shouldLogVerbose()) {
          logVerbose("media: file attachment skipped (extract): ");
        }
        continue;
      }
    }
    ...
    const safeName = (bufferResult.fileName ?? "file-").replace(/[\r\n\t]+/g, " ").trim();
...
export async function applyMediaUnderstanding(params) {
  ...
  const originalUserText = (commandCandidates.map((value) => extractMediaUserText(value)).find((value) => (value && value.trim())) ?? undefined);
  const attachments = normalizeMediaAttachments(ctx);
  const providerRegistry = buildProviderRegistry(params.providers);
  const cache = createMediaAttachmentCache(attachments);
  try {
    {
      const fileBlocks = await extractFileBlocks({ attachments, cache, limits: resolveFileLimits(cfg) });
      const tasks = CAPABILITY_ORDER.map((capability) => async () => {
        const config = cfg.tools?.media?.[capability];
        return await runCapability({ capability, cfg, ctx, attachments: cache, media: attachments, agentDir: params.agentDir, providerRegistry, config, activeModel: params.activeModel });
      });
      ...
      if (((outputs.length > 0) || (fileBlocks.length > 0))) {
        finalizeInboundContext(ctx, { forceBodyForAgent: true, forceBodyForCommands: (outputs.length > 0) });
      }
      return { outputs, decisions, appliedImage: outputs.some((output) => (output.kind === "image.description")), appliedAudio: outputs.some((output) => (output.kind === "audio.transcription")), appliedVideo: outputs.some((output) => (output.kind === "video.description")), appliedFile: (fileBlocks.length > 0) };
    }
  }
  finally {
    {
