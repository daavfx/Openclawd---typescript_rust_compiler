...
const DEFAULT_IMAGE_MODELS = { openai: "gpt-5-mini", anthropic: "claude-opus-4-5", google: "gemini-3-flash-preview", minimax: "MiniMax-VL-01" };
export 
export 
...
function candidateBinaryNames(name) {
  ...
  const pathext = (process.env.PATHEXT ?? ".EXE;.CMD;.BAT;.COM").split(";").map((item) => item.trim()).filter(Boolean).map((item) => item.startsWith(".") ? item : ".");
  const unique = Array.from(new Set(pathext));
  return [name, ...unique.map((item) => "")];
}
...
async function probeGeminiCli() {
  ...
  const resolved = async () => {
    ...
    try {
      {
        const {stdout} = await runExec("gemini", ["--output-format", "json", "ok"], { timeoutMs: 8000 });
...
async function resolveLocalWhisperCppEntry() {
  ...
  return { type: "cli", command: "whisper-cli", args: ["-m", modelPath, "-otxt", "-of", "{{OutputBase}}", "-np", "-nt", "{{MediaPath}}"] };
}
async function resolveLocalWhisperEntry() {
  if (!await hasBinary("whisper")) {
    return null;
  }
  return { type: "cli", command: "whisper", args: ["--model", "turbo", "--output_format", "txt", "--output_dir", "{{OutputDir}}", "--verbose", "False", "{{MediaPath}}"] };
}
async function resolveSherpaOnnxEntry() {
  ...
  return { type: "cli", command: "sherpa-onnx-offline", args: ["--tokens=", "--encoder=", "--decoder=", "--joiner=", "{{MediaPath}}"] };
}
...
async function resolveGeminiCliEntry(_capability) {
  ...
  return { type: "cli", command: "gemini", args: ["--output-format", "json", "--allowed-tools", "read_many_files", "--include-directories", "{{MediaDir}}", "{{Prompt}}", "Use read_many_files to read {{MediaPath}} and respond with only the text output."] };
}
...
export async function resolveAutoImageModel(params) {
  ...
  const activeEntry = await resolveActiveModelEntry({ cfg: params.cfg, agentDir: params.agentDir, providerRegistry, capability: "image", activeModel: params.activeModel });
  const resolvedActive = toActive(activeEntry);
  if (resolvedActive) {
    return resolvedActive;
  }
  const keyEntry = await resolveKeyEntry({ cfg: params.cfg, agentDir: params.agentDir, providerRegistry, capability: "image", activeModel: params.activeModel });
...
async function resolveActiveModelEntry(params) {
  ...
  try {
    {
      await resolveApiKeyForProvider({ provider: providerId, cfg: params.cfg, agentDir: params.agentDir });
    }
  }
  catch {
    {
  ...
  return { type: "provider", provider: providerId, model: params.activeModel?.model };
}
...
function resolveWhisperOutputPath(args, mediaPath) {
  ...
  return path.join(outputDir, ".txt");
}
...
function resolveProviderQuery(params) {
  ...
  const mergedOptions = normalizeProviderQuery({ ...config?.providerOptions?.[providerId]: , ...entry.providerOptions?.[providerId]:  });
...
function buildModelDecision(params) {
  if ((params.entryType === "cli")) {
    ...
    return { type: "cli", provider: (command ?? "cli"), model: (params.entry.model ?? command), outcome: params.outcome, reason: params.reason };
  }
  ...
  return { type: "provider", provider: (providerId ?? providerIdRaw), model: params.entry.model, outcome: params.outcome, reason: params.reason };
}
function formatDecisionSummary(decision) {
  ...
  const modelLabel = provider ? model ? "/" : provider : undefined;
  ...
  const viaLabel = modelLabel ? " via " : "";
  const reasonLabel = shortReason ? " reason=" : "";
  return ": ";
}
async function runProviderEntry(params) {
  ...
  if (!providerIdRaw) {
    throw new Error("Provider entry missing provider for ");
  }
  ...
  if ((capability === "image")) {
    ...
    const media = await params.cache.getBuffer({ attachmentIndex: params.attachmentIndex, maxBytes, timeoutMs });
    const provider = getMediaUnderstandingProvider(providerId, params.providerRegistry);
    const result = provider?.describeImage ? await provider.describeImage({ buffer: media.buffer, fileName: media.fileName, mime: media.mime, model: modelId, provider: providerId, prompt, timeoutMs, profile: entry.profile, preferredProfile: entry.preferredProfile, agentDir: params.agentDir, cfg: params.cfg }) : await describeImageWithModel({ buffer: media.buffer, fileName: media.fileName, mime: media.mime, model: modelId, provider: providerId, prompt, timeoutMs, profile: entry.profile, preferredProfile: entry.preferredProfile, agentDir: params.agentDir, cfg: params.cfg });
    return { kind: "image.description", attachmentIndex: params.attachmentIndex, text: trimOutput(result.text, maxChars), provider: providerId, model: (result.model ?? modelId) };
  }
  const provider = getMediaUnderstandingProvider(providerId, params.providerRegistry);
  if (!provider) {
  ...
  if ((capability === "audio")) {
    if (!provider.transcribeAudio) {
      throw new Error("Audio transcription provider \"\" not available.");
    }
    const media = await params.cache.getBuffer({ attachmentIndex: params.attachmentIndex, maxBytes, timeoutMs });
    const auth = await resolveApiKeyForProvider({ provider: providerId, cfg, profileId: entry.profile, preferredProfile: entry.preferredProfile, agentDir: params.agentDir });
    const apiKey = requireApiKey(auth, providerId);
    const providerConfig = cfg.models?.providers?.[providerId];
    const baseUrl = ((entry.baseUrl ?? params.config?.baseUrl) ?? providerConfig?.baseUrl);
    const mergedHeaders = { ...providerConfig?.headers: , ...params.config?.headers: , ...entry.headers:  };
    const headers = (Object.keys(mergedHeaders).length > 0) ? mergedHeaders : undefined;
    const providerQuery = resolveProviderQuery({ providerId, config: params.config, entry });
    const model = ((entry.model?.trim() || DEFAULT_AUDIO_MODELS[providerId]) || entry.model);
    const result = await provider.transcribeAudio({ buffer: media.buffer, fileName: media.fileName, mime: media.mime, apiKey, baseUrl, headers, model, language: ((entry.language ?? params.config?.language) ?? cfg.tools?.media?.audio?.language), prompt, query: providerQuery, timeoutMs });
    return { kind: "audio.transcription", attachmentIndex: params.attachmentIndex, text: trimOutput(result.text, maxChars), provider: providerId, model: (result.model ?? model) };
  }
  if (!provider.describeVideo) {
    throw new Error("Video understanding provider \"\" not available.");
  }
  const media = await params.cache.getBuffer({ attachmentIndex: params.attachmentIndex, maxBytes, timeoutMs });
  const estimatedBase64Bytes = estimateBase64Size(media.size);
  const maxBase64Bytes = resolveVideoMaxBase64Bytes(maxBytes);
  if ((estimatedBase64Bytes > maxBase64Bytes)) {
    throw new MediaUnderstandingSkipError("maxBytes", "Video attachment  base64 payload  exceeds ");
  }
  const auth = await resolveApiKeyForProvider({ provider: providerId, cfg, profileId: entry.profile, preferredProfile: entry.preferredProfile, agentDir: params.agentDir });
  const apiKey = requireApiKey(auth, providerId);
  const providerConfig = cfg.models?.providers?.[providerId];
  const result = await provider.describeVideo({ buffer: media.buffer, fileName: media.fileName, mime: media.mime, apiKey, baseUrl: providerConfig?.baseUrl, headers: providerConfig?.headers, model: entry.model, prompt, timeoutMs });
  return { kind: "video.description", attachmentIndex: params.attachmentIndex, text: trimOutput(result.text, maxChars), provider: providerId, model: (result.model ?? entry.model) };
}
async function runCliEntry(params) {
  ...
  if (!command) {
    throw new Error("CLI entry missing command for ");
  }
  const maxBytes = resolveMaxBytes({ capability, entry, cfg, config: params.config });
  const maxChars = resolveMaxChars({ capability, entry, cfg, config: params.config });
  const timeoutMs = resolveTimeoutMs(((entry.timeoutSeconds ?? params.config?.timeoutSeconds) ?? cfg.tools?.media?.[capability]?.timeoutSeconds), DEFAULT_TIMEOUT_SECONDS[capability]);
  const prompt = resolvePrompt(capability, ((entry.prompt ?? params.config?.prompt) ?? cfg.tools?.media?.[capability]?.prompt), maxChars);
  const pathResult = await params.cache.getPath({ attachmentIndex: params.attachmentIndex, maxBytes, timeoutMs });
  const outputDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-media-cli-"));
  const mediaPath = pathResult.path;
  const outputBase = path.join(outputDir, path.parse(mediaPath).name);
  const templCtx = { ...ctx: , MediaPath: mediaPath, MediaDir: path.dirname(mediaPath), OutputDir: outputDir, OutputBase: outputBase, Prompt: prompt, MaxChars: maxChars };
  const argv = [command, ...args].map((part, index) => (index === 0) ? part : applyTemplate(part, templCtx));
  try {
    {
      if (shouldLogVerbose()) {
        logVerbose("Media understanding via CLI: ");
      }
      const {stdout} = await runExec(argv[0], argv.slice(1), { timeoutMs, maxBuffer: CLI_OUTPUT_MAX_BUFFER });
      const resolved = await resolveCliOutput({ command, args: argv.slice(1), stdout, mediaPath });
      const text = trimOutput(resolved, maxChars);
      if (!text) {
        return null;
      }
      return { kind: (capability === "audio") ? "audio.transcription" : ".description", attachmentIndex: params.attachmentIndex, text, provider: "cli", model: command };
    }
  }
  finally {
    {
...
async function runAttachmentEntries(params) {
  ...
  for (const entry of entries) {
    ...
    try {
      {
        const result = (entryType === "cli") ? await runCliEntry({ capability, entry, cfg: params.cfg, ctx: params.ctx, attachmentIndex: params.attachmentIndex, cache: params.cache, config: params.config }) : await runProviderEntry({ capability, entry, cfg: params.cfg, ctx: params.ctx, attachmentIndex: params.attachmentIndex, cache: params.cache, agentDir: params.agentDir, providerRegistry: params.providerRegistry, config: params.config });
        if (result) {
    ...
    catch (err) {
      {
        if (isMediaUnderstandingSkipError(err)) {
          attempts.push(buildModelDecision({ entry, entryType, outcome: "skipped", reason: ": " }));
          if (shouldLogVerbose()) {
            logVerbose("Skipping  model due to : ");
          }
          continue;
        }
        attempts.push(buildModelDecision({ entry, entryType, outcome: "failed", reason: String(err) }));
        if (shouldLogVerbose()) {
          logVerbose(" understanding failed: ");
        }
      }
    }
  }
...
export async function runCapability(params) {
  ...
  if ((config?.enabled === false)) {
    return { outputs: [], decision: { capability, outcome: "disabled", attachments: [] } };
  }
  const attachmentPolicy = config?.attachments;
  const selected = selectAttachments({ capability, attachments: params.media, policy: attachmentPolicy });
  if ((selected.length === 0)) {
    return { outputs: [], decision: { capability, outcome: "no-attachment", attachments: [] } };
  }
  const scopeDecision = resolveScopeDecision({ scope: config?.scope, ctx });
  if ((scopeDecision === "deny")) {
    if (shouldLogVerbose()) {
      logVerbose(" understanding disabled by scope policy.");
    }
    return { outputs: [], decision: { capability, outcome: "scope-deny", attachments: selected.map((item) => { attachmentIndex: item.index, attempts: [] }) } };
  }
  const activeProvider = params.activeModel?.provider?.trim();
  if (((capability === "image") && activeProvider)) {
    ...
    if (modelSupportsVision(entry)) {
      ...
      return { outputs: [], decision: { capability, outcome: "skipped", attachments: selected.map((item) => {
        const attempt = { type: "provider", provider: activeProvider, model: (model || undefined), outcome: "skipped", reason };
        return { attachmentIndex: item.index, attempts: [attempt], chosen: attempt };
      }) } };
    }
  }
  const entries = resolveModelEntries({ cfg, capability, config, providerRegistry: params.providerRegistry });
  let resolvedEntries = entries;
  if ((resolvedEntries.length === 0)) {
    resolvedEntries = await resolveAutoEntries({ cfg, agentDir: params.agentDir, providerRegistry: params.providerRegistry, capability, activeModel: params.activeModel });
  }
  if ((resolvedEntries.length === 0)) {
    return { outputs: [], decision: { capability, outcome: "skipped", attachments: selected.map((item) => { attachmentIndex: item.index, attempts: [] }) } };
  }
  const outputs = [];
  const attachmentDecisions = [];
  for (const attachment of selected) {
    const {output, attempts} = await runAttachmentEntries({ capability, cfg, ctx, attachmentIndex: attachment.index, agentDir: params.agentDir, providerRegistry: params.providerRegistry, cache: params.attachments, entries: resolvedEntries, config });
    if (output) {
      outputs.push(output);
    }
    attachmentDecisions.push({ attachmentIndex: attachment.index, attempts, chosen: attempts.find((attempt) => (attempt.outcome === "success")) });
  }
  const decision = { capability, outcome: (outputs.length > 0) ? "success" : "skipped", attachments: attachmentDecisions };
  if (shouldLogVerbose()) {
    logVerbose("Media understanding ");
  }
