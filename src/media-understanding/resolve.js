...
export function resolvePrompt(capability, prompt, maxChars) {
  ...
  return " Respond in at most  characters.";
}
...
export function resolveScopeDecision(params) {
  return resolveMediaUnderstandingScope({ scope: params.scope, sessionKey: params.ctx.SessionKey, channel: (params.ctx.Surface ?? params.ctx.Provider), chatType: normalizeMediaUnderstandingChatType(params.ctx.ChatType) });
}
...
export function resolveModelEntries(params) {
  ...
  const entries = [...(config?.models ?? []).map((entry) => { entry, source: "capability" }), ...sharedModels.map((entry) => { entry, source: "shared" })];
  if ((entries.length === 0)) {
    return [];
  }
  return entries.filter(({entry, source}) => {
    const caps = (entry.capabilities && (entry.capabilities.length > 0)) ? entry.capabilities : (source === "shared") ? resolveEntryCapabilities({ entry, providerRegistry: params.providerRegistry }) : undefined;
    if ((!caps || (caps.length === 0))) {
      if ((source === "shared")) {
        if (shouldLogVerbose()) {
          logVerbose("Skipping shared media model without capabilities: ");
        }
        return false;
      }
      return true;
...
export function resolveEntriesWithActiveFallback(params) {
  const entries = resolveModelEntries({ cfg: params.cfg, capability: params.capability, config: params.config, providerRegistry: params.providerRegistry });
  if ((entries.length > 0)) {
    return entries;
  }
  ...
  return [{ type: "provider", provider: activeProvider, model: params.activeModel?.model }];
}
