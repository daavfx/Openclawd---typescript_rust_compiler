...
describe("runCapability deepgram provider options", () => {
  it("merges provider options, headers, and baseUrl overrides", async () => {
    const tmpPath = path.join(os.tmpdir(), "openclaw-deepgram-.wav");
    ...
    try {
      {
        ...
        expect(seenHeaders).toMatchObject({ "X-Provider": "1", "X-Config": "2", "X-Entry": "3" });
        expect(seenQuery).toMatchObject({ detect_language: false, punctuate: false, smart_format: true });
