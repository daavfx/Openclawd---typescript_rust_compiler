...
export async function transcribeGeminiAudio(params) {
  ...
  const url = "/models/:generateContent";
  ...
  const body = { contents: [{ role: "user", parts: [{ text: resolvePrompt(params.prompt) }, { inline_data: { mime_type: (params.mime ?? "audio/wav"), data: params.buffer.toString("base64") } }] }] };
  const res = await fetchWithTimeout(url, { method: "POST", headers, body: JSON.stringify(body) }, params.timeoutMs, fetchFn);
  if (!res.ok) {
    const detail = await readErrorResponse(res);
    const suffix = detail ? ": " : "";
    throw new Error("Audio transcription failed (HTTP )");
  }
