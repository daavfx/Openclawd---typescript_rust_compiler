...
export async function describeGeminiVideo(params) {
  ...
  const body = { contents: [{ role: "user", parts: [{ text: resolvePrompt(params.prompt) }, { inline_data: { mime_type: (params.mime ?? "video/mp4"), data: params.buffer.toString("base64") } }] }] };
  const res = await fetchWithTimeout(url, { method: "POST", headers, body: JSON.stringify(body) }, params.timeoutMs, fetchFn);
  if (!res.ok) {
    const detail = await readErrorResponse(res);
    const suffix = detail ? ": " : "";
    throw new Error("Video description failed (HTTP )");
  }
