...
const PROVIDERS = [groqProvider, openaiProvider, googleProvider, anthropicProvider, minimaxProvider, deepgramProvider];
...
export function buildMediaUnderstandingRegistry(overrides) {
  ...
  if (overrides) {
    for (const [key, provider] of Object.entries(overrides)) {
      ...
      const merged = existing ? { ...existing: , ...provider: , capabilities: (provider.capabilities ?? existing.capabilities) } : provider;
