...
function extractCronChannels(schema) {
  ...
  const payloadWithChannel = union.find((entry) => Boolean((entry?.properties && ("channel" in entry.properties))));
  const channelSchema = payloadWithChannel?.properties ? payloadWithChannel.properties.channel : undefined;
  const channels = (channelSchema?.anyOf ?? []).map((entry) => entry?.const).filter((value) => (typeof value === "string"));
  return channels;
}
const UI_FILES = ["ui/src/ui/types.ts", "ui/src/ui/ui-types.ts", "ui/src/ui/views/cron.ts"];
const SWIFT_FILE_CANDIDATES = ["/GatewayConnection.swift"];
async function resolveSwiftFiles(cwd) {
  const matches = [];
  ...
  if ((matches.length === 0)) {
    throw new Error("Missing Swift cron definition. Tried: ");
  }
  return matches;
}
describe("cron protocol conformance", () => {
  it("ui + swift include all cron providers from gateway schema", async () => {
    const channels = extractCronChannels(CronPayloadSchema);
    ...
    for (const relPath of swiftFiles) {
      const content = await fs.readFile(path.join(cwd, relPath), "utf-8");
      for (const channel of channels) {
        const pattern = new RegExp("\\bcase\\s+\\b");
        expect(pattern.test(content), " missing case ").toBe(true);
      }
    }
  });
