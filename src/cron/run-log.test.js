...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    ...
    for (let i = 0; (i < 10); i++) {
      await appendCronRunLog(logPath, { ts: (1000 + i), jobId: "job-1", action: "finished", status: "ok", durationMs: i }, { maxBytes: 1, keepLines: 3 });
    }
...
").map((l) => l.trim()).filter(Boolean);
  ...
  it("reads newest entries and filters by jobId", async () => {
    ...
    await appendCronRunLog(logPathA, { ts: 1, jobId: "a", action: "finished", status: "ok" });
    await appendCronRunLog(logPathB, { ts: 2, jobId: "b", action: "finished", status: "error", error: "nope", summary: "oops" });
    await appendCronRunLog(logPathA, { ts: 3, jobId: "a", action: "finished", status: "skipped" });
    const allA = await readCronRunLogEntries(logPathA, { limit: 10 });
    expect(allA.map((e) => e.jobId)).toEqual(["a", "a"]);
    const onlyA = await readCronRunLogEntries(logPathA, { limit: 10, jobId: "a" });
    ...
    const onlyB = await readCronRunLogEntries(logPathB, { limit: 10, jobId: "b" });
    expect(onlyB[0]?.summary).toBe("oops");
    const wrongFilter = await readCronRunLogEntries(logPathA, { limit: 10, jobId: "b" });
