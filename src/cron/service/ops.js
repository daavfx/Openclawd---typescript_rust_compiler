...
export async function start(state) {
  await locked(state, async () => {
    ...
    await persist(state);
    armTimer(state);
    state.deps.log.info({ enabled: true, jobs: (state.store?.jobs.length ?? 0), nextWakeAtMs: (nextWakeAtMs(state) ?? null) }, "cron: started");
  });
}
...
export async function status(state) {
  return await locked(state, async () => {
    await ensureLoaded(state);
    return { enabled: state.deps.cronEnabled, storePath: state.deps.storePath, jobs: (state.store?.jobs.length ?? 0), nextWakeAtMs: (state.deps.cronEnabled === true) ? (nextWakeAtMs(state) ?? null) : null };
  });
}
