...
export async function executeJob(state, job, nowMs, opts) {
  ...
  const finish = async (status, err, summary, outputText) => {
    ...
    emit(state, { jobId: job.id, action: "finished", status, error: err, summary, runAtMs: startedAt, durationMs: job.state.lastDurationMs, nextRunAtMs: job.state.nextRunAtMs });
    ...
    if ((job.sessionTarget === "isolated")) {
      ...
      if ((mode === "full")) {
        ...
        if (fullText) {
          body = (fullText.length > maxChars) ? "…" : fullText;
        }
      }
      const statusPrefix = (status === "ok") ? prefix : " ()";
      state.deps.enqueueSystemEvent(": ", { agentId: job.agentId });
      if ((job.wakeMode === "now")) {
        state.deps.requestHeartbeatNow({ reason: "cron::post" });
      }
    }
  };
  try {
        ...
        if (((job.wakeMode === "now") && state.deps.runHeartbeatOnce)) {
          ...
          for (; ; ) {
            ...
            if (((state.deps.nowMs() - waitStartedAt) > maxWaitMs)) {
              heartbeatResult = { status: "skipped", reason: "timeout waiting for main lane to become idle" };
              break;
            }
            await delay(250);
          }
          ...
          state.deps.requestHeartbeatNow({ reason: "cron:" });
          await finish("ok", undefined, text);
        }
        return;
      }
