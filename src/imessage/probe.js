...
".trim();
      ...
      if ((normalized.includes("unknown command") && normalized.includes("rpc"))) {
        const fatal = { supported: false, fatal: true, error: "imsg CLI does not support the \"rpc\" subcommand (update imsg)" };
      ...
      return { supported: false, error: (combined || "imsg rpc --help failed (code )") };
    }
  }
  catch (err) {
    {
...
export async function probeIMessage(timeoutMs = 2000, opts = {  }) {
  ...
  if (!detected) {
    return { ok: false, error: "imsg not found ()" };
  }
  const rpcSupport = await probeRpcSupport(cliPath);
  if (!rpcSupport.supported) {
    return { ok: false, error: (rpcSupport.error ?? "imsg rpc unavailable"), fatal: rpcSupport.fatal };
  }
  const client = await createIMessageRpcClient({ cliPath, dbPath, runtime: opts.runtime });
  try {
    {
