...
export async function sendMessageIMessage(to, text, opts = {  }) {
  ...
  const account = resolveIMessageAccount({ cfg, accountId: opts.accountId });
  const cliPath = ((opts.cliPath?.trim() || account.config.cliPath?.trim()) || "imsg");
  const dbPath = (opts.dbPath?.trim() || account.config.dbPath?.trim());
  const target = parseIMessageTarget(opts.chatId ? formatIMessageChatTarget(opts.chatId) : to);
  const service = ((opts.service ?? (target.kind === "handle") ? target.service : undefined) ?? account.config.service);
  ...
  if (message.trim()) {
    const tableMode = resolveMarkdownTableMode({ cfg, channel: "imessage", accountId: account.accountId });
    message = convertMarkdownTables(message, tableMode);
  }
  const params = { text: message, service: (service || "auto"), region };
  if (filePath) {
  ...
  try {
    {
      ...
      return { messageId: (resolvedId ?? result?.ok ? "ok" : "unknown") };
    }
  }
  finally {
    {
