...
vi.mock("./client.js", () => { createIMessageRpcClient: vi.fn().mockResolvedValue({ request: (...args) => requestMock(...args), stop: (...args) => stopMock(...args) }) });
vi.mock("../web/media.js", () => { loadWebMedia: vi.fn().mockResolvedValue({ buffer: Buffer.from("data"), contentType: "image/jpeg" }) });
vi.mock("../media/store.js", () => { saveMediaBuffer: vi.fn().mockResolvedValue({ path: "/tmp/imessage-media.jpg", contentType: "image/jpeg" }) });
