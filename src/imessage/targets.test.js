...
import { formatIMessageChatTarget, isAllowedIMessageSender, normalizeIMessageHandle, parseIMessageTarget } from "./targets.js";
describe("imessage targets", () => {
  ...
  it("normalizes chat_identifier prefixes case-insensitively", () => {
    expect(normalizeIMessageHandle("CHAT_IDENTIFIER:iMessage;-;chat123")).toBe("chat_identifier:iMessage;-;chat123");
  ...
  it("checks allowFrom against chat_id", () => {
    const ok = isAllowedIMessageSender({ allowFrom: ["chat_id:9"], sender: "+1555", chatId: 9 });
    expect(ok).toBe(true);
  });
  it("checks allowFrom against handle", () => {
    const ok = isAllowedIMessageSender({ allowFrom: ["user@example.com"], sender: "User@Example.com" });
    expect(ok).toBe(true);
  });
