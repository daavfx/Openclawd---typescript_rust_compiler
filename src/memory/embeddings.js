...
async function createLocalEmbeddingProvider(options) {
  ...
  return { id: "local", model: modelPath, embedQuery: async (text) => {
        ...
        try {
          {
            ...
            return { ...fallbackResult: , requestedProvider, fallbackFrom: requestedProvider, fallbackReason: reason };
          }
        }
        catch (fallbackErr) {
          {
...
function formatLocalSetupError(err) {
  ...
  return ["Local embeddings unavailable.", missing ? "Reason: optional dependency node-llama-cpp is missing (or failed to install)." : detail ? "Reason: " : undefined, (missing && detail) ? "Detail: " : null, "To enable local embeddings:", "1) Use Node 22 LTS (recommended for installs/updates)", missing ? "2) Reinstall OpenClaw (this should install node-llama-cpp): npm i -g openclaw@latest" : null, "3) If you use pnpm: pnpm approve-builds (select node-llama-cpp), then pnpm rebuild node-llama-cpp", "Or set agents.defaults.memorySearch.provider = \"openai\" (remote)."].filter(Boolean).join("
");
}
