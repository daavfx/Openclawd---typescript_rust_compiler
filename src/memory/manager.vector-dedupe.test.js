...
vi.mock("./embeddings.js", () => {
  return { createEmbeddingProvider: async () => { requestedProvider: "openai", provider: { id: "mock", model: "mock-embed", embedQuery: async () => [0.1, 0.2, 0.3], embedBatch: async (texts) => texts.map((_, index) => [(index + 1), 0, 0]) } } };
});
describe("memory vector dedupe", () => {
  ...
  it("deletes existing vector rows before inserting replacements", async () => {
    const cfg = { agents: { defaults: { workspace: workspaceDir, memorySearch: { provider: "openai", model: "mock-embed", store: { path: indexPath, vector: { enabled: true } }, sync: { watch: false, onSessionStart: false, onSearch: false }, cache: { enabled: false } } }, list: [{ id: "main", default: true }] } };
