...
export async function createGeminiEmbeddingProvider(options) {
  ...
  const embedUrl = "/:embedContent";
  const batchUrl = "/:batchEmbedContents";
  const embedQuery = async (text) => {
    if (!text.trim()) {
      return [];
    }
    const res = await fetch(embedUrl, { method: "POST", headers: client.headers, body: JSON.stringify({ content: { parts: [{ text }] }, taskType: "RETRIEVAL_QUERY" }) });
    if (!res.ok) {
      const payload = await res.text();
      throw new Error("gemini embeddings failed:  ");
    }
  ...
  const embedBatch = async (texts) => {
    ...
    const requests = texts.map((text) => { model: client.modelPath, content: { parts: [{ text }] }, taskType: "RETRIEVAL_DOCUMENT" });
    const res = await fetch(batchUrl, { method: "POST", headers: client.headers, body: JSON.stringify({ requests }) });
    if (!res.ok) {
      const payload = await res.text();
      throw new Error("gemini embeddings failed:  ");
    }
  ...
  return { provider: { id: "gemini", model: client.model, embedQuery, embedBatch }, client };
}
...
export async function resolveGeminiEmbeddingClient(options) {
  ...
  const headers = { "Content-Type": "application/json", "x-goog-api-key": apiKey, ...headerOverrides:  };
  const model = normalizeGeminiModel(options.model);
  const modelPath = buildGeminiModelPath(model);
  debugLog("memory embeddings: gemini client", { rawBaseUrl, baseUrl, model, modelPath, embedEndpoint: "/:embedContent", batchEndpoint: "/:batchEmbedContents" });
