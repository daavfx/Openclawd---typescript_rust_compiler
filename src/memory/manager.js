...
export class MemoryIndexManager {
  ...
  static async get(params) {
    ...
    const providerResult = await createEmbeddingProvider({ config: cfg, agentDir: resolveAgentDir(cfg, agentId), provider: settings.provider, remote: settings.remote, model: settings.model, fallback: settings.fallback, local: settings.local });
    const manager = new MemoryIndexManager({ cacheKey: key, cfg, agentId, workspaceDir, settings, providerResult });
  ...
  constructor(params) {
    ...
    this.cache = { enabled: params.settings.cache.enabled, maxEntries: params.settings.cache.maxEntries };
    this.fts = { enabled: params.settings.query.hybrid.enabled, available: false };
    this.ensureSchema();
    this.vector = { enabled: params.settings.store.vector.enabled, available: null, extensionPath: params.settings.store.vector.extensionPath };
  ...
  constructor(sessionKey) {
    ...
    void this.sync({ reason: "session-start" }).catch((err) => {
      log.warn("memory sync failed (session-start): ");
    });
    if (key) {
  ...
  constructor(query, opts) {
    ...
    if ((this.settings.sync.onSearch && (this.dirty || this.sessionsDirty))) {
      void this.sync({ reason: "search" }).catch((err) => {
        log.warn("memory sync failed (search): ");
      });
    }
    ...
    const vectorResults = hasVector ? await this.searchVector(queryVec, candidates).catch(() => []) : [];
    if (!hybrid.enabled) {
      return vectorResults.filter((entry) => (entry.score >= minScore)).slice(0, maxResults);
    }
    const merged = this.mergeHybridResults({ vector: vectorResults, keyword: keywordResults, vectorWeight: hybrid.vectorWeight, textWeight: hybrid.textWeight });
    return merged.filter((entry) => (entry.score >= minScore)).slice(0, maxResults);
  }
  constructor(queryVec, limit) {
    const results = await searchVector({ db: this.db, vectorTable: VECTOR_TABLE, providerModel: this.provider.model, queryVec, limit, snippetMaxChars: SNIPPET_MAX_CHARS, ensureVectorReady: async (dimensions) => await this.ensureVectorReady(dimensions), sourceFilterVec: this.buildSourceFilter("c"), sourceFilterChunks: this.buildSourceFilter() });
  ...
  constructor(query, limit) {
    ...
    const results = await searchKeyword({ db: this.db, ftsTable: FTS_TABLE, providerModel: this.provider.model, query, limit, snippetMaxChars: SNIPPET_MAX_CHARS, sourceFilter, buildFtsQuery: (raw) => this.buildFtsQuery(raw), bm25RankToScore });
    return results.map((entry) => entry);
  }
  constructor(params) {
    const merged = mergeHybridResults({ vector: params.vector.map((r) => { id: r.id, path: r.path, startLine: r.startLine, endLine: r.endLine, source: r.source, snippet: r.snippet, vectorScore: r.score }), keyword: params.keyword.map((r) => { id: r.id, path: r.path, startLine: r.startLine, endLine: r.endLine, source: r.source, snippet: r.snippet, textScore: r.textScore }), vectorWeight: params.vectorWeight, textWeight: params.textWeight });
        ...
        try {
          {
            ...
            if (stat.isDirectory()) {
              if (((absPath === additionalPath) || absPath.startsWith(""))) {
...
"), path: relPath };
  ...
  constructor() {
    const sourceFilter = this.buildSourceFilter();
    const files = this.db.prepare("SELECT COUNT(*) as c FROM files WHERE 1=1").get(...sourceFilter.params);
    const chunks = this.db.prepare("SELECT COUNT(*) as c FROM chunks WHERE 1=1").get(...sourceFilter.params);
    const sourceCounts = () => {
      ...
      const fileRows = this.db.prepare("SELECT source, COUNT(*) as c FROM files WHERE 1=1 GROUP BY source").all(...sourceFilter.params);
      for (const row of fileRows) {
        const entry = (bySource.get(row.source) ?? { files: 0, chunks: 0 });
        entry.files = (row.c ?? 0);
        bySource.set(row.source, entry);
      }
      const chunkRows = this.db.prepare("SELECT source, COUNT(*) as c FROM chunks WHERE 1=1 GROUP BY source").all(...sourceFilter.params);
    ...
    return { files: (files?.c ?? 0), chunks: (chunks?.c ?? 0), dirty: this.dirty, workspaceDir: this.workspaceDir, dbPath: this.settings.store.path, provider: this.provider.id, model: this.provider.model, requestedProvider: this.requestedProvider, sources: Array.from(this.sources), extraPaths: this.settings.extraPaths, sourceCounts, cache: this.cache.enabled ? { enabled: true, entries: (this.db.prepare("SELECT COUNT(*) as c FROM ").get()?.c ?? 0), maxEntries: this.cache.maxEntries } : { enabled: false, maxEntries: this.cache.maxEntries }, fts: { enabled: this.fts.enabled, available: this.fts.available, error: this.fts.loadError }, fallback: this.fallbackReason ? { from: (this.fallbackFrom ?? "local"), reason: this.fallbackReason } : undefined, vector: { enabled: this.vector.enabled, available: (this.vector.available ?? undefined), extensionPath: this.vector.extensionPath, loadError: this.vector.loadError, dims: this.vector.dims }, batch: { enabled: this.batch.enabled, failures: this.batchFailureCount, limit: BATCH_FAILURE_LIMIT, wait: this.batch.wait, concurrency: this.batch.concurrency, pollIntervalMs: this.batch.pollIntervalMs, timeoutMs: this.batch.timeoutMs, lastError: this.batchFailureLastError, lastProvider: this.batchFailureLastProvider } };
  }
  constructor() {
  ...
  constructor(dimensions) {
    ...
    if (!this.vectorReady) {
      this.vectorReady = this.withTimeout(this.loadVectorExtension(), VECTOR_LOAD_TIMEOUT_MS, "sqlite-vec load timed out after s");
    }
    let ready = false;
    try {
      {
    ...
    catch (err) {
      {
        ...
        log.warn("sqlite-vec unavailable: ");
        return false;
      }
    }
  ...
  constructor() {
    ...
    try {
      {
        const resolvedPath = this.vector.extensionPath?.trim() ? resolveUserPath(this.vector.extensionPath) : undefined;
  ...
  constructor(dimensions) {
    ...
    this.db.exec(((("CREATE VIRTUAL TABLE IF NOT EXISTS  USING vec0(
" + "  id TEXT PRIMARY KEY,
") + "  embedding FLOAT[]
") + ")"));
  ...
  constructor() {
    try {
      {
        this.db.exec("DROP TABLE IF EXISTS ");
      }
    }
    catch (err) {
      {
        const message = (err instanceof Error) ? err.message : String(err);
        log.debug("Failed to drop : ");
      }
    }
  }
  constructor(alias) {
    ...
    const column = alias ? ".source" : "source";
    const placeholders = sources.map(() => "?").join(", ");
    return { sql: " AND  IN ()", params: sources };
  }
  constructor() {
  ...
  constructor(sourceDb) {
    ...
    try {
      {
        const rows = sourceDb.prepare("SELECT provider, model, provider_key, hash, embedding, dims, updated_at FROM ").all();
        if (!rows.length) {
          return;
        }
        const insert = this.db.prepare("INSERT INTO  (provider, model, provider_key, hash, embedding, dims, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)
  ...
  constructor(targetPath, tempPath) {
  ...
  constructor(basePath) {
    const suffixes = ["", "-wal", "-shm"];
    await Promise.all(suffixes.map((suffix) => fs.rm("", { force: true })));
  }
  constructor() {
    const result = ensureMemoryIndexSchema({ db: this.db, embeddingCacheTable: EMBEDDING_CACHE_TABLE, ftsTable: FTS_TABLE, ftsEnabled: this.fts.enabled });
    this.fts.available = result.ftsAvailable;
    if (result.ftsError) {
      this.fts.loadError = result.ftsError;
      log.warn("fts unavailable: ");
    }
  }
  constructor() {
    ...
    const watchPaths = new Set([path.join(this.workspaceDir, "MEMORY.md"), path.join(this.workspaceDir, "memory.md"), path.join(this.workspaceDir, "memory"), ...additionalPaths]);
    this.watcher = chokidar.watch(Array.from(watchPaths), { ignoreInitial: true, awaitWriteFinish: { stabilityThreshold: this.settings.sync.watchDebounceMs, pollInterval: 100 } });
    const markDirty = () => {
      this.dirty = true;
  ...
  constructor(sessionFile) {
    ...
    this.sessionWatchTimer = setTimeout(() => {
      ...
      void this.processSessionDeltaBatch().catch((err) => {
        log.warn("memory session delta failed: ");
      });
  ...
  constructor() {
    ...
    if (shouldSync) {
      void this.sync({ reason: "session-delta" }).catch((err) => {
        log.warn("memory sync failed (session-delta): ");
      });
    }
  }
  constructor(sessionFile) {
    ...
    if (((deltaBytes === 0) && (size === state.lastSize))) {
      return { deltaBytes: thresholds.deltaBytes, deltaMessages: thresholds.deltaMessages, pendingBytes: state.pendingBytes, pendingMessages: state.pendingMessages };
    }
  ...
  constructor() {
    ...
    this.intervalTimer = setInterval(() => {
      void this.sync({ reason: "interval" }).catch((err) => {
        log.warn("memory sync failed (interval): ");
      });
    }, ms);
  }
  constructor() {
    ...
    this.watchTimer = setTimeout(() => {
      ...
      void this.sync({ reason: "watch" }).catch((err) => {
        log.warn("memory sync failed (watch): ");
      });
    }, this.settings.sync.watchDebounceMs);
  }
  constructor(params, needsFullReindex = false) {
  ...
  constructor(params) {
    ...
    log.debug("memory sync: indexing memory files", { files: fileEntries.length, needsFullReindex: params.needsFullReindex, batch: this.batch.enabled, concurrency: this.getIndexConcurrency() });
    const activePaths = new Set(fileEntries.map((entry) => entry.path));
    if (params.progress) {
      params.progress.total += fileEntries.length;
      params.progress.report({ completed: params.progress.completed, total: params.progress.total, label: this.batch.enabled ? "Indexing memory files (batch)..." : "Indexing memory files…" });
    }
    ...
    for (const stale of staleRows) {
      ...
      try {
        {
          this.db.prepare("DELETE FROM  WHERE id IN (SELECT id FROM chunks WHERE path = ? AND source = ?)").run(stale.path, "memory");
        }
      }
      catch {
        {
      ...
      if ((this.fts.enabled && this.fts.available)) {
        try {
          {
            this.db.prepare("DELETE FROM  WHERE path = ? AND source = ? AND model = ?").run(stale.path, "memory", this.provider.model);
          }
        }
        catch {
          {
  ...
  constructor(params) {
    ...
    log.debug("memory sync: indexing session files", { files: files.length, indexAll, dirtyFiles: this.sessionsDirtyFiles.size, batch: this.batch.enabled, concurrency: this.getIndexConcurrency() });
    if (params.progress) {
      params.progress.total += files.length;
      params.progress.report({ completed: params.progress.completed, total: params.progress.total, label: this.batch.enabled ? "Indexing session files (batch)..." : "Indexing session files…" });
    }
    ...
    for (const stale of staleRows) {
      ...
      try {
        {
          this.db.prepare("DELETE FROM  WHERE id IN (SELECT id FROM chunks WHERE path = ? AND source = ?)").run(stale.path, "sessions");
        }
      }
      catch {
        {
      ...
      if ((this.fts.enabled && this.fts.available)) {
        try {
          {
            this.db.prepare("DELETE FROM  WHERE path = ? AND source = ? AND model = ?").run(stale.path, "sessions", this.provider.model);
          }
        }
        catch {
          {
  ...
  constructor(onProgress) {
    const state = { completed: 0, total: 0, label: undefined, report: (update) => {
      if (update.label) {
        state.label = update.label;
      }
      const label = ((update.total > 0) && state.label) ? " /" : state.label;
  ...
  constructor(params) {
    ...
    if (progress) {
      progress.report({ completed: progress.completed, total: progress.total, label: "Loading vector extension…" });
    }
    ...
    try {
      {
        if (needsFullReindex) {
          await this.runSafeReindex({ reason: params?.reason, force: params?.force, progress: (progress ?? undefined) });
          return;
        }
    ...
    catch (err) {
      {
        ...
        if (activated) {
          await this.runSafeReindex({ reason: (params?.reason ?? "fallback"), force: true, progress: (progress ?? undefined) });
          return;
        }
        throw err;
      }
  ...
  constructor() {
    ...
    const enabled = Boolean((batch?.enabled && ((this.openAi && (this.provider.id === "openai")) || (this.gemini && (this.provider.id === "gemini")))));
    return { enabled, wait: (batch?.wait ?? true), concurrency: Math.max(1, (batch?.concurrency ?? 2)), pollIntervalMs: (batch?.pollIntervalMs ?? 2000), timeoutMs: (((batch?.timeoutMinutes ?? 60) * 60) * 1000) };
  }
  constructor(reason) {
    ...
    const fallbackResult = await createEmbeddingProvider({ config: this.cfg, agentDir: resolveAgentDir(this.cfg, this.agentId), provider: fallback, remote: this.settings.remote, model: fallbackModel, fallback: "none", local: this.settings.local });
    ...
    this.providerKey = this.computeProviderKey();
    this.batch = this.resolveBatchConfig();
    log.warn("memory embeddings: switched to fallback provider ()", { reason });
    return true;
  }
  constructor(params) {
    const dbPath = resolveUserPath(this.settings.store.path);
    const tempDbPath = ".tmp-";
    const tempDb = this.openDatabaseAtPath(tempDbPath);
    const originalDb = this.db;
    let originalDbClosed = false;
    const originalState = { ftsAvailable: this.fts.available, ftsError: this.fts.loadError, vectorAvailable: this.vector.available, vectorLoadError: this.vector.loadError, vectorDims: this.vector.dims, vectorReady: this.vectorReady };
    ...
    try {
      {
        ...
        nextMeta = { model: this.provider.model, provider: this.provider.id, providerKey: this.providerKey, chunkTokens: this.settings.chunking.tokens, chunkOverlap: this.settings.chunking.overlap };
...
");
        return { path: this.sessionPathForFile(absPath), absPath, mtimeMs: stat.mtimeMs, size: stat.size, hash: hashText(content), content };
      }
    }
    catch (err) {
      {
        log.debug("Failed reading session file : ");
        return null;
      }
    }
  }
  ...
  constructor(hashes) {
    ...
    for (let start = 0; (start < unique.length); start += batchSize) {
      ...
      const rows = this.db.prepare(("SELECT hash, embedding FROM 
" + " WHERE provider = ? AND model = ? AND provider_key = ? AND hash IN ()")).all(...baseParams, ...batch);
      for (const row of rows) {
  ...
  constructor(entries) {
    ...
    const now = Date.now();
    const stmt = this.db.prepare(((((("INSERT INTO  (provider, model, provider_key, hash, embedding, dims, updated_at)
" + " VALUES (?, ?, ?, ?, ?, ?, ?)
") + " ON CONFLICT(provider, model, provider_key, hash) DO UPDATE SET
") + "   embedding=excluded.embedding,
") + "   dims=excluded.dims,
") + "   updated_at=excluded.updated_at"));
  ...
  constructor() {
    ...
    const row = this.db.prepare("SELECT COUNT(*) as c FROM ").get();
    const count = (row?.c ?? 0);
    if ((count <= max)) {
      return;
    }
...
") + " )")).run(excess);
  ...
  constructor() {
    if (((this.provider.id === "openai") && this.openAi)) {
      const entries = Object.entries(this.openAi.headers).filter(([key]) => (key.toLowerCase() !== "authorization")).sort(([a], [b]) => a.localeCompare(b)).map(([key, value]) => [key, value]);
      return hashText(JSON.stringify({ provider: "openai", baseUrl: this.openAi.baseUrl, model: this.openAi.model, headers: entries }));
    }
    if (((this.provider.id === "gemini") && this.gemini)) {
      ...
      return hashText(JSON.stringify({ provider: "gemini", baseUrl: this.gemini.baseUrl, model: this.gemini.model, headers: entries }));
    }
    return hashText(JSON.stringify({ provider: this.provider.id, model: this.provider.model }));
  }
  constructor(chunks, entry, source) {
    if (((this.provider.id === "openai") && this.openAi)) {
  ...
  constructor(chunks, entry, source) {
    ...
    for (const item of missing) {
      ...
      requests.push({ custom_id: customId, method: "POST", url: OPENAI_BATCH_ENDPOINT, body: { model: (this.openAi?.model ?? this.provider.model), input: chunk.text } });
    }
  ...
  constructor(chunks, entry, source) {
    ...
    for (const item of missing) {
      ...
      const customId = hashText(":::::");
      mapping.set(customId, { index: item.index, hash: chunk.hash });
      requests.push({ custom_id: customId, content: { parts: [{ text: chunk.text }] }, taskType: "RETRIEVAL_DOCUMENT" });
    }
      ...
      try {
        {
          ...
          log.debug("memory embeddings: batch start", { provider: this.provider.id, items: texts.length, timeoutMs });
          return await this.withTimeout(this.provider.embedBatch(texts), timeoutMs, "memory embeddings batch timed out after s");
        }
      }
      catch (err) {
        {
          ...
          log.warn("memory embeddings rate limited; retrying in ms");
  ...
  constructor(text) {
    ...
    return await this.withTimeout(this.provider.embedQuery(text), timeoutMs, "memory embeddings query timed out after s");
  }
    ...
    try {
    ...
    catch (err) {
      {
        ...
        if (this.isBatchTimeoutError(message)) {
          log.warn("memory embeddings:  batch timed out; retrying once");
          try {
            {
              return await params.run();
            }
  ...
  async runBatchWithFallback(params) {
    ...
    try {
      {
        const result = await this.runBatchWithTimeoutRetry({ provider: params.provider, run: params.run });
    ...
    catch (err) {
      {
        ...
        const failure = await this.recordBatchFailure({ provider: params.provider, message, attempts, forceDisable });
        const suffix = failure.disabled ? "disabling batch" : "keeping batch enabled";
        log.warn("memory embeddings:  batch failed (/); ; falling back to non-batch embeddings: ");
        return await params.fallback();
      }
    }
  }
  ...
  constructor(entry, options) {
    ...
    if (vectorReady) {
      try {
        {
          this.db.prepare("DELETE FROM  WHERE id IN (SELECT id FROM chunks WHERE path = ? AND source = ?)").run(entry.path, options.source);
        }
      }
      catch {
        {
    ...
    if ((this.fts.enabled && this.fts.available)) {
      try {
        {
          this.db.prepare("DELETE FROM  WHERE path = ? AND source = ? AND model = ?").run(entry.path, options.source, this.provider.model);
        }
      }
      catch {
        {
    ...
    for (let i = 0; (i < chunks.length); i++) {
      ...
      if ((vectorReady && (embedding.length > 0))) {
        try {
          {
            this.db.prepare("DELETE FROM  WHERE id = ?").run(id);
          }
        }
        catch {
          {
        ...
        this.db.prepare("INSERT INTO  (id, embedding) VALUES (?, ?)").run(id, vectorToBlob(embedding));
      }
      if ((this.fts.enabled && this.fts.available)) {
        this.db.prepare(("INSERT INTO  (text, id, path, source, model, start_line, end_line)
