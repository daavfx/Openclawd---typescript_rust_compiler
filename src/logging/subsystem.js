...
function formatConsoleLine(opts) {
  ...
  if ((opts.style === "json")) {
    return JSON.stringify({ time: new Date().toISOString(), level: opts.level, subsystem: displaySubsystem, message: opts.message, ...opts.meta:  });
  }
...
export function createSubsystemLogger(subsystem) {
  ...
  const emit = (level, message, meta) => {
    ...
    if (((!isVerbose() && (subsystem === "agent/embedded")) && /(sessionId|runId)=probe-/.test(consoleMessage))) {
      return;
    }
    const line = formatConsoleLine({ level, subsystem, message: (consoleSettings.style === "json") ? message : consoleMessage, style: consoleSettings.style, meta: fileMeta });
    writeConsoleLine(level, line);
  };
  const logger = { subsystem, trace: (message, meta) => emit("trace", message, meta), debug: (message, meta) => emit("debug", message, meta), info: (message, meta) => emit("info", message, meta), warn: (message, meta) => emit("warn", message, meta), error: (message, meta) => emit("error", message, meta), fatal: (message, meta) => emit("fatal", message, meta), raw: (message) => {
    logToFile(getFileLogger(), "info", message, { raw: true });
    if (shouldLogSubsystemToConsole(subsystem)) {
      if (((!isVerbose() && (subsystem === "agent/embedded")) && /(sessionId|runId)=probe-/.test(message))) {
        return;
      }
      writeConsoleLine("info", message);
    }
  }, child: (name) => createSubsystemLogger("/") };
  return logger;
}

export function runtimeForLogger(logger, exit = defaultRuntime.exit) {
  return { log: (message) => logger.info(message), error: (message) => logger.error(message), exit };
}
