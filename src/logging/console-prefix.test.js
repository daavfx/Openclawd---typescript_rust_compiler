...
describe("stripRedundantSubsystemPrefixForConsole", () => {
  ...
  it("drops '<subsystem> ' prefix", () => {
    expect(stripRedundantSubsystemPrefixForConsole("discord gateway: closed", "discord")).toBe("gateway: closed");
  });
  it("drops '[subsystem]' prefix", () => {
    expect(stripRedundantSubsystemPrefixForConsole("[discord] connection stalled", "discord")).toBe("connection stalled");
  });
