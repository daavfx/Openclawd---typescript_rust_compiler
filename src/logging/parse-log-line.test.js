...
describe("parseLogLine", () => {
  it("parses structured JSON log lines", () => {
    const line = JSON.stringify({ time: "2026-01-09T01:38:41.523Z", 0: "{\"subsystem\":\"gateway/channels/whatsapp\"}", 1: "connected", _meta: { name: "{\"subsystem\":\"gateway/channels/whatsapp\"}", logLevelName: "INFO" } });
  ...
  it("falls back to meta timestamp when top-level time is missing", () => {
    const line = JSON.stringify({ 0: "hello", _meta: { name: "{\"subsystem\":\"gateway\"}", logLevelName: "WARN", date: "2026-01-09T02:10:00.000Z" } });
    const parsed = parseLogLine(line);
