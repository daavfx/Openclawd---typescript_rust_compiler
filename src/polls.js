...
export function normalizePollInput(input, options = {  }) {
  ...
  if (((options.maxOptions !== undefined) && (cleaned.length > options.maxOptions))) {
    throw new Error("Poll supports at most  options");
  }
  ...
  const durationHours = ((typeof durationRaw === "number") && Number.isFinite(durationRaw)) ? Math.floor(durationRaw) : undefined;
