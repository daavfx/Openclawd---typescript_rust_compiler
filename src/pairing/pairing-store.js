...
const PAIRING_STORE_LOCK_OPTIONS = { retries: { retries: 10, factor: 2, minTimeout: 100, maxTimeout: 10000, randomize: true }, stale: 30000 };
export 
export 
...
function resolvePairingPath(channel, env = process.env) {
  return path.join(resolveCredentialsDir(env), "-pairing.json");
}
function resolveAllowFromPath(channel, env = process.env) {
  return path.join(resolveCredentialsDir(env), "-allowFrom.json");
}
...
export async function readChannelAllowFromStore(channel, env = process.env) {
  ...
  const {value} = await readJsonFile(filePath, { version: 1, allowFrom: [] });
...
export async function addChannelAllowFromStoreEntry(params) {
  ...
  return await withFileLock(filePath, { version: 1, allowFrom: [] }, async () => {
    ...
    await writeJsonFile(filePath, { version: 1, allowFrom: next });
...
export async function listChannelPairingRequests(channel, env = process.env) {
  ...
  return await withFileLock(filePath, { version: 1, requests: [] }, async () => {
    const {value} = await readJsonFile(filePath, { version: 1, requests: [] });
    ...
    return pruned.filter((r) => (((r && (typeof r.id === "string")) && (typeof r.code === "string")) && (typeof r.createdAt === "string"))).slice().sort((a, b) => a.createdAt.localeCompare(b.createdAt));
  });
}
...
export async function upsertChannelPairingRequest(params) {
  ...
  return await withFileLock(filePath, { version: 1, requests: [] }, async () => {
    ...
    const meta = (params.meta && (typeof params.meta === "object")) ? Object.fromEntries(Object.entries(params.meta).map(([k, v]) => [k, String((v ?? "")).trim()]).filter(([_, v]) => Boolean(v))) : undefined;
    ...
    if ((existingIdx >= 0)) {
      ...
      const next = { id, code, createdAt: (existing?.createdAt ?? now), lastSeenAt: now, meta: (meta ?? existing?.meta) };
    ...
    const next = { id, code, createdAt: now, lastSeenAt: now, ...meta ? { meta } : {  }:  };
    await writeJsonFile(filePath, { version: 1, requests: [...reqs, next] });
...
export async function approveChannelPairingCode(params) {
  ...
  return await withFileLock(filePath, { version: 1, requests: [] }, async () => {
    ...
    pruned.splice(idx, 1);
    await writeJsonFile(filePath, { version: 1, requests: pruned });
    await addChannelAllowFromStoreEntry({ channel: params.channel, entry: entry.id, env });
