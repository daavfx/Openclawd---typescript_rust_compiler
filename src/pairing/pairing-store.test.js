...
describe("pairing store", () => {
  it("reuses pending code and reports created=false", async () => {
    await withTempStateDir(async () => {
      const first = await upsertChannelPairingRequest({ channel: "discord", id: "u1" });
      const second = await upsertChannelPairingRequest({ channel: "discord", id: "u1" });
  ...
  it("expires pending requests after TTL", async () => {
    await withTempStateDir(async (stateDir) => {
      const created = await upsertChannelPairingRequest({ channel: "signal", id: "+15550001111" });
      ...
      const requests = (parsed.requests ?? []).map((entry) => { ...entry: , createdAt: expiredAt, lastSeenAt: expiredAt });
      await fs.writeFile(filePath, "
", "utf8");
      const list = await listChannelPairingRequests("signal");
      expect(list).toHaveLength(0);
      const next = await upsertChannelPairingRequest({ channel: "signal", id: "+15550001111" });
      ...
      try {
        {
          ...
          const first = await upsertChannelPairingRequest({ channel: "telegram", id: "123" });
          expect(first.code).toBe("AAAAAAAA");
          const sequence = Array(8).fill(0).concat(Array(8).fill(1));
          let idx = 0;
          spy.mockImplementation(() => (sequence[idx++] ?? 1));
          const second = await upsertChannelPairingRequest({ channel: "telegram", id: "456" });
  ...
  it("caps pending requests at the default limit", async () => {
    await withTempStateDir(async () => {
      ...
      for (const id of ids) {
        const created = await upsertChannelPairingRequest({ channel: "whatsapp", id });
        expect(created.created).toBe(true);
      }
      const blocked = await upsertChannelPairingRequest({ channel: "whatsapp", id: "+15550000004" });
