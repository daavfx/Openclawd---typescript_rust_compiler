...
function resolveScopeDecision(params) {
  return resolveMediaUnderstandingScope({ scope: params.config?.scope, sessionKey: params.ctx.SessionKey, channel: (params.ctx.Surface ?? params.ctx.Provider), chatType: normalizeMediaUnderstandingChatType(params.ctx.ChatType) });
}
...
async function runCliEntry(params) {
  ...
  const templCtx = { ...params.ctx: , LinkUrl: params.url };
  const argv = [command, ...args].map((part, index) => (index === 0) ? part : applyTemplate(part, templCtx));
  if (shouldLogVerbose()) {
    logVerbose("Link understanding via CLI: ");
  }
...
async function runLinkEntries(params) {
  ...
  for (const entry of params.entries) {
    try {
      {
        const output = await runCliEntry({ entry, ctx: params.ctx, url: params.url, config: params.config });
        if (output) {
          return output;
        }
      }
    ...
    catch (err) {
      {
        lastError = err;
        if (shouldLogVerbose()) {
          logVerbose("Link understanding failed for : ");
        }
      }
    }
  }
  if ((lastError && shouldLogVerbose())) {
    logVerbose("Link understanding exhausted for ");
  }
  return null;
}
export async function runLinkUnderstanding(params) {
  ...
  for (const url of links) {
    const output = await runLinkEntries({ entries, ctx: params.ctx, url, config });
    if (output) {
      outputs.push(output);
    }
  }
