...
describe("gateway.remote.transport", () => {
  it("accepts direct transport", async () => {
    ...
    const res = validateConfigObject({ gateway: { remote: { transport: "direct", url: "wss://gateway.example.ts.net" } } });
    expect(res.ok).toBe(true);
  });
  it("rejects unknown transport", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ gateway: { remote: { transport: "udp" } } });
