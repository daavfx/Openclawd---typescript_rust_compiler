...
describe("legacy config detection", () => {
  ...
  it("rejects routing.groupChat.requireMention", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ routing: { groupChat: { requireMention: false } } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it("migrates routing.allowFrom to channels.whatsapp.allowFrom when whatsapp configured", async () => {
    ...
    const res = migrateLegacyConfig({ routing: { allowFrom: ["+15555550123"] }, channels: { whatsapp: {  } } });
  ...
  it("drops routing.allowFrom when whatsapp missing", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ routing: { allowFrom: ["+15555550123"] } });
  ...
  it("migrates routing.groupChat.requireMention to channels whatsapp/telegram/imessage groups when whatsapp configured", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ routing: { groupChat: { requireMention: false } }, channels: { whatsapp: {  } } });
    expect(res.changes).toContain("Moved routing.groupChat.requireMention → channels.whatsapp.groups.\"*\".requireMention.");
    expect(res.changes).toContain("Moved routing.groupChat.requireMention → channels.telegram.groups.\"*\".requireMention.");
    expect(res.changes).toContain("Moved routing.groupChat.requireMention → channels.imessage.groups.\"*\".requireMention.");
  ...
  it("migrates routing.groupChat.requireMention to telegram/imessage when whatsapp missing", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ routing: { groupChat: { requireMention: false } } });
    expect(res.changes).toContain("Moved routing.groupChat.requireMention → channels.telegram.groups.\"*\".requireMention.");
    expect(res.changes).toContain("Moved routing.groupChat.requireMention → channels.imessage.groups.\"*\".requireMention.");
    expect(res.changes).not.toContain("Moved routing.groupChat.requireMention → channels.whatsapp.groups.\"*\".requireMention.");
    expect(res.config?.channels?.whatsapp).toBeUndefined();
    expect(res.config?.channels?.telegram?.groups?.["*"]?.requireMention).toBe(false);
    expect(res.config?.channels?.imessage?.groups?.["*"]?.requireMention).toBe(false);
    expect(res.config?.routing?.groupChat?.requireMention).toBeUndefined();
  ...
  it("migrates routing.groupChat.mentionPatterns to messages.groupChat.mentionPatterns", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ routing: { groupChat: { mentionPatterns: ["@openclaw"] } } });
    expect(res.changes).toContain("Moved routing.groupChat.mentionPatterns → messages.groupChat.mentionPatterns.");
  ...
  it("migrates routing agentToAgent/queue/transcribeAudio to tools/messages/media", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ routing: { agentToAgent: { enabled: true, allow: ["main"] }, queue: { mode: "queue", cap: 3 }, transcribeAudio: { command: ["whisper", "--model", "base"], timeoutSeconds: 2 } } });
    ...
    expect(res.config?.tools?.agentToAgent).toEqual({ enabled: true, allow: ["main"] });
    expect(res.config?.messages?.queue).toEqual({ mode: "queue", cap: 3 });
    expect(res.config?.tools?.media?.audio).toEqual({ enabled: true, models: [{ command: "whisper", type: "cli", args: ["--model", "base"], timeoutSeconds: 2 }] });
    expect(res.config?.routing).toBeUndefined();
  });
  it("migrates agent config into agents.defaults and tools", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ agent: { model: "openai/gpt-5.2", tools: { allow: ["sessions.list"], deny: ["danger"] }, elevated: { enabled: true, allowFrom: { discord: ["user:1"] } }, bash: { timeoutSec: 12 }, sandbox: { tools: { allow: ["browser.open"] } }, subagents: { tools: { deny: ["sandbox"] } } } });
    ...
    expect(res.config?.agents?.defaults?.model).toEqual({ primary: "openai/gpt-5.2", fallbacks: [] });
    expect(res.config?.tools?.allow).toEqual(["sessions.list"]);
    expect(res.config?.tools?.deny).toEqual(["danger"]);
    expect(res.config?.tools?.elevated).toEqual({ enabled: true, allowFrom: { discord: ["user:1"] } });
    expect(res.config?.tools?.exec).toEqual({ timeoutSec: 12 });
    expect(res.config?.tools?.sandbox?.tools).toEqual({ allow: ["browser.open"] });
    expect(res.config?.tools?.subagents?.tools).toEqual({ deny: ["sandbox"] });
    expect(res.config.agent).toBeUndefined();
  });
  it("migrates tools.bash to tools.exec", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ tools: { bash: { timeoutSec: 12 } } });
  ...
  it("accepts per-agent tools.elevated overrides", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ tools: { elevated: { allowFrom: { whatsapp: ["+15555550123"] } } }, agents: { list: [{ id: "work", workspace: "~/openclaw-work", tools: { elevated: { enabled: false, allowFrom: { whatsapp: ["+15555550123"] } } } }] } });
    expect(res.ok).toBe(true);
    if (res.ok) {
      expect(res.config?.agents?.list?.[0]?.tools?.elevated).toEqual({ enabled: false, allowFrom: { whatsapp: ["+15555550123"] } });
    }
  });
  it("rejects telegram.requireMention", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ telegram: { requireMention: true } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it("rejects gateway.token", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ gateway: { token: "legacy-token" } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it("migrates gateway.token to gateway.auth.token", async () => {
    vi.resetModules();
    const {migrateLegacyConfig} = await import("./config.js");
    const res = migrateLegacyConfig({ gateway: { token: "legacy-token" } });
  ...
  it("keeps gateway.bind tailnet", async () => {
    ...
    const res = migrateLegacyConfig({ gateway: { bind: "tailnet" } });
  ...
  it("rejects telegram.dmPolicy=\"open\" without allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { telegram: { dmPolicy: "open", allowFrom: ["123456789"] } } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it("accepts telegram.dmPolicy=\"open\" with allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { telegram: { dmPolicy: "open", allowFrom: ["*"] } } });
    expect(res.ok).toBe(true);
    if (res.ok) {
  ...
  it("rejects whatsapp.dmPolicy=\"open\" without allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { whatsapp: { dmPolicy: "open", allowFrom: ["+15555550123"] } } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it("accepts whatsapp.dmPolicy=\"open\" with allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { whatsapp: { dmPolicy: "open", allowFrom: ["*"] } } });
    expect(res.ok).toBe(true);
    if (res.ok) {
  ...
  it("rejects signal.dmPolicy=\"open\" without allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { signal: { dmPolicy: "open", allowFrom: ["+15555550123"] } } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
  ...
  it("accepts signal.dmPolicy=\"open\" with allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { signal: { dmPolicy: "open", allowFrom: ["*"] } } });
    expect(res.ok).toBe(true);
    if (res.ok) {
  ...
  it("rejects imessage.dmPolicy=\"open\" without allowFrom \"*\"", async () => {
    vi.resetModules();
    const {validateConfigObject} = await import("./config.js");
    const res = validateConfigObject({ channels: { imessage: { dmPolicy: "open", allowFrom: ["+15555550123"] } } });
    expect(res.ok).toBe(false);
    if (!res.ok) {
