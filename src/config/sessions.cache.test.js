...
describe("Session Store Cache", () => {
  ...
  let storePath;
  beforeEach(() => {
    testDir = path.join(os.tmpdir(), "session-cache-test-");
  ...
  it("should load session store from disk on first call", async () => {
    const testStore = { "session:1": { sessionId: "id-1", updatedAt: Date.now(), displayName: "Test Session 1" } };
  ...
  it("should not allow cached session mutations to leak across loads", async () => {
    const testStore = { "session:1": { sessionId: "id-1", updatedAt: Date.now(), cliSessionIds: { openai: "sess-1" }, skillsSnapshot: { prompt: "skills", skills: [{ name: "alpha" }] } } };
    await saveSessionStore(storePath, testStore);
    const loaded1 = loadSessionStore(storePath);
  ...
  it("should refresh cache when store file changes on disk", async () => {
    ...
    const loaded1 = loadSessionStore(storePath);
    expect(loaded1).toEqual(testStore);
    const modifiedStore = { "session:99": { sessionId: "id-99", updatedAt: Date.now() } };
  ...
  it("should invalidate cache on write", async () => {
    ...
    const loaded1 = loadSessionStore(storePath);
    expect(loaded1).toEqual(testStore);
    const updatedStore = { "session:1": { ...testStore["session:1"]: , displayName: "Updated Session 1" } };
  ...
  it("should respect OPENCLAW_SESSION_CACHE_TTL_MS=0 to disable cache", async () => {
    ...
    const loaded1 = loadSessionStore(storePath);
    expect(loaded1).toEqual(testStore);
    const modifiedStore = { "session:2": { sessionId: "id-2", updatedAt: Date.now(), displayName: "Test Session 2" } };
    fs.writeFileSync(storePath, JSON.stringify(modifiedStore, null, 2));
    const loaded2 = loadSessionStore(storePath);
    expect(loaded2).toEqual(modifiedStore);
  });
