...
const PROVIDER_PLUGIN_IDS = [{ pluginId: "google-antigravity-auth", providerId: "google-antigravity" }, { pluginId: "google-gemini-cli-auth", providerId: "google-gemini-cli" }, { pluginId: "qwen-portal-auth", providerId: "qwen-portal" }, { pluginId: "copilot-proxy", providerId: "copilot-proxy" }];
...
function isSlackConfigured(cfg, env) {
  if (((hasNonEmptyString(env.SLACK_BOT_TOKEN) || hasNonEmptyString(env.SLACK_APP_TOKEN)) || hasNonEmptyString(env.SLACK_USER_TOKEN))) {
    return true;
  }
  ...
  if (((hasNonEmptyString(entry.botToken) || hasNonEmptyString(entry.appToken)) || hasNonEmptyString(entry.userToken))) {
    return true;
  }
...
function isSignalConfigured(cfg) {
  ...
  if (((((hasNonEmptyString(entry.account) || hasNonEmptyString(entry.httpUrl)) || hasNonEmptyString(entry.httpHost)) || (typeof entry.httpPort === "number")) || hasNonEmptyString(entry.cliPath))) {
    return true;
  }
...
function resolveConfiguredPlugins(cfg, env) {
  ...
  for (const channelId of channelIds) {
    ...
    if (isChannelConfigured(cfg, channelId, env)) {
      changes.push({ pluginId: channelId, reason: " configured" });
    }
  }
  for (const mapping of PROVIDER_PLUGIN_IDS) {
    if (isProviderConfigured(cfg, mapping.providerId)) {
      changes.push({ pluginId: mapping.pluginId, reason: " auth configured" });
    }
  }
  return changes;
}
...
function enablePluginEntry(cfg, pluginId) {
  const entries = { ...cfg.plugins?.entries: , [pluginId]: { ...cfg.plugins?.entries?.[pluginId]: , enabled: true } };
  return { ...cfg: , plugins: { ...cfg.plugins: , entries, ...(cfg.plugins?.enabled === false) ? { enabled: true } : {  }:  } };
}
function formatAutoEnableChange(entry) {
  ...
  if (channelId) {
    ...
    reason = reason.replace(new RegExp("^\\b", "i"), label);
  }
