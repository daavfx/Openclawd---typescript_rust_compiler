...
export const LEGACY_CONFIG_MIGRATIONS_PART_2 = [{ id: "agent.model-config-v2", describe: "Migrate legacy agent.model/allowedModels/modelAliases/modelFallbacks/imageModelFallbacks to agent.models + model lists", apply: (raw, changes) => {
  ...
  const hasLegacy = (((((legacyModel || legacyImageModel) || (legacyAllowed.length > 0)) || (legacyModelFallbacks.length > 0)) || (legacyImageModelFallbacks.length > 0)) || (Object.keys(legacyAliases).length > 0));
  if (!hasLegacy) {
    return;
  }
  ...
  if (currentModel) {
    ...
    if (((legacyModelFallbacks.length > 0) && (!Array.isArray(currentModel.fallbacks) || (currentModel.fallbacks.length === 0)))) {
      currentModel.fallbacks = legacyModelFallbacks;
    }
    agent.model = currentModel;
  } else {
    if ((legacyModel || (legacyModelFallbacks.length > 0))) {
      agent.model = { primary: legacyModel, fallbacks: legacyModelFallbacks.length ? legacyModelFallbacks : [] };
    }
  }
  ...
  if (currentImageModel) {
    ...
    if (((legacyImageModelFallbacks.length > 0) && (!Array.isArray(currentImageModel.fallbacks) || (currentImageModel.fallbacks.length === 0)))) {
    ...
    if ((legacyImageModel || (legacyImageModelFallbacks.length > 0))) {
      agent.imageModel = { primary: legacyImageModel, fallbacks: legacyImageModelFallbacks.length ? legacyImageModelFallbacks : [] };
    }
  }
  agent.models = models;
  if ((legacyModel !== undefined)) {
    changes.push("Migrated .model string → .model.primary.");
  }
  if ((legacyModelFallbacks.length > 0)) {
    changes.push("Migrated .modelFallbacks → .model.fallbacks.");
  }
  if ((legacyImageModel !== undefined)) {
    changes.push("Migrated .imageModel string → .imageModel.primary.");
  }
  if ((legacyImageModelFallbacks.length > 0)) {
    changes.push("Migrated .imageModelFallbacks → .imageModel.fallbacks.");
  }
  if ((legacyAllowed.length > 0)) {
    changes.push("Migrated .allowedModels → .models.");
  }
  if ((Object.keys(legacyAliases).length > 0)) {
    changes.push("Migrated .modelAliases → .models.*.alias.");
  }
  delete agent.allowedModels;
  delete agent.modelAliases;
  delete agent.modelFallbacks;
  delete agent.imageModelFallbacks;
} }, { id: "routing.agents-v2", describe: "Move routing.agents/defaultAgentId to agents.list", apply: (raw, changes) => {
    ...
    for (const [rawId, entryRaw] of Object.entries(routingAgents)) {
      ...
      if (("mentionPatterns" in entryCopy)) {
        ...
        if ((groupChat.mentionPatterns === undefined)) {
          ...
          changes.push("Moved routing.agents..mentionPatterns → agents.list (id \"\").groupChat.mentionPatterns.");
        } else {
          changes.push("Removed routing.agents..mentionPatterns (agents.list groupChat mentionPatterns already set).");
        }
      ...
      if (legacySandbox) {
        ...
        if (sandboxTools) {
          ...
          changes.push("Moved routing.agents..sandbox.tools → agents.list (id \"\").tools.sandbox.tools.");
        }
  ...
  if (defaultAgentId) {
    ...
    if (!hasDefault) {
      ...
      entry.default = true;
      changes.push("Moved routing.defaultAgentId → agents.list (id \"\").default.");
    } else {
...
} }, { id: "routing.config-v2", describe: "Move routing bindings/groupChat/queue/agentToAgent/transcribeAudio", apply: (raw, changes) => {
  const routing = getRecord(raw.routing);
  if (!routing) {
    return;
  }
