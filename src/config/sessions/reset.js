...
export function evaluateSessionFreshness(params) {
  const dailyResetAt = (params.policy.mode === "daily") ? resolveDailyResetAtMs(params.now, params.policy.atHour) : undefined;
  const idleExpiresAt = (params.policy.idleMinutes != null) ? (params.updatedAt + (params.policy.idleMinutes * 60000)) : undefined;
  const staleDaily = ((dailyResetAt != null) && (params.updatedAt < dailyResetAt));
  const staleIdle = ((idleExpiresAt != null) && (params.now > idleExpiresAt));
  return { fresh: !(staleDaily || staleIdle), dailyResetAt, idleExpiresAt };
}
