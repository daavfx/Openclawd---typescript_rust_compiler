...
export function deriveGroupSessionPatch(params) {
  ...
  const nextGroupChannel = (explicitChannel ?? ((((resolution.chatType === "channel") || isChannelProvider) && subject) && subject.startsWith("#")) ? subject : undefined);
  const nextSubject = nextGroupChannel ? undefined : subject;
  const patch = { chatType: (resolution.chatType ?? "group"), channel, groupId: resolution.id };
  if (nextSubject) {
  ...
  const displayName = buildGroupDisplayName({ provider: channel, subject: (nextSubject ?? params.existing?.subject), groupChannel: (nextGroupChannel ?? params.existing?.groupChannel), space: (space ?? params.existing?.space), id: resolution.id, key: params.sessionKey });
  if (displayName) {
