...
import { appendAssistantMessageToSessionTranscript, resolveMirroredTranscriptText } from "./transcript.js";
describe("resolveMirroredTranscriptText", () => {
  it("prefers media filenames over text", () => {
    const result = resolveMirroredTranscriptText({ text: "caption here", mediaUrls: ["https://example.com/files/report.pdf?sig=123"] });
...
describe("appendAssistantMessageToSessionTranscript", () => {
  ...
  it("returns error for missing sessionKey", async () => {
    const result = await appendAssistantMessageToSessionTranscript({ sessionKey: "", text: "test", storePath });
  ...
  it("returns error for empty text", async () => {
    const result = await appendAssistantMessageToSessionTranscript({ sessionKey: "test-session", text: "   ", storePath });
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("returns error for unknown sessionKey", async () => {
    ...
    const result = await appendAssistantMessageToSessionTranscript({ sessionKey: "nonexistent", text: "test message", storePath });
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("creates transcript file and appends message for valid session", async () => {
    ...
    const store = { [sessionKey]: { sessionId, chatType: "direct", channel: "discord" } };
    fs.writeFileSync(storePath, JSON.stringify(store), "utf-8");
    const result = await appendAssistantMessageToSessionTranscript({ sessionKey, text: "Hello from delivery mirror!", storePath });
