...
export function buildGroupDisplayName(params) {
  ...
  const detail = ((groupChannel && space) ? "" : (((groupChannel || subject) || space) || "") || "");
...
export function resolveGroupSessionKey(ctx) {
  ...
  const looksLikeGroup = (((((normalizedChatType === "group") || (normalizedChatType === "channel")) || from.includes(":group:")) || from.includes(":channel:")) || isWhatsAppGroupId);
  if (!looksLikeGroup) {
    return null;
  }
  ...
  return { key: "::", channel: provider, id: finalId, chatType: (kind === "channel") ? "channel" : "group" };
}
