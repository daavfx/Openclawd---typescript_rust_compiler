...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves single file $include", () => {
    ...
    expect(resolve(obj, files)).toEqual({ agents: { list: [{ id: "main" }] } });
  });
  ...
  it("resolves array $include with deep merge", () => {
    const files = { [configPath("a.json")]: { "group-a": ["agent1"] }, [configPath("b.json")]: { "group-b": ["agent2"] } };
    const obj = { broadcast: { $include: ["./a.json", "./b.json"] } };
    expect(resolve(obj, files)).toEqual({ broadcast: { "group-a": ["agent1"], "group-b": ["agent2"] } });
  });
  it("deep merges overlapping keys in array $include", () => {
    const files = { [configPath("a.json")]: { agents: { defaults: { workspace: "~/a" } } }, [configPath("b.json")]: { agents: { list: [{ id: "main" }] } } };
    const obj = { $include: ["./a.json", "./b.json"] };
    expect(resolve(obj, files)).toEqual({ agents: { defaults: { workspace: "~/a" }, list: [{ id: "main" }] } });
  });
  ...
  it("throws when sibling keys are used with non-object includes", () => {
    ...
    expect(() => resolve(obj, files)).toThrow(/Sibling keys require included content to be an object/);
  });
  ...
  it("resolves nested includes", () => {
    const files = { [configPath("level1.json")]: { nested: { $include: "./level2.json" } }, [configPath("level2.json")]: { deep: "value" } };
    const obj = { $include: "./level1.json" };
    expect(resolve(obj, files)).toEqual({ nested: { deep: "value" } });
  });
  ...
  it("throws ConfigIncludeError for invalid JSON", () => {
    const resolver = { readFile: () => "{ invalid json }", parseJson: JSON.parse };
    const obj = { $include: "./bad.json" };
    expect(() => resolveConfigIncludes(obj, DEFAULT_BASE_PATH, resolver)).toThrow(ConfigIncludeError);
    expect(() => resolveConfigIncludes(obj, DEFAULT_BASE_PATH, resolver)).toThrow(/Failed to parse include file/);
  });
  it("throws CircularIncludeError for circular includes", () => {
    const aPath = configPath("a.json");
    const bPath = configPath("b.json");
    const resolver = { readFile: (filePath) => {
      if ((filePath === aPath)) {
  ...
  it("throws ConfigIncludeError for null/boolean include items", () => {
    const files = { [configPath("valid.json")]: { valid: true } };
    const cases = [{ value: null, expected: "object" }, { value: false, expected: "boolean" }];
    for (const item of cases) {
      const obj = { $include: ["./valid.json", item.value] };
      expect(() => resolve(obj, files)).toThrow(ConfigIncludeError);
      expect(() => resolve(obj, files)).toThrow(new RegExp("expected string, got "));
    }
  });
  it("respects max depth limit", () => {
    const files = {  };
    for (let i = 0; (i < 15); i++) {
      files[configPath("level.json")] = { $include: "./level.json" };
    }
  ...
  it("allows depth 10 but rejects depth 11", () => {
    const okFiles = {  };
    for (let i = 0; (i < 9); i++) {
      okFiles[configPath("ok.json")] = { $include: "./ok.json" };
    }
    okFiles[configPath("ok9.json")] = { done: true };
    expect(resolve({ $include: "./ok0.json" }, okFiles)).toEqual({ done: true });
    const failFiles = {  };
    for (let i = 0; (i < 10); i++) {
      failFiles[configPath("fail.json")] = { $include: "./fail.json" };
    }
  ...
  it("handles relative paths correctly", () => {
    const files = { [configPath("clients", "mueller", "agents.json")]: { id: "mueller" } };
    const obj = { agent: { $include: "./clients/mueller/agents.json" } };
    expect(resolve(obj, files)).toEqual({ agent: { id: "mueller" } });
  });
  it("applies nested includes before sibling overrides", () => {
    const files = { [configPath("base.json")]: { nested: { $include: "./nested.json" } }, [configPath("nested.json")]: { a: 1, b: 2 } };
    const obj = { $include: "./base.json", nested: { b: 9 } };
    expect(resolve(obj, files)).toEqual({ nested: { a: 1, b: 9 } });
  });
  it("resolves parent directory references", () => {
    const files = { [sharedPath("common.json")]: { shared: true } };
    const obj = { $include: "../../shared/common.json" };
    expect(resolve(obj, files, configPath("sub", "openclaw.json"))).toEqual({ shared: true });
  });
});
describe("real-world config patterns", () => {
  it("supports per-client agent includes", () => {
    const files = { [configPath("clients", "mueller.json")]: { agents: [{ id: "mueller-screenshot", workspace: "~/clients/mueller/screenshot" }, { id: "mueller-transcribe", workspace: "~/clients/mueller/transcribe" }], broadcast: { "group-mueller": ["mueller-screenshot", "mueller-transcribe"] } }, [configPath("clients", "schmidt.json")]: { agents: [{ id: "schmidt-screenshot", workspace: "~/clients/schmidt/screenshot" }], broadcast: { "group-schmidt": ["schmidt-screenshot"] } } };
    const obj = { gateway: { port: 18789 }, $include: ["./clients/mueller.json", "./clients/schmidt.json"] };
    expect(resolve(obj, files)).toEqual({ gateway: { port: 18789 }, agents: [{ id: "mueller-screenshot", workspace: "~/clients/mueller/screenshot" }, { id: "mueller-transcribe", workspace: "~/clients/mueller/transcribe" }, { id: "schmidt-screenshot", workspace: "~/clients/schmidt/screenshot" }], broadcast: { "group-mueller": ["mueller-screenshot", "mueller-transcribe"], "group-schmidt": ["schmidt-screenshot"] } });
  });
  it("supports modular config structure", () => {
    const files = { [configPath("gateway.json")]: { gateway: { port: 18789, bind: "loopback" } }, [configPath("channels", "whatsapp.json")]: { channels: { whatsapp: { dmPolicy: "pairing", allowFrom: ["+49123"] } } }, [configPath("agents", "defaults.json")]: { agents: { defaults: { sandbox: { mode: "all" } } } } };
    const obj = { $include: ["./gateway.json", "./channels/whatsapp.json", "./agents/defaults.json"] };
    expect(resolve(obj, files)).toEqual({ gateway: { port: 18789, bind: "loopback" }, channels: { whatsapp: { dmPolicy: "pairing", allowFrom: ["+49123"] } }, agents: { defaults: { sandbox: { mode: "all" } } } });
  });
});
