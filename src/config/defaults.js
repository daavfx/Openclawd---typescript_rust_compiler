...
const DEFAULT_MODEL_ALIASES = { opus: "anthropic/claude-opus-4-5", sonnet: "anthropic/claude-sonnet-4-5", gpt: "openai/gpt-5.2", "gpt-mini": "openai/gpt-5-mini", gemini: "google/gemini-3-pro-preview", "gemini-flash": "google/gemini-3-flash-preview" };
...
function resolveModelCost(raw) {
  return { input: (typeof raw?.input === "number") ? raw.input : DEFAULT_MODEL_COST.input, output: (typeof raw?.output === "number") ? raw.output : DEFAULT_MODEL_COST.output, cacheRead: (typeof raw?.cacheRead === "number") ? raw.cacheRead : DEFAULT_MODEL_COST.cacheRead, cacheWrite: (typeof raw?.cacheWrite === "number") ? raw.cacheWrite : DEFAULT_MODEL_COST.cacheWrite };
}
...
export function applySessionDefaults(cfg, options = {  }) {
  ...
  const next = { ...cfg: , session: { ...session: , mainKey: "main" } };
...
export function applyTalkApiKey(config) {
  ...
  return { ...config: , talk: { ...config.talk: , apiKey: resolved } };
}
  ...
  if (providerConfig) {
    ...
    for (const [providerId, provider] of Object.entries(providerConfig)) {
      ...
      const nextModels = models.map((model) => {
        ...
        const costMutated = ((((!raw.cost || (raw.cost.input !== cost.input)) || (raw.cost.output !== cost.output)) || (raw.cost.cacheRead !== cost.cacheRead)) || (raw.cost.cacheWrite !== cost.cacheWrite));
        if (costMutated) {
          modelMutated = true;
        }
        ...
        return { ...raw: , reasoning, input, cost, contextWindow, maxTokens };
      });
      if (!providerMutated) {
        continue;
      }
...
export function applyContextPruningDefaults(cfg) {
  ...
  if ((defaults.contextPruning?.mode === undefined)) {
    nextDefaults.contextPruning = { ...contextPruning: , mode: "cache-ttl", ttl: (defaults.contextPruning?.ttl ?? "1h") };
    mutated = true;
  }
  if ((defaults.heartbeat?.every === undefined)) {
    nextDefaults.heartbeat = { ...heartbeat: , every: (authMode === "oauth") ? "1h" : "30m" };
    mutated = true;
  }
  if ((authMode === "api_key")) {
    ...
    for (const [key, entry] of Object.entries(nextModels)) {
      ...
      nextModels[key] = { ...current: , params: { ...params: , cacheControlTtl: "1h" } };
      modelsMutated = true;
    }
...
export function applyCompactionDefaults(cfg) {
  ...
  return { ...cfg: , agents: { ...cfg.agents: , defaults: { ...defaults: , compaction: { ...compaction: , mode: "safeguard" } } } };
}
