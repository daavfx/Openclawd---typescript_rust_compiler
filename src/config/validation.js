...
function validateIdentityAvatar(config) {
  ...
  for (const [index, entry] of agents.entries()) {
    ...
    if (avatar.startsWith("~")) {
      issues.push({ path: "agents.list..identity.avatar", message: "identity.avatar must be a workspace-relative path, http(s) URL, or data URI." });
      continue;
    }
    ...
    if (!isWorkspaceAvatarPath(avatar, workspaceDir)) {
      issues.push({ path: "agents.list..identity.avatar", message: "identity.avatar must stay within the agent workspace." });
    }
  }
  return issues;
}
export function validateConfigObject(raw) {
  ...
  if ((legacyIssues.length > 0)) {
    return { ok: false, issues: legacyIssues.map((iss) => { path: iss.path, message: iss.message }) };
  }
  const validated = OpenClawSchema.safeParse(raw);
  if (!validated.success) {
    return { ok: false, issues: validated.error.issues.map((iss) => { path: iss.path.join("."), message: iss.message }) };
  }
  const duplicates = findDuplicateAgentDirs(validated.data);
  if ((duplicates.length > 0)) {
    return { ok: false, issues: [{ path: "agents.list", message: formatDuplicateAgentDirError(duplicates) }] };
  }
...
export function validateConfigObjectWithPlugins(raw) {
  ...
  const registry = loadPluginManifestRegistry({ config, workspaceDir: (workspaceDir ?? undefined) });
  const knownIds = new Set(registry.plugins.map((record) => record.id));
  for (const diag of registry.diagnostics) {
    let path = diag.pluginId ? "plugins.entries." : "plugins";
    if ((!diag.pluginId && diag.message.includes("plugin path not found"))) {
      path = "plugins.load.paths";
    }
    const pluginLabel = diag.pluginId ? "plugin " : "plugin";
    const message = ": ";
    if ((diag.level === "error")) {
  ...
  if ((entries && isRecord(entries))) {
    for (const pluginId of Object.keys(entries)) {
      if (!knownIds.has(pluginId)) {
        issues.push({ path: "plugins.entries.", message: "plugin not found: " });
      }
    }
  }
  ...
  for (const pluginId of allow) {
    ...
    if (!knownIds.has(pluginId)) {
      issues.push({ path: "plugins.allow", message: "plugin not found: " });
    }
  }
  ...
  for (const pluginId of deny) {
    ...
    if (!knownIds.has(pluginId)) {
      issues.push({ path: "plugins.deny", message: "plugin not found: " });
    }
  }
  const memorySlot = normalizedPlugins.slots.memory;
  if ((((typeof memorySlot === "string") && memorySlot.trim()) && !knownIds.has(memorySlot))) {
    issues.push({ path: "plugins.slots.memory", message: "plugin not found: " });
  }
  ...
  if ((config.channels && isRecord(config.channels))) {
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        issues.push({ path: "channels.", message: "unknown channel id: " });
      }
    }
  }
  ...
  const validateHeartbeatTarget = (target, path) => {
    ...
    issues.push({ path, message: "unknown heartbeat target: " });
  };
  validateHeartbeatTarget(config.agents?.defaults?.heartbeat?.target, "agents.defaults.heartbeat.target");
  if (Array.isArray(config.agents?.list)) {
    for (const [index, entry] of config.agents.list.entries()) {
      validateHeartbeatTarget(entry?.heartbeat?.target, "agents.list..heartbeat.target");
    }
  }
  ...
  for (const record of registry.plugins) {
    ...
    if (enabled) {
      const memoryDecision = resolveMemorySlotDecision({ id: pluginId, kind: record.kind, slot: memorySlot, selectedId: selectedMemoryPluginId });
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        const res = validateJsonSchemaValue({ schema: record.configSchema, cacheKey: ((record.schemaCacheKey ?? record.manifestPath) ?? pluginId), value: (entry?.config ?? {  }) });
        if (!res.ok) {
          for (const error of res.errors) {
            issues.push({ path: "plugins.entries..config", message: "invalid config: " });
          }
        }
      } else {
        issues.push({ path: "plugins.entries.", message: "plugin schema missing for " });
      }
    }
    if ((!enabled && entryHasConfig)) {
      warnings.push({ path: "plugins.entries.", message: "plugin disabled () but config is present" });
    }
  }
  if ((issues.length > 0)) {
