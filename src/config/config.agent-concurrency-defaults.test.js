...
import { DEFAULT_AGENT_MAX_CONCURRENT, DEFAULT_SUBAGENT_MAX_CONCURRENT, resolveAgentMaxConcurrent, resolveSubagentMaxConcurrent } from "./agent-limits.js";
...
describe("agent concurrency defaults", () => {
  ...
  it("resolves configured values", () => {
    const cfg = { agents: { defaults: { maxConcurrent: 6, subagents: { maxConcurrent: 9 } } } };
  ...
  it("clamps invalid values to at least 1", () => {
    const cfg = { agents: { defaults: { maxConcurrent: 0, subagents: { maxConcurrent: -3 } } } };
  ...
  it("injects defaults on load", async () => {
    await withTempHome(async (home) => {
      ...
      await fs.writeFile(path.join(configDir, "openclaw.json"), JSON.stringify({  }, null, 2), "utf-8");
      vi.resetModules();
