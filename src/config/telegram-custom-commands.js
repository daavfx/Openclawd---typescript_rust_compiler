...
export function resolveTelegramCustomCommands(params) {
  ...
  for (let index = 0; (index < entries.length); index += 1) {
    ...
    if (!normalized) {
      issues.push({ index, field: "command", message: "Telegram custom command is missing a command name." });
      continue;
    }
    if (!TELEGRAM_COMMAND_NAME_PATTERN.test(normalized)) {
      issues.push({ index, field: "command", message: "Telegram custom command \"/\" is invalid (use a-z, 0-9, underscore; max 32 chars)." });
      continue;
    }
    if ((checkReserved && reserved.has(normalized))) {
      issues.push({ index, field: "command", message: "Telegram custom command \"/\" conflicts with a native command." });
      continue;
    }
    if ((checkDuplicates && seen.has(normalized))) {
      issues.push({ index, field: "command", message: "Telegram custom command \"/\" is duplicated." });
      continue;
    }
    const description = normalizeTelegramCommandDescription(String((entry?.description ?? "")));
    if (!description) {
      issues.push({ index, field: "description", message: "Telegram custom command \"/\" is missing a description." });
      continue;
    }
    if (checkDuplicates) {
      seen.add(normalized);
