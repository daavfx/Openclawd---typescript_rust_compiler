...
export const LEGACY_CONFIG_MIGRATIONS_PART_3 = [{ id: "auth.anthropic-claude-cli-mode-oauth", describe: "Switch anthropic:claude-cli auth profile mode to oauth", apply: (raw, changes) => {
...
} }, { id: "tools.bash->tools.exec", describe: "Move tools.bash to tools.exec", apply: (raw, changes) => {
  ...
  delete tools.bash;
} }, { id: "messages.tts.enabled->auto", describe: "Move messages.tts.enabled to messages.tts.auto", apply: (raw, changes) => {
  ...
  delete tts.enabled;
  changes.push("Moved messages.tts.enabled → messages.tts.auto ().");
} }, { id: "agent.defaults-v2", describe: "Move agent config to agents.defaults and tools", apply: (raw, changes) => {
...
} }, { id: "identity->agents.list", describe: "Move identity to agents.list[].identity", apply: (raw, changes) => {
  ...
  if ((entry.identity === undefined)) {
    ...
    changes.push("Moved identity → agents.list (id \"\").identity.");
  } else {
