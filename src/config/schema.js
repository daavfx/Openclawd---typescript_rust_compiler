...
function mergeObjectSchema(base, extension) {
  ...
  const merged = { ...base: , ...extension: , properties: { ...base.properties: , ...extension.properties:  } };
...
function buildBaseHints() {
  ...
  for (const [group, label] of Object.entries(GROUP_LABELS)) {
    hints[group] = { label, group: label, order: GROUP_ORDER[group] };
  }
...
function applyPluginHints(hints, plugins) {
  ...
  for (const plugin of plugins) {
    ...
    const basePath = "plugins.entries.";
    next[basePath] = { ...next[basePath]: , label: name, help: plugin.description ? " (plugin: )" : "Plugin entry for ." };
    next[".enabled"] = { ...next[".enabled"]: , label: "Enable " };
    next[".config"] = { ...next[".config"]: , label: " Config", help: "Plugin-defined config payload for ." };
...
function applyChannelHints(hints, channels) {
  ...
  for (const channel of channels) {
    ...
    next[basePath] = { ...current: , ...label ? { label } : {  }: , ...help ? { help } : {  }:  };
...
function applyHeartbeatTargetHints(hints, channels) {
  ...
  const channelHelp = channelList.length ? " Known channels: ." : "";
  const help = "Delivery target (\"last\", \"none\", or a channel id).";
  const paths = ["agents.defaults.heartbeat.target", "agents.list.*.heartbeat.target"];
  for (const path of paths) {
    const current = (next[path] ?? {  });
    next[path] = { ...current: , help: (current.help ?? help), placeholder: (current.placeholder ?? "last") };
  }
  return next;
}
...
function buildBaseConfigSchema() {
  ...
  const next = { schema: stripChannelSchema(schema), uiHints: hints, version: VERSION, generatedAt: new Date().toISOString() };
  cachedBase = next;
  return next;
}
export function buildConfigSchema(params) {
  ...
  return { ...base: , schema: mergedSchema, uiHints: mergedHints };
}
