...
function resolveEffectiveAgentDir(cfg, agentId, deps) {
  ...
  const configured = Array.isArray(cfg.agents?.list) ? cfg.agents?.list.find((agent) => (normalizeAgentId(agent.id) === id))?.agentDir : undefined;
