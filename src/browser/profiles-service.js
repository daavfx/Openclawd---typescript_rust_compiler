...
export function createBrowserProfilesService(ctx) {
  ...
  const createProfile = async (params) => {
    ...
    if (rawCdpUrl) {
      ...
      profileConfig = { cdpUrl: parsed.normalized, ...driver ? { driver } : {  }: , color: profileColor };
    } else {
    ...
    const nextConfig = { ...cfg: , browser: { ...cfg.browser: , profiles: { ...rawProfiles: , [name]: profileConfig } } };
    ...
    return { ok: true, profile: name, cdpPort: resolved.cdpPort, cdpUrl: resolved.cdpUrl, color: resolved.color, isRemote: !resolved.cdpIsLoopback };
  };
  const deleteProfile = async (nameRaw) => {
    ...
    if ((name === defaultProfile)) {
      throw new Error("cannot delete the default profile \"\"; change browser.defaultProfile first");
    }
    let deleted = false;
    const state = ctx.state();
    const resolved = resolveProfile(state.resolved, name);
    ...
    const {[]: _removed, ...remainingProfiles} = profiles;
    const nextConfig = { ...cfg: , browser: { ...cfg.browser: , profiles: remainingProfiles } };
    await writeConfigFile(nextConfig);
    delete state.resolved.profiles[name];
    state.profiles.delete(name);
    return { ok: true, profile: name, deleted };
  };
  return { listProfiles, createProfile, deleteProfile };
}
