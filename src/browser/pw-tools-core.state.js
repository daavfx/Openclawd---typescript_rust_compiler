...
export async function setGeolocationViaPlaywright(opts) {
  ...
  await context.setGeolocation({ latitude: opts.latitude, longitude: opts.longitude, accuracy: (typeof opts.accuracy === "number") ? opts.accuracy : undefined });
...
export async function setDeviceViaPlaywright(opts) {
  ...
  if (descriptor.viewport) {
    await page.setViewportSize({ width: descriptor.viewport.width, height: descriptor.viewport.height });
  }
  await withCdpSession(page, async (session) => {
    if ((descriptor.userAgent || descriptor.locale)) {
      await session.send("Emulation.setUserAgentOverride", { userAgent: (descriptor.userAgent ?? ""), acceptLanguage: (descriptor.locale ?? undefined) });
    }
    if (descriptor.viewport) {
      await session.send("Emulation.setDeviceMetricsOverride", { mobile: Boolean(descriptor.isMobile), width: descriptor.viewport.width, height: descriptor.viewport.height, deviceScaleFactor: (descriptor.deviceScaleFactor ?? 1), screenWidth: descriptor.viewport.width, screenHeight: descriptor.viewport.height });
    }
