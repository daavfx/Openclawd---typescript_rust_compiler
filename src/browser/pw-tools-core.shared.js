...
export function toAIFriendlyError(error, selector) {
  ...
  if (message.includes("strict mode violation")) {
    ...
    return new Error(("Selector \"\" matched  elements. " + "Run a new snapshot to get updated refs, or use a different ref."));
  }
  if (((message.includes("Timeout") || message.includes("waiting for")) && (message.includes("to be visible") || message.includes("not visible")))) {
    return new Error(("Element \"\" not found or not visible. " + "Run a new snapshot to see current page elements."));
  }
  if (((message.includes("intercepts pointer events") || message.includes("not visible")) || message.includes("not receive pointer events"))) {
    return new Error(("Element \"\" is not interactable (hidden or covered). " + "Try scrolling it into view, closing overlays, or re-snapshotting."));
  }
