...
export async function captureScreenshotPng(opts) {
  return await captureScreenshot({ wsUrl: opts.wsUrl, fullPage: opts.fullPage, format: "png" });
}
...
export async function captureScreenshot(opts) {
  return await withCdpSocket(opts.wsUrl, async (send) => {
    ...
    const result = await send("Page.captureScreenshot", { format, ...(quality !== undefined) ? { quality } : {  }: , fromSurface: true, captureBeyondViewport: true, ...clip ? { clip } : {  }:  });
...
export async function evaluateJavaScript(opts) {
  return await withCdpSocket(opts.wsUrl, async (send) => {
    ...
    const evaluated = await send("Runtime.evaluate", { expression: opts.expression, awaitPromise: Boolean(opts.awaitPromise), returnByValue: (opts.returnByValue ?? true), userGesture: true, includeCommandLineAPI: true });
...
export function formatAriaSnapshot(nodes, limit) {
  ...
  while ((stack.length && (out.length < limit))) {
    ...
    const ref = "ax";
    out.push({ ref, role: (role || "unknown"), name: (name || ""), ...value ? { value } : {  }: , ...description ? { description } : {  }: , ...(typeof n.backendDOMNodeId === "number") ? { backendDOMNodeId: n.backendDOMNodeId } : {  }: , depth });
...
export async function snapshotDom(opts) {
  ...
  const evaluated = await evaluateJavaScript({ wsUrl: opts.wsUrl, expression, awaitPromise: true, returnByValue: true });
