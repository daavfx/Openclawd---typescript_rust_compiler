...
export async function hoverViaPlaywright(opts) {
  ...
  try {
    {
      await refLocator(page, ref).hover({ timeout: Math.max(500, Math.min(60000, (opts.timeoutMs ?? 8000))) });
    }
  }
  catch (err) {
    {
...
export async function dragViaPlaywright(opts) {
  ...
  try {
    {
      await refLocator(page, startRef).dragTo(refLocator(page, endRef), { timeout: Math.max(500, Math.min(60000, (opts.timeoutMs ?? 8000))) });
    }
  }
  catch (err) {
    {
...
export async function selectOptionViaPlaywright(opts) {
  ...
  try {
    {
      await refLocator(page, ref).selectOption(opts.values, { timeout: Math.max(500, Math.min(60000, (opts.timeoutMs ?? 8000))) });
    }
  }
  catch (err) {
    {
...
export async function takeScreenshotViaPlaywright(opts) {
  ...
  const buffer = await page.screenshot({ type, fullPage: Boolean(opts.fullPage) });
  return { buffer };
}
...
export async function screenshotWithLabelsViaPlaywright(opts) {
  ...
  const viewport = await page.evaluate(() => { scrollX: (window.scrollX || 0), scrollY: (window.scrollY || 0), width: (window.innerWidth || 0), height: (window.innerHeight || 0) });
    ...
    try {
      {
        ...
        boxes.push({ ref, x: (x0 - viewport.scrollX), y: (y0 - viewport.scrollY), w: Math.max(1, box.width), h: Math.max(1, box.height) });
      }
    }
    catch {
      {
