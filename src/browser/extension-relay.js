...
function parseBaseUrl(raw) {
  ...
  if (((parsed.protocol !== "http:") && (parsed.protocol !== "https:"))) {
    throw new Error("extension relay cdpUrl must be http(s), got ");
  }
  const host = parsed.hostname;
  const port = (parsed.port?.trim() !== "") ? Number(parsed.port) : (parsed.protocol === "https:") ? 443 : 80;
  if (((!Number.isFinite(port) || (port <= 0)) || (port > 65535))) {
    throw new Error("extension relay cdpUrl has invalid port: ");
  }
...
export async function ensureChromeExtensionRelayServer(opts) {
  ...
  if (!isLoopbackHost(info.host)) {
    throw new Error("extension relay requires loopback cdpUrl host (got )");
  }
  ...
  const ensureTargetEventsForClient = (ws, mode) => {
    for (const target of connectedTargets.values()) {
      if ((mode === "autoAttach")) {
        ws.send(JSON.stringify({ method: "Target.attachedToTarget", params: { sessionId: target.sessionId, targetInfo: { ...target.targetInfo: , attached: true }, waitingForDebugger: false } }));
      } else {
        ws.send(JSON.stringify({ method: "Target.targetCreated", params: { targetInfo: { ...target.targetInfo: , attached: true } } }));
      }
    }
  };
  const routeCdpCommand = async (cmd) => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return { protocolVersion: "1.3", product: "Chrome/OpenClaw-Extension-Relay", revision: "0", userAgent: "OpenClaw-Extension-Relay", jsVersion: "V8" };
      ...
      case "Target.getTargets":
        return { targetInfos: Array.from(connectedTargets.values()).map((t) => { ...t.targetInfo: , attached: true }) };
  ...
  const server = createServer((req, res) => {
    ...
    if ((listPaths.has(path) && ((req.method === "GET") || (req.method === "PUT")))) {
      const list = Array.from(connectedTargets.values()).map((t) => { id: t.targetId, type: (t.targetInfo.type ?? "page"), title: (t.targetInfo.title ?? ""), description: (t.targetInfo.title ?? ""), url: (t.targetInfo.url ?? ""), webSocketDebuggerUrl: cdpWsUrl, devtoolsFrontendUrl: "/devtools/inspector.html?ws=" });
      res.writeHead(200, { "Content-Type": "application/json" });
    ...
    if ((activateMatch && ((req.method === "GET") || (req.method === "PUT")))) {
      ...
      void async () => {
        try {
          {
            await sendToExtension({ id: nextExtensionId++, method: "forwardCDPCommand", params: { method: "Target.activateTarget", params: { targetId } } });
          }
        }
        catch {
          {
    ...
    if ((closeMatch && ((req.method === "GET") || (req.method === "PUT")))) {
      ...
      void async () => {
        try {
          {
            await sendToExtension({ id: nextExtensionId++, method: "forwardCDPCommand", params: { method: "Target.closeTarget", params: { targetId } } });
          }
        }
        catch {
          {
      ...
      if (((parsed && (typeof parsed === "object")) && ("method" in parsed))) {
        ...
        if ((method === "Target.attachedToTarget")) {
          ...
          if ((attached?.sessionId && attached?.targetInfo?.targetId)) {
            ...
            connectedTargets.set(attached.sessionId, { sessionId: attached.sessionId, targetId: nextTargetId, targetInfo: attached.targetInfo });
            if ((changedTarget && prevTargetId)) {
              broadcastToCdpClients({ method: "Target.detachedFromTarget", params: { sessionId: attached.sessionId, targetId: prevTargetId }, sessionId: attached.sessionId });
            }
  ...
  wssCdp.on("connection", (ws) => {
    ...
    ws.on("message", async (data) => {
      ...
      try {
        ...
        {
          sendResponseToCdp(ws, { id: cmd.id, sessionId: cmd.sessionId, error: { message: (err instanceof Error) ? err.message : String(err) } });
        }
      }
    });
    ws.on("close", () => {
  ...
  const baseUrl = "//:";
  const relay = { host, port, baseUrl, cdpWsUrl: "ws://:/cdp", extensionConnected: () => Boolean(extensionWs), stop: async () => {
