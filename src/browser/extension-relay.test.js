...
function createMessageQueue(ws) {
  ...
  const next = (timeoutMs = 5000) => new Promise((resolve, reject) => {
...
describe("chrome extension relay server", () => {
  ...
  it("advertises CDP WS only when extension is connected", async () => {
    ...
    const v1 = await fetch("/json/version").then((r) => r.json());
    expect(v1.webSocketDebuggerUrl).toBeUndefined();
    const ext = new WebSocket("ws://127.0.0.1:/extension");
    await waitForOpen(ext);
    const v2 = await fetch("/json/version").then((r) => r.json());
  ...
  it("tracks attached page targets and exposes them via CDP + /json/list", async () => {
    ...
    const ext = new WebSocket("ws://127.0.0.1:/extension");
    await waitForOpen(ext);
    ext.send(JSON.stringify({ method: "forwardCDPEvent", params: { method: "Target.attachedToTarget", params: { sessionId: "cb-tab-1", targetInfo: { targetId: "t1", type: "page", title: "Example", url: "https://example.com" }, waitingForDebugger: false } } }));
    const list = await fetch("/json/list").then((r) => r.json());
    expect(list.some((t) => ((t.id === "t1") && (t.url === "https://example.com")))).toBe(true);
    ext.send(JSON.stringify({ method: "forwardCDPEvent", params: { method: "Target.targetInfoChanged", params: { targetInfo: { targetId: "t1", type: "page", title: "DER STANDARD", url: "https://www.derstandard.at/" } } } }));
    const list2 = await waitForListMatch(async () => await fetch("/json/list").then((r) => r.json()), (list) => list.some((t) => (((t.id === "t1") && (t.url === "https://www.derstandard.at/")) && (t.title === "DER STANDARD"))));
    expect(list2.some((t) => (((t.id === "t1") && (t.url === "https://www.derstandard.at/")) && (t.title === "DER STANDARD")))).toBe(true);
    const cdp = new WebSocket("ws://127.0.0.1:/cdp");
    ...
    cdp.send(JSON.stringify({ id: 2, method: "Target.attachToTarget", params: { targetId: "t1" } }));
    const received = [];
  ...
  it("rebroadcasts attach when a session id is reused for a new target", async () => {
    ...
    await waitForOpen(cdp);
    const q = createMessageQueue(cdp);
    ext.send(JSON.stringify({ method: "forwardCDPEvent", params: { method: "Target.attachedToTarget", params: { sessionId: "shared-session", targetInfo: { targetId: "t1", type: "page", title: "First", url: "https://example.com" }, waitingForDebugger: false } } }));
    ...
    ext.send(JSON.stringify({ method: "forwardCDPEvent", params: { method: "Target.attachedToTarget", params: { sessionId: "shared-session", targetInfo: { targetId: "t2", type: "page", title: "Second", url: "https://example.org" }, waitingForDebugger: false } } }));
    const received = [];
    received.push(JSON.parse(await q.next()));
    received.push(JSON.parse(await q.next()));
