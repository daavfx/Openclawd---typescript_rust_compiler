...
export async function snapshotAiViaPlaywright(opts) {
  ...
  const result = await maybe._snapshotForAI({ timeout: Math.max(500, Math.min(60000, Math.floor((opts.timeoutMs ?? 5000)))), track: "response" });
  let snapshot = String((result?.full ?? ""));
  const maxChars = opts.maxChars;
  const limit = (((typeof maxChars === "number") && Number.isFinite(maxChars)) && (maxChars > 0)) ? Math.floor(maxChars) : undefined;
  let truncated = false;
...
[...TRUNCATED - page too large]";
  ...
  storeRoleRefsForTarget({ page, cdpUrl: opts.cdpUrl, targetId: opts.targetId, refs: built.refs, mode: "aria" });
...
export async function snapshotRoleViaPlaywright(opts) {
  ...
  if ((opts.refsMode === "aria")) {
    ...
    storeRoleRefsForTarget({ page, cdpUrl: opts.cdpUrl, targetId: opts.targetId, refs: built.refs, mode: "aria" });
    return { snapshot: built.snapshot, refs: built.refs, stats: getRoleSnapshotStats(built.snapshot, built.refs) };
  }
  ...
  storeRoleRefsForTarget({ page, cdpUrl: opts.cdpUrl, targetId: opts.targetId, refs: built.refs, frameSelector: (frameSelector || undefined), mode: "role" });
  return { snapshot: built.snapshot, refs: built.refs, stats: getRoleSnapshotStats(built.snapshot, built.refs) };
}
...
export async function resizeViewportViaPlaywright(opts) {
  const page = await getPageForTargetId(opts);
  ensurePageState(page);
  await page.setViewportSize({ width: Math.max(1, Math.floor(opts.width)), height: Math.max(1, Math.floor(opts.height)) });
}
