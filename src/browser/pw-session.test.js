...
import { ensurePageState, refLocator, rememberRoleRefsForTarget, restoreRoleRefsForTarget } from "./pw-session.js";
function fakePage() {
  ...
  const frameLocator = vi.fn(() => { getByRole: vi.fn(() => { nth: vi.fn(() => { ok: true }) }) });
  const locator = vi.fn(() => { nth: vi.fn(() => { ok: true }) });
  const page = { on, getByRole, frameLocator, locator };
...
describe("pw-session role refs cache", () => {
  it("restores refs for a different Page instance (same CDP targetId)", () => {
    ...
    const targetId = "t1";
    rememberRoleRefsForTarget({ cdpUrl, targetId, refs: { e1: { role: "button", name: "OK" } }, frameSelector: "iframe#main" });
    const {page, mocks} = fakePage();
...
describe("pw-session ensurePageState", () => {
  it("tracks page errors and network requests (best-effort)", () => {
    ...
    const state = ensurePageState(page);
    const req = { method: () => "GET", url: () => "https://example.com/api", resourceType: () => "xhr", failure: () => { errorText: "net::ERR_FAILED" } };
    const resp = { request: () => req, status: () => 500, ok: () => false };
    ...
    expect(state.requests.at(-1)).toMatchObject({ method: "GET", url: "https://example.com/api", resourceType: "xhr", status: 500, ok: false, failureText: "net::ERR_FAILED" });
  });
