...
import { allocateCdpPort, allocateColor, CDP_PORT_RANGE_END, CDP_PORT_RANGE_START, getUsedColors, getUsedPorts, isValidProfileName, PROFILE_COLORS } from "./profiles.js";
...
describe("getUsedPorts", () => {
  ...
  it("extracts ports from profile configs", () => {
    const profiles = { openclaw: { cdpPort: 18792 }, work: { cdpPort: 18793 }, personal: { cdpPort: 18795 } };
  ...
  it("extracts ports from cdpUrl when cdpPort is missing", () => {
    const profiles = { remote: { cdpUrl: "http://10.0.0.42:9222" }, secure: { cdpUrl: "https://example.com:9443" } };
    const used = getUsedPorts(profiles);
    expect(used).toEqual(new Set([9222, 9443]));
  });
  it("ignores invalid cdpUrl values", () => {
    const profiles = { bad: { cdpUrl: "notaurl" } };
    const used = getUsedPorts(profiles);
...
describe("color allocation", () => {
  ...
  it("skips multiple used colors", () => {
    const usedColors = new Set([PROFILE_COLORS[0].toUpperCase(), PROFILE_COLORS[1].toUpperCase(), PROFILE_COLORS[2].toUpperCase()]);
...
describe("getUsedColors", () => {
  ...
  it("extracts and uppercases colors from profile configs", () => {
    const profiles = { openclaw: { color: "#ff4500" }, work: { color: "#0066CC" } };
