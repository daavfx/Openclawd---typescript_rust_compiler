...
describe("cdp", () => {
  ...
  it("creates a target via the browser websocket", async () => {
    ...
    wsServer.on("connection", (socket) => {
      socket.on("message", (data) => {
        ...
        socket.send(JSON.stringify({ id: msg.id, result: { targetId: "TARGET_123" } }));
      });
    });
    ...
    const created = await createTargetViaCdp({ cdpUrl: "http://127.0.0.1:", url: "https://example.com" });
    ...
    wsServer.on("connection", (socket) => {
      socket.on("message", (data) => {
        ...
        if ((msg.method === "Runtime.evaluate")) {
          ...
          socket.send(JSON.stringify({ id: msg.id, result: { result: { type: "number", value: 2 } } }));
        }
      });
    });
    const res = await evaluateJavaScript({ wsUrl: "ws://127.0.0.1:", expression: "1+1" });
    ...
    wsServer.on("connection", (socket) => {
      socket.on("message", (data) => {
        ...
        if ((msg.method === "Accessibility.getFullAXTree")) {
          socket.send(JSON.stringify({ id: msg.id, result: { nodes: [{ nodeId: "1", role: { value: "RootWebArea" }, name: { value: "" }, childIds: ["2"] }, { nodeId: "2", role: { value: "button" }, name: { value: "OK" }, backendDOMNodeId: 42, childIds: [] }] } }));
          return;
        }
      });
    });
    const snap = await snapshotAria({ wsUrl: "ws://127.0.0.1:" });
  ...
  it("propagates auth and query params onto normalized websocket URLs", () => {
    const normalized = normalizeCdpWsUrl("ws://127.0.0.1:9222/devtools/browser/ABC", "https://user:pass@example.com?token=abc");
    expect(normalized).toBe("wss://user:pass@example.com/devtools/browser/ABC?token=abc");
  });
  it("upgrades ws to wss when CDP uses https", () => {
    const normalized = normalizeCdpWsUrl("ws://production-sfo.browserless.io", "https://production-sfo.browserless.io?token=abc");
