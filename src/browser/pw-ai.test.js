...
function createPage(opts) {
  const session = { send: vi.fn().mockResolvedValue({ targetInfo: { targetId: opts.targetId } }), detach: vi.fn().mockResolvedValue(undefined) };
  ...
  const page = { context: () => context, locator, on: vi.fn(), ...(opts.hasSnapshotForAI === false) ? {  } : { _snapshotForAI: vi.fn().mockResolvedValue({ full: (opts.snapshotFull ?? "SNAP") }) }:  };
  return { page, session, locator, click, fill };
}
function createBrowser(pages) {
  const ctx = { pages: () => pages, on: vi.fn() };
  return { contexts: () => [ctx], on: vi.fn(), close: vi.fn().mockResolvedValue(undefined) };
}
...
describe("pw-ai", () => {
  it("captures an ai snapshot via Playwright for a specific target", async () => {
    ...
    const mod = await importModule();
    const res = await mod.snapshotAiViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T2" });
...
");
    ...
    chromium.connectOverCDP.mockResolvedValue(browser);
    const mod = await importModule();
    const res = await mod.snapshotAiViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1" });
    expect(res.refs).toMatchObject({ e1: { role: "button", name: "OK" }, e2: { role: "link", name: "Docs" } });
    await mod.clickViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1", ref: "e1" });
  ...
  it("truncates oversized snapshots", async () => {
    ...
    chromium.connectOverCDP.mockResolvedValue(browser);
    const mod = await importModule();
    const res = await mod.snapshotAiViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1", maxChars: 10 });
  ...
  it("clicks a ref using aria-ref locator", async () => {
    ...
    chromium.connectOverCDP.mockResolvedValue(browser);
    const mod = await importModule();
    await mod.clickViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1", ref: "76" });
  ...
  it("fails with a clear error when _snapshotForAI is missing", async () => {
    ...
    chromium.connectOverCDP.mockResolvedValue(browser);
    const mod = await importModule();
    await expect(mod.snapshotAiViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1" })).rejects.toThrow(/_snapshotForAI/i);
  });
  it("reuses the CDP connection for repeated calls", async () => {
    ...
    const mod = await importModule();
    await mod.snapshotAiViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1" });
    await mod.clickViaPlaywright({ cdpUrl: "http://127.0.0.1:18792", targetId: "T1", ref: "1" });
