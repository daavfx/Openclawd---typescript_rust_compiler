...
export function registerBrowserBasicRoutes(app, ctx) {
  ...
  app.get("/", async (req, res) => {
    ...
    res.json({ enabled: current.resolved.enabled, profile: profileCtx.profile.name, running: cdpReady, cdpReady, cdpHttp, pid: (profileState?.running?.pid ?? null), cdpPort: profileCtx.profile.cdpPort, cdpUrl: profileCtx.profile.cdpUrl, chosenBrowser: (profileState?.running?.exe.kind ?? null), detectedBrowser, detectedExecutablePath, detectError, userDataDir: (profileState?.running?.userDataDir ?? null), color: profileCtx.profile.color, headless: current.resolved.headless, noSandbox: current.resolved.noSandbox, executablePath: (current.resolved.executablePath ?? null), attachOnly: current.resolved.attachOnly });
  });
    ...
    try {
      {
        ...
        res.json({ ok: true, stopped: result.stopped, profile: profileCtx.profile.name });
      }
    }
    catch (err) {
      {
  ...
  app.post("/profiles/create", async (req, res) => {
    ...
    try {
      {
        const service = createBrowserProfilesService(ctx);
        const result = await service.createProfile({ name, color: (color || undefined), cdpUrl: (cdpUrl || undefined), driver: (driver === "extension") ? "extension" : undefined });
        res.json(result);
      }
    }
    catch (err) {
