...
export function rememberRoleRefsForTarget(opts) {
  ...
  roleRefsByTarget.set(roleRefsKey(opts.cdpUrl, targetId), { refs: opts.refs, ...opts.frameSelector ? { frameSelector: opts.frameSelector } : {  }: , ...opts.mode ? { mode: opts.mode } : {  }:  });
...
export function storeRoleRefsForTarget(opts) {
  ...
  rememberRoleRefsForTarget({ cdpUrl: opts.cdpUrl, targetId: opts.targetId, refs: opts.refs, frameSelector: opts.frameSelector, mode: opts.mode });
}
...
export function ensurePageState(page) {
  ...
  const state = { console: [], errors: [], requests: [], requestIds: new WeakMap(), nextRequestId: 0, armIdUpload: 0, armIdDialog: 0, armIdDownload: 0 };
  pageStates.set(page, state);
  if (!observedPages.has(page)) {
    observedPages.add(page);
    page.on("console", (msg) => {
      const entry = { type: msg.type(), text: msg.text(), timestamp: new Date().toISOString(), location: msg.location() };
    ...
    page.on("pageerror", (err) => {
      state.errors.push({ message: err?.message ? String(err.message) : String(err), name: err?.name ? String(err.name) : undefined, stack: err?.stack ? String(err.stack) : undefined, timestamp: new Date().toISOString() });
    ...
    page.on("request", (req) => {
      ...
      state.requests.push({ id, timestamp: new Date().toISOString(), method: req.method(), url: req.url(), resourceType: req.resourceType() });
...
export function refLocator(page, ref) {
  ...
  if (/^e\d+$/.test(normalized)) {
    ...
    if ((state?.roleRefsMode === "aria")) {
      ...
      return scope.locator("aria-ref=");
    }
    const info = state?.roleRefs?.[normalized];
    if (!info) {
      throw new Error("Unknown ref \"\". Run a new snapshot and use a ref from that snapshot.");
    }
    const scope = state?.roleRefsFrameSelector ? page.frameLocator(state.roleRefsFrameSelector) : page;
    const locAny = scope;
...
export async function listPagesViaPlaywright(opts) {
  ...
  for (const page of pages) {
    const tid = await pageTargetId(page).catch(() => null);
    if (tid) {
      results.push({ targetId: tid, title: await page.title().catch(() => ""), url: page.url(), type: "page" });
    }
  }
  return results;
}
