...
import { decorateOpenClawProfile, ensureProfileCleanExit, findChromeExecutableMac, findChromeExecutableWindows, isChromeReachable, resolveBrowserExecutableForPlatform, stopOpenClawChrome } from "./chrome.js";
    ...
    try {
      {
        ...
        const marker = await fsp.readFile(path.join(userDataDir, ".openclaw-profile-decorated"), "utf-8");
...
describe("browser chrome helpers", () => {
  ...
  it("picks the first existing Chrome candidate on macOS", () => {
    const exists = vi.spyOn(fs, "existsSync").mockImplementation((p) => String(p).includes("Google Chrome.app/Contents/MacOS/Google Chrome"));
  ...
  it("resolves Windows executables without LOCALAPPDATA", () => {
    ...
    const marker = path.win32.join("Program Files", "Google", "Chrome", "Application", "chrome.exe");
    const exists = vi.spyOn(fs, "existsSync").mockImplementation((p) => String(p).includes(marker));
  ...
  it("reports reachability based on /json/version", async () => {
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => { webSocketDebuggerUrl: "ws://127.0.0.1/devtools" } }));
  ...
  it("stopOpenClawChrome no-ops when process is already killed", async () => {
    ...
    await stopOpenClawChrome({ proc, cdpPort: 12345 }, 10);
