...
describe("browser screenshot normalization", () => {
  it("shrinks oversized images to <=2000x2000 and <=5MB", async () => {
    ...
    const normalized = await normalizeBrowserScreenshot(bigPng, { maxSide: 2000, maxBytes: ((5 * 1024) * 1024) });
  ...
  it("keeps already-small screenshots unchanged", async () => {
    const jpeg = await sharp({ create: { width: 800, height: 600, channels: 3, background: { r: 255, g: 0, b: 0 } } }).jpeg({ quality: 80 }).toBuffer();
    const normalized = await normalizeBrowserScreenshot(jpeg, { maxSide: 2000, maxBytes: ((5 * 1024) * 1024) });
