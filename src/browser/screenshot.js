...
export async function normalizeBrowserScreenshot(buffer, opts) {
  ...
  const sideGrid = [sideStart, 1800, 1600, 1400, 1200, 1000, 800].map((v) => Math.min(maxSide, v)).filter((v, i, arr) => ((v > 0) && (arr.indexOf(v) === i))).sort((a, b) => (b - a));
  let smallest = null;
  for (const side of sideGrid) {
    for (const quality of qualities) {
      const out = await resizeToJpeg({ buffer, maxSide: side, quality, withoutEnlargement: true });
  ...
  throw new Error("Browser screenshot could not be reduced below MB (got MB)");
}
