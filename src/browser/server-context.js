...
function createProfileContext(opts, profile) {
  ...
  const listTabs = async () => {
    if (!profile.cdpIsLoopback) {
      ...
      if ((typeof listPagesViaPlaywright === "function")) {
        ...
        return pages.map((p) => { targetId: p.targetId, title: p.title, url: p.url, type: p.type });
      }
    }
    const raw = await fetchJson(appendCdpPath(profile.cdpUrl, "/json/list"));
    return raw.map((t) => { targetId: (t.id ?? ""), title: (t.title ?? ""), url: (t.url ?? ""), wsUrl: normalizeWsUrl(t.webSocketDebuggerUrl, profile.cdpUrl), type: t.type }).filter((t) => Boolean(t.targetId));
  };
  const openTab = async (url) => {
    ...
    const createdViaCdp = await createTargetViaCdp({ cdpUrl: profile.cdpUrl, url }).then((r) => r.targetId).catch(() => null);
    if (createdViaCdp) {
      const profileState = getProfileState();
    ...
    return { targetId: created.id, title: (created.title ?? ""), url: (created.url ?? url), wsUrl: normalizeWsUrl(created.webSocketDebuggerUrl, profile.cdpUrl), type: created.type };
  };
  ...
  const ensureBrowserAvailable = async () => {
    ...
    if ((isExtension && remoteCdp)) {
      throw new Error("Profile \"\" uses driver=extension but cdpUrl is not loopback ().");
    }
    if (isExtension) {
      if (!httpReachable) {
        ...
        if (await isHttpReachable(1200)) {
          ...
          throw new Error("Chrome extension relay for profile \"\" is not reachable at .");
        }
      }
      if (await isReachable(600)) {
        return;
      }
      throw new Error("Chrome extension relay is running, but no tab is connected. Click the OpenClaw Chrome extension icon on a tab to attach it (profile \"\").");
    }
    if (!httpReachable) {
      ...
      if ((current.resolved.attachOnly || remoteCdp)) {
        throw new Error(remoteCdp ? "Remote CDP for profile \"\" is not reachable at ." : "Browser attachOnly is enabled and profile \"\" is not running.");
      }
    ...
    if (!profileState.running) {
      throw new Error(("Port  is in use for profile \"\" but not by openclaw. " + "Run action=reset-profile profile= to kill the process."));
    }
    if ((current.resolved.attachOnly || remoteCdp)) {
      ...
      throw new Error(remoteCdp ? "Remote CDP websocket for profile \"\" is not reachable." : "Browser attachOnly is enabled and CDP websocket for profile \"\" is not reachable.");
    }
    ...
    if (!await isReachable(600)) {
      throw new Error("Chrome CDP websocket for profile \"\" is not reachable after restart.");
    }
  };
  const ensureTabAvailable = async (targetId) => {
    ...
    if ((tabs1.length === 0)) {
      if ((profile.driver === "extension")) {
        throw new Error(("tab not found (no attached Chrome tabs for profile \"\"). " + "Click the OpenClaw Browser Relay toolbar icon on the tab you want to control (badge ON)."));
      }
  ...
  const focusTab = async (targetId) => {
    ...
    if (!profile.cdpIsLoopback) {
      ...
      if ((typeof focusPageByTargetIdViaPlaywright === "function")) {
        await focusPageByTargetIdViaPlaywright({ cdpUrl: profile.cdpUrl, targetId: resolved.targetId });
        const profileState = getProfileState();
        profileState.lastTargetId = resolved.targetId;
        return;
      }
    }
    await fetchOk(appendCdpPath(profile.cdpUrl, "/json/activate/"));
    const profileState = getProfileState();
    profileState.lastTargetId = resolved.targetId;
  };
  const closeTab = async (targetId) => {
    ...
    if (!profile.cdpIsLoopback) {
      ...
      if ((typeof closePageByTargetIdViaPlaywright === "function")) {
        await closePageByTargetIdViaPlaywright({ cdpUrl: profile.cdpUrl, targetId: resolved.targetId });
        return;
      }
    }
    await fetchOk(appendCdpPath(profile.cdpUrl, "/json/close/"));
  };
  const stopRunningBrowser = async () => {
    if ((profile.driver === "extension")) {
      const stopped = await stopChromeExtensionRelayServer({ cdpUrl: profile.cdpUrl });
      return { stopped };
    }
    const profileState = getProfileState();
    if (!profileState.running) {
  ...
  const resetProfile = async () => {
    ...
    if (!profile.cdpIsLoopback) {
      throw new Error("reset-profile is only supported for local profiles (profile \"\" is remote).");
    }
  ...
  return { profile, ensureBrowserAvailable, ensureTabAvailable, isHttpReachable, isReachable, listTabs, openTab, focusTab, closeTab, stopRunningBrowser, resetProfile };
}
export function createBrowserRouteContext(opts) {
  ...
  const forProfile = (profileName) => {
    ...
    if (!profile) {
      ...
      throw new Error("Profile \"\" not found. Available profiles: ");
    }
  ...
  const listProfiles = async () => {
    ...
    for (const name of Object.keys(current.resolved.profiles)) {
      ...
      result.push({ name, cdpPort: profile.cdpPort, cdpUrl: profile.cdpUrl, color: profile.color, running, tabCount, isDefault: (name === current.resolved.defaultProfile), isRemote: !profile.cdpIsLoopback });
    }
    return result;
  };
