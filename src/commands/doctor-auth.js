...
export async function maybeRepairAnthropicOAuthProfileId(cfg, prompter) {
  ...
  const repair = repairOAuthProfileIdMismatch({ cfg, store, provider: "anthropic", legacyProfileId: "anthropic:default" });
  if ((!repair.migrated || (repair.changes.length === 0))) {
    return cfg;
  }
  note(repair.changes.map((c) => "- ").join("
"), "Auth profiles");
  const apply = await prompter.confirm({ message: "Update Anthropic OAuth profile id in config now?", initialValue: true });
  if (!apply) {
    return cfg;
  }
...
function pruneAuthProfiles(cfg, profileIds) {
  ...
  const nextAuth = (nextProfiles || prunedOrder.next) ? { ...cfg.auth: , profiles: (nextProfiles && (Object.keys(nextProfiles).length > 0)) ? nextProfiles : undefined, order: prunedOrder.next } : undefined;
  return { next: { ...cfg: , auth: nextAuth }, changed: true };
}
export async function maybeRemoveDeprecatedCliAuthProfiles(cfg, prompter) {
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push("-  (Anthropic): use setup-token → ");
  }
  if (deprecated.has(CODEX_CLI_PROFILE_ID)) {
    lines.push("-  (OpenAI Codex): use OAuth → ");
  }
  note(lines.join("
"), "Auth profiles");
  const shouldRemove = await prompter.confirmRepair({ message: "Remove deprecated CLI auth profiles now?", initialValue: true });
  if (!shouldRemove) {
    return cfg;
  }
  ...
  if (pruned.changed) {
    note(Array.from(deprecated.values()).map((id) => "- removed  from config").join("
"), "Doctor changes");
  }
  return pruned.next;
}
...
function formatAuthIssueHint(issue) {
  if (((issue.provider === "anthropic") && (issue.profileId === CLAUDE_CLI_PROFILE_ID))) {
    return "Deprecated profile. Use  or .";
  }
  if (((issue.provider === "openai-codex") && (issue.profileId === CODEX_CLI_PROFILE_ID))) {
    return "Deprecated profile. Use  or .";
  }
  return "Re-auth via `` or ``.";
}
function formatAuthIssueLine(issue) {
  const remaining = (issue.remainingMs !== undefined) ? " ()" : "";
  const hint = formatAuthIssueHint(issue);
  return "- : ";
}
export async function noteAuthProfileHealth(params) {
  const store = ensureAuthProfileStore(undefined, { allowKeychainPrompt: params.allowKeychainPrompt });
  const unusable = () => {
    const now = Date.now();
    const out = [];
...
"), "Auth profile cooldowns");
  ...
  let summary = buildAuthHealthSummary({ store, cfg: params.cfg, warnAfterMs: DEFAULT_OAUTH_WARN_MS });
  ...
  const shouldRefresh = await params.prompter.confirmRepair({ message: "Refresh expiring OAuth tokens now? (static tokens need re-auth)", initialValue: true });
  if (shouldRefresh) {
    ...
    for (const profile of refreshTargets) {
      try {
        {
          await resolveApiKeyForProfile({ cfg: params.cfg, store, profileId: profile.profileId });
        }
      }
      catch (err) {
        {
...
"), "OAuth refresh errors");
    ...
    summary = buildAuthHealthSummary({ store: ensureAuthProfileStore(undefined, { allowKeychainPrompt: false }), cfg: params.cfg, warnAfterMs: DEFAULT_OAUTH_WARN_MS });
    issues = findIssues();
  }
  if ((issues.length > 0)) {
    note(issues.map((issue) => formatAuthIssueLine({ profileId: issue.profileId, provider: issue.provider, status: issue.status, remainingMs: issue.remainingMs })).join("
"), "Model auth");
  }
}
