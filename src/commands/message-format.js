...
export function buildMessageCliJson(result) {
  return { action: result.action, channel: result.channel, dryRun: result.dryRun, handledBy: result.handledBy, payload: result.payload };
}
...
function renderObjectSummary(payload, opts) {
  ...
  const rows = keys.slice(0, 20).map((k) => {
    const v = obj[k];
    const value = (v == null) ? "null" : Array.isArray(v) ? " items" : (typeof v === "object") ? "object" : (typeof v === "string") ? v : (typeof v === "number") ? String(v) : (typeof v === "boolean") ? v ? "true" : "false" : (typeof v === "bigint") ? v.toString() : (typeof v === "symbol") ? v.toString() : (typeof v === "function") ? "function" : "unknown";
    return { Key: k, Value: shortenText(value, 96) };
  });
  return [renderTable({ width: opts.width, columns: [{ key: "Key", header: "Key", minWidth: 16 }, { key: "Value", header: "Value", flex: true, minWidth: 24 }], rows }).trimEnd()];
}
function renderMessageList(messages, opts, emptyLabel) {
  const rows = messages.slice(0, 25).map((m) => {
    ...
    return { Time: shortenText(time, 28), Author: shortenText(author, 22), Text: shortenText(text.replace(/\s+/g, " ").trim(), 90), Id: shortenText(id, 22) };
  });
  if ((rows.length === 0)) {
    return [theme.muted(emptyLabel)];
  }
  return [renderTable({ width: opts.width, columns: [{ key: "Time", header: "Time", minWidth: 14 }, { key: "Author", header: "Author", minWidth: 10 }, { key: "Text", header: "Text", flex: true, minWidth: 24 }, { key: "Id", header: "Id", minWidth: 10 }], rows }).trimEnd()];
}
...
function renderReactions(payload, opts) {
  ...
  const rows = reactions.slice(0, 50).map((r) => {
    ...
    return { Emoji: emoji, Count: count, Users: shortenText(userList.join(", "), 72) };
  });
  if ((rows.length === 0)) {
    return [theme.muted("No reactions.")];
  }
  return [renderTable({ width: opts.width, columns: [{ key: "Emoji", header: "Emoji", minWidth: 8 }, { key: "Count", header: "Count", align: "right", minWidth: 6 }, { key: "Users", header: "Users", flex: true, minWidth: 20 }], rows }).trimEnd()];
}
export function formatMessageCliText(result) {
  ...
  if ((result.handledBy === "dry-run")) {
    return [muted("[dry-run] would run  via ")];
  }
  if ((result.kind === "broadcast")) {
    const results = (result.payload.results ?? []);
    const rows = results.map((entry) => { Channel: resolveChannelLabel(entry.channel), Target: shortenText(formatTargetDisplay({ channel: entry.channel, target: entry.to }), 36), Status: entry.ok ? "ok" : "error", Error: entry.ok ? "" : shortenText((entry.error ?? "unknown error"), 48) });
    const okCount = results.filter((entry) => entry.ok).length;
    const total = results.length;
    const headingLine = ok("✅ Broadcast complete (/ succeeded,  failed)");
    return [headingLine, renderTable({ width: opts.width, columns: [{ key: "Channel", header: "Channel", minWidth: 10 }, { key: "Target", header: "Target", minWidth: 12, flex: true }, { key: "Status", header: "Status", minWidth: 6 }, { key: "Error", header: "Error", minWidth: 20, flex: true }], rows: rows.slice(0, 50) }).trimEnd()];
  }
  if ((result.kind === "send")) {
    if (((result.handledBy === "core") && result.sendResult)) {
      ...
      return [ok(formatGatewaySummary({ channel: send.channel, messageId: (gatewayResult?.messageId ?? null) }))];
    }
  ...
  if ((result.kind === "poll")) {
    if (((result.handledBy === "core") && result.pollResult)) {
      ...
      const lines = [ok(formatGatewaySummary({ action: "Poll sent", channel: poll.channel, messageId: msgId }))];
      if (pollId) {
        lines.push(ok("Poll id: "));
      }
      return lines;
    }
    const label = resolveChannelLabel(result.channel);
    const msgId = extractMessageId(result.payload);
    return [ok("✅ Poll sent via .")];
  }
