...
function createAgentMatcher(agentId) {
  ...
  return (item) => ((item.sessionKey === agentPrefix) || item.sessionKey.startsWith(":"));
}
async function confirmRecreate() {
  const result = await clackConfirm({ message: "This will stop and remove these containers. Continue?", initialValue: false });
...
async function removeContainer(containerName, removeFn, runtime) {
  try {
    ...
    {
      runtime.error("✗ Failed to remove : ");
