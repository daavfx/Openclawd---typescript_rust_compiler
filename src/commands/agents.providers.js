...
function formatChannelAccountLabel(params) {
  ...
  const account = params.name?.trim() ? " ()" : params.accountId;
  return " ";
}
...
export async function buildProviderStatusIndex(cfg) {
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    for (const accountId of accountIds) {
      ...
      const state = (plugin.status?.resolveAccountState?.({ account, cfg, configured: resolvedConfigured, enabled: resolvedEnabled }) ?? (typeof snapshot?.linked === "boolean") ? snapshot.linked ? "linked" : "not linked" : resolvedConfigured ? "configured" : "not configured");
      const name = (snapshot?.name ?? account.name);
      map.set(providerAccountKey(plugin.id, accountId), { provider: plugin.id, accountId, name, state, enabled, configured });
    }
  }
  return map;
}
...
function formatProviderEntry(entry) {
  const label = formatChannelAccountLabel({ provider: entry.provider, accountId: entry.accountId, name: entry.name });
  return ": ";
}
export function summarizeBindings(cfg, bindings) {
  ...
  for (const binding of bindings) {
    ...
    if (!seen.has(key)) {
      const label = formatChannelAccountLabel({ provider: channel, accountId });
      seen.set(key, label);
    }
  }
  return [...seen.values()];
