...
"), "UI");
        ...
        const shouldRepair = await prompter.confirmRepair({ message: "Build Control UI assets now?", initialValue: true });
        if (shouldRepair) {
          note("Building Control UI assets... (this may take a moment)", "UI");
          const uiScriptPath = path.join(root, "scripts/ui.js");
          const buildResult = await runCommandWithTimeout([process.execPath, uiScriptPath, "build"], { cwd: root, timeoutMs: 120000, env: { ...process.env: , FORCE_COLOR: "1" } });
          if ((buildResult.code === 0)) {
            note("UI build complete.", "UI");
          } else {
            const details = ["UI build failed (exit ).", buildResult.stderr.trim() ? buildResult.stderr.trim() : null].filter(Boolean).join("
");
            note(details, "UI");
          }
        }
      ...
      if ((schemaStats.mtime > uiStats.mtime)) {
        ...
        const gitLog = await runCommandWithTimeout(["git", "-C", root, "log", "--since=", "--format=%h %s", "src/gateway/protocol/schema.ts"], { timeoutMs: 5000 }).catch(() => null);
...
", "UI Freshness");
          const shouldRepair = await prompter.confirmAggressive({ message: "Rebuild UI now? (Detected protocol mismatch requiring update)", initialValue: true });
          if (shouldRepair) {
            const uiSourcesPath = path.join(root, "ui/package.json");
            const uiSourcesExist = await fs.stat(uiSourcesPath).catch(() => null);
            if (!uiSourcesExist) {
            ...
            if ((buildResult.code === 0)) {
              ...
              const details = ["UI rebuild failed (exit ).", buildResult.stderr.trim() ? buildResult.stderr.trim() : null].filter(Boolean).join("
");
              note(details, "UI");
            }
          }
