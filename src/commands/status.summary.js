...
export async function getStatusSummary() {
  ...
  const heartbeatAgents = agentList.agents.map((agent) => {
    ...
    return { agentId: agent.id, enabled: summary.enabled, every: summary.every, everyMs: summary.everyMs };
  });
  const channelSummary = await buildChannelSummary(cfg, { colorize: true, includeAllowFrom: true });
  ...
  const byAgent = agentList.agents.map((agent) => {
    ...
    return { agentId: agent.id, path: storePath, count: sessions.length, recent: sessions.slice(0, 10) };
  });
  const allSessions = Array.from(paths).flatMap((storePath) => buildSessionRows(loadStore(storePath))).sort((a, b) => ((b.updatedAt ?? 0) - (a.updatedAt ?? 0)));
  const recent = allSessions.slice(0, 10);
  const totalSessions = allSessions.length;
  return { linkChannel: linkContext ? { id: linkContext.plugin.id, label: (linkContext.plugin.meta.label ?? "Channel"), linked: linkContext.linked, authAgeMs: linkContext.authAgeMs } : undefined, heartbeat: { defaultAgentId: agentList.defaultId, agents: heartbeatAgents }, channelSummary, queuedSystemEvents, sessions: { paths: Array.from(paths), count: totalSessions, defaults: { model: (configModel ?? null), contextTokens: (configContextTokens ?? null) }, recent, byAgent } };
}
