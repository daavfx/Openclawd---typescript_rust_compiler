...
import { applyAgentBindings, applyAgentConfig, buildAgentSummaries, pruneAgentConfig } from "./agents.js";
describe("agents helpers", () => {
  it("buildAgentSummaries includes default + configured agents", () => {
    const cfg = { agents: { defaults: { workspace: "/main-ws", model: { primary: "anthropic/claude" } }, list: [{ id: "main" }, { id: "work", default: true, name: "Work", workspace: "/work-ws", agentDir: "/state/agents/work/agent", model: "openai/gpt-4.1" }] }, bindings: [{ agentId: "work", match: { channel: "whatsapp", accountId: "biz" } }, { agentId: "main", match: { channel: "telegram" } }] };
  ...
  it("applyAgentConfig merges updates", () => {
    const cfg = { agents: { list: [{ id: "work", workspace: "/old-ws", model: "anthropic/claude" }] } };
    const next = applyAgentConfig(cfg, { agentId: "work", name: "Work", workspace: "/new-ws", agentDir: "/state/work/agent" });
  ...
  it("applyAgentBindings skips duplicates and reports conflicts", () => {
    const cfg = { bindings: [{ agentId: "main", match: { channel: "whatsapp", accountId: "default" } }] };
    const result = applyAgentBindings(cfg, [{ agentId: "main", match: { channel: "whatsapp", accountId: "default" } }, { agentId: "work", match: { channel: "whatsapp", accountId: "default" } }, { agentId: "work", match: { channel: "telegram" } }]);
  ...
  it("pruneAgentConfig removes agent, bindings, and allowlist entries", () => {
    const cfg = { agents: { list: [{ id: "work", default: true, workspace: "/work-ws" }, { id: "home", workspace: "/home-ws" }] }, bindings: [{ agentId: "work", match: { channel: "whatsapp" } }, { agentId: "home", match: { channel: "telegram" } }], tools: { agentToAgent: { enabled: true, allow: ["work", "home"] } } };
