...
export function buildAgentSummaries(cfg) {
  ...
  return ordered.map((id) => {
    ...
    const identitySource = identity ? "identity" : (configIdentity && (identityName || identityEmoji)) ? "config" : undefined;
    return { id, name: resolveAgentName(cfg, id), identityName, identityEmoji, identitySource, workspace, agentDir: resolveAgentDir(cfg, id), model: resolveAgentModel(cfg, id), bindings: (bindingCounts.get(id) ?? 0), isDefault: (id === defaultAgentId) };
  });
}
...
export function applyAgentConfig(cfg, params) {
  ...
  const nextEntry = { ...base: , ...name ? { name } : {  }: , ...params.workspace ? { workspace: params.workspace } : {  }: , ...params.agentDir ? { agentDir: params.agentDir } : {  }: , ...params.model ? { model: params.model } : {  }:  };
  const nextList = [...list];
  if ((index >= 0)) {
...
export function pruneAgentConfig(cfg, agentId) {
  ...
  const nextAgentsConfig = cfg.agents ? { ...cfg.agents: , list: nextAgents } : nextAgents ? { list: nextAgents } : undefined;
  const nextTools = cfg.tools?.agentToAgent ? { ...cfg.tools: , agentToAgent: { ...cfg.tools.agentToAgent: , allow: (filteredAllow.length > 0) ? filteredAllow : undefined } } : cfg.tools;
  return { config: { ...cfg: , agents: nextAgentsConfig, bindings: (filteredBindings.length > 0) ? filteredBindings : undefined, tools: nextTools }, removedBindings: (bindings.length - filteredBindings.length), removedAllow: (allow.length - filteredAllow.length) };
}
