...
export async function gatewayStatusCommand(opts, runtime) {
  ...
  const discoveryPromise = discoverGatewayBeacons({ timeoutMs: discoveryTimeoutMs, wideAreaDomain });
  ...
  const {discovery, probed} = await withProgress({ label: "Inspecting gateways…", indeterminate: true, enabled: (opts.json !== true) }, async () => {
    const tryStartTunnel = async () => {
      ...
      try {
        {
          const tunnel = await startSshPortForward({ target: sshTarget, identity: (sshIdentity ?? undefined), localPortPreferred: remotePort, remotePort, timeoutMs: Math.min(1500, overallTimeoutMs) });
    ...
    if ((!sshTarget && opts.sshAuto)) {
      ...
      const candidates = discovery.map((b) => {
        ...
        const base = user ? "@" : host.trim();
        return (sshPort !== 22) ? ":" : base;
    ...
    const tunnelTarget = tunnel ? { id: "sshTunnel", kind: "sshTunnel", url: "ws://127.0.0.1:", active: true, tunnel: { kind: "ssh", target: (sshTarget ?? ""), localPort: tunnel.localPort, remotePort, pid: tunnel.pid } } : null;
    ...
    try {
      {
        const probed = await Promise.all(targets.map(async (target) => {
          const auth = resolveAuthForTarget(cfg, target, { token: (typeof opts.token === "string") ? opts.token : undefined, password: (typeof opts.password === "string") ? opts.password : undefined });
  ...
  if ((sshTarget && !sshTunnelStarted)) {
    warnings.push({ code: "ssh_tunnel_failed", message: sshTunnelError ? "SSH tunnel failed: " : "SSH tunnel failed to start; falling back to direct probes." });
  }
  if (multipleGateways) {
    warnings.push({ code: "multiple_gateways", message: "Unconventional setup: multiple reachable gateways detected. Usually one gateway per network is recommended unless you intentionally run isolated profiles, like a rescue bot (see docs: /gateway#multiple-gateways-same-host).", targetIds: reachable.map((p) => p.target.id) });
  }
  if (opts.json) {
    runtime.log(JSON.stringify({ ok, ts: Date.now(), durationMs: (Date.now() - startedAt), timeoutMs: overallTimeoutMs, primaryTargetId: (primary?.target.id ?? null), warnings, network, discovery: { timeoutMs: discoveryTimeoutMs, count: discovery.length, beacons: discovery.map((b) => { instanceName: b.instanceName, displayName: (b.displayName ?? null), domain: (b.domain ?? null), host: (b.host ?? null), lanHost: (b.lanHost ?? null), tailnetDns: (b.tailnetDns ?? null), gatewayPort: (b.gatewayPort ?? null), sshPort: (b.sshPort ?? null), wsUrl: () => {
      ...
      return host ? "ws://:" : null;
    }() }) }, targets: probed.map((p) => { id: p.target.id, kind: p.target.kind, url: p.target.url, active: p.target.active, tunnel: (p.target.tunnel ?? null), connect: { ok: p.probe.ok, latencyMs: p.probe.connectLatencyMs, error: p.probe.error, close: p.probe.close }, self: p.self, config: p.configSummary, health: p.probe.health, summary: p.probe.status, presence: p.probe.presence }) }, null, 2));
    if (!ok) {
      runtime.exit(1);
    }
    return;
  ...
  runtime.log(ok ? ": yes" : ": no");
  runtime.log(colorize(rich, theme.muted, "Probe budget: ms"));
  ...
  const discoveryDomains = wideAreaDomain ? "local. + " : "local.";
  runtime.log((discovery.length > 0) ? "Found  gateway(s) via Bonjour ()" : "Found 0 gateways via Bonjour ()");
  ...
  for (const p of probed) {
    ...
    if ((p.probe.ok && p.self)) {
      ...
      const ip = p.self.ip ? " ()" : "";
      const platform = p.self.platform ? " · " : "";
      const version = p.self.version ? " · app " : "";
      runtime.log("  : ");
    }
    if (p.configSummary) {
...
async function resolveSshTarget(rawTarget, identity, overallTimeoutMs) {
  ...
  const config = await resolveSshConfig(parsed, { identity: (identity ?? undefined), timeoutMs: Math.min(800, overallTimeoutMs) });
  if (!config) {
    return { target: rawTarget, identity: (identity ?? undefined) };
  }
  const target = buildSshTarget({ user: (config.user ?? parsed.user), host: (config.host ?? parsed.host), port: (config.port ?? parsed.port) });
  if (!target) {
    return { target: rawTarget, identity: (identity ?? undefined) };
  }
