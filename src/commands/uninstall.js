...
export 
const multiselectStyled = (params) => multiselect({ ...params: , message: stylePromptMessage(params.message), options: params.options.map((opt) => (opt.hint === undefined) ? opt : { ...opt: , hint: stylePromptHint(opt.hint) }) });
...
async function stopAndUninstallService(runtime) {
  ...
  try {
    ...
    {
      runtime.error("Gateway service check failed: ");
      return false;
    }
  }
  if (!loaded) {
    runtime.log("Gateway service .");
    return true;
  }
  try {
    {
    ...
    {
      runtime.error("Gateway stop failed: ");
    }
  }
  try {
    {
    ...
    {
      runtime.error("Gateway uninstall failed: ");
      return false;
    }
  }
}
async function removeMacApp(runtime, dryRun) {
  ...
  await removePath("/Applications/OpenClaw.app", runtime, { dryRun, label: "/Applications/OpenClaw.app" });
}
export async function uninstallCommand(runtime, opts) {
  ...
  if (!hadExplicit) {
    ...
    const selection = await multiselectStyled({ message: "Uninstall which components?", options: [{ value: "service", label: "Gateway service", hint: "launchd / systemd / schtasks" }, { value: "state", label: "State + config", hint: "~/.openclaw" }, { value: "workspace", label: "Workspace", hint: "agent files" }, { value: "app", label: "macOS app", hint: "/Applications/OpenClaw.app" }], initialValues: ["service", "state", "workspace"] });
