...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return { ...actual: , loadConfig: () => { agents: { defaults: { model: { primary: "pi:opus" }, models: { "pi:opus": {  } }, contextTokens: 32000 } } } };
});
import { sessionsCommand } from "./sessions.js";
const makeRuntime = () => {
  const logs = [];
  return { runtime: { log: (msg) => logs.push(String(msg)), error: (msg) => {
...
const writeStore = (data) => {
  const file = path.join(os.tmpdir(), "sessions--.json");
...
describe("sessionsCommand", () => {
  ...
  it("renders a tabular view with token percentages", async () => {
    const store = writeStore({ "+15555550123": { sessionId: "abc123", updatedAt: (Date.now() - (45 * 60000)), inputTokens: 1200, outputTokens: 800, model: "pi:opus" } });
  ...
  it("shows placeholder rows when tokens are missing", async () => {
    const store = writeStore({ "discord:group:demo": { sessionId: "xyz", updatedAt: (Date.now() - (5 * 60000)), thinkingLevel: "high" } });
    const {runtime, logs} = makeRuntime();
    await sessionsCommand({ store }, runtime);
    fs.rmSync(store);
