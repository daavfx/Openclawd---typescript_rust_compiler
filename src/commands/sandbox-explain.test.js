...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return { ...actual: , loadConfig: vi.fn().mockImplementation(() => mockCfg) };
});
describe("sandbox explain command", () => {
  it("prints JSON shape + fix-it keys", async () => {
    mockCfg = { agents: { defaults: { sandbox: { mode: "all", scope: "agent", workspaceAccess: "none" } } }, tools: { sandbox: { tools: { deny: ["browser"] } }, elevated: { enabled: true, allowFrom: { whatsapp: ["*"] } } }, session: { store: "/tmp/openclaw-test-sessions-{agentId}.json" } };
    const {sandboxExplainCommand} = await import("./sandbox-explain.js");
    const logs = [];
    await sandboxExplainCommand({ json: true, session: "agent:main:main" }, { log: (msg) => logs.push(msg), error: (msg) => logs.push(msg), exit: (_code) => {
    } });
    const out = logs.join("");
