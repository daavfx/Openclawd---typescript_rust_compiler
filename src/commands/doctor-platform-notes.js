...
export async function noteMacLaunchAgentOverrides() {
  ...
  const lines = ["- LaunchAgent writes are disabled via .", "- To restore default behavior:", "  rm "].filter((line) => Boolean(line));
  note(lines.join("
"), "Gateway (macOS)");
}
...
export async function noteMacLaunchctlGatewayEnvOverrides(cfg, deps) {
  ...
  const deprecatedLaunchctlEntries = [["MOLTBOT_GATEWAY_TOKEN", await getenv("MOLTBOT_GATEWAY_TOKEN")], ["MOLTBOT_GATEWAY_PASSWORD", await getenv("MOLTBOT_GATEWAY_PASSWORD")], ["CLAWDBOT_GATEWAY_TOKEN", await getenv("CLAWDBOT_GATEWAY_TOKEN")], ["CLAWDBOT_GATEWAY_PASSWORD", await getenv("CLAWDBOT_GATEWAY_PASSWORD")]].filter((entry) => Boolean(entry[1]?.trim()));
  if ((deprecatedLaunchctlEntries.length > 0)) {
    const lines = ["- Deprecated launchctl environment variables detected (ignored).", ...deprecatedLaunchctlEntries.map(([key]) => "- `` is set; use `OPENCLAW_` instead.")];
...
"), "Gateway (macOS)");
  ...
  const lines = ["- launchctl environment overrides detected (can cause confusing unauthorized errors).", (envToken && envTokenKey) ? "- `` is set; it overrides config tokens." : undefined, envPassword ? "- `` is set; it overrides config passwords." : undefined, "- Clear overrides and restart the app/gateway:", envTokenKey ? "  launchctl unsetenv " : undefined, envPasswordKey ? "  launchctl unsetenv " : undefined].filter((line) => Boolean(line));
  (deps?.noteFn ?? note)(lines.join("
"), "Gateway (macOS)");
}
...
export function noteDeprecatedLegacyEnvVars(env = process.env, deps) {
  ...
  const lines = ["- Deprecated legacy environment variables detected (ignored).", "- Use OPENCLAW_* equivalents instead:", ...entries.map((key) => {
