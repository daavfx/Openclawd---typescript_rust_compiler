...
export async function modelsFallbacksAddCommand(modelRaw, runtime) {
  const updated = await updateConfig((cfg) => {
    ...
    return { ...cfg: , agents: { ...cfg.agents: , defaults: { ...cfg.agents?.defaults: , model: { ...existingModel?.primary ? { primary: existingModel.primary } : undefined: , fallbacks: [...existing, targetKey] }, models: nextModels } } };
  });
...
export async function modelsFallbacksRemoveCommand(modelRaw, runtime) {
  const updated = await updateConfig((cfg) => {
    ...
    const existingModel = cfg.agents?.defaults?.model;
    return { ...cfg: , agents: { ...cfg.agents: , defaults: { ...cfg.agents?.defaults: , model: { ...existingModel?.primary ? { primary: existingModel.primary } : undefined: , fallbacks: filtered } } } };
  });
  logConfigUpdated(runtime);
  runtime.log("Fallbacks: ");
}
...
export async function modelsFallbacksClearCommand(runtime) {
  await updateConfig((cfg) => {
    const existingModel = cfg.agents?.defaults?.model;
    return { ...cfg: , agents: { ...cfg.agents: , defaults: { ...cfg.agents?.defaults: , model: { ...existingModel?.primary ? { primary: existingModel.primary } : undefined: , fallbacks: [] } } } };
  });
  logConfigUpdated(runtime);
