...
export function toModelRow(params) {
  ...
  if (!model) {
    return { key, name: key, input: "-", contextWindow: null, local: null, available: null, tags: [...tags, "missing"], missing: true };
  }
  ...
  const aliasTags = (aliases.length > 0) ? ["alias:"] : [];
  ...
  return { key, name: (model.name || model.id), input, contextWindow: (model.contextWindow ?? null), local, available, tags: Array.from(mergedTags), missing: false };
}
