...
export function printModelTable(rows, runtime, opts = {  }) {
  if (opts.json) {
    runtime.log(JSON.stringify({ count: rows.length, models: rows }, null, 2));
    return;
  }
  if (opts.plain) {
  ...
  const header = [pad("Model", MODEL_PAD), pad("Input", INPUT_PAD), pad("Ctx", CTX_PAD), pad("Local", LOCAL_PAD), pad("Auth", AUTH_PAD), "Tags"].join(" ");
  ...
  for (const row of rows) {
    ...
    const line = [rich ? theme.accent(keyLabel) : keyLabel, coloredInput, ctxLabel, coloredLocal, coloredAuth, tagsLabel].join(" ");
    runtime.log(line);
  }
}
