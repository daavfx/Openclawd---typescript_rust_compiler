...
export async function modelsStatusCommand(opts, runtime) {
  ...
  const envProbeProviders = ["anthropic", "github-copilot", "google-vertex", "openai", "google", "groq", "cerebras", "xai", "openrouter", "zai", "mistral", "synthetic"];
  for (const provider of envProbeProviders) {
    if (resolveEnvApiKey(provider)) {
      providersFromEnv.add(provider);
    }
  }
  const providers = Array.from(new Set([...providersFromStore, ...providersFromConfig, ...providersFromModels, ...providersFromEnv])).map((p) => p.trim()).filter(Boolean).sort((a, b) => a.localeCompare(b));
  ...
  const missingProvidersInUse = Array.from(providersInUse).filter((provider) => !providerAuthMap.has(provider)).sort((a, b) => a.localeCompare(b));
  ...
  const rawCandidates = [(rawModel || resolvedLabel), ...fallbacks, imageModel, ...imageFallbacks, ...allowed].filter(Boolean);
  const resolvedCandidates = rawCandidates.map((raw) => resolveModelRefFromString({ raw: String((raw ?? "")), defaultProvider: DEFAULT_PROVIDER, aliasIndex })?.ref).filter((ref) => Boolean(ref));
  const modelCandidates = resolvedCandidates.map((ref) => "/");
  let probeSummary;
  if (opts.probe) {
    probeSummary = await withProgressTotals({ label: "Probing auth profiles…", total: 1 }, async (update) => {
      return await runAuthProbes({ cfg, providers, modelCandidates, options: { provider: opts.probeProvider, profileIds: probeProfileIds, timeoutMs: probeTimeoutMs, concurrency: probeConcurrency, maxTokens: probeMaxTokens }, onProgress: update });
    });
  }
  ...
  const authHealth = buildAuthHealthSummary({ store, cfg, warnAfterMs: DEFAULT_OAUTH_WARN_MS, providers });
  ...
  const unusableProfiles = () => {
    ...
    for (const profileId of Object.keys((store.usageStats ?? {  }))) {
      ...
      out.push({ profileId, provider: store.profiles[profileId]?.provider, kind, reason: stats?.disabledReason, until: unusableUntil, remainingMs: (unusableUntil - now) });
    }
  ...
  const displayDefault = (rawModel && (rawModel !== resolvedLabel)) ? " (from )" : resolvedLabel;
  runtime.log(" ");
  runtime.log(" ");
  runtime.log(" ");
  runtime.log(" ");
  ...
  for (const entry of providerAuth) {
    ...
    const bits = [];
    bits.push(formatKeyValue("effective", ":"));
    if ((entry.profiles.count > 0)) {
      bits.push(formatKeyValue("profiles", " (oauth=, token=, api_key=)"));
      if ((entry.profiles.labels.length > 0)) {
        bits.push(colorize(rich, theme.info, entry.profiles.labels.join(", ")));
      }
    }
    if (entry.env) {
      bits.push(formatKeyValue("env", ""));
    }
    if (entry.modelsJson) {
      bits.push(formatKeyValue("models.json", ""));
    }
    runtime.log("-  ");
  }
  if ((missingProvidersInUse.length > 0)) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint = (provider === "anthropic") ? "Run `claude setup-token`, then `` or ``." : "Run `` or set an API key env var.";
      runtime.log("-  ");
    }
  }
  runtime.log("");
  ...
  if ((oauthProfiles.length === 0)) {
    ...
    if ((usageProviders.length > 0)) {
      try {
        {
          const usageSummary = await loadProviderUsageSummary({ providers: usageProviders, agentDir, timeoutMs: 3500 });
    ...
    for (const [provider, profiles] of profilesByProvider) {
      ...
      const usageSuffix = usage ? colorize(rich, theme.muted, " usage: ") : "";
      runtime.log("- ");
      for (const profile of profiles) {
        const labelText = (profile.label || profile.profileId);
        const label = colorize(rich, theme.accent, labelText);
        const status = formatStatus(profile.status);
        const expiry = (profile.status === "static") ? "" : profile.expiresAt ? " expires in " : " expires unknown";
        runtime.log("  -  ");
      }
    }
  }
...
" : "";
        const statusLabel = "";
        return { Model: colorize(rich, theme.heading, modelLabel), Profile: profile, Status: statusLabel };
      });
      runtime.log(renderTable({ width: tableWidth, columns: [{ key: "Model", header: "Model", minWidth: 18 }, { key: "Profile", header: "Profile", minWidth: 24 }, { key: "Status", header: "Status", minWidth: 12 }], rows }).trimEnd());
