...
export async function modelsAuthOrderGetCommand(opts, runtime) {
  ...
  if (opts.json) {
    runtime.log(JSON.stringify({ agentId, agentDir, provider, authStorePath: shortenHomePath("/auth-profiles.json"), order: (order.length > 0) ? order : null }, null, 2));
    return;
  }
  runtime.log("Agent: ");
  runtime.log("Provider: ");
  runtime.log("Auth file: ");
  runtime.log((order.length > 0) ? "Order override: " : "Order override: (none)");
}
...
export async function modelsAuthOrderSetCommand(opts, runtime) {
  ...
  for (const profileId of requested) {
    ...
    if (!cred) {
      throw new Error("Auth profile \"\" not found in .");
    }
    if ((normalizeProviderId(cred.provider) !== providerKey)) {
      throw new Error("Auth profile \"\" is for , not .");
    }
  }
  const updated = await setAuthProfileOrder({ agentDir, provider, order: requested });
  if (!updated) {
    throw new Error("Failed to update auth-profiles.json (lock busy?).");
  }
  runtime.log("Agent: ");
  runtime.log("Provider: ");
  runtime.log("Order override: ");
}
