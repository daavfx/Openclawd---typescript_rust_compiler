...
export function resolveConfiguredEntries(cfg) {
  ...
  modelFallbacks.forEach((raw, idx) => {
    const resolved = resolveModelRefFromString({ raw: String((raw ?? "")), defaultProvider: DEFAULT_PROVIDER, aliasIndex });
    if (!resolved) {
      return;
    }
    addEntry(resolved.ref, "fallback#");
  });
  if (imagePrimary) {
    const resolved = resolveModelRefFromString({ raw: imagePrimary, defaultProvider: DEFAULT_PROVIDER, aliasIndex });
    if (resolved) {
  ...
  imageFallbacks.forEach((raw, idx) => {
    ...
    addEntry(resolved.ref, "img-fallback#");
  });
  ...
  const entries = order.map((key) => {
    ...
    return { key, ref: { provider, model }, tags: (tagsByKey.get(key) ?? new Set()), aliases: (aliasesByKey.get(key) ?? []) };
  });
  return { entries };
}
