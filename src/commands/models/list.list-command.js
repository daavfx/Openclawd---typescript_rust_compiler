...
export async function modelsListCommand(opts, runtime) {
  ...
  const providerFilter = () => {
    ...
    const parsed = parseModelRef("/_", DEFAULT_PROVIDER);
  ...
  if (opts.all) {
    ...
    for (const model of sorted) {
      ...
      rows.push(toModelRow({ model, key, tags: configured ? Array.from(configured.tags) : [], aliases: (configured?.aliases ?? []), availableKeys, cfg, authStore }));
    }
  } else {
    for (const entry of entries) {
      ...
      rows.push(toModelRow({ model, key: entry.key, tags: Array.from(entry.tags), aliases: entry.aliases, availableKeys, cfg, authStore }));
    }
  }
  if ((rows.length === 0)) {
