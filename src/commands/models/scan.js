...
const MODEL_PAD = 42;
const CTX_PAD = 8;
const multiselect = (params) => clackMultiselect({ ...params: , message: stylePromptMessage(params.message), options: params.options.map((opt) => (opt.hint === undefined) ? opt : { ...opt: , hint: stylePromptHint(opt.hint) }) });
...
function buildScanHint(result) {
  const toolLabel = result.tool.ok ? "tool " : "tool fail";
  const imageLabel = result.image.skipped ? "img skip" : result.image.ok ? "img " : "img fail";
  const ctxLabel = result.contextLength ? "ctx " : "ctx ?";
  const paramLabel = result.inferredParamB ? "b" : null;
...
function printScanSummary(results, runtime) {
  ...
  runtime.log("Scan results: tested , tool ok , image ok , tool+image ok , image only ");
}
function printScanTable(results, runtime) {
  const header = [pad("Model", MODEL_PAD), pad("Tool", 10), pad("Image", 10), pad("Ctx", CTX_PAD), pad("Params", 8), "Notes"].join(" ");
  runtime.log(header);
  for (const entry of results) {
    ...
    const paramsLabel = pad(entry.inferredParamB ? "b" : "-", 8);
    const notes = entry.modality ? "modality:" : "";
...
export async function modelsScanCommand(opts, runtime) {
  ...
  if (probe) {
    try {
      {
        const resolved = await resolveApiKeyForProvider({ provider: "openrouter", cfg });
  ...
  const results = await withProgressTotals({ label: "Scanning OpenRouter models...", indeterminate: false, enabled: (opts.json !== true) }, async (update) => await scanOpenRouterModels({ apiKey: (storedKey ?? undefined), minParamB: minParams, maxAgeDays, providerFilter: opts.provider, timeoutMs: timeout, concurrency, probe, onProgress: ({phase, completed, total}) => {
    if ((phase !== "probe")) {
      return;
    }
  ...
  if (!probe) {
    if (!opts.json) {
      runtime.log("Found  OpenRouter free models (metadata only; pass --probe to test tools/images).");
  ...
  if (canPrompt) {
    const selection = await multiselect({ message: "Select fallback models (ordered)", options: toolSorted.map((entry) => { value: entry.modelRef, label: entry.modelRef, hint: buildScanHint(entry) }), initialValues: preselected });
    ...
    if ((imageSorted.length > 0)) {
      const imageSelection = await multiselect({ message: "Select image fallback models (ordered)", options: imageSorted.map((entry) => { value: entry.modelRef, label: entry.modelRef, hint: buildScanHint(entry) }), initialValues: imagePreselected });
  ...
  const _updated = await updateConfig((cfg) => {
    ...
    const nextImageModel = (selectedImages.length > 0) ? { ...existingImageModel?.primary ? { primary: existingImageModel.primary } : undefined: , fallbacks: selectedImages, ...opts.setImage ? { primary: selectedImages[0] } : {  }:  } : cfg.agents?.defaults?.imageModel;
    const existingModel = cfg.agents?.defaults?.model;
    const defaults = { ...cfg.agents?.defaults: , model: { ...existingModel?.primary ? { primary: existingModel.primary } : undefined: , fallbacks: selected, ...opts.setDefault ? { primary: selected[0] } : {  }:  }, ...nextImageModel ? { imageModel: nextImageModel } : {  }: , models: nextModels };
    return { ...cfg: , agents: { ...cfg.agents: , defaults } };
  });
  if (opts.json) {
    runtime.log(JSON.stringify({ selected, selectedImages, setDefault: Boolean(opts.setDefault), setImage: Boolean(opts.setImage), results, warnings: [] }, null, 2));
    return;
  }
  logConfigUpdated(runtime);
  runtime.log("Fallbacks: ");
  if ((selectedImages.length > 0)) {
    runtime.log("Image fallbacks: ");
  }
  if (opts.setDefault) {
    runtime.log("Default model: ");
  }
  if ((opts.setImage && (selectedImages.length > 0))) {
