...
export async function dashboardCommand(runtime = defaultRuntime, options = {  }) {
  ...
  const links = resolveControlUiLinks({ port, bind, customBindHost, basePath });
  const authedUrl = token ? "?token=" : links.httpUrl;
  runtime.log("Dashboard URL: ");
  ...
  if (!options.noOpen) {
    ...
    if (!opened) {
      hint = formatControlUiSshHint({ port, basePath, token: (token || undefined) });
    }
  } else {
