...
vi.mock("../gateway/call.js", () => { callGateway: (...args) => callGatewayMock(...args) });
describe("healthCommand", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  it("outputs JSON from gateway", async () => {
    const agentSessions = { path: "/tmp/sessions.json", count: 1, recent: [{ key: "+1555", updatedAt: Date.now(), age: 0 }] };
    const snapshot = { ok: true, ts: Date.now(), durationMs: 5, channels: { whatsapp: { accountId: "default", linked: true, authAgeMs: 5000 }, telegram: { accountId: "default", configured: true, probe: { ok: true, elapsedMs: 1 } }, discord: { accountId: "default", configured: false } }, channelOrder: ["whatsapp", "telegram", "discord"], channelLabels: { whatsapp: "WhatsApp", telegram: "Telegram", discord: "Discord" }, heartbeatSeconds: 60, defaultAgentId: "main", agents: [{ agentId: "main", isDefault: true, heartbeat: { enabled: true, every: "1m", everyMs: 60000, prompt: "hi", target: "last", ackMaxChars: 160 }, sessions: agentSessions }], sessions: agentSessions };
