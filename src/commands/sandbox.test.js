...
const mocks = vi.hoisted(() => { listSandboxContainers: vi.fn(), listSandboxBrowsers: vi.fn(), removeSandboxContainer: vi.fn(), removeSandboxBrowserContainer: vi.fn(), clackConfirm: vi.fn() });
vi.mock("../agents/sandbox.js", () => { listSandboxContainers: mocks.listSandboxContainers, listSandboxBrowsers: mocks.listSandboxBrowsers, removeSandboxContainer: mocks.removeSandboxContainer, removeSandboxBrowserContainer: mocks.removeSandboxBrowserContainer });
vi.mock("@clack/prompts", () => { confirm: mocks.clackConfirm });
import { sandboxListCommand, sandboxRecreateCommand } from "./sandbox.js";
const NOW = Date.now();
function createContainer(overrides = {  }) {
  return { containerName: "openclaw-sandbox-test", sessionKey: "test-session", image: "openclaw/sandbox:latest", imageMatch: true, running: true, createdAtMs: (NOW - 3600000), lastUsedAtMs: (NOW - 600000), ...overrides:  };
}
function createBrowser(overrides = {  }) {
  return { containerName: "openclaw-browser-test", sessionKey: "test-session", image: "openclaw/browser:latest", imageMatch: true, running: true, createdAtMs: (NOW - 3600000), lastUsedAtMs: (NOW - 600000), cdpPort: 9222, noVncPort: 5900, ...overrides:  };
}
...
describe("sandboxRecreateCommand", () => {
  ...
  describe("validation", () => {
    ...
    it("should error if multiple filters specified", async () => {
      await sandboxRecreateCommand({ all: true, session: "test", browser: false, force: false }, runtime);
  ...
  describe("filtering", () => {
    it("should filter by session", async () => {
      ...
      await sandboxRecreateCommand({ session: "target-session", browser: false, force: true }, runtime);
    ...
    it("should filter by agent (exact + subkeys)", async () => {
      ...
      await sandboxRecreateCommand({ agent: "work", browser: false, force: true }, runtime);
  ...
  describe("execution", () => {
    ...
    it("should handle removal errors and exit with code 1", async () => {
      mocks.listSandboxContainers.mockResolvedValue([createContainer({ containerName: "success" }), createContainer({ containerName: "failure" })]);
      mocks.removeSandboxContainer.mockResolvedValueOnce(undefined).mockRejectedValueOnce(new Error("Removal failed"));
      await sandboxRecreateCommand({ all: true, browser: false, force: true }, runtime);
