...
export async function channelsResolveCommand(opts, runtime) {
  ...
  if (!plugin?.resolver?.resolveTargets) {
    throw new Error("Channel  does not support resolve.");
  }
  const preferredKind = resolvePreferredKind(opts.kind);
  let results = [];
  if (preferredKind) {
    const resolved = await plugin.resolver.resolveTargets({ cfg, accountId: (opts.account ?? null), inputs: entries, kind: preferredKind, runtime });
    results = resolved.map((entry) => { input: entry.input, resolved: entry.resolved, id: entry.id, name: entry.name, note: entry.note });
  } else {
    ...
    for (const [kind, inputs] of byKind.entries()) {
      const batch = await plugin.resolver.resolveTargets({ cfg, accountId: (opts.account ?? null), inputs, kind, runtime });
      resolved.push(...batch);
    }
    const byInput = new Map(resolved.map((entry) => [entry.input, entry]));
    results = entries.map((input) => {
      const entry = byInput.get(input);
      return { input, resolved: (entry?.resolved ?? false), id: entry?.id, name: entry?.name, note: entry?.note };
    });
  }
  if (opts.json) {
  ...
  for (const result of results) {
    if ((result.resolved && result.id)) {
      ...
      runtime.error(danger(" -> unresolved"));
    }
  }
}
