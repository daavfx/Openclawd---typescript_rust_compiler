...
export async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
  ...
  if (useWizard) {
    ...
    let nextConfig = await setupChannels(cfg, runtime, prompter, { allowDisable: false, allowSignalInstall: true, promptAccountIds: true, onSelection: (value) => {
      selection = value;
    ...
    const wantsNames = await prompter.confirm({ message: "Add display names for these accounts? (optional)", initialValue: false });
    if (wantsNames) {
      for (const channel of selection) {
        ...
        const name = await prompter.text({ message: " account name ()", initialValue: existingName });
        if (name?.trim()) {
          nextConfig = applyAccountName({ cfg: nextConfig, channel, accountId, name });
        }
      }
    }
  ...
  if ((!channel && catalogEntry)) {
    ...
    const result = await ensureOnboardingPluginInstalled({ cfg: nextConfig, entry: catalogEntry, prompter, runtime, workspaceDir });
  ...
  if (!channel) {
    const hint = catalogEntry ? "Plugin  could not be loaded after install." : "Unknown channel: ";
    runtime.error(hint);
    runtime.exit(1);
    return;
  }
