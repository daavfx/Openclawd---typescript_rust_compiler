...
const TEAMS_GRAPH_PERMISSION_HINTS = { "ChannelMessage.Read.All": "channel history", "Chat.Read.All": "chat history", "Channel.ReadBasic.All": "channel list", "Team.ReadBasic.All": "team list", "TeamsActivity.Read.All": "teams activity", "Sites.Read.All": "files (SharePoint)", "Files.Read.All": "files (OneDrive)" };
function normalizeTimeout(raw, fallback = 10000) {
...
function summarizeDiscordTarget(raw) {
  ...
  if ((target.kind === "channel")) {
    return { raw, normalized: target.normalized, kind: "channel", channelId: target.id };
  }
  if ((target.kind === "user")) {
    return { raw, normalized: target.normalized, kind: "user" };
  }
...
function formatDiscordIntents(intents) {
  ...
  return ["messageContent=", "guildMembers=", "presence="].join(" ");
}
function formatProbeLines(channelId, probe) {
  ...
  if ((channelId === "telegram")) {
    ...
    if ((typeof canJoinGroups === "boolean")) {
      flags.push("joinGroups=");
    }
    if ((typeof canReadAll === "boolean")) {
      flags.push("readAllGroupMessages=");
    }
    if ((typeof inlineQueries === "boolean")) {
      flags.push("inlineQueries=");
    }
    if ((flags.length > 0)) {
      lines.push("Flags: ");
    }
      ...
      if ((graph.ok === false)) {
        ...
        if (((roles.length > 0) || (scopes.length > 0))) {
          const formatPermission = (permission) => {
            ...
            return hint ? " ()" : permission;
          };
          if ((roles.length > 0)) {
            lines.push("Graph roles: ");
          }
          if ((scopes.length > 0)) {
...
async function buildDiscordPermissions(params) {
  ...
  try {
    {
      const perms = await fetchChannelPermissionsDiscord(target.channelId, { token, accountId: (params.account.accountId ?? undefined) });
      const missing = REQUIRED_DISCORD_PERMISSIONS.filter((permission) => !perms.permissions.includes(permission));
      return { target, report: { channelId: perms.channelId, guildId: perms.guildId, isDm: perms.isDm, channelType: perms.channelType, permissions: perms.permissions, missingRequired: missing.length ? missing : [], raw: perms.raw } };
    }
  }
  catch (err) {
    {
...
async function resolveChannelReports(params) {
  ...
  for (const accountId of accountIds) {
    ...
    if ((((plugin.id === "slack") && configured) && enabled)) {
      ...
      const scopeReports = [];
      if (botToken) {
        scopeReports.push({ tokenType: "bot", result: await fetchSlackScopes(botToken, timeoutMs) });
      } else {
        scopeReports.push({ tokenType: "bot", result: { ok: false, error: "Slack bot token missing." } });
      }
      if (userToken) {
        scopeReports.push({ tokenType: "user", result: await fetchSlackScopes(userToken, timeoutMs) });
      }
    ...
    if (((plugin.id === "discord") && params.target)) {
      const perms = await buildDiscordPermissions({ account: resolvedAccount, target: params.target });
      discordTarget = perms.target;
      discordPermissions = perms.report;
    }
    reports.push({ channel: plugin.id, accountId, accountName: (typeof resolvedAccount.name === "string") ? (resolvedAccount.name?.trim() || undefined) : undefined, configured, enabled, support: plugin.capabilities, probe, target: discordTarget, channelPermissions: discordPermissions, actions, slackScopes });
  }
  return reports;
}
export async function channelsCapabilitiesCommand(opts, runtime = defaultRuntime) {
  ...
  if ((!selected || (selected.length === 0))) {
    runtime.error(danger("Unknown channel \"\"."));
    runtime.exit(1);
    return;
  }
  const reports = [];
  for (const plugin of selected) {
    ...
    reports.push(...await resolveChannelReports({ plugin, cfg, timeoutMs, accountOverride, target: (rawTarget && (plugin.id === "discord")) ? rawTarget : undefined }));
  }
  if (opts.json) {
  ...
  for (const report of reports) {
    const label = formatChannelAccountLabel({ channel: report.channel, accountId: report.accountId, name: report.accountName, channelStyle: theme.accent, accountStyle: theme.heading });
    ...
    if ((probeLines.length > 0)) {
      ...
      if ((report.configured && report.enabled)) {
    ...
    if (((report.channel === "discord") && report.channelPermissions)) {
      ...
      if ((((report.channel === "discord") && rawTarget) && !report.channelPermissions)) {
