...
function formatAccountLine(params) {
  ...
  const label = formatChannelAccountLabel({ channel: channel.id, accountId: snapshot.accountId, name: snapshot.name, channelStyle: theme.accent, accountStyle: theme.heading });
  const bits = [];
...
export async function channelsListCommand(opts, runtime = defaultRuntime) {
  ...
  const authProfiles = Object.entries(authStore.profiles).map(([profileId, profile]) => { id: profileId, provider: profile.provider, type: profile.type, isExternal: false });
  if (opts.json) {
  ...
  for (const plugin of plugins) {
    ...
    for (const accountId of accounts) {
      ...
      lines.push(formatAccountLine({ channel: plugin, snapshot }));
    }
  }
  lines.push("");
