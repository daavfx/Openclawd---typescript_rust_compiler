...
export async function agentsSetIdentityCommand(opts, runtime = defaultRuntime) {
  ...
  if (!agentId) {
    ...
    if ((matches.length === 0)) {
      runtime.error("No agent workspace matches . Pass --agent to target a specific agent.");
      runtime.exit(1);
      return;
    }
    if ((matches.length > 1)) {
      runtime.error("Multiple agents match : . Pass --agent to choose one.");
      runtime.exit(1);
      return;
    }
    agentId = matches[0];
  ...
  if (wantsIdentityFile) {
    ...
    if (!identityFromFile) {
      ...
      runtime.error("No identity data found in .");
      runtime.exit(1);
      return;
    }
  }
  const fileTheme = (((identityFromFile?.theme ?? identityFromFile?.creature) ?? identityFromFile?.vibe) ?? undefined);
  const incomingIdentity = { ...(nameRaw || identityFromFile?.name) ? { name: (nameRaw ?? identityFromFile?.name) } : {  }: , ...(emojiRaw || identityFromFile?.emoji) ? { emoji: (emojiRaw ?? identityFromFile?.emoji) } : {  }: , ...(themeRaw || fileTheme) ? { theme: (themeRaw ?? fileTheme) } : {  }: , ...(avatarRaw || identityFromFile?.avatar) ? { avatar: (avatarRaw ?? identityFromFile?.avatar) } : {  }:  };
  ...
  const nextIdentity = { ...base.identity: , ...incomingIdentity:  };
  const nextEntry = { ...base: , identity: nextIdentity };
  const nextList = [...list];
  if ((index >= 0)) {
  ...
  if (opts.json) {
    runtime.log(JSON.stringify({ agentId, identity: nextIdentity, workspace: (workspaceDir ?? null), identityFile: (identityFilePath ?? null) }, null, 2));
    return;
  }
