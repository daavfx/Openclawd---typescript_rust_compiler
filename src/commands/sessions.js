...
const formatFlagsCell = (row, rich) => {
  const flags = [row.thinkingLevel ? "think:" : null, row.verboseLevel ? "verbose:" : null, row.reasoningLevel ? "reasoning:" : null, row.elevatedLevel ? "elev:" : null, row.responseUsage ? "usage:" : null, row.groupActivation ? "activation:" : null, row.systemSent ? "system" : null, row.abortedLastRun ? "aborted" : null, row.sessionId ? "id:" : null].filter(Boolean);
...
export async function sessionsCommand(opts, runtime) {
  ...
  if (opts.json) {
    runtime.log(JSON.stringify({ path: storePath, count: rows.length, activeMinutes: (activeMinutes ?? null), sessions: rows.map((r) => { ...r: , contextTokens: (((r.contextTokens ?? lookupContextTokens(r.model)) ?? configContextTokens) ?? null), model: ((r.model ?? configModel) ?? null) }) }, null, 2));
    return;
  }
  runtime.log(info("Session store: "));
  runtime.log(info("Sessions listed: "));
  if (activeMinutes) {
    runtime.log(info("Filtered to last  minute(s)"));
  }
  if ((rows.length === 0)) {
  ...
  const rich = isRich();
  const header = ["Kind".padEnd(KIND_PAD), "Key".padEnd(KEY_PAD), "Age".padEnd(AGE_PAD), "Model".padEnd(MODEL_PAD), "Tokens (ctx %)".padEnd(TOKENS_PAD), "Flags"].join(" ");
  ...
  for (const row of rows) {
    ...
    const line = [formatKindCell(row.kind, rich), keyCell, formatAgeCell(row.updatedAt, rich), formatModelCell(model, rich), formatTokensCell(total, (contextTokens ?? null), rich), formatFlagsCell(row, rich)].join(" ");
