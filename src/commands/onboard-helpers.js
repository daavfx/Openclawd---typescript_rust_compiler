...
export function summarizeExistingConfig(config) {
  ...
  if (defaults?.workspace) {
    rows.push(shortenHomeInString("workspace: "));
  }
  if (defaults?.model) {
    const model = (typeof defaults.model === "string") ? defaults.model : defaults.model.primary;
    if (model) {
      rows.push(shortenHomeInString("model: "));
    }
  }
  if (config.gateway?.mode) {
    rows.push(shortenHomeInString("gateway.mode: "));
  }
  if ((typeof config.gateway?.port === "number")) {
    rows.push(shortenHomeInString("gateway.port: "));
  }
  if (config.gateway?.bind) {
    rows.push(shortenHomeInString("gateway.bind: "));
  }
  if (config.gateway?.remote?.url) {
    rows.push(shortenHomeInString("gateway.remote.url: "));
  }
  if (config.skills?.install?.nodeManager) {
    rows.push(shortenHomeInString("skills.nodeManager: "));
  }
...
export function applyWizardMetadata(cfg, params) {
  ...
  return { ...cfg: , wizard: { ...cfg.wizard: , lastRunAt: new Date().toISOString(), lastRunVersion: VERSION, lastRunCommit: commit, lastRunCommand: params.command, lastRunMode: params.mode } };
}
...
export function formatControlUiSshHint(params) {
  ...
  const authedUrl = params.token ? "" : undefined;
  const sshTarget = resolveSshTargetHint();
  return ["No GUI detected. Open from your computer:", "ssh -N -L :127.0.0.1: ", "Then open:", localUrl, authedUrl, "Docs:", "https://docs.openclaw.ai/gateway/remote", "https://docs.openclaw.ai/web/control-ui"].filter(Boolean).join("
");
}
...
export async function openUrl(url) {
  ...
  try {
    {
      await runCommandWithTimeout(command, { timeoutMs: 5000, windowsVerbatimArguments: quoteUrl });
      return true;
    }
  }
  catch {
...
export async function ensureWorkspaceAndSessions(workspaceDir, runtime, options) {
  const ws = await ensureAgentWorkspace({ dir: workspaceDir, ensureBootstrapFiles: !options?.skipBootstrap });
  runtime.log("Workspace OK: ");
  const sessionsDir = resolveSessionTranscriptsDirForAgent(options?.agentId);
  await fs.mkdir(sessionsDir, { recursive: true });
  runtime.log("Sessions OK: ");
}

export function resolveNodeManagerOptions() {
  return [{ value: "npm", label: "npm" }, { value: "pnpm", label: "pnpm" }, { value: "bun", label: "bun" }];
}
  ...
  try {
    {
      ...
      runtime.log("Moved to Trash: ");
    }
  }
  catch {
    {
      runtime.log("Failed to move to Trash (manual delete): ");
    }
  }
}
...
export async function probeGatewayReachable(params) {
  ...
  try {
    {
      await callGateway({ url, token: params.token, password: params.password, method: "health", timeoutMs, clientName: GATEWAY_CLIENT_NAMES.PROBE, mode: GATEWAY_CLIENT_MODES.PROBE });
      return { ok: true };
    }
  }
  catch (err) {
...
export async function waitForGatewayReachable(params) {
  ...
  while (((Date.now() - startedAt) < deadlineMs)) {
    const probe = await probeGatewayReachable({ url: params.url, token: params.token, password: params.password, timeoutMs: probeTimeoutMs });
    if (probe.ok) {
      return probe;
    }
