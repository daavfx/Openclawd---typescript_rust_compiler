...
const mocks = vi.hoisted(() => { runCommandWithTimeout: vi.fn(async () => { stdout: "", stderr: "", code: 0, signal: null, killed: false }), pickPrimaryTailnetIPv4: vi.fn(() => undefined) });
vi.mock("../process/exec.js", () => { runCommandWithTimeout: mocks.runCommandWithTimeout });
vi.mock("../infra/tailnet.js", () => { pickPrimaryTailnetIPv4: mocks.pickPrimaryTailnetIPv4 });
afterEach(() => {
  vi.unstubAllEnvs();
});
describe("openUrl", () => {
  it("quotes URLs on win32 so '&' is not treated as cmd separator", async () => {
    ...
    expect(options).toMatchObject({ timeoutMs: 5000, windowsVerbatimArguments: true });
...
describe("resolveControlUiLinks", () => {
  it("uses customBindHost for custom bind", () => {
    const links = resolveControlUiLinks({ port: 18789, bind: "custom", customBindHost: "192.168.1.100" });
  ...
  it("falls back to loopback for invalid customBindHost", () => {
    const links = resolveControlUiLinks({ port: 18789, bind: "custom", customBindHost: "192.168.001.100" });
    expect(links.httpUrl).toBe("http://127.0.0.1:18789/");
    expect(links.wsUrl).toBe("ws://127.0.0.1:18789");
  });
  it("uses tailnet IP for tailnet bind", () => {
    mocks.pickPrimaryTailnetIPv4.mockReturnValueOnce("100.64.0.9");
    const links = resolveControlUiLinks({ port: 18789, bind: "tailnet" });
  ...
  it("keeps loopback for auto even when tailnet is present", () => {
    mocks.pickPrimaryTailnetIPv4.mockReturnValueOnce("100.64.0.9");
    const links = resolveControlUiLinks({ port: 18789, bind: "auto" });
    expect(links.httpUrl).toBe("http://127.0.0.1:18789/");
    expect(links.wsUrl).toBe("ws://127.0.0.1:18789");
  });
});
