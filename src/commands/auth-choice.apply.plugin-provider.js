...
export async function applyAuthChoicePluginProvider(params, options) {
  ...
  if (!enableResult.enabled) {
    await params.prompter.note(" plugin is disabled ().", options.label);
  ...
  if (!provider) {
    await params.prompter.note(" auth plugin is not available. Enable it and re-run the wizard.", options.label);
    return { config: nextConfig };
  }
  const method = (pickAuthMethod(provider, options.methodId) ?? provider.auth[0]);
  if (!method) {
    await params.prompter.note(" auth method missing.", options.label);
    return { config: nextConfig };
  }
  const isRemote = isRemoteEnvironment();
  const result = await method.run({ config: nextConfig, agentDir, workspaceDir, prompter: params.prompter, runtime: params.runtime, isRemote, openUrl: async (url) => {
    await openUrl(url);
  ...
  for (const profile of result.profiles) {
    ...
    nextConfig = applyAuthProfileConfig(nextConfig, { profileId: profile.profileId, provider: profile.credential.provider, mode: (profile.credential.type === "token") ? "token" : profile.credential.type, ...(("email" in profile.credential) && profile.credential.email) ? { email: profile.credential.email } : {  }:  });
  }
