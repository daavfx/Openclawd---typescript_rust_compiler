...
export async function messageCommand(opts, deps, runtime) {
  ...
  const run = async () => await runMessageAction({ cfg, action, params: opts, deps: outboundDeps, gateway: { clientName: GATEWAY_CLIENT_NAMES.CLI, mode: GATEWAY_CLIENT_MODES.CLI } });
  const json = (opts.json === true);
  const dryRun = (opts.dryRun === true);
  const needsSpinner = ((!json && !dryRun) && ((action === "send") || (action === "poll")));
  const result = needsSpinner ? await withProgress({ label: (action === "poll") ? "Sending poll..." : "Sending...", indeterminate: true, enabled: true }, run) : await run();
  if (json) {
