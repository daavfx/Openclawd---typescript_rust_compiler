...
async function promptManualModel(params) {
  const modelInput = await params.prompter.text({ message: params.allowBlank ? "Default model (blank to keep)" : "Default model", initialValue: params.initialValue, placeholder: "provider/model", validate: params.allowBlank ? undefined : (value) => value?.trim() ? undefined : "Required" });
...
export async function promptDefaultModel(params) {
  ...
  const preferredProvider = preferredProviderRaw ? normalizeProviderId(preferredProviderRaw) : undefined;
  const configuredRaw = resolveConfiguredModelRaw(cfg);
  const resolved = resolveConfiguredModelRef({ cfg, defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL });
  const resolvedKey = modelKey(resolved.provider, resolved.model);
  const configuredKey = configuredRaw ? resolvedKey : "";
  const catalog = await loadModelCatalog({ config: cfg, useCache: false });
  if ((catalog.length === 0)) {
    return promptManualModel({ prompter: params.prompter, allowBlank: allowKeep, initialValue: ((configuredRaw || resolvedKey) || undefined) });
  }
  const aliasIndex = buildModelAliasIndex({ cfg, defaultProvider: DEFAULT_PROVIDER });
  let models = catalog;
  if (!ignoreAllowlist) {
    const {allowedCatalog} = buildAllowedModelSet({ cfg, catalog, defaultProvider: DEFAULT_PROVIDER });
  ...
  if (shouldPromptProvider) {
    const selection = await params.prompter.select({ message: "Filter models by provider", options: [{ value: "*", label: "All providers" }, ...providers.map((provider) => {
      const count = models.filter((entry) => (entry.provider === provider)).length;
      return { value: provider, label: provider, hint: " model" };
    })] });
    if ((selection !== "*")) {
  ...
  const options = [];
  if (allowKeep) {
    options.push({ value: KEEP_VALUE, label: configuredRaw ? "Keep current ()" : "Keep current (default: )", hint: (configuredRaw && (configuredRaw !== resolvedKey)) ? "resolves to " : undefined });
  }
  if (includeManual) {
  ...
  const addModelOption = (entry) => {
    ...
    options.push({ value: key, label: key, hint: (hints.length > 0) ? hints.join(" · ") : undefined });
    seen.add(key);
  };
  ...
  if ((configuredKey && !seen.has(configuredKey))) {
    options.push({ value: configuredKey, label: configuredKey, hint: "current (not in catalog)" });
  }
  ...
  const selection = await params.prompter.select({ message: (params.message ?? "Default model"), options, initialValue });
  if ((selection === KEEP_VALUE)) {
    return {  };
  }
  if ((selection === MANUAL_VALUE)) {
    return promptManualModel({ prompter: params.prompter, allowBlank: false, initialValue: ((configuredRaw || resolvedKey) || undefined) });
  }
...
export async function promptModelAllowlist(params) {
  ...
  const resolved = resolveConfiguredModelRef({ cfg, defaultProvider: DEFAULT_PROVIDER, defaultModel: DEFAULT_MODEL });
  const resolvedKey = modelKey(resolved.provider, resolved.model);
  const initialSeeds = normalizeModelKeys([...existingKeys, resolvedKey, ...(params.initialSelections ?? [])]);
  ...
  if (((catalog.length === 0) && (allowedKeys.length === 0))) {
    const raw = await params.prompter.text({ message: (params.message ?? "Allowlist models (comma-separated provider/model; blank to keep current)"), initialValue: existingKeys.join(", "), placeholder: "openai-codex/gpt-5.2, anthropic/claude-opus-4-5" });
  ...
  for (const key of supplementalKeys) {
    ...
    options.push({ value: key, label: key, hint: allowedKeySet ? "allowed (not in catalog)" : "configured (not in catalog)" });
    seen.add(key);
  }
  if ((options.length === 0)) {
    return {  };
  }
  const selection = await params.prompter.multiselect({ message: (params.message ?? "Models in /model picker (multi-select)"), options, initialValues: (initialKeys.length > 0) ? initialKeys : undefined });
  ...
  const confirmClear = await params.prompter.confirm({ message: "Clear the model allowlist? (shows all models)", initialValue: false });
  if (!confirmClear) {
    return {  };
  }
  return { models: [] };
...
export function applyPrimaryModel(cfg, model) {
  ...
  const fallbacks = (((typeof existingModel === "object") && (existingModel !== null)) && ("fallbacks" in existingModel)) ? existingModel.fallbacks : undefined;
  return { ...cfg: , agents: { ...cfg.agents: , defaults: { ...defaults: , model: { ...fallbacks ? { fallbacks } : undefined: , primary: model }, models: { ...existingModels: , [model]: (existingModels?.[model] ?? {  }) } } } };
}
...
export function applyModelFallbacksFromSelection(cfg, selection) {
  ...
  const defaults = cfg.agents?.defaults;
  const existingModel = defaults?.model;
  const existingPrimary = (typeof existingModel === "string") ? existingModel : (existingModel && (typeof existingModel === "object")) ? existingModel.primary : undefined;
  const fallbacks = normalized.filter((key) => (key !== resolvedKey));
  return { ...cfg: , agents: { ...cfg.agents: , defaults: { ...defaults: , model: { ...(typeof existingModel === "object") ? existingModel : undefined: , primary: (existingPrimary ?? resolvedKey), fallbacks } } } };
}
