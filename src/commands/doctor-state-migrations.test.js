...
import { autoMigrateLegacyStateDir, autoMigrateLegacyState, detectLegacyStateMigrations, resetAutoMigrateLegacyStateDirForTest, resetAutoMigrateLegacyStateForTest, runLegacyStateMigrations } from "./doctor-state-migrations.js";
let tempRoot = null;
...
describe("doctor legacy state migrations", () => {
  it("migrates legacy sessions into agents/<id>/sessions", async () => {
    ...
    writeJson5(path.join(legacySessionsDir, "sessions.json"), { "+1555": { sessionId: "a", updatedAt: 10 }, "+1666": { sessionId: "b", updatedAt: 20 }, "slack:channel:C123": { sessionId: "c", updatedAt: 30 }, "group:abc": { sessionId: "d", updatedAt: 40 }, "subagent:xyz": { sessionId: "e", updatedAt: 50 } });
    fs.writeFileSync(path.join(legacySessionsDir, "a.jsonl"), "a", "utf-8");
    fs.writeFileSync(path.join(legacySessionsDir, "b.jsonl"), "b", "utf-8");
    const detected = await detectLegacyStateMigrations({ cfg, env: { OPENCLAW_STATE_DIR: root } });
    ...
    const store = JSON.parse(fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"));
  ...
  it("auto-migrates legacy agent dir on startup", async () => {
    ...
    const result = await autoMigrateLegacyState({ cfg, env: { OPENCLAW_STATE_DIR: root }, log });
    const targetAgentDir = path.join(root, "agents", "main", "agent");
  ...
  it("auto-migrates legacy sessions on startup", async () => {
    ...
    const legacySessionsDir = path.join(root, "sessions");
    fs.mkdirSync(legacySessionsDir, { recursive: true });
    writeJson5(path.join(legacySessionsDir, "sessions.json"), { "+1555": { sessionId: "a", updatedAt: 10 } });
    fs.writeFileSync(path.join(legacySessionsDir, "a.jsonl"), "a", "utf-8");
    const log = { info: vi.fn(), warn: vi.fn() };
    const result = await autoMigrateLegacyState({ cfg, env: { OPENCLAW_STATE_DIR: root }, log, now: () => 123 });
    expect(result.migrated).toBe(true);
    expect(log.info).toHaveBeenCalled();
    const targetDir = path.join(root, "agents", "main", "sessions");
    expect(fs.existsSync(path.join(targetDir, "a.jsonl"))).toBe(true);
  ...
  it("routes legacy state to the default agent entry", async () => {
    const root = await makeTempRoot();
    const cfg = { agents: { list: [{ id: "alpha", default: true }] } };
    const legacySessionsDir = path.join(root, "sessions");
    fs.mkdirSync(legacySessionsDir, { recursive: true });
    writeJson5(path.join(legacySessionsDir, "sessions.json"), { "+1555": { sessionId: "a", updatedAt: 10 } });
    const detected = await detectLegacyStateMigrations({ cfg, env: { OPENCLAW_STATE_DIR: root } });
  ...
  it("honors session.mainKey when seeding the direct-chat bucket", async () => {
    ...
    const legacySessionsDir = path.join(root, "sessions");
    fs.mkdirSync(legacySessionsDir, { recursive: true });
    writeJson5(path.join(legacySessionsDir, "sessions.json"), { "+1555": { sessionId: "a", updatedAt: 10 }, "+1666": { sessionId: "b", updatedAt: 20 } });
    const detected = await detectLegacyStateMigrations({ cfg, env: { OPENCLAW_STATE_DIR: root } });
    await runLegacyStateMigrations({ detected, now: () => 123 });
    const targetDir = path.join(root, "agents", "main", "sessions");
    const store = JSON.parse(fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"));
  ...
  it("canonicalizes legacy main keys inside the target sessions store", async () => {
    ...
    const cfg = {  };
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), { main: { sessionId: "legacy", updatedAt: 10 }, "agent:main:main": { sessionId: "fresh", updatedAt: 20 } });
    const detected = await detectLegacyStateMigrations({ cfg, env: { OPENCLAW_STATE_DIR: root } });
    await runLegacyStateMigrations({ detected, now: () => 123 });
    const store = JSON.parse(fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"));
  ...
  it("prefers the newest entry when collapsing main aliases", async () => {
    ...
    const cfg = { session: { mainKey: "work" } };
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), { "agent:main:main": { sessionId: "legacy", updatedAt: 50 }, "agent:main:work": { sessionId: "canonical", updatedAt: 10 } });
    const detected = await detectLegacyStateMigrations({ cfg, env: { OPENCLAW_STATE_DIR: root } });
    await runLegacyStateMigrations({ detected, now: () => 123 });
    const store = JSON.parse(fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"));
  ...
  it("lowercases agent session keys during canonicalization", async () => {
    ...
    const cfg = {  };
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), { "agent:main:slack:channel:C123": { sessionId: "legacy", updatedAt: 10 } });
    const detected = await detectLegacyStateMigrations({ cfg, env: { OPENCLAW_STATE_DIR: root } });
    await runLegacyStateMigrations({ detected, now: () => 123 });
    const store = JSON.parse(fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"));
  ...
  it("auto-migrates when only target sessions contain legacy keys", async () => {
    ...
    const cfg = {  };
    const targetDir = path.join(root, "agents", "main", "sessions");
    writeJson5(path.join(targetDir, "sessions.json"), { main: { sessionId: "legacy", updatedAt: 10 } });
    const log = { info: vi.fn(), warn: vi.fn() };
    const result = await autoMigrateLegacyState({ cfg, env: { OPENCLAW_STATE_DIR: root }, log });
    const store = JSON.parse(fs.readFileSync(path.join(targetDir, "sessions.json"), "utf-8"));
    expect(result.migrated).toBe(true);
  ...
  it("skips state dir migration when env override is set", async () => {
    ...
    const result = await autoMigrateLegacyStateDir({ env: { OPENCLAW_STATE_DIR: "/custom/state" }, homedir: () => root });
