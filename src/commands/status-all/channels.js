...
function summarizeSources(sources) {
  ...
  const parts = [...counts.entries()].sort((a, b) => (b[1] - a[1])).map(([key, n]) => "");
...
const buildAccountSnapshot = (params) => {
  ...
  return { enabled: params.enabled, configured: params.configured, ...described: , accountId: params.accountId };
};
const formatAllowFrom = (params) => {
  if (params.plugin.config.formatAllowFrom) {
    return params.plugin.config.formatAllowFrom({ cfg: params.cfg, accountId: params.accountId, allowFrom: params.allowFrom });
  }
...
const buildAccountNotes = (params) => {
  ...
  if (allowFrom?.length) {
    const formatted = formatAllowFrom({ plugin, cfg, accountId: snapshot.accountId, allowFrom }).slice(0, 3);
...
function summarizeTokenConfig(params) {
  ...
  if (hasBotOrAppTokenFields) {
    ...
    if ((partial.length > 0)) {
      return { state: "warn", detail: "partial tokens (need bot+app) · accounts " };
    }
    if ((ready.length === 0)) {
    ...
    const hint = (botHint || appHint) ? " (bot , app )" : "";
    return { state: "ok", detail: "tokens ok (bot , app ) · accounts /" };
  }
  const ready = enabled.filter((a) => {
    const rec = asRecord(a.account);
  ...
  return { state: "ok", detail: "token  · accounts /" };
}
export async function buildChannelsTable(cfg, opts) {
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    for (const accountId of resolvedAccountIds) {
      ...
      const snapshot = buildAccountSnapshot({ plugin, cfg, accountId, account, enabled, configured });
    ...
    const summary = plugin.status?.buildChannelSummary ? await plugin.status.buildChannelSummary({ account: (defaultEntry?.account ?? {  }), cfg, defaultAccountId, snapshot: (defaultEntry?.snapshot ?? { accountId: defaultAccountId }) }) : undefined;
    const link = resolveLinkFields(summary);
    const missingPaths = collectMissingPaths(enabledAccounts);
    const tokenSummary = summarizeTokenConfig({ plugin, cfg, accounts, showSecrets });
    ...
    rows.push({ id: plugin.id, label, enabled: anyEnabled, state, detail });
    if ((configuredAccounts.length > 0)) {
      details.push({ title: " accounts", columns: ["Account", "Status", "Notes"], rows: configuredAccounts.map((entry) => {
        const notes = buildAccountNotes({ plugin, cfg, entry });
        return { Account: formatAccountLabel({ accountId: entry.accountId, name: entry.snapshot.name }), Status: (entry.enabled !== false) ? "OK" : "WARN", Notes: notes.join(" · ") };
      }) });
    }
  }
  return { rows, details };
