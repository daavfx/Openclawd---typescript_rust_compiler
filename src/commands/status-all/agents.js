...
export async function getAgentLocalStatuses(cfg) {
  ...
  const agents = await Promise.all(agentList.agents.map(async (agent) => {
    ...
    return { id: agent.id, name: agent.name, workspaceDir, bootstrapPending, sessionsPath, sessionsCount, lastUpdatedAt, lastActiveAgeMs };
  }));
  ...
  return { defaultId: agentList.defaultId, agents, totalSessions, bootstrapPendingCount };
}
