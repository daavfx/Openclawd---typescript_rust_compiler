...
export function summarizeLogTail(rawLines, opts) {
  ...
  for (let i = 0; (i < lines.length); i += 1) {
    ...
    if (embedded) {
      ...
      addGroup("embedded:", "Embedded agent: OAuth token refresh failed ()");
      continue;
    }
  ...
  const kept = [...deduped.slice(0, head), "…  lines omitted …", ...deduped.slice(-tail)];
  return kept;
}
