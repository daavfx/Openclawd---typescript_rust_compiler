...
vi.mock("../channel-web.js", () => { loginWeb: vi.fn(async () => {
}) });
vi.mock("./onboard-helpers.js", () => { detectBinary: vi.fn(async () => false) });
describe("setupChannels", () => {
  ...
  it("QuickStart uses single-select (no multiselect) and doesn't prompt for Telegram token when WhatsApp is chosen", async () => {
    ...
    }), select, multiselect, text: text, confirm: vi.fn(async () => false), progress: vi.fn(() => { update: vi.fn(), stop: vi.fn() }) };
    const runtime = { log: vi.fn(), error: vi.fn(), exit: vi.fn((code) => {
      throw new Error("exit:");
    }) };
    await setupChannels({  }, runtime, prompter, { skipConfirm: true, quickstartDefaults: true, forceAllowFromChannels: ["whatsapp"] });
  ...
  it("prompts for configured channel action and skips configuration when told to skip", async () => {
    ...
    await setupChannels({ channels: { telegram: { botToken: "token" } } }, runtime, prompter, { skipConfirm: true, quickstartDefaults: true });
    expect(select).toHaveBeenCalledWith(expect.objectContaining({ message: "Select channel (QuickStart)" }));
  ...
  it("adds disabled hint to channel selection when a channel is disabled", async () => {
    ...
    await setupChannels({ channels: { telegram: { botToken: "token", enabled: false } } }, runtime, prompter, { skipConfirm: true });
