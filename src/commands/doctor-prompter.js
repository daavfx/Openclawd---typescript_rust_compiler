...
export function createDoctorPrompter(params) {
  ...
  const confirmDefault = async (p) => {
    ...
    return (guardCancel(await confirm({ ...p: , message: stylePromptMessage(p.message) }), params.runtime) === true);
  };
  return { confirm: confirmDefault, confirmRepair: async (p) => {
    if (nonInteractive) {
      return false;
    }
    ...
    return guardCancel(await select({ ...p: , message: stylePromptMessage(p.message), options: p.options.map((opt) => (opt.hint === undefined) ? opt : { ...opt: , hint: stylePromptHint(opt.hint) }) }), params.runtime);
  }, shouldRepair, shouldForce };
}
