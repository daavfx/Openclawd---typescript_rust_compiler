...
import { countMismatches, countRunning, formatAge, formatImageMatch, formatSimpleStatus, formatStatus } from "./sandbox-formatters.js";
describe("sandbox-formatters", () => {
  ...
  describe("countRunning", () => {
    it("should count running items", () => {
      const items = [{ running: true, name: "a" }, { running: false, name: "b" }, { running: true, name: "c" }, { running: false, name: "d" }];
    ...
    it("should return 0 when no items running", () => {
      const items = [{ running: false, name: "a" }, { running: false, name: "b" }];
      expect(countRunning(items)).toBe(0);
    });
    it("should count all when all running", () => {
      const items = [{ running: true, name: "a" }, { running: true, name: "b" }, { running: true, name: "c" }];
      expect(countRunning(items)).toBe(3);
    });
  });
  describe("countMismatches", () => {
    it("should count image mismatches", () => {
      const items = [{ imageMatch: true, name: "a" }, { imageMatch: false, name: "b" }, { imageMatch: true, name: "c" }, { imageMatch: false, name: "d" }, { imageMatch: false, name: "e" }];
    ...
    it("should return 0 when all match", () => {
      const items = [{ imageMatch: true, name: "a" }, { imageMatch: true, name: "b" }];
      expect(countMismatches(items)).toBe(0);
    });
    it("should count all when none match", () => {
      const items = [{ imageMatch: false, name: "a" }, { imageMatch: false, name: "b" }, { imageMatch: false, name: "c" }];
      expect(countMismatches(items)).toBe(3);
    });
  });
});
