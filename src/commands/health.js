...
const formatDurationParts = (ms) => {
  ...
  const units = [{ label: "w", size: ((((7 * 24) * 60) * 60) * 1000) }, { label: "d", size: (((24 * 60) * 60) * 1000) }, { label: "h", size: ((60 * 60) * 1000) }, { label: "m", size: (60 * 1000) }, { label: "s", size: 1000 }];
...
const buildSessionSummary = (storePath) => {
  ...
  const sessions = Object.entries(store).filter(([key]) => ((key !== "global") && (key !== "unknown"))).map(([key, entry]) => { key, updatedAt: (entry?.updatedAt ?? 0) }).sort((a, b) => (b.updatedAt - a.updatedAt));
  const recent = sessions.slice(0, 5).map((s) => { key: s.key, updatedAt: (s.updatedAt || null), age: s.updatedAt ? (Date.now() - s.updatedAt) : null });
  return { path: storePath, count: sessions.length, recent };
};
...
const formatAccountProbeTiming = (summary) => {
  ...
  const handle = botUsername ? "@" : accountId;
  const timing = (elapsedMs != null) ? "ms" : "ok";
  return "::";
};
...
export const formatHealthChannelLines = (summary, opts = {  }) => {
  ...
  for (const channelId of channelOrder) {
    ...
    const filteredSummaries = (accountIds && (accountIds.length > 0)) ? accountIds.map((accountId) => accountSummaries[accountId]).filter((entry) => Boolean(entry)) : undefined;
    ...
    if ((linked !== null)) {
      if (linked) {
        ...
        const authLabel = (authAgeMs != null) ? " (auth age m)" : "";
        lines.push(": linked");
      } else {
...
export async function getHealthSnapshot(params) {
  ...
  const agents = ordered.map((entry) => {
    ...
    return { agentId: entry.id, name: entry.name, isDefault: (entry.id === defaultAgentId), heartbeat: resolveHeartbeatSummary(cfg, entry.id), sessions };
  });
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    const preferredAccountId = resolvePreferredAccountId({ accountIds, defaultAccountId, boundAccounts });
    ...
    debugHealth("channel", { id: plugin.id, accountIds, defaultAccountId, boundAccounts, preferredAccountId, accountIdsToProbe });
    ...
    for (const accountId of accountIdsToProbe) {
      ...
      if ((((enabled && configured) && doProbe) && plugin.status?.probeAccount)) {
        try {
          {
            probe = await plugin.status.probeAccount({ account, timeoutMs: cappedTimeout, cfg });
      ...
      const snapshot = { accountId, enabled, configured };
      if ((probe !== undefined)) {
      ...
      const summary = plugin.status?.buildChannelSummary ? await plugin.status.buildChannelSummary({ account, cfg, defaultAccountId: accountId, snapshot }) : undefined;
      const record = (summary && (typeof summary === "object")) ? summary : { accountId, configured, probe, lastProbeAt };
    ...
    if (fallbackSummary) {
      channels[plugin.id] = { ...fallbackSummary: , accounts: accountSummaries };
    }
  }
  const summary = { ok: true, ts: Date.now(), durationMs: (Date.now() - start), channels, channelOrder, channelLabels, heartbeatSeconds, defaultAgentId, agents, sessions: { path: sessions.path, count: sessions.count, recent: sessions.recent } };
  return summary;
}

export async function healthCommand(opts, runtime) {
  const cfg = (opts.config ?? loadConfig());
  const summary = await withProgress({ label: "Checking gateway health…", indeterminate: true, enabled: (opts.json !== true) }, async () => await callGateway({ method: "health", params: opts.verbose ? { probe: true } : undefined, timeoutMs: opts.timeoutMs, config: cfg }));
  const fatal = false;
  if (opts.json) {
...
")) {
    ...
    const fallbackAgents = localAgents.ordered.map((entry) => {
      const storePath = resolveStorePath(cfg.session?.store, { agentId: entry.id });
      return { agentId: entry.id, name: entry.name, isDefault: (entry.id === localAgents.defaultAgentId), heartbeat: resolveHeartbeatSummary(cfg, entry.id), sessions: buildSessionSummary(storePath) };
    });
    ...
    const channelAccountFallbacks = Object.fromEntries(listChannelPlugins().map((plugin) => {
      const accountIds = plugin.config.listAccountIds(cfg);
      const defaultAccountId = resolveChannelDefaultAccountId({ plugin, cfg, accountIds });
      const preferred = resolvePreferredAccountId({ accountIds, defaultAccountId, boundAccounts: (channelBindings.get(plugin.id)?.get(defaultAgentId) ?? []) });
    ...
    const channelLines = (Object.keys(accountIdsByChannel).length > 0) ? formatHealthChannelLines(summary, { accountMode: opts.verbose ? "all" : "default", accountIdsByChannel }) : formatHealthChannelLines(summary, { accountMode: opts.verbose ? "all" : "default" });
    ...
    for (const plugin of listChannelPlugins()) {
      ...
      const account = plugin.config.resolveAccount(cfg, accountId);
      plugin.status.logSelfId({ account, cfg, runtime, includeChannelPrefix: true });
    }
    if ((resolvedAgents.length > 0)) {
      const agentLabels = resolvedAgents.map((agent) => agent.isDefault ? " (default)" : agent.agentId);
      runtime.log(info("Agents: "));
    }
    ...
    if ((heartbeatParts.length > 0)) {
      runtime.log(info("Heartbeat interval: "));
    }
    if ((displayAgents.length === 0)) {
      runtime.log(info("Session store:  ( entries)"));
