...
vi.mock("../gateway/client.js", () => { GatewayClient: function() {
} });
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    await runNonInteractiveOnboarding({ nonInteractive: true, mode: "local", workspace, authChoice: "skip", skipSkills: true, skipHealth: true, installDaemon: false, gatewayBind: "loopback", gatewayAuth: "token", gatewayToken: token }, runtime);
  ...
  it("writes gateway.remote url/token and callGateway uses them", async () => {
    ...
    const {runNonInteractiveOnboarding} = await import("./onboard-non-interactive.js");
    await runNonInteractiveOnboarding({ nonInteractive: true, mode: "remote", remoteUrl: "ws://127.0.0.1:", remoteToken: token, authChoice: "skip", json: true }, runtime);
    const {resolveConfigPath} = await import("../config/config.js");
    const cfg = JSON.parse(await fs.readFile(resolveConfigPath(), "utf8"));
    expect(cfg.gateway?.mode).toBe("remote");
    expect(cfg.gateway?.remote?.url).toBe("ws://127.0.0.1:");
    ...
    expect(lastCall?.url).toBe("ws://127.0.0.1:");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    const {runNonInteractiveOnboarding} = await import("./onboard-non-interactive.js");
    await runNonInteractiveOnboarding({ nonInteractive: true, mode: "local", workspace, authChoice: "skip", skipSkills: true, skipHealth: true, installDaemon: false, gatewayPort: port, gatewayBind: "lan" }, runtime);
    const {resolveConfigPath} = await import("../config/paths.js");
    const configPath = resolveConfigPath(process.env, stateDir);
    const cfg = JSON.parse(await fs.readFile(configPath, "utf8"));
