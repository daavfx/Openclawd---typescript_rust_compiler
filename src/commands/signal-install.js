  ...
  await new Promise((resolve, reject) => {
    const req = request(url, (res) => {
      ...
      if ((!res.statusCode || (res.statusCode >= 400))) {
        reject(new Error("HTTP  downloading file"));
        return;
      }
...
export async function installSignalCli(runtime) {
  if ((process.platform === "win32")) {
    return { ok: false, error: "Signal CLI auto-install is not supported on Windows yet." };
  }
  const apiUrl = "https://api.github.com/repos/AsamK/signal-cli/releases/latest";
  const response = await fetch(apiUrl, { headers: { "User-Agent": "openclaw", Accept: "application/vnd.github+json" } });
  if (!response.ok) {
    return { ok: false, error: "Failed to fetch release info ()" };
  }
  ...
  if ((!assetName || !assetUrl)) {
    return { ok: false, error: "No compatible release asset found for this platform." };
  }
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openclaw-signal-"));
  const archivePath = path.join(tmpDir, assetName);
  runtime.log("Downloading signal-cli  ()…");
  await downloadToFile(assetUrl, archivePath);
  const installRoot = path.join(CONFIG_DIR, "tools", "signal-cli", version);
  await fs.mkdir(installRoot, { recursive: true });
  if (assetName.endsWith(".zip")) {
    await runCommandWithTimeout(["unzip", "-q", archivePath, "-d", installRoot], { timeoutMs: 60000 });
  } else {
    if ((assetName.endsWith(".tar.gz") || assetName.endsWith(".tgz"))) {
      await runCommandWithTimeout(["tar", "-xzf", archivePath, "-C", installRoot], { timeoutMs: 60000 });
    } else {
  ...
  if (!cliPath) {
    return { ok: false, error: "signal-cli binary not found after extracting " };
  }
