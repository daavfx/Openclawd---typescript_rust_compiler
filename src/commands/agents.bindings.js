...
function bindingMatchKey(match) {
  ...
  return [match.channel, accountId, (match.peer?.kind ?? ""), (match.peer?.id ?? ""), (match.guildId ?? ""), (match.teamId ?? "")].join("|");
}
...
export function applyAgentBindings(cfg, bindings) {
  ...
  return { config: { ...cfg: , bindings: [...existing, ...added] }, added, skipped, conflicts };
}
...
export function parseBindingSpecs(params) {
  ...
  for (const raw of specs) {
    ...
    if (!channel) {
      errors.push("Unknown channel \"\".");
      continue;
    }
    let accountId = accountRaw?.trim();
    if (((accountRaw !== undefined) && !accountId)) {
      errors.push("Invalid binding \"\" (empty account id).");
      continue;
    }
    if (!accountId) {
      const plugin = getChannelPlugin(channel);
