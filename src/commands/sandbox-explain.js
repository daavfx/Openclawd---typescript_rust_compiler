...
function normalizeExplainSessionKey(params) {
  ...
  return buildAgentMainSessionKey({ agentId: params.agentId, mainKey: normalizeMainKey(raw) });
}
...
function resolveActiveChannel(params) {
  ...
  return inferProviderFromSessionKey({ cfg: params.cfg, sessionKey: params.sessionKey });
}
export async function sandboxExplainCommand(opts, runtime) {
  ...
  const sessionKey = normalizeExplainSessionKey({ cfg, agentId: resolvedAgentId, session: opts.session });
  ...
  const channel = resolveActiveChannel({ cfg, agentId: resolvedAgentId, sessionKey });
  ...
  if (!elevatedAgentEnabled) {
    elevatedFailures.push({ gate: "enabled", key: "agents.list[].tools.elevated.enabled" });
  }
  if ((channel && (globalAllowTokens.length === 0))) {
    elevatedFailures.push({ gate: "allowFrom", key: "tools.elevated.allowFrom." });
  }
  if (((channel && elevatedAgent?.allowFrom) && (agentAllowTokens.length === 0))) {
    elevatedFailures.push({ gate: "allowFrom", key: "agents.list[].tools.elevated.allowFrom." });
  }
  const fixIt = [];
...
");
  ...
  if (((payload.sandbox.mode === "non-main") && payload.sandbox.sessionIsSandboxed)) {
    lines.push("");
    lines.push(" sandbox mode is non-main; use main session key to run direct: ");
  }
  lines.push("");
