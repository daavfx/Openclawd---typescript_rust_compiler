...
export function resolveTargets(cfg, explicitUrl) {
  ...
  if (remoteUrl) {
    add({ id: "configRemote", kind: "configRemote", url: remoteUrl, active: (cfg.gateway?.mode === "remote") });
  }
  const port = resolveGatewayPort(cfg);
  add({ id: "localLoopback", kind: "localLoopback", url: "ws://127.0.0.1:", active: (cfg.gateway?.mode !== "remote") });
  return targets;
}
...
export function resolveAuthForTarget(cfg, target, overrides) {
  ...
  if (((target.kind === "configRemote") || (target.kind === "sshTunnel"))) {
    ...
    return { token: (token.length > 0) ? token : undefined, password: (password.length > 0) ? password : undefined };
  }
  ...
  return { token: ((envToken || cfgToken) || undefined), password: ((envPassword || cfgPassword) || undefined) };
}
...
export function pickGatewaySelfPresence(presence) {
  ...
  return { host: (typeof self.host === "string") ? self.host : undefined, ip: (typeof self.ip === "string") ? self.ip : undefined, version: (typeof self.version === "string") ? self.version : undefined, platform: (typeof self.platform === "string") ? self.platform : undefined };
}
...
export function renderProbeSummaryLine(probe, rich) {
  if (probe.ok) {
    const latency = (typeof probe.connectLatencyMs === "number") ? "ms" : "unknown";
    return " () · ";
  }
