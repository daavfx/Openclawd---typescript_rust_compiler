...
export async function resetCommand(runtime, opts) {
  ...
  if (!scope) {
    ...
    const selection = await selectStyled({ message: "Reset scope", options: [{ value: "config", label: "Config only", hint: "openclaw.json" }, { value: "config+creds+sessions", label: "Config + credentials + sessions", hint: "keeps workspace + auth profiles" }, { value: "full", label: "Full reset", hint: "state dir + workspace" }], initialValue: "config+creds+sessions" });
  ...
  if ((interactive && !opts.yes)) {
    const ok = await confirm({ message: stylePromptMessage("Proceed with  reset?") });
    if ((isCancel(ok) || !ok)) {
      cancel((stylePromptTitle("Reset cancelled.") ?? "Reset cancelled."));
      runtime.exit(0);
      return;
