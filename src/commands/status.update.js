...
export async function getUpdateCheckResult(params) {
  ...
  return await checkUpdateStatus({ root, timeoutMs: params.timeoutMs, fetchGit: params.fetchGit, includeRegistry: params.includeRegistry });
}
...
export function resolveUpdateAvailability(update) {
  ...
  return { available: (hasGitUpdate || hasRegistryUpdate), hasGitUpdate, hasRegistryUpdate, latestVersion: hasRegistryUpdate ? latestVersion : null, gitBehind };
}
...
export function formatUpdateOneLiner(update) {
  ...
  if (((update.installKind === "git") && update.git)) {
    const branch = update.git.branch ? "git " : "git";
    parts.push(branch);
