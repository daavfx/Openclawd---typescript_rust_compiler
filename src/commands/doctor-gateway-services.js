...
export async function maybeRepairGatewayServiceConfig(cfg, mode, runtime, prompter) {
  ...
  const audit = await auditGatewayServiceConfig({ env: process.env, command });
  ...
  if (((expectedEntrypoint && currentEntrypoint) && (normalizeExecutablePath(expectedEntrypoint) !== normalizeExecutablePath(currentEntrypoint)))) {
    audit.issues.push({ code: SERVICE_AUDIT_CODES.gatewayEntrypointMismatch, message: "Gateway service entrypoint does not match the current install.", detail: " -> ", level: "recommended" });
  }
  if ((audit.issues.length === 0)) {
    return;
  }
  note(audit.issues.map((issue) => issue.detail ? "-  ()" : "- ").join("
"), "Gateway service config");
  ...
  const repair = needsAggressive ? await prompter.confirmAggressive({ message: "Overwrite gateway service config with current defaults now?", initialValue: Boolean(prompter.shouldForce) }) : await prompter.confirmRepair({ message: "Update gateway service config to the recommended defaults now?", initialValue: true });
  if (!repair) {
    return;
  }
  try {
    ...
    {
      runtime.error("Gateway service update failed: ");
    }
  }
}
...
export async function maybeScanExtraGatewayServices(options, runtime, prompter) {
  ...
  note(extraServices.map((svc) => "-  (, )").join("
"), "Other gateway-like services detected");
  const legacyServices = extraServices.filter((svc) => (svc.legacy === true));
  if ((legacyServices.length > 0)) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({ message: "Remove legacy gateway services (clawdbot/moltbot) now?", initialValue: true });
    if (shouldRemove) {
      const removed = [];
      const failed = [];
      for (const svc of legacyServices) {
        ...
        if (!plistPath) {
          failed.push(" (missing plist path)");
          continue;
        }
        const dest = await cleanupLegacyLaunchdService({ label: svc.label, plistPath });
        removed.push(dest ? " -> " : svc.label);
      }
      if ((removed.length > 0)) {
        note(removed.map((line) => "- ").join("
"), "Legacy gateway removed");
      }
      if ((failed.length > 0)) {
        note(failed.map((line) => "- ").join("
"), "Legacy gateway cleanup skipped");
  ...
  if ((cleanupHints.length > 0)) {
    note(cleanupHints.map((hint) => "- ").join("
"), "Cleanup hints");
  }
  note(["Recommendation: run a single gateway per machine for most setups.", "One gateway supports multiple agents.", "If you need multiple gateways (e.g., a rescue bot on the same host), isolate ports + config/state (see docs: /gateway#multiple-gateways-same-host)."].join("
