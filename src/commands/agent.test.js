...
function mockConfig(home, storePath, agentOverrides, telegramOverrides, agentsList) {
  configSpy.mockReturnValue({ agents: { defaults: { model: { primary: "anthropic/claude-opus-4-5" }, models: { "anthropic/claude-opus-4-5": {  } }, workspace: path.join(home, "openclaw"), ...agentOverrides:  }, list: agentsList }, session: { store: storePath, mainKey: "main" }, telegram: telegramOverrides ? { ...telegramOverrides:  } : undefined });
}
beforeEach(() => {
  vi.clearAllMocks();
...
describe("agentCommand", () => {
  ...
  it("resumes when session-id is provided", async () => {
    await withTempHome(async (home) => {
      ...
      fs.writeFileSync(store, JSON.stringify({ foo: { sessionId: "session-123", updatedAt: Date.now(), systemSent: true } }, null, 2));
      mockConfig(home, store);
  ...
  it("does not duplicate agent events from embedded runs", async () => {
    await withTempHome(async (home) => {
      ...
      const stop = onAgentEvent((evt) => {
        ...
        assistantEvents.push({ runId: evt.runId, text: (typeof evt.data?.text === "string") ? evt.data.text : undefined });
      });
      vi.mocked(runEmbeddedPiAgent).mockImplementationOnce(async (params) => {
        ...
        return { payloads: [{ text: "hello" }], meta: { agentMeta: { provider: "p", model: "m" } } };
      });
  ...
  it("uses provider/model from agents.defaults.model.primary", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store, { model: { primary: "openai/gpt-4.1-mini" }, models: { "anthropic/claude-opus-4-5": {  }, "openai/gpt-4.1-mini": {  } } });
      await agentCommand({ message: "hi", to: "+1555" }, runtime);
      const callArgs = vi.mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0];
  ...
  it("keeps explicit sessionKey even when sessionId exists elsewhere", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      fs.mkdirSync(path.dirname(store), { recursive: true });
      fs.writeFileSync(store, JSON.stringify({ "agent:main:main": { sessionId: "sess-main", updatedAt: Date.now() } }, null, 2));
      mockConfig(home, store);
      await agentCommand({ message: "hi", sessionId: "sess-main", sessionKey: "agent:main:subagent:abc" }, runtime);
      const callArgs = vi.mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0];
  ...
  it("derives session key from --agent when no routing target is provided", async () => {
    await withTempHome(async (home) => {
      ...
      expect(callArgs?.sessionFile).toContain("agentsopssessions");
    });
  });
  it("rejects unknown agent overrides", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);
      await expect(agentCommand({ message: "hi", agentId: "ghost" }, runtime)).rejects.toThrow("Unknown agent id \"ghost\"");
    });
  });
  ...
  it("prints JSON payload when requested", async () => {
    await withTempHome(async (home) => {
      vi.mocked(runEmbeddedPiAgent).mockResolvedValue({ payloads: [{ text: "json-reply", mediaUrl: "http://x.test/a.jpg" }], meta: { durationMs: 42, agentMeta: { sessionId: "s", provider: "p", model: "m" } } });
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);
  ...
  it("passes through telegram accountId when delivering", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        {
          await agentCommand({ message: "hi", to: "123", deliver: true, channel: "telegram" }, runtime, deps);
          expect(deps.sendMessageTelegram).toHaveBeenCalledWith("123", "ok", expect.objectContaining({ accountId: undefined, verbose: false }));
        }
      }
      finally {
        {
  ...
  it("prefers runContext for embedded routing", async () => {
    await withTempHome(async (home) => {
      const store = path.join(home, "sessions.json");
      mockConfig(home, store);
      await agentCommand({ message: "hi", to: "+1555", channel: "whatsapp", runContext: { messageChannel: "slack", accountId: "acct-2" } }, runtime);
      const callArgs = vi.mocked(runEmbeddedPiAgent).mock.calls.at(-1)?.[0];
      expect(callArgs?.messageChannel).toBe("slack");
