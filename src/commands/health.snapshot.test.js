...
vi.mock("../config/sessions.js", () => { resolveStorePath: () => "/tmp/sessions.json", loadSessionStore: () => testStore, readSessionUpdatedAt: vi.fn(() => undefined), recordSessionMetaFromInbound: vi.fn().mockResolvedValue(undefined), updateLastRoute: vi.fn().mockResolvedValue(undefined) });
vi.mock("../web/auth-store.js", () => { webAuthExists: vi.fn(async () => true), getWebAuthAgeMs: vi.fn(() => 1234), readWebSelfId: vi.fn(() => { e164: null, jid: null }), logWebSelfId: vi.fn(), logoutWeb: vi.fn() });
describe("getHealthSnapshot", () => {
  ...
  it("skips telegram probe when not configured", async () => {
    ...
    testStore = { global: { updatedAt: Date.now() }, unknown: { updatedAt: Date.now() }, main: { updatedAt: 1000 }, foo: { updatedAt: 2000 } };
  ...
  it("probes telegram getMe + webhook info when configured", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (url) => {
      ...
      if (url.includes("/getMe")) {
        return { ok: true, status: 200, json: async () => { ok: true, result: { id: 1, username: "bot" } } };
      }
      if (url.includes("/getWebhookInfo")) {
        return { ok: true, status: 200, json: async () => { ok: true, result: { url: "https://example.com/h", has_custom_certificate: false } } };
      }
...
", "utf-8");
  ...
  it("disables heartbeat for agents without heartbeat blocks", async () => {
    testConfig = { agents: { defaults: { heartbeat: { every: "30m", target: "last" } }, list: [{ id: "main", default: true }, { id: "ops", heartbeat: { every: "1h", target: "whatsapp" } }] } };
    testStore = {  };
