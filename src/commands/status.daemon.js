...
async function buildDaemonStatusSummary(service, fallbackLabel) {
  try {
    {
      const [loaded, runtime, command] = await Promise.all([service.isLoaded({ env: process.env }).catch(() => false), service.readRuntime(process.env).catch(() => undefined), service.readCommand(process.env).catch(() => null)]);
    ...
    {
      return { label: fallbackLabel, installed: null, loadedText: "unknown", runtimeShort: null };
    }
  }
}
