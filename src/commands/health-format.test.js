...
describe("formatHealthCheckFailure", () => {
  it("keeps non-rich output stable", () => {
    ...
    expect(formatHealthCheckFailure(err, { rich: false })).toBe("Health check failed: ");
  });
  it("formats gateway connection details as indented key/value lines", () => {
    const err = new Error(["gateway closed (1006 abnormal closure (no close frame)): no close reason", "Gateway target: ws://127.0.0.1:19001", "Source: local loopback", "Config: /Users/steipete/.openclaw-dev/openclaw.json", "Bind: loopback"].join("
"));
