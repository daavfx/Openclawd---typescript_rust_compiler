...
async function withTimeout(promise, timeoutMs, label) {
  const resolved = ((typeof timeoutMs === "number") && Number.isFinite(timeoutMs)) ? Math.max(1, Math.floor(timeoutMs)) : undefined;
  if (!resolved) {
...
async function readBrowserProxyFile(filePath) {
  ...
  if ((stat.size > BROWSER_PROXY_MAX_FILE_BYTES)) {
    throw new Error("browser proxy file exceeds MB");
  }
...
function redactExecApprovals(file) {
  ...
  return { ...file: , socket: socketPath ? { path: socketPath } : undefined };
}
...
async function runCommand(argv, cwd, env, timeoutMs) {
  return await new Promise((resolve) => {
    ...
    const finalize = (exitCode, error) => {
      ...
      resolve({ exitCode, timedOut, success: (((exitCode === 0) && !timedOut) && !error), stdout, stderr, error: (error ?? null), truncated });
    };
    child.on("error", (err) => {
...
async function runViaMacAppExecHost(params) {
  ...
  return await requestExecHostViaSocket({ socketPath: approvals.socketPath, token: approvals.token, request });
}
export async function runNodeHost(opts) {
  ...
  const gateway = { host: opts.gatewayHost, port: opts.gatewayPort, tls: ((opts.gatewayTls ?? loadConfig().gateway?.tls?.enabled) ?? false), tlsFingerprint: opts.gatewayTlsFingerprint };
  ...
  const client = new GatewayClient({ url, token: (token?.trim() || undefined), password: (password?.trim() || undefined), instanceId: nodeId, clientName: GATEWAY_CLIENT_NAMES.NODE_HOST, clientDisplayName: displayName, clientVersion: VERSION, platform: process.platform, mode: GATEWAY_CLIENT_MODES.NODE, role: "node", scopes: [], caps: ["system", ...browserProxyEnabled ? ["browser"] : []], commands: ["system.run", "system.which", "system.execApprovals.get", "system.execApprovals.set", ...browserProxyEnabled ? ["browser.proxy"] : []], pathEnv, permissions: undefined, deviceIdentity: loadOrCreateDeviceIdentity(), tlsFingerprint: gateway.tlsFingerprint, onEvent: (evt) => {
    ...
    console.error("node host gateway connect failed: ");
  }, onClose: (code, reason) => {
    console.error("node host gateway closed (): ");
  } });
...
async function handleInvoke(frame, client, skillBins) {
    ...
    try {
      {
        ...
        const payload = { path: snapshot.path, exists: snapshot.exists, hash: snapshot.hash, file: redactExecApprovals(snapshot.file) };
    ...
    catch (err) {
      {
        ...
        await sendInvokeResult(client, frame, { ok: false, error: { code, message } });
      }
    }
    return;
  }
    ...
    try {
      {
        ...
        const next = { ...normalized: , socket: { path: socketPath, token } };
        saveExecApprovals(next);
        const nextSnapshot = readExecApprovalsSnapshot();
        const payload = { path: nextSnapshot.path, exists: nextSnapshot.exists, hash: nextSnapshot.hash, file: redactExecApprovals(nextSnapshot.file) };
        await sendInvokeResult(client, frame, { ok: true, payloadJSON: JSON.stringify(payload) });
      }
    }
    catch (err) {
      {
        await sendInvokeResult(client, frame, { ok: false, error: { code: "INVALID_REQUEST", message: String(err) } });
      }
    }
    return;
  }
    ...
    try {
      {
        ...
        const path = pathValue.startsWith("/") ? pathValue : "/";
        ...
        const response = await withTimeout(dispatcher.dispatch({ method: (method === "DELETE") ? "DELETE" : (method === "POST") ? "POST" : "GET", path, query, body }), params.timeoutMs, "browser proxy request");
        if ((response.status >= 400)) {
          const message = ((response.body && (typeof response.body === "object")) && ("error" in response.body)) ? String(response.body.error) : "HTTP ";
        ...
        if ((paths.length > 0)) {
          const loaded = await Promise.all(paths.map(async (p) => {
            try {
              ...
              {
                throw new Error("browser proxy file read failed for : ");
              }
            }
          }));
          if ((loaded.length > 0)) {
  ...
  if ((command !== "system.run")) {
    await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: "command not supported" } });
    return;
  }
  let params;
  try {
  ...
  if ((!Array.isArray(params.command) || (params.command.length === 0))) {
    await sendInvokeResult(client, frame, { ok: false, error: { code: "INVALID_REQUEST", message: "command required" } });
    return;
  }
  ...
  const approvals = resolveExecApprovals(agentId, { security: configuredSecurity, ask: configuredAsk });
  ...
  let segments = [];
  if (rawCommand) {
    const allowlistEval = evaluateShellAllowlist({ command: rawCommand, allowlist: approvals.allowlist, safeBins, cwd: (params.cwd ?? undefined), env, skillBins: bins, autoAllowSkills });
    ...
    const allowlistEval = evaluateExecAllowlist({ analysis, allowlist: approvals.allowlist, safeBins, cwd: (params.cwd ?? undefined), skillBins: bins, autoAllowSkills });
  ...
  if (useMacAppExec) {
    ...
    const execRequest = { command: argv, rawCommand: (rawCommand || null), cwd: (params.cwd ?? null), env: (params.env ?? null), timeoutMs: (params.timeoutMs ?? null), needsScreenRecording: (params.needsScreenRecording ?? null), agentId: (agentId ?? null), sessionKey: (sessionKey ?? null), approvalDecision };
    const response = await runViaMacAppExecHost({ approvals, request: execRequest });
    if (!response) {
      if ((execHostEnforced || !execHostFallbackAllowed)) {
        await sendNodeEvent(client, "exec.denied", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, reason: "companion-unavailable" }));
        await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: "COMPANION_APP_UNAVAILABLE: macOS app exec host unreachable" } });
        return;
      }
    } else {
      if (!response.ok) {
        const reason = (response.error.reason ?? "approval-required");
        await sendNodeEvent(client, "exec.denied", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, reason }));
        await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: response.error.message } });
        return;
      } else {
        const result = response.payload;
        const combined = [result.stdout, result.stderr, result.error].filter(Boolean).join("
");
        await sendNodeEvent(client, "exec.finished", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, exitCode: result.exitCode, timedOut: result.timedOut, success: result.success, output: combined }));
  ...
  if ((security === "deny")) {
    await sendNodeEvent(client, "exec.denied", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, reason: "security=deny" }));
    await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: "SYSTEM_RUN_DISABLED: security=deny" } });
    return;
  }
  const requiresAsk = requiresExecApproval({ ask, security, analysisOk, allowlistSatisfied });
  const approvalDecision = ((params.approvalDecision === "allow-once") || (params.approvalDecision === "allow-always")) ? params.approvalDecision : null;
  const approvedByAsk = ((approvalDecision !== null) || (params.approved === true));
  if ((requiresAsk && !approvedByAsk)) {
    await sendNodeEvent(client, "exec.denied", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, reason: "approval-required" }));
    await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: "SYSTEM_RUN_DENIED: approval required" } });
    return;
  }
  ...
  if ((((security === "allowlist") && (!analysisOk || !allowlistSatisfied)) && !approvedByAsk)) {
    await sendNodeEvent(client, "exec.denied", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, reason: "allowlist-miss" }));
    await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: "SYSTEM_RUN_DENIED: allowlist miss" } });
    return;
  }
  ...
  if ((params.needsScreenRecording === true)) {
    await sendNodeEvent(client, "exec.denied", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, reason: "permission:screenRecording" }));
    await sendInvokeResult(client, frame, { ok: false, error: { code: "UNAVAILABLE", message: "PERMISSION_MISSING: screenRecording" } });
    return;
  }
...
");
  await sendNodeEvent(client, "exec.finished", buildExecEventPayload({ sessionKey, runId, host: "node", command: cmdText, exitCode: result.exitCode, timedOut: result.timedOut, success: result.success, output: combined }));
  await sendInvokeResult(client, frame, { ok: true, payloadJSON: JSON.stringify({ exitCode: result.exitCode, timedOut: result.timedOut, success: result.success, stdout: result.stdout, stderr: result.stderr, error: (result.error ?? null) }) });
}
...
function coerceNodeInvokePayload(payload) {
  ...
  const paramsJSON = (typeof obj.paramsJSON === "string") ? obj.paramsJSON : (obj.params !== undefined) ? JSON.stringify(obj.params) : null;
...
export function buildNodeInvokeResultParams(frame, result) {
  const params = { id: frame.id, nodeId: frame.nodeId, ok: result.ok };
...
async function sendNodeEvent(client, event, payload) {
  try {
    {
      await client.request("node.event", { event, payloadJSON: payload ? JSON.stringify(payload) : null });
    }
  }
  catch {
    {
