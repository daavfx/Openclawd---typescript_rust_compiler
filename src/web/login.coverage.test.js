...
vi.mock("../config/config.js", () => { loadConfig: () => { channels: { whatsapp: { accounts: { default: { enabled: true, authDir } } } } } });
vi.mock("./session.js", () => {
  ...
  const formatError = vi.fn((err) => "formatted:");
  return { createWaSocket, waitForWaConnection, formatError, WA_WEB_AUTH_DIR: authDir, logoutWeb: vi.fn(async (params) => {
    await fs.rm((params.authDir ?? authDir), { recursive: true, force: true });
    return true;
  }) };
});
...
describe("loginWeb coverage", () => {
  ...
  it("restarts once when WhatsApp requests code 515", async () => {
    waitForWaConnection.mockRejectedValueOnce({ output: { statusCode: 515 } }).mockResolvedValueOnce(undefined);
  ...
  it("clears creds and throws when logged out", async () => {
    waitForWaConnection.mockRejectedValueOnce({ output: { statusCode: DisconnectReason.loggedOut } });
    await expect(loginWeb(false, waitForWaConnection)).rejects.toThrow(/cache cleared/i);
    expect(rmMock).toHaveBeenCalledWith(authDir, { recursive: true, force: true });
  });
  it("formats and rethrows generic errors", async () => {
    waitForWaConnection.mockRejectedValueOnce(new Error("boom"));
    await expect(loginWeb(false, waitForWaConnection)).rejects.toThrow("formatted:Error: boom");
