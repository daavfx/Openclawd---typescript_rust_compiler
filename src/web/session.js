...
export async function createWaSocket(printQr, verbose, opts = {  }) {
  ...
  const sock = makeWASocket({ auth: { creds: state.creds, keys: makeCacheableSignalKeyStore(state.keys, logger) }, version, logger, printQRInTerminal: false, browser: ["openclaw", "cli", VERSION], syncFullHistory: false, markOnlineOnConnect: false });
    ...
    try {
      {
        ...
        if ((connection === "close")) {
          ...
          if ((status === DisconnectReason.loggedOut)) {
            console.error(danger("WhatsApp session logged out. Run: "));
          }
        }
...
export function formatError(err) {
  ...
  const messageCandidates = [boom?.message, (typeof err?.message === "string") ? err.message : undefined, (typeof err?.error?.message === "string") ? err.error?.message : undefined].filter((v) => Boolean((v && (v.trim().length > 0))));
