...
export async function sendMessageWhatsApp(to, body, options) {
  ...
  const tableMode = resolveMarkdownTableMode({ cfg, channel: "whatsapp", accountId: (resolvedAccountId ?? options.accountId) });
  ...
  try {
    {
      ...
      const sendOptions = (options.gifPlayback || accountId) ? { ...options.gifPlayback ? { gifPlayback: true } : {  }: , accountId } : undefined;
      ...
      outboundLog.info("Sent message  ->  (ms)");
  ...
  try {
    {
      ...
      outboundLog.info("Sending reaction \"\" -> message ");
      ...
      outboundLog.info("Sent reaction \"\" -> message ");
  ...
  try {
    {
      ...
      outboundLog.info("Sending poll -> : \"\"");
      logger.info({ jid, question: normalized.question, optionCount: normalized.options.length, maxSelections: normalized.maxSelections }, "sending poll");
      const result = await active.sendPoll(to, normalized);
      const messageId = (result?.messageId ?? "unknown");
      const durationMs = (Date.now() - startedAt);
      outboundLog.info("Sent poll  ->  (ms)");
