...
vi.mock("./session.js", () => {
  ...
  const getStatusCode = vi.fn((err) => (err?.output?.statusCode ?? err?.status));
  const webAuthExists = vi.fn(async () => false);
  const readWebSelfId = vi.fn(() => { e164: null, jid: null });
  const logoutWeb = vi.fn(async () => true);
  return { createWaSocket, waitForWaConnection, formatError, getStatusCode, webAuthExists, readWebSelfId, logoutWeb };
});
vi.mock("./qr-image.js", () => { renderQrPngBase64: vi.fn(async () => "base64") });
