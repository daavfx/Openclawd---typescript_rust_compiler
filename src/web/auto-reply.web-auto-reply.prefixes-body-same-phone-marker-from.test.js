...
describe("web auto-reply", () => {
  ...
  it("prefixes body with same-phone marker when from === to", async () => {
    setLoadConfigMock(() => { channels: { whatsapp: { allowFrom: ["*"] } }, messages: { messagePrefix: "[same-phone]", responsePrefix: undefined } });
    ...
    await capturedOnMessage?.({ body: "hello", from: "+1555", to: "+1555", id: "msg1", sendComposing: vi.fn(), reply: vi.fn(), sendMedia: vi.fn() });
  ...
  it("does not prefix body when from !== to", async () => {
    ...
    await monitorWebChannel(false, listenerFactory, false, resolver);
    expect(capturedOnMessage).toBeDefined();
    await capturedOnMessage?.({ body: "hello", from: "+1555", to: "+2666", id: "msg1", sendComposing: vi.fn(), reply: vi.fn(), sendMedia: vi.fn() });
    const callArg = resolver.mock.calls[0]?.[0];
    expect(callArg?.Body).toContain("[WhatsApp +1555");
  ...
  it("forwards reply-to context to resolver", async () => {
    ...
    await monitorWebChannel(false, listenerFactory, false, resolver);
    expect(capturedOnMessage).toBeDefined();
    await capturedOnMessage?.({ body: "hello", from: "+1555", to: "+2666", id: "msg1", replyToId: "q1", replyToBody: "original", replyToSender: "+1999", sendComposing: vi.fn(), reply: vi.fn(), sendMedia: vi.fn() });
    const callArg = resolver.mock.calls[0]?.[0];
  ...
  it("applies responsePrefix to regular replies", async () => {
    setLoadConfigMock(() => { channels: { whatsapp: { allowFrom: ["*"] } }, messages: { messagePrefix: undefined, responsePrefix: "🦞" } });
    let capturedOnMessage;
    const reply = vi.fn();
    const listenerFactory = async (opts) => {
      capturedOnMessage = opts.onMessage;
    ...
    await monitorWebChannel(false, listenerFactory, false, resolver);
    expect(capturedOnMessage).toBeDefined();
    await capturedOnMessage?.({ body: "hi", from: "+1555", to: "+2666", id: "msg1", sendComposing: vi.fn(), reply, sendMedia: vi.fn() });
    expect(reply).toHaveBeenCalledWith("🦞 hello there");
    resetLoadConfigMock();
  });
  it("defaults responsePrefix for self-chat replies when unset", async () => {
    setLoadConfigMock(() => { agents: { list: [{ id: "main", default: true, identity: { name: "Mainbot", emoji: "🦞", theme: "space lobster" } }] }, channels: { whatsapp: { allowFrom: ["+1555"] } }, messages: { messagePrefix: undefined, responsePrefix: undefined } });
    let capturedOnMessage;
    const reply = vi.fn();
    const listenerFactory = async (opts) => {
      capturedOnMessage = opts.onMessage;
    ...
    await monitorWebChannel(false, listenerFactory, false, resolver);
    expect(capturedOnMessage).toBeDefined();
    await capturedOnMessage?.({ body: "hi", from: "+1555", to: "+1555", selfE164: "+1555", chatType: "direct", id: "msg1", sendComposing: vi.fn(), reply, sendMedia: vi.fn() });
  ...
  it("does not deliver HEARTBEAT_OK responses", async () => {
    ...
    await monitorWebChannel(false, listenerFactory, false, resolver);
    expect(capturedOnMessage).toBeDefined();
    await capturedOnMessage?.({ body: "test", from: "+1555", to: "+2666", id: "msg1", sendComposing: vi.fn(), reply, sendMedia: vi.fn() });
