...
async function writeTempFile(buffer, ext) {
  const file = path.join(os.tmpdir(), "openclaw-media--");
  tmpFiles.push(file);
...
describe("web media loading", () => {
  it("compresses large local images under the provided cap", async () => {
    const buffer = await sharp({ create: { width: 1600, height: 1600, channels: 3, background: "#ff0000" } }).jpeg({ quality: 95 }).toBuffer();
  ...
  it("sniffs mime before extension when loading local files", async () => {
    const pngBuffer = await sharp({ create: { width: 2, height: 2, channels: 3, background: "#00ff00" } }).png().toBuffer();
  ...
  it("adds extension to URL fileName when missing", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({ ok: true, body: true, arrayBuffer: async () => Buffer.from("%PDF-1.4").buffer, headers: { get: () => "application/pdf" }, status: 200 });
  ...
  it("includes URL + status in fetch errors", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({ ok: false, body: true, text: async () => "Not Found", headers: { get: () => null }, status: 404, statusText: "Not Found", url: "https://example.com/missing.jpg" });
    await expect(loadWebMedia("https://example.com/missing.jpg", (1024 * 1024))).rejects.toThrow(/Failed to fetch media from https:\/\/example\.com\/missing\.jpg.*HTTP 404/i);
    fetchMock.mockRestore();
  });
  it("uses content-disposition filename when available", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({ ok: true, body: true, arrayBuffer: async () => Buffer.from("%PDF-1.4").buffer, headers: { get: (name) => {
  ...
  it("preserves GIF animation by skipping JPEG optimization", async () => {
    const gifBuffer = Buffer.from([71, 73, 70, 56, 57, 97, 1, 0, 1, 0, 0, 0, 0, 44, 0, 0, 0, 0, 1, 0, 1, 0, 0, 2, 1, 68, 0, 59]);
  ...
  it("preserves GIF from URL without JPEG conversion", async () => {
    const gifBytes = new Uint8Array([71, 73, 70, 56, 57, 97, 1, 0, 1, 0, 0, 0, 0, 44, 0, 0, 0, 0, 1, 0, 1, 0, 0, 2, 1, 68, 0, 59]);
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValueOnce({ ok: true, body: true, arrayBuffer: async () => gifBytes.buffer.slice(gifBytes.byteOffset, (gifBytes.byteOffset + gifBytes.byteLength)), headers: { get: () => "image/gif" }, status: 200 });
  ...
  it("preserves PNG alpha when under the cap", async () => {
    const buffer = await sharp({ create: { width: 64, height: 64, channels: 4, background: { r: 255, g: 0, b: 0, alpha: 0.5 } } }).png().toBuffer();
  ...
  it("falls back to JPEG when PNG alpha cannot fit under cap", async () => {
    ...
    if ((jpegOptimized.buffer.length >= smallestPng.optimizedSize)) {
      throw new Error("JPEG fallback did not shrink below PNG (jpeg=, png=)");
    }
