...
describe("web auto-reply", () => {
  ...
  it("uses identity.name for messagePrefix when set", async () => {
    setLoadConfigMock(() => { agents: { list: [{ id: "main", default: true, identity: { name: "Mainbot", emoji: "🦞", theme: "space lobster" } }, { id: "rich", identity: { name: "Richbot", emoji: "🦁", theme: "lion bot" } }] }, bindings: [{ agentId: "rich", match: { channel: "whatsapp", peer: { kind: "dm", id: "+1555" } } }] });
    let capturedOnMessage;
    const reply = vi.fn();
    const listenerFactory = async (opts) => {
      capturedOnMessage = opts.onMessage;
