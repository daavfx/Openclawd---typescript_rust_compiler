...
export async function startWebLoginWithQr(opts = {  }) {
  ...
  if ((hasWeb && !opts.force)) {
    ...
    return { message: "WhatsApp is already linked (). Say “relink” if you want a fresh QR." };
  }
  ...
  try {
    {
      sock = await createWaSocket(false, Boolean(opts.verbose), { authDir: account.authDir, onQr: (qr) => {
        if (pendingQr) {
          return;
        }
        pendingQr = qr;
  ...
  catch (err) {
    {
      clearTimeout(qrTimer);
      await resetActiveLogin(account.accountId);
      return { message: "Failed to start WhatsApp login: " };
    }
  }
  const login = { accountId: account.accountId, authDir: account.authDir, isLegacyAuthDir: account.isLegacyAuthDir, id: randomUUID(), sock, startedAt: Date.now(), connected: false, waitPromise: Promise.resolve(), restartAttempted: false, verbose: Boolean(opts.verbose) };
  ...
  try {
  ...
  catch (err) {
    {
      clearTimeout(qrTimer);
      await resetActiveLogin(account.accountId);
      return { message: "Failed to get QR: " };
    }
  }
  const base64 = await renderQrPngBase64(qr);
  login.qrDataUrl = "data:image/png;base64,";
  ...
  while (true) {
    ...
    if (login.error) {
      if ((login.errorStatus === DisconnectReason.loggedOut)) {
        await logoutWeb({ authDir: login.authDir, isLegacyAuthDir: login.isLegacyAuthDir, runtime });
      ...
      const message = "WhatsApp login failed: ";
      await resetActiveLogin(account.accountId, message);
      runtime.log(danger(message));
      return { connected: false, message };
    }
