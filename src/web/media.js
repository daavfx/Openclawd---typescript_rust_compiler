...
function formatCapLimit(label, cap, size) {
  return " exceeds MB limit (got MB)";
}
function formatCapReduce(label, cap, size) {
  return " could not be reduced below MB (got MB)";
}
...
function logOptimizedImage(params) {
  ...
  if ((params.optimized.format === "png")) {
    logVerbose("Optimized PNG (preserving alpha) from MB to MB (side≤px)");
    return;
  }
  logVerbose("Optimized media from MB to MB (side≤px, q=)");
}
async function optimizeImageWithFallback(params) {
  ...
  if (hasAlpha) {
    ...
    if (shouldLogVerbose()) {
      logVerbose("PNG with alpha still exceeds MB after optimization; falling back to JPEG");
    }
  }
...
async function loadWebMediaInternal(mediaUrl, options = {  }) {
  ...
  const optimizeAndClampImage = async (buffer, cap, meta) => {
    ...
    return { buffer: optimized.buffer, contentType, kind: "image", fileName };
  };
  const clampAndFinalize = async (params) => {
    ...
    if ((params.kind === "image")) {
      ...
      if ((isGif || !optimizeImages)) {
        ...
        return { buffer: params.buffer, contentType: params.contentType, kind: params.kind, fileName: params.fileName };
      }
    ...
    return { buffer: params.buffer, contentType: (params.contentType ?? undefined), kind: params.kind, fileName: params.fileName };
  };
  ...
  return await clampAndFinalize({ buffer: data, contentType: mime, kind, fileName });
}
...
export async function loadWebMediaRaw(mediaUrl, maxBytes) {
  return await loadWebMediaInternal(mediaUrl, { maxBytes, optimizeImages: false });
}
...
export async function optimizeImageToJpeg(buffer, maxBytes, opts = {  }) {
  ...
  for (const side of sides) {
    for (const quality of qualities) {
      try {
        {
          const out = await resizeToJpeg({ buffer: source, maxSide: side, quality, withoutEnlargement: true });
          const size = out.length;
          if ((!smallest || (size < smallest.size))) {
            smallest = { buffer: out, size, resizeSide: side, quality };
          }
          if ((size <= maxBytes)) {
            return { buffer: out, optimizedSize: size, resizeSide: side, quality };
          }
        }
      }
      catch {
  ...
  if (smallest) {
    return { buffer: smallest.buffer, optimizedSize: smallest.size, resizeSide: smallest.resizeSide, quality: smallest.quality };
  }
