...
export async function monitorWebInbox(options) {
  ...
  try {
    ...
    {
      logVerbose("Failed to send 'available' presence on connect: ");
    }
  }
  const selfJid = sock.user?.id;
  const selfE164 = selfJid ? jidToE164(selfJid) : null;
  const debouncer = createInboundDebouncer({ debounceMs: (options.debounceMs ?? 0), buildKey: (msg) => {
    ...
    return "::";
  }, shouldDebounce: options.shouldDebounce, onFlush: async (entries) => {
...
");
    const combinedMessage = { ...last: , body: combinedBody, mentionedJids: (mentioned.size > 0) ? Array.from(mentioned) : undefined };
    await options.onMessage(combinedMessage);
  }, onError: (err) => {
    inboundLogger.error({ error: String(err) }, "failed handling inbound web message");
    inboundConsoleLog.error("Failed handling inbound web message: ");
  } });
    ...
    try {
      {
        ...
        const entry = { subject: meta.subject, participants, expires: (Date.now() + GROUP_META_TTL_MS) };
      ...
      {
        logVerbose("Failed to fetch group metadata for : ");
  ...
  const handleMessagesUpsert = async (upsert) => {
    ...
    for (const msg of (upsert.messages ?? [])) {
      recordChannelActivity({ channel: "whatsapp", accountId: options.accountId, direction: "inbound" });
      ...
      const access = await checkInboundAccessControl({ accountId: options.accountId, from, selfE164, senderE164, group, pushName: (msg.pushName ?? undefined), isFromMe: Boolean(msg.key?.fromMe), messageTimestampMs, connectedAtMs, sock: { sendMessage: (jid, content) => sock.sendMessage(jid, content) }, remoteJid });
      if (!access.allowed) {
        continue;
      }
        ...
        try {
          {
            ...
            if (shouldLogVerbose()) {
              const suffix = participant ? " (participant )" : "";
              logVerbose("Marked message  as read for ");
            }
          }
        }
        catch (err) {
          {
            logVerbose("Failed to mark message  read: ");
          }
        }
      } else {
        if (((id && access.isSelfChat) && shouldLogVerbose())) {
          logVerbose("Self-chat mode: skipping read receipt for ");
        }
      }
...
").trim();
      ...
      try {
        ...
        {
          logVerbose("Inbound media download failed: ");
        }
      }
      ...
      const sendComposing = async () => {
        try {
          ...
          {
            logVerbose("Presence update failed: ");
          }
        }
      };
    ...
    try {
      {
        if ((update.connection === "close")) {
          ...
          resolveClose({ status, isLoggedOut: (status === DisconnectReason.loggedOut), error: update.lastDisconnect?.error });
        }
      }
    }
    catch (err) {
  ...
  const sendApi = createWebSendApi({ sock: { sendMessage: (jid, content) => sock.sendMessage(jid, content), sendPresenceUpdate: (presence, jid) => sock.sendPresenceUpdate(presence, jid) }, defaultAccountId: options.accountId });
  return { close: async () => {
    try {
      {
        const ev = sock.ev;
