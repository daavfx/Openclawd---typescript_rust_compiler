...
function extractContactPlaceholder(rawMessage) {
  ...
  if (contact) {
    const {name, phones} = describeContact({ displayName: contact.displayName, vcard: contact.vcard });
  ...
  if (live) {
    ...
    if (((latitudeRaw != null) && (longitudeRaw != null))) {
      ...
      if ((Number.isFinite(latitude) && Number.isFinite(longitude))) {
        return { latitude, longitude, accuracy: (live.accuracyInMeters ?? undefined), caption: (live.caption ?? undefined), source: "live", isLive: true };
      }
    }
  }
  ...
  if (location) {
    ...
    if (((latitudeRaw != null) && (longitudeRaw != null))) {
      ...
      if ((Number.isFinite(latitude) && Number.isFinite(longitude))) {
        ...
        return { latitude, longitude, accuracy: (location.accuracyInMeters ?? undefined), name: (location.name ?? undefined), address: (location.address ?? undefined), caption: (location.comment ?? undefined), source: isLive ? "live" : (location.name || location.address) ? "place" : "pin", isLive };
      }
    }
  }
  return null;
...
").trim();
  ...
  if (!body) {
    ...
    logVerbose("Quoted message missing extractable body");
    return null;
  }
  ...
  return { id: contextInfo?.stanzaId ? String(contextInfo.stanzaId) : undefined, body, sender, senderJid, senderE164 };
}
