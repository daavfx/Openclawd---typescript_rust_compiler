...
export async function checkInboundAccessControl(params) {
  ...
  const groupAllowFrom = (account.groupAllowFrom ?? (configuredAllowFrom && (configuredAllowFrom.length > 0)) ? configuredAllowFrom : undefined);
  ...
  if ((params.group && (groupPolicy === "disabled"))) {
    ...
    return { allowed: false, shouldMarkRead: false, isSelfChat, resolvedAccountId: account.accountId };
  }
  if ((params.group && (groupPolicy === "allowlist"))) {
    ...
    const senderAllowed = (groupHasWildcard || ((params.senderE164 != null) && normalizedGroupAllowFrom.includes(params.senderE164)));
    if (!senderAllowed) {
      logVerbose("Blocked group message from  (groupPolicy: allowlist)");
      return { allowed: false, shouldMarkRead: false, isSelfChat, resolvedAccountId: account.accountId };
    }
  }
  if (!params.group) {
      ...
      if (!allowed) {
        if ((dmPolicy === "pairing")) {
          if (suppressPairingReply) {
            logVerbose("Skipping pairing reply for historical DM from .");
          } else {
            const {code, created} = await upsertChannelPairingRequest({ channel: "whatsapp", id: candidate, meta: { name: ((params.pushName ?? "").trim() || undefined) } });
            if (created) {
              logVerbose("whatsapp pairing request sender= name=");
              try {
                {
                  await params.sock.sendMessage(params.remoteJid, { text: buildPairingReply({ channel: "whatsapp", idLine: "Your WhatsApp phone number: ", code }) });
                }
              }
              catch (err) {
                {
                  logVerbose("whatsapp pairing reply failed for : ");
                }
              }
            }
          }
        } else {
          logVerbose("Blocked unauthorized sender  (dmPolicy=)");
        }
        return { allowed: false, shouldMarkRead: false, isSelfChat, resolvedAccountId: account.accountId };
      }
    }
