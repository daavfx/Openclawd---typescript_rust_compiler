  ...
  try {
  ...
  catch (err) {
    {
      ...
      if ((code === DisconnectReason.loggedOut)) {
        await logoutWeb({ authDir: account.authDir, isLegacyAuthDir: account.isLegacyAuthDir, runtime });
        console.error(danger("WhatsApp reported the session is logged out. Cleared cached web session; please rerun  and scan the QR again."));
        throw new Error("Session logged out; cache cleared. Re-run login.");
      }
      const formatted = formatError(err);
      console.error(danger("WhatsApp Web connection ended before fully opening. "));
