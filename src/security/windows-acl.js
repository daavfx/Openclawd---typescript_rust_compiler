...
const WORLD_PRINCIPALS = new Set(["everyone", "users", "builtin\\users", "authenticated users", "nt authority\\authenticated users"]);
const TRUSTED_BASE = new Set(["nt authority\\system", "system", "builtin\\administrators", "creator owner"]);
...
export function parseIcaclsOutput(output, targetPath) {
  ...
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    if (lower.startsWith(lowerTarget)) {
      ...
      if (lower.startsWith(quotedLower)) {
...
export function createIcaclsResetCommand(targetPath, opts) {
  ...
  const grant = opts.isDir ? "(OI)(CI)F" : "F";
  const args = [targetPath, "/inheritance:r", "/grant:r", ":", "/grant:r", "SYSTEM:"];
