...
async function safeAclReset(params) {
  const display = formatIcaclsResetCommand(params.path, { isDir: (params.require === "dir"), env: params.env });
  try {
    {
      const st = await fs.lstat(params.path);
      if (st.isSymbolicLink()) {
      ...
      const cmd = createIcaclsResetCommand(params.path, { isDir: st.isDirectory(), env: params.env });
      if (!cmd) {
...
function setGroupPolicyAllowlist(params) {
  ...
  if ((topPolicy === "open")) {
    ...
    params.changes.push("channels..groupPolicy=open -> allowlist");
    params.policyFlips.add("channels..");
  }
  ...
  for (const [accountId, accountValue] of Object.entries(accounts)) {
    ...
    if ((account.groupPolicy === "open")) {
      ...
      params.changes.push("channels..accounts..groupPolicy=open -> allowlist");
      params.policyFlips.add("channels..accounts..");
    }
  }
}
function setWhatsAppGroupAllowFromFromStore(params) {
  ...
  const maybeApply = (prefix, obj) => {
    ...
    params.changes.push("groupAllowFrom=pairing-store");
  };
  ...
  for (const [accountId, accountValue] of Object.entries(accounts)) {
    ...
    const account = accountValue;
    maybeApply("channels.whatsapp.accounts..", account);
  }
}
...
export async function fixSecurityFootguns(opts) {
  ...
  if (snap.valid) {
    ...
    if ((whatsappStoreAllowFrom.length > 0)) {
      setWhatsAppGroupAllowFromFromStore({ cfg: fixed.cfg, storeAllowFrom: whatsappStoreAllowFrom, changes, policyFlips: fixed.policyFlips });
    }
    if ((changes.length > 0)) {
      try {
        {
        ...
        {
          errors.push("writeConfigFile failed: ");
        }
      }
    }
  }
  ...
  if (snap.exists) {
    const includePaths = await collectIncludePathsRecursive({ configPath: snap.path, parsed: snap.parsed }).catch(() => []);
    for (const p of includePaths) {
      actions.push(await applyPerms({ path: p, mode: 384, require: "file" }));
    }
  }
  await chmodCredentialsAndAgentState({ env, stateDir, cfg: (snap.config ?? {  }), actions, applyPerms }).catch((err) => {
    errors.push("chmodCredentialsAndAgentState failed: ");
  });
