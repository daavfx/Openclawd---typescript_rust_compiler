...
", "utf-8");
    ...
    await fs.writeFile(path.join(credsDir, "whatsapp-allowFrom.json"), "
", "utf-8");
    const env = { ...process.env: , OPENCLAW_STATE_DIR: stateDir, OPENCLAW_CONFIG_PATH: "" };
    const res = await fixSecurityFootguns({ env });
    expect(res.ok).toBe(true);
    expect(res.configWritten).toBe(true);
    expect(res.changes).toEqual(expect.arrayContaining(["channels.telegram.groupPolicy=open -> allowlist", "channels.whatsapp.groupPolicy=open -> allowlist", "channels.discord.groupPolicy=open -> allowlist", "channels.signal.groupPolicy=open -> allowlist", "channels.imessage.groupPolicy=open -> allowlist", "logging.redactSensitive=off -> \"tools\""]));
...
", "utf-8");
    ...
    const configPath = path.join(stateDir, "openclaw.json");
    await fs.writeFile(configPath, "{ \"$include\": \"./includes/extra.json5\", channels: { whatsapp: { groupPolicy: \"open\" } } }
", "utf-8");
    await fs.chmod(configPath, 420);
    const credsDir = path.join(stateDir, "credentials");
    await fs.mkdir(credsDir, { recursive: true });
    const allowFromPath = path.join(credsDir, "whatsapp-allowFrom.json");
    await fs.writeFile(allowFromPath, "
", "utf-8");
