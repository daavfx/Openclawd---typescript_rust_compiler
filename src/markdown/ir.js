...
function appendCell(state, cell) {
  ...
  for (const span of cell.styles) {
    state.styles.push({ start: (start + span.start), end: (start + span.end), style: span.style });
  }
  for (const link of cell.links) {
    state.links.push({ start: (start + link.start), end: (start + link.end), href: link.href });
  }
}
...
export function markdownToIRWithMeta(markdown, options = {  }) {
  ...
  const state = { text: "", styles: [], openStyles: [], links: [], linkStack: [], env, headingStyle: (options.headingStyle ?? "none"), blockquotePrefix: (options.blockquotePrefix ?? ""), enableSpoilers: (options.enableSpoilers ?? false), tableMode, table: null, hasTables: false };
  ...
  return { ir: { text: finalText, styles: mergeStyleSpans(clampStyleSpans(state.styles, finalLength)), links: clampLinkSpans(state.links, finalLength) }, hasTables: state.hasTables };
}
...
export function chunkMarkdownIR(ir, limit) {
  ...
  chunks.forEach((chunk, index) => {
    ...
    results.push({ text: chunk, styles: sliceStyleSpans(ir.styles, start, end), links: sliceLinkSpans(ir.links, start, end) });
    cursor = end;
  });
  return results;
}
