...
const DEFAULT_ELEVENLABS_VOICE_SETTINGS = { stability: 0.5, similarityBoost: 0.75, style: 0, useSpeakerBoost: true, speed: 1 };
const TELEGRAM_OUTPUT = { openai: "opus", elevenlabs: "opus_48000_64", extension: ".opus", voiceCompatible: true };
const DEFAULT_OUTPUT = { openai: "mp3", elevenlabs: "mp3_44100_128", extension: ".mp3", voiceCompatible: false };
const TELEPHONY_OUTPUT = { openai: { format: "pcm", sampleRate: 24000 }, elevenlabs: { format: "pcm_22050", sampleRate: 22050 } };
...
function resolveModelOverridePolicy(overrides) {
  ...
  if (!enabled) {
    return { enabled: false, allowText: false, allowProvider: false, allowVoice: false, allowModelId: false, allowVoiceSettings: false, allowNormalization: false, allowSeed: false };
  }
  const allow = (value) => (value ?? true);
  return { enabled: true, allowText: allow(overrides?.allowText), allowProvider: allow(overrides?.allowProvider), allowVoice: allow(overrides?.allowVoice), allowModelId: allow(overrides?.allowModelId), allowVoiceSettings: allow(overrides?.allowVoiceSettings), allowNormalization: allow(overrides?.allowNormalization), allowSeed: allow(overrides?.allowSeed) };
}
...
export function buildTtsSystemPromptHint(cfg) {
  ...
  const autoHint = (autoMode === "inbound") ? "Only use TTS when the user's last message includes audio/voice." : (autoMode === "tagged") ? "Only use TTS when you include [[tts]] or [[tts:text]] tags." : undefined;
  return ["Voice (TTS) is enabled.", autoHint, "Keep spoken text ≤ chars to avoid auto-summary (summary ).", "Use [[tts:...]] and optional [[tts:text]]...[[/tts:text]] to control voice/expressiveness."].filter(Boolean).join("
");
}
...
function requireInRange(value, min, max, label) {
  if (((!Number.isFinite(value) || (value < min)) || (value > max))) {
    throw new Error(" must be between  and ");
  }
}
          ...
          switch (key) {
            case "provider":
              ...
              if ((((rawValue === "openai") || (rawValue === "elevenlabs")) || (rawValue === "edge"))) {
                ...
                warnings.push("unsupported provider \"\"");
              }
              break;
            case "voice":
            case "openai_voice":
            case "openaivoice":
              ...
              if (isValidOpenAIVoice(rawValue)) {
                ...
                warnings.push("invalid OpenAI voice \"\"");
              }
              break;
            case "voiceid":
            case "voice_id":
            ...
            case "elevenlabsvoice":
              ...
              if (isValidVoiceId(rawValue)) {
                ...
                warnings.push("invalid ElevenLabs voiceId \"\"");
              }
              break;
            case "model":
            case "modelid":
            ...
            case "stability":
              ...
              }
              {
                ...
                overrides.elevenlabs = { ...overrides.elevenlabs: , voiceSettings: { ...overrides.elevenlabs?.voiceSettings: , stability: value } };
              }
              break;
            case "similarity":
            ...
            case "similarity_boost":
              ...
              }
              {
                ...
                overrides.elevenlabs = { ...overrides.elevenlabs: , voiceSettings: { ...overrides.elevenlabs?.voiceSettings: , similarityBoost: value } };
              }
              break;
            case "style":
              if (!policy.allowVoiceSettings) {
              ...
              }
              {
                ...
                overrides.elevenlabs = { ...overrides.elevenlabs: , voiceSettings: { ...overrides.elevenlabs?.voiceSettings: , style: value } };
              }
              break;
            case "speed":
              if (!policy.allowVoiceSettings) {
              ...
              }
              {
                ...
                overrides.elevenlabs = { ...overrides.elevenlabs: , voiceSettings: { ...overrides.elevenlabs?.voiceSettings: , speed: value } };
              }
              break;
            case "speakerboost":
            case "speaker_boost":
            ...
            case "use_speaker_boost":
              ...
              }
              {
                ...
                overrides.elevenlabs = { ...overrides.elevenlabs: , voiceSettings: { ...overrides.elevenlabs?.voiceSettings: , useSpeakerBoost: value } };
              }
              break;
            case "normalize":
            ...
            case "apply_text_normalization":
              ...
              overrides.elevenlabs = { ...overrides.elevenlabs: , applyTextNormalization: normalizeApplyTextNormalization(rawValue) };
              break;
            case "language":
            case "languagecode":
            case "language_code":
              ...
              overrides.elevenlabs = { ...overrides.elevenlabs: , languageCode: normalizeLanguageCode(rawValue) };
              break;
            case "seed":
              if (!policy.allowSeed) {
                break;
              }
              overrides.elevenlabs = { ...overrides.elevenlabs: , seed: normalizeSeed(Number.parseInt(rawValue, 10)) };
              break;
            default:
              break;
          }
...
export const OPENAI_TTS_VOICES = ["alloy", "ash", "coral", "echo", "fable", "onyx", "nova", "sage", "shimmer"]
...
function resolveSummaryModelRef(cfg, config) {
  ...
  const resolved = resolveModelRefFromString({ raw: override, defaultProvider: defaultRef.provider, aliasIndex });
  if (!resolved) {
    return { ref: defaultRef, source: "default" };
  }
...
async function summarizeText(params) {
  ...
  if (((targetLength < 100) || (targetLength > 10000))) {
    throw new Error("Invalid targetLength: ");
  }
  const startTime = Date.now();
  const {ref} = resolveSummaryModelRef(cfg, config);
  const resolved = resolveModel(ref.provider, ref.model, undefined, cfg);
  if (!resolved.model) {
    throw new Error((resolved.error ?? "Unknown summary model: /"));
  }
  ...
  try {
    {
      ...
      try {
        {
          const res = await completeSimple(resolved.model, { messages: [{ role: "user", content: ((("You are an assistant that summarizes texts concisely while keeping the most important information. " + "Summarize the text to approximately  characters. Maintain the original tone and style. ") + "Reply only with the summary, without additional explanations.

") + "<text_to_summarize>

</text_to_summarize>"), timestamp: Date.now() }] }, { apiKey, maxTokens: Math.ceil((targetLength / 2)), temperature: 0.3, signal: controller.signal });
          const summary = res.content.filter(isTextContentBlock).map((block) => block.text.trim()).filter(Boolean).join(" ").trim();
          if (!summary) {
            throw new Error("No summary returned");
          }
          return { summary, latencyMs: (Date.now() - startTime), inputLength: text.length, outputLength: summary.length };
        }
      }
      finally {
        {
...
async function openaiTTS(params) {
  ...
  try {
    {
      const response = await fetch("/audio/speech", { method: "POST", headers: { Authorization: "Bearer ", "Content-Type": "application/json" }, body: JSON.stringify({ model, input: text, voice, response_format: responseFormat }), signal: controller.signal });
      if (!response.ok) {
...
async function edgeTTS(params) {
  ...
  const tts = new EdgeTTS({ voice: config.voice, lang: config.lang, outputFormat: config.outputFormat, saveSubtitles: config.saveSubtitles, proxy: config.proxy, rate: config.rate, pitch: config.pitch, volume: config.volume, timeout: (config.timeoutMs ?? timeoutMs) });
...
export async function textToSpeech(params) {
  ...
  if ((params.text.length > config.maxTextLength)) {
    return { success: false, error: "Text too long ( chars, max )" };
  }
    ...
    try {
      {
        if ((provider === "edge")) {
          ...
          const attemptEdgeTts = async (outputFormat) => {
            ...
            const audioPath = path.join(tempDir, "voice-");
            await edgeTTS({ text: params.text, outputPath: audioPath, config: { ...config.edge: , outputFormat }, timeoutMs: config.timeoutMs });
          ...
          try {
          ...
          catch (err) {
            {
              if ((fallbackEdgeOutputFormat && (fallbackEdgeOutputFormat !== edgeOutputFormat))) {
                logVerbose("TTS: Edge output  failed; retrying with .");
          ...
          return { success: true, audioPath: edgeResult.audioPath, latencyMs: (Date.now() - providerStart), provider, outputFormat: edgeResult.outputFormat, voiceCompatible };
        }
        const apiKey = resolveTtsApiKey(config, provider);
        if (!apiKey) {
          lastError = "No API key for ";
          continue;
        }
        let audioBuffer;
        if ((provider === "elevenlabs")) {
          ...
          const voiceSettings = { ...config.elevenlabs.voiceSettings: , ...params.overrides?.elevenlabs?.voiceSettings:  };
          ...
          audioBuffer = await openaiTTS({ text: params.text, apiKey, model: (openaiModelOverride ?? config.openai.model), voice: (openaiVoiceOverride ?? config.openai.voice), responseFormat: output.openai, timeoutMs: config.timeoutMs });
        }
        ...
        scheduleCleanup(tempDir);
        return { success: true, audioPath, latencyMs, provider, outputFormat: (provider === "openai") ? output.openai : output.elevenlabs, voiceCompatible: output.voiceCompatible };
      }
    }
    catch (err) {
      {
  ...
  return { success: false, error: "TTS conversion failed: " };
}
    ...
    try {
      {
        ...
        if ((provider === "elevenlabs")) {
          ...
          const audioBuffer = await elevenLabsTTS({ text: params.text, apiKey, baseUrl: config.elevenlabs.baseUrl, voiceId: config.elevenlabs.voiceId, modelId: config.elevenlabs.modelId, outputFormat: output.format, seed: config.elevenlabs.seed, applyTextNormalization: config.elevenlabs.applyTextNormalization, languageCode: config.elevenlabs.languageCode, voiceSettings: config.elevenlabs.voiceSettings, timeoutMs: config.timeoutMs });
          return { success: true, audioBuffer, latencyMs: (Date.now() - providerStart), provider, outputFormat: output.format, sampleRate: output.sampleRate };
        }
        const output = TELEPHONY_OUTPUT.openai;
        const audioBuffer = await openaiTTS({ text: params.text, apiKey, model: config.openai.model, voice: config.openai.voice, responseFormat: output.format, timeoutMs: config.timeoutMs });
        return { success: true, audioBuffer, latencyMs: (Date.now() - providerStart), provider, outputFormat: output.format, sampleRate: output.sampleRate };
      }
    }
    catch (err) {
...
export async function maybeApplyTtsToPayload(params) {
  ...
  if ((directives.warnings.length > 0)) {
    logVerbose("TTS: ignored directive overrides ()");
  }
  const cleanedText = directives.cleanedText;
  const trimmedCleaned = cleanedText.trim();
  const visibleText = (trimmedCleaned.length > 0) ? trimmedCleaned : "";
  const ttsText = (directives.ttsText?.trim() || visibleText);
  const nextPayload = (visibleText === text.trim()) ? params.payload : { ...params.payload: , text: (visibleText.length > 0) ? visibleText : undefined };
  ...
  if ((textForAudio.length > maxLength)) {
    if (!isSummarizationEnabled(prefsPath)) {
      logVerbose("TTS: truncating long text ( > ), summarization disabled.");
      textForAudio = "...";
    } else {
      try {
        {
          ...
          if ((textForAudio.length > config.maxTextLength)) {
            logVerbose("TTS: summary exceeded hard limit ( > ); truncating.");
            textForAudio = "...";
          }
        }
      }
        ...
        {
          const error = err;
          logVerbose("TTS: summarization failed, truncating instead: ");
          textForAudio = "...";
        }
      }
    }
  ...
  const result = await textToSpeech({ text: textForAudio, cfg: params.cfg, prefsPath, channel: params.channel, overrides: directives.overrides });
  if ((result.success && result.audioPath)) {
    lastTtsAttempt = { timestamp: Date.now(), success: true, textLength: text.length, summarized: wasSummarized, provider: result.provider, latencyMs: result.latencyMs };
    const channelId = resolveChannelId(params.channel);
    const shouldVoice = ((channelId === "telegram") && (result.voiceCompatible === true));
    const finalPayload = { ...nextPayload: , mediaUrl: result.audioPath, audioAsVoice: (shouldVoice || params.payload.audioAsVoice) };
    return finalPayload;
  }
  lastTtsAttempt = { timestamp: Date.now(), success: false, textLength: text.length, summarized: wasSummarized, error: result.error };
  const latency = (Date.now() - ttsStart);
  logVerbose("TTS: conversion failed after ms ().");
  return nextPayload;
}

export const _test = { isValidVoiceId, isValidOpenAIVoice, isValidOpenAIModel, OPENAI_TTS_MODELS, OPENAI_TTS_VOICES, parseTtsDirectives, resolveModelOverridePolicy, summarizeText, resolveOutputFormat, resolveEdgeOutputFormat }
