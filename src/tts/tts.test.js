...
vi.mock("../agents/pi-embedded-runner/model.js", () => { resolveModel: vi.fn((provider, modelId) => { model: { provider, id: modelId, name: modelId, api: "openai-completions", reasoning: false, input: ["text"], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 128000, maxTokens: 8192 }, authStorage: { profiles: {  } }, modelRegistry: { find: vi.fn() } }) });
vi.mock("../agents/model-auth.js", () => { getApiKeyForModel: vi.fn(async () => { apiKey: "test-api-key", source: "test", mode: "api-key" }), requireApiKey: vi.fn((auth) => (auth.apiKey ?? "")) });
const {_test, resolveTtsConfig, maybeApplyTtsToPayload, getTtsProvider} = tts;
const {isValidVoiceId, isValidOpenAIVoice, isValidOpenAIModel, OPENAI_TTS_MODELS, OPENAI_TTS_VOICES, parseTtsDirectives, resolveModelOverridePolicy, summarizeText, resolveOutputFormat, resolveEdgeOutputFormat} = _test;
describe("tts", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(completeSimple).mockResolvedValue({ content: [{ type: "text", text: "Summary" }] });
  });
  ...
  describe("resolveEdgeOutputFormat", () => {
    const baseCfg = { agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } }, messages: { tts: {  } } };
    ...
    it("uses configured output format when provided", () => {
      const config = resolveTtsConfig({ ...baseCfg: , messages: { tts: { edge: { outputFormat: "audio-24khz-96kbitrate-mono-mp3" } } } });
...
" + "[[tts:text]](laughs) Read the song once more.[[/tts:text]]");
  ...
  describe("summarizeText", () => {
    ...
    it("summarizes text and returns result with metrics", async () => {
      ...
      vi.mocked(completeSimple).mockResolvedValue({ content: [{ type: "text", text: mockSummary }] });
      const longText = "A".repeat(2000);
      const result = await summarizeText({ text: longText, targetLength: 1500, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 });
    ...
    it("calls the summary model with the expected parameters", async () => {
      await summarizeText({ text: "Long text to summarize", targetLength: 500, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 });
    ...
    it("uses summaryModel override when configured", async () => {
      const cfg = { agents: { defaults: { model: { primary: "anthropic/claude-opus-4-5" } } }, messages: { tts: { summaryModel: "openai/gpt-4.1-mini" } } };
      const config = resolveTtsConfig(cfg);
      await summarizeText({ text: "Long text to summarize", targetLength: 500, cfg, config, timeoutMs: 30000 });
    ...
    it("rejects targetLength below minimum (100)", async () => {
      await expect(summarizeText({ text: "text", targetLength: 99, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 })).rejects.toThrow("Invalid targetLength: 99");
    });
    it("rejects targetLength above maximum (10000)", async () => {
      await expect(summarizeText({ text: "text", targetLength: 10001, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 })).rejects.toThrow("Invalid targetLength: 10001");
    });
    it("accepts targetLength at boundaries", async () => {
      await expect(summarizeText({ text: "text", targetLength: 100, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 })).resolves.toBeDefined();
      await expect(summarizeText({ text: "text", targetLength: 10000, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 })).resolves.toBeDefined();
    });
    it("throws error when no summary is returned", async () => {
      vi.mocked(completeSimple).mockResolvedValue({ content: [] });
      await expect(summarizeText({ text: "text", targetLength: 500, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 })).rejects.toThrow("No summary returned");
    });
    it("throws error when summary content is empty", async () => {
      vi.mocked(completeSimple).mockResolvedValue({ content: [{ type: "text", text: "   " }] });
      await expect(summarizeText({ text: "text", targetLength: 500, cfg: baseCfg, config: baseConfig, timeoutMs: 30000 })).rejects.toThrow("No summary returned");
    });
  });
  describe("getTtsProvider", () => {
    ...
    const withEnv = (env, run) => {
      const snapshot = { OPENAI_API_KEY: process.env.OPENAI_API_KEY, ELEVENLABS_API_KEY: process.env.ELEVENLABS_API_KEY, XI_API_KEY: process.env.XI_API_KEY };
      try {
        {
    ...
    it("prefers OpenAI when no provider is configured and API key exists", () => {
      withEnv({ OPENAI_API_KEY: "test-openai-key", ELEVENLABS_API_KEY: undefined, XI_API_KEY: undefined }, () => {
        const config = resolveTtsConfig(baseCfg);
    ...
    it("prefers ElevenLabs when OpenAI is missing and ElevenLabs key exists", () => {
      withEnv({ OPENAI_API_KEY: undefined, ELEVENLABS_API_KEY: "test-elevenlabs-key", XI_API_KEY: undefined }, () => {
        const config = resolveTtsConfig(baseCfg);
    ...
    it("falls back to Edge when no API keys are present", () => {
      withEnv({ OPENAI_API_KEY: undefined, ELEVENLABS_API_KEY: undefined, XI_API_KEY: undefined }, () => {
        const config = resolveTtsConfig(baseCfg);
  ...
  describe("maybeApplyTtsToPayload", () => {
    const baseCfg = { agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } }, messages: { tts: { auto: "inbound", provider: "openai", openai: { apiKey: "test-key", model: "gpt-4o-mini-tts", voice: "alloy" } } } };
    it("skips auto-TTS when inbound audio gating is on and the message is not audio", async () => {
      const prevPrefs = process.env.OPENCLAW_TTS_PREFS;
      process.env.OPENCLAW_TTS_PREFS = "/tmp/tts-test-.json";
      const originalFetch = globalThis.fetch;
      const fetchMock = vi.fn(async () => { ok: true, arrayBuffer: async () => new ArrayBuffer(1) });
      globalThis.fetch = fetchMock;
      const payload = { text: "Hello world" };
      const result = await maybeApplyTtsToPayload({ payload, cfg: baseCfg, kind: "final", inboundAudio: false });
    ...
    it("attempts auto-TTS when inbound audio gating is on and the message is audio", async () => {
      ...
      const fetchMock = vi.fn(async () => { ok: true, arrayBuffer: async () => new ArrayBuffer(1) });
      globalThis.fetch = fetchMock;
      const result = await maybeApplyTtsToPayload({ payload: { text: "Hello world" }, cfg: baseCfg, kind: "final", inboundAudio: true });
    ...
    it("skips auto-TTS in tagged mode unless a tts tag is present", async () => {
      ...
      const fetchMock = vi.fn(async () => { ok: true, arrayBuffer: async () => new ArrayBuffer(1) });
      globalThis.fetch = fetchMock;
      const cfg = { ...baseCfg: , messages: { ...baseCfg.messages: , tts: { ...baseCfg.messages.tts: , auto: "tagged" } } };
      const payload = { text: "Hello world" };
    ...
    it("runs auto-TTS in tagged mode when tags are present", async () => {
      ...
      globalThis.fetch = fetchMock;
      const cfg = { ...baseCfg: , messages: { ...baseCfg.messages: , tts: { ...baseCfg.messages.tts: , auto: "tagged" } } };
      const result = await maybeApplyTtsToPayload({ payload: { text: "[[tts:text]]Hello world[[/tts:text]]" }, cfg, kind: "final" });
      expect(result.mediaUrl).toBeDefined();
      expect(fetchMock).toHaveBeenCalledTimes(1);
      globalThis.fetch = originalFetch;
      process.env.OPENCLAW_TTS_PREFS = prevPrefs;
