...
vi.doMock("node:os", () => { default: { homedir: () => HOME, tmpdir: () => realOs.tmpdir() }, homedir: () => HOME, tmpdir: () => realOs.tmpdir() });
vi.doMock("node:https", () => { request: (...args) => mockRequest(...args) });
...
describe("media store redirects", () => {
  ...
  it("follows redirects and keeps detected mime/extension", async () => {
    ...
    mockRequest.mockImplementation((_url, _opts, cb) => {
      ...
      const req = { on: (event, handler) => {
        if ((event === "error")) {
          res.on("error", handler);
        }
        return req;
      }, end: () => undefined, destroy: () => res.destroy() };
      if ((call === 1)) {
        res.statusCode = 302;
