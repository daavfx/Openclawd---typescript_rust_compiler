...
vi.mock("./store.js", async (importOriginal) => {
  ...
  return { ...actual: , getMediaDir: () => MEDIA_DIR, cleanOldMedia };
});
...
const waitForFileRemoval = async (file, timeoutMs = 200) => {
  ...
  throw new Error("timed out waiting for  removal");
};
describe("media server", () => {
  ...
  it("serves media and cleans up after send", async () => {
    ...
    const server = await startMediaServer(0, 5000);
    const port = server.address().port;
    const res = await fetch("http://localhost:/media/file1");
  ...
  it("expires old media", async () => {
    ...
    const server = await startMediaServer(0, 1000);
    const port = server.address().port;
    const res = await fetch("http://localhost:/media/old");
  ...
  it("blocks path traversal attempts", async () => {
    const server = await startMediaServer(0, 5000);
    const port = server.address().port;
    const res = await fetch("http://localhost:/media/%2e%2e%2fpackage.json");
  ...
  it("blocks symlink escaping outside media dir", async () => {
    ...
    const server = await startMediaServer(0, 5000);
    const port = server.address().port;
    const res = await fetch("http://localhost:/media/link-out");
    expect(res.status).toBe(400);
    expect(await res.text()).toBe("invalid path");
    await new Promise((r) => server.close(r));
  });
  it("rejects invalid media ids", async () => {
    ...
    const server = await startMediaServer(0, 5000);
    const port = server.address().port;
    const res = await fetch("http://localhost:/media/invalid%20id");
    expect(res.status).toBe(400);
    expect(await res.text()).toBe("invalid path");
    await new Promise((r) => server.close(r));
  });
  it("rejects oversized media files", async () => {
    ...
    const server = await startMediaServer(0, 5000);
    const port = server.address().port;
    const res = await fetch("http://localhost:/media/big");
