...
export const DEFAULT_INPUT_FILE_MIMES = ["text/plain", "text/markdown", "text/html", "text/csv", "application/json", "application/pdf"]
  ...
  try {
    {
      while (true) {
        ...
        try {
          {
            const response = await fetch(parsedUrl, { signal: controller.signal, headers: { "User-Agent": "OpenClaw-Gateway/1.0" }, redirect: "manual", dispatcher });
            ...
            if (contentLength) {
              ...
              if ((size > params.maxBytes)) {
                throw new Error("Content too large:  bytes (limit:  bytes)");
              }
            }
...
");
  ...
  try {
    ...
    {
      logWarn("media: PDF image extraction skipped; ");
      return { text, images: [] };
    }
  }
  ...
  for (let pageNum = 1; (pageNum <= maxPages); pageNum += 1) {
    ...
    await page.render({ canvas: canvas, viewport: scaled }).promise;
...
export async function extractImageContentFromSource(source, limits) {
  if ((source.type === "base64")) {
    ...
    if ((buffer.byteLength > limits.maxBytes)) {
      throw new Error("Image too large:  bytes (limit:  bytes)");
    }
  ...
  if (((source.type === "url") && source.url)) {
    ...
    const result = await fetchWithGuard({ url: source.url, maxBytes: limits.maxBytes, timeoutMs: limits.timeoutMs, maxRedirects: limits.maxRedirects });
    if (!limits.allowedMimes.has(result.mimeType)) {
      throw new Error("Unsupported image MIME type from URL: ");
    }
...
export async function extractFileContentFromSource(params) {
  ...
  if ((buffer.byteLength > limits.maxBytes)) {
    throw new Error("File too large:  bytes (limit:  bytes)");
  }
  if (!mimeType) {
