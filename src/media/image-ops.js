...
async function sipsMetadataFromBuffer(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const {stdout} = await runExec("/usr/bin/sips", ["-g", "pixelWidth", "-g", "pixelHeight", input], { timeoutMs: 10000, maxBuffer: (512 * 1024) });
...
async function sipsResizeToJpeg(params) {
  return await withTempDir(async (dir) => {
    ...
    await runExec("/usr/bin/sips", ["-Z", String(Math.max(1, Math.round(params.maxSide))), "-s", "format", "jpeg", "-s", "formatOptions", String(Math.max(1, Math.min(100, Math.round(params.quality)))), input, "--out", output], { timeoutMs: 20000, maxBuffer: (1024 * 1024) });
...
async function sipsConvertToJpeg(buffer) {
  return await withTempDir(async (dir) => {
    ...
    const output = path.join(dir, "out.jpg");
    await fs.writeFile(input, buffer);
    await runExec("/usr/bin/sips", ["-s", "format", "jpeg", input, "--out", output], { timeoutMs: 20000, maxBuffer: (1024 * 1024) });
    return await fs.readFile(output);
  });
}
...
async function sipsApplyOrientation(buffer, orientation) {
  ...
  return await withTempDir(async (dir) => {
    ...
    const output = path.join(dir, "out.jpg");
    await fs.writeFile(input, buffer);
    await runExec("/usr/bin/sips", [...ops, input, "--out", output], { timeoutMs: 20000, maxBuffer: (1024 * 1024) });
    return await fs.readFile(output);
  });
}
    ...
    if ((params.withoutEnlargement !== false)) {
      ...
      if (meta) {
        ...
        if (((maxDim > 0) && (maxDim <= params.maxSide))) {
          return await sipsResizeToJpeg({ buffer: normalized, maxSide: maxDim, quality: params.quality });
        }
      }
    }
    return await sipsResizeToJpeg({ buffer: normalized, maxSide: params.maxSide, quality: params.quality });
  }
  const sharp = await loadSharp();
  return await sharp(params.buffer).rotate().resize({ width: params.maxSide, height: params.maxSide, fit: "inside", withoutEnlargement: (params.withoutEnlargement !== false) }).jpeg({ quality: params.quality, mozjpeg: true }).toBuffer();
}
...
export async function resizeToPng(params) {
  ...
  return await sharp(params.buffer).rotate().resize({ width: params.maxSide, height: params.maxSide, fit: "inside", withoutEnlargement: (params.withoutEnlargement !== false) }).png({ compressionLevel }).toBuffer();
}
...
export async function optimizeImageToPng(buffer, maxBytes) {
  ...
  for (const side of sides) {
    for (const compressionLevel of compressionLevels) {
      try {
        {
          const out = await resizeToPng({ buffer, maxSide: side, compressionLevel, withoutEnlargement: true });
          const size = out.length;
          if ((!smallest || (size < smallest.size))) {
            smallest = { buffer: out, size, resizeSide: side, compressionLevel };
          }
          if ((size <= maxBytes)) {
            return { buffer: out, optimizedSize: size, resizeSide: side, compressionLevel };
          }
        }
      }
      catch {
  ...
  if (smallest) {
    return { buffer: smallest.buffer, optimizedSize: smallest.size, resizeSide: smallest.resizeSide, compressionLevel: smallest.compressionLevel };
  }
