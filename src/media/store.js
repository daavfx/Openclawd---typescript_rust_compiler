...
async function downloadToFile(url, dest, headers, maxRedirects = 5) {
  return await new Promise((resolve, reject) => {
    ...
    if (!["http:", "https:"].includes(parsedUrl.protocol)) {
      reject(new Error("Invalid URL protocol: . Only HTTP/HTTPS allowed."));
      return;
    }
    ...
    resolvePinnedHostname(parsedUrl.hostname).then((pinned) => {
      const req = requestImpl(parsedUrl, { headers, lookup: pinned.lookup }, (res) => {
        ...
        if ((!res.statusCode || (res.statusCode >= 400))) {
          reject(new Error("HTTP  downloading media"));
          return;
        }
        let total = 0;
        const sniffChunks = [];
        ...
        pipeline(res, out).then(() => {
          ...
          resolve({ headerMime, sniffBuffer, size: total });
        }).catch(reject);
      });
      req.on("error", reject);
      req.end();
...
export async function saveMediaSource(source, headers, subdir = "") {
  ...
  if (looksLikeUrl(source)) {
    const tempDest = path.join(dir, ".tmp");
...
export async function saveMediaBuffer(buffer, contentType, subdir = "inbound", maxBytes = MAX_BYTES, originalFilename) {
  if ((buffer.byteLength > maxBytes)) {
    throw new Error("Media exceeds MB limit");
  }
