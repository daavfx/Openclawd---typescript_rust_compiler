...
describe("media store", () => {
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(".openclawmedia");
  ...
  it("saves buffers and enforces size limit", async () => {
    await withTempStore(async (store) => {
      ...
      const jpeg = await sharp({ create: { width: 2, height: 2, channels: 3, background: "#123456" } }).jpeg({ quality: 80 }).toBuffer();
  ...
  it("renames media based on detected mime even when extension is wrong", async () => {
    await withTempStore(async (store, home) => {
      const pngBytes = await sharp({ create: { width: 2, height: 2, channels: 3, background: "#00ff00" } }).png().toBuffer();
  ...
  describe("extractOriginalFilename", () => {
    it("extracts original filename from embedded pattern", async () => {
      await withTempStore(async (store) => {
        ...
        const result = store.extractOriginalFilename("/path/to/");
    ...
    it("handles uppercase UUID pattern", async () => {
      await withTempStore(async (store) => {
        ...
        const result = store.extractOriginalFilename("/media/inbound/");
    ...
    it("falls back to basename for non-matching patterns", async () => {
      await withTempStore(async (store) => {
        ...
        expect(store.extractOriginalFilename("/path/")).toBe(uuidOnly);
    ...
    it("preserves original name with special characters", async () => {
      await withTempStore(async (store) => {
        ...
        const result = store.extractOriginalFilename("/media/");
