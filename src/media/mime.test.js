...
async function makeOoxmlZip(opts) {
  const zip = new JSZip();
  zip.file("[Content_Types].xml", "<Types><Override PartName=\"\" ContentType=\".main+xml\"/></Types>");
...
describe("mime detection", () => {
  ...
  it("detects docx from buffer", async () => {
    const buf = await makeOoxmlZip({ mainMime: "application/vnd.openxmlformats-officedocument.wordprocessingml.document", partPath: "/word/document.xml" });
  ...
  it("detects pptx from buffer", async () => {
    const buf = await makeOoxmlZip({ mainMime: "application/vnd.openxmlformats-officedocument.presentationml.presentation", partPath: "/ppt/presentation.xml" });
    const mime = await detectMime({ buffer: buf, filePath: "/tmp/file.bin" });
  ...
  it("prefers extension mapping over generic zip", async () => {
    ...
    const mime = await detectMime({ buffer: buf, filePath: "/tmp/file.xlsx" });
