...
async function main() {
  ...
  const [{loadConfig}, {startGatewayServer}, {setGatewayWsLogStyle}, {setVerbose}, {acquireGatewayLock, GatewayLockError}, {consumeGatewaySigusr1RestartAuthorization, isGatewaySigusr1RestartExternallyAllowed}, {defaultRuntime}, {enableConsoleCapture, setConsoleTimestampPrefix}] = await Promise.all([import("../config/config.js"), import("../gateway/server.js"), import("../gateway/ws-logging.js"), import("../globals.js"), import("../infra/gateway-lock.js"), import("../infra/restart.js"), import("../runtime.js"), import("../logging.js")]);
  ...
  const request = (action, signal) => {
    if (shuttingDown) {
      defaultRuntime.log("gateway: received  during shutdown; ignoring");
      return;
    }
    shuttingDown = true;
    const isRestart = (action === "restart");
    defaultRuntime.log("gateway: received ; ");
    ...
    void async () => {
      try {
        ...
        {
          defaultRuntime.error("gateway: shutdown error: ");
        }
      }
      finally {
        {
      ...
      try {
      ...
      catch (err) {
        {
          if ((err instanceof GatewayLockError)) {
            defaultRuntime.error("Gateway start blocked: ");
            process.exit(1);
          }
          throw err;
        }
