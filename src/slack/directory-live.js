...
export async function listSlackDirectoryPeersLive(params) {
  ...
  const rows = filtered.map((member) => {
    ...
    return { kind: "user", id: "user:", name: (display || undefined), handle: handle ? "@" : undefined, rank: buildUserRank(member), raw: member };
  }).filter(Boolean);
...
export async function listSlackDirectoryGroupsLive(params) {
  ...
  const rows = filtered.map((channel) => {
    ...
    return { kind: "group", id: "channel:", name, handle: "#", rank: buildChannelRank(channel), raw: channel };
  }).filter(Boolean);
  if (((typeof params.limit === "number") && (params.limit > 0))) {
    return rows.slice(0, params.limit);
  }
