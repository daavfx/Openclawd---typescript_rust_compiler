...
function resolveToken(params) {
  ...
  if (!fallback) {
    logVerbose("slack send: missing bot token for account= explicit= source=");
    throw new Error("Slack bot token missing for account \"\" (set channels.slack.accounts..botToken or SLACK_BOT_TOKEN for default).");
  }
  return fallback;
}
...
async function uploadSlackFile(params) {
  ...
  const basePayload = { channel_id: params.channelId, file: buffer, filename: fileName, ...params.caption ? { initial_comment: params.caption } : {  }:  };
...
export async function sendMessageSlack(to, message, opts = {  }) {
  ...
  const token = resolveToken({ explicit: opts.token, accountId: account.accountId, fallbackToken: account.botToken, fallbackSource: account.botTokenSource });
  ...
  const mediaMaxBytes = (typeof account.config.mediaMaxMb === "number") ? ((account.config.mediaMaxMb * 1024) * 1024) : undefined;
  let lastMessageId = "";
  if (opts.mediaUrl) {
    const [firstChunk, ...rest] = chunks;
    lastMessageId = await uploadSlackFile({ client, channelId, mediaUrl: opts.mediaUrl, caption: firstChunk, threadTs: opts.threadTs, maxBytes: mediaMaxBytes });
    for (const chunk of rest) {
      const response = await client.chat.postMessage({ channel: channelId, text: chunk, thread_ts: opts.threadTs });
  ...
  return { messageId: (lastMessageId || "unknown"), channelId };
}
