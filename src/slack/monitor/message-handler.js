...
export function createSlackMessageHandler(params) {
  ...
  const debouncer = createInboundDebouncer({ debounceMs, buildKey: (entry) => {
    ...
    const threadKey = entry.message.thread_ts ? ":" : (entry.message.parent_user_id && messageTs) ? ":maybe-thread:" : entry.message.channel;
    return "slack:::";
...
");
    ...
    const syntheticMessage = { ...last.message: , text: combinedText };
    const prepared = await prepareSlackMessage({ ctx, account, message: syntheticMessage, opts: { ...last.opts: , wasMentioned: (combinedMentioned || last.opts.wasMentioned) } });
    if (!prepared) {
      return;
    }
    if ((entries.length > 1)) {
    ...
    ctx.runtime.error?.("slack inbound debounce flush failed: ");
  } });
