    ...
    try {
      {
        ...
        if (!ctx.isChannelAllowed({ channelId: item.channel, channelName: channelInfo?.name, channelType })) {
          return;
        }
        const channelLabel = resolveSlackChannelLabel({ channelId: item.channel, channelName: channelInfo?.name });
        ...
        const baseText = "Slack reaction : :: by  in  msg ";
        const text = authorLabel ? " from " : baseText;
        const sessionKey = ctx.resolveSlackSystemEventSessionKey({ channelId: item.channel, channelType });
        enqueueSystemEvent(text, { sessionKey, contextKey: "slack:reaction:::::" });
      }
    }
    catch (err) {
      {
        ctx.runtime.error?.(danger("slack reaction handler failed: "));
      }
    }
  };
