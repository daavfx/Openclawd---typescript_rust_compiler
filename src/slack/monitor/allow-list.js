...
export function resolveSlackAllowListMatch(params) {
  ...
  const candidates = [{ value: id, source: "id" }, { value: id ? "slack:" : undefined, source: "prefixed-id" }, { value: id ? "user:" : undefined, source: "prefixed-user" }, { value: name, source: "name" }, { value: name ? "slack:" : undefined, source: "prefixed-name" }, { value: slug, source: "slug" }];
...
export function resolveSlackUserAllowed(params) {
  ...
  return allowListMatches({ allowList, id: params.userId, name: params.userName });
}
