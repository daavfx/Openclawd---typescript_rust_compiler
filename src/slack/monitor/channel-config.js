...
export function shouldEmitSlackReactionNotification(params) {
  ...
  if ((effectiveMode === "allowlist")) {
    ...
    return allowListMatches({ allowList: users, id: userId, name: (userName ?? undefined) });
  }
  return true;
}
...
export function resolveSlackChannelConfig(params) {
  ...
  const candidates = buildChannelKeyCandidates(channelId, channelName ? "#" : undefined, directName, normalizedName);
  const match = resolveChannelEntryMatchWithFallback({ entries, keys: candidates, wildcardKey: "*" });
  ...
  const result = { allowed, requireMention, allowBots, users, skills, systemPrompt };
