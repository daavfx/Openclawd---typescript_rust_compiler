...
export function createSlackMonitorContext(params) {
  ...
  const resolveSlackSystemEventSessionKey = (p) => {
    ...
    const from = isDirectMessage ? "slack:" : isGroup ? "slack:group:" : "slack:channel:";
  ...
  const resolveChannelName = async (channelId) => {
    ...
    try {
      {
        const info = await params.app.client.conversations.info({ token: params.botToken, channel: channelId });
        const name = (info.channel && ("name" in info.channel)) ? info.channel.name : undefined;
        const channel = (info.channel ?? undefined);
        const type = channel?.is_im ? "im" : channel?.is_mpim ? "mpim" : channel?.is_channel ? "channel" : channel?.is_group ? "group" : undefined;
  ...
  const resolveUserName = async (userId) => {
    ...
    try {
      {
        const info = await params.app.client.users.info({ token: params.botToken, user: userId });
  ...
  const setSlackThreadStatus = async (p) => {
    ...
    const payload = { token: params.botToken, channel_id: p.channelId, thread_ts: p.threadTs, status: p.status };
    ...
    try {
      ...
      {
        logVerbose("slack status update failed for channel : ");
      }
    }
  };
  const isChannelAllowed = (p) => {
    ...
    if ((isGroupDm && (groupDmChannels.length > 0))) {
      ...
      const candidates = [p.channelId, p.channelName ? "#" : undefined, p.channelName, p.channelName ? normalizeSlackSlug(p.channelName) : undefined].filter((value) => Boolean(value)).map((value) => value.toLowerCase());
    ...
    if ((isRoom && p.channelId)) {
      const channelConfig = resolveSlackChannelConfig({ channelId: p.channelId, channelName: p.channelName, channels: params.channelsConfig, defaultRequireMention });
      ...
      if (!isSlackChannelAllowedByPolicy({ groupPolicy: params.groupPolicy, channelAllowlistConfigured, channelAllowed })) {
        logVerbose("slack: drop channel  (groupPolicy=, )");
        return false;
      }
      const hasExplicitConfig = Boolean(channelConfig?.matchSource);
      if ((!channelAllowed && ((params.groupPolicy !== "open") || hasExplicitConfig))) {
        logVerbose("slack: drop channel  ()");
        return false;
      }
      logVerbose("slack: allow channel  ()");
    }
    return true;
  };
