...
async function resolveThreadTsFromHistory(params) {
  try {
    {
      const response = await params.client.conversations.history({ channel: params.channelId, latest: params.messageTs, oldest: params.messageTs, inclusive: true, limit: 1 });
    ...
    {
      if (shouldLogVerbose()) {
        logVerbose("slack inbound: failed to resolve thread_ts via conversations.history for channel= ts=: ");
      }
      return undefined;
    }
  }
...
export function createSlackThreadTsResolver(params) {
  ...
  return { resolve: async (request) => {
    ...
    if (shouldLogVerbose()) {
      logVerbose("slack inbound: missing thread_ts for thread reply channel= ts= source=");
    }
    let pending = inflight.get(cacheKey);
    if (!pending) {
      pending = resolveThreadTsFromHistory({ client: params.client, channelId: message.channel, messageTs: message.ts });
    ...
    if (resolved) {
      if (shouldLogVerbose()) {
        logVerbose("slack inbound: resolved missing thread_ts channel= ts= -> thread_ts=");
      }
      return { ...message: , thread_ts: resolved };
    }
    if (shouldLogVerbose()) {
      logVerbose("slack inbound: could not resolve missing thread_ts channel= ts=");
    }
    return message;
  } };
}
