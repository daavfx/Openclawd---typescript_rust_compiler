...
const baseParams = () => { cfg: {  }, accountId: "default", botToken: "token", app: { client: {  } }, runtime: {  }, botUserId: "B1", teamId: "T1", apiAppId: "A1", historyLimit: 0, sessionScope: "per-sender", mainKey: "main", dmEnabled: true, dmPolicy: "open", allowFrom: [], groupDmEnabled: true, groupDmChannels: [], defaultRequireMention: true, groupPolicy: "open", useAccessGroups: false, reactionMode: "off", reactionAllowlist: [], replyToMode: "off", slashCommand: { enabled: false, name: "openclaw", sessionPrefix: "slack:slash", ephemeral: true }, textLimit: 4000, ackReactionScope: "group-mentions", mediaMaxBytes: 1, removeAckAfterReply: false };
...
describe("isChannelAllowed with groupPolicy and channelsConfig", () => {
  it("allows unlisted channels when groupPolicy is open even with channelsConfig entries", () => {
    const ctx = createSlackMonitorContext({ ...baseParams(): , groupPolicy: "open", channelsConfig: { C_LISTED: { requireMention: true } } });
  ...
  it("blocks unlisted channels when groupPolicy is allowlist", () => {
    const ctx = createSlackMonitorContext({ ...baseParams(): , groupPolicy: "allowlist", channelsConfig: { C_LISTED: { requireMention: true } } });
    expect(ctx.isChannelAllowed({ channelId: "C_LISTED", channelType: "channel" })).toBe(true);
  ...
  it("blocks explicitly denied channels even when groupPolicy is open", () => {
    const ctx = createSlackMonitorContext({ ...baseParams(): , groupPolicy: "open", channelsConfig: { C_ALLOWED: { allow: true }, C_DENIED: { allow: false } } });
  ...
  it("allows all channels when groupPolicy is open and channelsConfig is empty", () => {
    const ctx = createSlackMonitorContext({ ...baseParams(): , groupPolicy: "open", channelsConfig: undefined });
