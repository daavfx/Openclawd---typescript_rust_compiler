...
export async function monitorSlackProvider(opts = {  }) {
  ...
  let account = resolveSlackAccount({ cfg, accountId: opts.accountId });
  ...
  if ((!botToken || ((slackMode !== "http") && !appToken))) {
    const missing = (slackMode === "http") ? "Slack bot token missing for account \"\" (set channels.slack.accounts..botToken or SLACK_BOT_TOKEN for default)." : "Slack bot + app tokens missing for account \"\" (set channels.slack.accounts..botToken/appToken or SLACK_BOT_TOKEN/SLACK_APP_TOKEN for default).";
    throw new Error(missing);
  }
  if (((slackMode === "http") && !signingSecret)) {
    throw new Error("Slack signing secret missing for account \"\" (set channels.slack.signingSecret or channels.slack.accounts..signingSecret).");
  }
  ...
  if (((((slackCfg.groupPolicy === undefined) && (slackCfg.channels === undefined)) && (defaultGroupPolicy === undefined)) && (groupPolicy === "open"))) {
  ...
  const receiver = (slackMode === "http") ? new HTTPReceiver({ signingSecret: (signingSecret ?? ""), endpoints: slackWebhookPath }) : null;
  const clientOptions = resolveSlackWebClientOptions();
  const app = new App((slackMode === "socket") ? { token: botToken, appToken, socketMode: true, clientOptions } : { token: botToken, receiver: (receiver ?? undefined), clientOptions });
  ...
  if (((apiAppId && expectedApiAppIdFromAppToken) && (apiAppId !== expectedApiAppIdFromAppToken))) {
    runtime.error?.("slack token mismatch: bot token api_app_id= but app token looks like api_app_id=");
  }
  const ctx = createSlackMonitorContext({ cfg, accountId: account.accountId, botToken, app, runtime, botUserId, teamId, apiAppId, historyLimit, sessionScope, mainKey, dmEnabled, dmPolicy, allowFrom, groupDmEnabled, groupDmChannels, defaultRequireMention: slackCfg.requireMention, channelsConfig, groupPolicy, useAccessGroups, reactionMode, reactionAllowlist, replyToMode, threadHistoryScope, threadInheritParent, slashCommand, textLimit, ackReactionScope, mediaMaxBytes, removeAckAfterReply });
  ...
  if (((slackMode === "http") && slackHttpHandler)) {
    unregisterHttpHandler = registerSlackHttpHandler({ path: slackWebhookPath, handler: slackHttpHandler, log: runtime.log, accountId: account.accountId });
  }
  if (resolveToken) {
    void async () => {
      ...
      if ((channelsConfig && (Object.keys(channelsConfig).length > 0))) {
        try {
          ...
          {
            runtime.log?.("slack channel resolve failed; using config entries. ");
          }
        }
      }
      ...
      if ((allowEntries.length > 0)) {
        try {
          {
            const resolvedUsers = await resolveSlackUserAllowlist({ token: resolveToken, entries: allowEntries.map((entry) => String(entry)) });
            const mapping = [];
            const unresolved = [];
            const additions = [];
          ...
          {
            runtime.log?.("slack user resolve failed; using config entries. ");
          }
        }
      }
      if ((channelsConfig && (Object.keys(channelsConfig).length > 0))) {
        ...
        if ((userEntries.size > 0)) {
          try {
            {
              const resolvedUsers = await resolveSlackUserAllowlist({ token: resolveToken, entries: Array.from(userEntries) });
            ...
            {
              runtime.log?.("slack channel user resolve failed; using config entries. ");
            }
          }
        }
      }
  ...
  try {
    {
      if ((slackMode === "socket")) {
        ...
        runtime.log?.("slack http mode listening at ");
      }
      if (opts.abortSignal?.aborted) {
        return;
      }
