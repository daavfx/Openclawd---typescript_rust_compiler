...
async function listSlackChannels(client) {
  ...
  let cursor;
  do {
    const res = await client.conversations.list({ types: "public_channel,private_channel", exclude_archived: false, limit: 1000, cursor });
    for (const channel of (res.channels ?? [])) {
      ...
      channels.push({ id, name, archived: Boolean(channel.is_archived), isPrivate: Boolean(channel.is_private) });
    }
...
export async function resolveSlackChannelAllowlist(params) {
  ...
  for (const input of params.entries) {
    ...
    if (parsed.id) {
      ...
      results.push({ input, resolved: true, id: parsed.id, name: (match?.name ?? parsed.name), archived: match?.archived });
      continue;
    }
    if (parsed.name) {
      const match = resolveByName(parsed.name, channels);
      if (match) {
        results.push({ input, resolved: true, id: match.id, name: match.name, archived: match.archived });
        continue;
      }
    }
