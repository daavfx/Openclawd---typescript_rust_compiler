...
export function markdownToSlackMrkdwn(markdown, options = {  }) {
  const ir = markdownToIR((markdown ?? ""), { linkify: false, autolink: false, headingStyle: "bold", blockquotePrefix: "> ", tableMode: options.tableMode });
  return renderMarkdownWithMarkers(ir, { styleMarkers: { bold: { open: "*", close: "*" }, italic: { open: "_", close: "_" }, strikethrough: { open: "~", close: "~" }, code: { open: "`", close: "`" }, code_block: { open: "```
...
export function markdownToSlackMrkdwnChunks(markdown, limit, options = {  }) {
  ...
  return chunks.map((chunk) => renderMarkdownWithMarkers(chunk, { styleMarkers: { bold: { open: "*", close: "*" }, italic: { open: "_", close: "_" }, strikethrough: { open: "~", close: "~" }, code: { open: "`", close: "`" }, code_block: { open: "```
", close: "```" } }, escapeText: escapeSlackMrkdwnText, buildLink: buildSlackLink }));
}
