...
function resolveToken(explicit, accountId) {
  ...
  if (!token) {
    logVerbose("slack actions: missing bot token for account= explicit= source=");
...
export async function reactSlackMessage(channelId, messageId, emoji, opts = {  }) {
  ...
  await client.reactions.add({ channel: channelId, timestamp: messageId, name: normalizeEmoji(emoji) });
}

export async function removeSlackReaction(channelId, messageId, emoji, opts = {  }) {
  const client = await getClient(opts);
  await client.reactions.remove({ channel: channelId, timestamp: messageId, name: normalizeEmoji(emoji) });
}
...
export async function removeOwnSlackReactions(channelId, messageId, opts = {  }) {
  ...
  await Promise.all(Array.from(toRemove, (name) => client.reactions.remove({ channel: channelId, timestamp: messageId, name })));
  return Array.from(toRemove);
}

export async function listSlackReactions(channelId, messageId, opts = {  }) {
  const client = await getClient(opts);
  const result = await client.reactions.get({ channel: channelId, timestamp: messageId, full: true });
  const message = result.message;
  return (message?.reactions ?? []);
}

export async function sendSlackMessage(to, content, opts = {  }) {
  return await sendMessageSlack(to, content, { accountId: opts.accountId, token: opts.token, mediaUrl: opts.mediaUrl, client: opts.client, threadTs: opts.threadTs });
}

export async function editSlackMessage(channelId, messageId, content, opts = {  }) {
  const client = await getClient(opts);
  await client.chat.update({ channel: channelId, ts: messageId, text: content });
}
...
export async function readSlackMessages(channelId, opts = {  }) {
  const client = await getClient(opts);
  if (opts.threadId) {
    const result = await client.conversations.replies({ channel: channelId, ts: opts.threadId, limit: opts.limit, latest: opts.before, oldest: opts.after });
    return { messages: (result.messages ?? []).filter((message) => (message?.ts !== opts.threadId)), hasMore: Boolean(result.has_more) };
  }
  const result = await client.conversations.history({ channel: channelId, limit: opts.limit, latest: opts.before, oldest: opts.after });
