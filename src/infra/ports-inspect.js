...
async function runCommandSafe(argv, timeoutMs = 5000) {
  try {
    ...
    {
      return { stdout: "", stderr: "", code: 1, error: String(err) };
    }
  }
}
...
async function resolveWindowsImageName(pid) {
  const res = await runCommandSafe(["tasklist", "/FI", "PID eq ", "/FO", "LIST"]);
  if ((res.code !== 0)) {
    return undefined;
  }
...
async function resolveWindowsCommandLine(pid) {
  const res = await runCommandSafe(["wmic", "process", "where", "ProcessId=", "get", "CommandLine", "/value"]);
  if ((res.code !== 0)) {
    return undefined;
  }
  for (const rawLine of res.stdout.split(/\r?\n/)) {
...
export async function inspectPortUsage(port) {
  ...
  return { port, status, listeners, hints, detail: result.detail, errors: (errors.length > 0) ? errors : undefined };
}
