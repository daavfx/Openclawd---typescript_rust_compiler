...
function resolveMinutesInTimeZone(nowMs, timeZone) {
  try {
    {
      const parts = new Intl().DateTimeFormat("en-US", { timeZone, hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).formatToParts(new Date(nowMs));
      const map = {  };
...
export function resolveHeartbeatSummaryForAgent(cfg, agentId) {
  ...
  if (!enabled) {
    return { enabled: false, every: "disabled", everyMs: null, prompt: resolveHeartbeatPromptText(defaults?.prompt), target: (defaults?.target ?? DEFAULT_HEARTBEAT_TARGET), model: defaults?.model, ackMaxChars: Math.max(0, (defaults?.ackMaxChars ?? DEFAULT_HEARTBEAT_ACK_MAX_CHARS)) };
  }
  ...
  const ackMaxChars = Math.max(0, (((merged?.ackMaxChars ?? defaults?.ackMaxChars) ?? overrides?.ackMaxChars) ?? DEFAULT_HEARTBEAT_ACK_MAX_CHARS));
...
function resolveHeartbeatAgents(cfg) {
  ...
  if (hasExplicitHeartbeatAgents(cfg)) {
    return list.filter((entry) => entry?.heartbeat).map((entry) => {
...
function normalizeHeartbeatReply(payload, responsePrefix, ackMaxChars) {
  const stripped = stripHeartbeatToken(payload.text, { mode: "heartbeat", maxAckChars: ackMaxChars });
...
export async function runHeartbeatOnce(opts) {
  ...
  try {
    {
      ...
      if ((isHeartbeatContentEffectivelyEmpty(heartbeatFileContent) && !isExecEventReason)) {
        emitHeartbeatEvent({ status: "skipped", reason: "empty-heartbeat-file", durationMs: (Date.now() - startedAt) });
  ...
  const visibility = (delivery.channel !== "none") ? resolveHeartbeatVisibility({ cfg, channel: delivery.channel, accountId: delivery.accountId }) : { showOk: false, showAlerts: true, useIndicator: true };
  ...
  const ctx = { Body: prompt, From: sender, To: sender, Provider: hasExecCompletion ? "exec-event" : "heartbeat", SessionKey: sessionKey };
  if (((!visibility.showAlerts && !visibility.showOk) && !visibility.useIndicator)) {
    emitHeartbeatEvent({ status: "skipped", reason: "alerts-disabled", durationMs: (Date.now() - startedAt), channel: (delivery.channel !== "none") ? delivery.channel : undefined });
    return { status: "skipped", reason: "alerts-disabled" };
  }
  const heartbeatOkText = responsePrefix ? " " : HEARTBEAT_TOKEN;
  ...
  const maybeSendHeartbeatOk = async () => {
    ...
    if (heartbeatPlugin?.heartbeat?.checkReady) {
      const readiness = await heartbeatPlugin.heartbeat.checkReady({ cfg, accountId: delivery.accountId, deps: opts.deps });
      if (!readiness.ok) {
        return false;
      }
    }
    await deliverOutboundPayloads({ cfg, channel: delivery.channel, to: delivery.to, accountId: delivery.accountId, payloads: [{ text: heartbeatOkText }], deps: opts.deps });
    return true;
  };
  try {
    {
      ...
      const reasoningPayloads = includeReasoning ? resolveHeartbeatReasoningPayloads(replyResult).filter((payload) => (payload !== replyPayload)) : [];
      if ((!replyPayload || ((!replyPayload.text && !replyPayload.mediaUrl) && !replyPayload.mediaUrls?.length))) {
        ...
        emitHeartbeatEvent({ status: "ok-empty", reason: opts.reason, durationMs: (Date.now() - startedAt), channel: (delivery.channel !== "none") ? delivery.channel : undefined, silent: !okSent, indicatorType: visibility.useIndicator ? resolveIndicatorType("ok-empty") : undefined });
      ...
      if ((shouldSkipMain && (reasoningPayloads.length === 0))) {
        await restoreHeartbeatUpdatedAt({ storePath, sessionKey, updatedAt: previousUpdatedAt });
        const okSent = await maybeSendHeartbeatOk();
        emitHeartbeatEvent({ status: "ok-token", reason: opts.reason, durationMs: (Date.now() - startedAt), channel: (delivery.channel !== "none") ? delivery.channel : undefined, silent: !okSent, indicatorType: visibility.useIndicator ? resolveIndicatorType("ok-token") : undefined });
        return { status: "ran", durationMs: (Date.now() - startedAt) };
      }
      ...
      const isDuplicateMain = (((((!shouldSkipMain && !mediaUrls.length) && Boolean(prevHeartbeatText.trim())) && (normalized.text.trim() === prevHeartbeatText.trim())) && (typeof prevHeartbeatAt === "number")) && ((startedAt - prevHeartbeatAt) < (((24 * 60) * 60) * 1000)));
      if (isDuplicateMain) {
        await restoreHeartbeatUpdatedAt({ storePath, sessionKey, updatedAt: previousUpdatedAt });
        emitHeartbeatEvent({ status: "skipped", reason: "duplicate", preview: normalized.text.slice(0, 200), durationMs: (Date.now() - startedAt), hasMedia: false, channel: (delivery.channel !== "none") ? delivery.channel : undefined });
        return { status: "ran", durationMs: (Date.now() - startedAt) };
      }
      const previewText = shouldSkipMain ? reasoningPayloads.map((payload) => payload.text).filter((text) => Boolean(text?.trim())).join("
") : normalized.text;
      if (((delivery.channel === "none") || !delivery.to)) {
        emitHeartbeatEvent({ status: "skipped", reason: (delivery.reason ?? "no-target"), preview: previewText?.slice(0, 200), durationMs: (Date.now() - startedAt), hasMedia: (mediaUrls.length > 0) });
        return { status: "ran", durationMs: (Date.now() - startedAt) };
      }
      if (!visibility.showAlerts) {
        await restoreHeartbeatUpdatedAt({ storePath, sessionKey, updatedAt: previousUpdatedAt });
        emitHeartbeatEvent({ status: "skipped", reason: "alerts-disabled", preview: previewText?.slice(0, 200), durationMs: (Date.now() - startedAt), channel: delivery.channel, hasMedia: (mediaUrls.length > 0), indicatorType: visibility.useIndicator ? resolveIndicatorType("sent") : undefined });
        return { status: "ran", durationMs: (Date.now() - startedAt) };
      }
      const deliveryAccountId = delivery.accountId;
      const heartbeatPlugin = getChannelPlugin(delivery.channel);
      if (heartbeatPlugin?.heartbeat?.checkReady) {
        const readiness = await heartbeatPlugin.heartbeat.checkReady({ cfg, accountId: deliveryAccountId, deps: opts.deps });
        if (!readiness.ok) {
          emitHeartbeatEvent({ status: "skipped", reason: readiness.reason, preview: previewText?.slice(0, 200), durationMs: (Date.now() - startedAt), hasMedia: (mediaUrls.length > 0), channel: delivery.channel });
          log.info("heartbeat: channel not ready", { channel: delivery.channel, reason: readiness.reason });
          return { status: "skipped", reason: readiness.reason };
        }
      }
      await deliverOutboundPayloads({ cfg, channel: delivery.channel, to: delivery.to, accountId: deliveryAccountId, payloads: [...reasoningPayloads, ...shouldSkipMain ? [] : [{ text: normalized.text, mediaUrls }]], deps: opts.deps });
      if ((!shouldSkipMain && normalized.text.trim())) {
        const store = loadSessionStore(storePath);
        const current = store[sessionKey];
        if (current) {
          store[sessionKey] = { ...current: , lastHeartbeatText: normalized.text, lastHeartbeatSentAt: startedAt };
          await saveSessionStore(storePath, store);
        }
      }
      emitHeartbeatEvent({ status: "sent", to: delivery.to, preview: previewText?.slice(0, 200), durationMs: (Date.now() - startedAt), hasMedia: (mediaUrls.length > 0), channel: delivery.channel, indicatorType: visibility.useIndicator ? resolveIndicatorType("sent") : undefined });
      return { status: "ran", durationMs: (Date.now() - startedAt) };
    }
  }
  catch (err) {
    {
      ...
      emitHeartbeatEvent({ status: "failed", reason, durationMs: (Date.now() - startedAt), channel: (delivery.channel !== "none") ? delivery.channel : undefined, indicatorType: visibility.useIndicator ? resolveIndicatorType("failed") : undefined });
...
export function startHeartbeatRunner(opts) {
  ...
  const state = { cfg: (opts.cfg ?? loadConfig()), runtime, agents: new Map(), timer: null, stopped: false };
  ...
  const updateConfig = (cfg) => {
    ...
    for (const agent of resolveHeartbeatAgents(cfg)) {
      ...
      nextAgents.set(agent.agentId, { agentId: agent.agentId, heartbeat: agent.heartbeat, intervalMs, lastRunMs: prevState?.lastRunMs, nextDueMs });
    }
    state.cfg = cfg;
  ...
  const run = async (params) => {
    ...
    for (const agent of state.agents.values()) {
      ...
      const res = await runOnce({ cfg: state.cfg, agentId: agent.agentId, heartbeat: agent.heartbeat, reason, deps: { runtime: state.runtime } });
