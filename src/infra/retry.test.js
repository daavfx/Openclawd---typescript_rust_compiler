...
describe("retryAsync", () => {
  ...
  it("calls onRetry before retrying", async () => {
    ...
    const onRetry = vi.fn();
    const res = await retryAsync(fn, { attempts: 2, minDelayMs: 0, maxDelayMs: 0, onRetry });
    expect(res).toBe("ok");
  ...
  it("uses retryAfterMs when provided", async () => {
    ...
    const fn = vi.fn().mockRejectedValueOnce(new Error("boom")).mockResolvedValueOnce("ok");
    const delays = [];
    const promise = retryAsync(fn, { attempts: 2, minDelayMs: 0, maxDelayMs: 1000, jitter: 0, retryAfterMs: () => 500, onRetry: (info) => delays.push(info.delayMs) });
  ...
  it("clamps retryAfterMs to maxDelayMs", async () => {
    ...
    const fn = vi.fn().mockRejectedValueOnce(new Error("boom")).mockResolvedValueOnce("ok");
    const delays = [];
    const promise = retryAsync(fn, { attempts: 2, minDelayMs: 0, maxDelayMs: 100, jitter: 0, retryAfterMs: () => 500, onRetry: (info) => delays.push(info.delayMs) });
    await vi.runAllTimersAsync();
    await expect(promise).resolves.toBe("ok");
