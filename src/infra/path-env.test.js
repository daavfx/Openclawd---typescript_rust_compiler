...
", "utf-8");
        ...
        try {
          {
            ensureOpenClawCliOnPath({ execPath: cliPath, cwd: tmp, homeDir: tmp, platform: "darwin" });
  ...
  it("is idempotent", () => {
    ...
    try {
      {
        ensureOpenClawCliOnPath({ execPath: "/tmp/does-not-matter", cwd: "/tmp", homeDir: "/tmp", platform: "darwin" });
...
", "utf-8");
        ...
        process.env.PATH = "/usr/bin";
        delete process.env.OPENCLAW_PATH_BOOTSTRAPPED;
        ensureOpenClawCliOnPath({ execPath: appCli, cwd: tmp, homeDir: tmp, platform: "darwin" });
        const updated = (process.env.PATH ?? "");
    ...
    try {
      {
        ...
        ensureOpenClawCliOnPath({ execPath: path.join(execDir, "node"), cwd: tmp, homeDir: tmp, platform: "linux" });
        const updated = (process.env.PATH ?? "");
        const parts = updated.split(path.delimiter);
