...
export async function findTailscaleBinary() {
  const checkBinary = async (path) => {
    ...
    try {
      {
        await Promise.race([runExec(path, ["--version"], { timeoutMs: 3000 }), new Promise((_, reject) => setTimeout(() => reject(new Error("timeout")), 3000))]);
        return true;
      }
    }
    catch {
  ...
  try {
    {
      const {stdout} = await runExec("find", ["/Applications", "-maxdepth", "3", "-name", "Tailscale", "-path", "*/Tailscale.app/Contents/MacOS/Tailscale"], { timeoutMs: 5000 });
...
export async function getTailnetHostname(exec = runExec, detectedBinary) {
  ...
  for (const candidate of candidates) {
    ...
    try {
      {
        const {stdout} = await exec(candidate, ["status", "--json"], { timeoutMs: 5000, maxBuffer: 400000 });
        const parsed = stdout ? parsePossiblyNoisyJsonObject(stdout) : {  };
        const self = ((typeof parsed.Self === "object") && (parsed.Self !== null)) ? parsed.Self : undefined;
...
export async function readTailscaleStatusJson(exec = runExec, opts) {
  ...
  const {stdout} = await exec(tailscaleBin, ["status", "--json"], { timeoutMs: (opts?.timeoutMs ?? 5000), maxBuffer: 400000 });
...
async function execWithSudoFallback(exec, bin, args, opts) {
  try {
  ...
  catch (err) {
    {
      ...
      logVerbose("Command failed, retrying with sudo:  ");
      try {
        {
  ...
  try {
    {
      ...
      logVerbose("Enabling funnel on port …");
      const {stdout} = await execWithSudoFallback(exec, tailscaleBin, ["funnel", "--yes", "--bg", ""], { maxBuffer: 200000, timeoutMs: 15000 });
      if (stdout.trim()) {
  ...
  catch (err) {
    {
      ...
      if (stdout.includes("Funnel is not enabled")) {
        ...
        if (linkMatch) {
          console.error(info("Enable it here: "));
        } else {
      ...
      runtime.error(info("Tip: Funnel is optional for OpenClaw. You can keep running the web gateway without it: ``"));
      if (shouldLogVerbose()) {
        const rich = isRich();
        if (stdout.trim()) {
          runtime.error(colorize(rich, theme.muted, "stdout: "));
        }
        if (stderr.trim()) {
          runtime.error(colorize(rich, theme.muted, "stderr: "));
        }
        runtime.error(err);
      }
      runtime.exit(1);
...
export async function enableTailscaleServe(port, exec = runExec) {
  const tailscaleBin = await getTailscaleBinary();
  await execWithSudoFallback(exec, tailscaleBin, ["serve", "--bg", "--yes", ""], { maxBuffer: 200000, timeoutMs: 15000 });
}

export async function disableTailscaleServe(exec = runExec) {
  const tailscaleBin = await getTailscaleBinary();
  await execWithSudoFallback(exec, tailscaleBin, ["serve", "reset"], { maxBuffer: 200000, timeoutMs: 15000 });
}

export async function enableTailscaleFunnel(port, exec = runExec) {
  const tailscaleBin = await getTailscaleBinary();
  await execWithSudoFallback(exec, tailscaleBin, ["funnel", "--bg", "--yes", ""], { maxBuffer: 200000, timeoutMs: 15000 });
}

export async function disableTailscaleFunnel(exec = runExec) {
  const tailscaleBin = await getTailscaleBinary();
  await execWithSudoFallback(exec, tailscaleBin, ["funnel", "reset"], { maxBuffer: 200000, timeoutMs: 15000 });
}
...
function parseWhoisIdentity(payload) {
  ...
  const name = ((((getString(userProfile?.DisplayName) ?? getString(userProfile?.Name)) ?? getString(userProfile?.displayName)) ?? getString(payload.DisplayName)) ?? getString(payload.name));
  return { login, name };
}
...
export async function readTailscaleWhoisIdentity(ip, exec = runExec, opts) {
  ...
  try {
    {
      const tailscaleBin = await getTailscaleBinary();
      const {stdout} = await exec(tailscaleBin, ["whois", "--json", normalized], { timeoutMs: (opts?.timeoutMs ?? 5000), maxBuffer: 200000 });
      const parsed = stdout ? parsePossiblyNoisyJsonObject(stdout) : {  };
