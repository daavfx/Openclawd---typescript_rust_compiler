...
vi.mock("node:child_process", () => {
  const spawn = vi.fn(() => {
    ...
    child.kill = vi.fn();
    process.nextTick(() => {
      stdout.emit("data", ["user steipete", "hostname peters-mac-studio-1.sheep-coho.ts.net", "port 2222", "identityfile none", "identityfile /tmp/id_ed25519", ""].join("
"));
      child.emit("exit", 0);
    });
    return child;
...
describe("ssh-config", () => {
  it("parses ssh -G output", async () => {
    ...
    const parsed = parseSshConfigOutput("user bob
