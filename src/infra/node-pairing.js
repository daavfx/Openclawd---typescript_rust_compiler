...
async function loadState(baseDir) {
  ...
  const state = { pendingById: (pending ?? {  }), pairedByNodeId: (paired ?? {  }) };
...
export async function updatePairedNodeMetadata(nodeId, patch, baseDir) {
  await withLock(async () => {
    ...
    const next = { ...existing: , displayName: (patch.displayName ?? existing.displayName), platform: (patch.platform ?? existing.platform), version: (patch.version ?? existing.version), coreVersion: (patch.coreVersion ?? existing.coreVersion), uiVersion: (patch.uiVersion ?? existing.uiVersion), deviceFamily: (patch.deviceFamily ?? existing.deviceFamily), modelIdentifier: (patch.modelIdentifier ?? existing.modelIdentifier), remoteIp: (patch.remoteIp ?? existing.remoteIp), caps: (patch.caps ?? existing.caps), commands: (patch.commands ?? existing.commands), bins: (patch.bins ?? existing.bins), permissions: (patch.permissions ?? existing.permissions), lastConnectedAtMs: (patch.lastConnectedAtMs ?? existing.lastConnectedAtMs) };
