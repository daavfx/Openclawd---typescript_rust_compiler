...
function resolvePaths(baseDir) {
  ...
  return { dir, pendingPath: path.join(dir, "pending.json"), pairedPath: path.join(dir, "paired.json") };
}
...
async function loadState(baseDir) {
  ...
  const state = { pendingById: (pending ?? {  }), pairedByDeviceId: (paired ?? {  }) };
...
export async function requestDevicePairing(req, baseDir) {
  return await withLock(async () => {
    ...
    const request = { requestId: randomUUID(), deviceId, publicKey: req.publicKey, displayName: req.displayName, platform: req.platform, clientId: req.clientId, clientMode: req.clientMode, role: req.role, roles: req.role ? [req.role] : undefined, scopes: req.scopes, remoteIp: req.remoteIp, silent: req.silent, isRepair, ts: Date.now() };
...
export async function approveDevicePairing(requestId, baseDir) {
  return await withLock(async () => {
    ...
    if (roleForToken) {
      ...
      const now = Date.now();
      tokens[roleForToken] = { token: newToken(), role: roleForToken, scopes: nextScopes, createdAtMs: (existingToken?.createdAtMs ?? now), rotatedAtMs: existingToken ? now : undefined, revokedAtMs: undefined, lastUsedAtMs: existingToken?.lastUsedAtMs };
    }
    const device = { deviceId: pending.deviceId, publicKey: pending.publicKey, displayName: pending.displayName, platform: pending.platform, clientId: pending.clientId, clientMode: pending.clientMode, role: pending.role, roles, scopes, remoteIp: pending.remoteIp, tokens, createdAtMs: (existing?.createdAtMs ?? now), approvedAtMs: now };
...
export async function updatePairedDeviceMetadata(deviceId, patch, baseDir) {
  return await withLock(async () => {
    ...
    state.pairedByDeviceId[deviceId] = { ...existing: , ...patch: , deviceId: existing.deviceId, createdAtMs: existing.createdAtMs, approvedAtMs: existing.approvedAtMs, role: (patch.role ?? existing.role), roles, scopes };
    await persistState(state, baseDir);
  });
}
...
export function summarizeDeviceTokens(tokens) {
  ...
  const summaries = Object.values(tokens).map((token) => { role: token.role, scopes: token.scopes, createdAtMs: token.createdAtMs, rotatedAtMs: token.rotatedAtMs, revokedAtMs: token.revokedAtMs, lastUsedAtMs: token.lastUsedAtMs }).sort((a, b) => a.role.localeCompare(b.role));
...
export async function ensureDeviceToken(params) {
  return await withLock(async () => {
    ...
    const now = Date.now();
    const next = { token: newToken(), role, scopes: requestedScopes, createdAtMs: (existing?.createdAtMs ?? now), rotatedAtMs: existing ? now : undefined, revokedAtMs: undefined, lastUsedAtMs: existing?.lastUsedAtMs };
    tokens[role] = next;
...
export async function rotateDeviceToken(params) {
  return await withLock(async () => {
    ...
    const now = Date.now();
    const next = { token: newToken(), role, scopes: requestedScopes, createdAtMs: (existing?.createdAtMs ?? now), rotatedAtMs: now, revokedAtMs: undefined, lastUsedAtMs: existing?.lastUsedAtMs };
    tokens[role] = next;
    device.tokens = tokens;
