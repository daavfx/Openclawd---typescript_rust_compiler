...
const mocks = vi.hoisted(() => { createService: vi.fn(), shutdown: vi.fn(), registerUnhandledRejectionHandler: vi.fn(), logWarn: vi.fn(), logDebug: vi.fn() });
...
vi.mock("../logger.js", async () => {
  ...
  return { ...actual: , logWarn: (message) => logWarn(message), logDebug: (message) => logDebug(message), logInfo: vi.fn(), logError: vi.fn(), logSuccess: vi.fn() };
});
vi.mock("@homebridge/ciao", () => {
  return { Protocol: { TCP: "tcp" }, getResponder: () => { createService, shutdown } };
});
vi.mock("./unhandled-rejections.js", () => {
  return { registerUnhandledRejectionHandler: (handler) => registerUnhandledRejectionHandler(handler) };
});
const {startGatewayBonjourAdvertiser} = await import("./bonjour.js");
describe("gateway bonjour advertiser", () => {
  const prevEnv = { ...process.env:  };
  beforeEach(() => {
    vi.spyOn(logging, "getLogger").mockReturnValue({ info: (...args) => getLoggerInfo(...args) });
  });
  afterEach(() => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    const advertise = vi.fn().mockImplementation(async () => await new Promise((resolve) => {
      setTimeout(resolve, 250);
    }));
    createService.mockImplementation((options) => {
      return { advertise, destroy, serviceState: "announced", on: vi.fn(), getFQDN: () => "..", getHostname: () => asString(options.hostname, "unknown"), getPort: () => Number((options.port ?? -1)) };
    });
    const started = await startGatewayBonjourAdvertiser({ gatewayPort: 18789, sshPort: 2222, tailnetDns: "host.tailnet.ts.net", cliPath: "/opt/homebrew/bin/openclaw" });
  ...
  it("omits cliPath and sshPort in minimal mode", async () => {
    ...
    const started = await startGatewayBonjourAdvertiser({ gatewayPort: 18789, sshPort: 2222, cliPath: "/opt/homebrew/bin/openclaw", minimal: true });
    const [gatewayCall] = createService.mock.calls;
  ...
  it("attaches conflict listeners for services", async () => {
    ...
    createService.mockImplementation((options) => {
      ...
      return { advertise, destroy, serviceState: "announced", on, getFQDN: () => "..", getHostname: () => asString(options.hostname, "unknown"), getPort: () => Number((options.port ?? -1)) };
    });
    const started = await startGatewayBonjourAdvertiser({ gatewayPort: 18789, sshPort: 2222 });
  ...
  it("logs advertise failures and retries via watchdog", async () => {
    ...
    createService.mockImplementation((options) => {
      return { advertise, destroy, serviceState: "unannounced", on: vi.fn(), getFQDN: () => "..", getHostname: () => asString(options.hostname, "unknown"), getPort: () => Number((options.port ?? -1)) };
    });
    const started = await startGatewayBonjourAdvertiser({ gatewayPort: 18789, sshPort: 2222 });
    expect(advertise).toHaveBeenCalledTimes(1);
