...
export function createPinnedLookup(params) {
  ...
  const records = params.addresses.map((address) => { address, family: address.includes(":") ? 6 : 4 });
  let index = 0;
...
export async function resolvePinnedHostname(hostname, lookupFn = dnsLookup) {
  ...
  return { hostname: normalized, addresses, lookup: createPinnedLookup({ hostname: normalized, addresses }) };
}
