...
export async function loadProviderUsageSummary(opts = {  }) {
  ...
  const auths = await resolveProviderAuths({ providers: (opts.providers ?? usageProviders), auth: opts.auth, agentDir: opts.agentDir });
  if ((auths.length === 0)) {
  ...
  const tasks = auths.map((auth) => withTimeout(async () => {
    switch (auth.provider) {
      ...
      case "xiaomi":
        return { provider: "xiaomi", displayName: PROVIDER_LABELS.xiaomi, windows: [] };
      case "zai":
        return await fetchZaiUsage(auth.token, timeoutMs, fetchFn);
      default:
        return { provider: auth.provider, displayName: PROVIDER_LABELS[auth.provider], windows: [], error: "Unsupported provider" };
    }
  }(), (timeoutMs + 1000), { provider: auth.provider, displayName: PROVIDER_LABELS[auth.provider], windows: [], error: "Timeout" }));
