...
async function discoverViaDnsSd(domain, timeoutMs, run) {
  const browse = await run(["dns-sd", "-B", GATEWAY_SERVICE_TYPE, domain], { timeoutMs });
  const instances = parseDnsSdBrowse(browse.stdout);
  const results = [];
  for (const instance of instances) {
    const resolved = await run(["dns-sd", "-L", instance, GATEWAY_SERVICE_TYPE, domain], { timeoutMs });
...
async function discoverWideAreaViaTailnetDns(domain, timeoutMs, run) {
  ...
  for (const candidate of tailscaleCandidates) {
    try {
      {
        const res = await run([candidate, "status", "--json"], { timeoutMs: Math.max(1, Math.min(700, remainingMs())) });
  ...
  const worker = async () => {
    while ((nameserver === null)) {
      ...
      try {
        {
          const probe = await run(["dig", "+short", "+time=1", "+tries=1", "@", probeName, "PTR"], { timeoutMs: Math.max(1, Math.min(250, budget)) });
  ...
  for (const ptr of ptrs) {
    ...
    const srv = await run(["dig", "+short", "+time=1", "+tries=1", nameserverArg, ptrName, "SRV"], { timeoutMs: Math.max(1, Math.min(350, budget)) }).catch(() => null);
    ...
    if ((txtBudget <= 0)) {
      results.push({ instanceName: (instanceName || ptrName), displayName: (instanceName || ptrName), domain, host: srvParsed.host, port: srvParsed.port });
      continue;
    }
    const txt = await run(["dig", "+short", "+time=1", "+tries=1", nameserverArg, ptrName, "TXT"], { timeoutMs: Math.max(1, Math.min(350, txtBudget)) }).catch(() => null);
    const txtTokens = txt ? parseDigTxt(txt.stdout) : [];
    const txtMap = (txtTokens.length > 0) ? parseTxtTokens(txtTokens) : {  };
    const beacon = { instanceName: (instanceName || ptrName), displayName: ((txtMap.displayName || instanceName) || ptrName), domain, host: srvParsed.host, port: srvParsed.port, txt: Object.keys(txtMap).length ? txtMap : undefined, gatewayPort: parseIntOrNull(txtMap.gatewayPort), sshPort: parseIntOrNull(txtMap.sshPort), tailnetDns: (txtMap.tailnetDns || undefined), cliPath: (txtMap.cliPath || undefined) };
...
")) {
    ...
    if ((line.startsWith("=") && line.includes(GATEWAY_SERVICE_TYPE))) {
      ...
      current = { instanceName, displayName: instanceName };
      continue;
    }
    if (!current) {
      continue;
...
async function discoverViaAvahi(domain, timeoutMs, run) {
  ...
  return parseAvahiBrowse(browse.stdout).map((beacon) => { ...beacon: , domain });
}
export async function discoverGatewayBeacons(opts = {  }) {
  ...
  const domains = (domainsRaw.length > 0) ? domainsRaw : defaultDomains.map((d) => String(d).trim()).filter(Boolean).map((d) => d.endsWith(".") ? d : ".");
  try {
    {
      if ((platform === "darwin")) {
