...
function canonicalizeSessionKeyForAgent(params) {
  ...
  const canonicalMain = canonicalizeMainSessionAlias({ cfg: { session: { scope: params.scope, mainKey: params.mainKey } }, agentId, sessionKey: raw });
  ...
  if (raw.toLowerCase().startsWith("subagent:")) {
    ...
    return "agent::subagent:".toLowerCase();
  }
  if (raw.startsWith("group:")) {
    ...
    return "agent:::group:".toLowerCase();
  }
  if ((!raw.includes(":") && raw.toLowerCase().includes("@g.us"))) {
    return "agent::whatsapp:group:".toLowerCase();
  }
...
function canonicalizeSessionStore(params) {
  ...
  for (const [key, entry] of Object.entries(params.store)) {
    ...
    const canonicalKey = canonicalizeSessionKeyForAgent({ key, agentId: params.agentId, mainKey: params.mainKey, scope: params.scope });
...
function listLegacySessionKeys(params) {
  ...
  for (const key of Object.keys(params.store)) {
    const canonical = canonicalizeSessionKeyForAgent({ key, agentId: params.agentId, mainKey: params.mainKey, scope: params.scope });
    if ((canonical !== key)) {
      legacy.push(key);
    }
  }
...
function formatStateDirMigration(legacyDir, targetDir) {
  return "State dir:  →  (legacy path now symlinked)";
}
...
export async function autoMigrateLegacyStateDir(params) {
  ...
  if ((!legacyStat.isDirectory() && !legacyStat.isSymbolicLink())) {
    warnings.push("Legacy state path is not a directory: ");
    return { migrated: false, skipped: false, changes, warnings };
  }
  let symlinkDepth = 0;
  while (legacyStat.isSymbolicLink()) {
    ...
    if (!legacyTarget) {
      warnings.push("Legacy state dir is a symlink (); could not resolve target.");
      return { migrated: false, skipped: false, changes, warnings };
    }
    ...
    if (legacyDirs.some((dir) => (path.resolve(dir) === path.resolve(legacyTarget)))) {
      ...
      if (!legacyStat) {
        warnings.push("Legacy state dir missing after symlink resolution: ");
        return { migrated: false, skipped: false, changes, warnings };
      }
      if ((!legacyStat.isDirectory() && !legacyStat.isSymbolicLink())) {
        warnings.push("Legacy state path is not a directory: ");
      ...
      symlinkDepth += 1;
      if ((symlinkDepth > 2)) {
        warnings.push("Legacy state dir symlink chain too deep: ");
        return { migrated: false, skipped: false, changes, warnings };
      }
      continue;
    }
    warnings.push("Legacy state dir is a symlink ( → ); skipping auto-migration.");
    return { migrated: false, skipped: false, changes, warnings };
  }
  if (isDirPath(targetDir)) {
    warnings.push("State dir migration skipped: target already exists (). Remove or merge manually.");
    return { migrated: false, skipped: false, changes, warnings };
  }
  try {
    {
    ...
    {
      warnings.push("Failed to move legacy state dir ( → ): ");
      return { migrated: false, skipped: false, changes, warnings };
    }
  }
  try {
          ...
          try {
            {
              ...
              warnings.push("State dir migration rolled back (failed to link legacy path): ");
              return { migrated: false, skipped: false, changes: [], warnings };
            }
          }
          catch (rollbackErr) {
            {
              warnings.push("State dir moved but failed to link legacy path ( → ): ");
              warnings.push("Rollback failed; set OPENCLAW_STATE_DIR= to avoid split state: ");
              changes.push("State dir:  → ");
            }
          }
        }
      }
...
export async function detectLegacyStateMigrations(params) {
  ...
  const targetSessionParsed = fileExists(sessionsTargetStorePath) ? readSessionStoreJson5(sessionsTargetStorePath) : { store: {  }, ok: true };
  const legacyKeys = targetSessionParsed.ok ? listLegacySessionKeys({ store: targetSessionParsed.store, agentId: targetAgentId, mainKey: targetMainKey, scope: targetScope }) : [];
  ...
  if (hasLegacySessions) {
    preview.push("- Sessions:  → ");
  }
  if ((legacyKeys.length > 0)) {
    preview.push("- Sessions: canonicalize legacy keys in ");
  }
  if (hasLegacyAgentDir) {
    preview.push("- Agent dir:  → ");
  }
  if (hasLegacyWhatsAppAuth) {
    preview.push("- WhatsApp auth:  →  (keep oauth.json)");
  }
  return { targetAgentId, targetMainKey, targetScope, stateDir, oauthDir, sessions: { legacyDir: sessionsLegacyDir, legacyStorePath: sessionsLegacyStorePath, targetDir: sessionsTargetDir, targetStorePath: sessionsTargetStorePath, hasLegacy: (hasLegacySessions || (legacyKeys.length > 0)), legacyKeys }, agentDir: { legacyDir: legacyAgentDir, targetDir: targetAgentDir, hasLegacy: hasLegacyAgentDir }, whatsappAuth: { legacyDir: oauthDir, targetDir: targetWhatsAppAuthDir, hasLegacy: hasLegacyWhatsAppAuth }, preview };
}
...
async function migrateLegacySessions(detected, now) {
  ...
  const legacyParsed = fileExists(detected.sessions.legacyStorePath) ? readSessionStoreJson5(detected.sessions.legacyStorePath) : { store: {  }, ok: true };
  const targetParsed = fileExists(detected.sessions.targetStorePath) ? readSessionStoreJson5(detected.sessions.targetStorePath) : { store: {  }, ok: true };
  const legacyStore = legacyParsed.store;
  const targetStore = targetParsed.store;
  const canonicalizedTarget = canonicalizeSessionStore({ store: targetStore, agentId: detected.targetAgentId, mainKey: detected.targetMainKey, scope: detected.targetScope });
  const canonicalizedLegacy = canonicalizeSessionStore({ store: legacyStore, agentId: detected.targetAgentId, mainKey: detected.targetMainKey, scope: detected.targetScope });
  const merged = { ...canonicalizedTarget.store:  };
  for (const [key, entry] of Object.entries(canonicalizedLegacy.store)) {
    merged[key] = mergeSessionEntry({ existing: merged[key], incoming: entry, preferIncomingOnTie: false });
  }
  const mainKey = buildAgentMainSessionKey({ agentId: detected.targetAgentId, mainKey: detected.targetMainKey });
  if (!merged[mainKey]) {
    const latest = pickLatestLegacyDirectEntry(legacyStore);
    if (latest?.sessionId) {
      merged[mainKey] = latest;
      changes.push("Migrated latest direct-chat session → ");
    }
  }
  if (!legacyParsed.ok) {
    warnings.push("Legacy sessions store unreadable; left in place at ");
  }
  if (((legacyParsed.ok || targetParsed.ok) && ((Object.keys(legacyStore).length > 0) || (Object.keys(targetStore).length > 0)))) {
    ...
    if ((canonicalizedTarget.legacyKeys.length > 0)) {
      changes.push("Canonicalized  legacy session key(s)");
    }
  }
    ...
    try {
      {
        ...
        changes.push("Moved  → agents//sessions");
      }
    }
    catch (err) {
      {
        warnings.push("Failed moving : ");
      }
    }
  }
  if (legacyParsed.ok) {
  ...
  if ((legacyLeft.length > 0)) {
    const backupDir = ".legacy-";
    try {
      {
        fs.renameSync(detected.sessions.legacyDir, backupDir);
        warnings.push("Left legacy sessions at ");
      }
    }
    catch {
      {
...
export async function migrateLegacyAgentDir(detected, now) {
  ...
  for (const entry of entries) {
    ...
    try {
      {
        fs.renameSync(from, to);
        changes.push("Moved agent file  → agents//agent");
      }
    }
    catch (err) {
      {
  ...
  if (!emptyDirOrMissing(detected.agentDir.legacyDir)) {
    const backupDir = path.join(detected.stateDir, "agents", detected.targetAgentId, "agent.legacy-");
    try {
      {
        fs.renameSync(detected.agentDir.legacyDir, backupDir);
        warnings.push("Left legacy agent dir at ");
      }
    }
    catch (err) {
      {
        warnings.push("Failed relocating legacy agent dir: ");
      }
    }
  }
  return { changes, warnings };
...
async function migrateLegacyWhatsAppAuth(detected) {
  ...
  for (const entry of entries) {
    ...
    try {
      {
        fs.renameSync(from, to);
        changes.push("Moved WhatsApp auth  → whatsapp/default");
      }
    }
    catch (err) {
      {
...
export async function runLegacyStateMigrations(params) {
  ...
  return { changes: [...sessions.changes, ...agentDir.changes, ...whatsappAuth.changes], warnings: [...sessions.warnings, ...agentDir.warnings, ...whatsappAuth.warnings] };
}
...
export async function autoMigrateLegacyState(params) {
  ...
  const env = (params.env ?? process.env);
  const stateDirResult = await autoMigrateLegacyStateDir({ env, homedir: params.homedir, log: params.log });
  if ((env.OPENCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim())) {
    return { migrated: stateDirResult.migrated, skipped: true, changes: stateDirResult.changes, warnings: stateDirResult.warnings };
  }
  const detected = await detectLegacyStateMigrations({ cfg: params.cfg, env, homedir: params.homedir });
  if ((!detected.sessions.hasLegacy && !detected.agentDir.hasLegacy)) {
    return { migrated: stateDirResult.migrated, skipped: false, changes: stateDirResult.changes, warnings: stateDirResult.warnings };
  }
  const now = (params.now ?? () => Date.now());
  const sessions = await migrateLegacySessions(detected, now);
  const agentDir = await migrateLegacyAgentDir(detected, now);
