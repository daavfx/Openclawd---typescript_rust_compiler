...
import { assertSupportedRuntime, detectRuntime, isAtLeast, parseSemver, runtimeSatisfies } from "./runtime-guard.js";
describe("runtime-guard", () => {
  ...
  it("validates runtime thresholds", () => {
    const nodeOk = { kind: "node", version: "22.0.0", execPath: "/usr/bin/node", pathEnv: "/usr/bin" };
    const nodeOld = { ...nodeOk: , version: "21.9.0" };
    const unknown = { kind: "unknown", version: null, execPath: null, pathEnv: "/usr/bin" };
  ...
  it("throws via exit when runtime is too old", () => {
    ...
    const details = { kind: "node", version: "20.0.0", execPath: "/usr/bin/node", pathEnv: "/usr/bin" };
  ...
  it("returns silently when runtime meets requirements", () => {
    const runtime = { log: vi.fn(), error: vi.fn(), exit: vi.fn() };
    const details = { ...detectRuntime(): , kind: "node", version: "22.0.0", execPath: "/usr/bin/node" };
