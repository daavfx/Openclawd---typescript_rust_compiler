...
async function waitForLocalListener(port, timeoutMs) {
  ...
  throw new Error("ssh tunnel did not start listening on localhost:");
}
export async function startSshPortForward(opts) {
  ...
  const userHost = parsed.user ? "@" : parsed.host;
  const args = ["-N", "-L", ":127.0.0.1:", "-p", String(parsed.port), "-o", "ExitOnForwardFailure=yes", "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=accept-new", "-o", "UpdateHostKeys=yes", "-o", "ConnectTimeout=5", "-o", "ServerAliveInterval=15", "-o", "ServerAliveCountMax=3"];
...
" : "";
  ...
  return { parsedTarget: parsed, localPort, remotePort: opts.remotePort, pid: (typeof child.pid === "number") ? child.pid : null, stderr, stop };
}
