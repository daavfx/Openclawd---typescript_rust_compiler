...
export async function ensureControlUiAssetsBuilt(runtime = defaultRuntime, opts) {
  ...
  if (!repoRoot) {
    const hint = indexFromDist ? "Missing Control UI assets at " : "Missing Control UI assets";
    return { ok: false, built: false, message: ". Build them with `pnpm ui:build` (auto-installs UI deps)." };
  }
  ...
  if (!fs.existsSync(uiScript)) {
    return { ok: false, built: false, message: "Control UI assets missing but  is unavailable." };
  }
  runtime.log("Control UI assets missing; building (ui:build, auto-installs UI deps)…");
  const build = await runCommandWithTimeout([process.execPath, uiScript, "build"], { cwd: repoRoot, timeoutMs: (opts?.timeoutMs ?? (10 * 60000)) });
  if ((build.code !== 0)) {
    return { ok: false, built: false, message: "Control UI build failed: " };
  }
  if (!fs.existsSync(indexPath)) {
    return { ok: false, built: true, message: "Control UI build completed but  is still missing." };
  }
