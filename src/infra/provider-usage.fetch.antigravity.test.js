...
describe("fetchAntigravityUsage", () => {
  it("returns 3 windows when both endpoints succeed", async () => {
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 750, planInfo: { monthlyPromptCredits: 1000 }, planType: "Standard", currentTier: { id: "tier1", name: "Standard Tier" } });
      }
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-pro-1.5": { quotaInfo: { remainingFraction: 0.6, resetTime: "2026-01-08T00:00:00Z", isExhausted: false } }, "gemini-flash-2.0": { quotaInfo: { remainingFraction: 0.8, resetTime: "2026-01-08T00:00:00Z", isExhausted: false } } } });
      }
  ...
  it("returns Credits only when loadCodeAssist succeeds but fetchAvailableModels fails", async () => {
    const mockFetch = vi.fn(async (input) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 250, planInfo: { monthlyPromptCredits: 1000 }, currentTier: { name: "Free" } });
      }
      if (url.includes("fetchAvailableModels")) {
  ...
  it("returns model IDs when fetchAvailableModels succeeds but loadCodeAssist fails", async () => {
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-pro-1.5": { quotaInfo: { remainingFraction: 0.5, resetTime: "2026-01-08T00:00:00Z" } }, "gemini-flash-2.0": { quotaInfo: { remainingFraction: 0.7, resetTime: "2026-01-08T00:00:00Z" } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("uses cloudaicompanionProject string as project id", async () => {
    ...
    const mockFetch = vi.fn(async (input, init) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 900, planInfo: { monthlyPromptCredits: 1000 }, cloudaicompanionProject: "projects/alpha" });
      }
      if (url.includes("fetchAvailableModels")) {
  ...
  it("uses cloudaicompanionProject object id when present", async () => {
    ...
    const mockFetch = vi.fn(async (input, init) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 900, planInfo: { monthlyPromptCredits: 1000 }, cloudaicompanionProject: { id: "projects/beta" } });
      }
      if (url.includes("fetchAvailableModels")) {
        capturedBody = init?.body?.toString();
        return makeResponse(200, { models: {  } });
  ...
  it("extracts plan info from currentTier.name", async () => {
    const mockFetch = vi.fn(async (input) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 500, planInfo: { monthlyPromptCredits: 1000 }, planType: "Basic", currentTier: { id: "tier2", name: "Premium Tier" } });
      }
      if (url.includes("fetchAvailableModels")) {
  ...
  it("falls back to planType when currentTier.name is missing", async () => {
    const mockFetch = vi.fn(async (input) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 500, planInfo: { monthlyPromptCredits: 1000 }, planType: "Basic Plan" });
      }
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(500, "Error");
      }
  ...
  it("includes reset times in model windows", async () => {
    ...
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-pro-experimental": { quotaInfo: { remainingFraction: 0.3, resetTime } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("parses string numbers correctly", async () => {
    const mockFetch = vi.fn(async (input) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: "600", planInfo: { monthlyPromptCredits: "1000" } });
      }
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-flash-lite": { quotaInfo: { remainingFraction: "0.9" } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("skips internal models", async () => {
    const mockFetch = vi.fn(async (input) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 500, planInfo: { monthlyPromptCredits: 1000 }, cloudaicompanionProject: "projects/internal" });
      }
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { chat_hidden: { quotaInfo: { remainingFraction: 0.1 } }, tab_hidden: { quotaInfo: { remainingFraction: 0.2 } }, "gemini-pro-1.5": { quotaInfo: { remainingFraction: 0.7 } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("sorts models by usage and shows individual model IDs", async () => {
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-pro-1.0": { quotaInfo: { remainingFraction: 0.8 } }, "gemini-pro-1.5": { quotaInfo: { remainingFraction: 0.3 } }, "gemini-flash-1.5": { quotaInfo: { remainingFraction: 0.6 } }, "gemini-flash-2.0": { quotaInfo: { remainingFraction: 0.9 } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("handles empty models array gracefully", async () => {
    const mockFetch = vi.fn(async (input) => {
      const url = (typeof input === "string") ? input : (input instanceof URL) ? input.toString() : input.url;
      if (url.includes("loadCodeAssist")) {
        return makeResponse(200, { availablePromptCredits: 800, planInfo: { monthlyPromptCredits: 1000 } });
      }
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: {  } });
      }
  ...
  it("handles missing credits fields gracefully", async () => {
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-flash-experimental": { quotaInfo: { remainingFraction: 0.5 } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("handles invalid reset time gracefully", async () => {
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-pro-test": { quotaInfo: { remainingFraction: 0.4, resetTime: "invalid-date" } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
  ...
  it("handles network errors with graceful degradation", async () => {
    const mockFetch = vi.fn(async (input) => {
      ...
      if (url.includes("fetchAvailableModels")) {
        return makeResponse(200, { models: { "gemini-flash-stable": { quotaInfo: { remainingFraction: 0.85 } } } });
      }
      return makeResponse(404, "not found");
    });
    const snapshot = await fetchAntigravityUsage("token-123", 5000, mockFetch);
