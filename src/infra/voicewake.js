...
export async function loadVoiceWakeConfig(baseDir) {
  ...
  return { triggers: sanitizeTriggers(existing.triggers), updatedAtMs: ((typeof existing.updatedAtMs === "number") && (existing.updatedAtMs > 0)) ? existing.updatedAtMs : 0 };
}
...
export async function setVoiceWakeTriggers(triggers, baseDir) {
  ...
  return await withLock(async () => {
    const next = { triggers: sanitized, updatedAtMs: Date.now() };
