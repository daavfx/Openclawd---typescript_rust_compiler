...
async function generateSelfSignedCert(params) {
  ...
  await execFileAsync("openssl", ["req", "-x509", "-newkey", "rsa:2048", "-sha256", "-days", "3650", "-nodes", "-keyout", params.keyPath, "-out", params.certPath, "-subj", "/CN=openclaw-gateway"]);
  await fs.chmod(params.keyPath, 384).catch(() => {
  });
  await fs.chmod(params.certPath, 384).catch(() => {
  });
  params.log?.info?.("gateway tls: generated self-signed cert at ");
}
export async function loadGatewayTlsRuntime(cfg, log) {
  ...
  if (((!hasCert && !hasKey) && autoGenerate)) {
    try {
      ...
      {
        return { enabled: false, required: true, certPath, keyPath, error: "gateway tls: failed to generate cert ()" };
      }
    }
  }
  if ((!await fileExists(certPath) || !await fileExists(keyPath))) {
    return { enabled: false, required: true, certPath, keyPath, error: "gateway tls: cert/key missing" };
  }
  try {
    {
      ...
      if (!fingerprintSha256) {
        return { enabled: false, required: true, certPath, keyPath, caPath, error: "gateway tls: unable to compute certificate fingerprint" };
      }
      return { enabled: true, required: true, certPath, keyPath, caPath, fingerprintSha256, tlsOptions: { cert, key, ca, minVersion: "TLSv1.2" } };
    }
  }
  catch (err) {
    {
      return { enabled: false, required: true, certPath, keyPath, caPath, error: "gateway tls: failed to load cert ()" };
    }
  }
}
