...
function initSelfPresence() {
  ...
  const text = "Gateway:  · app  · mode gateway · reason self";
  const selfEntry = { host, ip, version, platform, deviceFamily, modelIdentifier, mode: "gateway", reason: "self", text, ts: Date.now() };
...
function parsePresence(text) {
  ...
  return { host: host.trim(), ip: ip.trim(), version: version.trim(), lastInputSeconds: Number.isFinite(lastInputSeconds) ? lastInputSeconds : undefined, mode: mode.trim(), reason, text: trimmed, ts: Date.now() };
}
...
export function updateSystemPresence(payload) {
  ...
  return { key, previous: hadExisting ? existing : undefined, next: merged, changes, changedKeys };
}
...
export function upsertPresence(key, presence) {
  ...
  const merged = { ...existing: , ...presence: , roles, scopes, ts: Date.now(), text: ((presence.text || existing.text) || "Node:  · mode ") };
