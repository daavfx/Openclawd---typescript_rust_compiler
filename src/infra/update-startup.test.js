...
describe("update-startup", () => {
  ...
  it("logs update hint for npm installs when newer tag exists", async () => {
    ...
    vi.mocked(checkUpdateStatus).mockResolvedValue({ root: "/opt/openclaw", installKind: "package", packageManager: "npm" });
    vi.mocked(resolveNpmChannelTag).mockResolvedValue({ tag: "latest", version: "2.0.0" });
    const log = { info: vi.fn() };
    await runGatewayUpdateCheck({ cfg: { update: { channel: "stable" } }, log, isNixMode: false, allowInTests: true });
  ...
  it("uses latest when beta tag is older than release", async () => {
    ...
    vi.mocked(resolveNpmChannelTag).mockResolvedValue({ tag: "latest", version: "2.0.0" });
    const log = { info: vi.fn() };
    await runGatewayUpdateCheck({ cfg: { update: { channel: "beta" } }, log, isNixMode: false, allowInTests: true });
    expect(log.info).toHaveBeenCalledWith(expect.stringContaining("update available (latest): v2.0.0"));
    const statePath = path.join(tempDir, "update-check.json");
    const raw = await fs.readFile(statePath, "utf-8");
    const parsed = JSON.parse(raw);
  ...
  it("skips update check when disabled in config", async () => {
    const {runGatewayUpdateCheck} = await import("./update-startup.js");
    const log = { info: vi.fn() };
    await runGatewayUpdateCheck({ cfg: { update: { checkOnStart: false } }, log, isNixMode: false, allowInTests: true });
