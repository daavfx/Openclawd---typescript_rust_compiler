...
describe("diagnostic-events", () => {
  ...
  test("emits message-flow events", async () => {
    ...
    emitDiagnosticEvent({ type: "message.queued", channel: "telegram", source: "telegram", queueDepth: 1 });
    emitDiagnosticEvent({ type: "session.state", state: "processing", reason: "run_started" });
    stop();
