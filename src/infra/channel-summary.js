...
export 
const DEFAULT_OPTIONS = { colorize: false, includeAllowFrom: false };
...
const buildAccountSnapshot = (params) => {
  const described = params.plugin.config.describeAccount ? params.plugin.config.describeAccount(params.account, params.cfg) : undefined;
...
const buildAccountDetails = (params) => {
  ...
  if ((params.includeAllowFrom && snapshot.allowFrom?.length)) {
    const formatted = formatAllowFrom({ plugin: params.plugin, cfg: params.cfg, accountId: snapshot.accountId, allowFrom: snapshot.allowFrom }).slice(0, 2);
...
export async function buildChannelSummary(cfg, options) {
  ...
  for (const plugin of listChannelPlugins()) {
    ...
    for (const accountId of resolvedAccountIds) {
      ...
      const snapshot = buildAccountSnapshot({ plugin, account, cfg: effective, accountId, enabled, configured });
    ...
    const summary = plugin.status?.buildChannelSummary ? await plugin.status.buildChannelSummary({ account: (fallbackEntry?.account ?? {  }), cfg: effective, defaultAccountId, snapshot: (fallbackEntry?.snapshot ?? { accountId: defaultAccountId }) }) : undefined;
    ...
    if ((configuredEntries.length > 0)) {
      for (const entry of configuredEntries) {
        const details = buildAccountDetails({ entry, plugin, cfg: effective, includeAllowFrom: resolved.includeAllowFrom });
        lines.push(accountLine(formatAccountLabel({ accountId: entry.accountId, name: entry.snapshot.name }), details));
      }
    }
  }
  return lines;
