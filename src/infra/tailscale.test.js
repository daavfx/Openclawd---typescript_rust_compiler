...
const {ensureGoInstalled, ensureTailscaledInstalled, getTailnetHostname, enableTailscaleServe, disableTailscaleServe, ensureFunnel} = tailscale;
...
describe("tailscale helpers", () => {
  ...
  it("parses DNS name from tailscale status", async () => {
    const exec = vi.fn().mockResolvedValue({ stdout: JSON.stringify({ Self: { DNSName: "host.tailnet.ts.net.", TailscaleIPs: ["100.1.1.1"] } }) });
    const host = await getTailnetHostname(exec);
    expect(host).toBe("host.tailnet.ts.net");
  });
  it("falls back to IP when DNS missing", async () => {
    const exec = vi.fn().mockResolvedValue({ stdout: JSON.stringify({ Self: { TailscaleIPs: ["100.2.2.2"] } }) });
    const host = await getTailnetHostname(exec);
  ...
  it("ensureGoInstalled installs when missing and user agrees", async () => {
    ...
    const runtime = { error: vi.fn(), log: vi.fn(), exit: (code) => {
  ...
  it("enableTailscaleServe attempts normal first, then sudo", async () => {
    ...
    expect(exec).toHaveBeenNthCalledWith(1, tailscaleBin, expect.arrayContaining(["serve", "--bg", "--yes", "3000"]), expect.any(Object));
    expect(exec).toHaveBeenNthCalledWith(2, "sudo", expect.arrayContaining(["-n", tailscaleBin, "serve", "--bg", "--yes", "3000"]), expect.any(Object));
  });
  it("enableTailscaleServe does NOT use sudo if first attempt succeeds", async () => {
    ...
    expect(exec).toHaveBeenCalledWith(tailscaleBin, expect.arrayContaining(["serve", "--bg", "--yes", "3000"]), expect.any(Object));
  });
  it("disableTailscaleServe uses fallback", async () => {
    ...
    expect(exec).toHaveBeenNthCalledWith(2, "sudo", expect.arrayContaining(["-n", tailscaleBin, "serve", "reset"]), expect.any(Object));
  });
  it("ensureFunnel uses fallback for enabling", async () => {
    vi.spyOn(tailscale, "getTailscaleBinary").mockResolvedValue("tailscale");
    const exec = vi.fn().mockResolvedValueOnce({ stdout: JSON.stringify({ BackendState: "Running" }) }).mockRejectedValueOnce(new Error("permission denied")).mockResolvedValueOnce({ stdout: "" });
    ...
    expect(exec).toHaveBeenNthCalledWith(2, tailscaleBin, expect.arrayContaining(["funnel", "--yes", "--bg", "8080"]), expect.any(Object));
    expect(exec).toHaveBeenNthCalledWith(3, "sudo", expect.arrayContaining(["-n", tailscaleBin, "funnel", "--yes", "--bg", "8080"]), expect.any(Object));
  });
  ...
  it("enableTailscaleServe rethrows original error if sudo fails", async () => {
    vi.spyOn(tailscale, "getTailscaleBinary").mockResolvedValue("tailscale");
    const originalError = Object.assign(new Error("permission denied"), { stderr: "permission denied" });
    const exec = vi.fn().mockRejectedValueOnce(originalError).mockRejectedValueOnce(new Error("sudo: a password is required"));
