...
async function resolveOAuthToken(params) {
  ...
  for (const profileId of deduped) {
    ...
    try {
      {
        const resolved = await resolveApiKeyForProfile({ cfg: undefined, store, profileId, agentDir: params.agentDir });
        if (!resolved?.apiKey) {
          continue;
        }
        ...
        return { provider: params.provider, token, accountId: ((cred.type === "oauth") && ("accountId" in cred)) ? cred.accountId : undefined };
      }
    }
    catch {
      {
...
function resolveOAuthProviders(agentDir) {
  ...
  const cfg = loadConfig();
  const providers = ["anthropic", "github-copilot", "google-gemini-cli", "google-antigravity", "openai-codex"];
  ...
  return providers.filter((provider) => {
    ...
    const configuredProfiles = Object.entries((cfg.auth?.profiles ?? {  })).filter(([, profile]) => (normalizeProviderId(profile.provider) === normalized)).map(([id]) => id).filter(isOAuthLikeCredential);
...
export async function resolveProviderAuths(params) {
  ...
  for (const provider of params.providers) {
    ...
    const auth = await resolveOAuthToken({ provider, agentDir: params.agentDir });
    if (auth) {
      auths.push(auth);
    }
  }
