...
export function normalizeOutboundPayloads(payloads) {
  return normalizeReplyPayloadsForDelivery(payloads).map((payload) => {
    ...
    const normalized = { text: (payload.text ?? ""), mediaUrls: (payload.mediaUrls ?? payload.mediaUrl ? [payload.mediaUrl] : []) };
...
export function normalizeOutboundPayloadsForJson(payloads) {
  return normalizeReplyPayloadsForDelivery(payloads).map((payload) => { text: (payload.text ?? ""), mediaUrl: (payload.mediaUrl ?? null), mediaUrls: (payload.mediaUrls ?? payload.mediaUrl ? [payload.mediaUrl] : undefined), channelData: payload.channelData });
}
