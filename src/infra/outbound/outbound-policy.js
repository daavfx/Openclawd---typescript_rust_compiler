...
export 
const CONTEXT_GUARDED_ACTIONS = new Set(["send", "poll", "reply", "sendWithEffect", "sendAttachment", "thread-create", "thread-reply", "sticker"]);
const CONTEXT_MARKER_ACTIONS = new Set(["send", "poll", "reply", "sendWithEffect", "sendAttachment", "thread-reply", "sticker"]);
...
export function enforceCrossContextPolicy(params) {
  ...
  if ((currentProvider && (currentProvider !== params.channel))) {
    if (!allowAcrossProviders) {
      throw new Error("Cross-context messaging denied: action= target provider \"\" while bound to \"\".");
    }
    return;
  }
  ...
  throw new Error("Cross-context messaging denied: action= target=\"\" while bound to \"\" (channel=).");
}
...
export async function buildCrossContextDecoration(params) {
  ...
  const currentName = (await lookupDirectoryDisplay({ cfg: params.cfg, channel: params.channel, targetId: params.toolContext.currentChannelId, accountId: (params.accountId ?? undefined) }) ?? params.toolContext.currentChannelId);
  const originLabel = formatTargetDisplay({ channel: params.channel, target: params.toolContext.currentChannelId, display: currentName });
  ...
  const embeds = adapter.supportsEmbeds ? (adapter.buildCrossContextEmbeds?.(originLabel) ?? undefined) : undefined;
