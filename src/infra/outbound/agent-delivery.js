...
export function resolveAgentDeliveryPlan(params) {
  ...
  const baseDelivery = resolveSessionDeliveryTarget({ entry: params.sessionEntry, requestedChannel: (requestedChannel === INTERNAL_MESSAGE_CHANNEL) ? "last" : requestedChannel, explicitTo, explicitThreadId: params.explicitThreadId });
  ...
  const deliveryTargetMode = explicitTo ? "explicit" : isDeliverableMessageChannel(resolvedChannel) ? "implicit" : undefined;
  const resolvedAccountId = (normalizeAccountId(params.accountId) ?? (deliveryTargetMode === "implicit") ? baseDelivery.accountId : undefined);
  ...
  return { baseDelivery, resolvedChannel, resolvedTo, resolvedAccountId, resolvedThreadId: baseDelivery.threadId, deliveryTargetMode };
}
...
export function resolveAgentOutboundTarget(params) {
  ...
  if (!isDeliverableMessageChannel(params.plan.resolvedChannel)) {
    return { resolvedTarget: null, resolvedTo: params.plan.resolvedTo, targetMode };
  }
  if (((params.validateExplicitTarget !== true) && params.plan.resolvedTo)) {
    return { resolvedTarget: null, resolvedTo: params.plan.resolvedTo, targetMode };
  }
  const resolvedTarget = resolveOutboundTarget({ channel: params.plan.resolvedChannel, to: params.plan.resolvedTo, cfg: params.cfg, accountId: params.plan.resolvedAccountId, mode: targetMode });
  return { resolvedTarget, resolvedTo: resolvedTarget.ok ? resolvedTarget.to : params.plan.resolvedTo, targetMode };
}
