...
function buildBaseSessionKey(params) {
  return buildAgentSessionKey({ agentId: params.agentId, channel: params.channel, accountId: params.accountId, peer: params.peer, dmScope: (params.cfg.session?.dmScope ?? "main"), identityLinks: params.cfg.session?.identityLinks });
}
async function resolveSlackChannelType(params) {
  ...
  const cached = SLACK_CHANNEL_TYPE_CACHE.get(":");
  if (cached) {
    return cached;
  }
  ...
  if (((((groupChannels.includes(channelIdLower) || groupChannels.includes("slack:")) || groupChannels.includes("channel:")) || groupChannels.includes("group:")) || groupChannels.includes("mpim:"))) {
    SLACK_CHANNEL_TYPE_CACHE.set(":", "group");
    return "group";
  }
  const channelKeys = Object.keys((account.channels ?? {  }));
  if (channelKeys.some((key) => {
    const normalized = key.trim().toLowerCase();
    return (((normalized === channelIdLower) || (normalized === "channel:")) || (normalized.replace(/^#/, "") === channelIdLower));
  })) {
    SLACK_CHANNEL_TYPE_CACHE.set(":", "channel");
    return "channel";
  }
  const token = (account.botToken?.trim() || (typeof account.config.userToken === "string") ? account.config.userToken.trim() : "");
  if (!token) {
    SLACK_CHANNEL_TYPE_CACHE.set(":", "unknown");
    return "unknown";
  }
  try {
    {
      ...
      SLACK_CHANNEL_TYPE_CACHE.set(":", type);
      return type;
    }
  }
  catch {
...
async function resolveSlackSession(params) {
  ...
  if ((!isDm && /^G/i.test(parsed.id))) {
    const channelType = await resolveSlackChannelType({ cfg: params.cfg, accountId: params.accountId, channelId: parsed.id });
  ...
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "slack", accountId: params.accountId, peer });
  ...
  return { sessionKey: threadKeys.sessionKey, baseSessionKey, peer, chatType: (peerKind === "dm") ? "direct" : "channel", from: (peerKind === "dm") ? "slack:" : (peerKind === "group") ? "slack:group:" : "slack:channel:", to: (peerKind === "dm") ? "user:" : "channel:", threadId };
}
function resolveDiscordSession(params) {
  ...
  const isDm = (parsed.kind === "user");
  const peer = { kind: isDm ? "dm" : "channel", id: parsed.id };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "discord", accountId: params.accountId, peer });
  ...
  const threadKeys = resolveThreadSessionKeys({ baseSessionKey, threadId: threadCandidate, useSuffix: false });
  return { sessionKey: threadKeys.sessionKey, baseSessionKey, peer, chatType: isDm ? "direct" : "channel", from: isDm ? "discord:" : "discord:channel:", to: isDm ? "user:" : "channel:", threadId: (explicitThreadId ?? undefined) };
}
function resolveTelegramSession(params) {
  ...
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "telegram", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isGroup ? "group" : "direct", from: isGroup ? "telegram:group:" : "telegram:", to: "telegram:", threadId: resolvedThreadId };
}
function resolveWhatsAppSession(params) {
  ...
  const peer = { kind: isGroup ? "group" : "dm", id: normalized };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "whatsapp", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isGroup ? "group" : "direct", from: normalized, to: normalized };
}
function resolveSignalSession(params) {
  ...
  if (lowered.startsWith("group:")) {
    ...
    const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "signal", accountId: params.accountId, peer });
    return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: "group", from: "group:", to: "group:" };
  }
  ...
  const sender = resolveSignalSender({ sourceUuid: looksLikeUuid(uuidCandidate) ? uuidCandidate : null, sourceNumber: looksLikeUuid(uuidCandidate) ? null : recipient });
  const peerId = sender ? resolveSignalPeerId(sender) : recipient;
  const displayRecipient = sender ? resolveSignalRecipient(sender) : recipient;
  const peer = { kind: "dm", id: peerId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "signal", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: "direct", from: "signal:", to: "signal:" };
}
function resolveIMessageSession(params) {
  ...
  if ((parsed.kind === "handle")) {
    ...
    const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "imessage", accountId: params.accountId, peer });
    return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: "direct", from: "imessage:", to: "imessage:" };
  }
  ...
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: "group", from: "imessage:group:", to: ":" };
}
function resolveMatrixSession(params) {
  ...
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "matrix", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isUser ? "direct" : "channel", from: isUser ? "matrix:" : "matrix:channel:", to: "room:" };
}
function resolveMSTeamsSession(params) {
  ...
  const peer = { kind: isUser ? "dm" : isChannel ? "channel" : "group", id: conversationId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "msteams", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isUser ? "direct" : isChannel ? "channel" : "group", from: isUser ? "msteams:" : isChannel ? "msteams:channel:" : "msteams:group:", to: isUser ? "user:" : "conversation:" };
}
function resolveMattermostSession(params) {
  ...
  const peer = { kind: isUser ? "dm" : "channel", id: rawId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "mattermost", accountId: params.accountId, peer });
  const threadId = normalizeThreadId((params.replyToId ?? params.threadId));
  const threadKeys = resolveThreadSessionKeys({ baseSessionKey, threadId });
  return { sessionKey: threadKeys.sessionKey, baseSessionKey, peer, chatType: isUser ? "direct" : "channel", from: isUser ? "mattermost:" : "mattermost:channel:", to: isUser ? "user:" : "channel:", threadId };
}
function resolveBlueBubblesSession(params) {
  ...
  const peer = { kind: isGroup ? "group" : "dm", id: peerId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "bluebubbles", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isGroup ? "group" : "direct", from: isGroup ? "group:" : "bluebubbles:", to: "bluebubbles:" };
}
function resolveNextcloudTalkSession(params) {
  ...
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "nextcloud-talk", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: "group", from: "nextcloud-talk:room:", to: "nextcloud-talk:" };
}
function resolveZaloSession(params) {
  ...
  const peer = { kind: isGroup ? "group" : "dm", id: peerId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "zalo", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isGroup ? "group" : "direct", from: isGroup ? "zalo:group:" : "zalo:", to: "zalo:" };
}
function resolveZalouserSession(params) {
  ...
  const peerId = stripKindPrefix(trimmed);
  const peer = { kind: isGroup ? "group" : "dm", id: peerId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "zalouser", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isGroup ? "group" : "direct", from: isGroup ? "zalouser:group:" : "zalouser:", to: "zalouser:" };
}
function resolveNostrSession(params) {
  ...
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "nostr", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: "direct", from: "nostr:", to: "nostr:" };
}
...
function resolveTlonSession(params) {
  ...
  const peer = { kind: isGroup ? "group" : "dm", id: peerId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: "tlon", accountId: params.accountId, peer });
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType: isGroup ? "group" : "direct", from: isGroup ? "tlon:group:" : "tlon:", to: "tlon:" };
}
function resolveFallbackSession(params) {
  ...
  const peerKind = inferPeerKind({ channel: params.channel, resolvedTarget: params.resolvedTarget });
  const peerId = stripKindPrefix(trimmed);
  if (!peerId) {
    return null;
  }
  const peer = { kind: peerKind, id: peerId };
  const baseSessionKey = buildBaseSessionKey({ cfg: params.cfg, agentId: params.agentId, channel: params.channel, peer });
  const chatType = (peerKind === "dm") ? "direct" : (peerKind === "channel") ? "channel" : "group";
  const from = (peerKind === "dm") ? ":" : "::";
  const toPrefix = (peerKind === "dm") ? "user" : "channel";
  return { sessionKey: baseSessionKey, baseSessionKey, peer, chatType, from, to: ":" };
}
