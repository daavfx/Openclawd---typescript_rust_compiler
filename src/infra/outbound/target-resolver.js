...
export function formatTargetDisplay(params) {
  ...
  if (plugin?.messaging?.formatTargetDisplay) {
    return plugin.messaging.formatTargetDisplay({ target: params.target, display: params.display, kind: params.kind });
  }
  const trimmedTarget = params.target.trim();
  const lowered = trimmedTarget.toLowerCase();
  const display = params.display?.trim();
  const kind = (params.kind ?? lowered.startsWith("user:") ? "user" : lowered.startsWith("channel:") ? "group" : undefined);
  if (display) {
...
async function listDirectoryEntries(params) {
  ...
  if ((params.kind === "user")) {
    ...
    return await fn({ cfg: params.cfg, accountId: (params.accountId ?? undefined), query: (params.query ?? undefined), limit: undefined, runtime });
  }
...
async function getDirectoryEntries(params) {
  ...
  const cacheKey = buildDirectoryCacheKey({ channel: params.channel, accountId: params.accountId, kind: params.kind, source: "cache", signature });
  const cached = directoryCache.get(cacheKey, params.cfg);
  if (cached) {
    return cached;
  }
  const entries = await listDirectoryEntries({ cfg: params.cfg, channel: params.channel, accountId: params.accountId, kind: params.kind, query: params.query, runtime: params.runtime, source: "cache" });
  if (((entries.length > 0) || !params.preferLiveOnMiss)) {
    directoryCache.set(cacheKey, entries, params.cfg);
    return entries;
  }
  const liveKey = buildDirectoryCacheKey({ channel: params.channel, accountId: params.accountId, kind: params.kind, source: "live", signature });
  const liveEntries = await listDirectoryEntries({ cfg: params.cfg, channel: params.channel, accountId: params.accountId, kind: params.kind, query: params.query, runtime: params.runtime, source: "live" });
...
function pickAmbiguousMatch(entries, mode) {
  ...
  const ranked = entries.map((entry) => { entry, rank: (typeof entry.rank === "number") ? entry.rank : 0 });
...
export async function resolveMessagingTarget(params) {
  ...
  if (looksLikeTargetId()) {
    ...
    return { ok: true, target: { to: directTarget, kind, display: stripTargetPrefixes(raw), source: "normalized" } };
  }
  const query = stripTargetPrefixes(raw);
  const entries = await getDirectoryEntries({ cfg: params.cfg, channel: params.channel, accountId: params.accountId, kind: (kind === "user") ? "user" : "group", query, runtime: params.runtime, preferLiveOnMiss: true });
  const match = resolveMatch({ channel: params.channel, entries, query });
  if ((match.kind === "single")) {
    const entry = match.entry;
    return { ok: true, target: { to: normalizeDirectoryEntryId(params.channel, entry), kind, display: ((entry.name ?? entry.handle) ?? stripTargetPrefixes(entry.id)), source: "directory" } };
  }
  if ((match.kind === "ambiguous")) {
    ...
    if ((mode !== "error")) {
      ...
      if (best) {
        return { ok: true, target: { to: normalizeDirectoryEntryId(params.channel, best), kind, display: ((best.name ?? best.handle) ?? stripTargetPrefixes(best.id)), source: "directory" } };
      }
    }
    return { ok: false, error: ambiguousTargetError(providerLabel, raw, hint), candidates: match.entries };
  }
  if ((((params.channel === "bluebubbles") || (params.channel === "imessage")) && /^\+?\d{6,}$/.test(query))) {
    const directTarget = preserveTargetCase(params.channel, raw, normalized);
    return { ok: true, target: { to: directTarget, kind, display: stripTargetPrefixes(raw), source: "normalized" } };
  }
  return { ok: false, error: unknownTargetError(providerLabel, raw, hint) };
}
...
export async function lookupDirectoryDisplay(params) {
  ...
  const [groups, users] = await Promise.all([getDirectoryEntries({ cfg: params.cfg, channel: params.channel, accountId: params.accountId, kind: "group", runtime: params.runtime, preferLiveOnMiss: false }), getDirectoryEntries({ cfg: params.cfg, channel: params.channel, accountId: params.accountId, kind: "user", runtime: params.runtime, preferLiveOnMiss: false })]);
