...
export function formatOutboundDeliverySummary(channel, result) {
  if (!result) {
    return "✅ Sent via . Message ID: unknown";
  }
  const label = resolveChannelLabel(result.channel);
  const base = "✅ Sent via . Message ID: ";
  if (("chatId" in result)) {
    return " (chat )";
  }
...
export function buildOutboundDeliveryJson(params) {
  ...
  const payload = { channel, via: (params.via ?? "direct"), to, messageId, mediaUrl: (params.mediaUrl ?? null) };
...
export function formatGatewaySummary(params) {
  ...
  const channelSuffix = params.channel ? " ()" : "";
  const messageId = (params.messageId ?? "unknown");
  return "✅  via gateway. Message ID: ";
}
