...
function normalizeSystemdUnit(raw, profile) {
  ...
  return unit.endsWith(".service") ? unit : ".service";
}
export function triggerOpenClawRestart() {
  ...
  if ((process.platform !== "darwin")) {
    if ((process.platform === "linux")) {
      ...
      const userRestart = spawnSync("systemctl", userArgs, { encoding: "utf8", timeout: SPAWN_TIMEOUT_MS });
      if ((!userRestart.error && (userRestart.status === 0))) {
        return { ok: true, method: "systemd", tried };
      }
      const systemArgs = ["restart", unit];
      tried.push("systemctl ");
      const systemRestart = spawnSync("systemctl", systemArgs, { encoding: "utf8", timeout: SPAWN_TIMEOUT_MS });
      if ((!systemRestart.error && (systemRestart.status === 0))) {
        return { ok: true, method: "systemd", tried };
      }
      const detail = ["user: ", "system: "].join("; ");
  ...
  const target = (uid !== undefined) ? "gui//" : label;
  const args = ["kickstart", "-k", target];
  tried.push("launchctl ");
  const res = spawnSync("launchctl", args, { encoding: "utf8", timeout: SPAWN_TIMEOUT_MS });
  if ((!res.error && (res.status === 0))) {
    return { ok: true, method: "launchctl", tried };
  }
  return { ok: false, method: "launchctl", detail: formatSpawnDetail(res), tried };
}

export 
export function scheduleGatewaySigusr1Restart(opts) {
  const delayMsRaw = ((typeof opts?.delayMs === "number") && Number.isFinite(opts.delayMs)) ? Math.floor(opts.delayMs) : 2000;
  ...
  return { ok: true, pid, signal: "SIGUSR1", delayMs, reason, mode: hasListener ? "emit" : "signal" };
}

export const __testing = { resetSigusr1State: function() {
  sigusr1AuthorizedCount = 0;
  sigusr1AuthorizedUntil = 0;
