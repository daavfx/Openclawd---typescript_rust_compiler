...
function describeNode(nodeId) {
  ...
  const base = (name && (name !== nodeId)) ? " ()" : nodeId;
...
function logRemoteBinProbeFailure(nodeId, err) {
  ...
  if ((message?.includes("node not connected") || message?.includes("node disconnected"))) {
    log.info("remote bin probe skipped: node unavailable ()");
    return;
  }
  if ((message?.includes("invoke timed out") || message?.includes("timeout"))) {
    log.warn("remote bin probe timed out (); check node connectivity for ");
    return;
  }
  log.warn("remote bin probe error (): ");
}
...
function upsertNode(record) {
  ...
  remoteNodes.set(record.nodeId, { nodeId: record.nodeId, displayName: (record.displayName ?? existing?.displayName), platform: (record.platform ?? existing?.platform), deviceFamily: (record.deviceFamily ?? existing?.deviceFamily), commands: (record.commands ?? existing?.commands), remoteIp: (record.remoteIp ?? existing?.remoteIp), bins });
}
  ...
  try {
    {
      ...
      for (const node of list.paired) {
        upsertNode({ nodeId: node.nodeId, displayName: node.displayName, platform: node.platform, deviceFamily: node.deviceFamily, commands: node.commands, remoteIp: node.remoteIp, bins: node.bins });
    ...
    {
      log.warn("failed to prime remote skills cache: ");
    }
  }
}
...
function buildBinProbeScript(bins) {
  const escaped = bins.map((bin) => "''").join(" ");
  return "for b in ; do if command -v \"$b\" >/dev/null 2>&1; then echo \"$b\"; fi; done";
}
  ...
  try {
    {
      ...
      const res = await remoteRegistry.invoke(canWhich ? { nodeId: params.nodeId, command: "system.which", params: { bins: binsList }, timeoutMs: (params.timeoutMs ?? 15000) } : { nodeId: params.nodeId, command: "system.run", params: { command: ["/bin/sh", "-lc", buildBinProbeScript(binsList)] }, timeoutMs: (params.timeoutMs ?? 15000) });
      if (!res.ok) {
...
export function getRemoteSkillEligibility() {
  ...
  const note = (labels.length > 0) ? "Remote macOS node available (). Run macOS-only skills via nodes.run on that node." : "Remote macOS node available. Run macOS-only skills via nodes.run on that node.";
  return { platforms: ["darwin"], hasBin: (bin) => bins.has(bin), hasAnyBin: (required) => required.some((bin) => bins.has(bin)), note };
}
...
export async function refreshRemoteBinsForConnectedNodes(cfg) {
  ...
  for (const node of connected) {
    await refreshRemoteNodeBins({ nodeId: node.nodeId, platform: node.platform, deviceFamily: node.deviceFamily, commands: node.commands, cfg });
  }
}
