...
function mergeLegacyAgent(current, legacy) {
  ...
  return { security: (current.security ?? legacy.security), ask: (current.ask ?? legacy.ask), askFallback: (current.askFallback ?? legacy.askFallback), autoAllowSkills: (current.autoAllowSkills ?? legacy.autoAllowSkills), allowlist: (allowlist.length > 0) ? allowlist : undefined };
}
...
export function normalizeExecApprovals(file) {
  ...
  const normalized = { version: 1, socket: { path: (socketPath && (socketPath.length > 0)) ? socketPath : undefined, token: (token && (token.length > 0)) ? token : undefined }, defaults: { security: file.defaults?.security, ask: file.defaults?.ask, askFallback: file.defaults?.askFallback, autoAllowSkills: file.defaults?.autoAllowSkills }, agents };
  return normalized;
}
...
export function readExecApprovalsSnapshot() {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    return { path: filePath, exists: false, raw: null, file, hash: hashExecApprovalsRaw(null) };
  }
  ...
  const file = (parsed?.version === 1) ? normalizeExecApprovals(parsed) : normalizeExecApprovals({ version: 1, agents: {  } });
  return { path: filePath, exists: true, raw, file, hash: hashExecApprovalsRaw(raw) };
}
...
export function ensureExecApprovals() {
  ...
  const updated = { ...next: , socket: { path: (socketPath && (socketPath.length > 0)) ? socketPath : resolveExecApprovalsSocketPath(), token: (token && (token.length > 0)) ? token : generateToken() } };
...
export function resolveExecApprovals(agentId, overrides) {
  ...
  return resolveExecApprovalsFromFile({ file, agentId, overrides, path: resolveExecApprovalsPath(), socketPath: expandHome((file.socket?.path ?? resolveExecApprovalsSocketPath())), token: (file.socket?.token ?? "") });
}
...
export function resolveExecApprovalsFromFile(params) {
  ...
  const resolvedDefaults = { security: normalizeSecurity(defaults.security, fallbackSecurity), ask: normalizeAsk(defaults.ask, fallbackAsk), askFallback: normalizeSecurity((defaults.askFallback ?? fallbackAskFallback), fallbackAskFallback), autoAllowSkills: Boolean((defaults.autoAllowSkills ?? fallbackAutoAllowSkills)) };
  const resolvedAgent = { security: normalizeSecurity(((agent.security ?? wildcard.security) ?? resolvedDefaults.security), resolvedDefaults.security), ask: normalizeAsk(((agent.ask ?? wildcard.ask) ?? resolvedDefaults.ask), resolvedDefaults.ask), askFallback: normalizeSecurity(((agent.askFallback ?? wildcard.askFallback) ?? resolvedDefaults.askFallback), resolvedDefaults.askFallback), autoAllowSkills: Boolean(((agent.autoAllowSkills ?? wildcard.autoAllowSkills) ?? resolvedDefaults.autoAllowSkills)) };
  const allowlist = [...Array.isArray(wildcard.allowlist) ? wildcard.allowlist : [], ...Array.isArray(agent.allowlist) ? agent.allowlist : []];
  return { path: (params.path ?? resolveExecApprovalsPath()), socketPath: expandHome(((params.socketPath ?? file.socket?.path) ?? resolveExecApprovalsSocketPath())), token: ((params.token ?? file.socket?.token) ?? ""), defaults: resolvedDefaults, agent: resolvedAgent, allowlist, file };
}
...
function splitShellPipeline(command) {
  ...
  if ((emptySegment || (result.parts.length === 0))) {
    return { ok: false, reason: (result.parts.length === 0) ? "empty command" : "empty pipeline segment", segments: [] };
  }
...
function parseSegmentsFromParts(parts, cwd, env) {
  ...
  for (const raw of parts) {
    ...
    segments.push({ raw, argv, resolution: resolveCommandResolutionFromArgv(argv, cwd, env) });
  }
  return segments;
}
...
export function analyzeArgvCommand(params) {
  ...
  return { ok: true, segments: [{ raw: argv.join(" "), argv, resolution: resolveCommandResolutionFromArgv(argv, params.cwd, params.env) }] };
}
...
export function isSafeBinUsage(params) {
  ...
  const matchesSafeBin = (params.safeBins.has(execName) || ((process.platform === "win32") && params.safeBins.has(path.parse(execName).name)));
  if (!matchesSafeBin) {
    return false;
  }
  if (!resolution?.resolvedPath) {
...
function evaluateSegments(segments, params) {
  ...
  const satisfied = segments.every((segment) => {
    ...
    const safe = isSafeBinUsage({ argv: segment.argv, resolution: segment.resolution, safeBins: params.safeBins, cwd: params.cwd });
...
export function evaluateExecAllowlist(params) {
  ...
  if (params.analysis.chains) {
    for (const chainSegments of params.analysis.chains) {
      const result = evaluateSegments(chainSegments, { allowlist: params.allowlist, safeBins: params.safeBins, cwd: params.cwd, skillBins: params.skillBins, autoAllowSkills: params.autoAllowSkills });
      if (!result.satisfied) {
        return { allowlistSatisfied: false, allowlistMatches: [] };
      }
  ...
  const result = evaluateSegments(params.analysis.segments, { allowlist: params.allowlist, safeBins: params.safeBins, cwd: params.cwd, skillBins: params.skillBins, autoAllowSkills: params.autoAllowSkills });
...
export function evaluateShellAllowlist(params) {
  const chainParts = splitCommandChain(params.command);
  if (!chainParts) {
    const analysis = analyzeShellCommand({ command: params.command, cwd: params.cwd, env: params.env });
    if (!analysis.ok) {
      return { analysisOk: false, allowlistSatisfied: false, allowlistMatches: [], segments: [] };
    }
    const evaluation = evaluateExecAllowlist({ analysis, allowlist: params.allowlist, safeBins: params.safeBins, cwd: params.cwd, skillBins: params.skillBins, autoAllowSkills: params.autoAllowSkills });
    return { analysisOk: true, allowlistSatisfied: evaluation.allowlistSatisfied, allowlistMatches: evaluation.allowlistMatches, segments: analysis.segments };
  }
  const allowlistMatches = [];
  const segments = [];
  for (const part of chainParts) {
    const analysis = analyzeShellCommand({ command: part, cwd: params.cwd, env: params.env });
    if (!analysis.ok) {
      return { analysisOk: false, allowlistSatisfied: false, allowlistMatches: [], segments: [] };
    }
    ...
    if (!evaluation.allowlistSatisfied) {
      return { analysisOk: true, allowlistSatisfied: false, allowlistMatches, segments };
    }
  }
  return { analysisOk: true, allowlistSatisfied: true, allowlistMatches, segments };
}
...
export function recordAllowlistUse(approvals, agentId, entry, command, resolvedPath) {
  ...
  const nextAllowlist = allowlist.map((item) => (item.pattern === entry.pattern) ? { ...item: , id: (item.id ?? crypto.randomUUID()), lastUsedAt: Date.now(), lastUsedCommand: command, lastResolvedPath: resolvedPath } : item);
