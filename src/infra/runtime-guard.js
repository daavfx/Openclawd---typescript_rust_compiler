...
export function parseSemver(version) {
  ...
  return { major: Number.parseInt(major, 10), minor: Number.parseInt(minor, 10), patch: Number.parseInt(patch, 10) };
}
...
export function detectRuntime() {
  ...
  return { kind, version, execPath: (process.execPath ?? null), pathEnv: (process.env.PATH ?? "(not set)") };
}
...
export function assertSupportedRuntime(runtime = defaultRuntime, details = detectRuntime()) {
  ...
  const runtimeLabel = (details.kind === "unknown") ? "unknown runtime" : " ";
  const execLabel = (details.execPath ?? "unknown");
  runtime.error(["openclaw requires Node >=22.0.0.", "Detected:  (exec: ).", "PATH searched: ", "Install Node: https://nodejs.org/en/download", "Upgrade Node and re-run openclaw."].join("
"));
  runtime.exit(1);
}
