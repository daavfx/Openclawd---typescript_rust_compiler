...
import { clearAgentRunContext, emitAgentEvent, getAgentRunContext, onAgentEvent, registerAgentRunContext, resetAgentRunContextForTest } from "./agent-events.js";
describe("agent-events sequencing", () => {
  ...
  test("preserves compaction ordering on the event bus", async () => {
    ...
    emitAgentEvent({ runId: "run-1", stream: "compaction", data: { phase: "end", willRetry: false } });
    stop();
