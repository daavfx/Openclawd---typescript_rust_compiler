...
function buildRequestMessage(request, nowMs) {
  const lines = ["🔒 Exec approval required", "ID: "];
  lines.push("Command: ");
...
function buildResolvedMessage(resolved) {
  const base = "✅ Exec approval .";
  const by = resolved.resolvedBy ? " Resolved by ." : "";
  return " ID: ";
}
function buildExpiredMessage(request) {
  return "⏱️ Exec approval expired. ID: ";
}
function defaultResolveSessionTarget(params) {
  ...
  return { channel: target.channel, to: target.to, accountId: target.accountId, threadId: target.threadId };
}
async function deliverToTargets(params) {
  const deliveries = params.targets.map(async (target) => {
    ...
    try {
      {
        await params.deliver({ cfg: params.cfg, channel, to: target.to, accountId: target.accountId, threadId: target.threadId, payloads: [{ text: params.text }] });
      }
    }
    catch (err) {
      {
        log.error("exec approvals: failed to deliver to :: ");
      }
    }
  });
...
export function createExecApprovalForwarder(deps = {  }) {
  ...
  const handleRequested = async (request) => {
    ...
    await deliverToTargets({ cfg, targets, text, deliver, shouldSend: () => (pending.get(request.id) === pendingEntry) });
  };
