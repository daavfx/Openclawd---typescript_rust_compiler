...
function waitForLine(stream, timeoutMs = 10000) {
...
describe("attachChildProcessBridge", () => {
  ...
  it("forwards SIGTERM to the wrapped child", async () => {
    ...
    const child = spawn(process.execPath, [childPath], { stdio: ["ignore", "pipe", "inherit"], env: process.env });
    ...
    await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error("timeout waiting for child exit")), 10000);
      child.once("exit", () => {
        clearTimeout(timeout);
        resolve();
      });
