...
export async function spawnWithFallback(params) {
  ...
  const attempts = [{ options: baseOptions }, ...fallbacks.map((fallback) => { label: fallback.label, options: { ...baseOptions: , ...fallback.options:  } })];
  let lastError;
    ...
    try {
      {
        ...
        return { child, usedFallback: (index > 0), fallbackLabel: attempt.label };
      }
    }
    catch (err) {
      {
