...
const diagnosticMocks = vi.hoisted(() => { logLaneEnqueue: vi.fn(), logLaneDequeue: vi.fn(), diag: { debug: vi.fn(), warn: vi.fn(), error: vi.fn() } });
vi.mock("../logging/diagnostic.js", () => { logLaneEnqueue: diagnosticMocks.logLaneEnqueue, logLaneDequeue: diagnosticMocks.logLaneDequeue, diagnosticLogger: diagnosticMocks.diag });
...
describe("command queue", () => {
  ...
  it("runs tasks one at a time in order", async () => {
    ...
    const results = await Promise.all([enqueueCommand(makeTask(1)), enqueueCommand(makeTask(2)), enqueueCommand(makeTask(3))]);
