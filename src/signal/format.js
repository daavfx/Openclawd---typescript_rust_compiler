  ...
  for (const insertion of sortedInsertions) {
    ...
    for (const span of updated) {
      ...
      if ((span.start >= insertion.pos)) {
        next.push({ start: (span.start + insertion.length), end: (span.end + insertion.length), style: span.style });
        continue;
      }
      if (((span.start < insertion.pos) && (span.end > insertion.pos))) {
        if ((insertion.pos > span.start)) {
          next.push({ start: span.start, end: insertion.pos, style: span.style });
        }
        const shiftedStart = (insertion.pos + insertion.length);
        const shiftedEnd = (span.end + insertion.length);
        if ((shiftedEnd > shiftedStart)) {
          next.push({ start: shiftedStart, end: shiftedEnd, style: span.style });
        }
      }
    }
    updated = next;
...
function renderSignalText(ir) {
  ...
  const clamped = clampStyles(adjusted.map((span) => { start: span.start, length: (span.end - span.start), style: span.style }), trimmedLength);
  return { text: trimmedText, styles: mergeStyles(clamped) };
}
export function markdownToSignalText(markdown, options = {  }) {
  const ir = markdownToIR((markdown ?? ""), { linkify: true, enableSpoilers: true, headingStyle: "none", blockquotePrefix: "", tableMode: options.tableMode });
