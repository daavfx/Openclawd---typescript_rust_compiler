...
export async function sendReactionSignal(recipient, targetTimestamp, emoji, opts = {  }) {
  const accountInfo = resolveSignalAccount({ cfg: loadConfig(), accountId: opts.accountId });
  ...
  const targetAuthorParams = resolveTargetAuthorParams({ targetAuthor: opts.targetAuthor, targetAuthorUuid: opts.targetAuthorUuid, fallback: normalizedRecipient });
  if ((groupId && !targetAuthorParams.targetAuthor)) {
    throw new Error("targetAuthor is required for group reactions");
  }
  const params = { emoji: emoji.trim(), targetTimestamp, ...targetAuthorParams:  };
  ...
  const result = await signalRpcRequest("sendReaction", params, { baseUrl, timeoutMs: opts.timeoutMs });
...
export async function removeReactionSignal(recipient, targetTimestamp, emoji, opts = {  }) {
  ...
  const params = { emoji: emoji.trim(), targetTimestamp, remove: true, ...targetAuthorParams:  };
  if (normalizedRecipient) {
    params.recipients = [normalizedRecipient];
  }
  if (groupId) {
