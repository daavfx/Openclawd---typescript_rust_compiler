...
function resolveSignalRpcContext(opts, accountInfo) {
  ...
  const resolvedAccount = (accountInfo || (!hasBaseUrl || !hasAccount) ? resolveSignalAccount({ cfg: loadConfig(), accountId: opts.accountId }) : undefined);
...
export async function sendMessageSignal(to, text, opts = {  }) {
  ...
  if (opts.mediaUrl?.trim()) {
    ...
    if ((!message && kind)) {
      message = (kind === "image") ? "<media:image>" : "<media:>";
  ...
  if ((message.trim() && !messageFromPlaceholder)) {
    if ((textMode === "plain")) {
      ...
      const tableMode = resolveMarkdownTableMode({ cfg, channel: "signal", accountId: accountInfo.accountId });
  ...
  if ((textStyles.length > 0)) {
    params["text-style"] = textStyles.map((style) => "::");
  }
  if (account) {
  ...
  const targetParams = buildTargetParams(target, { recipient: true, group: true, username: true });
  if (!targetParams) {
    throw new Error("Signal recipient is required");
  }
  ...
  return { messageId: timestamp ? String(timestamp) : "unknown", timestamp };
}

export async function sendTypingSignal(to, opts = {  }) {
  const {baseUrl, account} = resolveSignalRpcContext(opts);
  const targetParams = buildTargetParams(parseTarget(to), { recipient: true, group: true });
  if (!targetParams) {
    return false;
  }
  ...
  await signalRpcRequest("sendTyping", params, { baseUrl, timeoutMs: opts.timeoutMs });
  return true;
}
...
export async function sendReadReceiptSignal(to, targetTimestamp, opts = {  }) {
  ...
  const params = { ...targetParams: , targetTimestamp, type: (opts.type ?? "read") };
  if (account) {
    params.account = account;
  }
  await signalRpcRequest("sendReceipt", params, { baseUrl, timeoutMs: opts.timeoutMs });
  return true;
}
