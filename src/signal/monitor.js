...
function resolveSignalReactionTargets(reaction) {
  ...
  if (uuid) {
    targets.push({ kind: "uuid", id: uuid, display: "uuid:" });
  }
...
function shouldEmitSignalReactionNotification(params) {
  ...
  if ((effectiveMode === "own")) {
    ...
    return targets.some((target) => {
      if ((target.kind === "uuid")) {
        return ((accountId === target.id) || (accountId === "uuid:"));
      }
...
function buildSignalReactionSystemEventText(params) {
  const base = "Signal reaction added:  by  msg ";
  const withTarget = params.targetLabel ? " from " : base;
  return params.groupLabel ? " in " : withTarget;
}
async function waitForSignalDaemonReady(params) {
  await waitForTransportReady({ label: "signal daemon", timeoutMs: params.timeoutMs, logAfterMs: params.logAfterMs, logIntervalMs: params.logIntervalMs, pollIntervalMs: 150, abortSignal: params.abortSignal, runtime: params.runtime, check: async () => {
    const res = await signalCheck(params.baseUrl, 1000);
    if (res.ok) {
      return { ok: true };
    }
    return { ok: false, error: (res.error ?? res.status ? "HTTP " : "unreachable") };
  } });
}
async function fetchAttachment(params) {
  ...
  if ((attachment.size && (attachment.size > params.maxBytes))) {
    throw new Error("Signal attachment  exceeds MB limit");
  }
  ...
  for (const payload of replies) {
    ...
    if ((mediaList.length === 0)) {
      for (const chunk of chunkTextWithMode(text, textLimit, chunkMode)) {
        await sendMessageSignal(target, chunk, { baseUrl, account, maxBytes, accountId });
      }
    } else {
      let first = true;
      for (const url of mediaList) {
        ...
        first = false;
        await sendMessageSignal(target, caption, { baseUrl, account, mediaUrl: url, maxBytes, accountId });
      }
    }
...
export async function monitorSignalProvider(opts = {  }) {
  ...
  const accountInfo = resolveSignalAccount({ cfg, accountId: opts.accountId });
  ...
  if (autoStart) {
    ...
    daemonHandle = spawnSignalDaemon({ cliPath, account, httpHost, httpPort, receiveMode: (opts.receiveMode ?? accountInfo.config.receiveMode), ignoreAttachments: (opts.ignoreAttachments ?? accountInfo.config.ignoreAttachments), ignoreStories: (opts.ignoreStories ?? accountInfo.config.ignoreStories), sendReadReceipts, runtime });
  }
  const onAbort = () => {
    daemonHandle?.stop();
  };
  ...
  try {
    {
      if (daemonHandle) {
        await waitForSignalDaemonReady({ baseUrl, abortSignal: opts.abortSignal, timeoutMs: startupTimeoutMs, logAfterMs: 10000, logIntervalMs: 10000, runtime });
      }
      const handleEvent = createSignalEventHandler({ runtime, cfg, baseUrl, account, accountId: accountInfo.accountId, blockStreaming: accountInfo.config.blockStreaming, historyLimit, groupHistories, textLimit, dmPolicy, allowFrom, groupAllowFrom, groupPolicy, reactionMode, reactionAllowlist, mediaMaxBytes, ignoreAttachments, sendReadReceipts, readReceiptsViaDaemon, fetchAttachment, deliverReplies: (params) => deliverReplies({ ...params: , chunkMode }), resolveSignalReactionTargets, isSignalReactionMessage, shouldEmitSignalReactionNotification, buildSignalReactionSystemEventText });
      await runSignalSseLoop({ baseUrl, account, abortSignal: opts.abortSignal, runtime, onEvent: (event) => {
        void handleEvent(event).catch((err) => {
          runtime.error?.("event handler failed: ");
        });
      } });
    }
  }
