...
vi.mock("./client.js", () => { signalCheck: (...args) => signalCheckMock(...args), signalRpcRequest: (...args) => signalRpcRequestMock(...args) });
describe("probeSignal", () => {
  ...
  it("returns ok=false when /check fails", async () => {
    signalCheckMock.mockResolvedValueOnce({ ok: false, status: 503, error: "HTTP 503" });
    const res = await probeSignal("http://127.0.0.1:8080", 1000);
