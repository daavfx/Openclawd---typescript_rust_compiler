...
function scoreDiscordMember(member, query) {
  ...
  const candidates = [user.username, user.global_name, (member.nick ?? undefined)].map((value) => value?.toLowerCase()).filter(Boolean);
  let score = 0;
...
export async function resolveDiscordUserAllowlist(params) {
  ...
  for (const input of params.entries) {
    ...
    if (parsed.userId) {
      results.push({ input, resolved: true, id: parsed.userId });
      continue;
    }
    ...
    for (const guild of guildList) {
      const paramsObj = new URLSearchParams({ query, limit: "25" });
      const members = await fetchDiscord("/guilds//members/search?", token, fetcher);
    ...
    if (best) {
      ...
      results.push({ input, resolved: true, id: user.id, name, guildId: best.guild.id, guildName: best.guild.name, note: (matches > 1) ? "multiple matches; chose best" : undefined });
    } else {
      results.push({ input, resolved: false });
    }
  }
