...
export async function readMessagesDiscord(channelId, query = {  }, opts = {  }) {
  ...
  const limit = ((typeof query.limit === "number") && Number.isFinite(query.limit)) ? Math.min(Math.max(Math.floor(query.limit), 1), 100) : undefined;
  const params = {  };
  if (limit) {
    params.limit = limit;
  }
...
export async function editMessageDiscord(channelId, messageId, payload, opts = {  }) {
  const rest = resolveDiscordRest(opts);
  return await rest.patch(Routes.channelMessage(channelId, messageId), { body: { content: payload.content } });
}
...
export async function searchMessagesDiscord(query, opts = {  }) {
  ...
  return await rest.get("/guilds//messages/search?");
}
