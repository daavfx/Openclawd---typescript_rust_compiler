...
const INFO_DEBUG_MARKERS = ["WebSocket connection closed", "Reconnecting with backoff", "Attempting resume with backoff"];
...
export function attachDiscordGatewayLogging(params) {
  ...
  const onGatewayDebug = (msg) => {
    ...
    logVerbose("discord gateway: ");
    if (shouldPromoteGatewayDebug(message)) {
      runtime.log?.("discord gateway: ");
    }
  };
  const onGatewayWarning = (warning) => {
    logVerbose("discord gateway warning: ");
  };
  const onGatewayMetrics = (metrics) => {
    logVerbose("discord gateway metrics: ");
  };
