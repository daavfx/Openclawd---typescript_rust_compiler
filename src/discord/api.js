...
const DISCORD_API_RETRY_DEFAULTS = { attempts: 3, minDelayMs: 500, maxDelayMs: 30000, jitter: 0.1 };
...
function parseRetryAfterSeconds(text, response) {
  ...
  const retryAfter = ((payload && (typeof payload.retry_after === "number")) && Number.isFinite(payload.retry_after)) ? payload.retry_after : undefined;
...
function formatDiscordApiErrorText(text) {
  ...
  return retryAfter ? " (retry after )" : message;
}
...
export async function fetchDiscord(path, token, fetcher = fetch, options) {
  ...
  return retryAsync(async () => {
    const res = await fetchImpl("", { headers: { Authorization: "Bot " } });
    if (!res.ok) {
      ...
      throw new DiscordApiError("Discord API  failed ()", res.status, retryAfter);
    }
    return await res.json();
  }, { ...retryConfig: , label: (options?.label ?? path), shouldRetry: (err) => ((err instanceof DiscordApiError) && (err.status === 429)), retryAfterMs: (err) => ((err instanceof DiscordApiError) && (typeof err.retryAfter === "number")) ? (err.retryAfter * 1000) : undefined });
}
