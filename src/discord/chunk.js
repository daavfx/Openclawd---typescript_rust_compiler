...
function parseFenceLine(line) {
  ...
  return { indent, markerChar: (marker[0] ?? "`"), markerLen: marker.length, openLine: line };
}
  ...
  for (const originalLine of lines) {
    ...
    if (fenceInfo) {
      if (!openFence) {
        ...
        if (((openFence.markerChar === fenceInfo.markerChar) && (fenceInfo.markerLen >= openFence.markerLen))) {
          nextOpenFence = null;
        }
      }
    }
    ...
    const segments = splitLongLine(originalLine, segmentLimit, { preserveWhitespace: wasInsideFence });
