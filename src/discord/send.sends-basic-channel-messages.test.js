...
import { deleteMessageDiscord, editMessageDiscord, fetchChannelPermissionsDiscord, fetchReactionsDiscord, pinMessageDiscord, reactMessageDiscord, readMessagesDiscord, removeOwnReactionsDiscord, removeReactionDiscord, searchMessagesDiscord, sendMessageDiscord, unpinMessageDiscord } from "./send.js";
vi.mock("../web/media.js", () => { loadWebMedia: vi.fn().mockResolvedValue({ buffer: Buffer.from("img"), fileName: "photo.jpg", contentType: "image/jpeg", kind: "image" }), loadWebMediaRaw: vi.fn().mockResolvedValue({ buffer: Buffer.from("img"), fileName: "asset.png", contentType: "image/png", kind: "image" }) });
const makeRest = () => {
  ...
  return { rest: { post: postMock, put: putMock, get: getMock, patch: patchMock, delete: deleteMock }, postMock, putMock, getMock, patchMock, deleteMock };
};
describe("sendMessageDiscord", () => {
  ...
  it("sends basic channel messages", async () => {
    ...
    const res = await sendMessageDiscord("channel:789", "hello world", { rest, token: "t" });
    expect(res).toEqual({ messageId: "msg1", channelId: "789" });
    expect(postMock).toHaveBeenCalledWith(Routes.channelMessages("789"), expect.objectContaining({ body: { content: "hello world" } }));
  });
  it("starts DM when recipient is a user", async () => {
    ...
    const res = await sendMessageDiscord("user:123", "hiya", { rest, token: "t" });
    expect(postMock).toHaveBeenNthCalledWith(1, Routes.userChannels(), expect.objectContaining({ body: { recipient_id: "123" } }));
    expect(postMock).toHaveBeenNthCalledWith(2, Routes.channelMessages("chan1"), expect.objectContaining({ body: { content: "hiya" } }));
    expect(res.channelId).toBe("chan1");
  });
  it("rejects bare numeric IDs as ambiguous", async () => {
    const {rest} = makeRest();
    await expect(sendMessageDiscord("273512430271856640", "hello", { rest, token: "t" })).rejects.toThrow(/Ambiguous Discord recipient/);
    await expect(sendMessageDiscord("273512430271856640", "hello", { rest, token: "t" })).rejects.toThrow(/user:273512430271856640/);
    await expect(sendMessageDiscord("273512430271856640", "hello", { rest, token: "t" })).rejects.toThrow(/channel:273512430271856640/);
  });
  it("adds missing permission hints on 50013", async () => {
    ...
    getMock.mockResolvedValueOnce({ id: "789", guild_id: "guild1", type: 0, permission_overwrites: [] }).mockResolvedValueOnce({ id: "bot1" }).mockResolvedValueOnce({ id: "guild1", roles: [{ id: "guild1", permissions: perms.toString() }] }).mockResolvedValueOnce({ roles: [] });
    let error;
    try {
      {
  ...
  it("uploads media attachments", async () => {
    ...
    const res = await sendMessageDiscord("channel:789", "photo", { rest, token: "t", mediaUrl: "file:///tmp/photo.jpg" });
    expect(res.messageId).toBe("msg");
    expect(postMock).toHaveBeenCalledWith(Routes.channelMessages("789"), expect.objectContaining({ body: expect.objectContaining({ files: [expect.objectContaining({ name: "photo.jpg" })] }) }));
  });
  it("includes message_reference when replying", async () => {
    const {rest, postMock} = makeRest();
    postMock.mockResolvedValue({ id: "msg1", channel_id: "789" });
    await sendMessageDiscord("channel:789", "hello", { rest, token: "t", replyTo: "orig-123" });
    const body = postMock.mock.calls[0]?.[1]?.body;
    expect(body?.message_reference).toEqual({ message_id: "orig-123", fail_if_not_exists: false });
  });
  it("replies only on the first chunk", async () => {
    const {rest, postMock} = makeRest();
    postMock.mockResolvedValue({ id: "msg1", channel_id: "789" });
    await sendMessageDiscord("channel:789", "a".repeat(2001), { rest, token: "t", replyTo: "orig-123" });
    expect(postMock).toHaveBeenCalledTimes(2);
    const firstBody = postMock.mock.calls[0]?.[1]?.body;
    const secondBody = postMock.mock.calls[1]?.[1]?.body;
    expect(firstBody?.message_reference).toEqual({ message_id: "orig-123", fail_if_not_exists: false });
...
describe("reactMessageDiscord", () => {
  ...
  it("reacts with custom emoji syntax", async () => {
    const {rest, putMock} = makeRest();
    await reactMessageDiscord("chan1", "msg1", "<:party_blob:123>", { rest, token: "t" });
...
describe("removeOwnReactionsDiscord", () => {
  ...
  it("removes all own reactions on a message", async () => {
    ...
    getMock.mockResolvedValue({ reactions: [{ emoji: { name: "✅", id: null } }, { emoji: { name: "party_blob", id: "123" } }] });
    const res = await removeOwnReactionsDiscord("chan1", "msg1", { rest, token: "t" });
...
describe("fetchReactionsDiscord", () => {
  ...
  it("returns reactions with users", async () => {
    ...
    getMock.mockResolvedValueOnce({ reactions: [{ count: 2, emoji: { name: "✅", id: null } }, { count: 1, emoji: { name: "party_blob", id: "123" } }] }).mockResolvedValueOnce([{ id: "u1", username: "alpha", discriminator: "0001" }]).mockResolvedValueOnce([{ id: "u2", username: "beta" }]);
    const res = await fetchReactionsDiscord("chan1", "msg1", { rest, token: "t" });
    expect(res).toEqual([{ emoji: { id: null, name: "✅", raw: "✅" }, count: 2, users: [{ id: "u1", username: "alpha", tag: "alpha#0001" }] }, { emoji: { id: "123", name: "party_blob", raw: "party_blob:123" }, count: 1, users: [{ id: "u2", username: "beta", tag: "beta" }] }]);
  });
});
describe("fetchChannelPermissionsDiscord", () => {
  ...
  it("calculates permissions from guild roles", async () => {
    ...
    getMock.mockResolvedValueOnce({ id: "chan1", guild_id: "guild1", permission_overwrites: [] }).mockResolvedValueOnce({ id: "bot1" }).mockResolvedValueOnce({ id: "guild1", roles: [{ id: "guild1", permissions: perms.toString() }, { id: "role2", permissions: "0" }] }).mockResolvedValueOnce({ roles: ["role2"] });
    const res = await fetchChannelPermissionsDiscord("chan1", { rest, token: "t" });
...
describe("edit/delete message helpers", () => {
  ...
  it("edits message content", async () => {
    ...
    expect(patchMock).toHaveBeenCalledWith(Routes.channelMessage("chan1", "m1"), expect.objectContaining({ body: { content: "hello" } }));
  });
...
describe("searchMessagesDiscord", () => {
  ...
  it("supports channel/author arrays and clamps limit", async () => {
    const {rest, getMock} = makeRest();
    getMock.mockResolvedValue({ total_results: 0, messages: [] });
    await searchMessagesDiscord({ guildId: "g1", content: "hello", channelIds: ["c1", "c2"], authorIds: ["u1"], limit: 99 }, { rest, token: "t" });
    const call = getMock.mock.calls[0];
    expect(call?.[0]).toBe("/guilds/g1/messages/search?content=hello&channel_id=c1&channel_id=c2&author_id=u1&limit=25");
  });
});
