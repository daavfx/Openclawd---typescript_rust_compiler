...
export async function fetchReactionsDiscord(channelId, messageId, opts = {  }) {
  ...
  for (const reaction of reactions) {
    ...
    summaries.push({ emoji: { id: (reaction.emoji.id ?? null), name: (reaction.emoji.name ?? null), raw: formatReactionEmoji(reaction.emoji) }, count: reaction.count, users: users.map((user) => { id: user.id, username: user.username, tag: (user.username && user.discriminator) ? "#" : user.username }) });
  }
  return summaries;
}
