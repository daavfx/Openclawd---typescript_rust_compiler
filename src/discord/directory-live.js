...
export async function listDiscordDirectoryGroupsLive(params) {
  ...
  for (const guild of guilds) {
    const channels = await fetchDiscord("/guilds//channels", token);
    for (const channel of channels) {
      ...
      rows.push({ kind: "group", id: "channel:", name, handle: "#", raw: channel });
...
export async function listDiscordDirectoryPeersLive(params) {
  ...
  for (const guild of guilds) {
    const paramsObj = new URLSearchParams({ query, limit: String(Math.min(limit, 100)) });
    const members = await fetchDiscord("/guilds//members/search?", token);
    for (const member of members) {
      ...
      rows.push({ kind: "user", id: "user:", name: (name || undefined), handle: user.username ? "@" : undefined, rank: buildUserRank(user), raw: member });
