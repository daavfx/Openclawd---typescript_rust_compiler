...
export async function auditDiscordChannelPermissions(params) {
  ...
  if ((!token || (params.channelIds.length === 0))) {
    return { ok: true, checkedChannels: 0, unresolvedChannels: 0, channels: [], elapsedMs: (Date.now() - started) };
  }
  ...
  for (const channelId of params.channelIds) {
    try {
      {
        const perms = await fetchChannelPermissionsDiscord(channelId, { token, accountId: (params.accountId ?? undefined) });
        const missing = required.filter((p) => !perms.permissions.includes(p));
        channels.push({ channelId, ok: (missing.length === 0), missing: missing.length ? missing : undefined, error: null, matchKey: channelId, matchSource: "id" });
      }
    }
    catch (err) {
      {
        channels.push({ channelId, ok: false, error: (err instanceof Error) ? err.message : String(err), matchKey: channelId, matchSource: "id" });
      }
    }
  }
