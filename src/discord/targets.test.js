...
vi.mock("./directory-live.js", () => { listDiscordDirectoryPeersLive: vi.fn() });
describe("parseDiscordTarget", () => {
  it("parses user mention and prefixes", () => {
    expect(parseDiscordTarget("<@123>")).toMatchObject({ kind: "user", id: "123", normalized: "user:123" });
    expect(parseDiscordTarget("<@!456>")).toMatchObject({ kind: "user", id: "456", normalized: "user:456" });
    expect(parseDiscordTarget("user:789")).toMatchObject({ kind: "user", id: "789", normalized: "user:789" });
    expect(parseDiscordTarget("discord:987")).toMatchObject({ kind: "user", id: "987", normalized: "user:987" });
  });
  it("parses channel targets", () => {
    expect(parseDiscordTarget("channel:555")).toMatchObject({ kind: "channel", id: "555", normalized: "channel:555" });
    expect(parseDiscordTarget("general")).toMatchObject({ kind: "channel", id: "general", normalized: "channel:general" });
  });
  ...
  it("accepts numeric ids when a default kind is provided", () => {
    expect(parseDiscordTarget("123", { defaultKind: "channel" })).toMatchObject({ kind: "channel", id: "123", normalized: "channel:123" });
  });
...
describe("resolveDiscordTarget", () => {
  ...
  it("returns a resolved user for usernames", async () => {
    ...
    await expect(resolveDiscordTarget("jane", { cfg, accountId: "default" })).resolves.toMatchObject({ kind: "user", id: "999", normalized: "user:999" });
  });
  it("falls back to parsing when lookup misses", async () => {
    listPeers.mockResolvedValueOnce([]);
    await expect(resolveDiscordTarget("general", { cfg, accountId: "default" })).resolves.toMatchObject({ kind: "channel", id: "general" });
  });
  it("does not call directory lookup for explicit user ids", async () => {
    listPeers.mockResolvedValueOnce([]);
    await expect(resolveDiscordTarget("user:123", { cfg, accountId: "default" })).resolves.toMatchObject({ kind: "user", id: "123" });
