...
function bitfieldToPermissions(bitfield) {
  return PERMISSION_ENTRIES.filter(([, value]) => ((bitfield & value) === value)).map(([name]) => name).sort();
}
...
export async function fetchChannelPermissionsDiscord(channelId, opts = {  }) {
  ...
  if (!guildId) {
    return { channelId, permissions: [], raw: "0", isDm: true, channelType };
  }
  ...
  return { channelId, guildId, permissions: bitfieldToPermissions(permissions), raw: permissions.toString(), isDm: false, channelType };
}
