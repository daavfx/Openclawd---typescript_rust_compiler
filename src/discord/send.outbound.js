...
export async function sendMessageDiscord(to, text, opts = {  }) {
  ...
  const tableMode = resolveMarkdownTableMode({ cfg, channel: "discord", accountId: accountInfo.accountId });
  ...
  try {
    ...
    {
      throw await buildDiscordSendError(err, { channelId, rest, token, hasMedia: Boolean(opts.mediaUrl) });
    }
  }
  recordChannelActivity({ channel: "discord", accountId: accountInfo.accountId, direction: "outbound" });
  return { messageId: result.id ? String(result.id) : "unknown", channelId: String((result.channel_id ?? channelId)) };
}
...
export async function sendStickerDiscord(to, stickerIds, opts = {  }) {
  ...
  const res = await request(() => rest.post(Routes.channelMessages(channelId), { body: { content: (content || undefined), sticker_ids: stickers } }), "sticker");
  return { messageId: res.id ? String(res.id) : "unknown", channelId: String((res.channel_id ?? channelId)) };
}
...
export async function sendPollDiscord(to, poll, opts = {  }) {
  ...
  const res = await request(() => rest.post(Routes.channelMessages(channelId), { body: { content: (content || undefined), poll: payload } }), "poll");
  return { messageId: res.id ? String(res.id) : "unknown", channelId: String((res.channel_id ?? channelId)) };
}
