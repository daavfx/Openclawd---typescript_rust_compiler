  ...
  try {
    {
      ...
      if (!channel) {
        DISCORD_CHANNEL_INFO_CACHE.set(channelId, { value: null, expiresAt: (Date.now() + DISCORD_CHANNEL_INFO_NEGATIVE_CACHE_TTL_MS) });
        return null;
      }
      ...
      const payload = { type: channel.type, name, topic, parentId, ownerId };
      DISCORD_CHANNEL_INFO_CACHE.set(channelId, { value: payload, expiresAt: (Date.now() + DISCORD_CHANNEL_INFO_CACHE_TTL_MS) });
      return payload;
    }
  }
  catch (err) {
    {
      logVerbose("discord: failed to fetch channel : ");
      DISCORD_CHANNEL_INFO_CACHE.set(channelId, { value: null, expiresAt: (Date.now() + DISCORD_CHANNEL_INFO_NEGATIVE_CACHE_TTL_MS) });
      return null;
    }
  }
...
export async function resolveMediaList(message, maxBytes) {
  ...
  for (const attachment of attachments) {
    try {
      {
        const fetched = await fetchRemoteMedia({ url: attachment.url, filePathHint: (attachment.filename ?? attachment.url) });
        const saved = await saveMediaBuffer(fetched.buffer, (fetched.contentType ?? attachment.content_type), "inbound", maxBytes);
        out.push({ path: saved.path, contentType: saved.contentType, placeholder: inferPlaceholder(attachment) });
      }
    }
    catch (err) {
      {
        const id = (attachment.id ?? attachment.url);
        logVerbose("discord: failed to download attachment : ");
      }
    }
  }
  return out;
...
function resolveDiscordForwardedMessagesText(message) {
  ...
  const forwardedBlocks = snapshots.map((snapshot) => {
    ...
    const heading = authorLabel ? "[Forwarded message from ]" : "[Forwarded message]";
    return "
";
