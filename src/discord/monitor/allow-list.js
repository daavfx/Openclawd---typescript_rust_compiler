...
export function resolveDiscordUserAllowed(params) {
  ...
  return allowListMatches(allowList, { id: params.userId, name: params.userName, tag: params.userTag });
}
...
export function resolveDiscordCommandAuthorized(params) {
  ...
  return allowListMatches(allowList, { id: params.author.id, name: params.author.username, tag: formatDiscordUserTag(params.author) });
}
...
function resolveDiscordChannelEntryMatch(channels, params, parentParams) {
  ...
  return resolveChannelEntryMatchWithFallback({ entries: channels, keys, parentKeys, wildcardKey: "*" });
}
function resolveDiscordChannelConfigEntry(entry) {
  const resolved = { allowed: (entry.allow !== false), requireMention: entry.requireMention, skills: entry.skills, enabled: entry.enabled, users: entry.users, systemPrompt: entry.systemPrompt, autoThread: entry.autoThread };
  return resolved;
}
export function resolveDiscordChannelConfig(params) {
  ...
  const match = resolveDiscordChannelEntryMatch(channels, { id: channelId, name: channelName, slug: channelSlug });
...
export function resolveDiscordChannelConfigWithFallback(params) {
  ...
  const match = resolveDiscordChannelEntryMatch(channels, { id: channelId, name: channelName, slug: channelSlug, allowNameMatch: (scope !== "thread") }, ((parentId || parentName) || parentSlug) ? { id: (parentId ?? ""), name: parentName, slug: resolvedParentSlug } : undefined);
...
export function resolveGroupDmAllow(params) {
  ...
  const candidates = [normalizeDiscordSlug(channelId), channelSlug, channelName ? normalizeDiscordSlug(channelName) : ""].filter(Boolean);
...
export function shouldEmitDiscordReactionNotification(params) {
  ...
  if ((mode === "allowlist")) {
    ...
    return allowListMatches(list, { id: params.userId, name: params.userName, tag: params.userTag });
  }
  return false;
}
