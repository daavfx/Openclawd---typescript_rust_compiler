...
async function deployDiscordCommands(params) {
  ...
  try {
  ...
  catch (err) {
    {
      ...
      params.runtime.error?.(danger("discord: failed to deploy native commands: "));
    }
  }
}
...
export async function monitorDiscordProvider(opts = {  }) {
  ...
  if (!token) {
    throw new Error("Discord bot token missing for account \"\" (set discord.accounts..token or DISCORD_BOT_TOKEN for default).");
  }
  const runtime = (opts.runtime ?? { log: console.log, error: console.error, exit: (code) => {
  ...
  if (((((discordCfg.groupPolicy === undefined) && (discordCfg.guilds === undefined)) && (defaultGroupPolicy === undefined)) && (groupPolicy === "open"))) {
  ...
  const textLimit = resolveTextChunkLimit(cfg, "discord", account.accountId, { fallbackLimit: 2000 });
  ...
  const nativeEnabled = resolveNativeCommandsEnabled({ providerId: "discord", providerSetting: discordCfg.commands?.native, globalSetting: cfg.commands?.native });
  const nativeSkillsEnabled = resolveNativeSkillsEnabled({ providerId: "discord", providerSetting: discordCfg.commands?.nativeSkills, globalSetting: cfg.commands?.nativeSkills });
  const nativeDisabledExplicit = isNativeCommandsExplicitlyDisabled({ providerSetting: discordCfg.commands?.native, globalSetting: cfg.commands?.native });
  ...
  if (token) {
      ...
      try {
        {
          ...
          for (const [guildKey, guildCfg] of Object.entries(guildEntries)) {
            ...
            for (const channelKey of channelKeys) {
              entries.push({ input: "/", guildKey, channelKey });
            }
          }
          if ((entries.length > 0)) {
            const resolved = await resolveDiscordChannelAllowlist({ token, entries: entries.map((entry) => entry.input) });
            ...
            for (const entry of resolved) {
              ...
              if ((source.channelKey && entry.channelId)) {
                ...
                if (sourceChannel) {
                  nextGuilds[entry.guildId] = { ...mergedGuild: , channels: { ...mergedChannels: , [entry.channelId]: { ...sourceChannel: , ...mergedChannels?.[entry.channelId]:  } } };
                }
              }
            }
        ...
        {
          runtime.log?.("discord channel resolve failed; using config entries. ");
        }
      }
    }
    ...
    if ((allowEntries.length > 0)) {
      try {
        {
          const resolvedUsers = await resolveDiscordUserAllowlist({ token, entries: allowEntries.map((entry) => String(entry)) });
          const mapping = [];
          const unresolved = [];
          const additions = [];
        ...
        {
          runtime.log?.("discord user resolve failed; using config entries. ");
        }
      }
    }
    if ((guildEntries && (Object.keys(guildEntries).length > 0))) {
      ...
      if ((userEntries.size > 0)) {
        try {
          {
            const resolvedUsers = await resolveDiscordUserAllowlist({ token, entries: Array.from(userEntries) });
            const resolvedMap = new Map(resolvedUsers.map((entry) => [entry.input, entry]));
            const mapping = resolvedUsers.filter((entry) => (entry.resolved && entry.id)).map((entry) => "→");
            ...
            for (const [guildKey, guildConfig] of Object.entries((guildEntries ?? {  }))) {
              ...
              if ((channels && (typeof channels === "object"))) {
                ...
                for (const [channelKey, channelConfig] of Object.entries(channels)) {
                  ...
                  nextChannels[channelKey] = { ...channelConfig: , users: mergeAllowlist({ existing: channelUsers, additions }) };
                }
          ...
          {
            runtime.log?.("discord channel user resolve failed; using config entries. ");
          }
        }
      }
    }
  ...
  if (shouldLogVerbose()) {
    logVerbose("discord: config dm= dmPolicy= allowFrom= groupDm= groupDmChannels= groupPolicy= guilds= historyLimit= mediaMaxMb= native= nativeSkills= accessGroups=");
  }
  ...
  if (((nativeEnabled && nativeSkillsEnabled) && (commandSpecs.length > maxDiscordCommands))) {
    ...
    runtime.log?.(warn("discord:  commands exceeds limit; removing per-skill commands and keeping /skill."));
  }
  if ((nativeEnabled && (commandSpecs.length > maxDiscordCommands))) {
    runtime.log?.(warn("discord:  commands exceeds limit; some commands may fail to deploy."));
  }
  const commands = commandSpecs.map((spec) => createDiscordNativeCommand({ command: spec, cfg, discordConfig: discordCfg, accountId: account.accountId, sessionPrefix, ephemeralDefault }));
  const execApprovalsConfig = (discordCfg.execApprovals ?? {  });
  const execApprovalsHandler = execApprovalsConfig.enabled ? new DiscordExecApprovalHandler({ token, accountId: account.accountId, config: execApprovalsConfig, cfg, runtime }) : null;
  const components = [createDiscordCommandArgFallbackButton({ cfg, discordConfig: discordCfg, accountId: account.accountId, sessionPrefix })];
  if (execApprovalsHandler) {
    components.push(createExecApprovalButton({ handler: execApprovalsHandler }));
  }
  const client = new Client({ baseUrl: "http://localhost", deploySecret: "a", clientId: applicationId, publicKey: "a", token, autoDeploy: false }, { commands, listeners: [], components }, [new GatewayPlugin({ reconnect: { maxAttempts: Number.POSITIVE_INFINITY }, intents: resolveDiscordGatewayIntents(discordCfg.intents), autoInteractions: true })]);
  ...
  try {
    ...
    {
      runtime.error?.(danger("discord: failed to fetch bot identity: "));
    }
  }
  const messageHandler = createDiscordMessageHandler({ cfg, discordConfig: discordCfg, accountId: account.accountId, token, runtime, botUserId, guildHistories, historyLimit, mediaMaxBytes, textLimit, replyToMode, dmEnabled, groupDmEnabled, groupDmChannels, allowFrom, guildEntries });
  registerDiscordListener(client.listeners, new DiscordMessageListener(messageHandler, logger));
  registerDiscordListener(client.listeners, new DiscordReactionListener({ cfg, accountId: account.accountId, runtime, botUserId, guildEntries, logger }));
  registerDiscordListener(client.listeners, new DiscordReactionRemoveListener({ cfg, accountId: account.accountId, runtime, botUserId, guildEntries, logger }));
  ...
  runtime.log?.("logged in to discord");
  if (execApprovalsHandler) {
    await execApprovalsHandler.start();
  }
  const gateway = client.getPlugin("gateway");
  const gatewayEmitter = getDiscordGatewayEmitter(gateway);
  const stopGatewayLogging = attachDiscordGatewayLogging({ emitter: gatewayEmitter, runtime });
  ...
  const onGatewayDebug = (msg) => {
    ...
    helloTimeoutId = setTimeout(() => {
      if (!gateway?.isConnected) {
        runtime.log?.(danger("connection stalled: no HELLO received within ms, forcing reconnect"));
        gateway?.disconnect();
  ...
  try {
    {
      await waitForDiscordGatewayStop({ gateway: gateway ? { emitter: gatewayEmitter, disconnect: () => gateway.disconnect() } : undefined, abortSignal, onGatewayError: (err) => {
        runtime.error?.(danger("discord gateway error: "));
...
async function clearDiscordNativeCommands(params) {
  try {
    ...
    {
      params.runtime.error?.(danger("discord: failed to clear native commands: "));
    }
  }
}
