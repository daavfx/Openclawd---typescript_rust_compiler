...
export async function fetchDiscordApplicationSummary(token, timeoutMs, fetcher = fetch) {
  ...
  try {
    {
      const res = await fetchWithTimeout("/oauth2/applications/@me", timeoutMs, fetcher, { Authorization: "Bot " });
      if (!res.ok) {
        return undefined;
      }
      const json = await res.json();
      const flags = ((typeof json.flags === "number") && Number.isFinite(json.flags)) ? json.flags : undefined;
      return { id: (json.id ?? null), flags: (flags ?? null), intents: (typeof flags === "number") ? resolveDiscordPrivilegedIntentsFromFlags(flags) : undefined };
    }
  }
  catch {
    {
...
export async function probeDiscord(token, timeoutMs, opts) {
  ...
  try {
    {
      const res = await fetchWithTimeout("/users/@me", timeoutMs, fetcher, { Authorization: "Bot " });
      if (!res.ok) {
        result.status = res.status;
        result.error = "getMe failed ()";
        return { ...result: , elapsedMs: (Date.now() - started) };
      }
      const json = await res.json();
      result.ok = true;
      result.bot = { id: (json.id ?? null), username: (json.username ?? null) };
