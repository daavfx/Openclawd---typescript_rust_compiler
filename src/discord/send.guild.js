...
export async function createScheduledEventDiscord(guildId, payload, opts = {  }) {
  const rest = resolveDiscordRest(opts);
  return await rest.post(Routes.guildScheduledEvents(guildId), { body: payload });
}
...
export async function timeoutMemberDiscord(payload, opts = {  }) {
  ...
  return await rest.patch(Routes.guildMember(payload.guildId, payload.userId), { body: { communication_disabled_until: (until ?? null) }, headers: payload.reason ? { "X-Audit-Log-Reason": encodeURIComponent(payload.reason) } : undefined });
}

export async function kickMemberDiscord(payload, opts = {  }) {
  const rest = resolveDiscordRest(opts);
  await rest.delete(Routes.guildMember(payload.guildId, payload.userId), { headers: payload.reason ? { "X-Audit-Log-Reason": encodeURIComponent(payload.reason) } : undefined });
  return { ok: true };
}

export async function banMemberDiscord(payload, opts = {  }) {
  const rest = resolveDiscordRest(opts);
  const deleteMessageDays = ((typeof payload.deleteMessageDays === "number") && Number.isFinite(payload.deleteMessageDays)) ? Math.min(Math.max(Math.floor(payload.deleteMessageDays), 0), 7) : undefined;
  await rest.put(Routes.guildBan(payload.guildId, payload.userId), { body: (deleteMessageDays !== undefined) ? { delete_message_days: deleteMessageDays } : undefined, headers: payload.reason ? { "X-Audit-Log-Reason": encodeURIComponent(payload.reason) } : undefined });
  return { ok: true };
}
