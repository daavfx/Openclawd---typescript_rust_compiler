...
function createDiscordClient(opts, cfg = loadConfig()) {
  ...
  const token = resolveToken({ explicit: opts.token, accountId: account.accountId, fallbackToken: account.token });
  const rest = resolveRest(token, opts.rest);
  const request = createDiscordRetryRunner({ retry: opts.retry, configRetry: account.config.retry, verbose: opts.verbose });
...
function normalizeReactionEmoji(raw) {
  ...
  const identifier = customMatch ? ":" : trimmed.replace(/[\uFE0E\uFE0F]/g, "");
  return encodeURIComponent(identifier);
}
function parseRecipient(raw) {
  const target = parseDiscordTarget(raw, { ambiguousMessage: "Ambiguous Discord recipient \"\". Use \"user:\" for DMs or \"channel:\" for channel messages." });
  if (!target) {
...
export async function parseAndResolveRecipient(raw, accountId) {
  ...
  const trimmed = raw.trim();
  const parseOptions = { ambiguousMessage: "Ambiguous Discord recipient \"\". Use \"user:\" for DMs or \"channel:\" for channel messages." };
  const resolved = await resolveDiscordTarget(raw, { cfg, accountId: accountInfo.accountId }, parseOptions);
  if (resolved) {
...
function normalizeDiscordPollInput(input) {
  ...
  const duration = normalizePollDurationHours(poll.durationHours, { defaultHours: 24, maxHours: DISCORD_POLL_MAX_DURATION_HOURS });
  return { question: { text: poll.question }, answers: poll.options.map((answer) => { poll_media: { text: answer } }), duration, allow_multiselect: (poll.maxSelections > 1), layout_type: PollLayoutType.Default };
}
function getDiscordErrorCode(err) {
  if ((!err || (typeof err !== "object"))) {
    return undefined;
  }
  const candidate = (("code" in err) && (err.code !== undefined)) ? err.code : ((("rawError" in err) && err.rawError) && (typeof err.rawError === "object")) ? err.rawError.code : undefined;
...
async function buildDiscordSendError(err, ctx) {
  ...
  try {
    {
      const permissions = await fetchChannelPermissionsDiscord(ctx.channelId, { rest: ctx.rest, token: ctx.token });
  ...
  const missingLabel = missing.length ? "missing permissions in channel : " : "missing permissions in channel ";
  return new DiscordSendError(". bot might be muted or blocked by role/channel overrides", { kind: "missing-permissions", channelId: ctx.channelId, missingPermissions: missing });
}
...
async function sendDiscordText(rest, channelId, text, replyTo, request, maxLinesPerMessage, embeds, chunkMode) {
  ...
  const chunks = chunkDiscordTextWithMode(text, { maxChars: DISCORD_TEXT_LIMIT, maxLines: maxLinesPerMessage, chunkMode });
  if ((!chunks.length && text)) {
    chunks.push(text);
  }
  if ((chunks.length === 1)) {
    const res = await request(() => rest.post(Routes.channelMessages(channelId), { body: { content: chunks[0], message_reference: messageReference, ...embeds?.length ? { embeds } : {  }:  } }), "text");
    return res;
  }
  let last = null;
  let isFirst = true;
  for (const chunk of chunks) {
    last = await request(() => rest.post(Routes.channelMessages(channelId), { body: { content: chunk, message_reference: isFirst ? messageReference : undefined, ...(isFirst && embeds?.length) ? { embeds } : {  }:  } }), "text");
    isFirst = false;
  }
  if (!last) {
...
async function sendDiscordMedia(rest, channelId, text, mediaUrl, replyTo, request, maxLinesPerMessage, embeds, chunkMode) {
  ...
  const chunks = text ? chunkDiscordTextWithMode(text, { maxChars: DISCORD_TEXT_LIMIT, maxLines: maxLinesPerMessage, chunkMode }) : [];
  if ((!chunks.length && text)) {
    chunks.push(text);
  }
  const caption = (chunks[0] ?? "");
  const messageReference = replyTo ? { message_id: replyTo, fail_if_not_exists: false } : undefined;
  const res = await request(() => rest.post(Routes.channelMessages(channelId), { body: { content: (caption || undefined), message_reference: messageReference, ...embeds?.length ? { embeds } : {  }: , files: [{ data: media.buffer, name: (media.fileName ?? "upload") }] } }), "media");
