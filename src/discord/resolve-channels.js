...
async function listGuilds(token, fetcher) {
  ...
  return raw.map((guild) => { id: guild.id, name: guild.name, slug: normalizeDiscordSlug(guild.name) });
}
async function listGuildChannels(token, fetcher, guildId) {
  const raw = await fetchDiscord("/guilds//channels", token, fetcher);
  return raw.filter((channel) => (Boolean(channel.id) && ("name" in channel))).map((channel) => {
    const archived = ("thread_metadata" in channel) ? channel.thread_metadata?.archived : undefined;
    return { id: channel.id, name: ("name" in channel) ? (channel.name ?? "") : "", guildId, type: channel.type, archived };
  }).filter((channel) => Boolean(channel.name));
}
async function fetchChannel(token, fetcher, channelId) {
  const raw = await fetchDiscord("/channels/", token, fetcher);
  if ((!raw || !("guild_id" in raw))) {
    return null;
  }
  return { id: raw.id, name: ("name" in raw) ? (raw.name ?? "") : "", guildId: (raw.guild_id ?? ""), type: raw.type };
}
...
export async function resolveDiscordChannelAllowlist(params) {
  ...
  for (const input of params.entries) {
    ...
    if (parsed.guildOnly) {
      const guild = (parsed.guildId && guilds.find((entry) => (entry.id === parsed.guildId))) ? guilds.find((entry) => (entry.id === parsed.guildId)) : parsed.guild ? resolveGuildByName(guilds, parsed.guild) : undefined;
      if (guild) {
        results.push({ input, resolved: true, guildId: guild.id, guildName: guild.name });
      } else {
        results.push({ input, resolved: false, guildId: parsed.guildId, guildName: parsed.guild });
      }
      continue;
    }
    if (parsed.channelId) {
      ...
      if (channel?.guildId) {
        ...
        results.push({ input, resolved: true, guildId: channel.guildId, guildName: guild?.name, channelId: channel.id, channelName: channel.name, archived: channel.archived });
      } else {
        results.push({ input, resolved: false, channelId: parsed.channelId });
      }
      continue;
    }
    if ((parsed.guildId || parsed.guild)) {
      ...
      if ((!guild || !channelQuery)) {
        results.push({ input, resolved: false, guildId: parsed.guildId, guildName: parsed.guild, channelName: (channelQuery ?? parsed.channel) });
        continue;
      }
      ...
      if (match) {
        results.push({ input, resolved: true, guildId: guild.id, guildName: guild.name, channelId: match.id, channelName: match.name, archived: match.archived });
      } else {
        results.push({ input, resolved: false, guildId: guild.id, guildName: guild.name, channelName: parsed.channel, note: "channel not found in guild " });
      }
      continue;
    }
    const channelName = input.trim().replace(/^#/, "");
    if (!channelName) {
      results.push({ input, resolved: false, channelName: channelName });
      continue;
    }
    const candidates = [];
    ...
    if (match) {
      ...
      results.push({ input, resolved: true, guildId: match.guildId, guildName: guild?.name, channelId: match.id, channelName: match.name, archived: match.archived, note: ((candidates.length > 1) && guild?.name) ? "matched multiple; chose " : undefined });
      continue;
    }
    results.push({ input, resolved: false, channelName: channelName });
  }
