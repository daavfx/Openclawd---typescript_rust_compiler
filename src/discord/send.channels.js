...
export async function moveChannelDiscord(payload, opts = {  }) {
  const rest = resolveDiscordRest(opts);
  const body = [{ id: payload.channelId, ...((payload.parentId !== undefined) && { parent_id: payload.parentId }): , ...((payload.position !== undefined) && { position: payload.position }):  }];
...
export async function setChannelPermissionDiscord(payload, opts = {  }) {
  ...
  await rest.put("/channels//permissions/", { body });
  return { ok: true };
}

export async function removeChannelPermissionDiscord(channelId, targetId, opts = {  }) {
  const rest = resolveDiscordRest(opts);
  await rest.delete("/channels//permissions/");
  return { ok: true };
}
