...
import { assertWebChannel, CONFIG_DIR, ensureDir, jidToE164, normalizeE164, normalizePath, resolveConfigDir, resolveJidToE164, resolveUserPath, sleep, toWhatsappJid, withWhatsAppPrefix } from "./utils.js";
...
describe("jidToE164", () => {
  it("maps @lid using reverse mapping file", () => {
    ...
    const spy = vi.spyOn(fs, "readFileSync").mockImplementation((path, encoding) => {
      if ((path === mappingPath)) {
        return "\"5551234\"";
      }
...
describe("resolveJidToE164", () => {
  it("resolves @lid via lidLookup when mapping file is missing", async () => {
    const lidLookup = { getPNForLID: vi.fn().mockResolvedValue("777:0@s.whatsapp.net") };
  ...
  it("skips lidLookup for non-lid JIDs", async () => {
    const lidLookup = { getPNForLID: vi.fn().mockResolvedValue("888:0@s.whatsapp.net") };
