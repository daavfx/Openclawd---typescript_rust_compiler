...
function normalizeSchemaNode(schema, path) {
  ...
  if ((type === "object")) {
    ...
    if ((type === "array")) {
      ...
      if ((((((type !== "string") && (type !== "number")) && (type !== "integer")) && (type !== "boolean")) && !normalized.enum)) {
        unsupported.add(pathLabel);
      }
    }
  }
...
function normalizeUnion(schema, path) {
  ...
  if (((literals.length > 0) && (remaining.length === 0))) {
    ...
    return { schema: { ...schema: , enum: unique, nullable, anyOf: undefined, oneOf: undefined, allOf: undefined }, unsupportedPaths: [] };
  }
  ...
  if ((((remaining.length > 0) && (literals.length === 0)) && remaining.every((entry) => (entry.type && primitiveTypes.includes(String(entry.type)))))) {
