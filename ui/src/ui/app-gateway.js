...
function normalizeSessionKeyForDefaults(value, defaults) {
  ...
  const isAlias = (((raw === "main") || (raw === mainKey)) || (defaultAgentId && ((raw === "agent::main") || (raw === "agent::"))));
...
function applySessionDefaults(host, defaults) {
  ...
  const nextSettings = { ...host.settings: , sessionKey: (resolvedSettingsSessionKey || nextSessionKey), lastActiveSessionKey: (resolvedLastActiveSessionKey || nextSessionKey) };
...
export function connectGateway(host) {
  ...
  host.client?.stop();
  host.client = new GatewayBrowserClient({ url: host.settings.gatewayUrl, token: host.settings.token.trim() ? host.settings.token : undefined, password: host.password.trim() ? host.password : undefined, clientName: "openclaw-control-ui", mode: "webchat", onHello: (hello) => {
    host.connected = true;
    host.lastError = null;
    host.hello = hello;
    ...
    host.connected = false;
    if ((code !== 1012)) {
      host.lastError = "disconnected (): ";
    }
  }, onEvent: (evt) => handleGatewayEvent(host, evt), onGap: ({expected, received}) => {
    host.lastError = "event gap detected (expected seq , got ); refresh recommended";
  } });
  host.client.start();
}
...
function handleGatewayEventUnsafe(host, evt) {
  host.eventLogBuffer = [{ ts: Date.now(), event: evt.event, payload: evt.payload }, ...host.eventLogBuffer].slice(0, 250);
  if ((host.tab === "debug")) {
