...
export async function loadCronJobs(state) {
  ...
  try {
    {
      const res = await state.client.request("cron.list", { includeDisabled: true });
  ...
  try {
    {
      ...
      const job = { name: state.cronForm.name.trim(), description: (state.cronForm.description.trim() || undefined), agentId: (agentId || undefined), enabled: state.cronForm.enabled, schedule, sessionTarget: state.cronForm.sessionTarget, wakeMode: state.cronForm.wakeMode, payload, isolation: (state.cronForm.postToMainPrefix.trim() && (state.cronForm.sessionTarget === "isolated")) ? { postToMainPrefix: state.cronForm.postToMainPrefix.trim() } : undefined };
      if (!job.name) {
        throw new Error("Name required.");
      }
      await state.client.request("cron.add", job);
      state.cronForm = { ...state.cronForm: , name: "", description: "", payloadText: "" };
...
export async function loadCronRuns(state, jobId) {
  ...
  try {
    {
      const res = await state.client.request("cron.runs", { id: jobId, limit: 50 });
