  ...
  try {
    {
      ...
      if (contextObj) {
        if ((typeof contextObj.subsystem === "string")) {
          ...
          if ((typeof contextObj.module === "string")) {
      ...
      if ((typeof obj["1"] === "string")) {
        ...
        if ((!contextObj && (typeof obj["0"] === "string"))) {
          message = obj["0"];
        } else {
...
export async function loadLogs(state, opts) {
  ...
  try {
    {
      const res = await state.client.request("logs.tail", { cursor: opts?.reset ? undefined : (state.logsCursor ?? undefined), limit: state.logsLimit, maxBytes: state.logsMaxBytes });
      const payload = res;
