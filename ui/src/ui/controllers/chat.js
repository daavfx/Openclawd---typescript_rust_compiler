...
export async function loadChatHistory(state) {
  ...
  try {
    {
      const res = await state.client.request("chat.history", { sessionKey: state.sessionKey, limit: 200 });
...
export async function sendChatMessage(state, message, attachments) {
  ...
  }).filter((a) => (a !== null)) : undefined;
  try {
    {
      await state.client.request("chat.send", { sessionKey: state.sessionKey, message: msg, deliver: false, idempotencyKey: runId, attachments: apiAttachments });
      return true;
    }
  }
  catch (err) {
    {
      ...
      state.chatMessages = [...state.chatMessages, { role: "assistant", content: [{ type: "text", text: ("Error: " + error) }], timestamp: Date.now() }];
      return false;
    }
  }
  finally {
