  ...
  try {
    {
      ...
      state.devicesList = { pending: Array.isArray(res?.pending) ? res.pending : [], paired: Array.isArray(res?.paired) ? res.paired : [] };
    }
  }
  catch (err) {
    {
  ...
  try {
    {
      ...
      if (res?.token) {
        ...
        if (((res.deviceId === identity.deviceId) || (params.deviceId === identity.deviceId))) {
          storeDeviceAuthToken({ deviceId: identity.deviceId, role, token: res.token, scopes: ((res.scopes ?? params.scopes) ?? []) });
        }
...
export async function revokeDeviceToken(state, params) {
  ...
  const confirmed = window.confirm("Revoke token for  ()?");
  if (!confirmed) {
    return;
  }
  try {
