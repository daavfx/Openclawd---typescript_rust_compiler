  ...
  try {
    {
      ...
      await state.client.request("config.apply", { raw, baseHash, sessionKey: state.applySessionKey });
      state.configFormDirty = false;
      await loadConfig(state);
    }
  }
...
export async function runUpdate(state) {
  ...
  try {
    {
      await state.client.request("update.run", { sessionKey: state.applySessionKey });
    }
  }
  catch (err) {
    {
